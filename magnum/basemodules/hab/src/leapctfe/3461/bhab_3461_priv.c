...
#include "bchp_3461_leap_hab_mem.h"
#include "bchp_3461_tm.h"
#include "bchp_3461_leap_l2.h"
    ...
    else
    {
        ...
        if(h3461Dev)
        {
            ...
            for(i=0; i<BHAB_DevId_eMax; i++){
                ...
                h3461Dev->InterruptCallbackInfo[i].parm2 = 0;
            }
...
)
{
    ...
    val = BHAB_HAB_DONE;
    BHAB_CHK_RETCODE(BHAB_3461_WriteRegister(handle, BCHP_LEAP_HOST_IRQ_CLEAR0, &val));
    val = BHAB_HAB_RESET;
...
BERR_Code BHAB_3461_WriteMemory(BHAB_Handle handle, uint32_t addr, const uint8_t *buf, uint32_t n)
{
    ...
    if(p3461->isSpi)
    {
        ...
        spiData[1].length = n;
        BHAB_CHK_RETCODE(BREG_SPI_Multiple_Write(p3461->hSpiRegister, spiData, 2));
        orig_bytes_left = (n%4);
...
BERR_Code BHAB_3461_P_RunAp(BHAB_Handle handle)
{
    ...
    buf = 0x80000001;
    BHAB_CHK_RETCODE(BHAB_3461_WriteRegister(handle, BCHP_TM_SFT1, &buf));
#else
    buf = 0;
    BHAB_CHK_RETCODE(BHAB_3461_WriteRegister(handle, BCHP_TM_SFT1, &buf));
#endif
