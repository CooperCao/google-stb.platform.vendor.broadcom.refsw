...
BDBG_MODULE(bhab_4517_priv);

#define BHAB_MAX_BBSI_RETRIES 10


/* local private functions */
BERR_Code BHAB_4517_P_WaitForBbsiDone(BHAB_Handle h);
BERR_Code BHAB_4517_P_EnableHostInterrupt(BHAB_Handle h, bool bEnable);
BERR_Code BHAB_4517_P_RunAp(BHAB_Handle h);
BERR_Code BHAB_4517_P_WaitForEvent(BHAB_Handle h, BKNI_EventHandle hEvent, int timeoutMsec);
BERR_Code BHAB_4517_P_ServiceHab(BHAB_Handle h, uint8_t *read_buf, uint16_t read_len, uint8_t ack_byte, bool bFixedLength);
BERR_Code BHAB_4517_P_DecodeInterrupt(BHAB_Handle h);
BERR_Code BHAB_4517_P_CheckHab(BHAB_Handle h);
BERR_Code BHAB_4517_P_ReadHab(BHAB_Handle h, uint16_t addr, uint8_t *buf, uint16_t n);
BERR_Code BHAB_4517_P_DecodeHabError(BHAB_Handle h);
BERR_Code BHAB_4517_P_DisableFrontendInterrupts(BHAB_Handle h);
...
)
{
   ...
   BDBG_ASSERT(hDev);
   h4517Dev = (BHAB_4517_P_Handle *)BKNI_Malloc(sizeof(BHAB_4517_P_Handle));
   BDBG_ASSERT(h4517Dev);
   BKNI_Memset( h4517Dev, 0x00, sizeof(BHAB_4517_P_Handle));
   hDev->pImpl = (void*)h4517Dev;

#ifdef BHAB_SUPPORT_SPI
   if (pDefSettings->isSpi)
      h4517Dev->hSpiRegister = (BREG_SPI_Handle)pReg;
   else
#endif
      h4517Dev->hI2cRegister = (BREG_I2C_Handle)pReg;

   h4517Dev->callbackInfo.func = NULL;
   h4517Dev->callbackInfo.callbackParams.pParm1 = NULL;
   h4517Dev->callbackInfo.callbackParams.parm2 = (int)NULL;

   BKNI_Memcpy((void*)(&(hDev->settings)), (void*)pDefSettings, sizeof(BHAB_Settings));

   /* create events */
   retCode = BKNI_CreateEvent(&(h4517Dev->hInterruptEvent));
   if (retCode != BERR_SUCCESS)
      goto done_0;
   retCode = BKNI_CreateEvent(&(h4517Dev->hApiEvent));
   if (retCode != BERR_SUCCESS)
      goto done_1;
   retCode = BKNI_CreateEvent(&(h4517Dev->hInitDoneEvent));
   if (retCode != BERR_SUCCESS)
      goto done_2;
   retCode = BKNI_CreateEvent(&(h4517Dev->hHabDoneEvent));
   if (retCode != BERR_SUCCESS)
   {
      BKNI_DestroyEvent(h4517Dev->hInitDoneEvent);

      done_2:
      BKNI_DestroyEvent(h4517Dev->hApiEvent);

      done_1:
      BKNI_DestroyEvent(h4517Dev->hInterruptEvent);

      done_0:
      BKNI_Free((void*)h4517Dev);
      BKNI_Free((void*)hDev);
...
/******************************************************************************
 BHAB_4517_P_Close()
******************************************************************************/
BERR_Code BHAB_4517_P_Close(BHAB_Handle handle)
{
   BERR_Code retCode = BERR_SUCCESS;
   BHAB_4517_P_Handle *p4517 = (BHAB_4517_P_Handle *)(handle->pImpl);

   BDBG_ASSERT(handle);

   /* disable host interrupts */
   retCode = BHAB_4517_P_DisableFrontendInterrupts(handle);
   if (retCode != BERR_SUCCESS)
   {
      BDBG_WRN(("BHAB_4517_P_DisableFrontendInterrupts() error 0x%X", retCode));
   }

   /* reset the LEAP */
   retCode = BHAB_4517_P_Reset(handle);

   /* clean up */
   BKNI_DestroyEvent(p4517->hInterruptEvent);
   BKNI_DestroyEvent(p4517->hApiEvent);
   BKNI_DestroyEvent(p4517->hInitDoneEvent);
   BKNI_DestroyEvent(p4517->hHabDoneEvent);
   BKNI_Free((void*)p4517);
...
BERR_Code BHAB_4517_P_InitAp(
    ...
    const uint8_t *pHexImage  /* [in] pointer to BCM4517 microcode image */
)
{
   BHAB_4517_P_Handle *pImpl = (BHAB_4517_P_Handle *)(h->pImpl);
   BERR_Code retCode = BERR_SUCCESS;
   uint32_t n, addr, init_done_mask, retries, strap;
...
#endif
   /* disable host interrupts */
   BHAB_CHK_RETCODE(BHAB_4517_P_EnableHostInterrupt(h, false));

   /* determine boot mode */
   BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_CTRL_CPU_STRAPS, &strap, 1));
   if (strap == 0)
   {
      /* boot mode 0 */

      /* reset the AP before downloading the firmware */
      BHAB_CHK_RETCODE(BHAB_4517_P_Reset(h));

      /* disable interrupts */
      BHAB_CHK_RETCODE(BHAB_4517_P_DisableFrontendInterrupts(h));
      ...
      while (1)
      {
         ...
         for (retries = 0; retries < 3; retries++)
         {
            ...
            retCode = BHAB_4517_P_WriteMemory(h, addr, (uint8_t*)pImage, n);
            if (retCode == BERR_SUCCESS)
            {
#ifdef BHAB_VERIFY_DOWNLOAD
               if (pVerifyBuf)
               {
                  ...
                  BHAB_CHK_RETCODE(BHAB_4517_P_ReadMemory(h, addr, (uint8_t *)pVerifyBuf, n));
            ...
            {
               BDBG_ERR(("BHAB_4517_P_WriteMemory() error 0x%X", retCode));
            }
         }
         if (retries >= 3)
         {
      ...
      init_done_mask = BHAB_4517_HIRQ0_INIT_DONE;
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_HOST_IRQ_CLEAR0, &init_done_mask, 1));

#ifndef BHAB_DONT_USE_INTERRUPT
      /* enable init done interrupt */
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_HOST_IRQ_MASK_CLEAR0, &init_done_mask, 1));
      BKNI_WaitForEvent(pImpl->hInitDoneEvent, 0);
#endif

      /* start running the AP */
      BHAB_CHK_RETCODE(BHAB_4517_P_RunAp(h));
      ...
      for (n = 0; n < 3000; n++)
      {
         BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_HOST_IRQ_STATUS0, &val32, 1));
         if (val32 & BHAB_4517_HIRQ0_INIT_DONE)
            break;
         BKNI_Sleep(1);
      }
      if ((val32 & BHAB_4517_HIRQ0_INIT_DONE) == 0)
      {
      ...
      if (BHAB_4517_P_WaitForEvent(h, pImpl->hInitDoneEvent, 3000) != BERR_SUCCESS)
      {
         BDBG_ERR(("AP initialization timeout"));
         BERR_TRACE(retCode = BHAB_ERR_AP_NOT_INIT);
         goto done;
      }
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_HOST_IRQ_MASK_SET0, &init_done_mask, 1));
#endif
   }
   else
   {
   ...
   {
      BDBG_ERR(("BHAB_4517_P_InitAp() error 0x%X", retCode));
   }
   return retCode;
}
...
{
   FEC_SystemVersionInfo info;
   BERR_Code retCode;

   BHAB_CHK_RETCODE(BHAB_4517_P_GetVersionInfo(handle, &info));

   *pFamilyId = info.familyId;
   *pChipId = info.chipId;
   *pChipVer = (uint16_t)(info.chipVersion & 0xFFFF);
   *pMajApVer = (uint8_t)(info.firmware.majorVersion & 0xFF);
   *pMinApVer = (uint8_t)(info.firmware.minorVersion & 0xFF);
...
******************************************************************************/
BERR_Code BHAB_4517_P_GetVersionInfo(BHAB_Handle handle, FEC_SystemVersionInfo *p)
{
   BERR_Code retCode;
   uint8_t buf[40];
...
)
{
   ...
   *pStatus = 0;
   ...
   BHAB_CHK_RETCODE(BHAB_4517_P_ReadBbsi(h,  BCHP_CSR_STATUS, &csr_status, 1));
   if ((csr_status & BCHP_CSR_STATUS_CPU_RUNNING_MASK) == 0)
      *pStatus |= BHAB_APSTATUS_RESET;
   else
   {
      /* AP is running */
      BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_CTRL_SW_SPARE0, &spare0, 1));
      if (spare0 & BHAB_4517_SPARE0_INIT_DONE)
      {
         /* AP has initialized */
         *pStatus |= BHAB_APSTATUS_INIT_DONE;

         BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_CTRL_HAB_REQUEST, &hab_request, 1));
         if ((hab_request & 1) == 0)
...
******************************************************************************/
BERR_Code BHAB_4517_P_WriteMemory(BHAB_Handle h, uint32_t start_addr, const uint8_t *buf, uint32_t n)
{
   BERR_Code retCode = BERR_SUCCESS;
   ...
   else
   {
      ...
      BHAB_CHK_RETCODE(BHAB_4517_P_WaitForBbsiDone(h));
      ...
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteBbsi(h, BCHP_CSR_CONFIG, i2c_buf, 5));
      ...
      for (i = 0; i < nWords; i++)
      {
         ...
         if (bIsRunning)
         {
            ...
            i2c_buf[2] = buf[(i<<2)+2];
            i2c_buf[3] = buf[(i<<2)+3];
            BHAB_CHK_RETCODE(BHAB_4517_P_WriteBbsi(h, BCHP_CSR_RBUS_DATA0, i2c_buf, 4));
         }
         else
         {
            ...
            if (nWords > 0)
            {
               BHAB_CHK_RETCODE(BHAB_4517_P_WriteBbsi(h, BCHP_CSR_RBUS_DATA0, (uint8_t *)buf, nWords<<2));
            }
            break;
         }
      }
...
******************************************************************************/
BERR_Code BHAB_4517_P_ReadMemory(BHAB_Handle h, uint32_t start_addr, uint8_t *buf, uint32_t n)
{
   BERR_Code retCode;
      ...
      for (i = buf_idx = 0; i < nReads; i++)
      {
         ...
         if (i == 0)
         {
            ...
            if (x)
            {
               ...
               BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, addr, &val32, 1));
               if (x == 3)
         ...
         BHAB_CHK_RETCODE(BHAB_4517_P_ReadBbsi(h, BCHP_CSR_RBUS_DATA0, i2c_buf, 4));
...
)
{
   ...
   if (reg < 0x100)
      bHsi = true;
   if ((reg > 0xBC30F) || ((reg < 0x79000) && !bHsi))
      return BERR_INVALID_PARAMETER;

   if (bHsi)
   {
      sb = (uint8_t)(*val & 0xFF);
      retCode = BHAB_4517_P_WriteBbsi(h, (uint8_t)reg, &sb, 1);
   }
   else
      retCode = BHAB_4517_P_WriteRbus(h, reg, val, 1);
   return retCode;
}
...
)
{
   ...
   if (bHsi)
   {
      retCode = BHAB_4517_P_ReadBbsi(h, (uint8_t)reg, &sb, 1);
      if (retCode == BERR_SUCCESS)
         *val = (uint32_t)sb;
   }
   else
      retCode = BHAB_4517_P_ReadRbus(h, reg, val, 1);
   return retCode;
}
...
{
   BHAB_4517_P_Handle *p4517;

   BDBG_ASSERT(handle);
   p4517 = (BHAB_4517_P_Handle *)(handle->pImpl);
   handle->settings.interruptEnableFunc(false, handle->settings.interruptEnableFuncParam);

   BKNI_SetEvent(p4517->hApiEvent);
   BKNI_SetEvent(p4517->hInterruptEvent);
   return BERR_SUCCESS;
}
...
)
{
   ...
   BHAB_CHK_RETCODE(BHAB_4517_P_DecodeInterrupt(handle));
   BHAB_4517_P_EnableHostInterrupt(handle, true);
...
)
{
   ...
   p4517->callbackInfo.func = fCallBack;
   p4517->callbackInfo.callbackParams.pParm1 = pParm1;
   p4517->callbackInfo.callbackParams.parm2 = parm2;
   return BERR_SUCCESS;
}
...
)
{
   ...
   p4517->callbackInfo.func = NULL;
   p4517->callbackInfo.callbackParams.pParm1 = NULL;
   p4517->callbackInfo.callbackParams.parm2 = 0;
   return BERR_SUCCESS;
}
...
)
{
   ...
   BHAB_CHK_RETCODE(BHAB_4517_P_CheckHab(handle));
   ...
   BHAB_CHK_RETCODE(BHAB_4517_P_WriteMemory(handle, BCHP_LEAP_HAB_MEM_WORDi_ARRAY_BASE, hab_buf, (uint32_t)(write_len+1)));

   /* wait for the AP to service the HAB, and then read any return data */
   BHAB_CHK_RETCODE(BHAB_4517_P_ServiceHab(handle, read_buf, write_len, (uint8_t)(hab_buf[0] | 0x80), bFixedLength));
...
{
    *hEvent = ((BHAB_4517_P_Handle *)(handle->pImpl))->hInterruptEvent;
    return BERR_SUCCESS;
}
...
/******************************************************************************
 BHAB_4517_P_EnableHostInterrupt()
******************************************************************************/
BERR_Code BHAB_4517_P_EnableHostInterrupt(
   BHAB_Handle h, /* [in] BHAB handle */
...
/******************************************************************************
 BHAB_4517_P_Reset()
******************************************************************************/
BERR_Code BHAB_4517_P_Reset(BHAB_Handle h)
{
   BERR_Code retCode;
   uint32_t ctrl, val32;
   uint8_t status, buf;
   ...
   ctrl = 0x0000A04B;
   BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_CTRL_CTRL, &ctrl, 1));

   /* verify that AP is reset */
   BHAB_CHK_RETCODE(BHAB_4517_P_ReadBbsi(h, BCHP_CSR_STATUS, &status, 1));
   ...
   {
      buf = 0x27;
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteBbsi(h, BCHP_HIF_SFT_RST0, &buf, 1));
      buf = 0x07;
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteBbsi(h, BCHP_HIF_SFT_RST1, &buf, 1));
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteBbsi(h, BCHP_HIF_SFT_RST2, &buf, 1));
      buf = 0x00;
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteBbsi(h, BCHP_HIF_SFT_RST0, &buf, 1));
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteBbsi(h, BCHP_HIF_SFT_RST1, &buf, 1));
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteBbsi(h, BCHP_HIF_SFT_RST2, &buf, 1));
      ...
      val32 = 0;
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_CTRL_SW_SPARE0, &val32, 1));
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_CTRL_SW_SPARE1, &val32, 1));
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_CTRL_SW_SPARE2, &val32, 1));
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_CTRL_SW_SPARE3, &val32, 1));
   }
...
/******************************************************************************
 BHAB_4517_P_WriteBbsi()
******************************************************************************/
BERR_Code BHAB_4517_P_WriteBbsi(BHAB_Handle h, uint8_t addr, uint8_t *buf, uint32_t n)
{
   return BREG_I2C_Write(((BHAB_4517_P_Handle*)h->pImpl)->hI2cRegister, h->settings.chipAddr, addr, buf, n);
}


/******************************************************************************
 BHAB_4517_P_ReadBbsi()
******************************************************************************/
BERR_Code BHAB_4517_P_ReadBbsi(BHAB_Handle h, uint8_t addr, uint8_t *buf, uint32_t n)
{
   return BREG_I2C_Read(((BHAB_4517_P_Handle*)h->pImpl)->hI2cRegister, h->settings.chipAddr, addr, buf, n);
}


/******************************************************************************
 BHAB_4517_P_ReadRbus()
******************************************************************************/
BERR_Code BHAB_4517_P_ReadRbus(BHAB_Handle h, uint32_t addr, uint32_t *buf, uint32_t n)
{
   BERR_Code retCode = BERR_SUCCESS;
   uint32_t i;
   uint8_t i2c_buf[5];
...
/******************************************************************************
 BHAB_4517_P_WriteRbus()
******************************************************************************/
BERR_Code BHAB_4517_P_WriteRbus(BHAB_Handle h, uint32_t addr, uint32_t *buf, uint32_t n)
{
   BERR_Code retCode = BERR_SUCCESS;
   uint32_t i, data_idx;
   uint8_t i2c_buf[9];
   ...
   for (i = 0; i < n; i++)
   {
      ...
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteBbsi(h, (uint8_t)(i ? BCHP_CSR_RBUS_DATA0 : BCHP_CSR_CONFIG), i2c_buf, data_idx));
   }
...
BERR_Code BHAB_4517_P_RunAp(BHAB_Handle h)
{
   ...
   if (status & BCHP_CSR_STATUS_CPU_RUNNING_MASK)
   {
      BDBG_WRN(("BHAB_4517_P_RunAp(): AP already running"));
   }
   else
   {
      BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_CTRL_CTRL, &ctrl, 1));
...
/******************************************************************************
 BHAB_4517_P_ServiceHab()
******************************************************************************/
BERR_Code BHAB_4517_P_ServiceHab(
   BHAB_Handle h,   /* [in] BHAB handle */
   uint8_t *read_buf,  /* [out] holds the data read from the HAB */
...
)
{
   ...
   mask = BHAB_4517_HIRQ0_HAB_DONE;
   BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_HOST_IRQ_CLEAR0, &mask, 1));
   BKNI_WaitForEvent(pImpl->hHabDoneEvent, 0);

#ifndef BHAB_DONT_USE_INTERRUPT
   /* enable HAB_DONE interrupt */
   BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_HOST_IRQ_MASK_CLEAR0, &mask, 1));
#endif

   /* send the command */
   val32 = BCHP_LEAP_CTRL_HAB_REQUEST_HAB_REQUEST_MASK;
   BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_CTRL_HAB_REQUEST, &val32, 1));

   /* assert the HAB interrupt on the LEAP */
   val32 = BCHP_LEAP_L2_CPU_SET_HAB_INTR_MASK;
   BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_L2_CPU_SET, &val32, 1));
   ...
   {
      BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_HOST_IRQ_STATUS0, &val32, 1));
      if (val32 & BHAB_4517_HIRQ0_HAB_DONE)
         break;
   }
   if ((val32 & BHAB_4517_HIRQ0_HAB_DONE) == 0)
   {
   ...
   if (BHAB_4517_P_WaitForEvent(h, pImpl->hHabDoneEvent, 500) == BERR_TIMEOUT)
   {
      uint32_t spare3, status0, mask0;
      BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_CTRL_SW_SPARE3, &spare3, 1));
      BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_HOST_IRQ_STATUS0, &status0, 1));
      BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_HOST_IRQ_MASK_STATUS0, &mask0, 1));
   ...
   BHAB_CHK_RETCODE(BHAB_4517_P_ReadHab(h, 0, read_buf, read_len));

   /* determine if the command was serviced */
   if (ack_byte != read_buf[0])
   {
      retCode = BHAB_4517_P_DecodeHabError(h);
      if (retCode == BERR_SUCCESS)
      {
...
/******************************************************************************
 BERR_Code BHAB_4517_P_WaitForEvent()
******************************************************************************/
BERR_Code BHAB_4517_P_WaitForEvent(
   BHAB_Handle h,             /* [in] BHAB handle */
...
)
{
   ...
   BHAB_4517_P_EnableHostInterrupt(h, true);
   while (1)
   {
...
/******************************************************************************
 BHAB_4517_P_DecodeInterrupt()
******************************************************************************/
BERR_Code BHAB_4517_P_DecodeInterrupt(BHAB_Handle h)
{
   BHAB_4517_P_Handle *pImpl = (BHAB_4517_P_Handle *)(h->pImpl);
   BERR_Code retCode;
   BHAB_4517_P_CallbackInfo *pCallbackInfo;
   uint32_t status0, status1, mask0, mask1, fstatus0, fstatus1, val32;
   uint32_t mask_set0 = 0, mask_set1 = 0;
   uint8_t channel;

   BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_HOST_IRQ_STATUS0, &status0, 1));
   BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_HOST_IRQ_STATUS1, &status1, 1));
   BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_HOST_IRQ_MASK_STATUS0, &mask0, 1));
   BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_HOST_IRQ_MASK_STATUS1, &mask1, 1));
   ...
   if (fstatus0)
   {
      if (fstatus0 & BHAB_4517_HIRQ0_INIT_DONE)
      {
         BKNI_SetEvent(pImpl->hInitDoneEvent);
         mask_set0 |= BHAB_4517_HIRQ0_INIT_DONE;
      }

      if (fstatus0 & BHAB_4517_HIRQ0_HAB_DONE)
      {
         BKNI_SetEvent(pImpl->hHabDoneEvent);
         mask_set0 |= BHAB_4517_HIRQ0_HAB_DONE;
      }

      if (fstatus0 & BHAB_4517_HIRQ0_DISEQC_DONE)
         mask_set0 |= BHAB_4517_HIRQ0_DISEQC_DONE;
   }
   ...
   if (fstatus1)
   {
      for (channel = 0; channel < 3; channel++)
      {
         ...
         {
            mask_set1 |= (BHAB_4517_HIRQ1_CHAN0_PEAK_SCAN_DONE << (channel*4));
         }
      }
   }

   if ((fstatus1 != 0) || (fstatus0 & ~(BHAB_4517_HIRQ0_INIT_DONE | BHAB_4517_HIRQ0_HAB_DONE | BHAB_4517_HIRQ0_HAB_ERROR | BHAB_4517_HIRQ0_FW_ERROR)))
   {
   ...
   BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_HOST_IRQ_CLEAR0, &fstatus0, 1));
   BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_HOST_IRQ_CLEAR1, &fstatus1, 1));

   if (mask_set0)
   {
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h,  BCHP_LEAP_HOST_IRQ_MASK_SET0, &mask_set0, 1));
   }
   if (mask_set1)
   {
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h,  BCHP_LEAP_HOST_IRQ_MASK_SET1, &mask_set1, 1));
   }
...
/******************************************************************************
 BHAB_4517_P_CheckHab()
******************************************************************************/
BERR_Code BHAB_4517_P_CheckHab(
   BHAB_Handle h    /* [in] BHAB handle */
)
{
...
BERR_Code BHAB_4517_P_ReadHab(BHAB_Handle h, uint16_t addr, uint8_t *buf, uint16_t n)
{
   ...
   return BHAB_4517_P_ReadMemory(h, BCHP_LEAP_HAB_MEM_WORDi_ARRAY_BASE + (uint32_t)addr, buf, (uint32_t)n);
}


/******************************************************************************
 BHAB_4517_P_DecodeHabError() - called when the command was not ack'd
******************************************************************************/
BERR_Code BHAB_4517_P_DecodeHabError(BHAB_Handle h)
{
   BERR_Code retCode = BERR_SUCCESS;
   ...
   BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_CTRL_SW_SPARE3, &spare3, 1));
   BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_HOST_IRQ_STATUS0, &status0, 1));
   BHAB_CHK_RETCODE(BHAB_4517_P_ReadRbus(h, BCHP_LEAP_L2_CPU_STATUS, &leap_status, 1));

   BDBG_ERR(("status0=0x%08X, spare3=0x%08X, leap_l2_status=0x%08X", status0, spare3, leap_status));

   if (status0 & BHAB_4517_HIRQ0_HAB_ERROR)
   {
      clear0 |= BHAB_4517_HIRQ0_HAB_ERROR;

      /* decode HAB error code */
      hab_error = (spare3 & BHAB_4517_SPARE3_HAB_ERROR_CODE_MASK) >> BHAB_4517_SPARE3_HAB_ERROR_CODE_SHIFT;
      switch (hab_error)
      {
         case 0: /* HAB_SUCCESS */
   ...
   else if (status0 & BHAB_4517_HIRQ0_FW_ERROR)
   {
      clear0 |= BHAB_4517_HIRQ0_FW_ERROR;

      /* decode FW error code */
      fw_error = (spare3 & BHAB_4517_SPARE3_FW_ERROR_CODE_MASK) >> BHAB_4517_SPARE3_FW_ERROR_CODE_SHIFT;
      /* TBD... */
   ...
   if (clear0)
   {
      BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h,  BCHP_LEAP_HOST_IRQ_CLEAR0, &clear0, 1));
   }
...
/******************************************************************************
 BHAB_4517_P_DisableFrontendInterrupts()
******************************************************************************/
BERR_Code BHAB_4517_P_DisableFrontendInterrupts(
   BHAB_Handle h   /* [in] BHAB handle */
)
{
   BERR_Code retCode;
   ...
   BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_HOST_IRQ_MASK_SET0, &mask, 1));
   BHAB_CHK_RETCODE(BHAB_4517_P_WriteRbus(h, BCHP_LEAP_HOST_IRQ_MASK_SET1, &mask, 1));
