...
#include "bchp_45308_leap_host_l1.h"
#include "bchp_45308_leap_host_l2.h"
#include "bchp_45308_leap_l2.h"
#include "bchp_45308_leap_rom_patch.h"
#include "bchp_45308_csr.h"
...
/* If building for 7366/7364 host platforms, work around issues with the interrupt line */
#if (BCHP_CHIP==7366) || (BCHP_CHIP==7364)
#define BHAB_DONT_USE_INTERRUPT 1
#endif
...
)
{
   ...
   BDBG_ASSERT(hDev);
   h45308Dev = (BHAB_45308_P_Handle *)BKNI_Malloc(sizeof(BHAB_45308_P_Handle));
   BDBG_ASSERT(h45308Dev);
   BKNI_Memset(h45308Dev, 0x00, sizeof(BHAB_45308_P_Handle));
   hDev->pImpl = (void*)h45308Dev;

   if (pDefSettings->isSpi)
      h45308Dev->hSpiRegister = (BREG_SPI_Handle)pReg;
   else
      h45308Dev->hI2cRegister = (BREG_I2C_Handle)pReg;

   BKNI_Memset((void*)&(h45308Dev->cbSat), 0, sizeof(BHAB_45308_P_CallbackInfo));
   BKNI_Memset((void*)&(h45308Dev->cbDsq), 0, sizeof(BHAB_45308_P_CallbackInfo));
   BKNI_Memset((void*)&(h45308Dev->cbFsk), 0, sizeof(BHAB_45308_P_CallbackInfo));
   BKNI_Memset((void*)&(h45308Dev->cbWfe), 0, sizeof(BHAB_45308_P_CallbackInfo));
   BKNI_Memcpy((void*)(&(hDev->settings)), (void*)pDefSettings, sizeof(BHAB_Settings));

   /* create events */
   retCode = BKNI_CreateEvent(&(h45308Dev->hInterruptEvent));
   BDBG_ASSERT(retCode == BERR_SUCCESS);
   retCode = BKNI_CreateEvent(&(h45308Dev->hApiEvent));
   BDBG_ASSERT(retCode == BERR_SUCCESS);
   retCode = BKNI_CreateEvent(&(h45308Dev->hInitDoneEvent));
   BDBG_ASSERT(retCode == BERR_SUCCESS);
...
BERR_Code BHAB_45308_P_Close(BHAB_Handle handle)
{
   ...
   {
      BDBG_WRN(("BHAB_45308_P_DisableLeapInterrupts() error 0x%X", retCode));
   }
   ...
   BKNI_DestroyEvent(p45308->hApiEvent);
   BKNI_Free((void*)p45308);
...
/******************************************************************************
 BHAB_45308_P_InitAp()
******************************************************************************/
BERR_Code BHAB_45308_P_InitAp(
    BHAB_Handle h,            /* [in] BHAB handle */
    const uint8_t *pHexImage  /* [in] pointer to BCM45308 microcode image */
)
{
   ...
   /* disable host interrupts */
   BHAB_CHK_RETCODE(BHAB_45308_P_EnableHostInterrupt(h, false));
   BHAB_CHK_RETCODE(BHAB_45308_P_DisableLeapInterrupts(h));

   /* determine boot mode */
   BHAB_CHK_RETCODE(BHAB_45308_P_ReadRbus(h, BCHP_LEAP_CTRL_CPU_STRAPS, &strap, 1));
   bootmode = strap & BCHP_LEAP_CTRL_CPU_STRAPS_BOOT_MODE_MASK;

   /* clear init_done interrupt status */
   init_done_mask = BHAB_45308_HIRQ0_INIT_DONE;
   BHAB_CHK_RETCODE(BHAB_45308_P_WriteRbus(h, BCHP_LEAP_HOST_L2_CLEAR0, &init_done_mask, 1));
   BHAB_45308_P_WaitForEvent(h, pImpl->hInitDoneEvent, 0);

   /* reset the AP before downloading the firmware */
   BHAB_CHK_RETCODE(BHAB_45308_P_Reset(h));
   if (bootmode == 0)
   {
      /* boot mode 0 */
      if (BHAB_45308_P_IsLeapRunning(h))
      ...
      while (1)
      {
         ...
         for (retries = 0; retries < 3; retries++)
         {
            ...
            retCode = BHAB_45308_P_WriteMemory(h, fwAddr, (uint8_t*)pImage, n);
            if (retCode == BERR_SUCCESS)
            {
#ifdef BHAB_VERIFY_DOWNLOAD
            ...
            {
               BDBG_ERR(("BHAB_45308_P_WriteMemory() error 0x%X", retCode));
            }
         }
      ...
      for (segment = 0; segment < num_segments; segment++) {
         ...
         {
             BDBG_MSG(("BHAB_45308_P_InitAp: img next[%d of %d]...", chunk, num_chunks));
            ...
            for (retries = 0; retries < 3; retries++)
            {
               ...
               retCode = BHAB_45308_P_WriteMemory(h, fwAddr + chunk*chunk_size, (uint8_t*)pImage, n);
               if (retCode == BERR_SUCCESS)
               {
#ifdef BHAB_VERIFY_DOWNLOAD
   ...
   BHAB_CHK_RETCODE(BHAB_45308_P_WriteRbus(h, BCHP_LEAP_HOST_L2_MASK_CLEAR0, &init_done_mask, 1));
#endif

   if (BHAB_45308_P_IsLeapRunning(h) == false)
   {
      /* start running the AP */
      BHAB_CHK_RETCODE(BHAB_45308_P_RunAp(h));
   ...
   {
      BHAB_45308_P_ReadRbus(h, BCHP_LEAP_HOST_L2_STATUS0, &val32, 1);
      if (val32 & BHAB_45308_HIRQ0_INIT_DONE)
         break;
      BKNI_Sleep(1);
   }
   ...
   if (BHAB_45308_P_WaitForEvent(h, pImpl->hInitDoneEvent, BHAB_LEAP_INIT_TIMEOUT) != BERR_SUCCESS)
   {
   ...
   BHAB_CHK_RETCODE(BHAB_45308_P_WriteRbus(h, BCHP_LEAP_HOST_L2_MASK_SET0, &init_done_mask, 1));
#endif
   ...
   {
      BDBG_ERR(("BHAB_45308_P_InitAp() error 0x%X", retCode));
   }
   return retCode;
}
...
******************************************************************************/
BERR_Code BHAB_45308_P_WriteRegister(
...
)
{
   ...
   {
      BDBG_ERR(("BHAB_45308_P_WriteRegister(): bad address (0x%X)", reg));
   ...
   {
      sb = (uint8_t)(*val & 0xFF);
      retCode = BHAB_45308_P_WriteBbsi(h, (uint8_t)reg, &sb, 1);
   }
   else
      retCode = BHAB_45308_P_WriteRbus(h, reg, val, 1);
...
******************************************************************************/
BERR_Code BHAB_45308_P_ReadRegister(
    BHAB_Handle h,    /* [in] BHAB PI Handle */
    uint32_t    reg,  /* [in] RBUS register address */
    uint32_t    *val  /* [in] value to write */
)
{
   ...
   if ((reg > 0x7d008ef) || ((reg < 0xc8000) && !bHsi))
   {
      BDBG_ERR(("BHAB_45308_P_ReadRegister(): bad address (0x%X)", reg));
      return BERR_INVALID_PARAMETER;
   }

   if (bHsi)
   {
      retCode = BHAB_45308_P_ReadBbsi(h, (uint8_t)reg, &sb, 1);
      if (retCode == BERR_SUCCESS)
         *val = (uint32_t)sb;
   }
   else
      retCode = BHAB_45308_P_ReadRbus(h, reg, val, 1);
...
******************************************************************************/
BERR_Code BHAB_45308_P_GetVersionInfo(BHAB_Handle handle, BFEC_SystemVersionInfo *p)
{
   BERR_Code retCode;
   uint32_t buf[11];

   BKNI_Memset(buf, 0, 11*sizeof(uint32_t));
   buf[0] = BHAB_45308_InitHeader(0x04, 0, 0, 0);
   BHAB_CHK_RETCODE(BHAB_45308_P_SendCommand(handle, buf, 11));
   p->familyId = buf[1];
   p->chipId = buf[2];
   p->chipVersion = buf[3];
...
******************************************************************************/
BERR_Code BHAB_45308_P_GetApStatus(
...
)
{
   ...
   *pStatus = 0;
   ...
   {
      /* LEAP is running */
      BHAB_45308_P_ReadRegister(h, BCHP_LEAP_CTRL_SW_SPARE3, &spare3);
      if (spare3 & BHAB_45308_SPARE3_INIT_DONE)
      {
         /* AP has initialized */
         *pStatus |= BHAB_APSTATUS_INIT_DONE;

         BHAB_45308_P_ReadRegister(h, BCHP_LEAP_CTRL_HAB_REQUEST, &hab_request);
         if ((hab_request & 1) == 0)
...
******************************************************************************/
BERR_Code BHAB_45308_P_WriteMemory(BHAB_Handle h, uint32_t start_addr, const uint8_t *buf, uint32_t n)
{
   BERR_Code retCode = BERR_SUCCESS;
   ...
   if (bIsRunning && ((addr < BCHP_LEAP_HAB_MEM_WORDi_ARRAY_BASE) || (addr & 0x80000000)))
   {
      ...
      while (nWordsLeft > 0)
      {
         ...
         hab[0] = BHAB_45308_InitHeader(0x01, 0, BHAB_45308_WRITE, 0);
         hab[1] = addr;
         hab[2] = len;
         for (i = 0; i < len; i++)
         {
         ...
         BHAB_CHK_RETCODE(BHAB_45308_P_SendCommand(h, hab, 4+len));
   ...
   else
   {
      ...
      BHAB_CHK_RETCODE(BHAB_45308_P_WaitForBbsiDone(h));
      ...
      BHAB_CHK_RETCODE(BHAB_45308_P_WriteBbsi(h, BCHP_CSR_CONFIG, i2c_buf, 5));
      ...
      for (i = 0; i < nWords; i++)
      {
         ...
         if (bIsRunning)
         {
            ...
            i2c_buf[2] = buf[(i<<2)+2];
            i2c_buf[3] = buf[(i<<2)+3];
            BHAB_CHK_RETCODE(BHAB_45308_P_WriteBbsi(h, BCHP_CSR_RBUS_DATA0, i2c_buf, 4));
         }
         else
         {
            if (h->settings.isSpi == false)
            {
               ...
               BHAB_CHK_RETCODE(BHAB_45308_P_WriteBbsi(h, BCHP_CSR_RBUS_DATA0, (uint8_t *)buf, nWords<<2));
            }
            else
            {
...
BERR_Code BHAB_45308_P_ReadMemory(BHAB_Handle h, uint32_t start_addr, uint8_t *buf, uint32_t n)
{
   ...
   if (bIsRunning && ((addr < BCHP_LEAP_HAB_MEM_WORDi_ARRAY_BASE) || (addr & 0x80000000)))
   {
      ...
      while (nReadsLeft > 0)
      {
         ...
         BKNI_Memset(hab, 0, 128*sizeof(uint32_t));
         hab[0] = BHAB_45308_InitHeader(0x01, 0, BHAB_45308_READ, 0);
         hab[1] = addr;
         hab[2] = x;
         BHAB_CHK_RETCODE(BHAB_45308_P_SendCommand(h, hab, 4+x));
      ...
      for (i = buf_idx = 0; i < nReads; i++)
      {
         ...
         if (i == 0)
         {
            ...
            if (x)
            {
               ...
               BHAB_CHK_RETCODE(BHAB_45308_P_ReadRbus(h, addr, &val32, 1));
               if (x == 3)
...
******************************************************************************/
BERR_Code BHAB_45308_P_InstallInterruptCallback(
...
)
{
   ...
   BHAB_45308_P_CallbackInfo *pCbInfo;
   ...
   switch (eDevId)
   {
      case BHAB_DevId_eSAT:
         pCbInfo = &(p45308->cbSat);
         break;

      case BHAB_DevId_eDSQ:
         pCbInfo = &(p45308->cbDsq);
         break;

      case BHAB_DevId_eFSK:
         pCbInfo = &(p45308->cbFsk);
         break;

      case BHAB_DevId_eWFE:
         pCbInfo = &(p45308->cbWfe);
         break;
...
******************************************************************************/
BERR_Code BHAB_45308_P_UnInstallInterruptCallback(
    BHAB_Handle handle,  /* [in] BHAB handle */
    BHAB_DevId eDevId    /* [in] Device ID */
)
{
...
/******************************************************************************
 BHAB_45308_P_SendCommand()
******************************************************************************/
BERR_Code BHAB_45308_P_SendCommand(BHAB_Handle h, uint32_t *pBuf, uint32_t n)
{
   return BHAB_45308_P_SendHabCommand(h, (uint8_t*)pBuf, (uint16_t)n, (uint8_t*)pBuf, (uint16_t)n, true, true, 0);
}
...
******************************************************************************/
BERR_Code BHAB_45308_P_SendHabCommand(
...
{
   BHAB_45308_P_Handle *pImpl = (BHAB_45308_P_Handle *)(handle->pImpl);
   BERR_Code retCode = BERR_SUCCESS;
   ...
   BHAB_CHK_RETCODE(BHAB_45308_P_CheckHab(handle));
...
#endif
      BHAB_CHK_RETCODE(BHAB_45308_P_WriteRegister(handle, pImpl->habBaseAddr + (i<<2), &pWriteBuf[i]));
   }

   /* wait for the AP to service the HAB, and then read any return data */
   BHAB_CHK_RETCODE(BHAB_45308_P_ServiceHab(handle, pReadBuf, read_len));
   ...
   else if ((pReadBuf[0] & BHAB_45308_ACK) == 0)
   {
      retCode = BHAB_45308_P_DecodeHabError(handle);
      if (retCode == BERR_SUCCESS)
      {
...
BERR_Code BHAB_45308_P_Reset(BHAB_Handle h)
{
   ...
   BHAB_CHK_RETCODE(BHAB_45308_P_WriteRegister(h, BCHP_LEAP_CTRL_CTRL, &ctrl));

   /* initialize some LEAP registers */
   val32 = 0x101;
   BHAB_CHK_RETCODE(BHAB_45308_P_WriteRegister(h, BCHP_LEAP_CTRL_HAB_CNTR_CLR, &val32));
   val32 = 0;
   BHAB_CHK_RETCODE(BHAB_45308_P_WriteRegister(h, BCHP_LEAP_CTRL_SW_SPARE0, &val32));
   BHAB_CHK_RETCODE(BHAB_45308_P_WriteRegister(h, BCHP_LEAP_CTRL_SW_SPARE1, &val32));
   ...
   BHAB_CHK_RETCODE(BHAB_45308_P_WriteRegister(h, BCHP_LEAP_HOST_L1_INTR_W0_MASK_CLEAR, &val32));
#if 0
   if (BHAB_45308_P_IsLeapRunning(h))
      return BERR_TRACE(BHAB_ERR_AP_FAIL);
#endif
...
BERR_Code BHAB_45308_P_RunAp(BHAB_Handle h)
{
   ...
   BHAB_45308_P_ReadRegister(h, BCHP_LEAP_CTRL_CTRL, &ctrl);
...
#endif
   BHAB_45308_P_WriteRegister(h, BCHP_LEAP_CTRL_CTRL, &ctrl);
...
)
{
   ...
   mask = BHAB_45308_HIRQ0_HAB_DONE;
   BHAB_45308_P_WriteRegister(h, BCHP_LEAP_HOST_L2_CLEAR0, &mask);
   BHAB_45308_P_WaitForEvent(h, pImpl->hHabDoneEvent, 0);

#ifndef BHAB_DONT_USE_INTERRUPT
   /* enable HAB_DONE interrupt */
   BHAB_45308_P_WriteRegister(h, BCHP_LEAP_HOST_L2_MASK_CLEAR0, &mask);
#endif

   /* send the command */
   val32 = BCHP_LEAP_CTRL_HAB_REQUEST_HAB_REQUEST_MASK;
   BHAB_45308_P_WriteRegister(h, BCHP_LEAP_CTRL_HAB_REQUEST, &val32);

   /* assert the HAB interrupt on the LEAP */
   val32 = BCHP_LEAP_L2_CPU_SET_HAB_INTR_MASK;
   BHAB_45308_P_WriteRegister(h, BCHP_LEAP_L2_CPU_SET, &val32);
   ...
   {
      val32 = 0;
      BHAB_45308_P_ReadRegister(h, BCHP_LEAP_HOST_L2_STATUS0, &val32);
      if (val32 & BHAB_45308_HIRQ0_HAB_DONE)
         break;
      BKNI_Sleep(1);
   }
   if ((val32 & BHAB_45308_HIRQ0_HAB_DONE) == 0)
   ...
   retCode = BHAB_45308_P_WaitForEvent(h, pImpl->hHabDoneEvent, BHAB_CMD_TIMEOUT);
#endif
   ...
   if (retCode == BERR_TIMEOUT)
   {
      ...
      if ((val32 & BHAB_45308_HIRQ0_HAB_DONE) == 0)
      {
         ...
         BHAB_45308_P_ReadRegister(h, BCHP_LEAP_CTRL_SW_SPARE3, &spare3);
         BHAB_45308_P_ReadRegister(h, BCHP_LEAP_HOST_L2_STATUS0, &status0);
         BHAB_45308_P_ReadRegister(h, BCHP_LEAP_HOST_L2_MASK_STATUS0, &mask0);
   ...
   {
      retCode = BHAB_45308_P_ReadRegister(h, pImpl->habBaseAddr + (i<<2), &read_buf[i]);
   }
...
******************************************************************************/
BERR_Code BHAB_45308_P_HandleInterrupt_isr(
    BHAB_Handle h /* [in] BHAB handle */
)
{
   BHAB_45308_P_Handle *pImpl = (BHAB_45308_P_Handle *)(h->pImpl);
...
BERR_Code BHAB_45308_P_DecodeInterrupt(BHAB_Handle h)
{
   ...
   BHAB_45308_P_ReadRegister(h, BCHP_LEAP_HOST_L2_STATUS0, &status0);
   BHAB_45308_P_ReadRegister(h, BCHP_LEAP_HOST_L2_STATUS1, &status1);
   BHAB_45308_P_ReadRegister(h, BCHP_LEAP_HOST_L2_MASK_STATUS0, &mask0);
   BHAB_45308_P_ReadRegister(h, BCHP_LEAP_HOST_L2_MASK_STATUS1, &mask1);
   ...
   if (fstatus0)
   {
      if (fstatus0 & BHAB_45308_HIRQ0_INIT_DONE)
      {
         BKNI_SetEvent(pImpl->hInitDoneEvent);
         mask_set0 |= BHAB_45308_HIRQ0_INIT_DONE;
      }

      if (fstatus0 & BHAB_45308_HIRQ0_HAB_DONE)
      {
         BKNI_SetEvent(pImpl->hHabDoneEvent);
         mask_set0 |= BHAB_45308_HIRQ0_HAB_DONE;
      }
      ...
      if (fstatus0 & BHAB_45308_HIRQ0_SAT_INIT_DONE)
         mask_set0 |= BHAB_45308_HIRQ0_SAT_INIT_DONE;

      if (fstatus0 & BHAB_45308_HIRQ0_FSK_MASK)
      {
         retCode = BHAB_45308_P_DispatchCallback(&(pImpl->cbFsk), fstatus0, fstatus1);
      }

      if (fstatus0 & BHAB_45308_HIRQ0_DSQ_MASK)
      {
         if (fstatus0 & BHAB_45308_HIRQ0_DSQ0_TX)
            mask_set0 |= BHAB_45308_HIRQ0_DSQ0_TX;
         if (fstatus0 & BHAB_45308_HIRQ0_DSQ0_RX)
            mask_set0 |= BHAB_45308_HIRQ0_DSQ0_RX;
         if (fstatus0 & BHAB_45308_HIRQ0_DSQ0_VSENSE)
            mask_set0 |= BHAB_45308_HIRQ0_DSQ0_VSENSE;

         if (fstatus0 & BHAB_45308_HIRQ0_DSQ1_TX)
            mask_set0 |= BHAB_45308_HIRQ0_DSQ1_TX;
         if (fstatus0 & BHAB_45308_HIRQ0_DSQ1_RX)
            mask_set0 |= BHAB_45308_HIRQ0_DSQ1_RX;
         if (fstatus0 & BHAB_45308_HIRQ0_DSQ1_VSENSE)
            mask_set0 |= BHAB_45308_HIRQ0_DSQ1_VSENSE;

         retCode = BHAB_45308_P_DispatchCallback(&(pImpl->cbDsq), fstatus0, fstatus1);
      }

      if (fstatus0 & BHAB_45308_HIRQ0_WFE_MASK)
      {
         if (fstatus0 & BHAB_45308_HIRQ0_WFE0_READY)
            mask_set0 |= BHAB_45308_HIRQ0_WFE0_READY;

         if (fstatus0 & BHAB_45308_HIRQ0_WFE1_READY)
            mask_set0 |= BHAB_45308_HIRQ0_WFE1_READY;

         if (fstatus0 & BHAB_45308_HIRQ0_WFE_SA_DONE)
            mask_set0 |= BHAB_45308_HIRQ0_WFE_SA_DONE;

         retCode = BHAB_45308_P_DispatchCallback(&(pImpl->cbWfe), fstatus0, fstatus1);
      }
   }

   if (fstatus1 || (fstatus0 & BHAB_45308_HIRQ0_SAT_MASK))
   {
      retCode = BHAB_45308_P_DispatchCallback(&(pImpl->cbSat), fstatus0, fstatus1);
   }

   /* clear the interrupt status */
   BHAB_45308_P_WriteRegister(h, BCHP_LEAP_HOST_L2_CLEAR0, &fstatus0);
   BHAB_45308_P_WriteRegister(h, BCHP_LEAP_HOST_L2_CLEAR1, &fstatus1);

   if (mask_set0)
   {
      /* disable specified interrupts */
      BHAB_45308_P_WriteRegister(h,  BCHP_LEAP_HOST_L2_MASK_SET0, &mask_set0);
   }
   if (mask_set1)
   {
      /* disable specified interrupts */
      BHAB_45308_P_WriteRegister(h,  BCHP_LEAP_HOST_L2_MASK_SET1, &mask_set1);
   }
...
)
{
   ...
   if (BHAB_45308_P_ReadBbsi(h,  BCHP_CSR_STATUS, &csr_status, 1) == BERR_SUCCESS)
...
BERR_Code BHAB_45308_P_DecodeHabError(BHAB_Handle h)
{
   ...
   BHAB_45308_P_ReadRegister(h, BCHP_LEAP_CTRL_SW_SPARE3, &spare3);
   BHAB_45308_P_ReadRegister(h, BCHP_LEAP_HOST_L2_STATUS0, &status0);
   BHAB_45308_P_ReadRegister(h, BCHP_LEAP_CTRL_GP26, &gp26);

   BDBG_ERR(("status0=0x%08X, spare3=0x%08X, gp26=0x%08X", status0, spare3, gp26));

   if (status0 & BHAB_45308_HIRQ0_FW_ERROR)
   {
      fw_err = spare3 & BHAB_45308_SPARE3_FW_ERROR_CODE_MASK;
      switch (fw_err)
      {
         case 0:
         ...
         case 2: /* SAT error */
            ...
            BHAB_45308_P_DumpGpRegisters(h, 0, 16);
            retCode = BHAB_ERR_CMD_PROCESSING_ERR;
            break;

         case 5: /* WFE error */
            BDBG_ERR(("WFE (LEAP) error:"));
            BHAB_45308_P_DumpGpRegisters(h, 17, 19);
            retCode = BHAB_ERR_CMD_PROCESSING_ERR;
            break;

         case 11: /* DSQ error */
            BDBG_ERR(("DSQ (LEAP) error:"));
            BHAB_45308_P_DumpGpRegisters(h, 20, 22);
            retCode = BHAB_ERR_CMD_PROCESSING_ERR;
            break;

         case 12: /* FSK error */
            BDBG_ERR(("FSK (LEAP) error:"));
            BHAB_45308_P_DumpGpRegisters(h, 23, 24);
            retCode = BHAB_ERR_CMD_PROCESSING_ERR;
            break;

         case 10:
            BDBG_ERR(("LEAP task closed unexpectedly:"));
            BHAB_45308_P_DumpGpRegisters(h, 25, 27);
...
)
{
   ...
   BHAB_45308_P_WriteRegister(h, BCHP_LEAP_HOST_L2_MASK_SET0, &mask);
   BHAB_45308_P_WriteRegister(h, BCHP_LEAP_HOST_L2_MASK_SET1, &mask);
...
#ifdef BHAB_VERIFY_DOWNLOAD
/******************************************************************************
 BHAB_45308_P_CompareMemory()
******************************************************************************/
int BHAB_45308_P_CompareMemory(const void *p1, const void *p2, size_t n)
{
#if 0
   ...
   for (i = 0; (nerrors < 256) && (i < n); i++)
   {
      ...
      if (data1 != data2)
      {
         BDBG_WRN(("BHAB_45308_P_CompareMemory error: i=0x%X, expected=0x%02X, read=0x%02X", i, data1, data2));
         nerrors++;
      }
   }
...
bool BHAB_45308_VerifyMemory(BHAB_Handle h, uint32_t addr, const uint8_t *pHexImage, uint32_t len)
{
   ...
   BHAB_45308_P_ReadMemory(h, addr, (uint8_t *)pVerifyBuf, len);

   if (BHAB_45308_P_CompareMemory(pHexImage, pVerifyBuf, len) == 0)
      b = true;
   else
      b = false;
...
BERR_Code BHAB_45308_P_WaitForEvent(BHAB_Handle h, BKNI_EventHandle hEvent, int timeoutMsec)
{
   ...
   BHAB_45308_P_EnableHostInterrupt(h, true);
   while (1)
   {
      ...
      {
         BHAB_45308_P_DecodeInterrupt(h);
...
******************************************************************************/
BERR_Code BHAB_45308_P_GetInterruptEventHandle(
   BHAB_Handle handle,            /* [in] BHAB handle */
   BKNI_EventHandle *hEvent       /* [out] interrupt event handle */
)
{
   *hEvent = ((BHAB_45308_P_Handle *)(handle->pImpl))->hInterruptEvent;
   return BERR_SUCCESS;
}
 ...
 ******************************************************************************/
BERR_Code BHAB_45308_P_ProcessInterruptEvent(BHAB_Handle handle)
{
   BERR_Code retCode = BERR_SUCCESS;
   BDBG_ASSERT(handle);

   BHAB_CHK_RETCODE(BHAB_45308_P_DecodeInterrupt(handle));
   BHAB_45308_P_EnableHostInterrupt(handle, true);
...
/******************************************************************************
 BHAB_45308_P_WriteBbsi()
******************************************************************************/
BERR_Code BHAB_45308_P_WriteBbsi(
   BHAB_Handle h,    /* [in] BHAB PI Handle */
...
)
{
   ...
   if (h->settings.isSpi)
   {
      if (n < 16)
      {
         ...
         retCode = BREG_SPI_Write(((BHAB_45308_P_Handle*)h->pImpl)->hSpiRegister, (const uint8_t*)spiBuf, n+2);
      }
      else
      {
#if 1
         ...
         retCode = BREG_SPI_Write(((BHAB_45308_P_Handle*)h->pImpl)->hSpiRegister, (const uint8_t*)pBuf, n+2);
         BKNI_Free(pBuf);
#else
         BDBG_ERR(("BHAB_45308_P_WriteBbsi(): length (%d) too large!", n));
         BERR_TRACE(retCode = BERR_INVALID_PARAMETER);
#endif
      }
   }
   else
      retCode = BREG_I2C_Write(((BHAB_45308_P_Handle*)h->pImpl)->hI2cRegister, h->settings.chipAddr, addr, buf, n);
...
/******************************************************************************
 BHAB_45308_P_ReadBbsi()
******************************************************************************/
BERR_Code BHAB_45308_P_ReadBbsi(
    BHAB_Handle h,    /* [in] BHAB PI Handle */
    uint8_t     addr, /* [in] address */
...
)
{
   ...
   if (h->settings.isSpi)
   {
      if (n > 4)
      {
         BDBG_ERR(("BHAB_45308_P_ReadBbsi(): length (%d) too large!", n));
         BERR_TRACE(retCode = BERR_INVALID_PARAMETER);
         goto done;
      }
      ...
      retCode = BREG_SPI_Read(((BHAB_45308_P_Handle*)h->pImpl)->hSpiRegister, spiWriteBuf, spiReadBuf, n+2);
      if (retCode == BERR_SUCCESS)
      {
         for (i = 0; i < n; i++)
      ...
      retCode = BREG_I2C_Read(((BHAB_45308_P_Handle*)h->pImpl)->hI2cRegister, h->settings.chipAddr, addr, buf, n);
...
/******************************************************************************
BHAB_45308_P_ReadRbus()
******************************************************************************/
BERR_Code BHAB_45308_P_ReadRbus(
    BHAB_Handle h,    /* [in] BHAB PI Handle */
...
/******************************************************************************
BHAB_45308_P_WriteRbus()
******************************************************************************/
BERR_Code BHAB_45308_P_WriteRbus(
    BHAB_Handle h,    /* [in] BHAB PI Handle */
    uint32_t    addr, /* [in] address */
...
)
{
   ...
   for (i = 0; i < n; i++)
   {
      ...
      BHAB_CHK_RETCODE(BHAB_45308_P_WriteBbsi(h, (uint8_t)(i ? BCHP_CSR_RBUS_DATA0 : BCHP_CSR_CONFIG), i2c_buf, data_idx));
...
BERR_Code BHAB_45308_P_WaitForBbsiDone(BHAB_Handle h)
{
   ...
   {
      BHAB_CHK_RETCODE(BHAB_45308_P_ReadBbsi(h, BCHP_CSR_STATUS, &status, 1));
...
BERR_Code BHAB_45308_P_GetAvsData(BHAB_Handle handle, BHAB_AvsData *pData)
{
   ...
   buf[0] = BHAB_45308_InitHeader(0x07, 0, 0, 0);
   BHAB_CHK_RETCODE(BHAB_45308_P_SendCommand(handle, buf, 5));
