...
static void BCHP_PWR_P_HW_HDMI_RX0_CLK_Control(BCHP_Handle handle, bool activate)
{
    ...
    mask = ( BCHP_CLKGEN_ONOFF_DVP_HR_INST_CLOCK_ENABLE_DVPHR_54_CLOCK_ENABLE_MASK |
            BCHP_CLKGEN_ONOFF_DVP_HR_INST_CLOCK_ENABLE_DVPHR_BVB_216_CLOCK_ENABLE_MASK |
...
static void BCHP_PWR_P_HW_HDMI_TX_CLK_Control(BCHP_Handle handle, bool activate)
{
    ...
    reg &= ~mask;
    reg |= activate?mask:0;
    BREG_Write32(handle->regHandle, BCHP_CLKGEN_ONOFF_DVP_MHT_INST_CLOCK_ENABLE, reg);

    reg = BREG_Read32(handle->regHandle, BCHP_CLKGEN_ONOFF_DVP_MHT_INST_ENABLE);
    mask = ( BCHP_CLKGEN_ONOFF_DVP_MHT_INST_ENABLE_DVPHT_CLK_BVB_216_ENABLE_MASK |
            BCHP_CLKGEN_ONOFF_DVP_MHT_INST_ENABLE_DVPHT_CLK_BVB_324_ENABLE_MASK |
            BCHP_CLKGEN_ONOFF_DVP_MHT_INST_ENABLE_DVPHT_CLK_BVB_648_ENABLE_MASK );
    reg &= ~mask;
    reg |= activate?mask:0;
    BREG_Write32(handle->regHandle, BCHP_CLKGEN_ONOFF_DVP_MHT_INST_ENABLE, reg);
#endif
}
...
static void BCHP_PWR_P_HW_HDMI_IIC_CLK_Control(BCHP_Handle handle, bool activate)
{
    ...
    reg &= ~mask;
    reg |= activate?0:mask;
    BREG_Write32(handle->regHandle, BCHP_CLKGEN_ONOFF_DVP_MHT_INST_CLOCK_DISABLE, reg);
#endif
}
...
static void BCHP_PWR_P_HW_AFEC0_SRAM_AFEC1_SRAM_Control(BCHP_Handle handle, bool activate)
{
    uint32_t mask, reg;

    BDBG_MSG(("HW_AFEC0_SRAM_AFEC1_SRAM: %s", activate?"on":"off"));

    reg = BREG_Read32(handle->regHandle, BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_IN_AFEC);
    mask = BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_IN_AFEC_SRAM_PDA_IN_AFEC_MASK;
    reg &= ~mask;
    reg |= activate?0:mask;
    BREG_Write32(handle->regHandle, BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_IN_AFEC, reg);

    {
        uint32_t val=0, cnt=50;
        while(cnt--) {
            BKNI_Delay(10);
            reg = BREG_Read32(handle->regHandle, BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_OUT_AFEC);
            mask = BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_OUT_AFEC_SRAM_PDA_OUT_AFEC_MASK;
            reg &= mask;
            val |= activate?0:mask;
            if (val == reg)
                break;
        ...
        if(!cnt)
            BDBG_ERR(("HW_AFEC0_SRAM_AFEC1_SRAM Timeout"));
    }
}
...
static void BCHP_PWR_P_HW_DSEC0_SRAM_Control(BCHP_Handle handle, bool activate)
{
    uint32_t mask, reg;

    BDBG_MSG(("HW_DSEC0_SRAM: %s", activate?"on":"off"));

    reg = BREG_Read32(handle->regHandle, BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_IN_DSEC);
    mask = BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_IN_DSEC_SRAM_PDA_IN_DSEC_MASK;
    reg &= ~mask;
    reg |= activate?0:mask;
    BREG_Write32(handle->regHandle, BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_IN_DSEC, reg);

    {
        uint32_t val=0, cnt=50;
        while(cnt--) {
            BKNI_Delay(10);
            reg = BREG_Read32(handle->regHandle, BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_OUT_DSEC);
            mask = BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_OUT_DSEC_SRAM_PDA_OUT_DSEC_MASK;
            reg &= mask;
            val |= activate?0:mask;
            if (val == reg)
                break;
        ...
        if(!cnt)
            BDBG_ERR(("HW_DSEC0_SRAM Timeout"));
    }
}

static void BCHP_PWR_P_HW_FSK_SRAM_Control(BCHP_Handle handle, bool activate)
{
    uint32_t mask, reg;

    BDBG_MSG(("HW_FSK_SRAM: %s", activate?"on":"off"));

    reg = BREG_Read32(handle->regHandle, BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_IN_FTM);
    mask = BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_IN_FTM_SRAM_PDA_IN_FTM_MASK;
    reg &= ~mask;
    reg |= activate?0:mask;
    BREG_Write32(handle->regHandle, BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_IN_FTM, reg);

    {
        uint32_t val=0, cnt=50;
        while(cnt--) {
            BKNI_Delay(10);
            reg = BREG_Read32(handle->regHandle, BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_OUT_FTM);
            mask = BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_OUT_FTM_SRAM_PDA_OUT_FTM_MASK;
            reg &= mask;
            val |= activate?0:mask;
            if (val == reg)
                break;
...
static void BCHP_PWR_P_HW_LEAP_SRAM_Control(BCHP_Handle handle, bool activate)
{
    ...
    reg = BREG_Read32(handle->regHandle, BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_IN_LEAP);
    mask = BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_IN_LEAP_SRAM_PDA_IN_LEAP_MASK;
    reg &= ~mask;
    reg |= activate?0:mask;
    BREG_Write32(handle->regHandle, BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_IN_LEAP, reg);

    {
        uint32_t val=0, cnt=50;
        while(cnt--) {
            BKNI_Delay(10);
            reg = BREG_Read32(handle->regHandle, BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_OUT_LEAP);
            mask = BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_OUT_LEAP_SRAM_PDA_OUT_LEAP_MASK;
            reg &= mask;
            val |= activate?0:mask;
            if (val == reg)
                break;
...
static void BCHP_PWR_P_HW_SDS0_SRAM_SDS1_SRAM_Control(BCHP_Handle handle, bool activate)
{
    uint32_t mask, reg;

    BDBG_MSG(("HW_SDS0_SRAM_SDS1_SRAM: %s", activate?"on":"off"));

    reg = BREG_Read32(handle->regHandle, BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_IN_SDS0);
    mask = BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_IN_SDS0_SRAM_PDA_IN_SDS0_MASK;
    reg &= ~mask;
    reg |= activate?0:mask;
    BREG_Write32(handle->regHandle, BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_IN_SDS0, reg);

    {
        uint32_t val=0, cnt=50;
        while(cnt--) {
            BKNI_Delay(10);
            reg = BREG_Read32(handle->regHandle, BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_OUT_SDS0);
            mask = BCHP_AVS_TOP_CTRL_SRAM_POWER_GATE_OUT_SDS0_SRAM_PDA_OUT_SDS0_MASK;
            reg &= mask;
            val |= activate?0:mask;
            if (val == reg)
                break;
        ...
        if(!cnt)
            BDBG_ERR(("HW_SDS0_SRAM_SDS1_SRAM Timeout"));
    }
}
...
static void BCHP_PWR_P_DV_PLL_AVX_CH0_Control(BCHP_Handle handle, unsigned *mult, unsigned *prediv, unsigned *postdiv, bool set)
{
    ...
    BCHP_PWR_P_DIV_CONTROL(CLKGEN_PLL_AVX_PLL_DIV, NDIV_INT, CLKGEN_PLL_AVX_PLL_DIV, PDIV, CLKGEN_PLL_AVX_PLL_CHANNEL_CTRL_CH_0, MDIV_CH0)
}
...
static void BCHP_PWR_P_DV_PLL_AVX_CH1_Control(BCHP_Handle handle, unsigned *mult, unsigned *prediv, unsigned *postdiv, bool set)
{
    ...
    BCHP_PWR_P_DIV_CONTROL(CLKGEN_PLL_AVX_PLL_DIV, NDIV_INT, CLKGEN_PLL_AVX_PLL_DIV, PDIV, CLKGEN_PLL_AVX_PLL_CHANNEL_CTRL_CH_1, MDIV_CH1)
}
...
static void BCHP_PWR_P_DV_PLL_AVX_CH2_Control(BCHP_Handle handle, unsigned *mult, unsigned *prediv, unsigned *postdiv, bool set)
{
    ...
    BCHP_PWR_P_DIV_CONTROL(CLKGEN_PLL_AVX_PLL_DIV, NDIV_INT, CLKGEN_PLL_AVX_PLL_DIV, PDIV, CLKGEN_PLL_AVX_PLL_CHANNEL_CTRL_CH_2, MDIV_CH2)
}
...
static void BCHP_PWR_P_DV_PLL_AVX_CH3_Control(BCHP_Handle handle, unsigned *mult, unsigned *prediv, unsigned *postdiv, bool set)
{
    ...
    BCHP_PWR_P_DIV_CONTROL(CLKGEN_PLL_AVX_PLL_DIV, NDIV_INT, CLKGEN_PLL_AVX_PLL_DIV, PDIV, CLKGEN_PLL_AVX_PLL_CHANNEL_CTRL_CH_3, MDIV_CH3)
}
...
static void BCHP_PWR_P_DV_PLL_AVX_CH4_Control(BCHP_Handle handle, unsigned *mult, unsigned *prediv, unsigned *postdiv, bool set)
{
    ...
    BCHP_PWR_P_DIV_CONTROL(CLKGEN_PLL_AVX_PLL_DIV, NDIV_INT, CLKGEN_PLL_AVX_PLL_DIV, PDIV, CLKGEN_PLL_AVX_PLL_CHANNEL_CTRL_CH_4, MDIV_CH4)
}
...
static void BCHP_PWR_P_DV_PLL_CPU_CH2_Control(BCHP_Handle handle, unsigned *mult, unsigned *prediv, unsigned *postdiv, bool set)
{
    ...
    BCHP_PWR_P_DIV_CONTROL(CLKGEN_PLL_CPU_PLL_DIV, NDIV_INT, CLKGEN_PLL_CPU_PLL_DIV, PDIV, CLKGEN_PLL_CPU_PLL_CHANNEL_CTRL_CH_2, MDIV_CH2)
}
...
static void BCHP_PWR_P_DV_PLL_CPU_CH3_Control(BCHP_Handle handle, unsigned *mult, unsigned *prediv, unsigned *postdiv, bool set)
{
    ...
    BCHP_PWR_P_DIV_CONTROL(CLKGEN_PLL_CPU_PLL_DIV, NDIV_INT, CLKGEN_PLL_CPU_PLL_DIV, PDIV, CLKGEN_PLL_CPU_PLL_CHANNEL_CTRL_CH_3, MDIV_CH3)
}
...
static void BCHP_PWR_P_DV_PLL_CPU_CH4_Control(BCHP_Handle handle, unsigned *mult, unsigned *prediv, unsigned *postdiv, bool set)
{
    ...
    BCHP_PWR_P_DIV_CONTROL(CLKGEN_PLL_CPU_PLL_DIV, NDIV_INT, CLKGEN_PLL_CPU_PLL_DIV, PDIV, CLKGEN_PLL_CPU_PLL_CHANNEL_CTRL_CH_4, MDIV_CH4)
}
...
static void BCHP_PWR_P_DV_PLL_CPU_CH5_Control(BCHP_Handle handle, unsigned *mult, unsigned *prediv, unsigned *postdiv, bool set)
{
    ...
    BCHP_PWR_P_DIV_CONTROL(CLKGEN_PLL_CPU_PLL_DIV, NDIV_INT, CLKGEN_PLL_CPU_PLL_DIV, PDIV, CLKGEN_PLL_CPU_PLL_CHANNEL_CTRL_CH_5, MDIV_CH5)
}
...
static void BCHP_PWR_P_DV_PLL_CPU_CH1_Control(BCHP_Handle handle, unsigned *mult, unsigned *prediv, unsigned *postdiv, bool set)
{
    ...
    BCHP_PWR_P_DIV_CONTROL(CLKGEN_PLL_CPU_PLL_DIV, NDIV_INT, CLKGEN_PLL_CPU_PLL_DIV, PDIV, CLKGEN_PLL_CPU_PLL_CHANNEL_CTRL_CH_1, MDIV_CH1)
}
