...
static void BCHP_PWR_P_HW_AVD0_CLK_Control(BCHP_Handle handle, bool activate)
{
	...
	mask = BCHP_CLKGEN_AVD0_TOP_CLOCK_ENABLE_SVD_108_CLOCK_ENABLE_MASK;
	BREG_AtomicUpdate32(handle->regHandle, BCHP_CLKGEN_AVD0_TOP_CLOCK_ENABLE, mask, mask);
...
static void BCHP_PWR_P_HW_GFX_SRAM_Control(BCHP_Handle handle, bool activate)
{
	...
	BKNI_Delay(10);
	BREG_AtomicUpdate32(handle->regHandle, BCHP_CLKGEN_GRAPHICS_POWER_SWITCH_MEMORY, mask, 1);
	BKNI_Delay(10);
...
static void BCHP_PWR_P_HW_RFM_Control(BCHP_Handle handle, bool activate)
{
    ...
    if (activate) {
		...
		BREG_AtomicUpdate32(handle->regHandle, BCHP_CLKGEN_RFM_TOP_CLOCK_ENABLE, mask, mask);
    }
    else {
		/* RFM 108M clock */
		mask = BCHP_CLKGEN_RFM_TOP_CLOCK_ENABLE_RFM_108_CLOCK_ENABLE_MASK;
		BREG_AtomicUpdate32(handle->regHandle, BCHP_CLKGEN_RFM_TOP_CLOCK_ENABLE, mask, 0);
    }
}
...
static void BCHP_PWR_P_HW_RFM_SRAM_Control(BCHP_Handle handle, bool activate)
{
    ...
    mask = BCHP_CLKGEN_RFM_TOP_MEMORY_STANDBY_ENABLE_RFMA_MEMORY_STANDBY_ENABLE_MASK;
}
