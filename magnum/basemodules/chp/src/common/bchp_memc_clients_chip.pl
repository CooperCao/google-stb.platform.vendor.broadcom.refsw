...
sub usage {
    ...
    print "\t<dest> - name of already _existing_ file to be populated with list of clients, only the file header will get preserved\n";
    exit(1);
}

usage () if(scalar @ARGV<2);

my $self = "perl $0 " . join(' ',@ARGV);
my $txt_file = $ARGV[0];
my $h_file = $ARGV[1];
...
if($txt_file eq 'IO') {
    ...
    shift @ARGV;
    $mode='IO';
    print "Reading IO data ....\n";
    while(<>) {
        my @line;
        s/\r|\n//g;
        if(/GISB_MASTER_0?(\d+)\s+(\w+)\s+(UN)?MASK/) {
            #GISB_MASTER_24    avs_0     MASK         1  // reserve for AVS_0
            push @data,[$2,$1];
        } elsif(/GISB_ARB_BP_WRITE_0_(\w+)_SHIFT\s+0?(\d+)/) {
            #define BCHP_SUN_GISB_ARB_BP_WRITE_0_jtag_0_SHIFT                  8
            my $client = $1;
            my $no = $2;
            push @data,[$client,$no] unless $client =~ /reserved/;
        }
    }
    #use Data::Dumper;
    #print Dumper(\@data);
    exit 1 unless scalar @data;
} elsif($txt_file ne 'VH') {
...
for(@data) {
    if($mode eq 'IO') {
        my ($client, $no) = @$_;
        $client = uc $client;
        print $fout "BCHP_P_IO_DEFINE_CLIENT($client,$no)";
    } else {
        my ($_memc,$cmnt) = @$_;
