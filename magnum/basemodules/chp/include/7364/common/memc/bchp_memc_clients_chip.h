/***************************************************************************
 *     Copyright (c) 2003-2014, Broadcom Corporation
 *     All Rights Reserved
 *     Confidential Property of Broadcom Corporation
 *
 *  THIS SOFTWARE MAY ONLY BE USED SUBJECT TO AN EXECUTED SOFTWARE LICENSE
 *  AGREEMENT  BETWEEN THE USER AND BROADCOM.  YOU HAVE NO RIGHT TO USE OR
 *  EXPLOIT THIS MATERIAL EXCEPT SUBJECT TO THE TERMS OF SUCH AN AGREEMENT.
 *
 * $brcm_Workfile: $
 * $brcm_Revision: $
 * $brcm_Date: $
 *
 * Module Description:
 *
 * Revision History:
 *
 * $brcm_Log: $
 * 
 ***************************************************************************/


/* this file should not be directly included by unprepared code, but properly constructed code included it multiple times  */
/* ************** THIS FILE IS AUTOGENERATED. DO NOT EDIT **************************/
/*****
GENERATED by:
perl magnum/basemodules/chp/src/common/bchp_memc_clients_chip.pl VH magnum/basemodules/chp/include/7364/common/memc/bchp_memc_clients_chip.h
*******/
BCHP_P_MEMC_DEFINE_CLIENT(XPT_WR_RS,0)                      /*  */
BCHP_P_MEMC_DEFINE_CLIENT(XPT_WR_XC,1)                      /*  */
BCHP_P_MEMC_DEFINE_CLIENT(XPT_WR_CDB,2)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(XPT_WR_ITB_MSG,3)                 /*  */
BCHP_P_MEMC_DEFINE_CLIENT(XPT_RD_RS,4)                      /*  */
BCHP_P_MEMC_DEFINE_CLIENT(XPT_RD_XC_RMX_MSG,5)              /*  */
BCHP_P_MEMC_DEFINE_CLIENT(XPT_RD_XC_RAVE,6)                 /*  */
BCHP_P_MEMC_DEFINE_CLIENT(XPT_RD_PB,7)                      /*  */
BCHP_P_MEMC_DEFINE_CLIENT(XPT_WR_MEMDMA,8)                  /*  */
BCHP_P_MEMC_DEFINE_CLIENT(XPT_RD_MEMDMA,9)                  /*  */
BCHP_P_MEMC_DEFINE_CLIENT(GENET0_WR,10)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(GENET0_RD,11)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(GENET1_WR,12)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(GENET1_RD,13)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(MOCA_MIPS,16)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(BSP,22)                           /*  */
BCHP_P_MEMC_DEFINE_CLIENT(SCPU,23)                          /*  */
BCHP_P_MEMC_DEFINE_CLIENT(FLASH_DMA,24)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(SDIO_EMMC,26)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(SDIO_CARD,27)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(TPCAP,28)                         /*  */
BCHP_P_MEMC_DEFINE_CLIENT(MCIF_RD,29)                       /*  */
BCHP_P_MEMC_DEFINE_CLIENT(MCIF_WR,30)                       /*  */
BCHP_P_MEMC_DEFINE_CLIENT(UART_DMA_RD,31)                   /*  */
BCHP_P_MEMC_DEFINE_CLIENT(UART_DMA_WR,32)                   /*  */
BCHP_P_MEMC_DEFINE_CLIENT(USB_HI_0,33)                      /*  */
BCHP_P_MEMC_DEFINE_CLIENT(USB_LO_0,34)                      /*  */
BCHP_P_MEMC_DEFINE_CLIENT(USB_X_WRITE_0,35)                 /*  */
BCHP_P_MEMC_DEFINE_CLIENT(USB_X_READ_0,36)                  /*  */
BCHP_P_MEMC_DEFINE_CLIENT(USB_X_CTRL_0,37)                  /*  */
BCHP_P_MEMC_DEFINE_CLIENT(RAAGA,40)                         /*  */
BCHP_P_MEMC_DEFINE_CLIENT(RAAGA_1,41)                       /*  */
BCHP_P_MEMC_DEFINE_CLIENT(AUD_AIO,44)                       /*  */
BCHP_P_MEMC_DEFINE_CLIENT(VICE_VIP0_INST0,55)               /*  */
BCHP_P_MEMC_DEFINE_CLIENT(VICE_VIP1_INST0,56)               /*  */
BCHP_P_MEMC_DEFINE_CLIENT(HVD0_DBLK,73)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(HVD0_DBLK1,74)                    /*  */
BCHP_P_MEMC_DEFINE_CLIENT(HVD0_ILCPU,75)                    /*  */
BCHP_P_MEMC_DEFINE_CLIENT(HVD0_OLCPU,76)                    /*  */
BCHP_P_MEMC_DEFINE_CLIENT(HVD0_CAB,77)                      /*  */
BCHP_P_MEMC_DEFINE_CLIENT(HVD0_ILSI,78)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(SID,87)                           /*  */
BCHP_P_MEMC_DEFINE_CLIENT(BVN_MAD_PIX_FD,94)                /*  */
BCHP_P_MEMC_DEFINE_CLIENT(BVN_MAD_QUANT,95)                 /*  */
BCHP_P_MEMC_DEFINE_CLIENT(BVN_MAD_PIX_CAP,96)               /*  */
BCHP_P_MEMC_DEFINE_CLIENT(BVN_MFD0,106)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(BVN_MFD0_1,107)                   /*  */
BCHP_P_MEMC_DEFINE_CLIENT(BVN_MFD1,108)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(BVN_MFD1_1,109)                   /*  */
BCHP_P_MEMC_DEFINE_CLIENT(BVN_VFD0,118)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(BVN_VFD1,119)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(BVN_CAP0,126)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(BVN_CAP1,127)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(BVN_GFD0,134)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(BVN_GFD1,135)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(BVN_RDC,144)                      /*  */
BCHP_P_MEMC_DEFINE_CLIENT(VEC_VBI_ENC0,145)                 /*  */
BCHP_P_MEMC_DEFINE_CLIENT(M2MC_0,147)                       /*  */
BCHP_P_MEMC_DEFINE_CLIENT(M2MC_1,148)                       /*  */
BCHP_P_MEMC_DEFINE_CLIENT(M2MC_2,149)                       /*  */
BCHP_P_MEMC_DEFINE_CLIENT(LEAP,176)                         /*  */
BCHP_P_MEMC_DEFINE_CLIENT(ISDBT_TDI,177)                    /*  */
BCHP_P_MEMC_DEFINE_CLIENT(DVBT2_DFRM0_WR,178)               /*  */
BCHP_P_MEMC_DEFINE_CLIENT(DVBT2_DFRM1_RD,179)               /*  */
BCHP_P_MEMC_DEFINE_CLIENT(HOST_CPU_MCP_R_HI,200)            /*  */
BCHP_P_MEMC_DEFINE_CLIENT(HOST_CPU_MCP_R_LO,201)            /*  */
BCHP_P_MEMC_DEFINE_CLIENT(HOST_CPU_MCP_W_HI,202)            /*  */
BCHP_P_MEMC_DEFINE_CLIENT(HOST_CPU_MCP_W_LO,203)            /*  */
BCHP_P_MEMC_DEFINE_CLIENT(V3D_MCP_R_HI,204)                 /*  */
BCHP_P_MEMC_DEFINE_CLIENT(V3D_MCP_R_LO,205)                 /*  */
BCHP_P_MEMC_DEFINE_CLIENT(V3D_MCP_W_HI,206)                 /*  */
BCHP_P_MEMC_DEFINE_CLIENT(V3D_MCP_W_LO,207)                 /*  */
BCHP_P_MEMC_DEFINE_CLIENT(HVD0_MOCOMP,216)                  /*  */
BCHP_P_MEMC_DEFINE_CLIENT(MEMC_TRACELOG,248)                /*  */
BCHP_P_MEMC_DEFINE_CLIENT(RSVD_0,249)                       /*  */
BCHP_P_MEMC_DEFINE_CLIENT(RSVD_1,250)                       /*  */
BCHP_P_MEMC_DEFINE_CLIENT(MEMC_MSA,251)                     /*  */
BCHP_P_MEMC_DEFINE_CLIENT(MEMC_DIS0,252)                    /*  */
BCHP_P_MEMC_DEFINE_CLIENT(MEMC_DIS1,253)                    /*  */
BCHP_P_MEMC_DEFINE_CLIENT(MEMC_DRAM_INIT_ZQCS,254)          /*  */
BCHP_P_MEMC_DEFINE_CLIENT(REFRESH,255)                      /*  */
