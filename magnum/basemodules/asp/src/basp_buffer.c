...
BDBG_MODULE(BASP_BUFFER);

typedef struct BASP_P_Allocator
{
   BMMA_Heap_Handle hHeap;
   BMMA_RangeAllocator_Handle hRange;
   BASP_P_Buffer_AllocSettings stSettings;
...
void
BASP_P_Buffer_GetDefaultAllocSettings(
   BASP_P_Buffer_AllocSettings *pstAllocSettings
   )
{
   BDBG_ENTER( BASP_P_Buffer_GetDefaultAllocSettings );

   BDBG_ASSERT( pstAllocSettings );

   BKNI_Memset( pstAllocSettings, 0, sizeof( BASP_P_Buffer_AllocSettings ) );

   BDBG_LEAVE( BASP_P_Buffer_GetDefaultAllocSettings );
}

size_t
BASP_P_Allocator_GetSize(
   BASP_P_Allocator_Handle hAllocator
   )
{
...
BMMA_DeviceOffset
BASP_P_Allocator_GetDeviceOffset(
   BASP_P_Allocator_Handle hAllocator
   )
{
   BDBG_ASSERT( hAllocator );
...
void
BASP_P_Allocator_Destroy(
   BASP_P_Allocator_Handle hAllocator
   )
{
   BDBG_ENTER( BASP_P_Allocator_Destroy );
   ...
   BDBG_LEAVE( BASP_P_Allocator_Destroy );
}

BERR_Code
BASP_P_Allocator_Create(
   BMMA_Heap_Handle hHeap,
   const BASP_P_Buffer_AllocSettings *pstAllocSettings, /* Non-Null Indicate Create a Sub-Heap Allocator */
   BASP_P_Allocator_Handle *phAllocator
   )
{
   BASP_P_Allocator_Handle hAllocator = NULL;

   BDBG_ENTER( BASP_P_Allocator_Create );
   BDBG_ASSERT( hHeap );
   BDBG_ASSERT( phAllocator );

   hAllocator = (BASP_P_Allocator_Handle) BKNI_Malloc( sizeof( BASP_P_Allocator ) );
   if ( NULL == hAllocator )
   {
      BDBG_ERR(("Error allocating Allocator context"));
      return BERR_TRACE( BERR_OUT_OF_SYSTEM_MEMORY );
   }
   BKNI_Memset( hAllocator, 0, sizeof( BASP_P_Allocator ) );
   ...
   if ( ( NULL != pstAllocSettings ) && ( 0 != pstAllocSettings->uiSize ) )
   {
      ...
      if ( NULL == hAllocator->hBlock )
      {
         ...
         BASP_P_Allocator_Destroy( hAllocator );
   ...
   *phAllocator = hAllocator;
   ...
   BDBG_LEAVE( BASP_P_Allocator_Create );

   return BERR_TRACE( BERR_SUCCESS );
}

void
BASP_P_Buffer_UnlockDeviceOffset(
   BASP_P_Buffer_Handle hBuffer
   )
{
   BDBG_ENTER( BASP_P_Buffer_UnlockDeviceOffset );
   ...
   BDBG_LEAVE( BASP_P_Buffer_UnlockDeviceOffset );
}
...
{
   BDBG_ENTER( BASP_P_Buffer_Free );
   if ( NULL != hBuffer )
   {
      hBuffer->stSettings.bLockAddress = false;
      BASP_P_Buffer_UnlockAddress( hBuffer );

      hBuffer->stSettings.bLockOffset= false;
      BASP_P_Buffer_UnlockDeviceOffset( hBuffer );
...
BMMA_DeviceOffset
BASP_P_Buffer_GetDeviceOffset_isrsafe(
   BASP_P_Buffer_Handle hBuffer
   )
{
   BDBG_ASSERT( hBuffer );
...
unsigned
BASP_P_Buffer_GetDeviceBaseOffset(
   BASP_P_Buffer_Handle hBuffer
   )
{
   BDBG_ASSERT( hBuffer );

   if ( 0 != BASP_P_Allocator_GetDeviceOffset( hBuffer->hAllocator ) )
   {
      return ( BASP_P_Buffer_GetDeviceOffset_isrsafe( hBuffer ) - BASP_P_Allocator_GetDeviceOffset( hBuffer->hAllocator ) );
   }
   else
   {
      return 0;
...
void* BASP_P_Buffer_LockAddress(
   BASP_P_Buffer_Handle hBuffer
   )
{
   BDBG_ENTER( BASP_P_Buffer_LockAddress );
   ...
   if ( false == hBuffer->bBufferValid )
   {
      if ( NULL != hBuffer->hAllocator->hRange )
      {
         ...
         {
            hBuffer->pBuffer = (uint8_t *)  hBuffer->hAllocator->pBufferBase + ( BASP_P_Buffer_GetDeviceOffset_isrsafe( hBuffer ) - hBuffer->hAllocator->uiDeviceOffset );
...
void BASP_P_Buffer_UnlockAddress(
   BASP_P_Buffer_Handle hBuffer
   )
{
   BDBG_ENTER( BASP_P_Buffer_UnlockAddress );
   ...
   BDBG_LEAVE( BASP_P_Buffer_UnlockAddress );
}

BMMA_DeviceOffset
BASP_P_Buffer_LockDeviceOffset(
   BASP_P_Buffer_Handle hBuffer
   )
{
   BDBG_ENTER(  );
...
BASP_P_Buffer_Alloc(
   BASP_P_Allocator_Handle hAllocator,
   const BASP_P_Buffer_AllocSettings *pstAllocSettings,
   BASP_P_Buffer_Handle *phBuffer
   )
{
   BASP_P_Buffer_Handle hBuffer = NULL;

   BDBG_ENTER( BASP_P_Buffer_Alloc );
   BDBG_ASSERT( hAllocator );
   BDBG_ASSERT( pstAllocSettings );
   BDBG_ASSERT( 0 != pstAllocSettings->uiSize );
   BDBG_ASSERT( phBuffer );

   hBuffer = (BASP_P_Buffer_Handle) BKNI_Malloc( sizeof( BASP_P_Buffer ) );
   ...
   BKNI_Memset( hBuffer, 0, sizeof( BASP_P_Buffer ) );
   ...
   {
      BASP_P_Buffer_LockDeviceOffset( hBuffer );
   }

   if ( true == hBuffer->stSettings.bLockAddress )
   {
      void *pBuffer = BASP_P_Buffer_LockAddress( hBuffer );
   ...
   *phBuffer = hBuffer;
   ...
   BDBG_LEAVE( BASP_P_Buffer_Alloc );

   return BERR_TRACE( BERR_SUCCESS );
}

void
BASP_P_Buffer_FlushCache_isr(
   BASP_P_Buffer_Handle hBuffer,
   void *pBuffer,
   size_t uiSize
   )
{
   ...
   BDBG_ENTER( BASP_P_Buffer_FlushCache_isr );
   ...
   {
      BDBG_ERR(("BASP_P_Buffer_FlushCache_isr() called for an unlocked buffer"));
   }

   BDBG_LEAVE( BASP_P_Buffer_FlushCache_isr );
}

size_t
BASP_P_Buffer_GetSize(
   BASP_P_Buffer_Handle hBuffer
   )
{
   BDBG_ASSERT( hBuffer );
