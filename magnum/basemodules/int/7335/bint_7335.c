...
BDBG_MODULE(interruptinterface_7335);
...
static void BINT_P_7335_ClearInt( BREG_Handle regHandle, uint32_t baseAddr, int shift );
static void BINT_P_7335_SetMask( BREG_Handle regHandle, uint32_t baseAddr, int shift );
static void BINT_P_7335_ClearMask( BREG_Handle regHandle, uint32_t baseAddr, int shift );
static uint32_t BINT_P_7335_ReadStatus( BREG_Handle regHandle, uint32_t baseAddr );
...
#define BINT_MAP_STD_SUFFIX(L1,name,core,suffix) {BINT_IS_STANDARD | (BCHP_HIF_CPU_INTR1_INTR_W##L1##_STATUS_##name##_CPU_INTR_##suffix##_SHIFT+L1*32), BCHP_##core##_STATUS, 0, #name"#"#suffix}

static const BINT_P_IntMap bint_7335[] =
{
    ...
    BINT_MAP_STD_SUFFIX(0, AVD0, AVD_INTR2_0_CPU, 0),
    BINT_MAP_STD_SUFFIX(0, BVNF, BVNF_INTR2_5_R5F, 3),
    /* TODO MMC_CPU_INTR, upg MC_IF Block */
    BINT_MAP_STD(0, SDS0_RCVR_1, SDS_INTR2_1_0_CPU),
    ...
    BINT_MAP_STD(0, MEMC_0, MEMC_0_L2_R5F),
    BINT_MAP_STD_SUFFIX(0, BVNF, BVNF_INTR2_1_R5F, 1),
    BINT_MAP_STD_SUFFIX(0, BVNF, BVNF_INTR2_0_R5F, 0),
...
#ifdef INCLUDE_SM
    /*SM interrupt */
    BINT_MAP_STD(1, SM, SM_L2_CPU),
#endif /* INCLUDE_SM */
    BINT_MAP_STD(0, GFX, GRAPHICS_L2_CPU),
    BINT_MAP_STD(0, AIO, AIO_INTH_R5F),
    BINT_MAP_STD(0, BSP, BSP_CONTROL_INTR2_CPU),
    BINT_MAP(0, XPT_ICAM, "", XPT_BUS_IF_INTR_STATUS3_REG, REGULAR, ALL, 0),
    ...
    BINT_MAP(0, XPT_STATUS, "_PB3", XPT_PB3_INTR, REGULAR, ALL, 0),
    BINT_MAP(0, XPT_STATUS, "_PB4", XPT_PB4_INTR, REGULAR, ALL, 0),
    BINT_MAP(1, XPT_MSG_STAT, "", XPT_MSG_DAT_ERR_INTR,       REGULAR, ALL, 0),
    BINT_MAP(1, XPT_FE, "", XPT_FE_INTR_STATUS_REG, REGULAR, ALL, 0),
    ...
    BINT_MAP_STD_SUFFIX(1, BVNF, BVNF_INTR2_6_R5F, 6),
    BINT_MAP_STD(1, SDS0_RCVR_0, SDS_INTR2_0_0_CPU),
    BINT_MAP_STD(1, SDS1_RCVR_0, SDS_INTR2_0_1_CPU),
   
    BINT_MAP_STD(2, FTM, FTM_INTR2_CPU),
    BINT_MAP_STD(2, MEMC_1, MEMC_1_L2_R5F),
    BINT_MAP(2, AFEC_CORE_0, "", AFEC_0_uP_IRQ_STAT, REGULAR, ALL, 0),
    BINT_MAP(2, AFEC_CORE_1, "", AFEC_1_uP_IRQ_STAT, REGULAR, ALL, 0),
    BINT_MAP_STD(2, QPSK_RCVR_1, QPSK_INTR2_1_CPU),
    BINT_MAP_STD(2, QPSK_RCVR_0, QPSK_INTR2_0_CPU),
    BINT_MAP_STD(2, SDS1_RCVR_1, SDS_INTR2_1_1_CPU),
    { -1, 0, 0, NULL} 
};

static const BINT_Settings bint_7335Settings =
{
    NULL,
    BINT_P_7335_ClearInt,
    BINT_P_7335_SetMask,
    BINT_P_7335_ClearMask,
    NULL,
    BINT_P_7335_ReadStatus, 
    bint_7335,
    "7335"
};
...
static void verify_map(const BINT_P_IntMap *m1, const BINT_P_IntMap *m2)
{
    ...
    for(i=0;;i++) {
        ...
        switch( m1[i].L2RegOffset )
        {
        BINT_P_STANDARD_L2_CASES
            if ( !(BINT_IS_STANDARD & (m1[i].L1Shift | m2[i].L1Shift))) {
                BDBG_WRN(("missing STANDARD at %u: %d:%d %u:%u %#x:%#x %s:%s", i, m1l1s, m2l1s, m1[i].L2RegOffset, m2[i].L2RegOffset, m1[i].L2InvalidMask, m2[i].L2InvalidMask, m1[i].L2Name, m2[i].L2Name));
            }
            break;
        default:
            if ( (BINT_IS_STANDARD & (m1[i].L1Shift | m2[i].L1Shift))) {
                BDBG_WRN(("incorrectly flagged as STANDARD at %u: %d:%d %u:%u %#x:%#x %s:%s", i, m1l1s, m2l1s, m1[i].L2RegOffset, m2[i].L2RegOffset, m1[i].L2InvalidMask, m2[i].L2InvalidMask, m1[i].L2Name, m2[i].L2Name));
            }
            break;
        }
    }
...
const BINT_Settings *BINT_7335_GetSettings( void )
{
    return &bint_7335Settings;
}
