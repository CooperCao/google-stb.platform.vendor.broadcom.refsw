...
static int32_t BRPC_P_InitSocketPool(BRPC_SocketData *pSockData)
{
	...
	for(j = 0; j < BRPC_RMAGNUM_CLNT_NSOCKETS; j++)
	{
		...
		BDBG_MSG((" trying to bind to %x for thread %px", port, current));
		...
		{
			/* success */
			BDBG_MSG((" UDP sock_ack ipany port %x sock_req %px ", port, clnt_data->sock_req));
			...
			pSockData->free_cnt++;
			BDBG_MSG((" New client data for %p08lx: fd = %pd buf = 0x%p08lx free_cnt = %d", current, clnt_data->sock_req, clnt_data->sock_req_buf, pSockData->free_cnt));
		}
	}
	return 0;
...
static inline BRPC_ClntData* BRPC_P_GetClntData(BRPC_SocketData *pSockData)
{
	...
	if(!BLST_S_EMPTY(&(pSockData->free_cd_list)))
	{
		...
		BDBG_MSG((" Got client data for %p08lx: fd = %px buf = %p08lx", current, clnt_data->sock_req, clnt_data->sock_req_buf));
	...
	for (i = 0; i < BRPC_RMAGNUM_CLNT_NPORTS; i++)
	{
		...
		else
		{
			...
			/* add the node to the list */
			BDBG_MSG((" New client data for %p08lx: fd = %pd buf = 0x%p08lx", current, clnt_data->sock_req, clnt_data->sock_req_buf));
	...
	uint32_t *outparams, unsigned outparams_num, BERR_Code *retVal)
{
	...
	BDBG_MSG((" UDP write: req %px", cdata->sock_req));
	...
	while(1)
	{
		...
		BDBG_MSG((" UDP read: ack %p08lx", cdata->sock_req));
		/* wait for reply */
