...
static int32_t BRPC_P_InitSocketPool(BRPC_SocketData *pSockData)
{
	...
	for(j = 0; j < BRPC_RMAGNUM_CLNT_NSOCKETS; j++)
	{
		...
		optval = 1;
		setsockopt (clnt_data->sock_req, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof (optval));
		...
		else
		{
			...
			pSockData->free_cnt++;
			BDBG_MSG((" New client data for %08lx: fd = %d buf = 0x%p08lx free_cnt = %d", (long unsigned int)getpid(), clnt_data->sock_req, clnt_data->sock_req_buf, pSockData->free_cnt));
		}
	}
	return 0;
...
****************************************************************************/
static __inline__ BRPC_ClntData* BRPC_P_GetClntData(BRPC_SocketData *pSockData)
{
	struct BRPC_ClntData *clnt_data;
	struct sockaddr_in sin_ctl;
	unsigned short int port;
	...
	if(!BLST_S_EMPTY(&(pSockData->free_cd_list)))
	{
		...
		BDBG_MSG((" Got client data for %08lx: fd = %x buf = %p08lx", (long unsigned int)getpid(), clnt_data->sock_req, clnt_data->sock_req_buf));
	...
	for (i = 0; i < BRPC_RMAGNUM_CLNT_NPORTS; i++)
	{
		...
		else
		{
			...
			/* add the node to the list */
			BDBG_MSG((" New client data for %08lx: fd = %d buf = 0x%p08lx", (long unsigned int)getpid(), clnt_data->sock_req, clnt_data->sock_req_buf));
...
****************************************************************************/
static __inline__ void BRPC_P_ReleaseClntData(BRPC_SocketData *pSockData, BRPC_ClntData *clnt_data)
{
...
BERR_Code BRPC_Open_SocketImpl(BRPC_Handle *handle, const BRPC_OpenSocketImplSettings *openSettings)
{
	...
	*handle = BKNI_Malloc(sizeof(struct BRPC_P_Handle));
	...
	/* make the local address reusable */
	optval = 1;
	setsockopt (pSockData->sock_ntf, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof (optval));
