...
/*
Summary:
This function asserts or deasserts the slave select with if the cont bit in the last CDRAM register is set.
*/
BERR_Code BREG_SPI_SetSlaveSelect(
    BREG_SPI_Handle spiHandle,      /* Device channel handle */
    bool bEnable                    /* Enable or disable continue after command flag.This allows us to hold the\
                                       chip select line low to do multiple spi transfers. */
);
