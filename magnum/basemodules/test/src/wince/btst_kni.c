...
BTST_CreateTask(BTST_TaskHandle *pTask,  BTST_TaskFunction task_fn, void *data)
{
    ...
    *pTask = task;
    ...
    task->hTask = CreateThread(NULL, 0 /* default stack size */, Win32ThreadProc, task, CREATE_SUSPENDED, &task->ThreadId);
    if (!task->hTask) {
        free(task);
        return BERR_TRACE(BERR_OS_ERROR);
    }

	// nPriority		: 0 highest, 255 lowest
	nPriority = 30;

	if (!CeSetThreadPriority(task->hTask, nPriority)) {
	    BDBG_MSG(("SetThreadPriority failed"));
		ret = BERR_OS_ERROR;
		BDBG_ASSERT(0);
	}

	if (ResumeThread(task->hTask) == 0xFFFFFFFF){ 
	    BDBG_MSG(("ResumeThread failed"));
		ret = BERR_OS_ERROR;
		BDBG_ASSERT(0);
	}
