...
)
{
    ...
    sCommand.uCommand.sRaagaInitCommand.sROImageSizeInfo.ui32RORdbVarSize= BDSP_IMG_SYSTEM_RDBVARS_SIZE;
    sCommand.uCommand.sRaagaInitCommand.sROImageSizeInfo.ui32RORomfsSize =
                    (BDSP_IMG_INIT_ROMFS_SIZE+BDSP_IMG_SYSTEM_CODE_SIZE+BDSP_IMG_SYSTEM_LIB_SIZE);
    sCommand.uCommand.sRaagaInitCommand.sROImageSizeInfo.ui32ROLoadableImgSize = (pDevice->memInfo.sROMemoryPool.ui32Size-
                    (BDSP_IMG_SYSTEM_KERNEL_SIZE+BDSP_IMG_SYSTEM_RDBVARS_SIZE+
                     BDSP_IMG_INIT_ROMFS_SIZE+BDSP_IMG_SYSTEM_CODE_SIZE+BDSP_IMG_SYSTEM_LIB_SIZE));

    sCommand.uCommand.sRaagaInitCommand.sCustomMMInfo.ui32UserProcessSpawnMemSize = BDSP_IMG_USER_PROCESS_SPAWN_MEM_SIZE;
    sCommand.uCommand.sRaagaInitCommand.sCustomMMInfo.ui64ProcessSpawnMemStartAddr= 0; /* Future Use*/
    ...
    sCommand.uCommand.sRaagaInitCommand.sRWImageSizeInfo.ui32RWCustomMMMemSize     =
                    ((BDSP_IMG_USER_PROCESS_SPAWN_MEM_SIZE*BDSP_MAX_NUM_USER_PROCESS)+
                    sCommand.uCommand.sRaagaInitCommand.sCustomMMInfo.ui32TotalWorkBufferSize);

    for(i = 0, heapCount = 0; i < BDSP_RAAGA_MAX_NUM_HEAPS; i++)
    {
        if(0 != pDevice->deviceSettings.heap[i].size)
        {
            sCommand.uCommand.sRaagaInitCommand.sHeapInfo.sHeapLimits[i].HeapBaseAddr = pDevice->deviceSettings.heap[heapCount].baseAddress;
            sCommand.uCommand.sRaagaInitCommand.sHeapInfo.sHeapLimits[i].ui64HeapSize = pDevice->deviceSettings.heap[heapCount].size;
            heapCount++;
        }
    }

    sCommand.uCommand.sRaagaInitCommand.sHeapInfo.ui32NumHeap =  heapCount;

    for(i = 0; i < heapCount; i++)
    {
        BDBG_MSG(("Heap %d base address  : " BDSP_MSG_FMT, i, BDSP_MSG_ARG(sCommand.uCommand.sRaagaInitCommand.sHeapInfo.sHeapLimits[i].HeapBaseAddr)));
    ...
    sCommand.uCommand.sRaagaInitCommand.sSchedulingInfo.ui32NumPreemptionLevels = BDSP_MAX_NUM_PREEMPTION_LEVELS;
    sCommand.uCommand.sRaagaInitCommand.sSchedulingInfo.ui32NumSchedulingLevels = BDSP_MAX_NUM_SCHED_LEVELS;

    for(i=0; i< BDSP_MAX_NUM_SCHED_LEVELS;i++)
    {
        sCommand.uCommand.sRaagaInitCommand.sSchedulingInfo.ui32PreemptiveThreshold[i] = 0;
    }
...
)
{
    ...
    errCode = BDSP_Raaga_P_GetResponse(pDevice->hGenRespQueue[dspindex], (void *)&sRsp, sizeof(BDSP_P_Response));
    if(errCode != BERR_SUCCESS)
    {
...
)
{
    ...
    errCode = BDSP_Raaga_P_GetResponse(pRaagaTask->hSyncQueue, (void *)&sRsp, sizeof(BDSP_P_Response));
    if(errCode != BERR_SUCCESS)
    {
...
)
{
    ...
    BDBG_ENTER(BDSP_Raaga_P_ProcessCITReConfigCommand);
    BDBG_MSG(("BDSP_Raaga_P_ProcessCITReConfigCommand"));
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Raaga_P_ProcessCITReConfigCommand: Error in Writing the command for Task (%d)",pRaagaTask->taskParams.taskId));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    errCode = BKNI_WaitForEvent(pRaagaTask->hEvent, BDSP_EVENT_TIMEOUT_IN_MS);
    if (BERR_TIMEOUT == errCode)
    {
        BDBG_ERR(("BDSP_Raaga_P_ProcessCITReConfigCommand: CIT RECONFIG timeout for Task (%d)",pRaagaTask->taskParams.taskId));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Raaga_P_ProcessCITReConfigCommand: Unable to read response for Task (%d)",pRaagaTask->taskParams.taskId));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    {
        BDBG_ERR(("BDSP_Raaga_P_ProcessCITReConfigCommand: CIT RECONFIG RESPONSE not received successfully!!, Status = %d, Response for CMD ID = %d",
            sRsp.sResponseHeader.eStatus, sRsp.sResponseHeader.eCommandID));
        errCode = BERR_TRACE(sRsp.sResponseHeader.eStatus);
        goto end;
    }
end:
    BDBG_LEAVE(BDSP_Raaga_P_ProcessCITReConfigCommand);
    return errCode;
}
...
)
{
    ...
    BDBG_ENTER(BDSP_Raaga_P_ProcessAudioOutputFreezeCommand);
    BKNI_Memset(&sCommand, 0, sizeof(BDSP_P_Command));
    BKNI_Memset(&sRsp, 0, sizeof(BDSP_P_Response));
...
)
{
    ...
    BDBG_ENTER(BDSP_Raaga_P_ProcessAudioOutputUnFreezeCommand);
    BKNI_Memset(&sCommand, 0, sizeof(BDSP_P_Command));
    BKNI_Memset(&sRsp, 0, sizeof(BDSP_P_Response));
