 ...
 * The following table provides an overview of the available API:
 ...
 -----------+-------------------------+-----------------------------
  ...
  access    |  TB_availableData       |
            ...
            |  TB_dumpToFile          |
 -----------+-------------------------+-----------------------------
  Access    |                         |  TB_attachToBuffers
 through a  |                         |  TB_detachFromBuffers
 descriptor |  TB_peek                |  TB_peek
            |  TB_seek                |  TB_seek
            |  TB_read                |  TB_read
            |  TB_lock                |  TB_lock
            |  TB_lockAll             |  TB_lockAll
 -----------+-------------------------+-----------------------------
...
typedef struct
{
    ...
    ADDR tb_addr;                   /**< location of the TB_shared structure */
    ADDR_SPACE buffer_addr_space;   /**< address space the storage, read_ptr and write_ptr fields in tb_addr refer to */
    TB_BUFF_SIZE buffer_size;       /**< storage size, cached value of @code ((TB_shared *) tb_addr)->storage_kbytes * 1024 @endcode */
...
#else
    TB_BUFF_PTR data;   /**< pointer to data, meaning is system dependent (see TB_data_descriptor) */
#endif
    TB_BUFF_SIZE size;  /**< size of the pointed data */
} TB_data_region;
...
typedef struct
{
    ...
    ADDR_SPACE        buffers_address_space;    /**< what address space the @p data fields in @p buffers elements refer to */
#endif

    TB_BUFF_SIZE      total_size;       /**< sum of the data sizes in @p buffers */
    TB_BUFF_SIZE      cur_offset;       /**< current read cursor, range [0, total_size] */
    unsigned          cur_buffer;       /**< index of the buffer where @p cur_pointer lives */

#if !IS_HOST(DSP_LESS)
    TB_BUFF_PTR       cur_pointer;
#else
    uint8_t          *cur_pointer;
#endif
...
/**
 * Attaches to a pre-initialised Target Buffer structure.
...
TB_data_descriptor *TB_attachToBuffers(TB_data_descriptor *data_descriptor,
                                     ...
                                     , DSP *dsp
                                     , ADDR_SPACE buffers_address_space
#endif
                                       );
...
/**
 * Maps data from the buffers described by the TB_data_descriptor argument for
 * direct access (see DSP_mapSystemAddress). The function looks at a range
 * starting at the descriptor current read cursor position and tries to map as
 * many contiguous bytes as possible up to the requested #size length.
 * Upon correct mapping, the read cursor is moved forward only if requested.
 *
 * Note: a pre-requisite for the function to succeed is fort the
 * TB_data_descriptor buffers to live in the 'system' address space.
 *
 * @param[in]  descriptor   the data descriptor
 * @param[out] addr         out variable where to store the mapped region
 * @param[in]  size         maximum size for the region to map
 * @param[in]  move_cursor  whether to move forward the read cursor after the mapping
 * @return                  the number of bytes actually mapped, between 1 and #size; 0 if the map failed
 */
__attribute__((nonnull(1,2)))
...
/**
 * Maps all the buffers described by the TB_data_descriptor argument for direct
 * access (see DSP_mapSystemAddress).
 * The TB_data_descriptor buffers are mapped to an array of MAPPED_ADDR
 * structures. The number of used entries is returned; if not enough entries
 * are provided, no mapping is performed and 0 is returned.
 * If called with addr_array == NULL, the function just returns the number of
 * necessary entries in the array to correctly map all buffers.
 *
 * Note: a pre-requisite for the function to succeed is fort the
 * TB_data_descriptor buffers to live in the 'system' address space.
 *
 * @param[in]  descriptor  the data descriptor
 * @param[out] addr_array  array where to store the mapped regions; NULL to query the required array size
 * @param[in]  addr_count  number of entries in the provided #addr_array
 * @return                 the number of valid entries returned in addr_array, 0 on error;
 *                         the number of required entries, if addr_array is NULL
 */
 __attribute__((nonnull(1)))
...
typedef struct
{
    ...
    TB_BUFF_SIZE              payload_offset;       /**< payload data read position inside a source TB_data_descriptor */
    ...
    TB_BUFF_SIZE              payload_length;       /**< length of payload data */
#if !IS_HOST(DSP_LESS)
