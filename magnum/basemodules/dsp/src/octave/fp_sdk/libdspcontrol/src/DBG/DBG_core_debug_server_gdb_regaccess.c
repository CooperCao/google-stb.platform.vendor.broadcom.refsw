...
typedef TX_fullContext DBG_full_Context;
#endif
 ...
 * */
static DBG_core_reg_info_t DBG_core_register_info[]=
{
#if defined (__FPM1015_ONWARDS__)
    {true, FP_REG_R0, FP_REG_R15, 4, offsetof(DBG_full_Context, commonRegisterContext) +
                                     offsetof(CS_registerContext, gregs[0]) },

    {true, FPM_REG_PSR, FPM_REG_PSR, 4, offsetof(DBG_full_Context, commonRegisterContext) +
                                      offsetof(CS_registerContext, psr)},

    {true, FPM_REG_PC, FPM_REG_PC, 4, offsetof(DBG_full_Context, commonRegisterContext) +
                                      offsetof(CS_registerContext, pc)},

    {true, FPM_REG_P0, FPM_REG_P1, 1, offsetof(DBG_full_Context, commonRegisterContext) +
                                      offsetof(CS_registerContext, pregs)}

#else
    {true,   FP_REG_R0,  FP_REG_R61, 8,  offsetof(DBG_full_Context, commonRegisterContext) +
    ...
    {true,   FP_REG_PSR, FP_REG_PSR,   8, offsetof(DBG_full_Context, commonRegisterContext) +
                                         offsetof(CS_registerContext, psr) },

    /*Special Registers - MSR*/
    {true,   FP_REG_MSR0, FP_REG_MSR3, 8, offsetof(DBG_full_Context, commonRegisterContext) +
                                         offsetof(CS_registerContext, mReg) +
                                         offsetof(CS_mRegContext, u64Msr[0]) },

    /*Special Registers - PC*/
    {true,   FP_REG_PC, FP_REG_PC, 8, offsetof(DBG_full_Context, commonRegisterContext) +
                                     offsetof(CS_registerContext, pc)},
    /*MAC registers*/
    {true,   FP_REG_M0, FP_REG_M7, 24, offsetof(DBG_full_Context, commonRegisterContext) +
                                      offsetof(CS_registerContext, mReg) +
...
#else
    {true,   FP_REG_P0,  FP_REG_P3,  1,  offsetof(DBG_full_Context, u64PredReg)},
#endif
    ...
    {true, FP_REGS_CREG_INDEX(CREG_BX_RES), FP_REGS_CREG_INDEX(CREG_BX_RES), 32, offsetof(DBG_full_Context, commonRegisterContext) +
    ...
    {true, FP_REGS_CREG_INDEX(CREG_BX_COUNT), FP_REGS_CREG_INDEX(CREG_BX_COUNT), 8, offsetof(DBG_full_Context, commonRegisterContext) +
                                            offsetof(CS_registerContext, bitFieldState) },

    {true, FP_REGS_CREG_INDEX(CREG_BX_TRELLIS), FP_REGS_CREG_INDEX( CREG_BX_TRELLIS), 8, offsetof(DBG_full_Context, commonRegisterContext) +
                                                offsetof(CS_registerContext, bitFieldState) },

    {true, FP_REGS_CREG_INDEX(CREG_BC_RES), FP_REGS_CREG_INDEX( CREG_BC_RES), 16, offsetof(DBG_full_Context, commonRegisterContext) +
                                         offsetof(CS_registerContext, bitFieldState) +
                                         offsetof(CS_bitFieldState, bcr)},

    {true, FP_REGS_CREG_INDEX(CREG_BC_COUNT), FP_REGS_CREG_INDEX( CREG_BC_COUNT), 8, offsetof(DBG_full_Context, commonRegisterContext) +
                                            offsetof(CS_registerContext, bitFieldState) },
#endif
    ...
    /* SIMD trellis decode */
    {true, FP_REGS_CREG_INDEX(CREG_TD_STATE), FP_REGS_CREG_INDEX( CREG_TD_STATE), 16, offsetof(DBG_full_Context, commonRegisterContext) +
                                             offsetof(CS_registerContext, trellisState)},

    /* SIMD trellis traceback */
    {true, FP_REGS_CREG_INDEX(CREG_TR_STATE), FP_REGS_CREG_INDEX( CREG_TR_STATE), 8, offsetof(DBG_full_Context, commonRegisterContext) +
                                            offsetof(CS_registerContext, trellisState) +
    ...
    /*FIR STATE*/
    {true, FP_REGS_CREG_INDEX(CREG_FIR_HOLD_0), FP_REGS_CREG_INDEX( CREG_FIR_HOLD_0), 8, offsetof(DBG_full_Context, commonRegisterContext) +
                                                offsetof(CS_registerContext, firState) + 0},

    {true, FP_REGS_CREG_INDEX(CREG_FIR_HOLD_1), FP_REGS_CREG_INDEX( CREG_FIR_HOLD_1), 8, offsetof(DBG_full_Context, commonRegisterContext) +
                                                offsetof(CS_registerContext, firState) + 8},

    {true, FP_REGS_CREG_INDEX(CREG_FIR_HOLD_2), FP_REGS_CREG_INDEX( CREG_FIR_HOLD_2), 8, offsetof(DBG_full_Context, commonRegisterContext) +
                                                offsetof(CS_registerContext, firState) + 16},

    {true, FP_REGS_CREG_INDEX(CREG_FIR_HOLD_3), FP_REGS_CREG_INDEX( CREG_FIR_HOLD_3), 8, offsetof(DBG_full_Context, commonRegisterContext) +
...
DBG_core_get_dba_reg_info(uint32_t u32_reg_num, DBG_core_cs_reg_info_t *p_cs_reg_info)
{
    ...
    for(index = 0; index < NUM_ELEMS(DBG_core_register_info); index++)
    {
        if((u32_reg_num >= DBG_core_register_info[index].fp_reg_lo) && (u32_reg_num <= DBG_core_register_info[index].fp_reg_hi))
        {
            DBG_core_reg_info_t *p_reg_info = &DBG_core_register_info[index];
            if(DBG_core_register_info[index].b_valid)
            {
