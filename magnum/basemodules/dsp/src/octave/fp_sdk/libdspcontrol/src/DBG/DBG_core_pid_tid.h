...
typedef struct
{
    ...
    int   num_segments;
    uint32_t *segments;
    uint32_t sba;                       /*SBA of the dynamic shared object*/
} dso_info_t;
...
typedef enum
{
    ...
    DBG_STOP_REASON_PROCESS_EXITED,
    DBG_STOP_REASON_FATAL,
} dbg_stop_reason_t;

typedef struct dbg_core_stop_reason_t
{
    dbg_pid_t   pid;
    dbg_tid_t   tid;
    ...
    uint32_t            u32_addr;
    struct dbg_core_stop_reason_t *p_stop_reason_next;
} dbg_core_stop_reason_t;

typedef struct
{
    dbg_core_stop_reason_t *p_stop_reason_head;
    dbg_core_stop_reason_t *p_stop_reason_tail;
} dbg_core_stop_reason_queue_t;
...
typedef struct dbg_core_pid_info_t
{
    ...
    uint32_t                    sba;                /*SBA of the main process executable*/
    uint32_t                    u32_freeze_count;   /**/
    char                        *p_process_name;    /*Process fpexe name*/
    ...
    /*Stop reason queue*/
    dbg_core_stop_reason_queue_t    stop_reason_queue;
...
/*Get the freeze count for the process denoted by pid
 * Invalid process returns 0xFFFFFFF*/
uint32_t
DBG_core_get_process_freeze_count(dbg_pid_t pid, dbg_core_pid_info_queue_t *p_pid_info_queue);

/*Get the process count for the process denoted by pid_info
 * Invalid process returns 0xFFFFFFFF*/
uint32_t
...
/*Allocate stop reason information*/
dbg_core_stop_reason_t *
DBG_core_stop_reason_alloc(dbg_pid_t pid,
                           dbg_tid_t tid,
                           dbg_stop_reason_t stop_reason,
                           DBG_BPT_WPT_TYPE type, uint32_t u32_addr);

/*Deallocate stop reason information*/
void
DBG_core_stop_reason_dealloc(dbg_core_stop_reason_t *p_stop_reason);

/*enqueue stop reason in the stop reason queue*/
void
DBG_core_enqueue_stop_reason(dbg_core_stop_reason_t *p_stop_reason,
                             dbg_core_stop_reason_queue_t *p_stop_reason_queue);

/*dequeue stop reason for a pid*/
dbg_core_stop_reason_t *
