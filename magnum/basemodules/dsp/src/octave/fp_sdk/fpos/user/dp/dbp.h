...
typedef enum
{
    ...
    DBP_CMD_PROCESS_GET_NAME,         //!< DBP_CMD_PROCESS_GET_NAME
    ...
    DBP_CMD_PROCESS_GET_DSO_SBA,      //!< DBP_CMD_PROCESS_GET_DSO_SBA
    DBP_CMD_PROCESS_GET_EXE_SEGMENTS, //!< DBP_CMD_PROCESS_GET_EXE_SEGMENTS
    DBP_CMD_PROCESS_GET_EXE_SBA,      //!< DBP_CMD_PROCESS_GET_EXE_SBA
    DBP_CMD_PROCESS_GET_DSO_INFO,

    /*Commands for creating core dumps */
    DBP_CMD_PROCESS_DUMP,
    DBP_CMD_SYSTEM_DUMP,

    DBP_CMD_LAST_COMMAND = DBP_CMD_SYSTEM_DUMP,
...
typedef enum
{
    ...
    DBP_ERR_LAST_ERROR = DBP_ERR_BAD_COMMAND
...
{
    uint16_t pid;               // The pid for which the list is required
} dbp_process_get_name_cmd_t;

typedef struct __packed
{
    uint16_t pid;              // The pid from which the list was obtained
    char     process_name[];   // Sequence of zero-terminated PROCESS NAME string
} dbp_process_get_name_rsp_t;
...
typedef struct __packed
{
    uint32_t argc;      // Number of arguments (first is path to program)
...
typedef struct __packed
{
    ...
    uint16_t tids[];            // List of tids for the process
...
typedef struct __packed
{
    ...
    uint32_t reg_offset;        // Offset of the register in the TX_fullContext structure
    uint32_t reg_size;          // Size of the register to be read
...
typedef struct __packed
{
    ...
    uint32_t reg_size;          // Size of the register
    uint8_t  value[];           // Value of the register
...
typedef struct __packed
{
    uint16_t           pid;             // the pid of the process we wrote
    uint16_t           tid;             // the tid of the thread we wrote
    TX_fullContext     context;         // Thread context (CS_regContext + predicates + others)
...
typedef struct __packed
{
    uint16_t   pid;         // The pid for the process
    uint32_t   index;       // Index of DSO to be queried
} dbp_process_get_dso_sba_cmd_t;

typedef struct __packed
{
    uint16_t   pid;         // The pid for the process
    uint32_t   index;       // Index of the DSO queried
    uint32_t   sba;         // Absolute SBA address
} dbp_process_get_dso_sba_rsp_t;
...
{
    uint16_t   pid;         // The pid for the process
} dbp_process_get_exe_sba_cmd_t;

typedef struct __packed
{
    uint16_t   pid;                 // The pid for the process
    uint32_t   sba;                 // SBA for the process executable
} dbp_process_get_exe_sba_rsp_t;
...
typedef struct __packed
{
    uint16_t pid;               // The pid of the process to dump
    uint16_t tid;               // the tid of the thread context to capture
} dbp_process_dump_cmd_t;

typedef struct __packed
{
    uint16_t pid;               // The pid of the process to dump
    uint16_t tid;               // the tid of the thread context to captur
} dbp_process_dump_rsp_t;
...
typedef struct __packed
{
    uint16_t pid;               // The pid of the process owning the thread
    uint16_t tid;               // the tid of the thread context to capture
} dbp_system_dump_cmd_t;

typedef struct __packed
{
    uint16_t pid;               // The pid of the process
    uint16_t tid;               // the tid of the thread context
} dbp_system_dump_rsp_t;
...
typedef struct __packed
{
    ...
    union
    {
        ...
        dbp_process_get_name_cmd_t          process_get_name;
        ...
        dbp_process_get_dso_sba_cmd_t       process_get_dso_sba;
        dbp_process_get_exe_segments_cmd_t  process_get_exe_segments;
        dbp_process_get_exe_sba_cmd_t       process_get_exe_sba;
        dbp_process_get_dso_info_cmd_t      process_get_dso_info;
        dbp_process_dump_cmd_t              process_dump;
        ...
        dbp_system_dump_cmd_t               system_dump;
    };
} dbp_command_t;
...
typedef struct
{
    ...
    union
    {
        ...
        dbp_process_get_name_rsp_t          process_get_name;
        ...
        dbp_process_get_dso_sba_rsp_t       process_get_dso_sba;
        dbp_process_get_exe_segments_rsp_t  process_get_exe_segments;
        dbp_process_get_exe_sba_rsp_t       process_get_exe_sba;
        dbp_process_get_dso_info_rsp_t      process_get_dso_info;
        dbp_process_dump_rsp_t              process_dump;
        ...
        dbp_system_dump_rsp_t               system_dump;
    };
} dbp_response_t;
