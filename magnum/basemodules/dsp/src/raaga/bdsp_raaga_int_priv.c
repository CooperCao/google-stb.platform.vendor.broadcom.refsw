    ...
    )
{
    ...
    BDBG_MSG(("BDSP_P_DSP2HostAsyn_isr: Unlicensed Algo (%s) interrupt occured for Task %d",
            AlgoIdEnum2Char[pUnlicensedAlgoInfo->ui32AudioAlgorithm],
            pRaagaTask->taskId));
 ...
 ***************************************************************************/
 ...
 )
 {
    ...
    for(i = 0; i < BDSP_RAAGA_MAX_FW_TASK_PER_DSP; i++)
    {
        ...
        if((void *)pRaagaTask->taskMemGrants.sTaskQueue.sAsyncMsgBufmem.pAddr == NULL)
        {
            continue;
        }
        ...
        if (ui32ASyncTaskIds & (1 << pRaagaTask->taskId))
        {
            ...
            pEventMsg = pRaagaTask->taskMemGrants.sTaskQueue.sAsyncMsgBufmem.pAddr;
