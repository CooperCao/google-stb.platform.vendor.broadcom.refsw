...
DBG_core_print_dbp_response(dbp_response_t *p_dbp_resp)
{
    ...
    DSPLOG_INFO("response: 0x%x", p_dbp_resp->response);
    DSPLOG_INFO("status : 0x%x", p_dbp_resp->status);
 ...
 * FIXME: Should be made to retrieve all pids for any core
 *
 * @param   p_dbg_server    Pointer to the debug server*/
DBG_DBP_ERR_CODE
 ...
 *
 * Read a register denoted by u16_regnum from a thread tid in a
 * process pid
 *
 * @param   p_dbg_server    Pointer to the debug server
 * @param   pid             process-id
...
{
    DSPLOG_INFO("dbp_thread_read_register(): read register 0x%x for p0x%x.0x%x", u16_regnum, pid, tid);
    ...
    dbp_thread_read_register.thread_read_register.regnum = u16_regnum;
...
DBG_handle_get_pid_list_resp(dbp_response_t *p_dbp_response, DBG* dbg)
{
    ...
    DSPLOG_INFO("Get pid list info received");
    p_dbg_server->u32_resp_flag = DBP_CMD_GET_PID_LIST;
#endif
}
...
#ifdef DSPCONTROL_DBG_TEST
    DSPLOG_INFO("Get pid state received");
    p_dbg_server->u32_resp_flag = DBP_CMD_PROCESS_GET_STATE;
#endif
...
#ifdef DSPCONTROL_DBG_TEST
    DSPLOG_INFO("Process freeze received");
    p_dbg_server->u32_resp_flag = DBP_CMD_PROCESS_FREEZE;
#endif
...
DBG_handle_process_terminate_resp(dbp_response_t *p_dbp_response, DBG *dbg)
{
    ...
    if(p_dbp_response->response & DBP_PACK)
    {
        ...
        bool b_kill_status = DBG_core_process_kill_pending(
                                p_dbp_response->process_terminate.pid,
                                &p_dbg_server->pid_info_queue);
        if(!b_kill_status)
...
DBG_handle_thread_read_register_resp(dbp_response_t *p_dbp_response, DBG *dbg)
{
    ...
    DBG_core_gdb_thread_read_register_resp(dbg, p_dbp_response->thread_read_register.value, b_err_status);
}
...
DBG_handle_thread_read_all_registers_resp(dbp_response_t *p_dbp_response, DBG *dbg)
{
    ...
    if(p_dbp_response->response & DBP_PACK)
    {
        ...
        DBG_core_gdb_thread_read_all_registers_resp(dbg,
                ...
                p_dbp_response->thread_read_all_registers.predicates, true);

    }
    else
    {
        DSPLOG_ERROR("DBP_CMD_THREAD_READ_ALL_REGISTERS NACK - error status 0x%x", p_dbp_response->status);
        DBG_core_gdb_thread_read_all_registers_resp(dbg, NULL, NULL, false);
    }
...
DBG_handle_breakpoint_hit(dbp_response_t *p_dbp_response, DBG *dbg)
{
    ...
    p_dbg_server->cur_ptid_info.pid = p_dbp_response->breakpoint_hit.pid;
    ...
    DBG_core_gdb_breakpoint_hit(dbg, p_dbp_response->breakpoint_hit.pid,
...
DBG_handle_single_step_complete(dbp_response_t *p_dbp_response, DBG *dbg)
{
    ...
    /*Switch this thread as a current thread*/
    p_dbg_server->cur_ptid_info.pid = p_dbp_response->single_step_complete.pid;
    p_dbg_server->cur_ptid_info.tid = p_dbp_response->single_step_complete.tid;

    /*Make cur_ptid point to the thread where the breakpoint is hit*/
    DBG_core_gdb_single_step_complete(dbg, p_dbp_response->single_step_complete.pid,
...
DBG_handle_watchpoint_hit(dbp_response_t *p_dbp_response, DBG *dbg)
{
    ...
    /*Switch this thread as a current thread*/
    p_dbg_server->cur_ptid_info.pid = p_dbp_response->watchpoint_hit.pid;
...
DBG_handle_resp_process_exited(dbp_response_t *p_dbp_response, DBG *dbg)
{
    ...
    {
        DSPLOG_INFO("resp_process_exited(): Current process exited");
        ...
        if(p_pid_info != NULL && p_pid_info->u8_kill_pending == 1)
            DBG_core_gdb_vKill_resp(dbg);
        else
...
DBG_handle_resp_thread_created(dbp_response_t *p_dbp_response, DBG *dbg)
{
    ...
    if(p_dbg_server->b_notify_thread_events)
    {
        ...
        /*FIXME: IF the pid is not the pid under debug - then we should enqueue this response and
         * send it to gdb whenever the user switches to this process
         * */
    }
    else
    {
