...
DSP_RET DSP_raagaFindExistingRegionByPhysical(DSP *dsp, DSP_RAAGA_MEM_REGION *found, uint32_t phys_addr, uint32_t length)
{
    ...
    for(i = 0; i < dsp->mem_regions_count; i++)
    {
        ...
        {
            uint32_t phys_offset = phys_addr - region->phys_base;
            uint32_t remaining_in_region = region->size - phys_offset;
...
DSP_RET DSP_raagaMemAlloc(DSP *dsp, DSP_RAAGA_MEM_REGION *region, size_t size, unsigned alignment)
{
    ...
    DSPLOG_JUNK("DSP: allocated %d bytes virtual = %p, physical = %#x",
...
DSP_RET DSP_raagaMemFree(DSP *dsp __attribute__((unused)), DSP_RAAGA_MEM_REGION *region)
{
    ...
    {
        DSPLOG_JUNK("DSP: freeing memory block at virtual = %p, physical = %#x",
