                 ...
                 uint8_t *p_u8_receive_buffer, size_t *receive_length)
{
    ...
    if(p_dbg_target->b_channel_configured)
    {
        DSPLOG_DEBUG("DBG_target_read(): Channel configured - reading from the shared buffer");
        if(DSP_readSystemData(p_dbg_target->dsp, (void *)p_u8_receive_buffer, (uint32_t)(uintptr_t)p_dbg_target->p_u8_channel_address, ((u16_pack_len+3)>>2)<<2) != DSP_SUCCESS)
        {
    ...
    else
    {
        ...
        while(u16_bytes_read)
        {
            p_u32_buf[mailbox_base_index-1] = DSP_readSharedRegister(p_dbg_target->dsp,
                                                             (MISC_COMM_DBG_MAILBOX_BASE_ADDR(p_dbg_target->dsp, p_dbg_target->dsp_core) + mailbox_base_index * 4) );
            mailbox_base_index++;
            u16_bytes_read -= sizeof(uint32_t);
        }
    }
