...
{
    COMPILE_TIME_ASSERT(DSP_RAAGA_ATU_CONFIG_NUM_ENTRIES <= BFPSDK_RAAGA_DSP_L2C_ADDR_TRANSLATION_TABLE_COUNT);
    COMPILE_TIME_ASSERT(DSP_RAAGA_ATU_CONFIG_NUM_ENTRIES <= BFPSDK_RAAGA_DSP_L2C_PHYSICAL_40B_BASE_ADDR_COUNT);
}
...
void DSP_readAtuEntry(DSP *dsp, unsigned num, DSP_RAAGA_ATU_ENTRY *entry)
{
    ...
    raw_virtual = DSP_readShared64BitRegister(dsp, BFPSDK_RAAGA_DSP_L2C_ADDR_TRANSLATION_TABLE(num));
    raw_physical = DSP_readSharedRegister(dsp, BFPSDK_RAAGA_DSP_L2C_PHYSICAL_40B_BASE_ADDR(num));
...
void DSP_writeAtuEntry(DSP *dsp, unsigned num, const DSP_RAAGA_ATU_ENTRY *entry)
{
    ...
    DSP_writeSharedRegister(dsp, BFPSDK_RAAGA_DSP_L2C_PHYSICAL_40B_BASE_ADDR(num), raw_physical);
    DSP_writeShared64BitRegister(dsp, BFPSDK_RAAGA_DSP_L2C_ADDR_TRANSLATION_TABLE(num), raw_virtual);
}


void DSP_readAtu(DSP *dsp, DSP_RAAGA_ATU *atu)
{
    DEBUG_PRINT("DSP_readAtu: reading ATU %u entries", DSP_RAAGA_ATU_CONFIG_NUM_ENTRIES);
    {
        unsigned i;
        for(i = 0; i < DSP_RAAGA_ATU_CONFIG_NUM_ENTRIES; i++)
...
{
    DEBUG_PRINT("DSP_writeAtu: writing ATU %u entries", DSP_RAAGA_ATU_CONFIG_NUM_ENTRIES);
    {
        unsigned i;
        for(i = 0; i < DSP_RAAGA_ATU_CONFIG_NUM_ENTRIES; i++)
...

    {
        ...
        for(i = 0; i < index->entries_count; i++)
        {
               ...
               virtual_addr < entry->virtual_start + entry->length)
            {
                ...
                *physical_addr = virtual_offset + entry->physical_start;
