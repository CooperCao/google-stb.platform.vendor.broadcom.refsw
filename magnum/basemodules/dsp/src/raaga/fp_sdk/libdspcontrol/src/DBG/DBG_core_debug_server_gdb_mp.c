...
DBG_core_gdb_thread_read_all_registers_resp(DBG *dbg,
                                            ...
                                            uint8_t *p_u8_pred, bool b_err_status)
{
    DBG_core_debug_server *p_dbg_server = dbg->dbg_core.p_dbg_server;
    ...
    for(index = FP_REG_R0; index <= FP_REGS_LIMIT; index++)
    {
        ...
        if(reg_info.b_valid)
        {
            /*copy predicates from the predicate context entry*/
            if(index >= FP_REG_P0 && index <= FP_REG_P3)
            {
                memToHex(&p_u8_pred[index-FP_REG_P0], p_ch_gdb_resp_buf, reg_info.u32_cs_reg_size);
            }
            else
            {
                /*Other registers from the cs register context memory*/
...
DBG_core_gdb_handle_read_register_mp(DBG *dbg, bool b_err_status)
{
    ...
    if(tx_regnum == TX_NUM_REGISTER_IDS)
    {
        /*Return xxxx*/
        DEBUG_SERVER_GDB_BUFFER_T *p_gdb_resp_buffer = &p_dbg_server->buffer_gdb_send_pack;
        char *p_ch_gdb_resp_buf = &p_gdb_resp_buffer->p_ch_buf[1];
    ...
    else
    {
        ...
        DBG_dbp_thread_read_register(p_dbg_server, p_dbg_server->cur_ptid_info.pid,
                p_dbg_server->cur_ptid_info.tid, (uint16_t)tx_regnum);
    }
...
/*FIXME: Pass the register number if possible to the response*/
void
DBG_core_gdb_thread_read_register_resp(DBG *dbg, uint64_t u64_value, bool b_err_status)
{
    ...
    memToHex((uint8_t *)&u64_value, p_ch_gdb_resp_buf, sizeof(uint64_t));
...
DBG_core_gdb_handle_vCont(DBG *dbg)
{
    ...
    p_pid_info = DBG_core_find_pid_info(pid, &p_dbg_server->pid_info_queue);
    ...
    while(p_thread_info != NULL)
    {
        if(b_default_action_present)
            ...
            //DBG_core_set_thread_resume_action(p_thread_info, DBG_core_get_resume_action(p_resume_info[default_action_index].resume_type));
        else
            p_thread_info->resume_action = DBG_core_get_resume_action(DBG_RESUME_TYPE_STOPPED);
            //DBG_core_set_thread_resume_action(p_thread_info, DBG_core_get_resume_action(DBG_RESUME_TYPE_STOPPED));
...
{
    DSPLOG_INFO("gdb_breakpoint_hit(): p%x.%x @addr 0x%x", pid, tid, u32_address);
...
{
    DSPLOG_INFO("gdb_single_step_complete(): p%x.%x", pid, tid);

    char ch_status_response[16];
    sprintf(ch_status_response, "T05thread:p%x.%x;", pid, tid);

    DBG_dbp_thread_clear_single_step(dbg->dbg_core.p_dbg_server, pid, tid);
...
DBG_core_gdb_handle_query_packet(DBG *dbg)
{
    ...
    if(strstr(p_gdb_packet->p_ch_gdb_pack, "Supported:"))
    {
        ...
        DBG_core_gdb_send_reply(dbg, "QStartNoAckMode+;multiprocess+;vContSupported+;no-resumed+;qXfer:libraries:read+;QThreadEvents+", true);
    }
                                  ...
                                  uint32_t *seg_info, bool b_err_status)
{
     ...
     * and the size of the segment in bytes. We need the base from each pair
     */
    uint32_t i;
    for(i = 0; i<num_segments; i++)
        DSPLOG_INFO("qOffsets_resp(): segment[%d] - 0x%x", i, seg_info[2*i]);

    sprintf(ch_gdb_resp_buf, "TextSeg=%x;DataSeg=%x", seg_info[0],seg_info[2]);
