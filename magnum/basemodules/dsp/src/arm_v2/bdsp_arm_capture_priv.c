...
)
{
    ...
    for(index=0; index< pArmCapture->maxBuffers; index++)
    {
        ...
        pDescriptor->pEndPtr  = Memory.pAddr;
        pDescriptor->pWritePtr= (void *)((uint8_t *)Memory.pAddr+ pCaptureCreateSettings->channelBufferSize);
		...
		case BDSP_ConnectionType_eFmmBuffer:
            {
                ...
                for(i=0;i<numbuffers;i++)
                {
                    ...
                    pArmCapture->CapturePointerInfo[i].ui32StartWriteAddr = psOutputBuffer->ui32EndAddr + sizeof(dramaddr_t);
...
)
{
    ...
    switch(eBufferType)
    {
		case BDSP_AF_P_BufferType_eFMM:
            ...
            if((ReadAddr ^ WriteAddr) == BDSP_ARM_FMM_WRAP_MASK)/* 40 bit is wrap bit */
            {
                /* Buffer is full */
            ...
            {
				ReadAddr  &= BDSP_ARM_FMM_ADDR_MASK; /* Get LSB 39 Bits */
				WriteAddr &= BDSP_ARM_FMM_ADDR_MASK; /* Get LSB 39 Bits */
    ...
    *pSize = depth;
    pReadMemory->pAddr = (void *)((uint8_t *)pReadMemory->pAddr -((shadowRead&BDSP_ARM_FMM_ADDR_MASK)-BaseAddr));
...
)
{
    ...
    switch(eBufferType)
    {
        case BDSP_AF_P_BufferType_eFMM:
            WriteAddr = BDSP_ReadFMMReg(regHandle, pBuffer->ui32WriteAddr);
            WriteAddr  = (WriteAddr & BDSP_ARM_FMM_ADDR_MASK); /* use only the LSB 39 bits */
            shadowRead = (shadowRead & BDSP_ARM_FMM_ADDR_MASK); /* use only the LSB 39 bits */
            break;
#if 0
        case BDSP_AF_P_BufferType_eRDB:
            WriteAddr = BDSP_ReadFIFOReg(regHandle, pBuffer->ui32WriteAddr);
...
)
{
    ...
    switch(eBufferType)
    {
        case BDSP_AF_P_BufferType_eFMM:
            BaseAddr = BDSP_ReadFMMReg(regHandle, pBuffer->ui32BaseAddr);
            EndAddr  = BDSP_ReadFMMReg(regHandle, pBuffer->ui32EndAddr);
            shadowReadAddr = (shadowReadAddr & BDSP_ARM_FMM_ADDR_MASK); /* use only the LSB 39 bits */
            if(shadowReadAddr > EndAddr)
            {
                shadowReadAddr = BaseAddr+((shadowReadAddr-EndAddr)-1);
				/* Flip bit 40 on a wrap */
				shadowReadAddr ^= BDSP_ARM_FMM_WRAP_MASK;
            }
            break;
#if 0
        case BDSP_AF_P_BufferType_eRDB:
