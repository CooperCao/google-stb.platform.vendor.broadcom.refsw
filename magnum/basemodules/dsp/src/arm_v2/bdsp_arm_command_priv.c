...
)
{
    ...
    BDBG_WRN(("Soft FMM not supported"));
    sCommand.uCommand.sArmInitCommand.sSchedulingInfo.ui32SoftFMMSupported = 0;
#else
    BDBG_WRN(("Soft FMM supported"));
    sCommand.uCommand.sArmInitCommand.sSchedulingInfo.ui32SoftFMMSupported = 1;
#endif
	...
	for(i=0; i< BDSP_MAX_NUM_SCHED_LEVELS;i++)
	{
		for(; j<BDSP_P_TaskType_eLast; j++)
		{
			...
			{
				sCommand.uCommand.sArmInitCommand.sSchedulingInfo.ui32PreemptiveThreshold[i] =
	...
	BDBG_MSG(("Num Scheduling level = %d", sCommand.uCommand.sArmInitCommand.sSchedulingInfo.ui32NumSchedulingLevels));
	for(i =0; i<sCommand.uCommand.sArmInitCommand.sSchedulingInfo.ui32NumSchedulingLevels; i++)
	{
			BDBG_MSG(("Threshold level[%d] = 0x%x", i, sCommand.uCommand.sArmInitCommand.sSchedulingInfo.ui32PreemptiveThreshold[i]));
	}

	for(i=0; i<BDSP_P_TaskType_eLast; i++)
	{
		sCommand.uCommand.sArmInitCommand.sSchedulingInfo.sTaskSchedulingMatrix[i].schedulingLevel =
			pDevice->systemSchedulingInfo.sTaskSchedulingInfo[i].schedulingLevel;
		sCommand.uCommand.sArmInitCommand.sSchedulingInfo.sTaskSchedulingMatrix[i].schedulingThreshold=
...
)
{
    ...
    BDSP_P_Command sCommand;
    BDSP_P_Response sRsp;
    BDSP_P_CommandID eCommandID = BDSP_P_CommandID_SOFT_FMM_START;

    BDBG_ENTER(BDSP_Arm_P_ProcessStartFMMCommand);
    BDBG_MSG(("BDSP_Arm_P_ProcessStartFMMCommand"));
    BDBG_MSG(("\t Scheduling Mode      = %s", FirmwareSchedulingType[pPayload->eSchedulingMode]));
    BDBG_MSG(("\t Task Type            = %s", TaskType[pPayload->eTaskType]));
    BDBG_MSG(("\t Scheduling Level     = %d", pPayload->ui32SchedulingLevel));
    BDBG_MSG(("\t Task ID              = %d", pPayload->ui32TaskId));
    ...
    BDBG_MSG(("\t Async FIFO ID        = %d", pPayload->ui32AsyncQueueFifoId));
    BDBG_MSG(("\t Event Mask Enable    = 0x%x", pPayload->ui32EventEnableMask));
    BDBG_MSG(("\t Process   Memory Offset ="BDSP_MSG_FMT" Size = %d ",BDSP_MSG_ARG(pPayload->sTaskMemoryInfo.BaseAddr),pPayload->sTaskMemoryInfo.Size));

    BKNI_Memset(&sCommand, 0, sizeof(BDSP_P_Command));
    BKNI_Memset(&sRsp, 0, sizeof(BDSP_P_Response));
    pDevice = pArmSoftFMM->pDevice;

    sCommand.sCommandHeader.eCommandID              = eCommandID;
    sCommand.sCommandHeader.ui32CommandCounter      = pArmSoftFMM->ui32CommandCounter++;
    sCommand.sCommandHeader.ui32TaskID              = pPayload->ui32TaskId;
    sCommand.sCommandHeader.eResponseType           = BDSP_P_ResponseType_eRequired;
    sCommand.sCommandHeader.ui32CommandSizeInBytes  = sizeof(BDSP_P_Command);
    /*sCommand.sCommandHeader.ui32CommandTimeStamp    = BDSP_ReadReg32(pDevice->regHandle,BCHP_RAAGA_DSP_TIMERS_TSM_TIMER_VALUE);*/

    BKNI_Memcpy((void *)&sCommand.uCommand.sStartFMM, pPayload, sizeof(BDSP_P_StartFMMCommand));
    BKNI_ResetEvent(pArmSoftFMM->hEvent);

    errCode = BDSP_Arm_P_SendCommand(pDevice->hCmdQueue[0], &sCommand);
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessStartFMMCommand: Error in Writing the command for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    errCode = BKNI_WaitForEvent(pArmSoftFMM->hEvent, BDSP_EVENT_TIMEOUT_IN_MS);
    if (BERR_TIMEOUT == errCode)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessStartFMMCommand: START Soft FMM RESPONSE timeout for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }

    errCode = BDSP_Arm_P_GetResponse(pArmSoftFMM->hSyncQueue, (void *)&sRsp, sizeof(BDSP_P_Response));
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessStartFMMCommand: Unable to read response for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }

    if((sRsp.sResponseHeader.eStatus != BERR_SUCCESS)||
       (sRsp.sResponseHeader.eCommandID != eCommandID))
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessStartFMMCommand: START SOFT FMM response not received successfully!!, Status = %d, Response for CMD ID = %d",
            sRsp.sResponseHeader.eStatus, sRsp.sResponseHeader.eCommandID));
        errCode = BERR_TRACE(sRsp.sResponseHeader.eStatus);
        goto end;
    }
    ...
    BDBG_LEAVE(BDSP_Arm_P_ProcessStartFMMCommand);
    return errCode;
}
...
)
{
    ...
    BDSP_P_Command sCommand;
    BDSP_P_Response sRsp;
    BDSP_P_CommandID eCommandID = BDSP_P_CommandID_SOFT_FMM_CREATE_MIXER;

    BDBG_ENTER(BDSP_Arm_P_ProcessCreateFMMMixerCommand);
    BDBG_MSG(("BDSP_Arm_P_ProcessCreateFMMMixerCommand"));
#if 0
    BDBG_MSG(("BDSP_Arm_P_ProcessStartTaskCommand"));
    BDBG_MSG(("\t Scheduling Mode      = %s", FirmwareSchedulingType[pPayload->eSchedulingMode]));
    BDBG_MSG(("\t Task Type            = %s", TaskType[pPayload->eTaskType]));
    ...
    sCommand.sCommandHeader.eCommandID              = eCommandID;
    sCommand.sCommandHeader.ui32CommandCounter      = pArmSoftFMM->ui32CommandCounter++;
    sCommand.sCommandHeader.ui32TaskID              = 0x100;
    sCommand.sCommandHeader.eResponseType           = BDSP_P_ResponseType_eRequired;
    sCommand.sCommandHeader.ui32CommandSizeInBytes  = sizeof(BDSP_P_Command);
    /*sCommand.sCommandHeader.ui32CommandTimeStamp    = BDSP_ReadReg32(pDevice->regHandle,BCHP_RAAGA_DSP_TIMERS_TSM_TIMER_VALUE);*/

    BKNI_Memcpy((void *)&sCommand.uCommand.sCreateSoftFMMMixer, pPayload, sizeof(BDSP_P_SoftFMMCreateMixerCommand));
    BKNI_ResetEvent(pArmSoftFMM->hEvent);

    errCode = BDSP_Arm_P_SendCommand(pDevice->hCmdQueue[0], &sCommand);
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessCreateFMMMixerCommand: Error in Writing the command for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if (BERR_TIMEOUT == errCode)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessCreateFMMMixerCommand: Create SOFT FMM Mixer response timeout for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessCreateFMMMixerCommand: Unable to read response for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if((sRsp.sResponseHeader.eStatus != BERR_SUCCESS)||
       (sRsp.sResponseHeader.eCommandID != eCommandID))
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessCreateFMMMixerCommand: Create SOFT FMM Mixer response not received successfully!!, Status = %d, Response for CMD ID = %d",
            sRsp.sResponseHeader.eStatus, sRsp.sResponseHeader.eCommandID));
        errCode = BERR_TRACE(sRsp.sResponseHeader.eStatus);
        goto end;
    }
    ...
    BDBG_LEAVE(BDSP_Arm_P_ProcessCreateFMMMixerCommand);
    return errCode;
}
...
)
{
    ...
    BDSP_P_Command sCommand;
    BDSP_P_Response sRsp;
    BDSP_P_CommandID eCommandID = BDSP_P_CommandID_SOFT_FMM_START_MIXER;

    BDBG_ENTER(BDSP_Arm_P_ProcessStartFMMMixerCommand);
    BDBG_MSG(("BDSP_Arm_P_ProcessStartFMMMixerCommand"));
#if 0
    BDBG_MSG(("BDSP_Arm_P_ProcessStartTaskCommand"));
    BDBG_MSG(("\t Scheduling Mode      = %s", FirmwareSchedulingType[pPayload->eSchedulingMode]));
    BDBG_MSG(("\t Task Type            = %s", TaskType[pPayload->eTaskType]));
    ...
    BKNI_Memcpy((void *)&sCommand.uCommand.sStartSoftFMMMixer, pPayload, sizeof(BDSP_P_SoftFMMStartMixerCommand));
    BKNI_ResetEvent(pArmSoftFMM->hEvent);

    errCode = BDSP_Arm_P_SendCommand(pDevice->hCmdQueue[0], &sCommand);
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessStartFMMMixerCommand: Error in Writing the command for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if (BERR_TIMEOUT == errCode)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessStartFMMMixerCommand: START SOFT FMM Mixer response timeout for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessStartFMMMixerCommand: Unable to read response for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if((sRsp.sResponseHeader.eStatus != BERR_SUCCESS)||
       (sRsp.sResponseHeader.eCommandID != eCommandID))
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessStartFMMMixerCommand: START SOFT FMM Mixer response not received successfully!!, Status = %d, Response for CMD ID = %d",
            sRsp.sResponseHeader.eStatus, sRsp.sResponseHeader.eCommandID));
        errCode = BERR_TRACE(sRsp.sResponseHeader.eStatus);
        goto end;
    }
    ...
    BDBG_LEAVE(BDSP_Arm_P_ProcessStartFMMMixerCommand);
    return errCode;
}
...
)
{
    ...
    BDSP_P_Command sCommand;
    BDSP_P_Response sRsp;
    BDSP_P_CommandID eCommandID = BDSP_P_CommandID_SOFT_FMM_STOP_MIXER;

    BDBG_ENTER(BDSP_Arm_P_ProcessStopFMMMixerCommand);
    BDBG_MSG(("BDSP_Arm_P_ProcessStopFMMMixerCommand"));
#if 0
    BDBG_MSG(("BDSP_Arm_P_ProcessStartTaskCommand"));
    BDBG_MSG(("\t Scheduling Mode      = %s", FirmwareSchedulingType[pPayload->eSchedulingMode]));
    BDBG_MSG(("\t Task Type            = %s", TaskType[pPayload->eTaskType]));
    ...
    BKNI_Memcpy((void *)&sCommand.uCommand.sStopSoftFMMMixer, pPayload, sizeof(BDSP_P_SoftFMMStopMixerCommand));
    BKNI_ResetEvent(pArmSoftFMM->hEvent);

    errCode = BDSP_Arm_P_SendCommand(pDevice->hCmdQueue[0], &sCommand);
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessStopFMMMixerCommand: Error in Writing the command for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if (BERR_TIMEOUT == errCode)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessStopFMMMixerCommand: SOFT FMM Mixer Stop response timeout for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessStopFMMMixerCommand: Unable to read response for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if((sRsp.sResponseHeader.eStatus != BERR_SUCCESS)||
       (sRsp.sResponseHeader.eCommandID != eCommandID))
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessStopFMMMixerCommand: SOFT FMM Mixer Stop response not received successfully!!, Status = %d, Response for CMD ID = %d",
            sRsp.sResponseHeader.eStatus, sRsp.sResponseHeader.eCommandID));
        errCode = BERR_TRACE(sRsp.sResponseHeader.eStatus);
        goto end;
    }
    ...
    BDBG_LEAVE(BDSP_Arm_P_ProcessStopFMMMixerCommand);
    return errCode;
}
...
)
{
    ...
    BDSP_P_Command sCommand;
    BDSP_P_Response sRsp;
    BDSP_P_CommandID eCommandID = BDSP_P_CommandID_SOFT_FMM_ADD_INPUT;

    BDBG_ENTER(BDSP_Arm_P_ProcessAddSoftFMMInputCommand);
    BDBG_MSG(("BDSP_Arm_P_ProcessAddSoftFMMInputCommand"));
#if 0
    BDBG_MSG(("BDSP_Arm_P_ProcessStartTaskCommand"));
    BDBG_MSG(("\t Scheduling Mode      = %s", FirmwareSchedulingType[pPayload->eSchedulingMode]));
    BDBG_MSG(("\t Task Type            = %s", TaskType[pPayload->eTaskType]));
    ...
    BKNI_Memcpy((void *)&sCommand.uCommand.sAddSoftFMMInput, pPayload, sizeof(BDSP_P_SoftFMMAddInputCommand));
    BKNI_ResetEvent(pArmSoftFMM->hEvent);

    errCode = BDSP_Arm_P_SendCommand(pDevice->hCmdQueue[0], &sCommand);
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessAddSoftFMMInputCommand: Error in Writing the command for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if (BERR_TIMEOUT == errCode)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessAddSoftFMMInputCommand: Soft FMM add input response timeout for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessAddSoftFMMInputCommand: Unable to read response for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if((sRsp.sResponseHeader.eStatus != BERR_SUCCESS)||
       (sRsp.sResponseHeader.eCommandID != eCommandID))
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessAddSoftFMMInputCommand: Soft FMM add input response not received successfully!!, Status = %d, Response for CMD ID = %d",
            sRsp.sResponseHeader.eStatus, sRsp.sResponseHeader.eCommandID));
        errCode = BERR_TRACE(sRsp.sResponseHeader.eStatus);
        goto end;
    }
    ...
    BDBG_LEAVE(BDSP_Arm_P_ProcessAddSoftFMMInputCommand);
    return errCode;
}
...
)
{
    ...
    BDSP_P_Command sCommand;
    BDSP_P_Response sRsp;
    BDSP_P_CommandID eCommandID = BDSP_P_CommandID_SOFT_FMM_ADD_OUTPUT;

    BDBG_ENTER(BDSP_Arm_P_ProcessAddSoftFMMOutputCommand);
    BDBG_MSG(("BDSP_Arm_P_ProcessAddSoftFMMOutputCommand, command id: 0x%x",eCommandID));
#if 0
    BDBG_MSG(("BDSP_Arm_P_ProcessStartTaskCommand"));
    BDBG_MSG(("\t Scheduling Mode      = %s", FirmwareSchedulingType[pPayload->eSchedulingMode]));
    BDBG_MSG(("\t Task Type            = %s", TaskType[pPayload->eTaskType]));
    ...
    BKNI_Memcpy((void *)&sCommand.uCommand.sAddSoftFMMOutput, pPayload, sizeof(BDSP_P_SoftFMMAddOutputCommand));
    BKNI_ResetEvent(pArmSoftFMM->hEvent);

    errCode = BDSP_Arm_P_SendCommand(pDevice->hCmdQueue[0], &sCommand);
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessAddSoftFMMOutputCommand: Error in Writing the command for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if (BERR_TIMEOUT == errCode)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessAddSoftFMMOutputCommand: Soft FMM add output response timeout for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessAddSoftFMMOutputCommand: Unable to read response for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if((sRsp.sResponseHeader.eStatus != BERR_SUCCESS)||
       (sRsp.sResponseHeader.eCommandID != eCommandID))
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessAddSoftFMMOutputCommand: Soft FMM add output response not received successfully!!, Status = %d, Response for CMD ID = %d",
            sRsp.sResponseHeader.eStatus, sRsp.sResponseHeader.eCommandID));
        errCode = BERR_TRACE(sRsp.sResponseHeader.eStatus);
        goto end;
    }
    ...
    BDBG_LEAVE(BDSP_Arm_P_ProcessAddSoftFMMOutputCommand);
    return errCode;
}
...
)
{
    ...
    BDSP_P_Command sCommand;
    BDSP_P_Response sRsp;
    BDSP_P_CommandID eCommandID = BDSP_P_CommandID_SOFT_FMM_REMOVE_INPUT;

    BDBG_ENTER(BDSP_Arm_P_ProcessRemoveSoftFMMInputCommand);
    BDBG_MSG(("BDSP_Arm_P_ProcessRemoveSoftFMMInputCommand"));
#if 0
    BDBG_MSG(("BDSP_Arm_P_ProcessStartTaskCommand"));
    BDBG_MSG(("\t Scheduling Mode      = %s", FirmwareSchedulingType[pPayload->eSchedulingMode]));
    BDBG_MSG(("\t Task Type            = %s", TaskType[pPayload->eTaskType]));
    ...
    BKNI_Memcpy((void *)&sCommand.uCommand.sRemoveSoftFMMInput, pPayload, sizeof(BDSP_P_SoftFMMRemoveInputCommand));
    BKNI_ResetEvent(pArmSoftFMM->hEvent);

    errCode = BDSP_Arm_P_SendCommand(pDevice->hCmdQueue[0], &sCommand);
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessRemoveSoftFMMInputCommand: Error in Writing the command for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if (BERR_TIMEOUT == errCode)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessRemoveSoftFMMInputCommand: Soft FMM remove input response timeout for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessRemoveSoftFMMInputCommand: Unable to read response for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if((sRsp.sResponseHeader.eStatus != BERR_SUCCESS)||
       (sRsp.sResponseHeader.eCommandID != eCommandID))
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessRemoveSoftFMMInputCommand: Soft FMM remove input response not received successfully!!, Status = %d, Response for CMD ID = %d",
            sRsp.sResponseHeader.eStatus, sRsp.sResponseHeader.eCommandID));
        errCode = BERR_TRACE(sRsp.sResponseHeader.eStatus);
        goto end;
    }
    ...
    BDBG_LEAVE(BDSP_Arm_P_ProcessRemoveSoftFMMInputCommand);
    return errCode;
}
...
)
{
    ...
    BDSP_P_Command sCommand;
    BDSP_P_Response sRsp;
    BDSP_P_CommandID eCommandID = BDSP_P_CommandID_SOFT_FMM_REMOVE_OUTPUT;

    BDBG_ENTER(BDSP_Arm_P_ProcessRemoveSoftFMMOutputCommand);
    BDBG_MSG(("BDSP_Arm_P_ProcessRemoveSoftFMMOutputCommand"));
#if 0
    BDBG_MSG(("BDSP_Arm_P_ProcessStartTaskCommand"));
    BDBG_MSG(("\t Scheduling Mode      = %s", FirmwareSchedulingType[pPayload->eSchedulingMode]));
    BDBG_MSG(("\t Task Type            = %s", TaskType[pPayload->eTaskType]));
    ...
    BKNI_Memcpy((void *)&sCommand.uCommand.sRemoveSoftFMMOutput, pPayload, sizeof(BDSP_P_SoftFMMRemoveOutputCommand));
    BKNI_ResetEvent(pArmSoftFMM->hEvent);

    errCode = BDSP_Arm_P_SendCommand(pDevice->hCmdQueue[0], &sCommand);
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessRemoveSoftFMMOutputCommand: Error in Writing the command for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if (BERR_TIMEOUT == errCode)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessRemoveSoftFMMOutputCommand: Soft FMM remove output response timeout for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessRemoveSoftFMMOutputCommand: Unable to read response for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if((sRsp.sResponseHeader.eStatus != BERR_SUCCESS)||
       (sRsp.sResponseHeader.eCommandID != eCommandID))
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessRemoveSoftFMMOutputCommand: Soft FMM remove output response not received successfully!!, Status = %d, Response for CMD ID = %d",
            sRsp.sResponseHeader.eStatus, sRsp.sResponseHeader.eCommandID));
        errCode = BERR_TRACE(sRsp.sResponseHeader.eStatus);
        goto end;
    }
    ...
    BDBG_LEAVE(BDSP_Arm_P_ProcessRemoveSoftFMMOutputCommand);
    return errCode;
}
...
)
{
    ...
    BDSP_P_Command sCommand;
    BDSP_P_Response sRsp;
    BDSP_P_CommandID eCommandID = BDSP_P_CommandID_SOFT_FMM_MIXER_RECONFIG;

    BDBG_ENTER(BDSP_Arm_P_ProcessSoftFMMMixerReconfigCommand);
    BDBG_MSG(("BDSP_Arm_P_ProcessSoftFMMMixerReconfigCommand"));
#if 0
    BDBG_MSG(("BDSP_Arm_P_ProcessStartTaskCommand"));
    BDBG_MSG(("\t Scheduling Mode      = %s", FirmwareSchedulingType[pPayload->eSchedulingMode]));
    BDBG_MSG(("\t Task Type            = %s", TaskType[pPayload->eTaskType]));
    ...
    BKNI_Memcpy((void *)&sCommand.uCommand.sSoftFMMMixerReconfig, pPayload, sizeof(BDSP_P_SoftFMMMixerReConfigCommand));
    BKNI_ResetEvent(pArmSoftFMM->hEvent);

    errCode = BDSP_Arm_P_SendCommand(pDevice->hCmdQueue[0], &sCommand);
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessSoftFMMMixerReconfigCommand: Error in Writing the command for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if (BERR_TIMEOUT == errCode)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessSoftFMMMixerReconfigCommand: Soft FMM Mixer Reconfig response timeout for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessSoftFMMMixerReconfigCommand: Unable to read response for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if((sRsp.sResponseHeader.eStatus != BERR_SUCCESS)||
       (sRsp.sResponseHeader.eCommandID != eCommandID))
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessSoftFMMMixerReconfigCommand: Soft FMM Mixer Reconfig response not received successfully!!, Status = %d, Response for CMD ID = %d",
            sRsp.sResponseHeader.eStatus, sRsp.sResponseHeader.eCommandID));
        errCode = BERR_TRACE(sRsp.sResponseHeader.eStatus);
        goto end;
    }
    ...
    BDBG_LEAVE(BDSP_Arm_P_ProcessSoftFMMMixerReconfigCommand);
    return errCode;
}
...
)
{
    ...
    BDBG_ENTER(BDSP_Arm_P_ProcessSoftFMMMixerReconfigCommand_isr);
    BKNI_Memset(&sCommand, 0, sizeof(BDSP_P_Command));
    pDevice = pArmSoftFMM->pDevice;
    ...
    errCode = BDSP_Arm_P_SendCommand_isr(pDevice->hCmdQueue[0], &sCommand);
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessSoftFMMMixerReconfigCommand_isr: Error in Writing the command for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }

end:
    BDBG_LEAVE(BDSP_Arm_P_ProcessSoftFMMMixerReconfigCommand_isr);
    return errCode;
}
...
)
{
    ...
    BDSP_P_Command sCommand;
    BDSP_P_Response sRsp;
    BDSP_P_CommandID eCommandID = BDSP_P_CommandID_SOFT_FMM_INPUT_RECONFIG;

    BDBG_ENTER(BDSP_Arm_P_ProcessSoftFMMInputReconfigCommand);
    BDBG_MSG(("BDSP_Arm_P_ProcessSoftFMMInputReconfigCommand"));
#if 0
    BDBG_MSG(("BDSP_Arm_P_ProcessStartTaskCommand"));
    BDBG_MSG(("\t Scheduling Mode      = %s", FirmwareSchedulingType[pPayload->eSchedulingMode]));
    BDBG_MSG(("\t Task Type            = %s", TaskType[pPayload->eTaskType]));
    ...
    BKNI_Memcpy((void *)&sCommand.uCommand.sSoftFMMInputReconfig, pPayload, sizeof(BDSP_P_SoftFMMInputReConfigCommand));
    BKNI_ResetEvent(pArmSoftFMM->hEvent);

    errCode = BDSP_Arm_P_SendCommand(pDevice->hCmdQueue[0], &sCommand);
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessSoftFMMInputReconfigCommand: Error in Writing the command for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if (BERR_TIMEOUT == errCode)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessSoftFMMInputReconfigCommand: Soft FMM Input Reconfig response timeout for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessSoftFMMInputReconfigCommand: Unable to read response for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if((sRsp.sResponseHeader.eStatus != BERR_SUCCESS)||
       (sRsp.sResponseHeader.eCommandID != eCommandID))
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessSoftFMMInputReconfigCommand: Soft FMM Input Reconfig response not received successfully!!, Status = %d, Response for CMD ID = %d",
            sRsp.sResponseHeader.eStatus, sRsp.sResponseHeader.eCommandID));
        errCode = BERR_TRACE(sRsp.sResponseHeader.eStatus);
        goto end;
    }
    ...
    BDBG_LEAVE(BDSP_Arm_P_ProcessSoftFMMInputReconfigCommand);
    return errCode;
}
...
)
{
    ...
    BDBG_ENTER(BDSP_Arm_P_ProcessSoftFMMInputReconfigCommand_isr);
    BKNI_Memset(&sCommand, 0, sizeof(BDSP_P_Command));
    pDevice = pArmSoftFMM->pDevice;
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessSoftFMMInputReconfigCommand_isr: Error in Writing the command for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }

end:
    BDBG_LEAVE(BDSP_Arm_P_ProcessSoftFMMInputReconfigCommand_isr);
    return errCode;
}
...
)
{
    ...
    BDSP_P_Command sCommand;
    BDSP_P_Response sRsp;
    BDSP_P_CommandID eCommandID = BDSP_P_CommandID_SOFT_FMM_OUTPUT_RECONFIG;

    BDBG_ENTER(BDSP_Arm_P_ProcessSoftFMMOutputReconfigCommand);
    BDBG_MSG(("BDSP_Arm_P_ProcessSoftFMMOutputReconfigCommand"));
#if 0
    BDBG_MSG(("BDSP_Arm_P_ProcessStartTaskCommand"));
    BDBG_MSG(("\t Scheduling Mode      = %s", FirmwareSchedulingType[pPayload->eSchedulingMode]));
    BDBG_MSG(("\t Task Type            = %s", TaskType[pPayload->eTaskType]));
    ...
    BKNI_Memcpy((void *)&sCommand.uCommand.sSoftFMMOutputReconfig, pPayload, sizeof(BDSP_P_SoftFMMOutputReConfigCommand));
    BKNI_ResetEvent(pArmSoftFMM->hEvent);

    errCode = BDSP_Arm_P_SendCommand(pDevice->hCmdQueue[0], &sCommand);
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessSoftFMMOutputReconfigCommand: Error in Writing the command for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if (BERR_TIMEOUT == errCode)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessSoftFMMOutputReconfigCommand: SOFT FMM Output Reconfig RESPONSE timeout for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessSoftFMMOutputReconfigCommand: Unable to read response for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }
    ...
    if((sRsp.sResponseHeader.eStatus != BERR_SUCCESS)||
       (sRsp.sResponseHeader.eCommandID != eCommandID))
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessSoftFMMOutputReconfigCommand: SOFT FMM Output Reconfig response not received successfully!!, Status = %d, Response for CMD ID = %d",
            sRsp.sResponseHeader.eStatus, sRsp.sResponseHeader.eCommandID));
        errCode = BERR_TRACE(sRsp.sResponseHeader.eStatus);
        goto end;
    }
    ...
    BDBG_LEAVE(BDSP_Arm_P_ProcessSoftFMMOutputReconfigCommand);
    return errCode;
}
...
)
{
    ...
    BDBG_ENTER(BDSP_Arm_P_ProcessSoftFMMOutputReconfigCommand_isr);
    BKNI_Memset(&sCommand, 0, sizeof(BDSP_P_Command));
    pDevice = pArmSoftFMM->pDevice;
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ProcessSoftFMMOutputReconfigCommand_isr: Error in Writing the command for Soft FMM (%p)",(void *)pArmSoftFMM));
        errCode = BERR_TRACE(errCode);
        goto end;
    }

end:
    BDBG_LEAVE(BDSP_Arm_P_ProcessSoftFMMOutputReconfigCommand_isr);
    return errCode;
}
