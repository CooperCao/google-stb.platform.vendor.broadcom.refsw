...
static void BDSP_Arm_P_Device_Memory_details(
	BDSP_Arm_P_DeviceMemoryInfo *pMemInfo,
	unsigned numDsp
)
{
	unsigned i = 0, size = 0, j = 0, k = 0;
	BDBG_ENTER(BDSP_Arm_P_Device_Memory_details);
	BDBG_MSG(("-------------------------------------------- "));
	...
	for(i=0; i<numDsp; i++)
	{
		...
		BDBG_MSG(("\tIO MEMORY RANGE\t["BDSP_MSG_FMT":"BDSP_MSG_FMT"] Allocated = %d \t Used = %d",
			BDSP_MSG_ARG(pMemInfo->sIOMemoryPool[i].Memory.offset),
			BDSP_MSG_ARG(pMemInfo->sIOMemoryPool[i].Memory.offset+ pMemInfo->sROMemoryPool.ui32Size),
			pMemInfo->sIOMemoryPool[i].ui32Size,
			pMemInfo->sIOMemoryPool[i].ui32UsedSize));
		BDBG_MSG(("-------------------------------------------- "));
		...
		BDBG_MSG(("\t \tCFG Register RANGE\t["BDSP_MSG_FMT":"BDSP_MSG_FMT"] Size =%d", BDSP_MSG_ARG(pMemInfo->CfgRegisters[i].Buffer.offset),
			BDSP_MSG_ARG(pMemInfo->CfgRegisters[i].Buffer.offset+pMemInfo->CfgRegisters[i].ui32Size),
			pMemInfo->CfgRegisters[i].ui32Size));
		BDBG_MSG(("\t \tSOFT FIFO RANGE\t\t["BDSP_MSG_FMT":"BDSP_MSG_FMT"] Size =%d", BDSP_MSG_ARG(pMemInfo->softFifo[i].Buffer.offset),
			BDSP_MSG_ARG(pMemInfo->softFifo[i].Buffer.offset+pMemInfo->softFifo[i].ui32Size),
			pMemInfo->softFifo[i].ui32Size));
		...
		{
			BDBG_MSG(("\t \tWORKBUFFER[Level %d] RANGE\t["BDSP_MSG_FMT":"BDSP_MSG_FMT"] Size =%d", k,
				BDSP_MSG_ARG(pMemInfo->WorkBufferMemory[i][k].Buffer.offset),
				BDSP_MSG_ARG(pMemInfo->WorkBufferMemory[i][k].Buffer.offset+pMemInfo->WorkBufferMemory[i][k].ui32Size),
				pMemInfo->WorkBufferMemory[i][k].ui32Size));
		}
		BDBG_MSG(("-------------------------------------------- "));
	}
	BDBG_LEAVE(BDSP_Arm_P_Device_Memory_details);
}

static void BDSP_Arm_P_Device_Generic_details(
	BDSP_Arm  *pDevice
)
{
	unsigned index = 0;
	...
	BDBG_ENTER(BDSP_Arm_P_Device_Generic_details);
	/*pBoxConfig = (BBOX_Config *)BKNI_Malloc(sizeof(BBOX_Config));
    if(pBoxConfig == NULL)
    {
        BDBG_ERR(("BDSP_Arm_P_Device_Generic_details: Unable to allocate memory for BOX Config"));
	...
	if(BERR_SUCCESS != ret)
	{
		BDBG_ERR(("BDSP_Arm_P_Device_Generic_details: Error in retrieving the Num DSP from BOX MODE"));
		goto exit;
	}
...
/*exit:*/
	BDBG_LEAVE(BDSP_Arm_P_Device_Generic_details);
...
{
	BDBG_ENTER(BDSP_Arm_P_Device_Diagnostics);
	BDBG_MSG(("============================================ "));
	BDBG_MSG(("\t HOST DIAGNOSTIC INFORMATION "));
	BDBG_MSG(("-------------------------------------------- "));
	BDBG_MSG(("\t SYSTEM INFORMATION "));
	/*BDBG_MSG(("\t LICENSE DETAILS T.B.D"));*/
	BDSP_Arm_P_Device_Generic_details(pDevice);
	BDBG_MSG(("-------------------------------------------- "));
	BDSP_Arm_P_Device_Memory_details(&pDevice->memInfo, pDevice->numDsp);
	BDBG_MSG(("============================================ "));
	BDBG_LEAVE(BDSP_Arm_P_Device_Diagnostics);
}
...
)
{
	...
	BDBG_ENTER(BDSP_Arm_P_Stage_PortDiagnostics);
		...
		if(pStageConnectionDetails->eValid)
		{
			switch(pStageConnectionDetails->eConnectionType)
			{
				...
				case BDSP_ConnectionType_eRaveBuffer:
					...
					base = BDSP_ReadReg64(pArmStage->pContext->pDevice->regHandle,pRaveContextMap->CDB_Base);
					end  = BDSP_ReadReg64(pArmStage->pContext->pDevice->regHandle,pRaveContextMap->CDB_End);
					size = end - base + 1;
					BDBG_MSG(("\t \tCDB BUFFER  RANGE \t["BDSP_MSG_FMT":"BDSP_MSG_FMT"] Size = %d",	BDSP_MSG_ARG(base),
					BDSP_MSG_ARG(end),
					size));
					base = BDSP_ReadReg64(pArmStage->pContext->pDevice->regHandle,pRaveContextMap->ITB_Base);
					end  = BDSP_ReadReg64(pArmStage->pContext->pDevice->regHandle,pRaveContextMap->ITB_End);
					size = end - base + 1;
				...
				case BDSP_ConnectionType_eFmmBuffer:
					...
					{
						base = BDSP_ReadFMMReg(pArmStage->pContext->pDevice->regHandle,pFMMDescriptor->buffers[index].base);
						end  = BDSP_ReadFMMReg(pArmStage->pContext->pDevice->regHandle,pFMMDescriptor->buffers[index].end);
						size = end - base + 1;
                ...
                case BDSP_ConnectionType_eRDBBuffer:
                    /*pQueueBuffer = (BDSP_ArmQueue *)pStageConnectionDetails->connectionHandle.rdb.pQHandle->pQueueHandle;
	...
	BDBG_LEAVE(BDSP_Arm_P_Stage_PortDiagnostics);
}
