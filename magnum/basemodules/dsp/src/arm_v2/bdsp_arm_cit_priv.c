            ...
            for(index=0; index<BDSP_AF_P_MAX_OP_FORKS; index++)
            {
                    ...
                    (pArmConnectStage->sStageConnectionInfo.sStageOutput[index].eValid == BDSP_AF_P_eValid))
                {
                    pSchedulingConfig->eBufferType = BDSP_AF_P_BufferType_eDRAM;
                    {
                        ...
                        BDSP_AF_P_sCIRCULAR_BUFFER *psCircularBuffer;
                        ...
                        psCircularBuffer = (BDSP_AF_P_sCIRCULAR_BUFFER *)(pInterTaskBuffer->bufferDescriptorAddr[BDSP_AF_P_PortBuffer_Type_Data][0]);

                        pSchedulingConfig->sExtendedBuffer.sCircularBuffer.BaseAddr  = (dramaddr_t)&(psCircularBuffer->BaseAddr);
                        pSchedulingConfig->sExtendedBuffer.sCircularBuffer.EndAddr   = (dramaddr_t)&(psCircularBuffer->EndAddr);
                        pSchedulingConfig->sExtendedBuffer.sCircularBuffer.ReadAddr  = (dramaddr_t)&(psCircularBuffer->ReadAddr);
                        pSchedulingConfig->sExtendedBuffer.sCircularBuffer.WriteAddr = (dramaddr_t)&(psCircularBuffer->WriteAddr);
                        pSchedulingConfig->sExtendedBuffer.sCircularBuffer.WrapAddr  = (dramaddr_t)&(psCircularBuffer->WrapAddr);
                    }
                    independentDelay = 0;
                    goto next_step_2;
                }
