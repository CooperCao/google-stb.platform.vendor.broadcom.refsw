...
static BERR_Code BDSP_Arm_P_WriteMsg_isr(
...
)
{
    ...
    dramaddr_t  *pWriteAddr, *pReadAddr;
    void *pSrc = NULL;
    ...
    pWriteAddr = (dramaddr_t  *)((uint8_t *)hMsgQueue->FifoMemory.pAddr+(2*sizeof(dramaddr_t)));;
    pReadAddr  =  (dramaddr_t *)((uint8_t *)pWriteAddr+(sizeof(dramaddr_t)));
    ReadOffset  = *pReadAddr;
    WriteOffset = *pWriteAddr;

    BDBG_MSG(("BDSP_Arm_P_WriteMsg_isr, Before Write: Write Offset="BDSP_MSG_FMT",Read Offset="BDSP_MSG_FMT,
    ...
    {
        BDBG_ERR(("BDSP_Arm_P_WriteMsg_isr: No space available to write the Message, Freespace (%d) MsgSize (%d)",uiFreeSpace,uiMsgSize));
        BDBG_ASSERT(0);
    }
    ...
    {
        BDBG_ERR(("BDSP_Arm_P_WriteMsg_isr: Error in Writing Chunk 1"));
        BDBG_ASSERT(0);
    }
    ...
    if(uiChunk2)
    {
        ...
        if(errCode != BERR_SUCCESS)
        {
            BDBG_ERR(("BDSP_Arm_P_WriteMsg_isr: Error in Writing Chunk 2"));
            BDBG_ASSERT(0);
        }
        WriteOffset = WriteOffset+uiChunk2;
    }

    *pWriteAddr = WriteOffset;
    hMsgQueue->Address.WriteOffset = WriteOffset;

    Memory = hMsgQueue->FifoMemory;
    Memory.pAddr = (void *)pWriteAddr;
    BDSP_MMA_P_FlushCache(Memory,sizeof(dramaddr_t));

    BDBG_MSG(("BDSP_Arm_P_WriteMsg_isr, After Write: Write Offset="BDSP_MSG_FMT",Read Offset="BDSP_MSG_FMT,
        BDSP_MSG_ARG(WriteOffset),BDSP_MSG_ARG(ReadOffset)));
...
)
{
    ...
    BDBG_ENTER(BDSP_Arm_P_ReadMsg_isr);
    BDBG_ASSERT(hMsgQueue);
    BDBG_ASSERT(pMsg);
    ...
    BDBG_MSG(("BDSP_Arm_P_ReadMsg_isr: Before Read, Write Offset="BDSP_MSG_FMT",Read Offset="BDSP_MSG_FMT,
        BDSP_MSG_ARG(WriteOffset),BDSP_MSG_ARG(ReadOffset)));
    ...
    {
        BDBG_ERR(("BDSP_Arm_P_ReadMsg_isr: No messages available, BytestoRead (%d) MsgSize (%d)",uiBytesToRead,uiMsgSize));
        BDBG_ASSERT(0);
    }
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_ReadMsg_isr: Error in Writing Chunk 1"));
        BDBG_ASSERT(0);
    }
    ...
    *pReadAddr = ReadOffset;
    hMsgQueue->Address.ReadOffset = ReadOffset;

    Memory = hMsgQueue->FifoMemory;
    Memory.pAddr = (void *)pReadAddr;
    BDSP_MMA_P_FlushCache(Memory,sizeof(dramaddr_t));

    BDBG_MSG(("BDSP_Arm_P_ReadMsg_isr: After Read, Write Offset="BDSP_MSG_FMT",Read Offset="BDSP_MSG_FMT,
        BDSP_MSG_ARG(WriteOffset),BDSP_MSG_ARG(ReadOffset)));

    BDBG_LEAVE(BDSP_Arm_P_ReadMsg_isr);
    return errCode;
}
...
)
{
    ...
    errCode = BDSP_Arm_P_WriteMsg_isr(hMsgQueue, (void *)psCommand, uiCommandSize);
    if(BERR_SUCCESS != errCode)
    {
        BDBG_ERR(("BDSP_Arm_P_SendCommand_isr: Error in writing the Message into Queue"));
        goto end;
    }
...
)
{
    ...
    errCode= BDSP_Arm_P_SendCommand_isr(hMsgQueue, psCommand);
...
{
    BERR_Code   errCode = BERR_SUCCESS;
    BDBG_ENTER(BDSP_Arm_P_GetResponse);

    BKNI_EnterCriticalSection();
    errCode = BDSP_Arm_P_ReadMsg_isr(hMsgQueue, pMsgBuf, uiMsgSize);
    BKNI_LeaveCriticalSection();

    BDBG_LEAVE(BDSP_Arm_P_GetResponse);
    return errCode;
}
...
)
{
    ...
    BDBG_MSG(("BDSP_Arm_P_GetAsyncMsg_isr: Before Read, Write Offset="BDSP_MSG_FMT",Read Offset="BDSP_MSG_FMT,
        BDSP_MSG_ARG(WriteOffset),BDSP_MSG_ARG(ReadOffset)));
    ...
    if ((ReadOffset > hMsgQueue->Address.EndOffset)||
        (ReadOffset < hMsgQueue->Address.BaseOffset))
    {
        BDBG_ERR(("BDSP_Arm_P_GetAsyncMsg_isr: Read pointer not within bounds in Message Queue, Fifo ID =%d , DSPOffset = %d, Read Offset = " BDSP_MSG_FMT
                    ",End Offset = " BDSP_MSG_FMT ",Base Offset = " BDSP_MSG_FMT, hMsgQueue->ui32FifoId,
                    hMsgQueue->ui32DspOffset, BDSP_MSG_ARG(ReadOffset), BDSP_MSG_ARG(hMsgQueue->Address.EndOffset),
                    BDSP_MSG_ARG(hMsgQueue->Address.BaseOffset)));
        BDBG_ASSERT(0);
    ...
    if ((WriteOffset > hMsgQueue->Address.EndOffset)||
        (WriteOffset < hMsgQueue->Address.BaseOffset))
    {
        BDBG_ERR(("BDSP_Arm_P_GetAsyncMsg_isr: Write pointer not within bounds in Message Queue, Fifo ID =%d , DSPOffset = %d, Read Offset = " BDSP_MSG_FMT
                    ",End Offset = " BDSP_MSG_FMT ",Base Offset = " BDSP_MSG_FMT, hMsgQueue->ui32FifoId,
                    hMsgQueue->ui32DspOffset, BDSP_MSG_ARG(WriteOffset), BDSP_MSG_ARG(hMsgQueue->Address.EndOffset),
                    BDSP_MSG_ARG(hMsgQueue->Address.BaseOffset)));
        BDBG_ASSERT(0);
    ...
    if(WriteOffset == ReadOffset)
    {
        ...
        BDBG_MSG(("BDSP_Arm_P_GetAsyncMsg_isr: The Message Queue is empty. No message is present."));
        return(BERR_SUCCESS);
    }
    else if(WriteOffset > ReadOffset)
    {
    ...
    {
        BDBG_ERR(("BDSP_Arm_P_GetAsyncMsg_isr: Bytes to read(%d) is less than size of the size of single Async message",uiBytesToRead));
        BDBG_ASSERT(0);
    }
    BDBG_MSG(("BDSP_Arm_P_GetAsyncMsg_isr: Payload size = %lu and bytes to read = %d",(unsigned long)(sizeof(BDSP_P_AsynMsg)),uiBytesToRead));
    ...
    *puiNumMsgs = uiBytesToRead/sizeof(BDSP_P_AsynMsg);
    ...
    if(errCode != BERR_SUCCESS)
    {
        BDBG_ERR(("BDSP_Arm_P_GetAsyncMsg_isr: Error in Writing Chunk 1"));
        BDBG_ASSERT(0);
    }
    ReadOffset = ReadOffset+uiChunk1;
    ...
    if(uiChunk2)
    {
        ...
        if(errCode != BERR_SUCCESS)
        {
            BDBG_ERR(("BDSP_Arm_P_GetAsyncMsg_isr: Error in Writing Chunk 2"));
            BDBG_ASSERT(0);
        }
        ReadOffset = ReadOffset+uiChunk2;
    }
    ...
    *pReadAddr = ReadOffset;
    ...
    BDBG_MSG(("BDSP_Arm_P_GetAsyncMsg_isr: After Read, Write Offset="BDSP_MSG_FMT",Read Offset="BDSP_MSG_FMT,
        BDSP_MSG_ARG(WriteOffset),BDSP_MSG_ARG(ReadOffset)));
