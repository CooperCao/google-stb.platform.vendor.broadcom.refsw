...
/* Function for getting the number of capture buffer present in the input queue to the encoder */
BERR_Code BDSP_VideoEncode_inputPictureBufferCount_isr(
...

{
    ...
    {
        return task->inputPictureBufferCount_isr(task->pTaskHandle, pPpbCount);
    }
    else
    {
...
BERR_Code BDSP_VideoEncode_putCcData_isr(
                                                BDSP_TaskHandle     task,
                                                void            *pCCDAddress
                                                )
{
    BDBG_OBJECT_ASSERT(task, BDSP_Task);
    if ( task->putCcData_isr)
    {
        return task->putCcData_isr(task->pTaskHandle, pCCDAddress);
    }
    else
    {
        return BERR_TRACE(BERR_NOT_SUPPORTED);
