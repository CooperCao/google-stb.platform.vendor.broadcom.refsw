   ...
   if ( true == bBindPicture )
   {
      ...
      if ( true == bFieldAppended )
      {
         ...
         for( i=0; i < pstSelectedPicture->stPicParms.stTSM.stStatic.uiNumElements; i++ )
         {
            ...
            {
               BXDM_PPFP_P_FixPtSub_isr( &stPtsActual, pstDelta, &stPtsActual );
               BXDM_PPFP_P_FixPtSub_isr( &stPstVirtual, pstDelta, &stPstVirtual );
            }
            else
            {
               BXDM_PPFP_P_FixPtAdd_isr( &stPtsActual, pstDelta, &stPtsActual );
               BXDM_PPFP_P_FixPtAdd_isr( &stPstVirtual, pstDelta, &stPstVirtual );
            }
         }
