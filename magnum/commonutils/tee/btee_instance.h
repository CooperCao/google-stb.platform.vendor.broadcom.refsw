...
typedef struct BTEE_InstanceCreateSettings
{
    ...
    BERR_Code (*ClientReceiveMessage)(void *pClientPrivate, void **pConnection, void *pMessage, uint32_t maxLength, uint32_t *pLength, int timeoutMsec);
    BERR_Code (*ClientAlloc)(void *pClientPrivate, uint32_t size, void **pMemory);
    void (*ClientFree)(void *pClientPrivate, void *pMemory);
    BERR_Code (*ClientSecureAlloc)(void *pClientPrivate, uint32_t size, uint64_t *pAddress);
    void (*ClientSecureFree)(void *pClientPrivate, uint64_t address);
    BERR_Code (*ClientAddrToOffset)(void *pClientPrivate, void *pMemory, uint64_t *pOffset);
    BERR_Code (*ClientOffsetToAddr)(void *pClientPrivate, uint64_t offset, void **pMemory);
    ...
    BERR_Code (*FileRead)(void *pFilePrivate, uint64_t address, uint32_t bytesToRead, uint32_t *pBytesRead);
    BERR_Code (*FileWrite)(void *pFilePrivate, uint64_t address, uint32_t bytesToWrite, uint32_t *pBytesWritten);
    ...
    BERR_Code (*ConnectionSendMessage)(void *pConnectionPrivate, const void *pMessage, uint32_t messageLength);
