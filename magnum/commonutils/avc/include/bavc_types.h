...
/***************************************************************************
Summary:
    Defines a dimensions tuple.

Description:
    This structure consists of a width and height.
...
/***************************************************************************
Summary:
    Defines a bounds tuple.

Description:
    This structure consists of a min and max.
...
/* Count of display primaries + white point */
#define BAVC_NUM_COLORIMETRY_POINTS 4

/***************************************************************************
Summary:
    Defines a color volume.

Description:
    This structure consists of values for the 3 color primaries and the white point.

See Also:
****************************************************************************/
typedef struct BAVC_ColorVolume
{
...
/***************************************************************************
Summary:
    Defines a structure carrying content light level data.

Description:
    This structure consists of values for the frame average max light level
    and the absolute max light level across the entire stream.

See Also:
****************************************************************************/

typedef struct BAVC_ContentLightLevel
{
    uint32_t                ulMaxFrameAvg; /* Pic Average Max Light level */
    uint32_t                ulMax; /* Max Light level */
} BAVC_ContentLightLevel;

/***************************************************************************
Summary:
    Defines a structure corresponding to the Mastering Display Color Volume
    SEI message.

Description:
    This structure consists of a color volume sub-struct and a bounds sub-struct
    for luminance;

See Also:
****************************************************************************/

typedef struct BAVC_MasteringDisplayColorVolume
{
    BAVC_ColorVolume stColorVolume;
    BAVC_Bounds stLuminance; /*
                              * units:
                              *   HDMI uses 0.0001 nits for min, 1 nits for max per CEA spec
                              *   HVD uses 0.0001 nits for min, 0.0001 nits for max per HEVC spec
                              *   All remaining Bcm code has adopted CEA convention
                              *   and will convert to HEVC convention for non-Brcm
                              *   code where required.
                              */
} BAVC_MasteringDisplayColorVolume;

/***************************************************************************
Summary:
Structure containing type 1 dynamic range static metadata (HDR10).
****************************************************************************/
typedef struct BAVC_StaticHdrMetadata
{
    BAVC_MasteringDisplayColorVolume stMasteringDisplayColorVolume;
    BAVC_ContentLightLevel stContentLightLevel;
