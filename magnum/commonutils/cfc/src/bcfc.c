    ...
    BCFC_ColorSpace          *pColorSpace )
{
    ...
    /* TODO: should this use same init as BAVC_GetDefaultStaticHdrMetadata_isrsafe? */
    BKNI_Memset(&pColorSpace->stMetadata.stStatic, 0, sizeof(pColorSpace->stMetadata.stStatic));

    pColorSpace->stMetadata.pDynamic = NULL;
}
...
{
    void *pMetaDataSave = pDstColorSpace->stMetadata.pDynamic;
    BKNI_Memcpy_isr(pDstColorSpace, pSrcColorSpace, sizeof(BCFC_ColorSpace));
    pDstColorSpace->stMetadata.pDynamic = pMetaDataSave;
}
    ...
    BCFC_Context          *pCfc )
{
    ...
    pCfc->stColorSpaceExtIn.stColorSpace.stMetadata.pDynamic = NULL;
