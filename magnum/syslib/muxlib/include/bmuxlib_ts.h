...
typedef struct BMUXlib_TS_Status
{
   ...
   struct
   {
      unsigned uiVideo; /* Average video bitrate (in bps) */
      unsigned uiAudio; /* Average audio bitrate (in bps) */
      unsigned uiSystemData; /* Average system data bitrate (in bps) */
      unsigned uiUserData; /* Average userdata bitrate (in bps) */
   } stAverageBitrate;
   unsigned uiEfficiency; /* Indicates efficiency of the output TS (e.g. 100 = no TS overhead, 95 = 5% TS overhead, etc.) */
   uint64_t uiTotalBytes; /* Total bytes generated by the TS MUXlib */
} BMUXlib_TS_Status;
