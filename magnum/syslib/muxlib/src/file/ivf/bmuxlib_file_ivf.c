...
/**************************
    Static Definitions
**************************/
static const BMUXlib_File_IVF_CreateSettings stDefaultCreateSettings =
{
   BMUXLIB_FILE_IVF_P_SIGNATURE_CREATESETTINGS,
};

/* Start Settings are set explicitly in GetDefaultStartSettings() */

static const BMUXlib_File_IVF_FinishSettings stDefaultFinishSettings =
{
   BMUXLIB_FILE_IVF_P_SIGNATURE_FINISHSETTINGS,
...
/*
   ...
   Returns:
      None
*/
void BMUXlib_File_IVF_GetDefaultCreateSettings(BMUXlib_File_IVF_CreateSettings *pCreateSettings)
{
   BDBG_ENTER(BMUXlib_File_IVF_GetDefaultCreateSettings);
   BDBG_ASSERT(pCreateSettings != NULL);

   *pCreateSettings = stDefaultCreateSettings;

   BDBG_LEAVE(BMUXlib_File_IVF_GetDefaultCreateSettings);
}
...
*/
BERR_Code BMUXlib_File_IVF_Create(BMUXlib_File_IVF_Handle *phIVFMux, const BMUXlib_File_IVF_CreateSettings *pCreateSettings)
{
   BERR_Code rc = BERR_UNKNOWN;
   BMUXlib_File_IVF_Handle hMux;
   BDBG_ENTER(BMUXlib_File_IVF_Create);

   BDBG_ASSERT(pCreateSettings != NULL);
   BDBG_ASSERT(phIVFMux != NULL);
   BDBG_ASSERT(BMUXLIB_FILE_IVF_P_SIGNATURE_CREATESETTINGS == pCreateSettings->uiSignature);

   BDBG_MSG(("====Creating IVF Mux===="));
...
/*
   ...
   Returns:
      None
*/
void BMUXlib_File_IVF_Destroy(BMUXlib_File_IVF_Handle hIVFMux)
{
...
/*
   ...
   Returns:
      None
*/
void BMUXlib_File_IVF_GetDefaultStartSettings(BMUXlib_File_IVF_StartSettings *pStartSettings)
{
   BDBG_ENTER(BMUXlib_File_IVF_GetDefaultStartSettings);
   ...
   BDBG_LEAVE(BMUXlib_File_IVF_GetDefaultStartSettings);
}
...
*/
BERR_Code BMUXlib_File_IVF_Start(BMUXlib_File_IVF_Handle hIVFMux, const BMUXlib_File_IVF_StartSettings *pStartSettings)
{
   BERR_Code rc = BERR_UNKNOWN;
   ...
   {
      BDBG_LEAVE(BMUXlib_File_IVF_Start);
      return BERR_TRACE(BERR_INVALID_PARAMETER);
   }

   BDBG_MSG(("====Starting IVF Mux===="));

   /* Start() can only be performed from the Stopped state - anything else is an error */
   if (BMUXlib_State_eStopped ==  BMUXLIB_FILE_IVF_P_GET_MUX_STATE(hIVFMux))
   {
      hIVFMux->stStartSettings = *pStartSettings;
      ...
      if ( BERR_SUCCESS == rc )
      {
         rc = BMUXlib_File_IVF_P_Start(hIVFMux);
      }

      if ( BERR_SUCCESS == rc )
      {
         /* mux is started ... */
         BMUXLIB_FILE_IVF_P_SET_MUX_STATE(hIVFMux, BMUXlib_State_eStarted);
      }
   }
   else
   {
      /* Start() invoked from an invalid state - error: do nothing */
      BDBG_ERR(("IVF Mux Start:: Invoked from invalid state: %d",  BMUXLIB_FILE_IVF_P_GET_MUX_STATE(hIVFMux)));
...
/*
   ...
   Returns:
      None
*/
void BMUXlib_File_IVF_GetDefaultFinishSettings(BMUXlib_File_IVF_FinishSettings *pFinishSettings)
{
   BDBG_ENTER(BMUXlib_File_IVF_GetDefaultFinishSettings);
   BDBG_ASSERT(pFinishSettings != NULL);

   *pFinishSettings = stDefaultFinishSettings;

   BDBG_LEAVE(BMUXlib_File_IVF_GetDefaultFinishSettings);
}
...
*/
BERR_Code BMUXlib_File_IVF_Finish(BMUXlib_File_IVF_Handle hIVFMux, const BMUXlib_File_IVF_FinishSettings *pFinishSettings)
{
   BERR_Code rc = BERR_UNKNOWN;
   ...
   switch (BMUXLIB_FILE_IVF_P_GET_MUX_STATE(hIVFMux))
   {
      case BMUXlib_State_eStarted:
         switch (pFinishSettings->eFinishMode)
         {
            case BMUXlib_FinishMode_ePrepareForStop:
               ...
               BDBG_MODULE_MSG(BMUX_IVF_FINISH, ("eStarted --> eFinishingInput"));
               BMUXLIB_FILE_IVF_P_SET_MUX_STATE(hIVFMux, BMUXlib_State_eFinishingInput);
               rc = BERR_SUCCESS;
               break;
      ...
      case BMUXlib_State_eFinished:
         ...
         break;
      default:
         BDBG_ERR(("IVF Mux Finish:: Invoked from invalid state: %d",  BMUXLIB_FILE_IVF_P_GET_MUX_STATE(hIVFMux)));
         rc = BERR_TRACE(BERR_NOT_SUPPORTED);
         break;
   }
   BDBG_LEAVE(BMUXlib_File_IVF_Finish);
   return rc;
}
...
*/
BERR_Code BMUXlib_File_IVF_Stop(BMUXlib_File_IVF_Handle hIVFMux)
{
   ...
   BDBG_LEAVE(BMUXlib_File_IVF_Stop);
   return rc;
}
...
*/
BERR_Code BMUXlib_File_IVF_DoMux(BMUXlib_File_IVF_Handle hIVFMux, BMUXlib_DoMux_Status *pStatus)
{
   BERR_Code rc = BERR_UNKNOWN;
   ...
   switch (BMUXLIB_FILE_IVF_P_GET_MUX_STATE(hIVFMux))
   {
      ...
      case BMUXlib_State_eStarted:
         if (BERR_SUCCESS != (rc = BMUXlib_File_IVF_P_ProcessOutputDescriptorsCompleted(hIVFMux)))
            break;
         if (BERR_SUCCESS != (rc = BMUXlib_File_IVF_P_ProcessInputDescriptors(hIVFMux)))
            break;
         if (BERR_SUCCESS != (rc = BMUXlib_File_IVF_P_ProcessOutputDescriptorsWaiting(hIVFMux)))
            break;
         /* If the EOS has been seen, automatically go to the finishing input state */
         if ( true == BMUXlib_File_IVF_P_EOSSeen(hIVFMux) )
         {
            BDBG_MODULE_MSG(BMUX_IVF_FINISH, ("eStarted --> eFinishingInput (Auto)"));
            BMUXLIB_FILE_IVF_P_SET_MUX_STATE(hIVFMux, BMUXlib_State_eFinishingInput);
         }
         break;
      ...
      case BMUXlib_State_eFinishingInput:
         ...
         if (BMUXlib_File_IVF_P_IsInputProcessingDone(hIVFMux))
         {
            BDBG_MODULE_MSG(BMUX_IVF_FINISH, ("eFinishingInput --> eFinishingOutput"));
            BMUXLIB_FILE_IVF_P_SET_MUX_STATE(hIVFMux, BMUXlib_State_eFinishingOutput);
         }
         break;
      ...
      case BMUXlib_State_eFinishingOutput:
         ...
         if (BMUXlib_File_IVF_P_IsOutputProcessingDone(hIVFMux))
         {
            BDBG_MODULE_MSG(BMUX_IVF_FINISH, ("eFinishingOutput --> eFinished"));
            BMUXLIB_FILE_IVF_P_SET_MUX_STATE(hIVFMux, BMUXlib_State_eFinished);
         }
         break;
      ...
      default:
         ...
         BDBG_ERR(("DoMux:: Invalid State detected: %d",  BMUXLIB_FILE_IVF_P_GET_MUX_STATE(hIVFMux)));
         rc = BERR_TRACE(BERR_NOT_SUPPORTED);
         break;
   }

   *pStatus = hIVFMux->stStatus;
   ...
   BDBG_LEAVE(BMUXlib_File_IVF_DoMux);
   return rc;
}
