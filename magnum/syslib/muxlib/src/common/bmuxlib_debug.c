...
bool BMuxlib_Debug_CSVOpen(BMuxlib_Debug_CSV_Handle *phCSV, const char *fname)
{
   ...
   hCSV = (BMuxlib_Debug_CSV_Handle) BMUXlib_Malloc(sizeof(BMuxlib_Debug_P_CSV_Context));
   if (NULL == hCSV)
   {
      return false;
   }
   ...
   )
{
   if (size > 4096)
   {
      BDBG_ERR(("BKNI_Malloc of %u in %s:%u", (unsigned)size, file, line));
   }
