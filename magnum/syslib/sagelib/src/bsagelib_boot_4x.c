    ...
    BSAGElib_Sage_ImageHolder *image)
{
    ...
    pCommonHeader = &(image->header->common);
    ...
    BSAGElib_Sage_ImageHolder *img)
{
    ...
    SUIF_CommonHeader *pCommonHeader = &(img->header->common);
    ...
    BSAGElib_Sage_ImageHolder* holder)
{
    ...
    holder->header = (SUIF_DummyImageHeader *)pBinary;
...
{
    BERR_Code rc = BERR_SUCCESS;
    SUIF_SBLImageHeader *header = (SUIF_SBLImageHeader *)blImg->header;

    uint16_t *pHeader_length_BE  = (uint16_t *)(header->common.headerLength_BE);
    uint32_t header_length  = SUIF_Get16(pHeader_length,BE);

    uint8_t *tier_key_start = (uint8_t *)header + header_length;

    uint32_t *pGlobalOwnerIdSelect_BE = (uint32_t *)(header->sbl.keyladder.globalOwnerIdSelect_BE);
    uint32_t globalOwnerIdSelect = SUIF_Get32(pGlobalOwnerIdSelect,BE);

    uint16_t *pTierKeyPackageSize_BE = (uint16_t *)(header->common.tierKeyPackageSize_BE);
    uint8_t *data = (uint8_t *)header + header_length + 3*SUIF_Get16(pTierKeyPackageSize,BE);

    uint32_t *pDataSize_BE = (uint32_t *)(header->common.dataSize_BE);
    uint32_t *pTextSize_BE = (uint32_t *)(header->common.textSize_BE);
    uint32_t data_len = SUIF_Get32(pTextSize,BE) + SUIF_Get32(pDataSize,BE);

    uint32_t *pMeta_data_size_BE  = (uint32_t *)(header->common.metadataSize_BE);
    uint32_t meta_data_size  = SUIF_Get32(pMeta_data_size,BE);

    uint16_t *pPkg_pad_size_BE = (uint16_t *)(header->common.packagePadSize_BE);
    uint32_t pkg_pad_size    = SUIF_Get16(pPkg_pad_size,BE);

    uint16_t *pCntrl_params_sz_BE = (uint16_t *)(header->common.controllingParamsSize_BE);
    uint32_t cntrl_params_sz = SUIF_Get16(pCntrl_params_sz,BE);

    uint8_t *signature = data + data_len + meta_data_size + pkg_pad_size + SUIF_SHA256_SIZE_BYTES + cntrl_params_sz;
    uint8_t *image_controlling_parameters = signature - cntrl_params_sz;

    uint32_t *pStbOwnerId_BE     = (uint32_t *)(header->sbl.keyladder.stbOwnerIdSelect_BE);
    uint32_t *pCaVendorId_BE     = (uint32_t *)(header->sbl.keyladder.caVendorId_BE);
    uint32_t *pGlobalKeyIndex_BE = (uint32_t *)(header->sbl.keyladder.globalKeyIndex_BE);
    ...
    /* if OTP_SAGE_VERIFY_ENABLE_BIT: verify SAGE boot loader */
    {
        ...
        if(header->common.key0Select == SUIF_Key0Select_eIndex )
        {
        ...
        pSecondTierKey = (SUIF_TierKeyControllingParameters *)tier_key_start;
    ...
    {
        uint32_t *pModuleId_BE       = (uint32_t *)(header->sbl.keyladder.moduleId_BE);
        {
            ...
            BKNI_Memcpy(generateRouteKeyIO.aucKeyData, header->sbl.keyladder.procIn3, 16 );
            ...
            generateRouteKeyIO.keySize            = BCMD_KeySize_e128;
            generateRouteKeyIO.ucKeyDataLen       = 16;
            BKNI_Memcpy(generateRouteKeyIO.aucKeyData, header->sbl.keyladder.procIn4, 16);
            generateRouteKeyIO.bIsRouteKeyRequired = false;
            generateRouteKeyIO.keyDestBlckType  = BCMD_KeyDestBlockType_eCPDescrambler;
            ...
            generateRouteKeyIO.keySize          = BCMD_KeySize_e128;
            generateRouteKeyIO.ucKeyDataLen       = 16;
            BKNI_Memcpy(generateRouteKeyIO.aucKeyData, header->sbl.keyladder.procIn5, 16);
            generateRouteKeyIO.bIsRouteKeyRequired = false;
            generateRouteKeyIO.keyDestBlckType  = BCMD_KeyDestBlockType_eCPDescrambler;
    ...
    { /* setup and verify region */
        ...
        verifyIO.region.startAddress = hSAGElib->i_memory_map.addr_to_offset(data);
        if(verifyIO.region.startAddress == 0) {
            BDBG_ERR(("%s - Cannot convert bootloader address to offset", BSTD_FUNCTION));
            goto end;
        }
        verifyIO.region.endAddress = verifyIO.region.startAddress + (data_len) - 1;
        ...
        _BSAGElib_P_Boot_SetBootParam( SageBootloaderEpochVersion, header->common.epochVersion );
...

    {
        ...
        pRsaKey = (SUIF_TierKeyControllingParameters *)tier_key_start;
...

    {
        ...
        BKNI_Memcpy( ladderLevelKey.ladderKey, header->sbl.keyladder.procIn3, SUIF_AES_PROCIN_SIZE_BYTES );
        ...
        BKNI_Memcpy( ladderLevelKey.ladderKey, header->sbl.keyladder.procIn4, SUIF_AES_PROCIN_SIZE_BYTES );
        ladderLevelKey.ladderKeySize = 128;
        ...
        BKNI_Memcpy( ladderLevelKey.ladderKey, header->sbl.keyladder.procIn5, SUIF_AES_PROCIN_SIZE_BYTES );
        ladderLevelKey.ladderKeySize = 128;
    ...
    startAddress = hSAGElib->i_memory_map.addr_to_offset(data);
