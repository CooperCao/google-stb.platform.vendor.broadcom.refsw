...
#define BSHM_P_RSA_BSP_STATUS_PENDING 0x3000
...
BERR_Code BHSM_Rsa_GetResult( BHSM_RsaHandle handle, BHSM_RsaExponentiateResult *pResult )
{
    ...
    BKNI_Memset( &bspParam, 0, sizeof(bspParam) );
    bspParam.suppressBspStatusErrorMessage = true;

    rc = BHSM_P_Crypto_PollRsa( handle->hHsm, &bspParam );
    if( rc != BERR_SUCCESS )
    {
        if ( bspParam.bspStatus == BSHM_P_RSA_BSP_STATUS_PENDING )
        {
            return BHSM_RSA_STATUS_IN_PROGRESS;
        }
        else
        {
