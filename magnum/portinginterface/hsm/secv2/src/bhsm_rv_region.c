...
BERR_Code BHSM_RvRegion_Enable( BHSM_RvRegionHandle handle )
{
    ...
    bspEnableRegion.in.instrCheck = pRvRegion->settings.instrCheck?0/*eEnable*/:0x0C;
    bspEnableRegion.in.bgCheck = pRvRegion->settings.backgroundCheck?0/*eEnable*/:0x0D;
    bspEnableRegion.in.allowRegionDisable = pRvRegion->settings.allowRegionDisable?0/*eAllow*/:0x0E;
    bspEnableRegion.in.enforceAuth = pRvRegion->settings.enforceAuth?0x0B/*eEnforce*/:0;
...
BERR_Code BHSM_RvRegion_Enable( BHSM_RvRegionHandle handle )
{
    ...
    if( pRegion->settings.signature.size )
    {
        ...
        endOffset = pRegion->settings.signature.address + pRegion->settings.signature.size-1;
        bspEnableRegion.in.sigEndAddrMsb = ( endOffset >> 32 ) & 0xFF;
        bspEnableRegion.in.sigEndAddr = endOffset & 0xFFFFFFFF;
    }
    ...
    bspEnableRegion.in.instrCheck = pRegion->settings.instrCheck ? Bsp_CmdRv_InstrCheck_eEnable
                                                                   : Bsp_CmdRv_InstrCheck_eDisable;
    ...
    bspEnableRegion.in.sigParamsStartAddrMsb = ( pRegion->settings.parameters.address >> 32 ) & 0xFF;
    bspEnableRegion.in.sigParamsStartAddr = pRegion->settings.parameters.address & 0xFFFFFFFF;
