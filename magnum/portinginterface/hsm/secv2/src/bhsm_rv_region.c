...
BDBG_MODULE( BHSM );
BDBG_OBJECT_ID( BHSM_P_RvRegion );
...
BERR_Code BHSM_RvRegion_Init_priv( BHSM_Handle hHsm, BHSM_RvRegionModuleSettings *pSettings )
{
    ...
    if( hHsm->modules.pRvRegions ) { return BERR_TRACE( BERR_NOT_AVAILABLE ); } /* already intialialised */
...
BHSM_RvRegionHandle BHSM_RvRegion_Allocate( BHSM_Handle hHsm, const BHSM_RvRegionAllocateSettings *pSettings )
{
    ...
    BDBG_OBJECT_SET( pRegion, BHSM_P_RvRegion );
...
void BHSM_RvRegion_Free( BHSM_RvRegionHandle handle )
{
    ...
    BDBG_OBJECT_ASSERT( pRegion, BHSM_P_RvRegion );
    ...
    BDBG_OBJECT_UNSET( pRegion, BHSM_P_RvRegion );
