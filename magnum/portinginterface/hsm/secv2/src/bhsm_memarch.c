...
BHSM_MemArchHandle BHSM_MemArch_EnableGeneric( BHSM_Handle hHsm, BHSM_MemArchEnableGeneric *pParam )
{
    ...
    if( pParam->addressRange.upper & 0xFF ) { BERR_TRACE( BHSM_STATUS_INPUT_PARM_ERR ); return 0; }
    if( pParam->addressRange.lower & 0xFF ) { BERR_TRACE( BHSM_STATUS_INPUT_PARM_ERR ); return 0; }
    ...
    if( rc != BERR_SUCCESS ) { BERR_TRACE( rc ); return 0; }

    BDBG_LEAVE( BHSM_MemArch_EnableGeneric );
    return (BHSM_MemArchHandle)( bspConfig.out.memcIndex | (bspConfig.out.archIndex << 8) );
}

BERR_Code BHSM_MemArch_Disable( BHSM_MemArchHandle hMemArch )
{

    BDBG_ENTER( BHSM_MemArch_Disable );

    BSTD_UNUSED( hMemArch );

    BDBG_LEAVE( BHSM_MemArch_Disable );
    return BERR_SUCCESS;
}
