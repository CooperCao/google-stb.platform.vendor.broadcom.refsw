...
static void _PrintParameter( BHSM_Handle hHsm, _Parameter *pParameter, uint32_t *pData );
static void _PrintParameterOwnership( BHSM_Handle hHsm, uint32_t *pData );
...
void BHSM_BspDebug_PrintCommand( BHSM_Handle hHsm, uint32_t *pData )
{
    unsigned componentIndex = 0;
    unsigned commandIndex = 0;
...
void BHSM_BspDebug_PrintResponse( BHSM_Handle hHsm, uint32_t *pData )
{
    unsigned componentIndex = 0;
    unsigned commandIndex = 0;
    unsigned parameterIndex = 0;
...
unsigned  _GetUnsigned( uint32_t *pData, unsigned byteOffset, unsigned size )
{
    unsigned rv = 0;
    ...
    if( pParameter->isArray ){
        ...
        if( pParameter->byteOffset % 4 == 0 )
        {
            ...
            for( x = 0; x < pParameter->size; x+=4 ) {
               ...
               /* BDBG_MSG(("   xxxxx" ));*/ /*only supporting arrays the span whole words at the moment!!!*/
        }
    }
