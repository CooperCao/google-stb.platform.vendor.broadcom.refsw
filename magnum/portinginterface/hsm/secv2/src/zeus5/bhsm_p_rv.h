...
typedef struct BHSM_P_RvQueryAllRegions
{
    ...
    uint16_t bspStatus; /* the status returned from the BSP */
    struct {
        uint32_t    regionStatus[36];
        unsigned   regionStatusSize_inPlace; /* size in bytes */
        uint8_t    *pRegionStatus_inPlace;
    }out;
...
typedef struct BHSM_P_RvEnableRegion
{
    ...
    struct {
        ...
        uint32_t    decryptionKey[4]; /* use this array or the following size and pointer. */
        unsigned   decryptionKeySize_inPlace; /* size in bytes. */
        uint8_t    *pDecryptionKey_inPlace;
        uint32_t    decryptionIv[4]; /* use this array or the following size and pointer. */
        unsigned   decryptionIvSize_inPlace; /* size in bytes. */
        uint8_t    *pDecryptionIv_inPlace;
