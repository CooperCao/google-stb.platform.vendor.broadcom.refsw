...
BERR_Code BHSM_P_Rv_GetSecureBootParams( BHSM_Handle hHsm, BHSM_P_RvGetSecureBootParams *pParam )
{
	...
	BHSM_BspMsgCreate_t msgCreate;
	BHSM_BspMsgConfigure_t msgConfig;
	Bsp_CmdRv_GetSecureBootParams_OutFields_t *pReceive = NULL;

	BDBG_ENTER( BHSM_P_Rv_GetSecureBootParams );
	...
	pReceive = (Bsp_CmdRv_GetSecureBootParams_OutFields_t*)msgCreate.pReceive;

	msgConfig.component = Bsp_CmdComponent_eRv;
	msgConfig.command = Bsp_CmdRv_eGetSecureBootParams;
	...
	pParam->out.bspParamsSizeBytes = pReceive->bspParamsSizeBytes;
	BKNI_Memcpy( pParam->out.bspParams, pReceive->bspParams, 256 );

BHSM_P_DONE_LABEL:

	(void)BHSM_BspMsg_Destroy( hMsg );

	BDBG_LEAVE( BHSM_P_Rv_GetSecureBootParams );
...
BERR_Code BHSM_P_Rv_QueryAllRegions( BHSM_Handle hHsm, BHSM_P_RvQueryAllRegions *pParam )
{
	...
	/* extract output paramters */
	BKNI_Memcpy( pParam->out.regionStatus, pReceive->regionStatus, 120 );
...
BERR_Code BHSM_P_Rv_EnableRegion( BHSM_Handle hHsm, BHSM_P_RvEnableRegion *pParam )
{
	...
	pSend->scbBurstSize = pParam->in.scbBurstSize;
	pSend->verifyPartialDigest = pParam->in.verifyPartialDigest;
	pSend->resetOnVerifyFailure = pParam->in.resetOnVerifyFailure;
	pSend->rsaKeyId = pParam->in.rsaKeyId;
	pSend->keyLayer = pParam->in.keyLayer;
	pSend->vklId = pParam->in.vklId;
	pSend->cpuType = pParam->in.cpuType;
	pSend->svpFwReleaseVersion = pParam->in.svpFwReleaseVersion;
	pSend->chipBindingSel = pParam->in.chipBindingSel;
	pSend->epochSel = pParam->in.epochSel;
	pSend->marketIdSel = pParam->in.marketIdSel;
	pSend->sigType = pParam->in.sigType;
	pSend->sigVersion = pParam->in.sigVersion;
	pSend->marketId = pParam->in.marketId;
	pSend->marketIdMask = pParam->in.marketIdMask;
	pSend->epoch = pParam->in.epoch;
	pSend->epochMask = pParam->in.epochMask;
	pSend->upperChipsetBinding = pParam->in.upperChipsetBinding;
	pSend->lowerChipsetBinding = pParam->in.lowerChipsetBinding;
	pSend->metadata1 = pParam->in.metadata1;
	pSend->metadata2 = pParam->in.metadata2;
...
BERR_Code BHSM_P_Rv_MemsysReadyForInit( BHSM_Handle hHsm, BHSM_P_RvMemsysReadyForInit *pParam )
{
	...
	BDBG_ENTER( BHSM_P_Rv_MemsysReadyForInit );
    BSTD_UNUSED( pParam );
	...
	msgConfig.component = Bsp_CmdComponent_eRv;
	msgConfig.command = Bsp_CmdRv_eMemsysReadyForInit;
...
BHSM_P_DONE_LABEL:
	...
	BDBG_LEAVE( BHSM_P_Rv_MemsysReadyForInit );
