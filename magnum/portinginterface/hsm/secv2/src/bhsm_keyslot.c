...
*/
BERR_Code BHSM_Keyslot_RouteEntryKey ( BHSM_KeyslotHandle handle,
                                      ...
                                      const BHSM_KeyslotRouteKey *pKey )
{
    ...
    BDBG_ENTER( BHSM_Keyslot_RouteEntryKey );
    ...
    bspConfig.in.blockType = _GetEntryBlockType(entry);
    bspConfig.in.entryType = _GetEntryPolarity(entry);
    bspConfig.in.keySlotType = _convertSlotType(pSlot->slotType);
    bspConfig.in.keySlotNumber = pSlot->number;
    bspConfig.in.keyMode = 0 /*Bsp_KeyMode_eRegular */;

    bspConfig.in.modeWords[0] = compileControl0_GlobalHi( handle );
    bspConfig.in.modeWords[1] = compileControl1_GlobalLo( handle );
    bspConfig.in.modeWords[2] = compileControl2_ModeHi( handle, entry );
    bspConfig.in.modeWords[3] = compileControl3_ModeLo( handle, entry );

    if( pEntry->settings.external.iv ) {
        bspConfig.in.extIvPtr = pEntry->pExternalSlot->offsetIv + pEntry->pExternalSlot->slotPtr;
    }

    bspConfig.in.vklId = pKey->keyLadderIndex;
    bspConfig.in.keyLayer = pKey->keyLadderLayer;

    rc = BHSM_P_KeyLadder_RouteKey( _GetHsmHandle(handle), &bspConfig );
    if( rc != BERR_SUCCESS ) { return BERR_TRACE( rc ); }

    BDBG_LEAVE( BHSM_Keyslot_RouteEntryKey );
    return BERR_SUCCESS;
}
...
*/
BERR_Code BHSM_Keyslot_RouteEntryIv( BHSM_KeyslotHandle handle,
                                     BHSM_KeyslotBlockEntry entry,   /* block (cps/ca/cpd) and entry (odd/even/clear) */
                                     const BHSM_KeyslotRouteIv *pIv )
{
    ...
    BDBG_ENTER( BHSM_Keyslot_RouteEntryIv );
    ...
    bspConfig.in.vklId = pIv->keyLadderIndex;
    bspConfig.in.keyLayer = pIv->keyLadderLayer;
    if(  pIv->configIv2 ){
    ...
    rc = BHSM_P_KeyLadder_RouteIv( _GetHsmHandle(handle), &bspConfig );
    if( rc != BERR_SUCCESS ) { return BERR_TRACE( rc ); }

    BDBG_LEAVE( BHSM_Keyslot_RouteEntryIv );
    return BERR_SUCCESS;
}
...
static BERR_Code _SetEntryIv( BHSM_KeyslotHandle handle, BHSM_KeyslotBlockEntry entry, const BHSM_KeyslotIv *pIv, const BHSM_KeyslotIv *pIv2)
{
    ...
    if( pIv )
    {
        ...
        bspSetIv.in.ivType = 1 /*Bsp_KeySlotIvType_eIv*/;
    ...
    if( pIv2 )
    {
        ...
        bspSetIv.in.ivType = 2 /* Bsp_KeySlotIvType_eAesShortIv*/;
...
static BERR_Code _Keyslot_LoadConfigurationFromBsp( BHSM_Handle hHsm )
{
    ...
    if( rc != BERR_SUCCESS ) {
        BDBG_LOG((" BHSM_P_KeySlot_Query ERROR workaround in place"));
        /* return BERR_TRACE( BERR_NOT_AVAILABLE ); */
    }

    BDBG_LOG(("#IvPerSlot128  [%d]", bspQuery.out.keySlotNumberIvPerSlot128 ));
    BDBG_LOG(("#IvPerBlock128 [%d]", bspQuery.out.keySlotNumberIvPerBlock128 ));
    BDBG_LOG(("#IvPerBlock256 [%d]", bspQuery.out.keySlotNumberIvPerBlock256 ));
    BDBG_LOG(("#IvPerEntry256 [%d]", bspQuery.out.keySlotNumberIvPerEntry256 ));

    /* ignoring ownership for now. */
