...
BDBG_OBJECT_ID(BHSM_P_KeySlot);

#define BHSM_MAX_KEYSLOTS               (228)
...
typedef struct BHSM_KeySlotModule
{
    ...
    struct{
        unsigned typeOffset;     /* offset into pKeySlotHandles of first slot of this type */
        unsigned numKeySlotsForType;  /* number of slots of this type. */
...
BERR_Code BHSM_Keyslot_Init( BHSM_Handle hHsm, BHSM_KeyslotModuleSettings *pSettings )
{
    ...
    for( i = 0; i < BHSM_KeyslotType_eMax; i++ ){
        pModule->types[i].typeOffset = count;
        pModule->types[i].numKeySlotsForType = pSettings->numKeySlotsForType[i];
...
BERR_Code BHSM_P_KeyslotModule_GetCapabilities( BHSM_Handle hHsm, BHSM_KeyslotModuleCapabilities *pCaps )
{
    ...
    for( i = 0; i < BHSM_KeyslotType_eMax; i++ )
    {
        pCaps->numKeySlotsForType[i] = pModule->types[i].numKeySlotsForType;
    }
...
BHSM_KeyslotHandle BHSM_Keyslot_Allocate( BHSM_Handle hHsm, const BHSM_KeyslotAllocateSettings *pSettings )
{
    ...
    unsigned typeOffset; /* offset to keyslot type */
    unsigned slotIndex = BHSM_MAX_KEYSLOTS;
    unsigned numKeySlotsForType;
   #ifdef BHSM_BUILD_HSM_FOR_HOST
    unsigned i;
   #endif
    ...
    /* find a free slot. */
    typeOffset = pModule->types[pSettings->slotType].typeOffset;
    numKeySlotsForType = pModule->types[pSettings->slotType].numKeySlotsForType;

    if( typeOffset >= BHSM_MAX_KEYSLOTS ) { BERR_TRACE( BERR_INVALID_PARAMETER ); goto error; }
    if( numKeySlotsForType >= BHSM_MAX_KEYSLOTS ) { BERR_TRACE( BERR_INVALID_PARAMETER ); goto error; }
    if( typeOffset+numKeySlotsForType >= BHSM_MAX_KEYSLOTS ) { BERR_TRACE( BERR_INVALID_PARAMETER ); goto error; }

   #ifdef BHSM_BUILD_HSM_FOR_HOST
    /* search for a free slot.*/
    for( i = typeOffset; (i < typeOffset+numKeySlotsForType) && (i < BHSM_MAX_KEYSLOTS); i++ )
    {
        if( pModule->pKeySlotHandles[i] == NULL )
        {
            ...
            slotIndex = i;
            pModule->pKeySlotHandles[slotIndex] = pSlot;

            BKNI_Memset( pSlot, 0, sizeof(*pSlot) );
            BDBG_OBJECT_SET( pSlot, BHSM_P_KeySlot );
            pSlot->slotType = pSettings->slotType;
            pSlot->number = i-typeOffset;
    ...
    if( !pSlot ){
        BDBG_ERR(("Failed to allocate keyslot. Type[%u] Max[%u]", pSettings->slotType, numKeySlotsForType ));
    ...
    /* slot number greater that number reserved for that type.*/
    if( pSettings->keySlotNumber >= numKeySlotsForType ){
        BERR_TRACE( BERR_INVALID_PARAMETER );
        goto error;
    }

    if( typeOffset + pSettings->keySlotNumber > BHSM_MAX_KEYSLOTS ) {
        BERR_TRACE( BERR_INVALID_PARAMETER );
        goto error;
    }

    if( pModule->pKeySlotHandles[typeOffset + pSettings->keySlotNumber] != NULL ){
        BERR_TRACE( BERR_NOT_AVAILABLE );
        BDBG_ERR(("Keyslot type[%u] #[%u] not available", pSettings->slotType,  pSettings->keySlotNumber ));
    ...
    slotIndex = typeOffset + pSettings->keySlotNumber;
    pModule->pKeySlotHandles[slotIndex] = pSlot;
 ...
 error:
    if( pSlot ) {
        BDBG_ASSERT(( slotIndex < BHSM_MAX_KEYSLOTS ));
        BKNI_Free( pSlot );
        pModule->pKeySlotHandles[slotIndex] = NULL;
    }
...
void BHSM_Keyslot_Free( BHSM_KeyslotHandle handle )
{
    ...
    slotOffset = pSlot->pModule->types[pSlot->slotType].typeOffset + pSlot->number;

    if( slotOffset < BHSM_MAX_KEYSLOTS ){
        pSlot->pModule->pKeySlotHandles[slotOffset] = NULL;
    }
    else{ BERR_TRACE(rc); }
    ...
    BDBG_OBJECT_UNSET( pSlot, BHSM_P_KeySlot );
    BKNI_Free( pSlot );
...
static BERR_Code _SetEntryKey( BHSM_KeyslotHandle handle, BHSM_KeyslotBlockEntry entry, const BHSM_KeyslotKey *pKey )
{
    ...
    if( pKey ) {
        ...
        if( keyOffset >= secureKeyMemorySize ) { rc = BERR_TRACE( BERR_INVALID_PARAMETER ); goto exit; }
        if( keyOffset + pKey->size > secureKeyMemorySize ) { rc = BERR_TRACE( BERR_INVALID_PARAMETER ); goto exit; }

        BKNI_Memcpy( pSecureKeyMemory + keyOffset, pKey->key, pKey->size );

        if( keySize  == Bsp_KeySize_e64 ) { /* duplicate the key. */

            if( keyOffset < pKey->size ) { rc = BERR_TRACE( BERR_INVALID_PARAMETER ); goto exit; }
...
static BERR_Code _InvalidateSlot( BHSM_KeyslotHandle handle, bool preserveOwner )
{
    ...
    bspInvalidate.in.invalidateMethod  = Bsp_KeySlotInvalidateOperation_eInvalidateAll;
...
static BERR_Code _InvalidateEntry( BHSM_KeyslotHandle handle, BHSM_KeyslotBlockEntry entry )
{
    ...
    {
        case BHSM_KeyslotBlockType_eCpd: { block = Bsp_KeySlotBlockType_eCpd; break; }
        case BHSM_KeyslotBlockType_eCa:  { block = Bsp_KeySlotBlockType_eCa;  break; }
        case BHSM_KeyslotBlockType_eCps: { block = Bsp_KeySlotBlockType_eCps; break; }
        default: return BERR_TRACE( BERR_INVALID_PARAMETER );
    }
    switch(_GetEntryPolarity(entry))
    {
        case BHSM_KeyslotPolarity_eOdd:   { polarity = Bsp_KeySlotEntryType_eOddKey; break; }
        case BHSM_KeyslotPolarity_eEven:  { polarity = Bsp_KeySlotEntryType_eEvenKey; break; }
        case BHSM_KeyslotPolarity_eClear: { polarity = Bsp_KeySlotEntryType_eClearKey; break; }
        default: return BERR_TRACE( BERR_INVALID_PARAMETER );
    }
    ...
    bspInvalidate.in.keySlotType       = BHSM_P_ConvertSlotType( pSlot->slotType );
    bspInvalidate.in.keySlotNumber     = pSlot->number;
    bspInvalidate.in.sc01ModeWordMapping  = Bsp_Sc01ModeWordMapping_eClear;
    bspInvalidate.in.invalidateMethod  = Bsp_KeySlotInvalidateOperation_eInvalidateOneEntry;
...
static uint32_t compileControl3_ModeLo( BHSM_KeyslotHandle handle, BHSM_KeyslotBlockEntry entry )
{
    ...
    pEntry = _GetEntry( handle, entry );
    if( !pEntry ) { BERR_TRACE( BERR_INVALID_PARAMETER ); return control; }
...
static BHSM_KeyslotSettings* _GetSlotSetting( BHSM_KeyslotHandle handle )
{
    ...
    if( !pSlot ) { BERR_TRACE( BERR_INVALID_PARAMETER ); return NULL; }
...
static BHSM_Handle _Keyslot_GetHsmHandle( BHSM_KeyslotHandle handle )
{
    ...
    if( !pSlot ) { BERR_TRACE( BERR_INVALID_PARAMETER ); return NULL; }
    if( !pSlot->pModule ) { BERR_TRACE( BERR_INVALID_PARAMETER ); return NULL; }
...
static BERR_Code _Keyslot_Init( BHSM_Handle hHsm, BHSM_KeyslotModuleSettings *pSettings )
{
    ...
    BDBG_LOG(("KeySlot types: IvPerSlot128[%u+%u] IvPerBlock128[%u] IvPerBlock256[%u] IvPerEntry256[%u]"
                                                             , hsmInit.in.keySlotNumberIvPerSlot128-BHSM_NUM_BYPASS_KEYSLOTS
                                                             , BHSM_NUM_BYPASS_KEYSLOTS
                                                             , hsmInit.in.keySlotNumberIvPerBlock128
                                                             , hsmInit.in.keySlotNumberIvPerBlock256
                                                             , hsmInit.in.keySlotNumberIvPerEntry256 ));
...
static BERR_Code _Keyslot_LoadConfigurationFromBsp( BHSM_Handle hHsm )
{
    ...
    if( rc != BERR_SUCCESS ) {
        BDBG_LOG((" BHSM_P_KeySlot_Query FAILED. "));  /* can happen in some early BFW.  */
        /*return*/ BERR_TRACE( BERR_NOT_AVAILABLE );  /* will not return for now.  */
    }
    ...
    for( i = 0; i < BHSM_KeyslotType_eMax; i++ ){
        ...
        numKeySlotsForType = 0;

        switch(i)
        {
            case BHSM_KeyslotType_eIvPerSlot:     numKeySlotsForType = bspQuery.out.keySlotNumberIvPerSlot128;  break;
            case BHSM_KeyslotType_eIvPerBlock:    numKeySlotsForType = bspQuery.out.keySlotNumberIvPerBlock128; break;
            case BHSM_KeyslotType_eIvPerEntry:    break; /* BSP doesn't support this */
            case BHSM_KeyslotType_eIvPerBlock256: numKeySlotsForType = bspQuery.out.keySlotNumberIvPerBlock256; break;
            case BHSM_KeyslotType_eIvPerEntry256: numKeySlotsForType = bspQuery.out.keySlotNumberIvPerEntry256; break;
            default: break;
        }

        pModule->types[i].typeOffset = typeOffset;
        pModule->types[i].numKeySlotsForType = numKeySlotsForType;
        typeOffset += numKeySlotsForType;
    }
...
BERR_Code BHSM_KeySlot_SetMulti2Key( BHSM_Handle hHsm, const BHSM_KeySlotSetMulti2Key *pKeyData )
{
    ...
    if( !hHsm ) { return BERR_TRACE( BERR_INVALID_PARAMETER ); }
    BDBG_OBJECT_ASSERT( hHsm, BHSM_P_Handle );
    if( !pKeyData ) { return BERR_TRACE( BERR_INVALID_PARAMETER ); }
...
static BERR_Code _GetKeyslotOwnership( BHSM_KeyslotHandle handle, BHSM_SecurityCpuContext* pOwner )
{
    ...
    index = pSlot->pModule->types[pSlot->slotType].typeOffset + pSlot->number;
...
/* Associate a pidChannel with a bypass keyslot. */
                                            ...
                                            BHSM_BypassKeySlot_e *pBypassKeyslot )
{
    ...
    BERR_TRACE( BERR_NOT_AVAILABLE ); /* not available */

    return 0;
}

BERR_Code BHSM_P_Keyslot_GetSlotDetails( BHSM_KeyslotHandle handle, BHSM_KeyslotSlotDetails *pSlotDetails )
{
    BHSM_P_KeySlot *pSlot = (BHSM_P_KeySlot*)handle;
    unsigned i = 0;
    ...
    if( !pSlotDetails ) { return BERR_TRACE( BERR_INVALID_PARAMETER ); }

    BKNI_Memset( pSlotDetails, 0, sizeof(*pSlotDetails) );

    pSlotDetails->hHsm = pSlot->pModule->hHsm;
    pSlotDetails->slotType = pSlot->slotType;
    pSlotDetails->number = pSlot->number;

    for( i = 0; i < BHSM_KeyslotBlockType_eMax; i++ ) {
        pSlotDetails->sc01[i].useEntry             = pSlot->settings.sc01[i].useEntry;
        pSlotDetails->sc01[i].outputPolarity.rPipe = pSlot->settings.sc01[i].outputPolarity.rPipe;
        pSlotDetails->sc01[i].outputPolarity.gPipe = pSlot->settings.sc01[i].outputPolarity.gPipe;
    }
                                          ...
                                          BHSM_KeyslotEntryDetails *pEntryDetails )
{
    ...
    BDBG_OBJECT_ASSERT( pSlot, BHSM_P_KeySlot );
    if( !pSlot->configured ) { return BERR_TRACE( BERR_INVALID_PARAMETER ); }
    if( !pEntryDetails ) { return BERR_TRACE( BERR_INVALID_PARAMETER ); }

    pEntry = _GetEntry( handle, entry );
    if( !pEntry ) { return BERR_TRACE( BERR_INVALID_PARAMETER ); }
    if( !pEntry->configured ) { return BERR_TRACE( BERR_INVALID_PARAMETER ); }

    BKNI_Memset( pEntryDetails, 0, sizeof(*pEntryDetails) );

    pEntryDetails->blockType = _GetEntryBlockType( entry );
    pEntryDetails->polarity = _GetEntryPolarity( entry );

    pEntryDetails->bspMapped.keyMode = _Keyslot_MapKeyMode( pEntry->settings.keyMode );
    pEntryDetails->bspMapped.ctrlWord0 = compileControl0_GlobalHi( handle );
    pEntryDetails->bspMapped.ctrlWord1 = compileControl1_GlobalLo( handle );
    pEntryDetails->bspMapped.ctrlWord2 = compileControl2_ModeHi( handle, entry );
    pEntryDetails->bspMapped.ctrlWord3 = compileControl3_ModeLo( handle, entry );

    pEntryDetails->bspMapped.slotType = BHSM_P_ConvertSlotType( pSlot->slotType );
    pEntryDetails->bspMapped.blockType = _GetEntryBlockType( entry );
    pEntryDetails->bspMapped.entryType = _GetEntryPolarity( entry );

    if( pEntryDetails->bspMapped.blockType >= BHSM_KeyslotBlockType_eMax ) return BERR_TRACE( BERR_INVALID_PARAMETER );

    if( pEntry->settings.external.iv ) {

        if( !pEntry->pExternalSlot ) { return BERR_TRACE( BERR_INVALID_PARAMETER ); }

        pEntryDetails->bspMapped.externalIvValid = true;
        pEntryDetails->bspMapped.externalIvOffset = pEntry->pExternalSlot->offsetIv + pEntry->pExternalSlot->slotPtr;
    }
...
uint8_t BHSM_P_ConvertSlotType( BHSM_KeyslotType type )
{
    switch ( type )
    {
        case BHSM_KeyslotType_eIvPerSlot:   return Bsp_KeySlotType_eIvPerSlot128;
        case BHSM_KeyslotType_eIvPerBlock:  return Bsp_KeySlotType_eIvPerBlock128;
        case BHSM_KeyslotType_eIvPerEntry:  return Bsp_KeySlotType_eIvPerEntry256; /* no 128bit per entry  on zeus5 */
        case BHSM_KeyslotType_eIvPerBlock256:  return Bsp_KeySlotType_eIvPerBlock256;
        case BHSM_KeyslotType_eIvPerEntry256:  return Bsp_KeySlotType_eIvPerEntry256;
...
/* Function restricted. Do not use without permission. */
BHSM_KeyslotHandle BHSM_P_GetKeySlotHandle( BHSM_Handle hHsm, BHSM_KeyslotType slotType, unsigned slotNumber )
{
    ...
    pModule = hHsm->modules.pKeyslots;
    typeOffset = pModule->types[slotType].typeOffset;
    numKeySlotsForType = pModule->types[slotType].numKeySlotsForType;

    if( slotNumber >= numKeySlotsForType ) { BERR_TRACE( BERR_INVALID_PARAMETER ); return NULL; }
    if( typeOffset + slotNumber >= BHSM_MAX_KEYSLOTS ) { BERR_TRACE( BERR_INVALID_PARAMETER ); return NULL; }

    return (BHSM_KeyslotHandle)pModule->pKeySlotHandles[typeOffset + slotNumber];
}
...
static uint8_t _Keyslot_MapKeyMode( BHSM_KeySlotKeyMode keyMode )
{
    switch( keyMode )
    {
        case BHSM_KeySlotKeyMode_eRegular: { return Bsp_KeyMode_eRegular; }
        case BHSM_KeySlotKeyMode_eDes56:   { return Bsp_KeyMode_eDes56; }
