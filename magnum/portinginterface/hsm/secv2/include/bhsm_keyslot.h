...
typedef enum
{
    ...
    BHSM_KeySlot_TerminationMode_eCoronado,
...
}BHSM_KeyslotRouteKey;   /* TODO ...deprecated, to be removed */
...
All pid channels must be associated with a keyslot. Pid channels handling encrypted data will
have a regular client managed keyslot associated with it.

For pid channels handling clear data (or encrypted data that is not to be decrypted) the system
will default to a Bypass keyslot capable of transferring data from GLR to GLR.

If a pid channel is handling clear data and has a destination in CRR (with source CRR or GLR),
