...
void BHDM_MONITOR_P_ResetHpdChanges_isr(const BHDM_Handle hHDMI)
{

#if BHDM_CONFIG_MONITOR_HPD_CHANGES

	/* show updates only */
	if (hHDMI->MonitorStatus.MonitoredHpdChanges)
	{
		BDBG_MSG(("HotPlug Changes in last %ds: %d   TOTAL:  %d",
			BHDM_MONITOR_HP_CHANGE_SECONDS,
			hHDMI->MonitorStatus.MonitoredHpdChanges,
			hHDMI->MonitorStatus.TotalHotPlugChanges)) ;
	}

	hHDMI->MonitorStatus.MonitoredHpdChanges = 0 ;
...
void BHDM_MONITOR_P_HpdChanges_isr(BHDM_Handle hHDMI)
{
	...
	/* update Total HotPlug Changes */
	hHDMI->MonitorStatus.TotalHotPlugChanges++ ;

 #if BHDM_CONFIG_MONITOR_HPD_CHANGES
	/* update Number of Hot Plug changes during the monitor period */
	hHDMI->MonitorStatus.MonitoredHpdChanges++ ;

	if (hHDMI->DeviceSettings.HotplugDetectThreshold)
	{
		/* some Rx will pulse the HP when powered off */
		/* ignore Rx HP intrs when Rx is powered OFF */
		if (!hHDMI->rxSensePowerDetected)
		{
			BDBG_MSG(("HP threshold check ignored when HDMI Rx is OFF")) ;
			return ;
		}

		if (hHDMI->MonitorStatus.MonitoredHpdChanges > hHDMI->DeviceSettings.HotplugDetectThreshold)
		{
			BDBG_LOG((" ")) ;
