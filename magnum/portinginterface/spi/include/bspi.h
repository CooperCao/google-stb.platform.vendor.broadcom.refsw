...
typedef enum BSPI_SpiCore
{
    ...
    BSPI_SpiCore_Hif                        /* not supported */
} BSPI_SpiCore;
...
typedef struct BSPI_Delay
{
    unsigned   postTransfer;      /* in units of nano seconds. If useUserDtlAndDsclk = true in BSPI_ChannelSettings,
                                                                This specifies length of delay after transfer. BSPI_SetDTLConfig() is deprecated. */
    unsigned   chipSelectToClock; /* in units of nano seconds. If useUserDtlAndDsclk = true in BSPI_ChannelSettings,
                                                                This specifies length of delay from chip select to clock transition. BSPI_SetRDSCLKConfig() is deprecated. */
} BSPI_Delay;
...
/***************************************************************************
Summary:
    This function sets the number of bits for every data transfer
...
BERR_Code BSPI_SetBitsPerTransfer(
    ...
    uint8_t bitsPerTxfr                 /* Bits per transfer */
    );
...
/***************************************************************************
Summary:
    This function gets the channel delay.

Description:
    This function gets the length of delay after transfer and delay from chip select to clock transition.
    BSPI_SetDTLConfig() and  BSPI_SetRDSCLKConfig() are deprecated.
...
/***************************************************************************
Summary:
    This function sets the channel delay.

Description:
    This function sets the length of delay after transfer and delay from chip select to clock transition.
    BSPI_SetDTLConfig() and  BSPI_SetRDSCLKConfig() are deprecated.
