...
BERR_Code BSAT_7366_P_ResetChannel(BSAT_ChannelHandle h, bool bDisableDemod)
{
   ...
   hab[0] = BHAB_7366_InitHeader(0x34, h->channel, 0, BHAB_7366_MODULE_SAT);
...
BERR_Code BSAT_7366_P_StartPsdScan(BSAT_ChannelHandle h, uint32_t freq, uint32_t adcSelect)
{
   ...
   BDBG_ENTER(BSAT_7366_P_StartPsdScan);

   if ((freq < 250000000UL) || (freq > 2300000000UL))
   {
      BDBG_WRN(("BSAT_7366_P_StartPsdScan(): invalid frequency (%d)", freq));
      goto invalid_parameter;
   }

   if (adcSelect > 3)
   {
      BDBG_WRN(("BSAT_7366_P_StartPsdScan(): invalid adcSelect (%d)", adcSelect));
      invalid_parameter:
      retCode = BERR_TRACE(BERR_INVALID_PARAMETER);
      goto done;
   }
   ...
   hab[0] = BHAB_7366_InitHeader(0x38, h->channel, 0, 0);
   hab[1] = freq;
   hab[2] = adcSelect;
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pImpl->hHab, hab, 4));
   ...
   BDBG_LEAVE(BSAT_7366_P_StartPsdScan);
   return retCode;
}
...
BERR_Code BSAT_7366_P_GetPsdScanStatus(BSAT_ChannelHandle h, BSAT_PsdScanStatus *pStatus)
{
   ...
   BDBG_ENTER(BSAT_7366_P_GetPsdScanStatus);

   hab[0] = BHAB_7366_InitHeader(0x39, h->channel, 0, 0);
   BSAT_7366_CHK_RETCODE(BSAT_7366_P_SendCommand(pDevImpl->hHab, hab, 6));
   ...
   BDBG_LEAVE(BSAT_7366_P_GetPsdScanStatus);
   return retCode;
}
