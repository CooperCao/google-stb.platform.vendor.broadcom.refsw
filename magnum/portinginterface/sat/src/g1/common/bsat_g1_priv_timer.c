...
******************************************************************************/
BERR_Code BSAT_g1_P_EnableTimer_isr(BSAT_ChannelHandle h, BSAT_TimerSelect t, uint32_t my_delay, BSAT_g1_FUNCT func)
{
   BSAT_g1_P_ChannelHandle *hChn = (BSAT_g1_P_ChannelHandle *)h->pImpl;
   BINT_CallbackHandle hCb = NULL;
   uint32_t tmrctl_reg = BCHP_SDS_MISC_TMRCTL;
   uint32_t tmrctl_set = 0, tmrctl_clr = 0;
   uint32_t count_reg, val, delay;
...
#if BCHP_CHIP==45402
   if ((t == BSAT_TimerSelect_eBaud) || (t == BSAT_TimerSelect_eGen3))
   {
      delay = my_delay;
   }
   else
   {
      /* actual sds clock is 112.9 MHz (instead of 108 MHz), 112.9/108=1.045 */
      delay = (uint32_t)((uint64_t)(my_delay * 1046) / (uint64_t)1000);
   }
#else
   delay = my_delay;
#endif
   ...
   if ((func == NULL) && (delay < 1000))
   {
      do
      {
         val = BSAT_g1_P_ReadRegister_isrsafe(h, count_reg);
      } while (val > 0);
   }
