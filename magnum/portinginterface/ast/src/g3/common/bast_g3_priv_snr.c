...
BERR_Code BAST_g3_P_GetSnr(BAST_ChannelHandle h, uint32_t *pSnr)
{
   ...
   retCode = BAST_g3_P_GetSnr_isr(h, pSnr);
...
/******************************************************************************
 BAST_g3_P_GetSnr_isr() - calculates snr in 1/256 dB
******************************************************************************/
BERR_Code BAST_g3_P_GetSnr_isr(BAST_ChannelHandle h, uint32_t *pSnr)
{
   ...
   BAST_g3_P_ToggleBit_isrsafe(h, BCHP_SDS_SNR_SNORECTL, 0x40);
   ...
   if (hChn->acqParams.symbolRate < 10000000)
   {
      ...
      {
         while (BAST_g3_P_IsTimerExpired_isrsafe(h, BAST_TimerSelect_eBer) == false)
            ;
      }
      else
      {
         BDBG_ERR(("BAST_g3_P_GetSnr_isr(): BAST_g3_P_EnableTimer() error"));
      }
#else
   ...
   BAST_g3_P_ReadRegister_isrsafe(h, BCHP_SDS_SNR_SNORESP, &sig_power);
   BAST_g3_P_ReadRegister_isrsafe(h, BCHP_SDS_SNR_SNORETP, &tot_power);
   BAST_DEBUG_SNR(BDBG_MSG(("BAST_g3_P_GetSnr_isr(): sig_power=%u, tot_power=%u", sig_power, tot_power)));
   ...
   BAST_DEBUG_SNR(BDBG_MSG(("BAST_g3_P_GetSnr_isr(): adjust: snr_power=%u, M=%d, N=%d", snr_power, M, N)));
   ...
   BAST_DEBUG_SNR(BDBG_MSG(("BAST_g3_P_GetSnr_isr(): err_power=%u, snr_power=%u", err_power, snr_power)));
   ...
   BAST_DEBUG_SNR(BDBG_MSG(("BAST_g3_P_GetSnr_isr(): target=0x%08X, idx=%d, snr_lo=%u, snr_hi=%u", target, idx, snr_lo, snr_hi)));
   ...
   Q_lo = (Q_lo + 1) >> 1;
   snr256 = snr_lo + Q_lo;
   BAST_DEBUG_SNR(BDBG_MSG(("BAST_g3_P_GetSnr_isr(): snr256=%u", snr256)));
...
/******************************************************************************
 BAST_g3_P_GetSnr_isr() - returns SNR in units of 1/256 dB
******************************************************************************/
BERR_Code BAST_g3_P_GetSnr_isr(BAST_ChannelHandle h, uint32_t *pSnr)
{
   BAST_g3_P_ChannelHandle *hChn = (BAST_g3_P_ChannelHandle *)h->pImpl;
   ...
   BAST_g3_P_ReadRegister_isrsafe(h, BCHP_SDS_SNR_SNRE, &snre);
