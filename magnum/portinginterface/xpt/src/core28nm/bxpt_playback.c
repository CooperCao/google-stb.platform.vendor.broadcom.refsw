    ...
    )
{
    ...
    MCPB_CHNL_WRITE( hPb, XPT_MCPB_CH0_DMA_BBUFF_CTRL, CRC_ERROR_PAUSE_EN, hPb->settings.pauseAtCrcMismatch ? 1 : 0 );
    ...
    )
{
    ...
    descWords[4] &= ~(7 << 8);
    if(flags->muxFlags.crcLoad)
        descWords[4] |= 1 << 10;
    if(flags->muxFlags.crcCompare)
        descWords[4] |= 1 << 9;
    if(flags->muxFlags.crcCompute)
        descWords[4] |= 1 << 8;
    descWords[9] = flags->muxFlags.crc;
    ...
    )
{
    ...
    return BXPT_Playback_P_ReadReg( hPb, BCHP_XPT_MCPB_CH0_DMA_BBUFF_CRC );
}
