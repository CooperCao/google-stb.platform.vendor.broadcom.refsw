    ...
    )
{
    ...
    if( ChannelNo > BXPT_NUM_REMULTIPLEXORS )
    {
        ...
        (!BXPT_P_InputBandIsSupported(ChannelSettings->OutputClock - BXPT_RemuxClock_eIb0)) )
    {
        BDBG_ERR(( "OutputClock %lu is not supported!", ( unsigned long ) ChannelSettings->OutputClock ));
        ExitCode = BERR_INVALID_PARAMETER;
        goto done;
    }
...
#endif
    BXPT_P_AcquireSubmodule(hXpt, BXPT_P_Submodule_eRemux);
    ...
    )
{
    ...
    BXPT_P_ReleaseSubmodule(hRmx->vhXpt, BXPT_P_Submodule_eRemux);
}
