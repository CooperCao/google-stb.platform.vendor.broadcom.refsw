    ...
    )
{
    ...
    Config->crcLoad = descWords[4] & (1 << 10) ? true : false;
    Config->crcCompare = descWords[4] & (1 << 9) ? true : false;
    Config->crcCompute = descWords[4] & (1 << 8) ? true : false;
    Config->crc = descWords[9];
}
#endif
