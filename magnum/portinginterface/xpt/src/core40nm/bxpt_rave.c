...
#define RAVE_REG_1_OFFSET           ( BCHP_XPT_RAVE_CX0_RAVE_Reg_1 - BCHP_XPT_RAVE_CX0_AV_CDB_WRITE_PTR )
#define RAVE_REG_2_OFFSET           ( BCHP_XPT_RAVE_CX0_RAVE_Reg_2 - BCHP_XPT_RAVE_CX0_AV_CDB_WRITE_PTR )
#define RAVE_REG_3_OFFSET           ( BCHP_XPT_RAVE_CX0_RAVE_Reg_3 - BCHP_XPT_RAVE_CX0_AV_CDB_WRITE_PTR )
#define RAVE_REG_4_OFFSET           ( BCHP_XPT_RAVE_CX0_RAVE_Reg_4 - BCHP_XPT_RAVE_CX0_AV_CDB_WRITE_PTR )
#define RAVE_REG_5_OFFSET           ( BCHP_XPT_RAVE_CX0_RAVE_Reg_5 - BCHP_XPT_RAVE_CX0_AV_CDB_WRITE_PTR )
    ...
    )
{
    ...
    /* No RDB defines for this, since it's temporary. */
    Reg = BREG_Read32( hCtx->hReg, hCtx->BaseAddr + AV_MISC_CFG5_OFFSET );
    remap->Enable = Reg & ( 1 << 26 ) ? true : false;

    Reg = BREG_Read32( hCtx->hReg, hCtx->BaseAddr + RAVE_REG_4_OFFSET );
    remap->NewIdFor0xC0 = (unsigned char) ((Reg >> 24) & 0xFF);
    remap->NewIdFor0xC1 = (unsigned char) ((Reg >> 16) & 0xFF);
    remap->NewIdFor0xC2 = (unsigned char) ((Reg >> 8) & 0xFF);
    remap->NewIdFor0xC3 = (unsigned char) (Reg & 0xFF);

    Reg = BREG_Read32( hCtx->hReg, hCtx->BaseAddr + RAVE_REG_5_OFFSET );
    remap->NewIdFor0xC4 = (unsigned char) ((Reg >> 8) & 0xFF);
    remap->NewIdFor0xC5 = (unsigned char) (Reg & 0xFF);
}
    ...
    )
{
    ...
    Reg = (unsigned) (remap->NewIdFor0xC0) << 24;
    Reg |= (unsigned) (remap->NewIdFor0xC1) << 16;
    Reg |= (unsigned) (remap->NewIdFor0xC2) << 8;
    Reg |= (unsigned) (remap->NewIdFor0xC3);
    BREG_Write32( hCtx->hReg, hCtx->BaseAddr + RAVE_REG_4_OFFSET, Reg );

    Reg = BREG_Read32( hCtx->hReg, hCtx->BaseAddr + RAVE_REG_5_OFFSET );
    Reg &= ~( 0xFFFF );
    Reg |= (unsigned) (remap->NewIdFor0xC4) << 8;
    Reg |= (unsigned) (remap->NewIdFor0xC5);
    BREG_Write32( hCtx->hReg, hCtx->BaseAddr + RAVE_REG_5_OFFSET, Reg );
}

static void InitPesRemapping(
    BXPT_RaveCx_Handle hCtx
    )
{
    BXPT_RAVE_P_PesRemap remap;
    ...
    remap.Enable = false;
    BXPT_RAVE_P_SetPesRemapping( hCtx, &remap );
}
#endif
    ...
    )
{
    ...
    ThisCtx->ItbFormat = BAVC_ItbEsType_eAvsVideo;
...
/* See SW7228-42 for details regarding what is done here and why. */
    ...
    )
{
    ...
    BXPT_RaveCx_Handle ThisDecCtx = NULL;
    BXPT_RaveCx_Handle ThisRefCtx = NULL;

    BERR_Code rc;

    BDBG_ASSERT( hRave );
    BDBG_ASSERT( DecodeContext );
    BDBG_ASSERT( ReferenceContext );

    /* Find two free SCDs. the +5 requirement comes from RAVE ASF support, which is unused in SW */
    for (referenceScdNum=6, decodeScdNum=referenceScdNum+5; decodeScdNum<BXPT_NUM_SCD; referenceScdNum++, decodeScdNum++) {
        if (hRave->ScdTable[referenceScdNum].Allocated == false && hRave->ScdTable[decodeScdNum].Allocated == false) { break; }
    }
    if (decodeScdNum == BXPT_NUM_SCD) {
        BDBG_ERR(("No SCD channel is available"));
        return BERR_TRACE(BXPT_ERR_NO_AVAILABLE_RESOURCES);
    }

    /* Get two RAVE decode contexts. The reference must be the lower-numbered of the two. Unlike the SCDs, they don't have to be consecutive. */
    if( GetNextRaveContext( hRave, pReferenceCxSettings->RequestedType, &referenceIndex ) != BERR_SUCCESS )
    {
    ...
    ThisDecCtx->avsRefHandle = ThisRefCtx;
    ThisRefCtx->avsDecHandle = ThisDecCtx;

    rc = setupAvsContext( hRave, pReferenceCxSettings->RequestedType, &(pReferenceCxSettings->BufferCfg), referenceIndex, ReferenceContext );
   ...
   rc = setupAvsContext( hRave, pDecodeCxSettings->RequestedType, &(pDecodeCxSettings->BufferCfg), decodeIndex, DecodeContext );
    ...
    SetScdNum( ThisDecCtx, 0, decodeScdNum );
    ...
    )
{
    ...
    BDBG_MSG(( "AllocContext_Priv: context %u (%s), CDB size %u, ITB size %u", Index,
        ...
        BufferCfg->Cdb.Length, BufferCfg->Itb.Length
        ));
    ...
    )
{
    ...
    /****
    In Rave firmware v2.21, a fix was added to the RAVE fw to correct a bug in timestamp processing.
    This caused a problem on RVU clients during rainfade (see CSP837171 and CSP943299).
    Setting bit 7 disables this fix.
    */
    Reg |= 0x00000080;
#endif
    ...
    )
{
    ...
    */
    ...
    if (ItbFormat==BAVC_ItbEsType_eAvsVideo) {
#if BXPT_P_HAS_AVS_PLUS_WORKAROUND
        if (Context->avsDecHandle || Context->avsRefHandle) {
            Reg |= (1 << 13); /* AV_MISC_CONFIG2[13] = 1 for both contexts */
        }
        if (Context->avsRefHandle) { /* this is the decode context */
            Reg |= (1 << 12); /* AV_MISC_CONFIG2[12] = 1 for decode context only */
        }
#elif BXPT_P_HAS_AVS_PLUS_WORKAROUND_TWO
        Reg |= (1 << 12);
#else
        BDBG_WRN(("RAVE configured for AVS/AVS+, but incomplete AVS/AVS+ support in XPT"));
#endif
    }
    else {
        Reg &= ~(3 << 12); /* reset */
    ...
    switch( ItbFormat )
    {
        ...
        EmulationPrevRemove = Context->avsDecHandle ? false : true; /* emu prevention is off for ref context. otherwise on */
    ...
    )
{
    ...
    if(Name == BXPT_RaveIntName_eLastCmd)
    {
    ...
    )
{
    ...
    ** multiplied by 4 to make a 32-bit offset into the queue area, and multiplied by 2
    ** because now the LOC_0 offsets have an MSB and LSB .
    */
    BREG_Write32( hCtx->hReg, SpliceQueueDmemAddr + ( WrPtr * 2 * 4 ) + SPLICE_QUEUE_LOC_0_MSB_OFFSET, (SplicePidChannel>>8)&0xFF );
    BREG_Write32( hCtx->hReg, SpliceQueueDmemAddr + ( WrPtr * 2 * 4 ) + SPLICE_QUEUE_LOC_0_LSB_OFFSET, SplicePidChannel&0xFF );
    WrPtr++;
...
#if BXPT_SW7425_4528_WORKAROUND
    BXPT_RAVE_P_GetPesRemapping( hCtx, &remap );
#endif
    ...
    )
{
   ...
   BMMA_DeviceOffset src_valid_copy, src_wrap_copy, dest_read_copy;
    ...
    if (DestCtx->SoftRave.mode == BXPT_RaveSoftMode_ePointersOnly)
    {
        ...
        BDBG_MSG(("advance rave ITB: %x, %x, %x", (unsigned)src_valid, (unsigned)src_wrap, (unsigned)dest_read));
    ...
    else
    {
     ...
     /*
        ...
        */
        ...
        BDBG_MSG(("copy ITB: %x --> %x to %x", cur_src_itb, (unsigned)src_valid, (unsigned)dest_valid));
