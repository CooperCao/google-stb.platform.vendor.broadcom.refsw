	...
	)
{
	...
	if( PidChannelNum >= BXPT_P_MAX_SAM_PID_CHANNEL )
	{
		/* Bad PID channel number. Complain. */
		BDBG_ERR(( "PidChannelNum %lu is out of range for SAM filtering!", ( unsigned long ) PidChannelNum ));
	...
	else
	{
		...
		uint32_t RegAddr = BCHP_XPT_MSG_MCAST_16_ADDR_i_ARRAY_BASE + ( 16 * PidChannelNum );
	...
	)
{
	...
	Reg = BREG_Read32( hXpt->hRegister, RegAddr );
	Reg &= ~(
		BCHP_MASK( XPT_MSG_PID_CTRL2_TABLE_i, PHY_EN ) |
		BCHP_MASK( XPT_MSG_PID_CTRL2_TABLE_i, NET_EN ) |
		BCHP_MASK( XPT_MSG_PID_CTRL2_TABLE_i, M40_EN ) |
		BCHP_MASK( XPT_MSG_PID_CTRL2_TABLE_i, M24_EN ) |
		BCHP_MASK( XPT_MSG_PID_CTRL2_TABLE_i, M16_EN ) |
		BCHP_MASK( XPT_MSG_PID_CTRL2_TABLE_i, ID_REJECT_EN ) |
		BCHP_MASK( XPT_MSG_PID_CTRL2_TABLE_i, SPECIAL_NOT ) |
		BCHP_MASK( XPT_MSG_PID_CTRL2_TABLE_i, SPECIAL_TYPE ) 
	);
	Reg |= (
		BCHP_FIELD_DATA( XPT_MSG_PID_CTRL2_TABLE_i, PHY_EN, Enables->PhysicalEn == true ? 1 : 0 ) |
		BCHP_FIELD_DATA( XPT_MSG_PID_CTRL2_TABLE_i, NET_EN, Enables->NetworkEn == true ? 1 : 0 ) |
		BCHP_FIELD_DATA( XPT_MSG_PID_CTRL2_TABLE_i, M40_EN, Enables->M40En == true ? 1 : 0 ) |
		BCHP_FIELD_DATA( XPT_MSG_PID_CTRL2_TABLE_i, M24_EN, Enables->M24En == true ? 1 : 0 ) |
		BCHP_FIELD_DATA( XPT_MSG_PID_CTRL2_TABLE_i, M16_EN, Enables->M16En == true ? 1 : 0 ) |
		BCHP_FIELD_DATA( XPT_MSG_PID_CTRL2_TABLE_i, ID_REJECT_EN, Enables->IdRejectEn == true ? 1 : 0 ) |
		BCHP_FIELD_DATA( XPT_MSG_PID_CTRL2_TABLE_i, SPECIAL_NOT, Enables->SamNotEn == true ? 1 : 0 ) |
