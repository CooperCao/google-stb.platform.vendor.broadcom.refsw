...
BERR_Code BSRF_g1_Tuner_P_Init(BSRF_ChannelHandle h)
{
   ...
   /* set default center freq and disable notch */
   BSRF_g1_P_Tune(h, BSRF_DEFAULT_FC_HZ);
   BSRF_g1_P_Notch(h, false, 0, 0);

#if (BCHP_CHIP==89730)
   /* enable clk48 and release reset for IQ output */
   BSRF_P_WriteRegister(h, BCHP_SRFE_FE_CLK_CTRL, 0x00000007);
   BSRF_P_WriteRegister(h, BCHP_SRFE_FE_RST_CTRL, 0x00000000);
#endif
