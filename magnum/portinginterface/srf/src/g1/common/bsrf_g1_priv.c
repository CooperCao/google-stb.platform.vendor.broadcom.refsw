...
)
{
   ...
   chG1->rfagcSettings.decaySettings.timeNs = 1200000;
...
BERR_Code BSRF_g1_P_PowerUp(BSRF_ChannelHandle h)
{
   ...
   /* TBD hangs cpu if agc frozen with high power input */
#endif
...
{
   BDBG_ASSERT(h);
   BDBG_ENTER(BSRF_g1_P_BoostMixPllCurrent);

   /* enable mixpll current boost */
   BSRF_g1_Ana_P_BoostMixPllCurrent(h, bBoost);

   BDBG_LEAVE(BSRF_g1_P_BoostMixPllCurrent);
   return BERR_SUCCESS;
}
...
void BSRF_g1_P_WinDetect_isr(void *p, int param)
{
   ...
   BINT_DisableCallback(hChn->hWinDetectCb);
#endif
}
