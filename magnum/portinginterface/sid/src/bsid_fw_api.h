...
#define BSID_FW_API_VERSION     0x00010003
#define BSID_FW_VERSION         0x00010003
...
typedef enum  {
  ...
  ERR_CODE_INSUFFICIENT_INPUT_DATA = 0x7006,
...
typedef struct BSID_Cmd_GetStreamInfo {
   ...
   uint32_t     sidMemStreamInfoBufVirtAddr; /* FIXME: Not Used - remove */
   uint32_t     sidMemStreamInfoBufSize;     /* FIXME: Not Used - remove */
   uint32_t     uiMetadataIndex;             /* FIXME: corresponds to streamInfoBufVirtAddr in FW API */
   uint32_t     bypass;
   uint32_t     lastDMAchunk;
   uint32_t     dbgMsgOn;
