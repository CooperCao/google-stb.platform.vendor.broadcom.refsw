...
/******************************************************************************
* Function name: BSID_P_LoadCode
*
* Comments:
*   NOTE: This loads chunks of a fixed pre-determined size that corresponds to
*   a chunk size as provided to the utility that creates the FW image.
*   It is up to the provided image interface to ensure that it supplied that
*   data.  This loader should have no dependency on the FW image interface.
*
******************************************************************************/
BERR_Code BSID_P_LoadCode(BSID_Handle hSid)
{
    ...
    for(uiChunk = 1; /* read until hit EOF marker */; uiChunk++)
    {
        ...
        if ((size_t)pImageData & (sizeof(uint32_t)-1))
        {
           BDBG_WRN(("Firmware loader expects chunks to be 32-bit aligned (got %p)", pImageData));
        }
