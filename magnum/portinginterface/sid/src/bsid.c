    ...
    const BSID_Settings *psOpenSettings)
{
   ...
   /* Check for Image Interface/Context */
    ...
    /* FIXME: ultimately, this information will come from InitCommand Response (is API version always same as FW version?) */
    BDBG_LOG(("[%d] FW v%d.%d.%d.%d (0x%08x) [HW v????]",
            psOpenSettings->ui32_SidInstance,
            ( stMyRevisionInfo.ui32_FWVersion >> 24 ) & 0xFF,
            ( stMyRevisionInfo.ui32_FWVersion >> 16 ) & 0xFF,
            ( stMyRevisionInfo.ui32_FWVersion >>  8 ) & 0xFF,
            ( stMyRevisionInfo.ui32_FWVersion >>  0 ) & 0xFF,
            stMyRevisionInfo.ui32_FWVersion));
