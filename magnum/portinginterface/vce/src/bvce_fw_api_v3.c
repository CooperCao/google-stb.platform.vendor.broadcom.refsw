...
#define HEVC_MAX_CPB_LEVEL_40  (1000*12000)
#define HEVC_MAX_CPB_LEVEL_41  (1000*20000)
#define HEVC_MAX_CPB_LEVEL_50  (1000*25000)
...
uint32_t BVCE_FW_P_CalcNonSecureMem ( const BVCE_FW_P_CoreSettings_t *pstCoreSettings, const BVCE_FW_P_NonSecureMemSettings_t *pstMemSettings )
{
    ...
    if( InputType == ENCODER_INPUT_TYPE_PROGRESSIVE )
    {
        ...
        size_of_reconstructed_luma = bvceEpmCalcStripeBufferSize( max_horizontal_size_in_pels , ((max_vertical_size_in_pels + 63) & 0xffffffc0) , NonDCXVBuffer , InputType , DramStripeWidth , X , Y);
        size_of_reconstructed_chroma = bvceEpmCalcStripeBufferSize( max_horizontal_size_in_pels , (((max_vertical_size_in_pels >> 1) + 63) & 0xffffffc0)  , NonDCXVBuffer , InputType , DramStripeWidth , X , Y);
...
uint32_t X,Y;
    ...
    DcxvEnable = pstMemSettings->DcxvEnable;
		...
		ReconstructedLumaBuffSize = bvceV3EpmCalcStripeBufferSize(MaxWidthInPel , ((MaxHeightInPel + 63) & 0xffffffc0)  , NonDCXVBuffer , !IsProgressive , StripeWidth , X , Y, DramConfigPageSize);
		ReconstructedLumaBuffSize = ALIGN12BIT32(ReconstructedLumaBuffSize);
		ReconstructedChromaBuffSize =         bvceV3EpmCalcStripeBufferSize(MaxWidthInPel , (((MaxHeightInPel >> 1) + 63) & 0xffffffc0) , NonDCXVBuffer , !IsProgressive , StripeWidth , X , Y, DramConfigPageSize);
