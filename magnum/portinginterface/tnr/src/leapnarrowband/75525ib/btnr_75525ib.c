...
BDBG_MODULE(btnr_75525);

#define DEV_MAGIC_ID                ((BERR_TNR_ID<<16) | 0xFACE)
#define BTNR_75525_SETTINGS_I2CADDR  (0x6C)          /* 7bit addr */
#define BTNR_75525_MAX_CHANNELS      1

/*******************************************************************************/
BERR_Code BTNR_75525Ib_Open(
    BTNR_Handle *phDev,                 /* [output] Returns handle */
    BTNR_75525Ib_Settings *pSettings, /* [Input] settings structure */
    BHAB_Handle hHab                  /* [Input] Hab Handle */
    )
{
    BERR_Code retCode = BERR_SUCCESS;
    BTNR_LeapIb_Handle h75525Dev;
    BTNR_P_LeapIb_Settings *pTnrImplData;
    BTNR_Handle hDev;

    BDBG_ENTER(BTNR_75525_Open);
    BDBG_ASSERT( hHab );

    hDev = NULL;
    /* Alloc memory from the system heap */
    h75525Dev = (BTNR_LeapIb_Handle) BKNI_Malloc( sizeof( BTNR_P_LeapIb_Handle ) );
    if( h75525Dev == NULL )
    {
        retCode = BERR_TRACE(BERR_OUT_OF_SYSTEM_MEMORY);
        BDBG_ERR(("BTNR_75525_Open: BKNI_malloc() failed\n"));
        goto done;
    }
    BKNI_Memset( h75525Dev, 0x00, sizeof( BTNR_P_LeapIb_Handle ) );
    ...
    h75525Dev->magicId = DEV_MAGIC_ID;
    pTnrImplData = &h75525Dev->settings;
    pTnrImplData->rfFreq = 0;
    pTnrImplData->tunerMode = BTNR_TunerMode_eLast;

    h75525Dev->channelNo = pSettings->channelNo;

    h75525Dev->hHab = hHab;

    hDev->hDevImpl = (void *) h75525Dev;
    ...
    )
{
    ...
    pDefSettings->i2cAddr = BTNR_75525_SETTINGS_I2CADDR;
    pDefSettings->mxChnNo = BTNR_75525_MAX_CHANNELS;
