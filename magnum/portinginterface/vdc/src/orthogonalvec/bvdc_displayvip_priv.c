	  ...
	  BAVC_EncodePictureBuffer *pPicture )
{
	...
	if(pBuffer) { /* move from captureQ to deliverQ */
		...
		BDBG_MODULE_MSG(BVDC_DISP_VIP,("[%d] GET pic[%u] buf[%d]: %ux%u%c", hVip->eId, pPicture->ulPictureId, pBuffer->ulBufferId,
	  ...
	  BAVC_Polarity                eFieldPolarity )
{
	...
	/* if STG is disabled, put VIP back to auto drain mode */
	if(!hDisplay->stCurInfo.bEnableStg) {
		BVDC_P_VIP_GET_REG_DATA(VICE2_VIP_0_0_CONFIG) = 0;
	}
	/* In cases:
	    1) not FULL and
	    2.1) non-ignore, or
