...
#ifndef BVDC_FOR_BOOTUPDATER  /* BFMT won't know properties of eMaxVideoFmt */
            ...
            if( !hDisplay->stCfcLutList.hMmaBlock[i] )
            {
                ...
                while(i--)
                {
                    if(hDisplay->stCfcLutList.hMmaBlock[i])
                    {
                        ...
                        BMMA_UnlockOffset(hDisplay->stCfcLutList.hMmaBlock[i] ,
                            hDisplay->stCfcLutList.ullStartDeviceAddr[i]);
      ...
      uint32_t                         ulSumOfTaps )
{
    ...
    if (bUseCh0)
    {
        ...
        if(bOverride)
        {
            ...
            {
                BKNI_Memcpy(
                    hDisplay->stNewInfo.aaulUserVfFilterCo[0],
                    paulFilterRegs,
                    BVDC_P_CHROMA_TABLE_SIZE * sizeof(uint32_t));
                hDisplay->stNewInfo.aulUserVfFilterCoSumOfTaps[0] =
                    ulSumOfTaps;
            }
            else
            {
                BKNI_Memcpy(
                    hDisplay->stNewInfo.aaulUserVfFilterCvbs[0],
                    paulFilterRegs,
                    BVDC_P_CHROMA_TABLE_SIZE * sizeof(uint32_t));
                hDisplay->stNewInfo.aulUserVfFilterCvbsSumOfTaps[0] =
                    ulSumOfTaps;
            }
        }
    }
    ...
    if (bUseCh1)
    {
        ...
        if(bOverride)
        {
            ...
            {
                BKNI_Memcpy(
                    hDisplay->stNewInfo.aaulUserVfFilterCo[1],
                        paulFilterRegs,
                        BVDC_P_CHROMA_TABLE_SIZE * sizeof(uint32_t));
                hDisplay->stNewInfo.aulUserVfFilterCoSumOfTaps[1] =
                    ulSumOfTaps;
            }
            else
            {
                BKNI_Memcpy(
                    hDisplay->stNewInfo.aaulUserVfFilterCvbs[1],
                        paulFilterRegs,
                        BVDC_P_CHROMA_TABLE_SIZE * sizeof(uint32_t));
                hDisplay->stNewInfo.aulUserVfFilterCvbsSumOfTaps[1] =
                    ulSumOfTaps;
            }
        }
    }
    ...
    if (bUseCh2)
    {
        ...
        if(bOverride)
        {
            ...
            {
                BKNI_Memcpy(
                    hDisplay->stNewInfo.aaulUserVfFilterCo[2],
                    paulFilterRegs,
                    BVDC_P_CHROMA_TABLE_SIZE * sizeof(uint32_t));
                hDisplay->stNewInfo.aulUserVfFilterCoSumOfTaps[2] =
                    ulSumOfTaps;
            }
            else
            {
                BKNI_Memcpy(
                    hDisplay->stNewInfo.aaulUserVfFilterCvbs[2],
                    paulFilterRegs,
                    BVDC_P_CHROMA_TABLE_SIZE * sizeof(uint32_t));
                hDisplay->stNewInfo.aulUserVfFilterCvbsSumOfTaps[2] =
                    ulSumOfTaps;
            }
        }
    }
      ...
      uint32_t                        *pulSumOfTaps)
{
    ...
    uint32_t ulUserVfFilterSumOfTaps = 0;
    int32_t lChannel;
    ...
    if (eDisplayOutput == BVDC_DisplayOutput_eComponent)
    {
        ...
        ulUserVfFilterSumOfTaps = hDisplay->stCurInfo.aulUserVfFilterCoSumOfTaps[lChannel];
    }
    else
    {
        ...
        ulUserVfFilterSumOfTaps = hDisplay->stCurInfo.aulUserVfFilterCvbsSumOfTaps[lChannel];
    }
    ...
    if(bOverride)
    {
        ...
        *pulSumOfTaps = ulUserVfFilterSumOfTaps;
    }
    else
    {
        BKNI_EnterCriticalSection();
...
{
    BDBG_ENTER(BVDC_Display_SetBarData);
    BDBG_OBJECT_ASSERT(hDisplay, BVDC_DSP);

    /* set new values */
    hDisplay->stNewInfo.eBarDataMode = eMode;
    hDisplay->stNewInfo.eBarDataType = eType;
    hDisplay->stNewInfo.ulTopLeftBarData  = ulTopLeftBarData;
    hDisplay->stNewInfo.ulBotRightBarData = ulBotRightBarData;

    /* Set display dirty bit */
    hDisplay->stNewInfo.stDirty.stBits.bMiscCtrl = BVDC_P_DIRTY;
    BDBG_LEAVE(BVDC_Display_SetBarData);
    return BERR_SUCCESS;
}
