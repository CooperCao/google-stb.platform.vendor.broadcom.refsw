	  ...
	  BVDC_P_ListInfo                 *pList )
{
	...
	*pList->pulCurrent++ = BRDC_REGISTER(BCHP_VF_0_FORMAT_ADDER + ulOffset);
	...
	else if (VIDEO_FORMAT_IS_625_LINES(pCurInfo->pFmtInfo->eVideoFmt))
	{
		ulVfMiscRegValue |=
			BCHP_FIELD_DATA(VF_0_MISC, BVB_LINE_REMOVE_TOP, 1);
	}
	  ...
	  BVDC_P_ListInfo                 *pList )
{
	...
	*pList->pulCurrent++ =
		...
		BCHP_FIELD_DATA(IT_0_CABLE_DETECT_SEL, LINE,
			(VIDEO_FORMAT_IS_NTSC(pCurInfo->pFmtInfo->eVideoFmt)   ? 2 :
			(VIDEO_FORMAT_IS_HD(pCurInfo->pFmtInfo->eVideoFmt) &&
			 !VIDEO_FORMAT_IS_ED(pCurInfo->pFmtInfo->eVideoFmt))   ? 6 :
			(VIDEO_FORMAT_IS_480P(pCurInfo->pFmtInfo->eVideoFmt)) ? 4 : 1)) |
	  ...
	  bool                             bProgramFull )
{
	...
	BDBG_ASSERT(pRmInfo);
	BDBG_MSG(("\tHdmi_%d's RM PxlClk = %sMhz", pstChan->ulDvi, pRmInfo->pchRate));
	...
	if(bProgramFull)
	{
		...
		*pList->pulCurrent++ =
		...
		if ((VIDEO_FORMAT_IS_ED(pFmtInfo->eVideoFmt)) &&
		    ((BAVC_HDMI_PixelRepetition_e2x == pCurInfo->stHdmiSettings.eHdmiPixelRepetition) ||
	...
	if(bProgramFull)
	{
		...
		*pList->pulCurrent++ =
		...
		BVDC_P_RD_MOD_WR_RUL(pList->pulCurrent,
			...
			BCHP_FIELD_DATA(HDMI_TX_PHY_PLL_CTL_1, FREQ_DOUBLER_ENABLE, pRmInfo->ulRefClkCfg) |
	...
	( BVDC_Display_Handle              hDisplay )
{
	...
	if (pTable == NULL && !VIDEO_FORMAT_IS_4kx2k(pNewInfo->pFmtInfo->eVideoFmt))
	{
	  ...
	  BVDC_P_DisplayDviChan    *pstChan )
{
	...
	if (BVDC_P_HdmiRmTable_isr(
		...
		pDispInfo->stHdmiSettings.stSettings.eColorComponent) == NULL)
	{
