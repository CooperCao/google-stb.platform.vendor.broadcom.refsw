...
{
#if (BCHP_CHIP==7125)  || (BCHP_CHIP==7231)  || (BCHP_CHIP==7325)  || \
	(BCHP_CHIP==7335)  || (BCHP_CHIP==7336)  || (BCHP_CHIP==7340)  || \
	(BCHP_CHIP==7342)  || (BCHP_CHIP==7344)  || (BCHP_CHIP==7346)  || \
	(BCHP_CHIP==7358)  || (BCHP_CHIP==7360)  || (BCHP_CHIP==7362)  || \
	(BCHP_CHIP==7400)  || (BCHP_CHIP==7405)  || (BCHP_CHIP==7408)  || \
...
#elif (BCHP_CHIP==7145) || (BCHP_CHIP==7435)
	/* 4 stg */
	/*  Disp0    Disp1    Disp2    Disp3    disp4   disp5   disp6 */
	        0,       0,       1,       1,       1,      1,       0
...
{
	{ BVDC_P_CMP_0_CLEAR_RECT_V0, BVDC_P_CMP_0_CLEAR_RECT_V1 },
	{ BVDC_P_CMP_1_CLEAR_RECT_V0, BVDC_P_CMP_1_CLEAR_RECT_V1 },
	{ BVDC_P_CMP_2_CLEAR_RECT_V0,                          0 },
	{ BVDC_P_CMP_3_CLEAR_RECT_V0,                          0 },
	{ BVDC_P_CMP_4_CLEAR_RECT_V0,                          0 },
	{ BVDC_P_CMP_5_CLEAR_RECT_V0,                          0 },
	{ BVDC_P_CMP_6_CLEAR_RECT_V0,                          0 }
};
...
{
#if ((BCHP_CHIP==7400) || (BCHP_CHIP==7405) || (BCHP_CHIP==11360))
...
#elif (BCHP_CHIP==7145)
	pRdc->ulMemcIndex = BBOX_MemcIndex_1;
#else
	  ...
	  BVDC_WinMemConfigSettings          *pWindow )
{
	...
	pWindow->ulMadMemcIndex = pWindow->ulMemcIndex;
	...
	( BVDC_P_MemConfig_SystemInfo        *pSystemConfigInfo )
{
	...
	pSystemConfigInfo->bSingleMemc = false;
	return BERR_SUCCESS;
}
	  ...
	  BVDC_P_MemConfig_SystemInfo        *pSystemConfigInfo )
{
	...
	BVDC_P_BufferHeap_GetHeapOrder(pHeapSettings, pHeapSizeInfo, NULL);
		...
		for(ulWinIndex = 0; ulWinIndex < BVDC_MAX_VIDEO_WINDOWS; ulWinIndex++)
		{
			...
			if(pWindow->bUsed)
			{
				if(ulMemcIndex == BBOX_MemcIndex_Invalid)
					pSystemConfigInfo->bSingleMemc = true;
				else if(ulMemcIndex != pWindow->ulMemcIndex)
					pSystemConfigInfo->bSingleMemc = false;
	  ...
	  uint32_t                            ulWinIndex )
{
	...
	if(pSystemConfigInfo->bSingleMemc)
	{
		if(pWinConfigInfo->bLipsync && pWinConfigInfo->bSyncLock)
			ulCapBufCnt += 1;
	}
	else
	{
...
#if BVDC_P_SUPPORT_MOSAIC_DEINTERLACE
	BVDC_P_BufferHeap_GetHeapIdBySize(pHeapSizeInfo, ulBufSize*6, &eBufHeapId);
#else
	BVDC_P_BufferHeap_GetHeapIdBySize(pHeapSizeInfo, ulBufSize, &eBufHeapId);
#endif
	ulIndex = pHeapSizeInfo->aulIndex[eBufHeapId];
