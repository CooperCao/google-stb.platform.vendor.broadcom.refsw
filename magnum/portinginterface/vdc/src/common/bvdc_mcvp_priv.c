	...
	( BVDC_P_Mcvp_Handle               hMcvp )
{
	...
	if (pBoxVdc->astDeinterlacerLimits[hMcvp->eId].ulHeight != BBOX_VDC_DISREGARD)
	{
		if (pBoxVdc->astDeinterlacerLimits[hMcvp->eId].ulHeight > hMcvp->ulMaxHeight)
		{
		...
		{
			hMcvp->ulMaxHeight = pBoxVdc->astDeinterlacerLimits[hMcvp->eId].ulHeight;
		}
	}

	if (pBoxVdc->astDeinterlacerLimits[hMcvp->eId].ulWidth != BBOX_VDC_DISREGARD)
	{
		if (pBoxVdc->astDeinterlacerLimits[hMcvp->eId].ulWidth > hMcvp->ulMaxWidth)
		{
		...
		{
			hMcvp->ulMaxWidth  = pBoxVdc->astDeinterlacerLimits[hMcvp->eId].ulWidth;
		}
	}

	if (pBoxVdc->aulDeinterlacerHsclThreshold[hMcvp->eId] != BBOX_VDC_DISREGARD)
	{
		hMcvp->ulHsclSizeThreshold = pBoxVdc->aulDeinterlacerHsclThreshold[hMcvp->eId];
	}
