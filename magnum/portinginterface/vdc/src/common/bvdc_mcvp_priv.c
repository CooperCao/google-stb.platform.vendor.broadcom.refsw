...
#ifdef BCHP_MVP_TOP_0_REVISION
        /* MAD core with revision less than 0.F does not support dual pixel
           bypass aka is not capable of 4k bypass */
        ulReg = BREG_Read32_isr(pMcvp->hRegister, BCHP_MVP_TOP_0_REVISION + pMcvp->ulRegOffset);
        pMcvp->b4kBypass = (BVDC_P_GET_FIELD(ulReg, MVP_TOP_0_REVISION, MAJOR) == 0 &&
                            BVDC_P_GET_FIELD(ulReg, MVP_TOP_0_REVISION, MINOR) < 0xF) ? false : true;
#endif
      ...
      bool                           bInit )
{
    ...
    if((hMcvp->bAnr) && (bAnr))
    {
        ...
        if(!BVDC_P_CMP_DBV_MODE(hMcvp->hWindow->hCompositor))
#endif
    ...
    if (bEnable)
    {
        ...
        {
            bool bDitherEn =
#if BVDC_CRC_CAPTURE || BVDC_DITHER_OFF
                false;
#else
