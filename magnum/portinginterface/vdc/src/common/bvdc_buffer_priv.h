...
typedef struct
{
    ...
    uint32_t    bRev32Locked              : 1;        /* reverse 3:2-pull-down locked, used with MTG */
...
typedef struct BVDC_P_PictureNode
{
    ...
    bool                               bMosaicMode;
    bool                               bMosaicIntra; /* mosaic incoming rectangle sizes are different*/
...
/***************************************************************************
 * Multibuffer MTG mode
 *
 * This enum defines the three cases of how MTG affects buffer (picture node)
 * handling
 *
...
typedef struct BVDC_P_BufferContext
{
    ...
    /* Only used in MTG cases wherein the source and display are both interlaced,
       have matching rates and there is an active deinterlacer but without
       3:2 lock. Indicates to multibuffer to display the source according to
