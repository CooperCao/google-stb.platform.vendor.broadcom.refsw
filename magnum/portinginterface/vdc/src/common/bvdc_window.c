	  ...
	  const BVDC_Window_Settings      *pDefSettings )
{
	...
	if (BVDC_P_SRC_IS_GFX(hSource->eId))
	{
			...
			pBoxVdc->ulNumXcodeGfd != BBOX_VDC_DISREGARD)
		{
			...
			if (hWindow->hCompositor->hVdc->ulXcodeGfd > pBoxVdc->ulNumXcodeGfd)
			{
				hWindow->hCompositor->hVdc->ulXcodeGfd--;
				BDBG_ERR(("Exceeded allowed number (%d) of GFD used with transcode.", pBoxVdc->ulNumXcodeGfd));
	...
	( BVDC_Window_Handle               hWindow )
{
	...
	if (BVDC_P_SRC_IS_GFX(hSource->eId))
	{
		...
		{
			hWindow->hCompositor->hVdc->ulXcodeGfd--;
			BDBG_ERR(("Decremented xcode GFD usage count [%d].", hWindow->hCompositor->hVdc->ulXcodeGfd));
		}
	  ...
	  uint32_t                        *pulShift )
{
	...
	BVDC_P_Window_GetColorMatrix(hWindow, hWindow->stNewInfo.bUserCsc, &hWindow->stPriCsc.stCscMC,
	  ...
	  const BVDC_Anr_Settings         *pAnrSettings )
{
	...
	BDBG_WRN(("ANR is not supported for this chipset"));
#endif
    ...
    )
{
	...
	BDBG_ASSERT(hBox);
	BDBG_ASSERT(eWindowId <= BVDC_WindowId_eAuto);
	BDBG_ASSERT(hSource);
	BDBG_ASSERT(hCompositor);
	BSTD_UNUSED(hBox);
	...
	pBoxVdcCap     = &hCompositor->hVdc->stBoxConfig.stVdc;
	pBoxVdcDispCap = &pBoxVdcCap->astDisplay[hCompositor->eId];

	/* Get window cores */
	if (pBoxVdcDispCap->bAvailable && pBoxVdcDispCap->astWindow[eWindowId].bAvailable)
	{
		BVDC_P_WindowId eId;
		const BVDC_P_ResourceFeature *pResource;
		const BBOX_Vdc_ResourceFeature *pBoxVdcResource;

		/* Get VDC private video window ID */
		if (eWindowId <= BVDC_WindowId_eVideo1)
		{
			eId = hCompositor->eId + eWindowId;

			/* Also add compositor ID */
			pCoreList->aeCores[BAVC_CoreId_eCMP_0 + hCompositor->eId] = true;

			/* Account for PIP windows in BVDC_P_WindowId enum */
			if (hCompositor->eId > BVDC_CompositorId_eCompositor0)
			{
				if (hCompositor->eId > BVDC_CompositorId_eCompositor1)
				{
					eId += 2;
				}
				else
			...
			pResource = BVDC_P_Window_GetResource(eId);
			pBoxVdcResource = &pBoxVdcDispCap->astWindow[eWindowId].stResource;
			...
			if (pBoxVdcResource->ulMad != BBOX_FTR_INVALID)
			{
				...
				uint32_t ulMad = (pBoxVdcResource->ulMad == BBOX_VDC_DISREGARD)
					? pResource->ulMad : pBoxVdcResource->ulMad;
				...
				{
					if(BVDC_P_ResourceType_eMad == eType)
					{
						pCoreList->aeCores[ulResId + BAVC_CoreId_eMAD_0] = true;
					}
					else
					{
#if (BVDC_P_SUPPORT_MCVP > BVDC_P_SUPPORT_MADR)
						/* Has MCDI */
						pCoreList->aeCores[ulResId + BAVC_CoreId_eMVP_0] = true;
#else
						/* MADR only */
						pCoreList->aeCores[ulResId + BAVC_CoreId_eMAD_0] = true;
#endif
			...
			/* Get CAP, VFD, SCL */
			if (pResource->ulCap != BVDC_P_Able_eInvalid)
			{
				pCoreList->aeCores[eId + BAVC_CoreId_eCAP_0] = true;
			}
			if (pResource->ulVfd != BVDC_P_Able_eInvalid)
			{
				pCoreList->aeCores[eId + BAVC_CoreId_eVFD_0] = true;
			}
			if (pResource->ulScl != BVDC_P_Able_eInvalid)
			{
				pCoreList->aeCores[eId + BAVC_CoreId_eSCL_0] = true;
			}
		}
		else /* Graphics window */
		{
			BDBG_ASSERT(BVDC_P_SRC_IS_GFX(hSource->eId));
			pCoreList->aeCores[hCompositor->eId + BAVC_CoreId_eGFD_0] = true;
		}
	}
	else
	{
		BDBG_ERR(("Window %d is not available for the given box mode."));
		return;
	}

	/* Get source core */
	if (pBoxVdcCap->astSource[hSource->eId].bAvailable)
	{
		if (BVDC_P_SRC_IS_MPEG(hSource->eId))
		{
			pCoreList->aeCores[hSource->eId + BAVC_CoreId_eMFD_0] = true;
		}
	}
