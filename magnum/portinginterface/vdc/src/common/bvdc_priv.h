 ...
 *    ulCanvasCoverage[0]: n = 1
 *    ulCanvasCoverage[1]: 2 <= n <= 3
 *    ulCanvasCoverage[2]: 4 <= n <= 6
 *    ulCanvasCoverage[3]: 7 <= n <= 12
 *
 *  n is the mosaic count
 */
typedef struct
{
	uint32_t   ulCanvasCoverage[4];     /* 7 <= n <= 12 */
...
uint32_t BVDC_P_GetBoxWindowId_isr
	( BBOX_Vdc_Capabilities           *pBoxVdc,
	  BVDC_P_WindowId                  eWindowId );
#define BVDC_P_GetBoxWindowId  BVDC_P_GetBoxWindowId_isr
...
BERR_Code BVDC_P_GetMosaicCoverage
	( BVDC_P_MosaicCanvasCoverage     *pCoverageTbl,
