      ...
      BVDC_Window_Handle            hWindow)
{
    ...
    if (pBoxVdc->astDeinterlacer[hMad->eId].stPictureLimits.ulHeight != BBOX_VDC_DISREGARD)
    {
        if (pBoxVdc->astDeinterlacer[hMad->eId].stPictureLimits.ulHeight > hMad->ulMaxHeight)
        {
        ...
        {
            hMad->ulMaxHeight = pBoxVdc->astDeinterlacer[hMad->eId].stPictureLimits.ulHeight;
        }
    }

    if (pBoxVdc->astDeinterlacer[hMad->eId].stPictureLimits.ulWidth != BBOX_VDC_DISREGARD)
    {
        if (pBoxVdc->astDeinterlacer[hMad->eId].stPictureLimits.ulWidth > hMad->ulMaxWidth)
        {
        ...
        {
            hMad->ulMaxWidth  = pBoxVdc->astDeinterlacer[hMad->eId].stPictureLimits.ulWidth;
        }
    }

    if (pBoxVdc->astDeinterlacer[hMad->eId].ulHsclThreshold != BBOX_VDC_DISREGARD)
    {
        hMad->ulHsclSizeThreshold = pBoxVdc->astDeinterlacer[hMad->eId].ulHsclThreshold;
    }
      ...
      const BFMT_VideoInfo            *pFmtInfo )
{
    ...
    if(BFMT_IS_NTSC(pFmtInfo->eVideoFmt))
    {
      ...
      const BFMT_VideoInfo            *pFmtInfo )
{
...
#endif
    {
        ...
        if(!hWindow->stCurInfo.bRev32Custom) /* Use default set */
        {
            ...
            if((hWindow->stCurInfo.hSource->ulSampleFactor) && BFMT_IS_NTSC(pFmtInfo->eVideoFmt))
            {
                /* PR45231, PR45707 */
    ...
    * unnecessary computations. */
    ...
    if(hMad->ulUpdateAll)
    {
        ...
        BVDC_P_MAD_GET_REG_DATA(MAD_0_MODE_CONTROL_2) |=  (
            ...
            !BFMT_IS_SECAM(pFmtInfo->eVideoFmt))); /* PR45224 */
#endif /* phase 6 */
#endif /* phase >= 5 */
      ...
      BVDC_Deinterlace_Reverse32Settings *pReverse32Settings )
{
    ...
    /* MAD_0_BWV_CONTROL_1.LUMA_32_THRESH */
    if((ulSampleFactor) && BFMT_IS_NTSC(pFmtInfo->eVideoFmt))
    {
