      ...
      int                              iParm2 )
{
    ...
    BVDC_P_BufLog_SetStateAndDumpTrigger(hWindow->hBuffer->pBufLog, eLogState, pfCallback, pvParm1, iParm2);
...
{
    BVDC_P_BufLog_DumpLog(hWindow->hBuffer->pBufLog, pLog, uiSizeToRead, puiReadCount);
}
...
void BVDC_Dbg_Window_SetBufLogManualTrigger
    ( const BVDC_Window_Handle  hWindow )
{
    BVDC_P_BufLog_SetManualTrigger(hWindow->hBuffer->pBufLog);
}
      ...
      bool                             bEnable )
{
    ...
    BVDC_P_BufLog_EnableBufLog(hWindow->hBuffer->pBufLog, hWindow->eId, bEnable);
    return BERR_SUCCESS;
}
#endif
