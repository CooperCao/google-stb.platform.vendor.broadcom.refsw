...
/* Canvas coverage for SD display */
static const BVDC_P_MosaicCanvasCoverage s_SD_CCTbl =
	{{ 200, 100, 100,  60 }};

/* Canvas coverage for 1080p display on 1080p datapath */
static const BVDC_P_MosaicCanvasCoverage s_1080P_CCTbl =
	{{ 100,  50,  50,  50 }};

/* Canvas coverage for 4k display */
static const BVDC_P_MosaicCanvasCoverage s_4K_CCTbl =
	{{ 100,  75,  66,  50 }};
	  ...
	  BVDC_Capabilities               *pCapabilities )
{
	...
	if(pCapabilities)
	{
		...
		pCapabilities->ulNumAtg = pCapabilities->ulNumCmp;
		pCapabilities->ulNumAlgPaths = BVDC_P_MAX_DACS;
		pCapabilities->bAlgStandAlone = true;
#else
	  ...
	  BVDC_P_WindowId                  eWindowId )
{
	...
	switch (eWindowId)
	{
		...
		case 17:
			...
			break;
		default:
			ulBoxWinId = BBOX_VDC_WINDOW_COUNT_PER_DISPLAY;
			BDBG_WRN(("No corresponding BOX window ID for Window %d.", eWindowId));
			break;
	}
	  ...
	  BVDC_P_MosaicCanvasCoverage     *pCoverageTbl )
{
	...
	/* Get coverage table */
	if(eBoxmodeMaxVideoFmt == BBOX_VDC_DISREGARD)
	{
		/* Legacy: Not specified in boxmode. Just use display Id */
		if(eDisplayId == BVDC_DisplayId_eDisplay0)
			*pCoverageTbl = s_SD_CCTbl;
		else
			*pCoverageTbl = s_1080P_CCTbl;
	}
	else if(VIDEO_FORMAT_IS_SD(eBoxmodeMaxVideoFmt))
	{
		*pCoverageTbl = s_SD_CCTbl;
	}
	else
	{
		if(VIDEO_FORMAT_IS_4kx2k(eBoxmodeMaxVideoFmt))
		{
#if (BCHP_CHIP == 7366)
			...
			{
				/* 4k Data path */
				*pCoverageTbl = s_4K_CCTbl;
			}
		}
		else
		{
			/* TODO: 7366 Boxmode 1: 4k display, 1080p CAP */
			*pCoverageTbl = s_1080P_CCTbl;
		}
		...
		if(bMode9)
		{
			for(i = 0; i < 4; i++)
			{
				pCoverageTbl->ulCanvasCoverage[i] =
					(pCoverageTbl->ulCanvasCoverage[i] * BVDC_P_MODE9_NORM_FACTOR) / BVDC_P_PERCENTAGE_FACTOR;
			}
		}
#endif
	}
...
{
	uint32_t               i, ulCoverage;

	BDBG_ASSERT(ulRectsCount);
	BDBG_ASSERT(ulRectsCount <= 12);

	/* Get index to coverage table */
	if(ulRectsCount == 1)
		i = 0;
	else if((2 <= ulRectsCount) && (ulRectsCount <= 3))
		i = 1;
	else if((4 <= ulRectsCount) && (ulRectsCount <= 6))
		i = 2;
	else
		i = 3;

	ulCoverage = pCoverageTbl->ulCanvasCoverage[i];
