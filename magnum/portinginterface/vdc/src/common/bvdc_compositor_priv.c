	...
	if(BVDC_P_CMP_OFFSET_COMPARE_FIELD_DATA(CMP_0_V0_SURFACE_CTRL, ulV0V1Offset, SURFACE_ENABLE, 1))
	{
		...
		if(hCompositor->ulCscAdjust[eVId])
		{
			...
			if(hCompositor->ahWindow[eVId]->stCurInfo.bClearRect)
			{
				...
				{
					BVDC_P_Window_BuildCscRul_isr(hCompositor, eWinInCmp, i, &hWindow->astMosaicCscList[i], hWindow->bBypassCmpCsc, pList);
				}
...
#endif
			{
				BVDC_P_Window_BuildCscRul_isr(hCompositor, eWinInCmp, 0, &hWindow->stPriCsc, hWindow->bBypassCmpCsc, pList);
			}

			{
				BVDC_P_Window_BuildCscRul_isr(hCompositor, eWinInCmp, 1, &hWindow->stSecCsc, hWindow->bBypassCmpCsc, pList);
			}
