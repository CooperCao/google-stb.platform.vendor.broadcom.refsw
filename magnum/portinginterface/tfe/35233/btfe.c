...
)
{
   ...
   hDev->pChannels = (BTFE_P_ChannelHandle **)BKNI_Malloc(BTFE_35233_MAX_CHANNELS * sizeof(BTFE_P_ChannelHandle *));
   BDBG_ASSERT(hDev->pChannels);
   hDev->totalChannels = BTFE_35233_MAX_CHANNELS;
   for (i = 0; i < BTFE_35233_MAX_CHANNELS; i++)
...
{
   BSTD_UNUSED(hTFE);
   *totalChannels = BTFE_35233_MAX_CHANNELS;
   return BERR_SUCCESS;
}
...
)
{
   ...
   BDBG_OBJECT_ASSERT(hTFE, BTFE);
   BDBG_ASSERT(chnNo < BTFE_35233_MAX_CHANNELS);   
