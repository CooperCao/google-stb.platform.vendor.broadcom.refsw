 ...
 * $brcm_Date: $
 *
 * Module Description:  Contains prototypes and constants used by scd_hal.c
 *
 * Revision History:
 *
 * $brcm_Log: $
...
SCD_RESULT BTFE_P_HalWriteChip(uint32_t instance, uint32_t nonDfeAccess, uint32_t offset, uint32_t length, uint8_t *buffer);
SCD_RESULT BTFE_P_HalReadChip(uint32_t instance, uint32_t nonDfeAccess, uint32_t offset, uint32_t length, uint8_t *buffer);
