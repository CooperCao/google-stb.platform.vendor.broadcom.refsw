 ...
 * $brcm_Date: $
 *
 * Module Description:  Contains SCD internal globals and constants
 *
 * Revision History:
 *
 * $brcm_Log: $
...
typedef SCD_RESULT (*SCD_CHIP_WRITE_FUNC)(SCD_HANDLE chip_handle, uint32_t aper, uint32_t offset, uint32_t data_len, uint8_t *buffer);
typedef SCD_RESULT (*SCD_CHIP_READ_FUNC)(SCD_HANDLE chip_handle, uint32_t aper, uint32_t offset, uint32_t data_len, uint8_t *buffer);
