...
DFE core on any ASIC based Broadcom chip, such as BCM35230 and future ones. 
...
/***************************************************************************
Summary:
   This structure defines J83 channel scan mode and scan result

Description:
   This structure defines J83 A and B channel scan input parameter and scan result. The scan result
   bSpectralInversion, constellation, symbolRate, carrierOffset are valid only when bSignalDetected
   is true.

See Also:
   BTFE_TryNextScan()
***************************************************************************/
typedef struct BTFE_J83ChannelScan
{
...
/***************************************************************************
Summary:
   This structure defines Dvb-C channel scan mode and scan result

Description:
   This structure defines Dvb-C channel scan input parameter and scan result. The scan result
   symbolRate, carrierOffset are valid only when bSignalDetected is true.

See Also:
   BTFE_J83ChannelScan(), BTFE_TryNextScan()
***************************************************************************/
typedef struct BTFE_DvbcScanData
{
    int32_t		symbolRate;
    uint8_t		numFeqSpan;		/* number of items in the frequency span array */
    int32_t		freqSpanArray[8];	/* frequency span array: used in seraching for signal presence */
    int32_t		logSweepFreq;	/* sweep frequency: used in the log search for carrier offset */
} BTFE_DvbcScanData;

typedef struct BTFE_ChannelScan
{
    uint8_t		numScanData;	/* number of items in the scanData array */
    BTFE_DvbcScanData *pScanData; /* scanData based on the symbolRate */
...
typedef struct BTFE_ConstellationDetect
{
   uint8_t		numConstellation;	/* number of items in the constellationArray */
   uint32_t 	constellationArray[8];
   uint32_t	symbolRate;
   
   /* search result */
   bool        	bConstellationDetected;
   uint32_t	constellation;
   bool        	bSpectralInversion;
} BTFE_ConstellationDetect;
...
Description:
   Tells the BCM35230 to start channel acquisition.
Returns:
   BERR_Code
******************************************************************************/
...
/***************************************************************************
Summary:
   Get memory allocation requirements for ISDB-T.

Description:
   This function is used to query the DFE core's external memory requirements
   for ISDB-T acquisition.  After memory allocation, BTFE_SetConfig() should be
   called specifying the physical address of the memory block in 
   BTFE_ConfigItem_eISDBT_MemBuffer.
...
/******************************************************************************
Summary:
   Initiates the next symbol operation in the symbol rate verify.
Description:
   This function is only used in J83ABC acquisitions where acqConfig is 
   BTFE_AcquireConfig_eSymbolRateVerify.  This function triggers the J83ABC
   acquisition state machine to try the next symbol rate specified in the 
   BTFE_ConfigJ83abc struct.  When the operation completes, the scan event will
   be set.
Returns:
   BERR_Code
******************************************************************************/
...
/******************************************************************************
Summary:
   Initiates the next symbol operation in the symbol rate verify, and constellation search
Description:
   This function is only used in J83ABC channel scan. This function triggers the J83ABC
   acquisition state machine to try the next symbol rate specified in the 
   BTFE_ConfigJ83abc struct.  When the operation completes, the scan result will
   be set.
Returns:
   BERR_Code
******************************************************************************/
...
Summary:
   Initiates the next symbol operation in the symbol rate verify, and constellation search
Description:
   This function is only used in DVB-C channel scan. This function triggers the J83ABC
   acquisition state machine to try the next symbol rate specified in the 
   BTFE_ConfigJ83abc struct.  When the operation completes, the scan result will
   be set.
Returns:
