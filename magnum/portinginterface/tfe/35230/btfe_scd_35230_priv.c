 ...
 * $brcm_Date: $
 *
 * Module Description:  SCD Chip Interface for 35230
 *
 * Revision History:
 *
 * $brcm_Log: $
...
static SCD_RESULT BTFE_P_ChipDoAdc1Setup(SCD_RESULT ret_val, SCD_HANDLE chip_handle)
{
   ...
   if(ret_val == SCD_RESULT__OK)
   {
        ...
        MODIFYFLD(temp8, MISC_MODULATION_FMT_RESET_CONTROL_2, soft_reset_adc1_fifo, 1);
        temp |= BTFE_P_ChipSetReg8(ret_val, chip_handle, ixMISC_MODULATION_FMT_RESET_CONTROL_2, temp8);
        MODIFYFLD(temp8, MISC_MODULATION_FMT_RESET_CONTROL_2, soft_reset_adc1_fifo, 0);
        temp |= BTFE_P_ChipSetReg8(ret_val, chip_handle, ixMISC_MODULATION_FMT_RESET_CONTROL_2, temp8);
...
*/
        ...
        if(vector)
        {
            ...
            MODIFYFLD(temp8, MISC_CORE_PADS_OE_CNTRL, pgm_agc_pads_oen, 0);
        ...
        else
        {
            ...
            ret_val = BTFE_P_ChipGetReg8(ret_val, chip_handle, ixMISC_CORE_PADS_OE_CNTRL, &temp8);
            MODIFYFLD(temp8, MISC_CORE_PADS_OE_CNTRL, pgm_agc_pads_oen, 1);
            ret_val = BTFE_P_ChipSetReg8(ret_val, chip_handle, ixMISC_CORE_PADS_OE_CNTRL, temp8);
...
static SCD_RESULT BTFE_P_ChipGetFat(SCD_HANDLE chip_handle, SCD_HANDLE handle, SCD_STATUS__FAT *data, uint32_t size)
{
    ...
    if(GET_HANDLE_TYPE(handle) == SCD_HANDLE_FAT)
    {
        ...
        if(data->Flags & SCD_STATUS_FAT__EQUALIZER_SNR)        
        {
            if(cofdm)
            {
                if((ret_val = BTFE_P_ChipGetReg8(ret_val, chip_handle, ixOFDM_CHC_SNR, &temp8)) == SCD_RESULT__OK)
                {
                    data->EqualizerSNR = temp8;
                }
            }
            else if (analog)
	     {
	...
	{						
        ...
        if(data->Flags & SCD_STATUS_FAT__CARRIER_OFFSET)
        {
		   ...
		   SCD_OFDM_MODE fftMode;
		   int freqFactor; 
		   int carrierOffset = 0;
		   
		   if((ret_val = BTFE_P_ChipDoMicroService(ret_val, chip_handle, SVC_GET_COFDM_STATUS, 0, NO_ARGS, 6, tempBuffer, SNR_TIMEOUT)) == SCD_RESULT__OK)            
			    carrierOffset = (tempBuffer[0] << 24) | (tempBuffer[1] << 16) | (tempBuffer[2] << 8) | (tempBuffer[3]);

		   bandwidth = tchip->Acquisition.bandWidthConfig;
		   /* fftMode = tchip->unifiedCofdm.mode; */
		   BTFE_P_ChipGetRegs(ret_val, chip_handle, ixHI_UNIFIED_COFDM_STATUS_AREA+1, 1, tempBuffer);
		   fftMode = (SCD_OFDM_MODE) tempBuffer[0];

		   freqFactor = 0;
		   if (tchip->unifiedCofdm.ofdmStandard == OFDM_STANDARD_DVBT)
		   {
		       if (bandwidth == SCD_BW_6MHZ)
		       {
		   	    if (fftMode == OFDM_MODE_8K)
		   		freqFactor = 86; /* 0.857 */
		   	    else if (fftMode == OFDM_MODE_2K)
		   		freqFactor = 343; /* 3.425 */
		       }
		       else if (bandwidth == SCD_BW_7MHZ)
		       {
		   	    if (fftMode == OFDM_MODE_8K)
		   		freqFactor = 1; /* 1 */
		   	    else if (fftMode == OFDM_MODE_2K)
		       ...
		       else if (bandwidth == SCD_BW_8MHZ)
		       {
		   	    if (fftMode == OFDM_MODE_8K)
		   		freqFactor = 114; /* 1.143 */
		   	    else if (fftMode == OFDM_MODE_2K)
		   		freqFactor = 457; /* 4.567 */
		       }
		   }
		   else /* ISDB-T */
		   {
		   	if (fftMode == OFDM_MODE_2K)
				freqFactor = 4;
		   	else if (fftMode == OFDM_MODE_4K)
				freqFactor = 2;
		   	else if (fftMode == OFDM_MODE_8K)
				freqFactor = 1;			
		   }

		   if (freqFactor == 0)
		   {
			BDBG_ERR(("GetFATstatus returning ERROR, bandwidth %d, fftMode %d !!!", bandwidth, fftMode));
			return SCD_RESULT__ARG_OUT_OF_RANGE;
                  }
		   
		    /*
		     * CarrierOffset = regVal * freqfactor * (1000 /(2 **14 ))
		     *
		     * freqFactor is multiplied by 100 if value > 10. The second part is to multiple 1000/ (2**14). 
		     * To avoid int32 overflow we do *10/(2**4) and *100/(2*10)
		     */
		    if (freqFactor > 10)
		        data->CarrierOffset = (((((carrierOffset * freqFactor) / 100) * 10) >> 4) *100) >> 10;
		    else 
		        data->CarrierOffset = (((((carrierOffset * freqFactor)) * 10) >> 4) *100) >> 10;				
            }
            else if (analog)
            {				
		...
		/* 1. Get the SNR values */
            ...
            if(cofdm)
            {
                ret_val = BTFE_P_ChipGetReg8(ret_val, chip_handle, ixOFDM_CHC_SNR, &temp8);
                snr = (int32_t) temp8;
            }
            else if (analog)
	    {
...
*/
    ...
    case SCD_HANDLE_FAT:
		...
		{
			MODIFYFLD(regVal8, MISC_CORE_PADS_OE_CNTRL, pgm_agc_pads_oen, 0);    /* 0 = Enable */
...
SCD_RESULT BTFE_P_X230AddChip(uint32_t chip_instance, uint32_t fat_tuner_instance, uint32_t fdc_tuner_instance, SCD_HANDLE *chip_handle, uint8_t *pMicroCode)
{
    SCD_RESULT r;
    ...
    if((r = BTFE_P_ScdAddFat(chip_instance, fat_tuner_instance, "X230 FAT", 0)) != SCD_RESULT__OK) return r;
