...
#if BCHP_CHIP == 35230
#include "bchp_35230.h"
#elif BCHP_CHIP == 35125
#include "bchp_35125.h"
#endif
...
*****************************************************************************/
#if (BTFE_FW == 1)
   #include "X230hex_Euro.c"  /* X320 uCode array */
#elif (BTFE_FW == 2)
   #include "X230hex_IFD.c"  /* X320 uCode array */
#else
   #include "X230hex.c"  /* X320 uCode array */
#endif
...
static uint8_t *g_X230micro[SCD_MAX_CHIP] = {0, 0};
...
SCD_RESULT BTFE_P_HalInitialize(uint32_t flags, void *reg_handle)
{
    ...
    */
    ...
    g_X230micro[CHIP_0] = X230micro;

    if (BTFE_P_X230AddChip(CHIP_0, CHIP_0_FAT_TUNER_NXPTD1636F_INSTANCE, CHIP_0_FDC_TUNER_INSTANCE, &g_chip_handle[CHIP_0], g_X230micro[CHIP_0]) != SCD_RESULT__OK) return SCD_RESULT__CHIP_NOT_AVAILABLE;
