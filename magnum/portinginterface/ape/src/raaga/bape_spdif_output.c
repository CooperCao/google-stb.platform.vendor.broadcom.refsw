...
static BERR_Code BAPE_SpdifOutput_P_Enable_IopOut(BAPE_OutputPort output)
{
    ...
    BDBG_MODULE_MSG(bape_fci, ("Linking SPDIF OUTPUT FCI Source"));
    BDBG_MODULE_MSG(bape_fci, ("  fci %x -> SPDIF %d [%d] ", output->sourceMixerFci.ids[0], handle->index, 0));
...
static BERR_Code BAPE_SpdifOutput_P_OpenHw_IopOut(BAPE_SpdifOutputHandle handle)
{
    ...
    BAPE_Reg_P_UpdateField(handle->deviceHandle, BAPE_SPDIF_Reg_P_GetAddress(BCHP_AUD_FMM_IOP_OUT_SPDIF_0_SPDIF_RAMP_BURST, handle->index), AUD_FMM_IOP_OUT_SPDIF_0_SPDIF_RAMP_BURST, STEPSIZE, 0);
