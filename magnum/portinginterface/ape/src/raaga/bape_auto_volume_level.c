...
{
    BERR_Code errCode;
    BDSP_Raaga_AVLStatus dspStatus;
    unsigned i;
    ...
    errCode = BDSP_Stage_GetStatus(handle->hStage, &dspStatus, sizeof(dspStatus));
    if ( errCode != BERR_SUCCESS )
    {
        BDBG_ERR(("Unable to query FW AVL status"));
        return BERR_TRACE(errCode);
    }

    if ( dspStatus.ui32StatusValid != 1 )
    {
        BDBG_WRN(("FW AVL status is not valid at this time"));
    ...
    pStatus->pts = dspStatus.ui32TimeSnapshot45KHz;
