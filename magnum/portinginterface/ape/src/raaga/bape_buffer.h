...
#else
#define BAPE_FLUSHCACHE_ISRSAFE(block, mem, size) do {BSTD_UNUSED((block)); BSTD_UNUSED((mem)); BSTD_UNUSED((size));} while(0)
#endif
...
typedef struct BAPE_BufferSettings
{
    ...
    BMMA_Block_Handle bufferBlock;      /* Must be populated if pBuffer != NULL */
} BAPE_BufferSettings;
