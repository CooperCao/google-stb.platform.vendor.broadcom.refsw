    ...
    )
{
    ...
    *pHandle = NULL;
    ...
    for (i = 0; i < pSettings->numBuffers; i++)
    {
        ...
        BKNI_Memset(hPlayback->pBuffer[i], 0, sizeof(BAPE_BufferInterface));
        BAPE_FLUSHCACHE_ISRSAFE(hPlayback->block[i], hPlayback->pBuffer[i], sizeof(BAPE_BufferInterface));
