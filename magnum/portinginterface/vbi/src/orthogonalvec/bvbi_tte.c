...
#if (BVBI_NUM_TTE_656 >= 1)
static BERR_Code BVBI_P_TT_Enc_656_Program_isr (
    BREG_Handle hReg,
    uint8_t hwCoreIndex,
    bool bActive,
    bool bXserActive,
...
static void BVBI_P_TT_Enc_Init_isr (
    BREG_Handle hReg, uint8_t hwCoreIndex);
#endif
#if (BVBI_NUM_TTE_656 >= 1)
static void BVBI_P_TT_Enc_656_Init_isr (
    BREG_Handle hReg, uint8_t hwCoreIndex);
#endif
...
void BVBI_P_TT_Enc_Init (BREG_Handle hReg, uint8_t hwCoreIndex)
{
    ...
    BVBI_P_TT_Enc_Init_isr (hReg, hwCoreIndex);
...
{
    BKNI_EnterCriticalSection();
    BVBI_P_TT_Enc_656_Init_isr (hReg, hwCoreIndex);
    BKNI_LeaveCriticalSection();
}
#endif
 ...
 *
 */
void BVBI_P_TT_Enc_Init_isr (BREG_Handle hReg, uint8_t hwCoreIndex)
{
    BDBG_ENTER(BVBI_P_TT_Enc_Init_isr);

    BVBI_P_VIE_SoftReset_isrsafe (hReg, false, hwCoreIndex, BVBI_P_SELECT_TT);
 ...
 *
 */
void BVBI_P_TT_Enc_656_Init_isr (BREG_Handle hReg, uint8_t hwCoreIndex)
{
    BDBG_ENTER(BVBI_P_TT_Enc_656_Init_isr);

    BVBI_P_VIE_SoftReset_isrsafe (hReg, true, hwCoreIndex, BVBI_P_SELECT_TT);

    BDBG_LEAVE(BVBI_P_TT_Enc_656_Init_isr);
}
#endif
...
)
{
    ...
    retval = BVBI_P_TT_Enc_Program_isr (
        hReg, hwCoreIndex, bActive, bXserActive,
...
{
    BERR_Code retval;
    BDBG_ENTER(BVBI_P_TT_Enc_656_Program);
    BKNI_EnterCriticalSection();
    retval = BVBI_P_TT_Enc_656_Program_isr (
        hReg, hwCoreIndex, bActive, bXserActive,
        eVideoFormat, tteShiftDirMsb2Lsb, xserSettings, topData, botData);
    BKNI_LeaveCriticalSection();
    BDBG_LEAVE(BVBI_P_TT_Enc_656_Program);
    return retval;
}
...
)
{
    ...
    BDBG_ENTER(BVBI_P_TT_Enc_656_Program_isr);
    ...
    if (ulCoreOffset == 0xFFFFFFFF)
    {
        /* This should never happen!  This parameter was checked by
           BVBI_Encode_Create() */
        BDBG_LEAVE(BVBI_P_TT_Enc_656_Program_isr);
        return BERR_TRACE(BERR_INVALID_PARAMETER);
    }
...
#if (BVBI_P_HAS_XSER_TT >= 1)
    iSerialPort = (bXserActive ?
        BCHP_TTE_ANCIL_0_control_serial_port_ENABLE :
        BCHP_TTE_ANCIL_0_control_serial_port_DISABLE);
    switch (xserSettings->xsSerialDataContent)
    {
    case BVBI_TTserialDataContent_None:
        iSerialPortMode = BCHP_TTE_ANCIL_0_control_serial_port_mode_DATA_ONLY;
        iSerialPort = BCHP_TTE_ANCIL_0_control_serial_port_DISABLE;
        break;
    case BVBI_TTserialDataContent_DataOnly:
        iSerialPortMode = BCHP_TTE_ANCIL_0_control_serial_port_mode_DATA_ONLY;
        break;
    case BVBI_TTserialDataContent_DataMag:
        iSerialPortMode = BCHP_TTE_ANCIL_0_control_serial_port_mode_MAGAZINE_DATA;
        break;
    case BVBI_TTserialDataContent_DataMagFrm:
        iSerialPortMode =
            BCHP_TTE_ANCIL_0_control_serial_port_mode_FRM_MAGAZINE_DATA;
        break;
    case BVBI_TTserialDataContent_DataMagFrmRun:
        iSerialPortMode =
            BCHP_TTE_ANCIL_0_control_serial_port_mode_RUNIN_FRM_MAGAZINE_DATA;
        break;
    default:
        iSerialPortMode = BCHP_TTE_ANCIL_0_control_serial_port_mode_DATA_ONLY;
        /* This should never happen!  This parameter was checked by
    ...
    if (tteShiftDirMsb2Lsb)
        ulShiftDir = BCHP_TTE_ANCIL_0_control_shift_direction_MSBToLSB;
    else
        ulShiftDir = BCHP_TTE_ANCIL_0_control_shift_direction_LSBToMSB;

    /* Start programming the TTE control register */
    ulControlReg = BREG_Read32 (hReg, BCHP_TTE_ANCIL_0_control + ulCoreOffset);
    ulControlReg &= ~(
#if (BVBI_P_HAS_XSER_TT >= 1)
        BCHP_MASK       (TTE_ANCIL_0_control, serial_port_mode         ) |
        BCHP_MASK       (TTE_ANCIL_0_control, serial_port              ) |
#endif
        BCHP_MASK       (TTE_ANCIL_0_control, constant_phase           ) |
        BCHP_MASK       (TTE_ANCIL_0_control, anci656_enable           ) |
        BCHP_MASK       (TTE_ANCIL_0_control, anci656_output_fc        ) |
        BCHP_MASK       (TTE_ANCIL_0_control, shift_direction          ) |
        BCHP_MASK       (TTE_ANCIL_0_control, enable_tf                ) |
        BCHP_MASK       (TTE_ANCIL_0_control, enable_bf                ) );
    ulControlReg |= (
#if (BVBI_P_HAS_XSER_TT >= 1)
         BCHP_FIELD_DATA (TTE_ANCIL_0_control, serial_port_mode,
                                                     iSerialPortMode) |
         BCHP_FIELD_DATA (TTE_ANCIL_0_control, serial_port, iSerialPort) |
#endif
        BCHP_FIELD_DATA (TTE_ANCIL_0_control, constant_phase,         0) |
        BCHP_FIELD_DATA (TTE_ANCIL_0_control, anci656_enable,         1) |
        BCHP_FIELD_DATA (TTE_ANCIL_0_control, anci656_output_fc,      1) |
        BCHP_FIELD_DATA (TTE_ANCIL_0_control, shift_direction, ulShiftDir) |
        BCHP_FIELD_DATA (TTE_ANCIL_0_control, enable_tf,              1) |
        BCHP_FIELD_DATA (TTE_ANCIL_0_control, enable_bf,              1) );

    /* Program the TTE top mask register */
    BREG_Write32 (hReg, BCHP_TTE_ANCIL_0_top_mask + ulCoreOffset, 0x0);

    /* Program the TTE bottom mask register */
    BREG_Write32 (hReg, BCHP_TTE_ANCIL_0_bottom_mask + ulCoreOffset, 0x0);
    ...
    case BFMT_VideoFmt_ePAL_M:
        ...
        /* Continue programming the control register */
        ulControlReg &= ~(
            BCHP_MASK       (TTE_ANCIL_0_control, start_delay         ) |
            BCHP_MASK       (TTE_ANCIL_0_control, teletext_mode       ) );
        ulControlReg |= (
            BCHP_FIELD_DATA (TTE_ANCIL_0_control, start_delay,    0x1F) |
            BCHP_FIELD_ENUM (TTE_ANCIL_0_control, teletext_mode, NABTS) );
    ...
    case BFMT_VideoFmt_eSECAM_H:
        ...
        ulControlReg &= ~(
            ...
            BCHP_MASK       (TTE_ANCIL_0_control, teletext_mode             ) );
        ulControlReg |= (
            BCHP_FIELD_DATA (TTE_ANCIL_0_control, start_delay,          0x00) |
            BCHP_FIELD_ENUM (TTE_ANCIL_0_control, teletext_mode, ETSTeletext) );
    ...
    offset = topData->mmaData.pHwAccess;
    ullAddressReg =
        BCHP_FIELD_DATA (TTE_ANCIL_0_read_address_top, SlabAddress, offset);
    BREG_WriteAddr (hReg,
        BCHP_TTE_ANCIL_0_read_address_top    + ulCoreOffset, ullAddressReg);
    offset = botData->mmaData.pHwAccess;
    ullAddressReg =
        BCHP_FIELD_DATA (TTE_ANCIL_0_read_address_bottom, SlabAddress, offset);
    BREG_WriteAddr (hReg,
        BCHP_TTE_ANCIL_0_read_address_bottom + ulCoreOffset, ullAddressReg);
    ...
    /* write the register with updated values */
    BREG_Write32 (
        hReg, BCHP_TTE_ANCIL_0_control       + ulCoreOffset, ulControlReg);
...
*/
    ...
    */
    ...
    if (ulCoreOffset == 0xFFFFFFFF)
    {
        /* This should never happen!  This parameter was checked by
           BVBI_Encode_Create() */
        BDBG_LEAVE(BVBI_P_TT_Encode_656_Data_isr);
        return BERR_TRACE(BERR_INVALID_PARAMETER);
    }

    H_ReAdTop = BCHP_TTE_ANCIL_0_read_address_top + ulCoreOffset;
    H_ReAdBot = BCHP_TTE_ANCIL_0_read_address_bottom + ulCoreOffset;
    H_MaskTop = BCHP_TTE_ANCIL_0_top_mask + ulCoreOffset;
    H_MaskBot = BCHP_TTE_ANCIL_0_bottom_mask + ulCoreOffset;
    H_Lines   = BCHP_TTE_ANCIL_0_lines_active + ulCoreOffset;
    H_Status  = BCHP_TTE_ANCIL_0_status + ulCoreOffset;
    ...
    {
        /* Check for hardware busy */
        if ((ulStatusReg & BCHP_MASK (TTE_ANCIL_0_status, data_sent_tf)) == 0)
        {
            ulErrInfo |= BVBI_LINE_ERROR_TELETEXT_OVERRUN;
            goto done;
        }
        ...
        /* Will clear hardware status */
        ulStatusReg = BCHP_MASK (TTE_ANCIL_0_status, data_sent_tf);

        /* Give hardware a new place to encode data from */
        ullAddressReg = BCHP_FIELD_DATA (
            TTE_ANCIL_0_read_address_top, SlabAddress, pMmaData->pHwAccess);
        BREG_WriteAddr (hReg, H_ReAdTop, ullAddressReg);
        ...
        *(uint32_t*)(pMmaData->pSwAccess) = lineMask;
        ...
        /* Continue programming the lines_active register */
        ulLinesReg &= ~BCHP_MASK (TTE_ANCIL_0_lines_active, startline_tf);
        ulLinesReg |=  BCHP_FIELD_DATA (
            TTE_ANCIL_0_lines_active, startline_tf,
            pTTDataNext->firstLine + usStartLineTF);
    ...
    {
        /* Check for hardware busy */
        if ((ulStatusReg & BCHP_MASK (TTE_ANCIL_0_status, data_sent_bf)) == 0)
        {
            ulErrInfo |= BVBI_LINE_ERROR_TELETEXT_OVERRUN;
            goto done;
        }
        ...
        /* Will clear hardware status */
        ulStatusReg = BCHP_MASK (TTE_ANCIL_0_status, data_sent_bf);

        /* Give hardware a new place to encode data from */
        ullAddressReg = BCHP_FIELD_DATA (
            TTE_ANCIL_0_read_address_bottom, SlabAddress, pMmaData->pHwAccess);
        BREG_WriteAddr (hReg, H_ReAdBot, ullAddressReg);
        ...
        *(uint32_t*)(pMmaData->pSwAccess) = lineMask;
        ...
        /* Continue programming the lines_active register */
        ulLinesReg &= ~BCHP_MASK (TTE_ANCIL_0_lines_active, startline_bf);
        ulLinesReg |=  BCHP_FIELD_DATA (
            TTE_ANCIL_0_lines_active, startline_bf,
            pTTDataNext->firstLine + usStartLineBF);
    }
    ...
    bool bEnable)
{
    ...
    BDBG_ENTER(BVBI_P_TT_Encode_656_Enable_isr);
    ...
    if (ulCoreOffset == 0xFFFFFFFF)
    {
        /* This should never happen!  This parameter was checked by
           BVBI_Encode_Create() */
        BDBG_LEAVE(BVBI_P_TT_Encode_656_Enable_isr);
        return BERR_TRACE(BERR_INVALID_PARAMETER);
    }
    ...
    {
        ulControlReg |= (
            BCHP_FIELD_DATA (TTE_ANCIL_0_control, enable_tf, 0) |
            BCHP_FIELD_DATA (TTE_ANCIL_0_control, enable_bf, 0) );
    }
    BREG_Write32 ( hReg, BCHP_TTE_ANCIL_0_control + ulCoreOffset, ulControlReg );
...
#if (BVBI_NUM_TTE >= 2)
    case 1:
        ulCoreOffset = (BCHP_TTE_1_REG_START - BCHP_TTE_0_REG_START);
        break;
#endif
#if (BVBI_NUM_TTE >= 3)
    case 2:
        ulCoreOffset = (BCHP_TTE_2_REG_START - BCHP_TTE_0_REG_START);
        break;
#endif
    default:
        break;
...
static uint32_t P_GetCoreOffset_656_isrsafe (uint8_t hwCoreIndex)
{
    ...
    case 1:
        ulCoreOffset =
            (BCHP_TTE_ANCIL_1_REG_START - BCHP_TTE_ANCIL_0_REG_START);
        break;
#endif
#if (BVBI_NUM_TTE_656 >= 3)
    case 2:
        ulCoreOffset =
            (BCHP_TTE_ANCIL_2_REG_START - BCHP_TTE_ANCIL_0_REG_START);
        break;
#endif
    default:
        break;
