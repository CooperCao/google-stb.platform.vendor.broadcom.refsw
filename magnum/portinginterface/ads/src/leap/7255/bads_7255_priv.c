...
*
*******************************************************************************/
BERR_Code BADS_7255_P_EventCallback_isr(
    ...
    )
{
    ...
    BDBG_ENTER(BADS_7255_ProcessNotification);
    BDBG_ASSERT( hChn );
    ...
    switch (event) {
        ...
        case BHAB_Interrupt_eQamAsyncStatusReady:
            ...
            break;
        default:
            BDBG_WRN((" unknown event code from 7255"));
            break;
    }

    BDBG_LEAVE(BADS_7255_P_EventCallback_isr);
    return retCode;
}
    ...
    )
{
    ...
    BADS_Handle hDev;
    unsigned int chnIdx;
    BADS_7255_Handle hImplDev = NULL;
    uint16_t chipVer;

    BDBG_ENTER(BADS_7255_Open);
    ...
    hImplDev = (BADS_7255_Handle) BKNI_Malloc( sizeof( BADS_P_7255_Handle ) );
    if( hImplDev == NULL )
    {
        retCode = BERR_TRACE(BERR_OUT_OF_SYSTEM_MEMORY);
    ...
    BKNI_Memset( hImplDev, 0x00, sizeof( BADS_P_7255_Handle ) );
    ...
    *pAds = hDev;
    ...
    BDBG_LEAVE(BADS_7255_Open);
    return retCode;
}

BERR_Code BADS_7255_Close(
    BADS_Handle hDev                    /* [in] Device handle */
    )
{
    BERR_Code retCode = BERR_SUCCESS;

    BDBG_ENTER(BADS_7255_Close);
    BDBG_ASSERT( hDev );
    ...
    BDBG_LEAVE(BADS_7255_Close);
    return retCode;
}

BERR_Code BADS_7255_Init(
    BADS_Handle hDev                    /* [in] Device handle */
    )
{
    BERR_Code retCode = BERR_SUCCESS;
    BADS_7255_Handle hImplDev;
    BFEC_SystemVersionInfo  versionInfo;

    BDBG_ENTER(BADS_7255_Init);
    BDBG_ASSERT( hDev );
    BDBG_ASSERT( hDev->magicId == DEV_MAGIC_ID );

    hImplDev = (BADS_7255_Handle) hDev->pImpl;
    ...
    BDBG_LEAVE(BADS_7255_Init);
done:
    return retCode;
}

BERR_Code BADS_7255_GetVersion(
    BADS_Handle hDev,                   /* [in] Device handle */
    ...
    )
{
    ...
    BDBG_ENTER(BADS_7255_GetVersion);
    BDBG_ASSERT( hDev );
    BDBG_ASSERT( hDev->magicId == DEV_MAGIC_ID );
    ...
    *pVersion = hImplDev->verInfo;      /* use saved data */
    ...
    BDBG_LEAVE(BADS_7255_GetVersion);
    return retCode;
}

BERR_Code BADS_7255_GetVersionInfo(
    BADS_Handle hDev,              /* [in] Device handle */
    ...
    )
{
    ...
    BDBG_ENTER(BADS_7255_GetVersionInfo);
    BDBG_ASSERT( hDev );
    BDBG_ASSERT( hDev->magicId == DEV_MAGIC_ID );
    ...
    BDBG_LEAVE(BADS_7255_GetVersionInfo);

done:
    return retCode;
}

BERR_Code BADS_7255_GetTotalChannels(
    BADS_Handle hDev,                   /* [in] Device handle */
    ...
    )
{
    ...
    BDBG_ENTER(BADS_7255_GetTotalChannels);
    BDBG_ASSERT( hDev );
    BDBG_ASSERT( hDev->magicId == DEV_MAGIC_ID );
    ...
    *totalChannels = hImplDev->mxChnNo; /* use saved data */
    ...
    BDBG_LEAVE(BADS_7255_GetTotalChannels);
    return retCode;
}

BERR_Code BADS_7255_GetChannelDefaultSettings(
    BADS_Handle hDev,                       /* [in] Device handle */
    ...
    )
{
    ...
    BDBG_ENTER(BADS_7255_GetChannelDefaultSettings);
    BDBG_ASSERT( hDev );
    BDBG_ASSERT( hDev->magicId == DEV_MAGIC_ID );
    ...
    {
        pChnDefSettings->ifFreq = 0;    /*The if freq for 7255 is set to 0 as the internal tuner does not spit out seperate if frequency. */
    ...
    BDBG_LEAVE(BADS_7255_GetChannelDefaultSettings);
    return retCode;
}

BERR_Code BADS_7255_OpenChannel(
    BADS_Handle hDev,                                   /* [in] Device handle */
    ...
    )
{
    BERR_Code retCode = BERR_SUCCESS;
    BADS_7255_Handle hImplDev;
    BADS_7255_ChannelHandle hImplChnDev = NULL;
    BADS_ChannelHandle hChnDev;

    BDBG_ENTER(BADS_7255_OpenChannel);
    BDBG_ASSERT( hDev );
    BDBG_ASSERT( hDev->magicId == DEV_MAGIC_ID );
    ...
    if( channelNo < hImplDev->mxChnNo )
    {
        if( hImplDev->hAdsChn[channelNo] == NULL )
        {
            ...
            hImplChnDev = (BADS_7255_ChannelHandle) BKNI_Malloc( sizeof( BADS_P_7255_ChannelHandle ) );
            if( hImplChnDev == NULL )
            {
                retCode = BERR_TRACE(BERR_OUT_OF_SYSTEM_MEMORY);
            ...
            BKNI_Memset( hImplChnDev, 0x00, sizeof( BADS_P_7255_ChannelHandle ) );
    ...
    BDBG_LEAVE(BADS_7255_OpenChannel);
    return retCode;
}

BERR_Code BADS_7255_CloseChannel(
    ...
    )
{
    ...
    BDBG_ENTER(BADS_7255_CloseChannel);
    BDBG_ASSERT( hChn );
    BDBG_ASSERT( hChn->magicId == DEV_MAGIC_ID );
    ...
    hAds = hChn->hAds;
    hImplDev = (BADS_7255_Handle) hAds->pImpl;
    BDBG_ASSERT( hImplDev );
    ...
    BDBG_LEAVE(BADS_7255_CloseChannel);
    return retCode;
}

BERR_Code BADS_7255_GetDevice(
    BADS_ChannelHandle hChn,            /* [in] Device channel handle */
    ...
    )
{
    ...
    BDBG_ENTER(BADS_7255_GetDevice);
    BDBG_ASSERT( hChn );
    BDBG_ASSERT( hChn->magicId == DEV_MAGIC_ID );

    *phDev = hChn->hAds;

    BDBG_LEAVE(BADS_7255_GetDevice);
    return retCode;
}

BERR_Code BADS_7255_GetDefaultAcquireParams(
...
BERR_Code BADS_7255_SetAcquireParams(
    BADS_ChannelHandle hChn ,           /* [in] Device channel handle */
    ...
    )
{
    ...
    BDBG_ENTER(BADS_7255_SetAcquireParams);
    BDBG_ASSERT( hChn );
    BDBG_ASSERT( hChn->magicId == DEV_MAGIC_ID );
    ...
    BDBG_LEAVE(BADS_7255_SetAcquireParams);
    return retCode;
}

BERR_Code BADS_7255_GetAcquireParams(
    BADS_ChannelHandle hChn ,           /* [in] Device channel handle */
    ...
    )
{
    ...
    BDBG_ENTER(BADS_7255_GetAcquireParams);
    BDBG_ASSERT( hChn );
    BDBG_ASSERT( hChn->magicId == DEV_MAGIC_ID );
    ...
    BDBG_LEAVE(BADS_7255_GetAcquireParams);
    return retCode;
}


BERR_Code BADS_7255_Acquire(
    BADS_ChannelHandle hChn,            /* [in] Device channel handle */
    ...
    )
{
    ...
    BDBG_LEAVE(BADS_7255_Acquire);
    return retCode;
}


BERR_Code BADS_7255_GetStatus(
    BADS_ChannelHandle hChn,            /* [in] Device channel handle */
...
BERR_Code BADS_7255_RequestAsyncStatus(
    BADS_ChannelHandle hChn            /* [in] Device channel handle */
    )
{
    BERR_Code retCode = BERR_SUCCESS;
    ...
    BDBG_ENTER(BADS_7255_RequestAsyncStatus);
    BDBG_ASSERT( hChn );
    BDBG_ASSERT( hChn->magicId == DEV_MAGIC_ID );
    ...
    BDBG_LEAVE(BADS_7255_RequestAsyncStatus);
    return retCode;
}

BERR_Code BADS_7255_GetAsyncStatus(
    BADS_ChannelHandle hChn,            /* [in] Device channel handle */
    BADS_Status *pStatus                /* [out] Returns status */
    )
{
    ...
    BDBG_ENTER(BADS_7255_GetAsyncStatus);
    BDBG_ASSERT( hChn );
    BDBG_ASSERT( hChn->magicId == DEV_MAGIC_ID );
    ...
    BDBG_LEAVE(BADS_7255_GetAsyncStatus);
    return retCode;
}

BERR_Code BADS_7255_GetScanStatus(
    BADS_ChannelHandle hChn,            /* [in] Device channel handle */
    ...
    )
{
    ...
    BDBG_ENTER(BADS_7255_GetScanStatus);
    BDBG_ASSERT( hChn );
    BDBG_ASSERT( hChn->magicId == DEV_MAGIC_ID );
    ...
    BDBG_LEAVE(BADS_7255_GetScanStatus);
    return retCode;
}


BERR_Code BADS_7255_GetSoftDecision(
    BADS_ChannelHandle hChn,            /* [in] Device channel handle */
    ...
    )
{
    ...
    BDBG_ENTER(BADS_7255_GetSoftDecision);
    BDBG_ASSERT( hChn );
    BDBG_ASSERT( hChn->magicId == DEV_MAGIC_ID );
    ...
    BDBG_LEAVE(BADS_7255_GetSoftDecision);
    return retCode;
}


BERR_Code BADS_7255_EnablePowerSaver(
    BADS_ChannelHandle hChn             /* [in] Device channel handle */
    )
{
    BERR_Code retCode = BERR_SUCCESS;
    ...
    BDBG_ENTER(BADS_7255_EnablePowerSaver);
    BDBG_ASSERT( hChn );
    BDBG_ASSERT( hChn->magicId == DEV_MAGIC_ID );
    ...
    BDBG_LEAVE(BADS_7255_EnablePowerSaver);
    return retCode;
}

BERR_Code BADS_7255_DisablePowerSaver(
    BADS_ChannelHandle hChn             /* [in] Device channel handle */
    )
{
    BERR_Code retCode = BERR_SUCCESS;
    ...
    BDBG_ENTER(BADS_7255_DisablePowerSaver);
    BDBG_ASSERT( hChn );
    BDBG_ASSERT( hChn->magicId == DEV_MAGIC_ID );
    ...
    BDBG_LEAVE(BADS_7255_DisablePowerSaver);
    return retCode;
}

BERR_Code BADS_7255_ProcessNotification(
    BADS_ChannelHandle hChn,                /* [in] Device channel handle */
...
BERR_Code BADS_7255_InstallCallback(
    BADS_ChannelHandle hChn,            /* [in] Device channel handle */
    ...
    )
{
    ...
    BDBG_ENTER(BADS_7255_InstallCallback);
    BDBG_ASSERT( hChn );
    BDBG_ASSERT( hChn->magicId == DEV_MAGIC_ID );
    ...
    BHAB_InstallInterruptCallback( hImplChnDev->hHab,   hImplChnDev->devId, BADS_7255_P_EventCallback_isr , (void *)hChn, callbackType);

    BDBG_LEAVE(BADS_7255_InstallCallback);
    return retCode;
}

BERR_Code BADS_7255_SetDaisyChain(
...
BERR_Code BADS_7255_GetDaisyChain(
    BADS_Handle hDev,           /* [in] Returns handle */
...
BERR_Code BADS_7255_ResetStatus(
    BADS_ChannelHandle hChn     /* [in] Device channel handle */
    )
{
    BERR_Code retCode = BERR_SUCCESS;
    ...
    BDBG_ENTER(BADS_7255_ResetStatus);
    BDBG_ASSERT( hChn );
    BDBG_ASSERT( hChn->magicId == DEV_MAGIC_ID );
    ...
    BDBG_LEAVE(BADS_7255_ResetStatus);
    return retCode;
}

BERR_Code BADS_7255_WriteSlave(
    BADS_ChannelHandle hChn,     /* [in] Device channel handle */
...
BERR_Code BADS_7255_SetScanParam(
    BADS_ChannelHandle hChn,            /* [in] Device channel handle */
    ...
    )
{
    ...
    BDBG_ENTER(BADS_7255_SetScanParam);
    BDBG_ASSERT( hChn );
    BDBG_ASSERT( hChn->magicId == DEV_MAGIC_ID );
    ...
    BDBG_LEAVE(BADS_7255_SetScanParam);
    return( retCode );
}

BERR_Code BADS_7255_GetScanParam(
    BADS_ChannelHandle hChn,            /* [in] Device channel handle */
    BADS_ChannelScanSettings *pChnScanSettings
    )
{
    ...
    BDBG_ENTER(BADS_7255_GetScanParam);
    BDBG_ASSERT( hChn );
    BDBG_ASSERT( hChn->magicId == DEV_MAGIC_ID );
    ...
    BDBG_LEAVE(BADS_7255_GetScanParam);
    return( retCode );
}
