...
)
{
	...
	if ((pSettings->bSecureMemory == true) && (hFmm->hRap->sSettings.hSecureMemory != NULL))	
	{
        ...
        if (pSettings->sExtSettings.pBufferStart == NULL)
        {
	        ...
	        {
	            BRAP_P_DRAMWRITE (((unsigned int)(hRBuf->sSettings.sExtSettings.pBufferStart) + i), 
	                0);
	        }			
  
      	    /* Have a DRAM read following DRAM writes, 
  	        in order to fix the Stream Arc memory coherency issue. */
       	    ui32DummyVar = BRAP_P_DRAMREAD((unsigned int)(hRBuf->sSettings.sExtSettings.pBufferStart));
#if ( BRAP_7405_FAMILY == 1) || ( BRAP_3548_FAMILY == 1)	
    ...
    BDBG_MSG (("BRAP_RBUF_P_Open: For RBUF[%d], StartAddr(virtual)=0x%x, StartAddr(Physical)=0x%x, Size(in bytes)=0x%x, WaterMark(percentage)=%dpAudioParams->uiStartWRPtrLocation ", 
                    uiRbufIndex, 
                    hRBuf->sSettings.sExtSettings.pBufferStart,
                    ui32PhyAddr,
...
)
{
    ...
    unsigned int ui32BaseAddr=0,ui32EndAddr=0;
    ...
    "hRBuf=0x%x, RBuf index=%d ",
    hRBuf, hRBuf->uiIndex));
    ui32BaseAddr = BRAP_Read32 (hRBuf->hRegister,
    ...
    ui32EndAddr = BRAP_Read32 (hRBuf->hRegister,
                                                    BCHP_AUD_FMM_BF_CTRL_RINGBUF_0_ENDADDR + 
                                                    hRBuf->ui32Offset);
    BRAP_Write32 (hRBuf->hRegister,  
                            BCHP_AUD_FMM_BF_CTRL_RINGBUF_0_START_WRPOINT + hRBuf->ui32Offset, 
                            ui32EndAddr);         
