 ...
 * 
 *********************************************************************/
#ifndef __BRPC_DOCSIS_H
#define __BRPC_DOCSIS_H
...
#define kADS_SW_VERSION    0x00000013
...
{
	BRPC_DevId_DOCSIS               = 0,
	BRPC_DevId_DOCSIS_US0            = 1,
	BRPC_DevId_DOCSIS_OB0           = 2,
	BRPC_DevId_DOCSIS_DS0		= 10,
	BRPC_DevId_DOCSIS_DS1		= 11,
	BRPC_DevId_DOCSIS_DS2		= 12,
	BRPC_DevId_DOCSIS_DS3		= 13,
	BRPC_DevId_DOCSIS_DS4		= 14,
	BRPC_DevId_DOCSIS_DS5		= 15,
	BRPC_DevId_DOCSIS_DS6		= 16,
	BRPC_DevId_DOCSIS_DS7		= 17,

	BRPC_DevId_DOCSIS_TNR0	        = 20,
	BRPC_DevId_DOCSIS_TNR1	        = 21,
	BRPC_DevId_DOCSIS_TNR2	= 22,
	BRPC_DevId_DOCSIS_TNR3	= 23,
	BRPC_DevId_DOCSIS_TNR4	= 24,
	BRPC_DevId_DOCSIS_TNR5	= 25,
	BRPC_DevId_DOCSIS_TNR6	= 26,
	BRPC_DevId_DOCSIS_TNR7	= 27,

	BRPC_DevId_DOCSIS_POD		= 30,
	#if (defined(VENDOR_GPIO_CONTROL) || defined(VENDOR_REQUEST))
	BRPC_DevId_DOCSIS_VEN     = 31,
	#endif

	BRPC_DevId_DOCSIS_TNR0_OOB= 40,
...
typedef enum BRPC_ProcId
{
	...
	BRPC_ProcId_ADS_GetScanStatus = 90,
	BRPC_ProcId_ADS_Scan = 91,

	BRPC_ProcId_ECM_SetDevId = 202,		/* reverse rmagnum support */
	BRPC_ProcId_ECM_TuneAck = 203,		/*  reverse rmagnum support */
	BRPC_ProcId_ECM_WfeRssiAck = 204,	/* reverse rmagnum support */
...
typedef struct BRPC_Param_SetDevid
{
	uint32_t devId;                     /* device id, see BRPC_DevId */
} BRPC_Param_SetDevid;
...
{
    /* Byte3 = CPPM-done(b7) This bit is clear on read. {1=completed, 0=not completed}
       Byte3-Byte2 = 15 bit chipid. Ex: 0x3412
     	...
     	+-------+-------+---------------+-------+
     |CPPM|LnaChipId | T1/T2|0/S/B/T |  AGC  |
     	+-------+-------+---------------+-------+
            B3      B2          B1         B0
    	*/
	uint32_t AgcVal;					/* Use for 34xx LNA gain value. */
} BRPC_Param_TNR_GetAgcVal;

typedef struct BRPC_Param_TNR_EnablePowerSaver
{
	uint32_t devId;						/* device id, see BRPC_DevId. Only support external channels TNR2 & TNR3 */
										/* Tune & Acquire API will power them back up.*/
...
/* 
  ...
  (1) To stop Docsis scanning or terminate Docsis & use DS0 for video channel, call this API to BRPC_DevId_DOCSIS_DS0 with BTNR_TunerMode_eDigital & a valid video freq.
  **note: shut down Docsis takes about 1-2sec**
  (2) To terminate DS0 video or start Docsis scanning, call this API to BRPC_DevId_DOCSIS_DS0 with BTNR_TunerMode_eDocsis & rfFreq=0.
  (3) To terminate DS0 video or ask Docsis to lock to a specific freq, call this API to BRPC_DevId_DOCSIS_DS0 with BTNR_TunerMode_eDocsis & rfFreq=desired_freq.
...
/* The next 3 APIs can be used for Docsis/video channels allocation */
/* ADS_GetTotalChannels: eCM return total DS channels available */
/* ADS_GetBondingCapability: eCM return the number of bonded channels stored & reserved for Docsis, starts from DS0 */
/*     Number of video channels available for video = ADS_GetTotalChannels - ADS_GetBondingCapability, starts from DS(ADS_GetBondingCapability) */
/*     Example 6 = 8 - 2.  Video channels starts from DS2 - DS7 */
/*     If ADS_GetBondingCapability == ADS_GetTotalChannels, this is a backward compatible mode where no channels are reserved. */
/*     Any channels can be used for Dcosis/Video. eSTB & eCM pre-known which Ds assigned to which */
/* ADS_ReserveChannel: TBD */
typedef struct BRPC_Param_ADS_ReserveChannel
{
	uint32_t num2Reserve;				/* Number of video channels to reserve */
} BRPC_Param_ADS_ReserveChannel;

typedef struct BRPC_Param_ADS_GetBondingCapability
{
	uint32_t maxNum;				    /* eCM return the number of bonded channels reserved for data */
...
typedef struct BRPC_Param_ADS_GetTotalChannels
{
    ...
    uint32_t externalChannels;          /* Number of external channels detected out of above totalChannels */
...
typedef struct BRPC_Param_ECM_GetStatus
{
	...
	uint32_t ecmMacAddressHi;                     /* eCM WAN Mac address */
	uint32_t ecmMacAddressLo;                     /* eCM WAN mac address */
	uint32_t isEcmIpMode;                         /* eCM IPV6: 0 no IP address yet, 1 IPV4 2 IPV6 */
	uint32_t ecmIpAddress;                        /* eCM WAN IPV4 address */
	uint32_t ecmIpv6Address0;                     /* eCM WAN IPv6 address */
	uint32_t ecmIpv6Address1;                     /* eCM WAN IPv6 address */
	uint32_t ecmIpv6Address2;                     /* eCM WAN IPv6 address */
	uint32_t ecmIpv6Address3;                     /* eCM WAN IPv6 address */
...
{
	uint32_t devId;				/* Set to BRPC_DevId_DOCSIS(0), see BRPC_DevId. Currently assume all external channels + LNA */
...
/* Same format for read & reply  */
typedef struct BRPC_Param_ECM_ReadXptBlock
{
    uint32_t  startRegisterAddress; /* Offset from the beginning of XPT block, 32bit aligned. b31 has special meaning  */
            /* b31: 0 --> indicate normal XPT register read from internal chip such as 3383/33843/3384.. */
            /* b31: 1 --> indicate special XPT register read from external chip such as 3128/3124.. */ 
            /* b30-b0: Offset value from the beginning of the XPT block*/
...
{
    uint32_t  startRegisterAddress; /* Offset from the beginning of XPT block, 32bit aligned. b31 ha special meaning  */
            /* b31: 0 --> indicate normal XPT register Write from internal chip such as 3383/33843/3384.. */
            /* b31: 1 --> indicate special XPT register Write from external chip such as 3128/3124.. */ 
            /* b30-b0: Offset value from the beginning of the XPT block*/
    uint32_t endRegisterAddress;   /* Offset from the begining of XPT block, 32bit aligned */
...
{
    uint32_t devId;                                         /* Not used, just set to BRPC_DevId_DOCSIS(0), see BRPC_DevId. */
...
/* Scan Mode */
typedef struct BRPC_Param_ADS_GetScanStatus
{
    uint32_t AcquisitionStatus; /* Acquisition Status (NoSignal, Unlocked, LockedFast, LockedSlow)  */
    uint32_t modType;           /* Follows enum BRPC_ADS_ModulationType above */
    uint32_t InvSpec_AutoAcq;   /* isSpectrumInverted and autoAcquire [1:0] */
    uint32_t Interleaver;       /* Interleaver [7:0] */
    uint32_t symbolRate;        /* Symbol _Rate [31:0] in Hertz */
    uint32_t RF_Offset;         /* RF_Offset [31:0] in Hertz */
} BRPC_Param_ADS_GetScanStatus;

typedef struct BRPC_Param_ADS_Scan
{
    uint32_t devId;             /* device id, see BRPC_DevId */
    ...
    uint32_t CarrierSearch;     /* b31-b16 	Reserved */
                                /* b15-b00  Carrier_Search [15:0] in units of 256*Hz*/

    uint32_t UpperBaudSearch;   /* in the unit of Hz */
    uint32_t LowerBaudSearch;   /* in the unit of Hz */
} BRPC_Param_ADS_Scan;
...
/* SW7584-133 Reverse Rmagnum support */
/*             New BRPC notification_cmd data format:           */
/*  Event code        total          32 bits                    */
/* |b31=1 bit30.......bit24|bit23.........................bit0| */
/* |8bit BRPC_ADS_Notif_Cmd|  24bit NotifCmd specific data    | */
typedef enum BRPC_Notification_Event
{
	BRPC_Notification_Event_No = 0,
	BRPC_Notification_Event_LockStatusChanged = 1,
	BRPC_Notification_Event_EcmReset = 2,
	BRPC_Notification_Event_EcmOperational = 3,
	BRPC_Notification_Event_EcmRmagnumReady = 4,	/* onetime notification to host from BNM when rMagnum is up during boot up */
	BRPC_Notification_Event_DsChannelPower = 5,
	BRPC_Notification_Event_TuneInit = 6,           /* | 8bit unused | 8bit IF_FREQ in Mhz | 8 bit DAC_GAIN adj | */
	BRPC_Notification_Event_Tune = 7,               /* | 20bit RF_FREQ in Khz unit | 4bit annex | */
	BRPC_Notification_Event_WfeRssi = 8,            /* | 24bit unused | */
...
/* SW7584-133 Reverse Rmagnum support */
typedef struct BRPC_Param_TuneAck
{
    uint32_t TuneAck;   /* 0=Not ready or 1=Ready */ 
} BRPC_Param_TuneAck;

typedef struct BRPC_Param_WfeRssiAck
{
    uint32_t WfeRssi;   /* dBmV? */
    uint32_t WfeRaw;    /* Format TBD */
    uint32_t WfeRaw2;   /* If 32bit is not enough to pack all the raw gain settings */
} BRPC_Param_WfeRssiAck;

#define BRPC_GET_NOTIFICATION_REVERSE_RMAGNUM_EVENT(x)	                ((x-0x80000000)>>24)	/* SW7584-133 Reverse Rmagnum support */
#define BRPC_GET_NOTIFICATION_REVERSE_RMAGNUM_EVENT_TUNE_RF_FREQ(x)     ((x&0xFFFFFF)>>4)		/* SW7584-133 Reverse Rmagnum support */
#define BRPC_GET_NOTIFICATION_REVERSE_RMAGNUM_EVENT_TUNE_ANNEX_MODE(x)  (x&0xF)					/* SW7584-133 Reverse Rmagnum support */
#define BRPC_GET_NOTIFICATION_REVERSE_RMAGNUM_EVENT_TUNEINIT_GAIN(x)    (x&0xFF)				/* SW7584-133 Reverse Rmagnum support */
