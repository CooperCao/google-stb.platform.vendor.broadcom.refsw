...
BDBG_MODULE(bwfe_7364_priv);


const uint32_t BWFE_g3_ChannelIntrID[BWFE_NUM_CHANNELS][BWFE_g3_MaxIntID] = {
   {
...
const uint32_t BWFE_g3_RefChannelIntrID[BWFE_NUM_REF_CHANNELS][BWFE_g3_MaxIntID] = {
   {
...
BERR_Code BWFE_P_SetAdcSampleFreq(BWFE_ChannelHandle h, uint32_t freqKhz)
{
   ...
   /* must power on adc pll + digital to clock chan and sds */
   BWFE_P_OrRegister(h, BCHP_WFE_ANA_SYS_CNTL, 0x00000001); /* power on ldo pll */
   BWFE_P_OrRegister(h, BCHP_WFE_ANA_SYS_CNTL, 0x00000002); /* power on pll */
   ...
   BWFE_P_OrRegister(h, BCHP_WFE_ANA_SYS_CNTL, 0x00010000); /* release pll reset */
   BWFE_P_OrRegister(h, BCHP_WFE_ANA_SYS_CNTL, 0x00020000); /* release postdiv reset */

   /* disable pll isolation to read pll status */
...
BERR_Code BWFE_P_InitAdc(BWFE_ChannelHandle h)
{
   ...
   /*if (!BWFE_P_IsAdcOn(h))*/
      BCHP_PWR_AcquireResource(hDev->hChip, BCHP_PWR_RESOURCE_AIFSAT0);
   ...
   BWFE_P_OrRegister(h, BCHP_WFE_ANA_CLK_CTRL, 0x00000031); /* enable wbadc, misc, and mdac clock */
...
/******************************************************************************
 BWFE_P_ShutdownAdc() - cannot power down ADC since clocking chan and sds
******************************************************************************/
BERR_Code BWFE_P_ShutdownAdc(BWFE_ChannelHandle h)
{
   ...
   /* release aif clock */
   BCHP_PWR_ReleaseResource(hDev->hChip, BCHP_PWR_RESOURCE_AIFSAT0);
...
{
   BWFE_g3_P_Handle *pDev = h->pDevice->pImpl;
   uint32_t val;

   val = BREG_Read32(pDev->hRegister, BCHP_CLKGEN_ONOFF_AIF_WB_SAT_TOP_INST_ENABLE);
   if (val & 0x1)
   {
      BWFE_P_ReadRegister(h, BCHP_WFE_ANA_CLK_CTRL, &val);
      if (val & 0x31)
         h->bEnabled = true;
      else
         h->bEnabled = false;
