...
/******************************************************************************
 BWFE_P_GetWfeClockEnableRegister()
******************************************************************************/
uint32_t BWFE_P_GetWfeClockEnableRegister(BWFE_ChannelHandle h)
{
   uint32_t addr;

#if (BCHP_VER>=BCHP_VER_B0)
   if (h->channel == 0)
      addr = BCHP_CLKGEN_AIF_WB_SAT_TOP_0_INST_CLOCK_ENABLE;
   else
      addr = BCHP_CLKGEN_AIF_WB_SAT_TOP_1_INST_CLOCK_ENABLE;
#else
   if (h->channel == 0)
      addr = BCHP_CLKGEN_ONOFF_AIF_WB_SAT_TOP_WRAPPER_0_INST_CLOCK_ENABLE;
   else
      addr = BCHP_CLKGEN_ONOFF_AIF_WB_SAT_TOP_WRAPPER_1_INST_CLOCK_ENABLE;
#endif
   return addr;
}
...
BERR_Code BWFE_P_InitAdc(BWFE_ChannelHandle h)
{
   ...
   if (h->channel == 0)
      BWFE_P_AndRegister(h, BCHP_WFE_ANA_SYS_CNTL, ~0x03FCFFFC); /* cannot power down adc0 pll */
   else
