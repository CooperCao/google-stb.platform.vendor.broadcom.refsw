...
BERR_Code BWFE_g3_Lic_P_PowerUp(BWFE_ChannelHandle h)
{
   ...
   for (slice = 0; slice < h->totalSlices; slice++)
   {
      ...
      BWFE_P_WriteSliceRegister(h, slice, BCHP_WFE_CORE_REG_NR_AGC_LF_INT_WDATA_PI_SLC, 0x00000000);  /* clear agc integrators */
      BWFE_P_WriteSliceRegister(h, slice, BCHP_WFE_CORE_REG_NR_AGC_LF_INT_WDATA_PO_SLC, 0x00000000);  /* clear agc integrators */
   }
