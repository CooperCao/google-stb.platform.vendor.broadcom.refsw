...
/*------------------------------------------------------------------------- Dolby pulse command line arguments -------------------------------------------------------------------------------------*/
...
/*-------------------------------------------------------------------------Starting and stopping decoders-------------------------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------Setting the config parameters as per the Configuration file entries---------------------------------------------------------*/
static void parse_config_file(struct dolby_pulse_command_args *dolby)
{
	...
	while(fgets( line, 400, f )){
	  ...
	  *value = 0;
	  ...
	  else if(!strcmp(line,"CAPTURE_SELECTION"))
		 dolby->capture_selection=atoi(value);
...
/*-------------------------------------------------------------------------------Change the config parameters at run time---------------------------------------------------------------------------*/
static void change_config(char *input,struct dolby_pulse_command_args *dolby)
{
   ...
   value = strchr(input, '=');
				  ...
				  else if(strstr(input,"CAPTURE_SELECTION"))
					   dolby->capture_selection = atoi(value);

			
                  else if(strstr(input,"TRANSCODEENABLED"))
				  {
					  if(atoi(value)==1) {
						  dolby->transcode=true;
					  }
...
static void print_settings(NEXUS_AudioDecoderSettings decodeSettings,NEXUS_AudioDecoderCodecSettings codecSettings,struct dolby_pulse_command_args *dolby)
{
    ...
    printf("\n(1)OUTPUTMODE = %d\n(2)OUTPUTLFECHANNEL = %d\n(3)DUALMONO_MODE = %d\n(4)DOWNMIX_MODE = %d\n(5)RFMODE = %d\n(6)DRCCUT = %d\n(7)DRCBOOST = %d\n(8)TRANSCODEENABLED = %d\n(9)CAPTURE_SELECTION = %d\n\n",decodeSettings.outputMode,decodeSettings.outputLfeMode,decodeSettings.dualMonoMode,(codecSettings.codecSettings.aacPlus.downmixMode)-2,codecSettings.codecSettings.aacPlus.drcMode,codecSettings.codecSettings.aacPlus.cut,codecSettings.codecSettings.aacPlus.boost,dolby->transcode,dolby->capture_selection);
...
int dolby_pulse_cmdline_parse(int argc, const char *argv[], struct util_opts_t *opts, struct dolby_pulse_command_args *dolby)
{
    int i;
    ...
    for (i=1;i<argc;i++) {
        if (!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help")) {
		   ...
		   printf("\n ./\\ nexus dolby_pulse_singledecode -probe streamname -config configfilename -loopback[optional] \n");
           printf("\n\n To change the value of any config parameters, type the command as 'CONFIG PARAMETER NAME= VALUE' \n Type 'quit' to terminate the application. \n\n\n");
...
int main(int argc, const char *argv[]) {
    ...
    NEXUS_Ac3EncodeHandle ac3Encode,ac3Encode_capture;
...
/*------------------------------------------------------Initialization------------------------------------------------------------------------------------------------------------------------------*/
	...
	if (dolby_pulse_cmdline_parse(argc, argv, &opts, &dolby_args)) {
		 return 0;
	 }
    ...
    BDBG_ASSERT(ac3Encode);

	ac3Encode_capture = NEXUS_Ac3Encode_Open(NULL);
	BDBG_ASSERT(ac3Encode_capture);
...
/*---------------------------------------------------------Adding Input to the respective Output ports----------------------------------------------------------------------------------------------*/

    

    if(dolby_args.transcode==true) {

						

						printf("\n\n You should expect AC3 audio at the SPDIF output\n\n");

						NEXUS_Ac3Encode_AddInput(ac3Encode,NEXUS_AudioDecoder_GetConnector(audioDecoder, NEXUS_AudioDecoderConnectorType_eMultichannel));
						...
						if(dolby_args.capture_selection==0) {
							printf("\n\n You should expect PCM Capture at the end of the playback\n\n");
							NEXUS_AudioOutput_AddInput(NEXUS_AudioCapture_GetConnector(capture),
													   NEXUS_AudioDecoder_GetConnector(audioDecoder, NEXUS_AudioDecoderConnectorType_eStereo));
						}
						else{
							printf("\n\n You should expect AC3 transcoded Capture at the end of the playback\n\n");
							NEXUS_AudioOutput_AddInput(NEXUS_AudioCapture_GetConnector(capture),
													   NEXUS_Ac3Encode_GetConnector(ac3Encode));
						}
...
/*----------------------------------------------------------------------Constantly responding to the user input ------------------------------------------------------------------------------------*/
   if(dolby_args.loopback==1) {
	for(exit=false;!exit;) {
        char buffer[256];
        char *buf;
        ...
        do {
			...
			else if ((strstr(buf, "OUTPUTMODE") != NULL)||(strstr(buf, "OUTPUTLFECHANNEL") != NULL)||(strstr(buf, "DUALMONO_MODE") != NULL)||(strstr(buf, "DOWNMIX_MODE") != NULL)||(strstr(buf, "RFMODE") != NULL)||(strstr(buf, "DRCCUT") != NULL)||(strstr(buf, "DRCBOOST") != NULL)||(strstr(buf, "NUMOUTPORTS") != NULL)||(strstr(buf, "TRANSCODETODD") != NULL)) {
...
/*---------------------------------------------------------------------Terminate the application if no loopback option is set-----------------------------------------------------------------------*/
    else{
	 #if B_HAS_PLAYBACK_MONITOR
					if(stickyFile) {
						BKNI_AcquireMutex(monitorState.lock);
...
/*----------------------------------------------------Stopping,Closing and Removing the used resources----------------------------------------------------------------------------------------------*/
