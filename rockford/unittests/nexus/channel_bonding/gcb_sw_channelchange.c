...
BDBG_MODULE(gcb_sw_channelchange);
    ...
    videoDecoder = NEXUS_VideoDecoder_Open(0, NULL);
{
    ...
    while (1)
    {
        ...
        if (j>=LOCK_ATTEMPTS) {
            BDBG_WRN(("Frontends did not lock. Asserting.."));
            BDBG_ASSERT(0);
        }
        BDBG_WRN(("frontends locked"));
        ...
        bondSettings.soft.timebase = NEXUS_Timebase_e0;
        ...
        if (i>=LOCK_ATTEMPTS) {
            BDBG_WRN(("Bond did not lock. Asserting.."));
            BDBG_ASSERT(0);
        }
        BDBG_WRN(("bond locked"));
        ...
        /* clock recovery using playback_ip */
    {
        ...
        NEXUS_ParserBand_GetBondingGroupStatus(NEXUS_ParserBand_e0, &bondStatus);
        if (!bondStatus.locked) { /* we could be in the middle of a wraparound */
            BKNI_Sleep(2000);
        }
        NEXUS_ParserBand_GetBondingGroupStatus(NEXUS_ParserBand_e0, &bondStatus);
        if (!bondStatus.locked) {
            BDBG_WRN(("Bonding status stayed unlocked. Asserting.."));
            BDBG_ASSERT(0);
        }

        {
            unsigned sleep = rand()%5000 + 2000;
            BDBG_WRN(("Sleeping %ums", sleep));
            BKNI_Sleep(sleep);
        }
        ...
        for (i=0; i<NUM_FRONTENDS; i++) {
            NEXUS_Frontend_Untune(frontend[i]);
        }
}
