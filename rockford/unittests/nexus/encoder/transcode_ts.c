...
void xcode_loopback_setup( TranscodeContext  *pContext )
{
	...
	if(!pContext->bNoVideo) {
		...
		BDBG_MSG(("Transcoder%d opened Loopback video decoder%d [%p].", pContext->contextId, decoderId-1, pContext->videoDecoderLoopback));
...
static void xcode_setup_mux_record( TranscodeContext  *pContext )
{
	...
	if (pContext->bMCPB) {
      ...
      BDBG_MSG(("Transcoder%d opened TS mux MCPB playpump%d [%p].", pContext->contextId, BTST_STREAM_MUX_VIDEO_PLAYPUMP_IDX+pContext->contextId*4, pContext->playpumpTranscodeVideo));
	...
	TranscodeContext  *pContext )
{
	...
	if(pInputSettings->resource != BTST_RESOURCE_HDMI) {
		if(!pContext->bNoVideo) {
			...
			if(!g_bSimulXcode || !g_activeXcodeCount) {
				...
				do {
					pContext->videoDecoder = NEXUS_VideoDecoder_Open(g_bDecoderZeroUp? decoderId : (cap.numVideoDecoders-decoderId-1),
					    &openSettings); /* take default capabilities */
					decoderId++;
				} while(!pContext->videoDecoder && decoderId < cap.numVideoDecoders);
				BDBG_MSG(("Transcoder%d opened source decoder %d.", pContext->contextId, g_bDecoderZeroUp? (decoderId-1) : cap.numVideoDecoders-decoderId));
				...
				if(0 == (g_bDecoderZeroUp? (decoderId-1) : cap.numVideoDecoders-decoderId)) {
		...
		if(pInputSettings->resource == BTST_RESOURCE_QAM)
		{
			...
			if(!pContext->bNoVideo) {
				...
				BDBG_MSG(("Transcoder%d opened PID channel for parser band %d.", pContext->contextId, pContext->parserBand));
			}
		...
		else if(g_simulDecoderMaster == pContext->contextId || !g_bSimulXcode)
		{
			...
			playbackSettings.stcChannel = pContext->stcVideoChannel; /* playback shares the same STC channel as a/v decoders */
	...
	*/
	...
	if(!pEncodeSettings->bCustom)
	{
		...
		BDBG_MSG(("Transcoder%d opened encoder display%d [%p].", pContext->contextId, get_display_index(pContext->contextId), pContext->displayTranscode));
	...
	TranscodeContext  *pContext )
{
	...
	if ( pContext->playpumpTranscodeVideo != pContext->playpumpTranscodeMCPB )	NEXUS_Playpump_Close(pContext->playpumpTranscodePcr);
	...
	if(pContext->inputSettings.bAudioInput)
	{
		...
		if ( pContext->playpumpTranscodeVideo != pContext->playpumpTranscodeMCPB ) NEXUS_Playpump_Close(pContext->playpumpTranscodeAudio);
