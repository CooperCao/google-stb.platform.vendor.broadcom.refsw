 ...
 * $brcm_Date: $
 * Module Description:
 * TSB 8 conversion and 2 TSB playback using clearkey encryption 
 * Revision History:
 *
 * $brcm_Log: $
 * 
...
BDBG_MODULE(dvr_test_8tsb_clearkey);
...
unsigned char changeToLocalVideoKey[16];
unsigned char changeToLocalAudioKey[16];
unsigned char controlWord[] = { 0x2e, 0xf6, 0xb6, 0xcc, 0x5b, 0x6c, 0x86, 0xf7 ,0x92, 0xa2, 0x48, 0x70, 0xac, 0xd9, 0x46, 0x73 }; 
...
void videoKeyLoaderCallback(NEXUS_KeySlotHandle keySlotHandle)
{
    ...
    {
        changeToLocalVideoKey[i] = controlWord[i];
        BDBG_MSG(("[VideoCW][%d] = 0x%x\n",i,changeToLocalVideoKey[i]));
    }
}
...
void audioKeyLoaderCallback(NEXUS_KeySlotHandle keySlotHandle)
{
    ...
    for (i=0;i<KEY_LENGTH ;i++)
    {
        changeToLocalAudioKey[i] = controlWord[i];
        BDBG_MSG(("[AudioCW][%d] = 0x%x\n",i,changeToLocalAudioKey[i]));
    }
}
...
void B_DVR_TSB_Encryption_Config(int pathIndex)
{
    ...
    if(pathIndex>=0 && pathIndex<2) /* CPS enc for main and sub*/
    {
        ...
        g_dvrTest->drmServiceSettings.keys = changeToLocalVideoKey;
        ...
        g_dvrTest->drmServiceSettings.keys = changeToLocalAudioKey;
    ...
    else /* M2M enc except main and sub */
    {
        ...
        g_dvrTest->streamPath[pathIndex].drmServiceRequest.dma = NEXUS_Dma_Open(0, NULL); /*g_dvrTest->streamPath[pathIndex].dma;*/
        g_dvrTest->streamPath[pathIndex].drmService[0] = B_DVR_DRMService_Open(&g_dvrTest->streamPath[pathIndex].drmServiceRequest);
...
void B_DVR_TSB_Decryption_Config(int index , unsigned char *key)
{
    unsigned count=0;
    B_DVR_ERROR dvrError = B_DVR_SUCCESS;
    ...
    g_dvrTest->drmServiceRequest.keyType = eB_DVR_DRMServiceKeyTypeClear;
...
void DvrTest_PlaybackDecodeStart(int index)
{
    ...
    if(g_dvrTest->streamPath[index].tsbService || g_dvrTest->streamPath[index].playbackService)
    {
        ...
        if(index>=0 && index<2) /* M2M decryption */
        {
            ...
            B_DVR_TSB_Decryption_Config(index,key);
        }
...
void B_DVR_TSB_Service_Start(int pathIndex)
{
    ...
    BDBG_MSG(("subDir(%s) programName(%s) volumeIndex(%d)",mediaNodeSettings.subDir,
        mediaNodeSettings.programName,mediaNodeSettings.volumeIndex));

    dvrError = B_DVR_Manager_SetKeyBlobLengthPerEsStream(g_dvrTest->dvrManager,&mediaNodeSettings,KEY_LENGTH);
    dvrError = B_DVR_Manager_SetKeyBlobPerEsStream(g_dvrTest->dvrManager,&mediaNodeSettings,
                        channelInfo[currentChannel].videoPids[0],changeToLocalVideoKey);

    dvrError = B_DVR_Manager_SetKeyBlobLengthPerEsStream(g_dvrTest->dvrManager,&mediaNodeSettings,KEY_LENGTH);
    dvrError = B_DVR_Manager_SetKeyBlobPerEsStream(g_dvrTest->dvrManager,&mediaNodeSettings,
                        channelInfo[currentChannel].audioPids[0],changeToLocalAudioKey);
