...
int main(void)
{
    ...
    pParam = BKNI_Malloc(sizeof(RecordingListThreadParam));
    if (!pParam) {
        printf("Error in creating a RecordingListThreadParam\n");
    }

    sprintf(pParam->subDir, "tsbConv");
    pParam->volumeIndex = volumeIndex;


    RecordingListThread = B_Thread_Create("Recording List Thread", DVR_Recordings_Info, pParam, NULL);
    if (!RecordingListThread) {
        printf("Error in creating a thead\n");
    }
