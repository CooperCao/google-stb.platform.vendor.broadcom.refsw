...
void B_DVR_Playback_Service_Start(int pathIndex, char *subDir, char *programName)
{
         if(g_dvrTest->streamPath[pathIndex].tsbService)
         {
...
void B_DVR_Playback_Service_Stop(int pathIndex)
{
...
int main(void)
{
    ...
    if(g_dvrTest->streamPath[Path1].tsbService)
    {
        B_DVR_ERROR rc = B_DVR_SUCCESS;
        B_DVR_TSBServiceStatus tsbStatus;
        B_DVR_TSBServicePermanentRecordingRequest recReq;

        B_DVR_TSBService_GetStatus(g_dvrTest->streamPath[Path1].tsbService, &tsbStatus);
        sprintf(recReq.programName, "TSBRecord%d", Path1);
        recReq.recStartTime = tsbStatus.tsbRecStartTime;
        recReq.recEndTime = recReq.recStartTime + DURATION;

        printf("\n \t program name %s \t start %lu \t end %lu",recReq.programName,recReq.recStartTime,recReq.recEndTime);
        sprintf(recReq.subDir,"tsbConv");
        rc = B_DVR_TSBService_ConvertStart(g_dvrTest->streamPath[Path1].tsbService,&recReq);
        if(rc!=B_DVR_SUCCESS)
        {
            printf("\n Invalid paramters");
        }
    ...
    B_DVR_TSBService_ConvertStop(g_dvrTest->streamPath[Path1].tsbService);


    if(g_dvrTest->streamPath[Path0].tsbService)
    {
        B_DVR_ERROR rc = B_DVR_SUCCESS;
        B_DVR_TSBServiceStatus tsbStatus;


        B_DVR_TSBService_GetStatus(g_dvrTest->streamPath[Path0].tsbService, &tsbStatus);

        rc = B_DVR_TSB_B2B_Convert_Start(tsbStatus.tsbRecStartTime, tsbStatus.tsbRecEndTime/2, "TSBPerm1", Path0);
        if(rc!=B_DVR_SUCCESS)
        {
            printf("\n Back to Back conversion of Perm1 Record is failed");
        }
        rc = B_DVR_TSB_B2B_Convert_Start(tsbStatus.tsbRecEndTime/2, tsbStatus.tsbRecEndTime, "TSBPerm2", Path0);
        if(rc!=B_DVR_SUCCESS)
        {
    ...
    while(B_Event_Wait(TSBConversionComplete, B_WAIT_FOREVER)!=B_ERROR_SUCCESS);
    B_Event_Reset(TSBConversionComplete);
    B_DVR_TSBService_ConvertStop(g_dvrTest->streamPath[Path0].tsbService);


    dumpMetaDataFile("/mnt/dvr0-metadata/tsbConv/TSBPerm1.ts");
    dumpMetaDataFile("/mnt/dvr0-metadata/tsbConv/TSBPerm2.ts");
    ...
    for (;;) {
       ...
       for(i=0;cmd[i]!='\0';i++) {
           ...
           case '?':
               printf("? - This help\n"
                        "0 - Start Playback Permanent1\n"
                        "1 - Stop Playback Permanent1\n"
                        "2 - Start Playback Permanent2\n"
                        "3 - Stop Playback Permanent2\n"
                        "4 - Print Permanent1 Metadata\n"
                        "5 - Print Permanent2 Metadata\n"
                        "6 - Delete Permanent1\n"
                        "7 - Delete Permanent2\n" 
                        "q - Quit\n");
            break;
            case '0':
                if (Perm1Deleted)
                {
                    printf("TSBPerm1.ts is not available\n");
                }
                else
                {
                    B_DVR_Playback_Service_Start(Path0, "tsbConv", "TSBPerm1");
                }
            break;
            case '1':
                if (Perm1Deleted)
                ...
                {
                    if(!g_dvrTest->streamPath[Path0].liveDecodeStarted)
                    {
                        B_DVR_Playback_Service_Stop(Path0);
                    }
                    else
                    {
            ...
            case '2':
                if (Perm2Deleted)
                {
                    printf("TSBPerm2.ts is not available\n");
                }
                else
                {
                    B_DVR_Playback_Service_Start(Path0, "tsbConv", "TSBPerm2");
                }
            break;
            case '3':
                if (Perm2Deleted)
            ...
            case '6':
                if (!Perm1Deleted)
                {
                    rc = B_DVR_Delete_Record(0, "tsbConv", "TSBPerm1");
                    if (rc == B_DVR_SUCCESS)
                    {
                        Perm1Deleted = 1;
                    }
                ...
                {
                    printf("TSB Converted Perm1 file is already removed!!!\n");
                }
            break;
            case '7':
                if (!Perm2Deleted)
                {
                    rc = B_DVR_Delete_Record(volumeIndex, "tsbConv", "TSBPerm2");
                    if (rc == B_DVR_SUCCESS)
                    {
                        Perm2Deleted = 1;
                    }
                ...
                {
                    printf("TSB Converted Perm2 file is already removed!!!\n");
                }
            break;
            case 'q':
                quit = true;
                if (!Perm1Deleted)
                {
                    B_DVR_Delete_Record(volumeIndex, "tsbConv", "TSBPerm1");
                }
                if (!Perm2Deleted)
                {
                    B_DVR_Delete_Record(volumeIndex, "tsbConv", "TSBPerm2");
                }
            break;
            default:
                break;
