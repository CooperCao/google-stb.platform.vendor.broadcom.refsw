 ...
 * $brcm_Date: $
 * Module Description:
 * 8 TSB conversion, 2 TSB playback, listing of recordings and data injection for the 8 TSB recordings
 * Revision History:
 *
 * $brcm_Log: $
 * 
...
B_EventHandle DataInjectionComplete[MAX_AVPATH];
...
void Data_Inject(void *context)
{
    ...
    do
    {
        for (index = 0; index < MAX_AVPATH; index++)
        {
            ...
            B_DVR_TSB_Data_Injection_Start(injectionType, PAT_PID, index);
            ...
            B_DVR_TSB_Data_Injection_Stop(index);
            /* wait for 100 milisec */
            BKNI_Sleep(100);

            /* Data Injection of PMT */
            B_DVR_TSB_Data_Injection_Start(injectionType, PMT_PID, index);
            while(B_Event_Wait(DataInjectionComplete[index], B_WAIT_FOREVER)!=B_ERROR_SUCCESS);
            printf("\n$$$ Data Injection is complete for the index %d $$$\n", index);
            B_Event_Reset(DataInjectionComplete[index]);
            B_DVR_TSB_Data_Injection_Stop(index);
...
int main(void)
{
    ...
    DataInjectionThread = B_Thread_Create("Data InjectionThread", Data_Inject, pDataInjectionParam, NULL);
    if (!DataInjectionThread) 
    {
         printf("Error in creating a Data Injection thread\n");
    }
