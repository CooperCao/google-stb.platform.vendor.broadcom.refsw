#include "dvr_test.h"
BDBG_MODULE(hnMulPFToMulStreamNet);

#define MAX_TSBPATH 8
#define DEFAULT_CLIENT_NUMBER 1
static int  Mulpathkey[MAX_TSBPATH] = {0,1,2,3,4,5,6,7};
static char Mulhnpfstreamingtxt[] = "hnmulpfstreaming.txt";
static int clientNum = 0;
B_EventHandle MultiplePFStreamReady;

static void MulPFInitHnMulStreamingpara(void);
static void MulPFCheckMulStreamTestName(char *buffer);
static int  MulPFHnMulStreamingLoadTxt(void);
static int  MulPFInitMulStreamUDPSocket(void);
static void MulPF_stream_UDP(int pathIndex);
static void MulPF_stream_MulThread_Created(int pathIndex);
static void MulPF_stream_MulThread_Active(int pathIndex);
static void MulPF_stream_MulThread_Destroy(int pathIndex);

void MulPFInitHnMulStreamingpara(void)
{
    int  pathIndex;
...
void MulPFCheckMulStreamTestName(char *buffer)
{
    char testpara[512];
    char strpara[512];
    int  paravalue = 0;
    ...
    if (n > 0)
    {
       ...
       else if(strcmp(testpara, "ClientNumber") == 0)
       {
          m = sscanf(buffer, "%s %s\n", testpara,strpara);
          if(m > 0)
          {
             clientNum = atoi(strpara);
             printf("clientNum = %d \n",clientNum);
          }
       }
...
int MulPFHnMulStreamingLoadTxt(void)
{
    char buffer[512];
    FILE *fd;
    int rc = 0;
    ...
    fd= fopen(Mulhnpfstreamingtxt, "r");
    if(fd == NULL)
    {
    ...
    while (fgets(buffer, 512, fd) != NULL)
    {
      if (buffer[0] != '#')
      {
         MulPFCheckMulStreamTestName(buffer);
      }
    }
    fclose(fd);
    return rc;
...
int MulPFInitMulStreamUDPSocket(void)
{
   int pathIndex,rc=0;
   int  errorcode = 0;
   ...
   printf("\n Enter InitUDPSocket>>>> \n");  
   if(clientNum == 0)
       clientNum = DEFAULT_CLIENT_NUMBER;
   for (pathIndex = 0; pathIndex < clientNum; pathIndex++)
   {
...
void MulPF_stream_MulThread_Created(int pathIndex)
{
...
void MulPFStreamAction(void *PathContext)
{
    ...
    if((g_dvrTest->streamPath[pathIndex].readOffset + g_dvrTest->streamPath[pathIndex].bufSize) < g_dvrTest->streamPath[pathIndex].mediaFileStats.endOffset)
    {
        ...
        if(g_dvrTest->streamPath[pathIndex].dtPeriod >= 35)
           printf("g_dvrTest->streamPath[%d].dtPeriod =%u\n",pathIndex,g_dvrTest->streamPath[pathIndex].dtPeriod);
    ...
    else
    {
        ...
        B_Event_Set(MultiplePFStreamReady);
...
void MulPF_stream_MulThread_Active(int pathIndex)
{
...
static void MulPF_Test_Scheduler(void *PathContext)
{
...
void hnMulPFToMulStreamNetTest(CuTest * tc)
{
    int index,returnValue; 

    MulPFInitHnMulStreamingpara();
    returnValue = MulPFHnMulStreamingLoadTxt();
    if (returnValue != 0)
    {
        printf("\n Missing input file!!!\n");
        return;
    ...
    returnValue = MulPFInitMulStreamUDPSocket();
    if (returnValue != 0)
    {
    ...
    g_dvrTest->PFthread = B_Thread_Create("MulPF_Test Scheduler", MulPF_Test_Scheduler,NULL, NULL);
    ...
    MultiplePFStreamReady = B_Event_Create(NULL);
    ...
    for (index = 0; index < clientNum; index++)
    {
        ...
        MulPF_stream_MulThread_Created(Mulpathkey[index]);
    }
    ...
    for (index = 0; index < clientNum; index++)
    {
        ...
        MulPF_stream_MulThread_Active(Mulpathkey[index]);
    }

    B_Mutex_Unlock(g_dvrTest->glbstreamMutex);

cntWaitEvent:
    while(B_Event_Wait(MultiplePFStreamReady, B_WAIT_FOREVER)!=B_ERROR_SUCCESS);
    if(g_dvrTest->multipleStreamFlag != 0)
    {
        printf("Need to reset MultiplePFStreamReady --mutiple thread flag=%x\n",g_dvrTest->multipleStreamFlag);
        B_Event_Reset(MultiplePFStreamReady);
        goto cntWaitEvent;
    }
    ...
    for (index = 0; index < clientNum; index++)
    {
        ...
        MulPF_stream_MulThread_Destroy(Mulpathkey[index]);
    }
    ...
    B_Event_Destroy(MultiplePFStreamReady);
    ...
    printf("\n End of hnMulPFToMulStreamNetTest");
