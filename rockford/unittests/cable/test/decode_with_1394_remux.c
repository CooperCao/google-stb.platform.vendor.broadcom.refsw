...
int main(void)
{
	...
	NEXUS_PidChannelHandle videoPidChannel2, audioPidChannel2;

    NEXUS_PidChannelHandle videoPidChannelToRemux, audioPidChannelToRemux;
...
/* Sending to decode */
    ...
    /* Map a parser band to loopback route from remux output */
    ...
    printf("Press ENTER to stop decode and start decode from the original source \n");
    getchar();
	printf("Decoding from the original source \n");
 ...
 NEXUS_AudioDecoder_Stop(pcmDecoder);
	...
	/* Open the audio and video pid channels */
	/* The original parser band = NEXUS_ParserBand_e0 */
	videoPidChannel2 = NEXUS_PidChannel_Open(NEXUS_ParserBand_e0, 0x11, NULL);
	audioPidChannel2 = NEXUS_PidChannel_Open(NEXUS_ParserBand_e0, 0x14, NULL);

	/* Set up decoder Start structures now. We need to know the audio codec to properly set up
	the audio outputs. */
	videoProgram.pidChannel = videoPidChannel2;
 ...
 NEXUS_AudioDecoder_Stop(pcmDecoder);
	...
	NEXUS_PidChannel_Close(videoPidChannel2);
	NEXUS_PidChannel_Close(audioPidChannel2);
