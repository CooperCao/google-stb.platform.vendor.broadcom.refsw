...
static void B_PSIP_ProcessTable(void * param)
{
...
#if 0
        BDBG_MSG(("%s: Error collecting data from application - RETRYING", BSTD_FUNCTION));
#endif
        return;
    }
    else
    if (B_ERROR_SUCCESS != status)
    {
        ...
        BDBG_MSG(("%s: Error collecting data from application - TIMEOUT", BSTD_FUNCTION));
    ...
    )
{
    ...
    if (NULL == pJob)
    {
        ...
        {
            BDBG_WRN(("Out of memory (%s:%d)!", BSTD_FUNCTION, __LINE__));
