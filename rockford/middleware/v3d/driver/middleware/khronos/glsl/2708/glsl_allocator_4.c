...
void (*line_capture_coord)(char * str) = NULL;
void (*capture_sched_info)(char * errstr, uint32_t type, bool fragThreaded) = NULL;
...
KHAPI void glsl_set_line_capture_coord(void(*p)(char *))
{
   line_capture_coord = p;
}

KHAPI void glsl_set_capture_sched_info(void(*p)(char *, uint32_t, bool))
{
   capture_sched_info = p;
}
...
typedef struct
{
   ...
   Dataflow *dataflow_src;    /* The dataflow source. Used for finding dependents of this if regfile space needs freeing */
...
static struct
{
   MEM_HANDLE_T op_struct, instr_struct, generated_instructions, uniform_map;
} big_arrays;

static void term_big_arrays(void)
{
   if (big_arrays.op_struct != MEM_HANDLE_INVALID)
   {
      mem_unlock(big_arrays.op_struct);
      MEM_ASSIGN(big_arrays.op_struct, MEM_HANDLE_INVALID);
   }
   if (big_arrays.instr_struct != MEM_HANDLE_INVALID)
   {
      mem_unlock(big_arrays.instr_struct);
      MEM_ASSIGN(big_arrays.instr_struct, MEM_HANDLE_INVALID);
   }
   if (big_arrays.generated_instructions != MEM_HANDLE_INVALID)
   {
      mem_unlock(big_arrays.generated_instructions);
      MEM_ASSIGN(big_arrays.generated_instructions, MEM_HANDLE_INVALID);
   }
   if (big_arrays.uniform_map != MEM_HANDLE_INVALID)
   {
      mem_unlock(big_arrays.uniform_map);
      MEM_ASSIGN(big_arrays.uniform_map, MEM_HANDLE_INVALID);
   }
}

typedef OP_STRUCT_T TWO_OP_STRUCT[2];

static TWO_OP_STRUCT *op_struct;
static INSTR_STRUCT_T *instr_struct;
static uint32_t last_used_timestamp;

static uint32_t *generated_instructions;
static uint32_t generated_instruction_count;
static bool finished;
static bool failed;

static uint32_t *uniform_map;
...
static OP_T op_add_alu(OP_FLAVOUR_T flavour, OP_SRC_DETAIL_T left_detail, OP_SRC_DETAIL_T right_detail,
                       OP_SRC_T *deps, uint32_t dep_count, bool allow_mov, INSTR_CONFLICT_T extra_conflict,
                       uint32_t first_timestamp);

static bool init_big_arrays(void)
{
   term_big_arrays();

   /* NB: khrn_mem_alloc initialises these blocks to 0, so no need to memset */
   big_arrays.op_struct = mem_alloc(MAX_INSTRUCTIONS*2*sizeof(OP_STRUCT_T), alignof(uint32_t), MEM_FLAG_NONE, "op_struct");
   if (big_arrays.op_struct == MEM_HANDLE_INVALID) goto fail;
   op_struct = (TWO_OP_STRUCT *)mem_lock(big_arrays.op_struct, NULL);

   big_arrays.instr_struct = mem_alloc(MAX_INSTRUCTIONS*sizeof(INSTR_STRUCT_T), alignof(uint32_t), MEM_FLAG_NONE, "instr_struct");
   if (big_arrays.instr_struct == MEM_HANDLE_INVALID) goto fail;
   instr_struct = (INSTR_STRUCT_T *)mem_lock(big_arrays.instr_struct, NULL);

   big_arrays.generated_instructions = mem_alloc(2*(MAX_INSTRUCTIONS+MAX_EXTRA_INSTRUCTIONS)*sizeof(uint32_t), alignof(uint32_t), MEM_FLAG_NONE, "generated_instructions");
   if (big_arrays.generated_instructions == MEM_HANDLE_INVALID) goto fail;
   generated_instructions = (uint32_t *)mem_lock(big_arrays.generated_instructions, NULL);

   big_arrays.uniform_map = mem_alloc(2*MAX_UNIFORMS*sizeof(uint32_t), alignof(uint32_t), MEM_FLAG_NONE, "uniform_map");
   if (big_arrays.uniform_map == MEM_HANDLE_INVALID) goto fail;
   uniform_map = (uint32_t *)mem_lock(big_arrays.uniform_map, NULL);
...
static OP_SRC_DETAIL_T src_detail_without_unpack(OP_SRC_DETAIL_T input_detail, bool retire)
{
   ...
   result.retire = retire;
   result.is_new = true;
   result.dataflow_src = input_detail.dataflow_src;
...
static void commit_alu_choice(OP_T op, CHOICE_T choice, Dataflow *dataflow_src)
{
   OP_STRUCT_T *op_struct;
   INSTR_STRUCT_T *instr;
   uint32_t i;
...
static OP_T op_add_alu(OP_FLAVOUR_T flavour, OP_SRC_DETAIL_T left_detail, OP_SRC_DETAIL_T right_detail, OP_SRC_T *deps, uint32_t dep_count, bool allow_mov, INSTR_CONFLICT_T extra_conflict, uint32_t first_timestamp_in)
{
   ...
   /* We can't unpack from the VPM and if we're allowed to try then there's a
    * risk that both halves of a binop will want to unpack the same VPM word.
...
static uint32_t instr_add_sig(INSTR_SIG_T sig, OP_SRC_T *deps, uint32_t dep_count)
{
   ...
   /* special hack: sbwait can't be the first two instructions */
   if (sig == SIG_SB_WAIT && first_timestamp < 2)
      first_timestamp = 2;
...
bool glsl_dataflow_is_unpack(const Dataflow *dataflow)
{
...
static OP_SRC_DETAIL_T find_src(Dataflow *dataflow, bool track, bool actually_care)
{
   ...
   */
   ...
   for (node = dataflow->dependents.first; node; node = node->next)
   {
      ...
      if (glsl_dataflow_is_unpack(node->dataflow))
      {
         /* TODO: double unpack?? */
   ...
   switch (dataflow->flavour)
   {
      ...
      case DATAFLOW_FRAG_GET_COL:
         ...
         break;
      case DATAFLOW_UNPACK_COL_R:
         return src_detail_with_unpack(find_src(dataflow->d.unary_op.operand, track, actually_care), UNPACK_8A_F);   /* TODO: what if there's already an unpack?? */
      case DATAFLOW_UNPACK_COL_G:
         return src_detail_with_unpack(find_src(dataflow->d.unary_op.operand, track, actually_care), UNPACK_8B_F);
      case DATAFLOW_UNPACK_COL_B:
         return src_detail_with_unpack(find_src(dataflow->d.unary_op.operand, track, actually_care), UNPACK_8C_F);
      case DATAFLOW_UNPACK_COL_A:
         return src_detail_with_unpack(find_src(dataflow->d.unary_op.operand, track, actually_care), UNPACK_8D_F);
      case DATAFLOW_UNPACK_16A:
         return src_detail_with_unpack(find_src(dataflow->d.unary_op.operand, track, actually_care), UNPACK_16A);
      case DATAFLOW_UNPACK_16A_F:
         return src_detail_with_unpack(find_src(dataflow->d.unary_op.operand, track, actually_care), UNPACK_16A_F);
      case DATAFLOW_UNPACK_16B:
         return src_detail_with_unpack(find_src(dataflow->d.unary_op.operand, track, actually_care), UNPACK_16B);
      case DATAFLOW_UNPACK_16B_F:
         return src_detail_with_unpack(find_src(dataflow->d.unary_op.operand, track, actually_care), UNPACK_16B_F);
      case DATAFLOW_UNPACK_8A:
         return src_detail_with_unpack(find_src(dataflow->d.unary_op.operand, track, actually_care), UNPACK_8A);
      case DATAFLOW_UNPACK_8B:
         return src_detail_with_unpack(find_src(dataflow->d.unary_op.operand, track, actually_care), UNPACK_8B);
      case DATAFLOW_UNPACK_8C:
         return src_detail_with_unpack(find_src(dataflow->d.unary_op.operand, track, actually_care), UNPACK_8C);
      case DATAFLOW_UNPACK_8D:
         return src_detail_with_unpack(find_src(dataflow->d.unary_op.operand, track, actually_care), UNPACK_8D);
      case DATAFLOW_UNPACK_8R:
         return src_detail_with_unpack(find_src(dataflow->d.unary_op.operand, track, actually_care), UNPACK_8R);
...
*/
void glsl_allocator_mem_term(void)
{
   term_big_arrays();
}

bool glsl_allocator_init(uint32_t type, bool is_threaded, uint32_t num_attribute_rows)
{
   uint32_t i;
...
static void add_iodependencies(Dataflow *dataflow, OP_SRC_T *deps, uint32_t *dep_count)
{
   ...
   switch (dataflow->flavour)
   {
      ...
      case DATAFLOW_UNPACK_8D:
      case DATAFLOW_UNPACK_8R:
         add_iodependencies(dataflow->d.unary_op.operand, deps, dep_count);
         break;
      default:
         break;
   }
...
void glsl_allocator_add_node(Dataflow *dataflow)
{
   ...
   switch (flavour)
   {
      ...
      {
         OP_SRC_DETAIL_T src_detail = find_src(dataflow->d.cond_op.true_value, true, true);
         OP_SRC_DETAIL_T background = find_src(dataflow->d.cond_op.false_value, true, true);
         OP_SRC_DETAIL_T flags_src = find_src(dataflow->d.cond_op.cond, true, true);
         OP_COND_T cond = bool_rep_to_cond(glsl_dataflow_get_bool_rep(dataflow->d.cond_op.cond), false);
      ...
      case DATAFLOW_INTRINSIC_RSQRT:
         output = true; op = DEST_EITHER(53); source = dataflow->d.unary_op.operand; break;
      case DATAFLOW_INTRINSIC_RCP:
         output = true; op = DEST_EITHER(52); source = dataflow->d.unary_op.operand; break;
      case DATAFLOW_INTRINSIC_LOG2:
         output = true; op = DEST_EITHER(55); source = dataflow->d.unary_op.operand; break;
      case DATAFLOW_INTRINSIC_EXP2:
         output = true; op = DEST_EITHER(54); source = dataflow->d.unary_op.operand; break;
      ...
      case DATAFLOW_TEX_SET_COORD_S:
         output = true; source = dataflow->d.texture_lookup_set.param;
         op = DEST_EITHER(56 + 4 * get_textunit(dataflow->d.texture_lookup_set.sampler));
         get_sampler_details(dataflow->d.texture_lookup_set.sampler, &dest_detail0, &dest_detail1);
         break;
      case DATAFLOW_TEX_SET_COORD_T:
         output = true; source = dataflow->d.texture_lookup_set.param;
         op = DEST_EITHER(57 + 4 * get_textunit(dataflow->d.texture_lookup_set.sampler));
         get_sampler_details(dataflow->d.texture_lookup_set.sampler, &dest_detail0, &dest_detail1);
         break;
      case DATAFLOW_TEX_SET_COORD_R:
         output = true; source = dataflow->d.texture_lookup_set.param;
         op = DEST_EITHER(58 + 4 * get_textunit(dataflow->d.texture_lookup_set.sampler));
         get_sampler_details(dataflow->d.texture_lookup_set.sampler, &dest_detail0, &dest_detail1);
         break;
      case DATAFLOW_TEX_SET_LOD:
      case DATAFLOW_TEX_SET_BIAS:
         output = true; source = dataflow->d.texture_lookup_set.param;
         op = DEST_EITHER(59 + 4 * get_textunit(dataflow->d.texture_lookup_set.sampler));
         get_sampler_details(dataflow->d.texture_lookup_set.sampler, &dest_detail0, &dest_detail1);
         break;
      case DATAFLOW_TEX_SET_DIRECT:
         output = true; source = dataflow->d.texture_lookup_set.param;
         op = DEST_A(56 + 4 * get_textunit(dataflow->d.texture_lookup_set.sampler));    /* TODO: not really DEST_A */
         break;
      case DATAFLOW_TEX_GET_CMP_R:
         sig = true;
         sig_code = get_textunit(dataflow->d.texture_lookup_get.sampler) ? SIG_TMU1 : SIG_TMU0;
         break;
      case DATAFLOW_FRAG_GET_COL:
      ...
      case DATAFLOW_FRAG_SUBMIT_Z:
         ...
         cond_node = dataflow->d.fragment_set.discard;
         cond_flip = true;
         break;
      case DATAFLOW_FRAG_SUBMIT_MS:
         output = true; op = DEST_EITHER(45); source = dataflow->d.fragment_set.param;
         cond_node = dataflow->d.fragment_set.discard;
         cond_flip = true;
         break;
      case DATAFLOW_FRAG_SUBMIT_ALL:
         output = true; op = DEST_EITHER(46); source = dataflow->d.fragment_set.param;
         cond_node = dataflow->d.fragment_set.discard;
         cond_flip = true;
         break;
      case DATAFLOW_FRAG_SUBMIT_R0:
         output = true; op = DEST_ACC(0); source = dataflow->d.fragment_set.param; cond_node = NULL;
         break;
      case DATAFLOW_FRAG_SUBMIT_R1:
         output = true; op = DEST_ACC(1); source = dataflow->d.fragment_set.param; cond_node = NULL;
         break;
      case DATAFLOW_FRAG_SUBMIT_R2:
         output = true; op = DEST_ACC(2); source = dataflow->d.fragment_set.param; cond_node = NULL;
         break;
      case DATAFLOW_FRAG_SUBMIT_R3:
         output = true; op = DEST_ACC(3); source = dataflow->d.fragment_set.param; cond_node = NULL;
         break;
      ...
      case DATAFLOW_VPM_READ_SETUP:
         output = true; op = DEST_A(49); source = dataflow->d.vertex_set.param; break;
      case DATAFLOW_VPM_WRITE_SETUP:
         output = true; op = DEST_B(49); source = dataflow->d.vertex_set.param; break;
   ...
   if (pack)
   {
      OP_SRC_DETAIL_T src_detail = find_src(dataflow->d.pack.operand, true, true);
      if (dataflow->d.pack.background)
      {
         OP_SRC_DETAIL_T background = find_src(dataflow->d.pack.background, true, true);
   ...
   if (uop)
   {
      left = right = dataflow->d.unary_op.operand;
   }
   if (binop && !reverse)
   {
      left = dataflow->d.binary_op.left;
      right = dataflow->d.binary_op.right;
   }
   if (binop && reverse)
   {
      left = dataflow->d.binary_op.right;
      right = dataflow->d.binary_op.left;
   }
...
bool glsl_allocator_finish()
{
   ...
   if (capture_sched_info)
      capture_sched_info("", shader_type, (shader_type & GLSL_BACKEND_TYPE_FRAGMENT) ? shader_is_threaded : false);
...
void glsl_allocator_dump(void)
{
   ...
   /*DEBUG*/
   {
      ...
      for (i = 0; i < generated_instruction_count; i++)
      {
         ...
         snprintf(buffer2, sizeof(buffer2), "[%2d,%2d] %s", op_struct[i][0].schedule_order, op_struct[i][1].schedule_order, buffer);
         ...
         else if ((line_capture_coord) && (shader_type & GLSL_BACKEND_TYPE_COORD))
            line_capture_coord(buffer2);
         debug_print(buffer2);
         debug_print("\n");
      }
      debug_print("\n");
...
void glsl_allocator_mark_failed_nodes_on_stack(void) {
   int i;
   for (i = 0; i < 32; i++)
   {
