...
/*
   ...
   hsadd(HS, TS)                 // add to token sequences hide sets
   {
      if TS is {} then
         return {};
      note TS must be T(HS)  TS
   ...
   ts(T)          Given a macro-name token, ts returns the replacement token sequence from the macros definition.

   fp(T)          Given a macro-name token, fp returns the (ordered) list of formal parameters from the macros definition.
