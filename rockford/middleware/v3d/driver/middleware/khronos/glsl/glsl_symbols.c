...
GLenum primitiveTypesToGLenums[PRIMITIVE_TYPES_COUNT] =
{
   ...
   GL_SAMPLER_EXTERNAL_OES, // PRIM_SAMPLEREXTERNAL
   GL_SAMPLER_CUBE, // PRIM_SAMPLERCUBE
};
Symbol primitiveParamSymbols[PRIMITIVE_TYPES_COUNT];
...
PrimitiveTypeIndex primitiveScalarTypeIndices[PRIMITIVE_TYPES_COUNT] =
{
   ...
   PRIM_SAMPLEREXTERNAL, // PRIM_SAMPLEREXTERNAL
...
PRIMITIVE_TYPE_FLAGS_T primitiveTypeFlags[PRIMITIVE_TYPES_COUNT] =
{
   ...
   PRIM_SAMPLER_TYPE, // PRIM_SAMPLEREXTERNAL
   PRIM_SAMPLER_TYPE, // PRIM_SAMPLERCUBE
};

void glsl_init_primitive_values(void)
{
   g_BoolFalse = glsl_dataflow_construct_const_bool(CONST_BOOL_FALSE);
   g_BoolTrue = glsl_dataflow_construct_const_bool(CONST_BOOL_TRUE);
   g_IntZero = glsl_dataflow_construct_const_int(0);
   g_IntOne = glsl_dataflow_construct_const_int(1);
   g_FloatZero = glsl_dataflow_construct_const_float(CONST_FLOAT_ZERO);
   g_FloatOne = glsl_dataflow_construct_const_float(CONST_FLOAT_ONE);
}
...
static void init_primitive_types(void)
{
   ...
   memset(primitiveParamSymbols, 0, sizeof(primitiveParamSymbols));
   ...
   primitiveTypes[PRIM_SAMPLEREXTERNAL].size_as_const = 0; // undefined, cannot be const
   ...
   primitiveTypes[PRIM_SAMPLEREXTERNAL].scalar_count = 1; // a reference to the sampler
   ...
   primitiveTypes[PRIM_SAMPLEREXTERNAL].name = "samplerExternalOES";
   ...
   primitiveTypeSubscriptTypes[PRIM_SAMPLEREXTERNAL] = NULL;
...
static void init_built_in_vars_and_struct_types(void)
{
      ...
      case SHADER_FRAGMENT:
         {
...

            {
               Dataflow *df;
               /* TODO: this is hacky */
               df = glsl_dataflow_construct_fragment_get(DATAFLOW_FRAG_GET_X);
               df = glsl_dataflow_construct_unary_op(DATAFLOW_ITOF, df);
               ...
               if (khrn_workarounds.FB_BOTTOM_UP || khrn_workarounds.FB_TOP_DOWN)
               {
                  ...
                  df_height = glsl_dataflow_construct_linkable_value(DATAFLOW_UNIFORM, PRIM_FLOAT);
                  df_height->u.linkable_value.row = BACKEND_UNIFORM_FBHEIGHT;
                  df_comp = glsl_dataflow_construct_linkable_value(DATAFLOW_UNIFORM, PRIM_BOOL);
                  df_comp->u.linkable_value.row = BACKEND_UNIFORM_EGL_OUTPUT_FORMAT;
                  df = glsl_dataflow_construct_fragment_get(DATAFLOW_FRAG_GET_Y);
                  df = glsl_dataflow_construct_unary_op(DATAFLOW_ITOF, df);
                  df_left = glsl_dataflow_construct_binary_op(DATAFLOW_SUB, df_height, df);
                  df_comp = glsl_dataflow_construct_cond_op(df_comp, df_left, df);
               ...
               df = glsl_dataflow_construct_fragment_get(DATAFLOW_FRAG_GET_Z);
               df = glsl_dataflow_construct_unary_op(DATAFLOW_ITOF, df);
               df = glsl_dataflow_construct_binary_op(DATAFLOW_MUL,
                  df,
                  glsl_dataflow_construct_const_float(cf(1.0f/16777215.0f)));
               g_BuiltInVar__gl_FragCoord->u.var_instance.scalar_values[2] = df;

               df = glsl_dataflow_construct_fragment_get(DATAFLOW_FRAG_GET_W);
               df = glsl_dataflow_construct_unary_op(DATAFLOW_INTRINSIC_RCP, df);
            ...
            g_BuiltInVar__gl_FrontFacing->u.var_instance.scalar_values[0] = glsl_dataflow_construct_fragment_get(DATAFLOW_FRAG_GET_FF);
            ...
            g_BuiltInVar__gl_PointCoord->u.var_instance.scalar_values[0] = glsl_dataflow_construct_linkable_value(DATAFLOW_VARYING, PRIM_FLOAT);
            g_BuiltInVar__gl_PointCoord->u.var_instance.scalar_values[0]->u.linkable_value.row = 0;
            if (khrn_workarounds.FB_BOTTOM_UP || khrn_workarounds.FB_TOP_DOWN)
            {
               Dataflow * df_left, * df_right, * df_comp;
               df_right = glsl_dataflow_construct_linkable_value(DATAFLOW_VARYING, PRIM_FLOAT);
               df_right->u.linkable_value.row = 1;
               df_comp = glsl_dataflow_construct_linkable_value(DATAFLOW_UNIFORM, PRIM_BOOL);
               df_comp->u.linkable_value.row = BACKEND_UNIFORM_EGL_OUTPUT_FORMAT;
               df_left = glsl_dataflow_construct_binary_op(DATAFLOW_SUB,
                                                           glsl_dataflow_construct_const_float(cf(1.0f)),
                                                           df_right);
               df_comp = glsl_dataflow_construct_cond_op(df_comp, df_left, df_right);
               g_BuiltInVar__gl_PointCoord->u.var_instance.scalar_values[1] = df_comp;
            }
            else
            {
               g_BuiltInVar__gl_PointCoord->u.var_instance.scalar_values[1] = glsl_dataflow_construct_linkable_value(DATAFLOW_VARYING, PRIM_FLOAT);
   ...
   g_BuiltInVar__gl_DepthRange->u.var_instance.scalar_values[0] = glsl_dataflow_construct_linkable_value(DATAFLOW_UNIFORM, PRIM_FLOAT);
   ...
   g_BuiltInVar__gl_DepthRange->u.var_instance.scalar_values[1] = glsl_dataflow_construct_linkable_value(DATAFLOW_UNIFORM, PRIM_FLOAT);
   ...
   g_BuiltInVar__gl_DepthRange->u.var_instance.scalar_values[2] = glsl_dataflow_construct_linkable_value(DATAFLOW_UNIFORM, PRIM_FLOAT);
               ...
               for (i = 0; i < type->scalar_count; i++)
               {
                  ...
                  switch (type_index)
                  {
                     ...
                     case PRIM_SAMPLERCUBE:
                        ...
                        result->u.var_instance.scalar_values[i] = glsl_dataflow_construct_const_sampler();
                        break;
         ...
         case TYPE_QUAL_UNIFORM:
            {
               ...
               for (i = 0; i < type->scalar_count; i++)
               {
                  ...
                  switch (type_index)
                  {
                     ...
                     case PRIM_INT:
                     case PRIM_FLOAT:
                        dataflow = glsl_dataflow_construct_linkable_value(dataflow_flavour, type_index);
                        break;

                     case PRIM_SAMPLER2D:
                     case PRIM_SAMPLEREXTERNAL:
                     case PRIM_SAMPLERCUBE:
                        dataflow = glsl_dataflow_construct_const_sampler();
                        break;
                  ...
                  if (dataflow_flavour == DATAFLOW_VARYING) {
                     if (g_ShaderFlavour == SHADER_VERTEX) {
                        ...
                        //dataflow = glsl_dataflow_construct_varying_tree(dataflow);
                     }
                  }
...
void glsl_symbol_construct_param_instance(Symbol* result, const char* name, SymbolType* type, TypeQualifier type_qual, ParamQualifier param_qual)
{
   ...
   result->u.var_instance.param_index = g_FunctionBuilderParamCount;
