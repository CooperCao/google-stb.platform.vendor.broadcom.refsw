...
Expr* glsl_expr_construct_subscript(Expr* aggregate, Expr* subscript)
{
   ...
   if (!subscript->constant_index_expression)
   {
      ...
      case SHADER_FRAGMENT:
         /* GNL: HACK this outrageously to support Asphalt 7 */
         if (aggregate->type == &primitiveTypes[PRIM_VEC4]) {
            Expr *(val[4]);
            Expr *(cond[4]);
            int i;
            for (i=0; i<4; i++) {
               Expr *e_i = glsl_expr_construct_value_int(i);
               val[i] = glsl_expr_construct_subscript(aggregate, e_i);
               cond[i] = glsl_expr_construct_binary_op_equality(EXPR_EQUAL, subscript, e_i);
            }
            return glsl_expr_construct_cond_op(cond[0], val[0],
               /* else */ glsl_expr_construct_cond_op(cond[1], val[1],
               /* else */ glsl_expr_construct_cond_op(cond[2], val[2], val[3]) ) );
         } else {
            // We shouldn't need to support any non-constant indexing at all in the fragment shader.
...
Expr* glsl_expr_construct_function_call(Symbol* overload_chain, ExprChain* args)
{
   ...
   if (const_function_code)
   {
      ...
      expr->u.function_call.function->u.function_instance.const_function_body_seen = true;
...
Expr* glsl_expr_construct_prim_constructor_call(PrimitiveTypeIndex return_index, ExprChain* args)
{
   ...
   else if (return_flags & (PRIM_VECTOR_TYPE | PRIM_MATRIX_TYPE))
   {
      ...
      /* Process arguments until we run out or the type is full */
      while (arg != NULL && return_components_left > 0)
      {
         arg_index = arg->expr->type->u.primitive_type.index;
         ...
         /* This arg may have components left, but these are dropped */
...
Expr* glsl_expr_construct_type_constructor_call(SymbolType* type, ExprChain* args)
{
   ...
   for (i = 0, arg = args->first; arg; i++, arg = arg->next)
   {
      ...
      {
         // Copy values.
         khrn_memcpy(dst, arg->expr->compile_time_value, arg->expr->type->size_as_const);
   ...
   case SYMBOL_PRIMITIVE_TYPE:
      ...
      if (aggregate->compile_time_value)
      {
         ...
         for (i = 0; i < len; i++)
         {
            ...
            khrn_memcpy(dst, src, component_size_as_const);
         }
      }
      else
      {
...
Expr* glsl_expr_construct_binary_op_arithmetic(ExprFlavour flavour, Expr* left, Expr* right)
{
...

   {
      ...
      if ( !(left_flags & PRIM_ARITHMETIC_BINARY) ||
         !(right_flags & PRIM_ARITHMETIC_BINARY)  )
         goto fail;
...

      {
         ...
         // Cases 2 and 3 - One scalar, one vector/matrix
         if (  ( left_scalar && !right_scalar ) ||
            ( right_scalar && !left_scalar )   )
         {
            /* Expr has the vector/matrix type */
            if (left_scalar) expr->type = right->type;
            else expr->type = left->type;
...
Expr* glsl_expr_construct_binary_op_logical(ExprFlavour flavour, Expr* left, Expr* right)
{
   ...
   left_index = left->type->u.primitive_type.index;
   if ( !(primitiveTypeFlags[left_index] & PRIM_LOGICAL_BINARY) ) goto fail;
...
Expr* glsl_expr_construct_intrinsic(ExprFlavour flavour, ExprChain* args)
{
   ...
   case EXPR_INTRINSIC_TEXTURE_2D_LOD:
      ...
      type = args->first->expr->type;
      if (type == &primitiveTypes[PRIM_SAMPLEREXTERNAL]) {
      ...
      } else if (type != &primitiveTypes[PRIM_SAMPLER2D]) {
         error = BAD_ARG_TYPES;
         goto end;
      }

      if (&primitiveTypes[PRIM_VEC2] != args->first->next->expr->type ||
         &primitiveTypes[PRIM_FLOAT] != args->first->next->next->expr->type)
      {
         error = BAD_ARG_TYPES;
         goto end;
      }
   ...
   case EXPR_INTRINSIC_TEXTURE_CUBE_LOD:
      {
         ...
         if (&primitiveTypes[PRIM_SAMPLERCUBE] != args->first->expr->type ||
            &primitiveTypes[PRIM_VEC3] != args->first->next->expr->type ||
            &primitiveTypes[PRIM_FLOAT] != args->first->next->next->expr->type)
         {
            error = BAD_ARG_TYPES;
            goto end;
   ...
   case EXPR_INTRINSIC_MAXABS:
      {
         ...
         if (&primitiveTypes[PRIM_FLOAT] != args->first->expr->type ||
            &primitiveTypes[PRIM_FLOAT] != args->first->next->expr->type)
         {
            error = BAD_ARG_TYPES;
            goto end;
         }
   ...
   if (error != 0)
   {
      glsl_compile_error(ERROR_CUSTOM, error, g_LineNumber, NULL);
      return NULL;
   }
...
Statement *glsl_statement_construct_iterator_for(Statement *init, Statement *cond_or_decl, Expr *loop, Statement *block)
{
   ...
   if (cond_or_decl != NULL && !cond_or_decl_is_bool(cond_or_decl)) {
