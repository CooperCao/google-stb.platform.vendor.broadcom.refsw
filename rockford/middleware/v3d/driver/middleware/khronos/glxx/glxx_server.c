...
static void create_ms_color(GLXX_SERVER_STATE_T *state, GLXX_TEXTURE_T *texture, uint32_t width, uint32_t height)
{
...
bool glxx_server_state_init(GLXX_SERVER_STATE_T *state, uint32_t name, uint64_t pid, MEM_HANDLE_T shared)
{
   ...
   */
   ...
   if (texture_external == MEM_INVALID_HANDLE)
       return false;
   ...
   */
   ...
   vcos_assert(state->mh_color_multi == MEM_INVALID_HANDLE);
   vcos_assert(state->mh_ds_multi == MEM_INVALID_HANDLE);
   vcos_assert(state->mh_preserve == MEM_INVALID_HANDLE);
                                   ...
                                   MEM_HANDLE_T hpreserve)
{
   ...
   MEM_ASSIGN(state->mh_preserve, hpreserve);
   state->swapchainc = swapchainc;
...
void glxx_server_state_term(void *v, uint32_t size)
{
   ...
   for (i = 0; i < GLXX_CONFIG_MAX_TEXTURE_UNITS; i++) {
      ...
      MEM_ASSIGN(state->bound_texture[i].mh_external, MEM_INVALID_HANDLE);
   ...
   MEM_ASSIGN(state->mh_default_texture_external, MEM_INVALID_HANDLE);
   ...
   if (state->mh_cache != MEM_INVALID_HANDLE) {
...
static KHRN_IMAGE_FORMAT_T set_palette(GLenum internalformat, const void *data, void **ret, uint32_t *ps)
{
   ...
   palette = (uint32_t *)khrn_platform_malloc(4 * palsize, "PALETTE");
   if (palette == NULL)
...
GLboolean glIsBuffer_impl (GLuint buffer)
{
   ...
   GLboolean result = glxx_shared_get_buffer((GLXX_SHARED_T *)mem_lock(state->mh_shared, NULL), buffer, false) != MEM_INVALID_HANDLE;
   mem_unlock(state->mh_shared);
...
GLboolean glIsTexture_impl (GLuint texture)
{
   ...
   GLboolean result = glxx_shared_get_texture((GLXX_SHARED_T *)mem_lock(state->mh_shared, NULL), texture) != MEM_INVALID_HANDLE;
   mem_unlock(state->mh_shared);
...
void glReadPixels_impl (GLint x, GLint y, GLsizei width, GLsizei height, GLenum format, GLenum type, GLint alignment, void *pixels)
{
   ...
   else
   {
      ...
      // We need to bump the nominal frame count so that EGLImages will convert next time around
      state->frame_number++;
...
#ifdef BIG_ENDIAN_CPU
      /* RGBA_8888_RSO means 0xRRGGBBAA as a variable, and RRGGBBAA in memory when written as a word (on big-endian machines) */
      if (format == GL_RGBA)
         khrn_image_interlock_wrap(&dst_wrap, RGBA_8888_RSO, width, height, k, 0, pixels, NULL);
      else if (format == GL_RGB)
         khrn_image_interlock_wrap(&dst_wrap, RGB_888_RSO, width, height, k, 0, pixels, NULL);
      else /* (format == GL_BGRA_EXT) */
         khrn_image_interlock_wrap(&dst_wrap, BGRA_8888_RSO, width, height, k, 0, pixels, NULL);
#else
      /* ABGR_8888_RSO means 0xAABBGGRR as a variable, and RRGGBBAA in memory when written as a word (on little-endian machines) */
      if (format == GL_RGBA)
         khrn_image_interlock_wrap(&dst_wrap, ABGR_8888_RSO, width, height, k, 0, pixels, NULL);
      else if (format == GL_RGB)
         khrn_image_interlock_wrap(&dst_wrap, RGB_888_RSO, width, height, k, 0, pixels, NULL);
      else /* (format == GL_BGRA_EXT) */
         khrn_image_interlock_wrap(&dst_wrap, ARGB_8888_RSO, width, height, k, 0, pixels, NULL);
#endif
...
GLboolean glTexImage2D_impl (GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, GLint alignment, const GLvoid *pixels)
{
   ...
   else
   {
      ...
      if (texture->mh_ms_image)
         create_ms_color(state, texture, width, height);
...
void glDiscardFramebufferEXT_impl(GLenum target, GLsizei numAttachments, const GLenum *attachments)
{
   ...
   else
   {
      ...
      if (!err)
         glxx_hw_invalidate_frame(state, color, ds, ds, color, color, false);
   }
...
GLboolean glIsRenderbuffer_impl (GLuint renderbuffer)
{
   ...
   GLboolean result = glxx_shared_get_renderbuffer((GLXX_SHARED_T *)mem_lock(state->mh_shared, NULL), renderbuffer, false) != MEM_INVALID_HANDLE;
   mem_unlock(state->mh_shared);
...
void glRenderbufferStorage_impl (GLenum target, GLenum internalformat, GLsizei width, GLsizei height)
{
   ...
   if (target != GL_RENDERBUFFER)
      ...
      if (!glxx_renderbuffer_storage((GLXX_RENDERBUFFER_T *)mem_lock(state->mh_bound_renderbuffer, NULL), internalformat, width, height))
         glxx_server_state_set_error(state, GL_OUT_OF_MEMORY);
   ...
   if (target == GL_RENDERBUFFER) {
      if (state->mh_bound_renderbuffer != MEM_INVALID_HANDLE) {
         ...
         case GL_RENDERBUFFER_INTERNAL_FORMAT:
             ...
             {
                switch (storage->format & ~IMAGE_FORMAT_MEM_LAYOUT_MASK) {
                case ABGR_8888:
...
GLboolean glIsFramebuffer_impl (GLuint framebuffer)
{
   ...
   GLboolean result = glxx_shared_get_framebuffer((GLXX_SHARED_T *)mem_lock(state->mh_shared, NULL), framebuffer, false) != MEM_INVALID_HANDLE;
   mem_unlock(state->mh_shared);
...
void glxx_RenderbufferStorageMultisampleEXT_impl (GLenum target, GLsizei samples, GLenum internalformat, GLsizei width, GLsizei height)
{
   GLXX_SERVER_STATE_T *state = GLXX_LOCK_SERVER_STATE();
   ...
   if (target != GL_RENDERBUFFER)
      ...
      if (!glxx_renderbuffer_storage_multisample((GLXX_RENDERBUFFER_T *)mem_lock(state->mh_bound_renderbuffer, NULL), samples, internalformat, width, height))
         glxx_server_state_set_error(state, GL_OUT_OF_MEMORY);
...
void glxx_FramebufferTexture2DMultisampleEXT_impl (GLenum target, GLenum a, GLenum textarget, GLuint texture, GLint level, GLsizei samples)
{
   GLXX_SERVER_STATE_T *state = GLXX_LOCK_SERVER_STATE();
               ...
               if (!glxx_is_texture_target(state, textarget))
                  ...
                  if (handle == MEM_INVALID_HANDLE)
                     ...
                     if (texture->mh_ms_image == MEM_INVALID_HANDLE)
                        create_ms_color(state, texture, texture->width, texture->height);
