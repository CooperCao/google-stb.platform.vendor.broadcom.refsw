...
{
   if (path->flags & VG_PATH_FLAG_INTERP_TO) {
      uint32_t size = mem_get_size(((VG_PATH_T *)mem_lock(path->extra.interp_to.from_a, NULL))->segments);
...
uint32_t vg_path_get_coords_size(VG_PATH_T *path)
{
   if (path->flags & VG_PATH_FLAG_INTERP_TO) {
      ...
      VG_PATH_T *begin = (VG_PATH_T *)mem_lock(path->extra.interp_to.from_a, NULL);
      uint32_t segments_count = mem_get_size(begin->segments);
      const uint8_t *segments = (const uint8_t *)mem_lock(begin->segments, NULL);
   ...
   float t)
{
   ...
   if ((mem_get_size(dst->segments) == 0) && /* nothing in the dst path at all */
      ...
      if (segments_count == mem_get_size(end->segments)) {
         bool ok = ok_for_late_interp((const uint8_t *)mem_lock(begin->segments, NULL), (const uint8_t *)mem_lock(end->segments, NULL), segments_count);
         ...
         if (ok) {
            ...
            if (begin->extra.n.u.interp_from.head_a != MEM_INVALID_HANDLE) {
               ((VG_PATH_T *)mem_lock(begin->extra.n.u.interp_from.head_a, NULL))->extra.interp_to.prev_a = dst_handle;
            ...
            if (end->extra.n.u.interp_from.head_b != MEM_INVALID_HANDLE) {
               ((VG_PATH_T *)mem_lock(end->extra.n.u.interp_from.head_b, NULL))->extra.interp_to.prev_b = dst_handle;
...
bool vg_path_get_length(MEM_HANDLE_T handle, VG_PATH_T *path, float *length, uint32_t segments_i, uint32_t segments_count)
{
   ...
   *length = vg_segment_lengths_get_length(
      (const uint8_t *)mem_lock(path->segments, NULL), segments_i, segments_count,
      mem_lock(path->extra.n.tess, NULL));
...
static void discard_interp_to(VG_PATH_T *path)
{
   ...
   */
   ...
   begin = (VG_PATH_T *)mem_lock(path->extra.interp_to.from_a, NULL);
   if (path->extra.interp_to.prev_a == MEM_INVALID_HANDLE) {
      begin->extra.n.u.interp_from.head_a = path->extra.interp_to.next_a;
   } else {
      ((VG_PATH_T *)mem_lock(path->extra.interp_to.prev_a, NULL))->extra.interp_to.next_a = path->extra.interp_to.next_a;
      mem_unlock(path->extra.interp_to.prev_a);
   }
   if (path->extra.interp_to.next_a != MEM_INVALID_HANDLE) {
      ((VG_PATH_T *)mem_lock(path->extra.interp_to.next_a, NULL))->extra.interp_to.prev_a = path->extra.interp_to.prev_a;
   ...
   */
   ...
   end = (VG_PATH_T *)mem_lock(path->extra.interp_to.from_b, NULL);
   if (path->extra.interp_to.prev_b == MEM_INVALID_HANDLE) {
      end->extra.n.u.interp_from.head_b = path->extra.interp_to.next_b;
   } else {
      ((VG_PATH_T *)mem_lock(path->extra.interp_to.prev_b, NULL))->extra.interp_to.next_b = path->extra.interp_to.next_b;
      mem_unlock(path->extra.interp_to.prev_b);
   }
   if (path->extra.interp_to.next_b != MEM_INVALID_HANDLE) {
      ((VG_PATH_T *)mem_lock(path->extra.interp_to.next_b, NULL))->extra.interp_to.prev_b = path->extra.interp_to.prev_b;
...
bool vg_path_bake_interps_from(VG_PATH_T *path)
{
   ...
   do {
      ...
      if (!vg_path_bake_interp_to((VG_PATH_T *)mem_lock(handle, NULL))) {
         mem_unlock(handle);
         return false;
      }
      mem_unlock(handle);
