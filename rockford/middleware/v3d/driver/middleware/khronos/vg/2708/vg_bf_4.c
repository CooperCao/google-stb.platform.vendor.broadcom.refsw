   ...
   float scale, float bias)
{
   ...
   temp = (float *)mem_lock(temp_handle, NULL);
...
static void do_clear_region(
   MSG_CLEAR_REGION_T *msg)
{
   khrn_image_clear_region((KHRN_IMAGE_T *)mem_lock(msg->image, NULL),
   ...
   void *message, uint32_t size)
{
   ...
   if (msg->type == MSG_TYPE_SCONV)
   {
      ...
      {
         vg_server_state_set_error((VG_SERVER_STATE_T *)mem_lock(msg->u.sconv.state, NULL), VG_OUT_OF_MEMORY_ERROR);
...
/* NEW_BE stuff to get around using the worker message queue */
static void post_end_now(MSG_T *msg, uint32_t size)
{
...
{
   KHRN_IMAGE_T *dst = (KHRN_IMAGE_T *)mem_lock(dst_handle, NULL);
   KHRN_IMAGE_T *src = (src_handle == MEM_INVALID_HANDLE) ? NULL : (KHRN_IMAGE_T *)mem_lock(src_handle, NULL);

   verify(khrn_interlock_write(&dst->interlock, KHRN_INTERLOCK_USER_TEMP));

   if (src)
      khrn_interlock_read(&src->interlock, KHRN_INTERLOCK_USER_TEMP);

   khrn_interlock_transfer(&dst->interlock, KHRN_INTERLOCK_USER_TEMP, KHRN_INTERLOCK_FIFO_WORKER);

   if (src)
   {
      khrn_interlock_transfer(&src->interlock, KHRN_INTERLOCK_USER_TEMP, KHRN_INTERLOCK_FIFO_WORKER);
   ...
   MEM_HANDLE_T src_handle, uint32_t src_x, uint32_t src_y)
{
   ...
   post_end_now(&msg, MSG_SIZE_COPY_REGION);
}
   ...
   const int32_t *scissor_rects, uint32_t scissor_rects_count)
{
   ...
   post_end_now(&msg, MSG_SIZE_COPY_SCISSOR_REGIONS);
}
   ...
   const float *matrix) /* matrix needs to be clean_float()ed */
{
   ...
   khrn_bf_color_matrix_prepare(
      ...
      matrix);
#endif
   post_end_now(&msg, MSG_SIZE_COLOR_MATRIX);
}
   ...
   KHRN_IMAGE_FORMAT_T output_format)
{
   ...
   post_end_now(&msg, MSG_SIZE_LOOKUP);
}
   ...
   KHRN_IMAGE_FORMAT_T output_format)
{
   ...
   post_end_now(&msg, MSG_SIZE_LOOKUP_SINGLE);
}
   ...
   MEM_HANDLE_T state_handle) /* for delayed error reporting */
{
   ...
   post_end_now(&msg, MSG_SIZE_SCONV);
}
   ...
   float scale, float bias)
{
   ...
   post_end_now(&msg, MSG_SIZE_CONV);
}
