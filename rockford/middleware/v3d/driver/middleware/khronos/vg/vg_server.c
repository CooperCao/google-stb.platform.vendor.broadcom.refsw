...
static uint32_t frame_width, frame_height, swapchainc;
...
MEM_HANDLE_T vg_server_shared_state_alloc(void)
{
   ...
   */
   ...
   shared_state = (VG_SERVER_SHARED_STATE_T *)mem_lock(handle, NULL);
...
MEM_HANDLE_T vg_server_state_alloc(MEM_HANDLE_T shared_state_handle, uint64_t pid)
{
   ...
   */
   ...
   state = (VG_SERVER_STATE_T *)mem_lock(handle, NULL);
...
{
   return (VGHandle)(handle + (VG_INVALID_HANDLE - MEM_INVALID_HANDLE));
}
...
void vg_buffers_changed(MEM_HANDLE_T f_handle, uint32_t m_swapchainc, MEM_HANDLE_T m_handle)
{
   frame_handle = f_handle;
   if (f_handle != MEM_INVALID_HANDLE) {
      KHRN_IMAGE_T *frame = (KHRN_IMAGE_T *)mem_lock(f_handle, NULL);
      frame_width = frame->width;
      frame_height = frame->height;
      swapchainc = m_swapchainc;
      mem_unlock(f_handle);
   }
...
uint32_t vg_get_swapchainc(void)
{
   return swapchainc;
}
...
void vg_compute_mask_clip_rec(MEM_HANDLE_T handle, VGMaskOperation operation,
   ...
   const VGint *values)
{
   ...
   case VG_CLIENT_OBJECT_TYPE_PAINT:
   {
      ...
      paint = (VG_PAINT_T *)mem_lock(handle, NULL);
      ...
      */
      ...
      case VG_PAINT_COLOR_RAMP_STOPS:
      {
         ...
         if (count == 0) {
            ...
            ramp_stops = (float *)mem_lock(paint->ramp_stops, NULL);
            for (i = 0; i != count; i += 5) {
   ...
   VGint *values)
{
   ...
   case VG_CLIENT_OBJECT_TYPE_FONT:
   {
      ...
      font = (VG_FONT_T *)mem_lock(handle, NULL);
   ...
   VGint width, VGint height)
{
   ...
   if (width > 0 && height > 0)
   {
      if ((mask_handle != MEM_INVALID_HANDLE) && /* all parameters have been checked now... */
         !vg_be_mask(handle, operation, dst_x, dst_y, width, height)) {
         set_error(state, VG_OUT_OF_MEMORY_ERROR);
      }
   }
   VG_UNLOCK_SERVER_STATE();
   ...
   VGMaskOperation operation)
{
   ...
   if (!vg_be_render_to_mask(state, handle, paint_modes, operation)) {
      set_error(state, VG_OUT_OF_MEMORY_ERROR);
   }
   VG_UNLOCK_SERVER_STATE();
}
   ...
   VGfloat value)
{
   ...
   mask_layer = (VG_MASK_LAYER_T *)mem_lock(handle, NULL);
   ...
   VGint width, VGint height)
{
   ...
   dst = (VG_MASK_LAYER_T *)mem_lock(dst_handle, NULL);
   ...
   VGbitfield caps)
{
   ...
   if (vg_is_path(handle)) {
      ...
      path_bprint = (VG_PATH_BPRINT_T *)mem_lock(handle, NULL);
   ...
   VGPath src_vg_handle)
{
   ...
   dst = (VG_PATH_T *)mem_lock(dst_handle, NULL);
   src = (VG_PATH_T *)mem_lock(src_handle, NULL);
   ...
   */
   ...
   khrn_memcpy((uint8_t *)mem_lock(dst->segments, NULL) + (mem_get_size(dst->segments) - segments_count), mem_lock(src->segments, NULL), segments_count);
   mem_unlock(src->segments);
   mem_unlock(dst->segments);

   dst_coords = (uint8_t *)mem_lock(dst->coords, NULL) + (mem_get_size(dst->coords) - dst_coords_size);
   src_coords = mem_lock(src->coords, NULL);
   ...
   VGuint coords_size, const void *coords)
{
   ...
   khrn_memcpy((uint8_t *)mem_lock(path->segments, NULL) + (mem_get_size(path->segments) - segments_count), segments, segments_count);
   mem_unlock(path->segments);

   path_coords = (uint8_t *)mem_lock(path->coords, NULL) + (mem_get_size(path->coords) - coords_size);
   if (path->datatype == VG_PATH_DATATYPE_F) {
      clean_floats((float *)path_coords, (const float *)coords, coords_size / sizeof(float));
   } else {
      khrn_memcpy(path_coords, coords, coords_size);
   }
   ...
   VGuint coords_offset, VGuint coords_size, const void *coords)
{
   ...
   path_coords = (uint8_t *)mem_lock(path->coords, NULL) + coords_offset;
   if (path->datatype == VG_PATH_DATATYPE_F) {
      clean_floats((float *)path_coords, (const float *)coords, coords_size / sizeof(float));
   } else {
      khrn_memcpy(path_coords, coords, coords_size);
   ...
   VGPath src_vg_handle)
{
   ...
   */
   ...
   segments_count = mem_get_size(src->segments);
   coords_count = get_transform_coords_count((const uint8_t *)mem_lock(src->segments, NULL), segments_count);
   mem_unlock(src->segments);
   dst_coords_size = coords_count * get_path_datatype_size((VGPathDatatype)dst->datatype);
   ...
   dst_segments = (uint8_t *)mem_lock(dst->segments, NULL) + (mem_get_size(dst->segments) - segments_count);
   src_segments = (const uint8_t *)mem_lock(src->segments, NULL);
   dst_coords = (uint8_t *)mem_lock(dst->coords, NULL) + (mem_get_size(dst->coords) - dst_coords_size);
   src_coords = mem_lock(src->coords, NULL);
   oo_dst_scale = recip_(dst->scale);
   s_x = 0.0f; s_y = 0.0f;
   ...
   VGfloat t)
{
   ...
   dst = (VG_PATH_T *)mem_lock(dst_handle, NULL);
   begin = (VG_PATH_T *)mem_lock(begin_handle, NULL);
   end = (VG_PATH_T *)mem_lock(end_handle, NULL);
   ...
   VGuint width, VGuint height)
{
   ...
   if (((int32_t)child_width != dst_width) ||
       ((int32_t)child_height != dst_height)) {
      /*
   ...
   dst_image = (KHRN_IMAGE_T *)mem_lock(dst->image, NULL);
   khrn_image_lock_interlock_wrap(dst_image, &dst_wrap, &dst_image->interlock);
   khrn_interlock_read_immediate(&tmp_interlock);
   ...
   khrn_interlock_write_immediate(&tmp_interlock);
   khrn_interlock_term(&tmp_interlock);
   ...
   VGint width, VGint height)
{
   ...
   if (((int32_t)child_width != src_width) ||
       ((int32_t)child_height != src_height)) {
      /*
         this can happen if we destroy the image and reuse the handle when
         creating another image all before this vgImageSubData command gets to
         the server. an appropriate error is therefore bad handle
   ...
   src_image = (KHRN_IMAGE_T *)mem_lock(src->image, NULL);
   khrn_image_lock_interlock_wrap(src_image, &src_wrap, &src_image->interlock);
   khrn_interlock_write_immediate(&tmp_interlock);
   ...
   VGint width, VGint height)
{
   ...
   int32_t actual_parent_width = 0, actual_parent_height = 0;
   ...
   {
      VG_IMAGE_BPRINT_T *parent = (VG_IMAGE_BPRINT_T *)mem_lock(parent_handle, NULL);
   ...
   {
      VG_IMAGE_T *parent = (VG_IMAGE_T *)mem_lock(parent_handle, NULL);
   ...
   {
      VG_CHILD_IMAGE_T *parent = (VG_CHILD_IMAGE_T *)mem_lock(parent_handle, NULL);
      actual_parent_width = parent->width;
      actual_parent_height = parent->height;
      mem_unlock(parent_handle);
      break;
   ...
   VGImage vg_handle)
{
   ...
   if (vg_is_image(handle)) {
      ...
      */
      ...
      ancestor_handle = ((VG_CHILD_IMAGE_T *)mem_lock(handle, NULL))->parent;
      mem_unlock(handle);
      for (;;) {
         if (vg_is_image(ancestor_handle)) {
            ...
            ancestor = (VG_CHILD_IMAGE_T *)mem_lock(ancestor_handle, NULL);
   ...
   VGint width, VGint height)
{
   ...
   dst = (KHRN_IMAGE_T *)mem_lock(frame_handle, NULL);
   khrn_image_lock_interlock_wrap(dst, &dst_wrap, &dst->interlock);
   ...
   VGint width, VGint height)
{
   ...
   src = (KHRN_IMAGE_T *)mem_lock(frame_handle, NULL);
   khrn_image_lock_interlock_wrap(src, &src_wrap, &src->interlock);
   ...
   VG_MAT3X3_T *t)
{
   ...
   i = *t;
   if (!vg_mat3x3_affine_try_invert(&i)) {
      return false;
   }
...
void vgDrawGlyphs_impl(
   VGFont vg_handle,
   VGuint glyphs_count, const VGuint *glyph_ids,
   ...
   const float *coords, uint32_t coords_count)
{
   ...
   path_coords = (uint8_t *)mem_lock(path->coords, NULL) + (mem_get_size(path->coords) - path_coords_size);
   ...
   bool first, bool close)
{
   ...
   path_segments = (uint8_t *)mem_lock(path->segments, NULL) + (mem_get_size(path->segments) - segments_count);
   khrn_memset(path_segments, VG_LINE_TO_ABS, segments_count);
   ...
   VGUArcType arc_type)
{
   ...
   khrn_memset(path_segments + 1, (angle_extent < 0.0f) ? VG_SCWARC_TO_ABS : VG_SCCWARC_TO_ABS, segments_count - 1);
...
VGImage vgCreateEGLImageTargetKHR_impl(VGeglImageKHR src_egl_handle, VGuint *format_width_height)
{
   ...
   src_handle = ((EGL_IMAGE_T *)mem_lock(eglimage_handle, NULL))->mh_image;
