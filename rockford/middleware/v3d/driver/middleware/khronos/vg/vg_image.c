   ...
   int32_t width, int32_t height)
{
   ...
   image_bprint = (VG_IMAGE_BPRINT_T *)mem_lock(handle, NULL);
...
bool vg_image_from_bprint(MEM_HANDLE_T handle)
{
   ...
   */
   ...
   image = (VG_IMAGE_T *)mem_lock(handle, NULL);
   ...
   MEM_HANDLE_T *handle_io)
{
   ...
   while (!vg_is_image(handle)) {
      ...
      child_image = (VG_CHILD_IMAGE_T *)mem_lock(handle, NULL);
      x += child_image->x;
      y += child_image->y;
      if (width == (uint32_t)-1) {
         width = child_image->width;
         height = child_image->height;
         vcos_assert(width != (uint32_t)-1);
      }
...
bool vg_image_leak(VG_IMAGE_T *image)
{
   if (khrn_image_is_ok_for_render_target(image->image_format, true)) {
      KHRN_IMAGE_T *image_image = (KHRN_IMAGE_T *)mem_lock(image->image, NULL);
      if (!khrn_image_can_use_as_render_target(image_image)) {
         ...
         mem_release(image->image);
         image->image = handle;
         image->image_format = ((KHRN_IMAGE_T *)mem_lock(handle, NULL))->format;
         mem_unlock(handle);
      } else {
         mem_unlock(image->image);
      }
   ...
   int32_t width, int32_t height)
{
   ...
   child_image_bprint = (VG_CHILD_IMAGE_T *)mem_lock(handle, NULL);
...
bool vg_child_image_from_bprint(MEM_HANDLE_T handle)
{
   ...
   */
   ...
   for (;;) {
      ...
      ancestor_parent_handle = ((VG_CHILD_IMAGE_T *)mem_lock(ancestor_handle, NULL))->parent;
