...
static bool increase_capacity(VG_FONT_T *font, uint32_t capacity)
{
   ...
   */
   ...
   old_entries = (uint32_t *)mem_lock(old_entries_handle, NULL);
   ...
   int32_t glyphs_capacity)
{
   ...
   font_bprint = (VG_FONT_BPRINT_T *)mem_lock(handle, NULL);
   ...
   VG_FONT_LOCKED_T *font_locked)
{
   ...
   font_locked->entries = (uint32_t *)mem_lock(font->entries, NULL);
   font_locked->slots = (VG_GLYPH_T *)mem_lock(font->slots, NULL);
}
