...
VGErrorCode vg_path_interpolate_slow(VG_PATH_T *dst, VG_PATH_T *begin, VG_PATH_T *end, float t)
{
   ...
   */
   ...
   if (segments_count == mem_get_size(end->segments)) {
      dst_coords_count = get_interpolate_coords_count((const uint8_t *)mem_lock(begin->segments, NULL), (const uint8_t *)mem_lock(end->segments, NULL), segments_count);
   ...
   begin_segments = (const uint8_t *)mem_lock(begin->segments, NULL);
   end_segments = (const uint8_t *)mem_lock(end->segments, NULL);
   dst_coords = (uint8_t *)mem_lock(dst->coords, NULL) + (mem_get_size(dst->coords) - dst_coords_size);
   begin_coords = mem_lock(begin->coords, NULL);
   end_coords = mem_lock(end->coords, NULL);
   omt = 1.0f - t;
...
bool vg_path_is_empty(MEM_HANDLE_T segments_handle)
{
   ...
   */
   ...
   uint8_t *segments = (uint8_t *)mem_lock(segments_handle, NULL);
