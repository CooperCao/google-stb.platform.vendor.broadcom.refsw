...
static GLXX_VEC4_T *texture_lookup_simple(uint32_t i, GLXX_VEC4_T *coord, bool *texture_rb_swap, bool point_coord, bool egl_output)
{
   ...
   sampler = glsl_dataflow_construct_const_sampler();
...
static GLXX_VEC4_T *texture_lookup_complex(uint32_t i, GLXX_VEC4_T *coord, bool *texture_rb_swap, bool point_coord, bool egl_output)
{
   Dataflow *rw = glxx_recip(coord->w);

   coord = glxx_vec4(glxx_mul(coord->x, rw), glxx_mul(coord->y, rw), NULL, NULL);
   return texture_lookup_simple(i, coord, texture_rb_swap, point_coord, egl_output);
}

static GLXX_VEC4_T *texturing(GLXX_FRAGMENT_CARD_T *card, GL11_CACHE_TEXUNIT_ABSTRACT_T *texunits, bool points, GLXX_VEC4_T *primary_color, Dataflow *point_x, Dataflow *point_y, bool *texture_rb_swap, bool egl_output)
{
   uint32_t i, j;
   ...
   for (i = 0; i < GL11_CONFIG_MAX_TEXTURE_UNITS; i++)
   {
      ...
      if (texunit->props.active)
      {
         ...
         if (texunit->props.complex)
            texture = texture_lookup_complex(i, coord, texture_rb_swap, points && texunit->coord_replace, egl_output);
         else
            texture = texture_lookup_simple(i, coord, texture_rb_swap, points && texunit->coord_replace, egl_output);
...
static Dataflow *perform_user_clip(int user_clip, bool egl_output)
{
   GLXX_VEC4_T *coord;
   Dataflow *scalar;
...
static GLXX_FRAGMENT_CARD_T *fshader(GL11_CACHE_KEY_T *v)
{
   ...
   if (v->user_clip_plane != 0)
      discard = glxx_logicor(discard, perform_user_clip(v->user_clip_plane, v->common.egl_output));

   /* Create fragment shader */
   card->root = glxx_backend(v->common.blend, color, discard, v->common.stencil_config, v->common.use_depth, v->common.render_alpha, v->common.rgb565, v->common.fb_rb_swap);
