...
void v3d_basic_logc_printer_line(struct v3d_basic_printer *root, const char *line)
{
   struct v3d_basic_logc_printer *p = (struct v3d_basic_logc_printer *)root;
   _VCOS_LOG_X(p->cat, p->level, "%s", line);
}

static const struct v3d_basic_printer_vtbl v3d_basic_logc_printer_vtbl = {
   V3D_BASIC_PRINTER_VTBL_INIT(v3d_basic_logc_printer)};

void v3d_basic_logc_printer_init(struct v3d_basic_logc_printer *p,
   VCOS_LOG_CAT_T *cat, VCOS_LOG_LEVEL_T level, const char *line_prefix)
{
   v3d_basic_printer_init(&p->base, line_prefix);
   p->base.base.vtbl = &v3d_basic_logc_printer_vtbl.base;
   p->cat = cat;
   p->level = level;
}
