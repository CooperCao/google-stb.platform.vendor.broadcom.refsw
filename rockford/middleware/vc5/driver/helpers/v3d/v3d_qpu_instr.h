...
static inline bool v3d_qpu_magic_waddr_is_tmu_direct(uint32_t addr)
{
   switch (addr)
   {
...
static inline bool v3d_qpu_magic_waddr_is_tmu_retiring(uint32_t addr)
{
   switch (addr)
   {
#if V3D_HAS_NEW_TMU_CFG
...
extern uint64_t v3d_qpu_instr_unused_mask(v3d_qpu_instr_type_t type, bool has_rotate);
