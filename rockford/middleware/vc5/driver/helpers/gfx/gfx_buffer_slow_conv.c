...
#include "helpers/gfx/gfx_buffer_uif_config.h"
#include "helpers/assert.h"
#include "tools/v3d/qpu_float/qpu_float.h"
#include "vcos.h"

#define VCOS_LOG_CATEGORY (&gfx_buffer_slow_conv_log)
static VCOS_LOG_CAT_T gfx_buffer_slow_conv_log =
   VCOS_LOG_INIT("gfx_buffer_slow_conv", VCOS_LOG_WARN);
         ...
         for (uint32_t y_in_blocks = 0; y_in_blocks != arrs[p].h; ++y_in_blocks)
         {
            ...
            for (uint32_t x_in_blocks = 0; x_in_blocks != arrs[p].w; ++x_in_blocks)
            {
               ...
               else
               {
                  ...
                  vcos_log_trace("%s: block_offset=%zu", VCOS_FUNCTION, block_offset);
                  if (VCOS_LOG_TRACE_ENABLED)
                  {
                     GFX_LFMT_BLOCK_ARR_SPRINT_ELEM_RAW(block_desc,
                        &arrs[p], x_in_blocks, y_in_blocks, z_in_blocks, /*compact=*/true);
                     vcos_log_trace("%s: block data: %s", VCOS_FUNCTION, block_desc);
                  }
               }
            }
         }
...
void gfx_buffer_blit_func(
   ...
   uint32_t width, uint32_t height, uint32_t depth)
{
   if (VCOS_LOG_INFO_ENABLED)
   {
      GFX_BUFFER_SPRINT_XFORM_SEQ(seq_desc, xform_seq);
      vcos_log_info("%s: %s", VCOS_FUNCTION, seq_desc);
   }
