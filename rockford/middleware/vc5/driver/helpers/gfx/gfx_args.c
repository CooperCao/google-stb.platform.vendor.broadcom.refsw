...
#include "helpers/gfx/gfx_args.h"
...
{
   return gfx_args_more(a) && gfx_try_strtoul(NULL, gfx_args_peek(a), 0);
}
...
{
   unsigned long ul;
   if (!gfx_try_strtoul(&ul, s, 0))
      gfx_args_fail(a, "Couldn't parse '%s' as a uint!", s);
   return ul;
}
