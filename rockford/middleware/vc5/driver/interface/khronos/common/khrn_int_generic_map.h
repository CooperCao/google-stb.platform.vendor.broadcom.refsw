...
/*
   ...
   - false is returned and map is still uninitialised, except for map->storage,
     which is "null"
*/
...
/*
   ...
   Either map->storage is "null", or map points to a valid map whose values are
   of type X:
   ...
   The structure map points to is uninitialised, except for map->storage, which
   is "null"
*/
...
extern bool khrn_generic_map(insert)(KHRN_GENERIC_MAP(T) *map,
   KHRN_GENERIC_MAP_KEY_T key, KHRN_GENERIC_MAP_VALUE_T value);

/* similar, but assumes key isn't present in the map */
extern bool khrn_generic_map(insert_not_present)(KHRN_GENERIC_MAP(T) *map,
   KHRN_GENERIC_MAP_KEY_T key, KHRN_GENERIC_MAP_VALUE_T value);
...
/* entry is allowed to be modified, but the key's hash must remain the same */
typedef void (*KHRN_GENERIC_MAP(MODIFY_CALLBACK_T))(KHRN_GENERIC_MAP(T) *map, KHRN_GENERIC_MAP(ENTRY_T) *entry, void *);
extern void khrn_generic_map(iterate_modify)(KHRN_GENERIC_MAP(T) *map,
   KHRN_GENERIC_MAP(MODIFY_CALLBACK_T) func, void *);
