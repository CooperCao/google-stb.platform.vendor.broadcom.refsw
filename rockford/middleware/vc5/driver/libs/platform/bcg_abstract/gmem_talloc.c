...
static block *talloc_block(void)
{
   ...
   if (b) {
      ...
      b->cur_map_ptr = gmem_map_internal(b->memory_handle, 0, BLOCK_SIZE, GMEM_USAGE_CPU_READ | GMEM_USAGE_CPU_WRITE, false);
...
static void* talloc_thread_main(void *arg)
{
   ...
   while (vcos_event_timed_wait(&handle->done, 1000) == VCOS_ETIMEDOUT) {
      ...
      // lock order is important, so you don't get deadlock
      // always lock from outer to inner
