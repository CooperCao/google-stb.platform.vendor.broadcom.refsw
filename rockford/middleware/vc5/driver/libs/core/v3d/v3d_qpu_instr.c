...
static bool sig_uses_cond(const struct v3d_qpu_sig *sig) {
   return sig->ldtlb || sig->ldtlbu || sig->ldvary || sig->ldtmu;
}
#endif
   ...
   uint32_t add_b, uint32_t waddr_a, uint32_t magic_add)
{
   ...
   case 188:
      ...
      switch(add_b)
      {
         case 0: op->opcode = V3D_QPU_OP_LDVPMV_IN; break;
         case 1: op->opcode = V3D_QPU_OP_LDVPMD_IN; break;
...
#else
      case V3D_QPU_OP_LDVPMV_IN: *op_add = 188; *add_b = 0; return !*magic_a;
      case V3D_QPU_OP_LDVPMD_IN: *op_add = 188; *add_b = 1; return !*magic_a;
#endif
   ...
   char err[V3D_QPU_INSTR_ERR_SIZE])
{
   ...
   if (v3d_qpu_instr_does_write(in->type))
   {
      ...
      if(in->type == V3D_QPU_INSTR_TYPE_ALU && sig_uses_cond(&in->u.alu.sig) )
      {
         ...
         if (cond != 0)
         {
            set_err(err, "Invalid combination of cond and sig");
            return false;
         }
#endif
      }
