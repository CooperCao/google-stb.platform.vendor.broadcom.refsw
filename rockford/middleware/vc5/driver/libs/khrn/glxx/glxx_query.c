   ...
   uint32_t render_rw_flags)
{
   ...
   block->handle = gmem_alloc(
      mem_size,
      mem_align,
      GMEM_USAGE_ALL | GMEM_USAGE_HINT_DYNAMIC,
      "Query buffer");
   if (!block->handle)
      goto error;

   // Zero fill and sync.
   block->cpu_addr = (uint32_t*)gmem_map(block->handle);
   if (!block->cpu_addr)
      goto error;

   memset(block->cpu_addr, 0, mem_size);
   gmem_sync_post_cpu_write(block->handle, GMEM_SYNC_CPU_WRITE);
...
static v3d_addr_t occlusion_query_counter_hw_addr(KHRN_FMEM_T *fmem, GLXX_QUERY_T *query)
{
   ...
   glxx_query_block *block = fmem_common->occlusion_query_list;
   ...
   if (block == NULL || block->used_queries == V3D_OCCLUSION_QUERY_COUNTER_SINGLE_CORE_CACHE_LINE_COUNTERS)
   {
      block = new_query_block(
         ...
         0,
         GMEM_SYNC_TLB_OQ_READ | GMEM_SYNC_TLB_OQ_WRITE);
      if (!block)
         return 0;

      block->prev = fmem_common->occlusion_query_list;
      fmem_common->occlusion_query_list = block;
   }
...
void glxx_occlusion_queries_update(glxx_query_block *query_list, bool valid_results)
{
   ...
   {
      if (valid_results)
         gmem_sync_pre_cpu_access(b->handle, GMEM_SYNC_CPU_READ);
...
{
   khrn_fmem_common_persist *fmem_common = khrn_fmem_get_common_persist(fmem);
   glxx_query_block *block = fmem_common->prim_counts_query_list;
   ...
   if (block == NULL || block->used_queries == num_queries)
   {
      ...
      block->prev = fmem_common->prim_counts_query_list;
      fmem_common->prim_counts_query_list = block;
   }
