...
bool glxx_server_tf_install_post_draw(GLXX_SERVER_STATE_T *state, GLXX_HW_RENDER_STATE_T *rs)
{
   ...
   GLXX_TRANSFORM_FEEDBACK_T *tf = state->transform_feedback.bound;
   const GLXX_PROGRAM_TFF_POST_LINK_T *ptf = &gl20_program_common_get(state)->transform_feedback;
   return glxx_tf_post_draw(rs, tf, ptf);
}
...
static void program_used_by_tf(KHRN_MAP_T *map, uint32_t key, void *value, void *info_)
{
...
GL_API void GL_APIENTRY glBeginTransformFeedback(GLenum primitiveMode)
{
   ...
   assert(tf->res_i == NULL);
   tf->res_i = khrn_res_interlock_create_no_handle();
   if (tf->res_i == NULL)
   {
   ...
   gmem_lock_list_init(&tf->lock_list);
   ...
   for (unsigned i = 0; i < tf->num_active_buffers; ++i)
   {
      ...
      active->res_i = glxx_buffer_get_res_interlock(binding->buffer.obj);
      ...
      if (active->res_i)
      {
         khrn_res_interlock_refinc(active->res_i);
         active->addr = gmem_lock(&tf->lock_list, active->res_i->handle);
      }
      else
      {
...
GL_API void GL_APIENTRY glEndTransformFeedback(void)
{
   ...
   for (unsigned i = 0 ; i < tf->num_active_buffers; i++)
   {
      ...
      tf->active_buffers[i].res_i = NULL;
