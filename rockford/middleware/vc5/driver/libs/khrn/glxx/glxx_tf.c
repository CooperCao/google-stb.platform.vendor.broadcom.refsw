...
static void tf_term(void *v, size_t size)
{
   ...
   {
      gmem_lock_list_unlock_and_destroy(&tf->lock_list);
      ...
      bool *requires_flush)
{
   ...
   for (unsigned i = 0; i < tf->num_active_buffers; ++i)
   {
      ...
      if (!khrn_fmem_record_res_interlock_self_read_conflicting_write(
            &rs->fmem, active->res_i, ACTION_BIN, requires_flush))
         return false;
   }
...
static bool emit_load_buffers_state(gmem_handle_t handle, KHRN_FMEM_T *fmem)
{
   ...
   v3d_addr_t addr = khrn_fmem_lock_and_sync(fmem, handle, GMEM_SYNC_PTB_PCF_READ, 0);
      ...
      KHRN_FMEM_T *fmem)
{
   ...
   {
      const glxx_tf_active_buffer *active = &active_buffers[i];
      v3d_addr_t addr = khrn_fmem_lock_and_sync(fmem, active->res_i->handle, GMEM_SYNC_PTB_TF_WRITE, 0);
      ...
      bool point_size_used)
{
   ...
   else
   {
      ...
      if (!khrn_fmem_record_res_interlock(fmem, tf->res_i, true, ACTION_BIN))
         return false;
...
static bool alloc_storage(KHRN_RES_INTERLOCK_T *res_i)
{
   ...
   gmem_handle_t handle = gmem_alloc(8 * sizeof(uint32_t), V3D_PRIM_COUNTS_ALIGN,
         GMEM_USAGE_V3D | GMEM_USAGE_HINT_DYNAMIC,
         "tf_storage");
...
bool glxx_store_tf_buffers_state(GLXX_HW_RENDER_STATE_T *rs)
{
   ...
   v3d_addr_t addr = khrn_fmem_lock_and_sync(&rs->fmem, res_i->handle, GMEM_SYNC_PTB_PCF_WRITE, 0);
