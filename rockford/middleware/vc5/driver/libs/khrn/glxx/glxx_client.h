...
static inline bool glxx_is_shader_type(GLenum type)
{
   return (type == GL_VERTEX_SHADER)   ||
          ...
          (type == GL_COMPUTE_SHADER && KHRN_GLES31_DRIVER) ||
          (type == GL_TESS_CONTROL_SHADER && (KHRN_GLES32_DRIVER || V3D_HAS_TNG)) ||
          (type == GL_TESS_EVALUATION_SHADER && (KHRN_GLES32_DRIVER || V3D_HAS_TNG)) ||
          (type == GL_GEOMETRY_SHADER && (KHRN_GLES32_DRIVER || V3D_HAS_TNG));
}
