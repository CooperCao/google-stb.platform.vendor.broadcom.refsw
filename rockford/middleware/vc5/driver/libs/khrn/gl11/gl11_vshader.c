                                     ...
                                     const struct spot_s *spot)
{
   ...
   spotAttenuation = glxx_cond(
      ...
      glxx_cfloat(0.0f),
      glxx_accu_pow(spotDot, spot->exponent));
                                ...
                                const struct mat_s   *m)
{
   ...
   nDotVP = glxx_fmax(zero, glxx_dot3(normal, VP));
   {
      ...
      glxx_v_accu_normalize3(&tmp, &tmp);
   ...
   pf = glxx_cond(
      ...
      zero,
      glxx_accu_pow(nDotHV, m->shininess));
                                              ...
                                              const struct point_params_s *params)
{
   ...
   attenuation = glxx_sum3(params->attenuation.x,
                                       ...
                                       const GLXX_VEC4_T *attr_normal)
{
   ...
   for (int j=0; j<16; j++) {
      unit_matrix[j] = glsl_dataflow_construct_address_load(DF_FLOAT, glxx_add(unit_idx, glxx_cint(4*j)));
   }
