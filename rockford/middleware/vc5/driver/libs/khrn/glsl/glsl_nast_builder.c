   ...
   switch (expr->flavour) {
      ...
      case EXPR_PRE_DEC:
         *result = false;
         break;
      case EXPR_INTRINSIC:
         if (expr->u.intrinsic.flavour == INTRINSIC_ATOMIC_ADD  ||
             expr->u.intrinsic.flavour == INTRINSIC_ATOMIC_SUB  ||
             expr->u.intrinsic.flavour == INTRINSIC_ATOMIC_MIN  ||
             expr->u.intrinsic.flavour == INTRINSIC_ATOMIC_MAX  ||
             expr->u.intrinsic.flavour == INTRINSIC_ATOMIC_AND  ||
             expr->u.intrinsic.flavour == INTRINSIC_ATOMIC_OR   ||
...
{
   glsl_nstmt_list_add(nstmts, glsl_nstmt_new_assign(rvalue->line_num, lvalue, rvalue));
}
...
static Expr *eval_expr(NStmtList *nstmts, Expr *expr, bool make_lvalue)
{
   ...
   if (!make_lvalue && expr_has_no_side_effects(expr)) {
      ...
      case EXPR_LOGICAL_OR: {
         ...
         glsl_nstmt_list_add(nstmts, glsl_nstmt_new_selection(expr->line_num, cond, other, NULL));
         return temp;
      }
      case EXPR_CONDITIONAL: {
         Expr *temp = glsl_expr_construct_instance(glsl_symbol_construct_temporary(expr->type));
         ...
         glsl_nstmt_list_add(nstmts, glsl_nstmt_new_selection(expr->line_num, cond, if_true, if_false));
         return temp;
      }
      case EXPR_POST_INC:
      case EXPR_POST_DEC:
      ...
      case EXPR_FUNCTION_CALL: {
         ...
         // call the function
         glsl_nstmt_list_add(nstmts, glsl_nstmt_new_function_call(expr->line_num, result, function, args));
...
static void eval_stmt(NStmtList *nstmts, Statement *stmt)
{
   ...
   switch(stmt->flavour) {
      ...
      case STATEMENT_FUNCTION_DEF: {
         ...
         def = glsl_nstmt_new_function_def(stmt->line_num, function, body);
      ...
      case STATEMENT_VAR_DECL: {
         ...
         glsl_nstmt_list_add(nstmts, glsl_nstmt_new_var_decl(stmt->line_num, var, initializer));
         break;
      }
      ...
      case STATEMENT_EXPR: {
         ...
         if (expr->flavour == EXPR_ASSIGN && expr_has_no_side_effects(expr->u.assign_op.rvalue)) {
            ...
            eval_root_expr(nstmts, stmt->u.expr.expr);
         }
         break;
      }
      case STATEMENT_SELECTION: {
         ...
         glsl_nstmt_list_add(nstmts, glsl_nstmt_new_selection(stmt->line_num, cond, if_true, if_false));
         break;
      }
      case STATEMENT_SWITCH: {
         ...
         glsl_nstmt_list_add(nstmts, glsl_nstmt_new_switch(stmt->line_num, cond, statements));
         break;
      }
      ...
      case STATEMENT_CONTINUE:
         glsl_nstmt_list_add(nstmts, glsl_nstmt_new(NSTMT_CONTINUE, stmt->line_num));
         break;

      case STATEMENT_BREAK:
         glsl_nstmt_list_add(nstmts, glsl_nstmt_new(NSTMT_BREAK, stmt->line_num));
         break;

      case STATEMENT_CASE:
         assert(stmt->u.case_stmt.expr->compile_time_value);
         glsl_nstmt_list_add(nstmts, glsl_nstmt_new_case(stmt->line_num, stmt->u.case_stmt.expr));
         break;

      case STATEMENT_DEFAULT:
         glsl_nstmt_list_add(nstmts, glsl_nstmt_new(NSTMT_DEFAULT, stmt->line_num));
         break;

      case STATEMENT_DISCARD:
         glsl_nstmt_list_add(nstmts, glsl_nstmt_new(NSTMT_DISCARD, stmt->line_num));
         break;

      case STATEMENT_RETURN:
         glsl_nstmt_list_add(nstmts, glsl_nstmt_new(NSTMT_RETURN, stmt->line_num));
         break;

      case STATEMENT_RETURN_EXPR: {
         Expr *expr = eval_root_expr(nstmts, stmt->u.return_expr.expr);
         glsl_nstmt_list_add(nstmts, glsl_nstmt_new_return_expr(stmt->line_num, expr));
         break;
      }
      case STATEMENT_BARRIER:
         glsl_nstmt_list_add(nstmts, glsl_nstmt_new(NSTMT_BARRIER, stmt->line_num));
         break;
