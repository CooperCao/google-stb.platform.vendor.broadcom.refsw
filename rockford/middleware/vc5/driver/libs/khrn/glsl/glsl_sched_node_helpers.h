...
static inline Backflow *tr_unif(uint32_t i)                      { return tr_typed_uniform(BACKEND_UNIFORM_PLAIN, i); }
static inline Backflow *tr_uniform_address(uint32_t row)         { return tr_typed_uniform(BACKEND_UNIFORM_ADDRESS, row); }
static inline Backflow *tr_uniform_block_address(uint32_t index) { return tr_typed_uniform(BACKEND_UNIFORM_UBO_ADDRESS, index); }
