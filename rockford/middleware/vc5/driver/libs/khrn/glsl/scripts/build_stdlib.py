...
def output_param(outf, param, primtypemap):
    print >>outf, "    glsl_symbol_construct_param_instance(&glsl_stdlib_params[%s], \"\", &%s, STORAGE_NONE, %s, %s);" % (
         param_index(param), typeenumname(make_type(None, param),primtypemap), param_qual(param), mem_quals(param))
...
def memory_qualifiers(vtype):
    ...
    return filter(lambda a: (a in qs), vtype);
...
def make_type(var_name, var_type):
    ...
    residual = filter(lambda a: (a != type_qual) and (a != storage_qual) and (a not in memory_quals) and (a != precision), var_type)
    tname = None
    array_levels = []
    for r in residual:
        if r[0] == "[":
