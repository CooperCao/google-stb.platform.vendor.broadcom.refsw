    ...
    for line in lines:
        ...
        while len(active_string)>0:
            ...
            if active_braces > 0:
                ...
                for i in xrange(len(active_string)):
                    if active_string[i] == "{":
                        active_braces += 1
        ...
        if not line[0].isspace():
            ...
            if len(line_props) > 0:
                if len(line_props[-1]) == 0 or line_props[-1][-1] != ":":
                    print >>sys.stderr, "Error: property lines must end with a ':'"
                    sys.exit(1)
            ...
            if len(active_props) == 0:
                print >>sys.stderr, "Error: bad indentation in properties file"
                sys.exit(1);
            else:
                ...
                if len(directives) > 0 or (len(functions) == 0 and len(variables)==0 and len(structs)==0):
                    print >>sys.stderr, "Error: Bad line %s in properties file" % line
                    sys.exit(1);
        ...
        for i,f in enumerate(fields):
            if f == "y" or f == "Y" or f == "t" or f == "T":
                ...
                if match:
                    ...
                    value = set(filter(lambda a: len(a) > 0, f.split(",")))
                else:
