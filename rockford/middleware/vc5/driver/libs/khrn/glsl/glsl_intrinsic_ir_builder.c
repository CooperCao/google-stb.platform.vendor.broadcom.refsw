...
static void calculate_dataflow_texture_lookup(BasicBlock *ctx, Dataflow **scalar_values, Expr *expr) {
   ...
   if (offset) {
      ...
      int const_offsets[3] = {0, };

      /* We don't support packing I_OFF into the constant offset part of the param */
      bool all_const = !(bits_value & DF_TEXBITS_I_OFF);
      for (int i=0; i<n_offsets && all_const; i++) {
         if (offset_scalar_values[i]->flavour == DATAFLOW_CONST)
            const_offsets[i] = offset_scalar_values[i]->u.constant.value;
         else
            all_const = false;
      }

      if (all_const)
         bits_value |= DF_TEXBITS_OFFSET(const_offsets[0], const_offsets[1], const_offsets[2]);
      else
...
static void calculate_dataflow_atomic_load(BasicBlock *ctx, Dataflow **scalar_values, Expr *expr) {
   ...
   scalar_values[0] = glsl_dataflow_construct_address_load(DF_UINT, addr);
}
