...
/* Log messages are output with multiple printfs, so we need a
 * global lock to avoid messages getting mixed up */
#define USE_PRINTF_LOCK

#ifdef USE_PRINTF_LOCK
...
{
   assert(!_vcos_thread_current_key_created);
...
static void _task_timer_expiration_routine(void *cxt)
{
   ...
   assert(thread->orig_task_timer_expiration_routine);
   thread->orig_task_timer_expiration_routine(thread->orig_task_timer_context);
   thread->orig_task_timer_expiration_routine = NULL;
}

static void ___unique_function_name_in_this_shared_library___() {}
static void *s_moduleHandle = NULL;

static VCOS_STATUS_T vcos_init_module_handle(void)
{
   if (s_moduleHandle == NULL)
   {
      // Find out where our own .so file lives. Opening it by name is the only way to be able
      // to get at the real symbol (unaffected by any LD_PRELOAD).
      Dl_info info;
      dladdr(___unique_function_name_in_this_shared_library___, &info);

      // Open our own .so
      s_moduleHandle = dlopen(info.dli_fname, RTLD_LAZY);
      ...
      if (s_moduleHandle == NULL)
         s_moduleHandle = dlopen(NULL, RTLD_LAZY);
   }

   return s_moduleHandle == NULL ? VCOS_EEXIST : VCOS_SUCCESS;
}

static void vcos_term_module_handle()
{
   if (s_moduleHandle != NULL)
   {
...
void *vcos_get_module_proc_address(const char *procname)
{
   ...
   return dlsym(s_moduleHandle, procname);
}
...
#else
   demand(local_attrs->ta_stackaddr == 0);
#endif
...
int vcos_log_to_file = 0;
#endif
int vcos_log_level_prefix = 0;

static char log_char[] =
{
   'U', /* VCOS_LOG_UNINITIALIZED */
   'N', /* VCOS_LOG_NEVER */
   'E', /* VCOS_LOG_ERROR */
   'W', /* VCOS_LOG_WARN */
   'I', /* VCOS_LOG_INFO */
...
void vcos_vlog_default_impl(const VCOS_LOG_CAT_T *cat, VCOS_LOG_LEVEL_T _level, const char *fmt, va_list args)
{
   ...
   if(NULL != log_fhandle)
   {
      if (vcos_log_level_prefix)
         fprintf(log_fhandle, "%c ", log_char[_level]);
...
enum
{
   VCOS_INIT_PRINTF_LOCK = (1 << 0),
   VCOS_INIT_MAIN_SEM    = (1 << 1),
...
void _vcos_task_timer_set(void (*pfn)(void*), void *cxt, VCOS_UNSIGNED ms)
{
   ...
   assert(thread->orig_task_timer_expiration_routine == NULL);
...
VCOS_THREAD_T *vcos_dummy_thread_create(void)
{
   ...
   thread_hndl = (VCOS_THREAD_T *)malloc(sizeof(VCOS_THREAD_T));
   assert(thread_hndl != NULL);
...
static void* _timer_thread(void *arg)
{
   ...
   {
      struct timespec now;
      // Make a local copy so we do not get a data race when another thread tries to change the
      // expires value whilst in the timedwait
      struct timespec expires = timer->expires;

      /* Wait until next expiry time, or until timer's settings are changed */
      if (_timespec_is_zero(&expires))
         pthread_cond_wait(&timer->settings_changed, &timer->lock);
      else
         pthread_cond_timedwait(&timer->settings_changed, &timer->lock, &expires);

      /* See if the timer has expired - reloop if it didn't */
      /* Return to using the timer->expires as may have been changed by another thread during the wait */
...
void vcos_timer_delete_pthreads(VCOS_TIMER_T *timer)
{
    ...
    */
   assert(pthread_self() != timer->thread);
...
const char * vcos_thread_current_get_name(void)
{
   VCOS_THREAD_T *self = vcos_thread_current();
   return self ? vcos_thread_get_name(self) : "<noname>";
}
