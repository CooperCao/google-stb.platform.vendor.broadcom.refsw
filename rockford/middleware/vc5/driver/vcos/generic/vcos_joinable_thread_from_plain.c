...
#include "generic/vcos_joinable_thread_from_plain.h"
...
static void vcos_thread_wrapper(void *arg)
{
   ...
   assert(thread->magic == VCOS_THREAD_MAGIC);
                                 ...
                                 void *arg)
{
   ...
   if (!stack && VCOS_CAN_SET_STACK_ADDR)
   {
      allocstack = malloc(attrs->ta_stacksz);
      if (!allocstack)
      {
         status = VCOS_ENOMEM;
         goto fail_stack;
                      ...
                      void **pData)
{
   assert(thread);
   assert(thread->magic == VCOS_THREAD_MAGIC);
   assert(!thread->joined); /* can't join more than once! */
                                         ...
                                         VCOS_UNSIGNED autostart)
{
   ...
   if (affinity == 0)
      affinity = VCOS_AFFINITY_CPU0; // ensure we explicitly set affinity
   ...
   assert((autostart == VCOS_START) || (autostart == VCOS_NO_START));
...
static void thread_timer_func(void *cxt)
{
   ...
   assert(self);
   assert(self->magic == VCOS_THREAD_MAGIC);
   assert(self->_timer.pfn);
   self->_timer.pfn = NULL;
   pfn(self->_timer.cxt);
}
...
extern VCOS_STATUS_T vcos_thread_at_exit(void (*pfn)(void*), void *cxt)
{
   ...
   if (!self)
   {
      assert(0); /* "vcos_thread_at_exit() : no thread." */
      return VCOS_EINVAL;
   }
   else if (self->magic != VCOS_THREAD_MAGIC)
   {
       assert(0); /* "vcos_thread_at_exit() : bad magic - not a vcos thread." */
       return VCOS_EINVAL;
   }
   else
   {
      assert(self->joined == 0); /* "vcos_thread_at_exit() - thread %s is already joined.", self->name ? self->name : "<unnamed>" */
      for (i=0; i<VCOS_MAX_EXIT_HANDLERS; i++)
      {
