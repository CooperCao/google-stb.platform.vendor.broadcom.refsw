...
#define VCOS_LOG_CATEGORY (&khrn_fence_cat)
static VCOS_LOG_CAT_T khrn_fence_cat =
   VCOS_LOG_INIT("khrn_fence_cat", VCOS_LOG_WARN);
...
{
   khrn_fence_flush(fence);
   return v3d_scheduler_create_fence(&fence->deps, deps_state);
}
...
void khrn_fence_print(const KHRN_FENCE_T *fence)
{
   ...
   vcos_log_error("%s fence=0x%p users=0x%x deps.n=%d", VCOS_FUNCTION, fence,
         fence->users, fence->deps.n);
   for (i = 0; i < fence->deps.n; i++)
   {
      vcos_log_error(" dependency[%d]=%" PRIu64 " ", i, fence->deps.dependency[i]);
   }
}
