...
static VCOS_LOG_CAT_T log_cat = VCOS_LOG_INIT("khrn_fmem_pool",
      VCOS_LOG_WARN);
...
bool khrn_fmem_pool_check_flush(KHRN_FMEM_POOL_T *pool, KHRN_RENDER_STATE_T *rs)
{
   /* if we are the rs that is using most of the buffers */
   if (pool->n_buffers >= KHRN_FMEM_MAX_PER_RS)
   {
...
void* khrn_fmem_pool_alloc(KHRN_FMEM_POOL_T *pool)
{
   ...
   if (buffer == NULL)
   {
      ...
      if (khrn_fmem_client_pool_get_num_free_and_submitted() != 0)
      {
         do
         {
            v3d_scheduler_wait_any();
            buffer = client_pool_get_buffer();
         }while(buffer == NULL);
      }
      else
      {
         vcos_log_warn("%s: all fmems are in use",
               VCOS_FUNCTION);
         return NULL;
      }
   }
   ...
   if (buffer->handle == GMEM_HANDLE_INVALID)
   {
      ...
      if (buffer->handle == GMEM_HANDLE_INVALID)
      {
         vcos_log_error("[%s] gmem_alloc failed", VCOS_FUNCTION);
         goto fail;
      }
  }
