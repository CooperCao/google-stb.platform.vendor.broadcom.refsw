...
#include "middleware/khronos/common/khrn_mr_crc.h"
#include "middleware/khronos/common/khrn_render_state.h"
#include "middleware/khronos/common/khrn_record.h"
#include "middleware/khronos/common/khrn_synclist_validate.h"
...
static VCOS_LOG_CAT_T log_cat = VCOS_LOG_INIT("khrn_fmem", VCOS_LOG_WARN);
...
{
   gmem_handle_t handle = gmem_alloc(size, V3D_TILE_STATE_ALIGN, GMEM_USAGE_V3D | GMEM_USAGE_HINT_DYNAMIC, "tile state");
...
bool khrn_fmem_check_flush(KHRN_FMEM_T *fmem)
{
   khrn_fmem_common_persist *fmem_common = khrn_fmem_get_common_persist(fmem);
   return khrn_fmem_pool_check_flush(&fmem_common->pool, fmem->render_state);
}
