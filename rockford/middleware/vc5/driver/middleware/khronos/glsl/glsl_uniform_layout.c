...
int deep_member_count(const MEMBER_LAYOUT_T *layout)
{
   ...
   switch(layout->flavour)
   {
      ...
      case MEMBER_PRIMITIVE_MATRIX:
         assert(layout->type->flavour==SYMBOL_PRIMITIVE_TYPE);
         return 1;
      case MEMBER_ARRAY:
         assert(layout->type->flavour==SYMBOL_ARRAY_TYPE);
         if(layout->type->u.array_type.member_type->flavour==SYMBOL_PRIMITIVE_TYPE)
            return 1;
         else
         {
            for(i=0; i<layout->u.array_layout.member_count; i++)
               count += deep_member_count(&layout->u.array_layout.member_layouts[i]);
            return count;
         }
      case MEMBER_STRUCT:
         assert(layout->type->flavour==SYMBOL_STRUCT_TYPE);
         for(i=0; i<layout->u.struct_layout.member_count; i++)
            count += deep_member_count(&layout->u.struct_layout.member_layouts[i]);
         return count;
      case MEMBER_BLOCK:
         assert(layout->type->flavour==SYMBOL_BLOCK_TYPE);
         for(i=0; i<layout->u.block_layout.member_count; i++)
            count += deep_member_count(&layout->u.block_layout.member_layouts[i]);
         return count;
      default:
         UNREACHABLE();
         return 0;
...
static int get_layout_spec(LayoutQualifier *lq) {
   if( (lq->qualified & UNIF_QUALED) && (lq->unif_bits & LAYOUT_STD140) )
      return STD140;
   else
      return STD430;
}
...
static int calculate_array_layout(MEMBER_LAYOUT_T *layout, SymbolType *type, int offset, int level_start, LayoutQualifier *lq, int layout_spec)
{
   ...
   layout->u.array_layout.member_layouts = malloc_fast(type->u.array_type.member_count * sizeof(MEMBER_LAYOUT_T));
   ...
   for(unsigned i=0; i<type->u.array_type.member_count; i++)
   {
