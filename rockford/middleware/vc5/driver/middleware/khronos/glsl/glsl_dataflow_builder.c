...
static void build_loads(Dataflow **scalar_offsets, Dataflow *base, Dataflow *offset,
                        MEMBER_LAYOUT_T *layout, SymbolType *type)
{
   assert(layout->flavour!=MEMBER_BLOCK);
      ...
      case MEMBER_STRUCT:
         {
            ...
            for(unsigned i=0; i<layout->u.struct_layout.member_count; i++)
            {
               ...
               build_loads(scalar_offsets + member_scalar_offset, base, member_offset, &layout->u.struct_layout.member_layouts[i], type->u.struct_type.member[i].type);
      ...
      case MEMBER_ARRAY:
         {
            ...
            for(unsigned i=0; i<layout->u.array_layout.member_count; i++)
            {
               ...
               build_loads(scalar_offsets + i * member_scalars, base, new_off, &layout->u.array_layout.member_layouts[i], type->u.array_type.member_type);
...
static void calculate_buffer_address(BasicBlock *ctx, Dataflow **address, Dataflow **offset, MEMBER_LAYOUT_T **layout, Expr *expr)
{
   Symbol *block_symbol;
      ...
      case EXPR_INSTANCE:
         {
            ...
            *layout = expr->u.instance.symbol->u.var_instance.block_info.layout;
            block_symbol = expr->u.instance.symbol->u.var_instance.block_info.block_symbol;
      ...
      case EXPR_FIELD_SELECTOR:
         {
            ...
            Dataflow *off;
            calculate_buffer_address(ctx, address, &off, &aggregate_layout, expr->u.field_selector.aggregate);
      ...
      case EXPR_SUBSCRIPT:
         {
                ...
                aggregate->type->u.array_type.member_type->flavour==SYMBOL_BLOCK_TYPE )
            {
               ...
               block_symbol = aggregate->u.instance.symbol->u.var_instance.block_info.block_symbol;
               block_type   = aggregate->type->u.array_type.member_type;
            ...
            else
            {
               ...
               Dataflow *off;
               // Recurse on aggregate.
               calculate_buffer_address(ctx, address, &off, &aggregate_layout, aggregate);
               ...
               switch(aggregate_layout->flavour)
               {
                  ...
                  case MEMBER_PRIMITIVE_NONMATRIX:
                     {
                        *layout = construct_prim_scalar_layout();
...
static void buffer_load_expr_calculate_dataflow(BasicBlock *ctx, Dataflow **scalar_values, Expr *expr)
{
   ...
   calculate_buffer_address(ctx, &address, &offset, &layout, expr);
   build_loads(scalar_values, address, offset, layout, expr->type);
}
...
static bool in_addressable_memory(Expr* expr)
{
   switch(expr->flavour)
   {
      case EXPR_INSTANCE:
