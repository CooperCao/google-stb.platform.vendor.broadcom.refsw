...
#define CONFLICT_PERIPH  (1<<4)      /* At most 1 peripheral access per instr */
#define CONFLICT_TLB     (1<<5)      /* TLB access has further restrictions */
...
/* A/B Regfile read conflicts handled separately */
...
typedef struct
{
   uint32_t current_instruction; /* Roughly where we are. */
   ...
   glsl_binary_shader_flavour_t shader_flavour; /* For determining where the final thrsw goes */
