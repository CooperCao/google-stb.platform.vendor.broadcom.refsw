...
// A special case where the third component value is ignored
...
vec4 __brcm_cubeShadow(vec4 coord) {
...
vec4  texelFetch ( sampler2DMS sampler, ivec2 P, int s) { return $$texture(8, sampler, 2*P + ivec2(s & 1, s >> 1), 0); }
ivec4 texelFetch (isampler2DMS sampler, ivec2 P, int s) { return $$texture(8, sampler, 2*P + ivec2(s & 1, s >> 1), 0); }
uvec4 texelFetch (usampler2DMS sampler, ivec2 P, int s) { return $$texture(8, sampler, 2*P + ivec2(s & 1, s >> 1), 0); }
...
vec4 texture2DLod(sampler2D sampler, vec2 coord, float lod) { return $$texture(64, sampler,coord,lod); }

vec4 texture2DProjLod(sampler2D sampler, vec3 coord, float lod) {
   return $$texture(64, sampler,__brcm_proj(coord),lod);
}
vec4 texture2DProjLod(sampler2D sampler, vec4 coord, float lod) {
   return $$texture(64, sampler,__brcm_proj2D(coord),lod);
}
...
vec4 textureCubeLod(samplerCube sampler, vec3 coord, float lod) {
   return $$texture(64, sampler,__brcm_cube(coord),lod);
}
