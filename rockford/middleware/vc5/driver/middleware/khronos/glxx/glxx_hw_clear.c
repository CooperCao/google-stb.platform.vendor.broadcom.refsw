...
bool glxx_hw_clear(GLXX_SERVER_STATE_T *state, GLXX_CLEAR_T *clear)
{
...

   {
      ...
      if (fmem_common->query_list != NULL || rs->tf_used)
      {
         keep_cl = true;
      }
      else
      {
         for (b = 0; b < hw_fb->rt_count; ++b)
            {
               if (!gfx_bufstate_can_discard_cl(rs->color_buffer_state[b],
            ...
            if (hw_fb->ms &&
                !gfx_bufstate_can_discard_cl(rs->ms_color_buffer_state[b],
                   clear->color && glxx_fb_is_valid_draw_buf(fb, GLXX_COLOR0_ATT + b),
                   partial_clear))
            {
               keep_cl = true;
         ...
         if (!gfx_bufstate_can_discard_cl(rs->depth_buffer_state, clear->depth, partial_clear))
            keep_cl = true;
         if (!gfx_bufstate_can_discard_cl(rs->stencil_buffer_state, clear->stencil, partial_clear))
            keep_cl = true;
      }
      ...
      if (!keep_cl)
      {
         for (b = 0; b < hw_fb->rt_count; ++b)
         {
            gfx_bufstate_discard_cl(&rs->color_buffer_state[b]);
            gfx_bufstate_discard_cl(&rs->ms_color_buffer_state[b]);
         }
         gfx_bufstate_discard_cl(&rs->depth_buffer_state);
         gfx_bufstate_discard_cl(&rs->stencil_buffer_state);
      }
      ...
      if (clear->color)
      {
         ...
         while (glxx_fb_iterate_valid_draw_bufs(fb, &i, &att_index))
         {
            ...
            if (clear->color_buffer_mask & (1 << b))
            {
               gfx_bufstate_t col_state = rs->color_buffer_state[b];
               gfx_bufstate_t ms_state = rs->ms_color_buffer_state[b];

               if (gfx_bufstate_try_fast_clear(&col_state, partial_clear) &&
                  (!hw_fb->ms || gfx_bufstate_try_fast_clear(&ms_state, partial_clear)))
               {
      ...
      if (clear->depth)
      {
         if (gfx_bufstate_try_fast_clear(&rs->depth_buffer_state, partial_clear))
         {
      ...
      if (clear->stencil)
      {
         if (gfx_bufstate_try_fast_clear(&rs->stencil_buffer_state, partial_clear))
         {
