...
#ifdef KHRN_GEOMD
#include "tools/v3d/geomd/geomd.h"
#endif
      ...
      stencil)
{
   ...
   for (b = 0; b < GLXX_MAX_RENDER_TARGETS; ++b)
   {
      ...
      if (color)
         gfx_bufstate_invalidate(&rs->color_buffer_state[b]);

      if (multisample)
         gfx_bufstate_invalidate(&rs->ms_color_buffer_state[b]);
   }

   if (depth)
      gfx_bufstate_invalidate(&rs->depth_buffer_state);

   if (stencil)
      gfx_bufstate_invalidate(&rs->stencil_buffer_state);
}

static bool install_image_plane(KHRN_IMAGE_PLANE_T *src, bool readonly,
   KHRN_IMAGE_PLANE_T *dst, gfx_bufstate_t *buffer_state, KHRN_FMEM_T *fmem)
{
   KHRN_RES_INTERLOCK_T *res_i;
   bool write = !readonly;
   ...
   if (!src->image)
   {
      *buffer_state = GFX_BUFSTATE_MISSING;
      return true;
   }
   ...
   *buffer_state = gfx_bufstate_begin(!khrn_interlock_is_valid(&res_i->interlock));
...
{
   // ensure sever isn't pointing to us after we're deleted
      ...
      GLXX_HW_FRAMEBUFFER_T *fb)
{
   ...
   gfx_ez_init(&rs->ez, khrn_options.early_z);
   ...
   if (GLXX_MULTICORE_BIN_ENABLED)
   {
      ...
      rs->do_multicore_bin = khrn_get_num_bin_cores() > 1;
   }
#ifndef NDEBUG
                              ...
                              uint32_t num_cores, uint32_t core)
{
   ...
   if (vcos_is_log_enabled((&glxx_render_log), VCOS_LOG_TRACE))
      glxx_trace_rs(&glxx_render_log, rs, "[create_master_cl]");;

   bool stencil_rcfg = rcfg->stencil.short_form_img_plane.image != NULL &&
      (rcfg->stencil.load || rcfg->stencil.store);

   // Workaround GFXH-1320 if occlusion queries we used in this control list.
   bool workaround_gfxh_1320 =   khrn_get_v3d_version() == V3D_MAKE_VER(3,2)
   ...
   v3d_cl_tile_rendering_mode_cfg_common(&instr,
      ...
      info->rcfg.store_short.stencil,             // short form stencil store
      info->rcfg.store_short.depth,               // short form depth store
      info->rcfg.store_short.color_disable_mask); // short form color store disable mask
   ...
   else for (unsigned b = 0; b < fb->rt_count; ++b)
   {
      ...
      BUFFER_OPS_T const *op  = &rcfg->color[b];
      ...
      if (op->short_form_img_plane.image != NULL)
      {
         ...
         GFX_LFMT_T lfmt = khrn_image_get_lfmt(op->short_form_img_plane.image, 0);
   ...
   {
      BUFFER_OPS_T const *op = &rcfg->depth;
   ...
   if (stencil_rcfg)
   {
      BUFFER_OPS_T const *op = &rcfg->stencil;
      v3d_decimate_t decimate = op->short_form_ms ? V3D_DECIMATE_ALL_SAMPLES : V3D_DECIMATE_SAMPLE0;
      v3d_addr_t addr = 0;
      uint32_t uif_height_in_ub = 0;
...
{
   GLXX_HW_FRAMEBUFFER_T *fb = &rs->installed_fb;
   vcos_logc_trace((&glxx_log), "[create_bin_cl] rs %u, num_tiles %u x %u, rt count = %u",
...

   {
      ...
      v3d_cl_prim_list_format(&instr, V3D_PRIM_TYPE_TRI, /*xy=*/false, /*d3dpvsf=*/false);
...
void glxx_hw_render_state_rw(GLXX_SERVER_STATE_T *state, GLXX_HW_RENDER_STATE_T *rs)
{
   ...
   if (color)
   {
      ...
      while (glxx_fb_iterate_valid_draw_bufs(fb, &i, &att_index))
      {
         ...
         gfx_bufstate_rw(&rs->color_buffer_state[b]);
         gfx_bufstate_rw(&rs->ms_color_buffer_state[b]);
      }
   }
   ...
   if (depth_w)
      ...
      gfx_bufstate_read(&rs->depth_buffer_state);
...
static bool create_master_cls(GLXX_HW_RENDER_STATE_T *rs, MASTER_CL_INFO_T const *info)
{
   ...
   // Driver tile isolation disables multi-core
   unsigned num_cores = khrn_options.isolate_frame == khrn_fmem_frame_i ? 1u : khrn_get_num_render_cores();
...
static bool create_bin_cls(GLXX_HW_RENDER_STATE_T *rs, MASTER_CL_INFO_T const *info, uint8_t* draw_clist_ptr)
{
   ...
   unsigned max_cores = rs->do_multicore_bin && rs->tf_waited_count == 0 ? khrn_get_num_bin_cores() : 1;
...
static void update_image_interlock_from_bufstate(KHRN_IMAGE_PLANE_T *image_plane, gfx_bufstate_t bufstate)
{
   ...
   if (gfx_bufstate_is_undefined(bufstate))
   {
      KHRN_RES_INTERLOCK_T *res_i;
      res_i = khrn_image_get_res_interlock(image_plane->image);
...
static void update_image_interlock_from_bufstates(
   ...
   gfx_bufstate_t bufstate_a,
   gfx_bufstate_t bufstate_b)
{
   ...
   if (
      gfx_bufstate_is_undefined(bufstate_a) &&
      gfx_bufstate_is_undefined(bufstate_b)
   )
   {
      KHRN_RES_INTERLOCK_T *res_i;
...
void glxx_hw_render_state_flush(GLXX_HW_RENDER_STATE_T *rs)
{
   ...
   if (vcos_is_log_enabled((&glxx_flush_log), VCOS_LOG_TRACE))
   {
      glxx_trace_rs(&glxx_flush_log, rs, "\n[glxx_hw_render_state_flush] begin");
   }
...
quit:
   ...
   if (vcos_is_log_enabled((&glxx_flush_log), VCOS_LOG_TRACE))
      glxx_trace_rs(&glxx_flush_log, rs, "[glxx_hw_render_state_flush] frame discarded");
...
void glxx_hw_discard_frame(GLXX_HW_RENDER_STATE_T *rs)
{
   ...
   vcos_logc_trace((&glxx_invalidate_log), "[hw_discard_frame]");
