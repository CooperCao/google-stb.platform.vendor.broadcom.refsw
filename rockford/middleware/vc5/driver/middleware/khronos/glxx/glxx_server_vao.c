...
{
   GLXX_VAO_T *vao = (GLXX_VAO_T *)v;
   int i;

   UNUSED(size);

   KHRN_MEM_ASSIGN(vao->element_array_binding.obj, NULL);
   for (i = 0; i < GLXX_CONFIG_MAX_VERTEX_ATTRIB_BINDINGS; i++)
...
void glintBindVertexArray(GLuint array)
{
   ...
   vcos_logc_trace((&glxx_vao_log), "glBindVertexArray(array = %u)", array);
...
void glintDeleteVertexArrays(GLsizei n, const GLuint* arrays)
{
   ...
   vcos_logc_trace((&glxx_vao_log), "glDeleteVertexArrays(n = %d, arrays = %p)", n, arrays);
...
void glintGenVertexArrays(GLsizei n, GLuint* arrays)
{
   ...
   vcos_logc_trace((&glxx_vao_log), "glGenVertexArrays(n = %d, arrays = %p)", n, arrays);
