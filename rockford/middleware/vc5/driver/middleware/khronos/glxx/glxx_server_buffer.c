...
GL_API void GL_APIENTRY glBufferData(GLenum target, GLsizeiptr size, const GLvoid *data, GLenum usage)
{
   ...
   khrn_stats_record_start(KHRN_STATS_BUFFER_DATA);
...
GL_API void GL_APIENTRY glBufferSubData(GLenum target, GLintptr offset, GLsizeiptr size, const GLvoid *data)
{
   ...
   vcos_log_trace("glBufferSubData(target = %s, offset = %ld, size = %ld, data)",
      khrn_glenum_to_str(target), (long)offset, (long)size);
   ...
   GLbitfield  access)
{
   ...
   vcos_log_trace("glMapBufferRange(target = %s, offset = %ld, length = %ld, access = 0x%x))",
      khrn_glenum_to_str(target), (long)offset, (long)length, access);
...
GL_API GLboolean GL_APIENTRY glUnmapBuffer(GLenum target)
{
   ...
   vcos_log_trace("glUnmapBuffer(target = %s)", khrn_glenum_to_str(target));
...
GL_API void GL_APIENTRY glFlushMappedBufferRange(GLenum target, GLintptr offset, GLsizeiptr length)
{
   ...
   vcos_log_trace("glFlushMappedBufferRanges(target = %s, offset = %ld, length = %ld)",
      khrn_glenum_to_str(target), (long)offset, (long)length);
   ...
   flush_exp = (buffer->mapped_access_flags & GL_MAP_FLUSH_EXPLICIT_BIT) == GL_MAP_FLUSH_EXPLICIT_BIT;
