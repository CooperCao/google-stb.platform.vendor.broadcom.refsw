...
static void set_buffer_release_callback(DisplayInterfaceWlClient *self,
      WaylandClientBuffer *surface)
{
   int result = 0;
   if (!surface->buffer_release_callback)
   {
      static const struct wl_buffer_listener listener =
      {
         .release = buffer_release_callback,
      };
      if (wl_buffer_add_listener(surface->buffer, &listener, self) == 0)
         surface->buffer_release_callback = buffer_release_callback;
   }
}
      ...
      int render_fence, int *display_fence)
{
   ...
   if (set_frame_callback(self))
   {
      ...
      wl_surface_attach(self->window->surface, buffer->buffer,
            self->window->dx, self->window->dy);

      self->window->dx = 0;
      self->window->dy = 0;
      self->window->attached_width = buffer->surface.windowInfo.width;
      self->window->attached_height = buffer->surface.windowInfo.height;

      pthread_mutex_unlock(&self->window->mutex);

      wl_surface_damage(self->window->surface, 0, 0,
            buffer->surface.windowInfo.width, buffer->surface.windowInfo.height);
