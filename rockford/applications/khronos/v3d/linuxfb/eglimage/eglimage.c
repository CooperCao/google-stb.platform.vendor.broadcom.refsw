...
static image_ctx* load_image(char * filename)
{
   ...
   if (p)
   {
      ...
      if (!LFPL_CreateCompatiblePixmap(lfpl_handle, &p->eglPixmap, &p->nexusSurf, &pixInfo))
         goto error4;
...
error5:
   LFPL_DestroyCompatiblePixmap(lfpl_handle, p->eglPixmap);
...
void TerminateGLState(void)
{
   ...
   for (i = 0; i < IMAGES; i++)
   {
      ...
      LFPL_DestroyCompatiblePixmap(lfpl_handle, images[i]->eglPixmap);
   }
}
