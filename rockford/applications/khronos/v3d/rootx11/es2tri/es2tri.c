...
main(int argc, char *argv[])
{
   ...
   int i;
   const char *s;
   RXPL_PlatformHandle rxpl_handle = 0;
   ...
   /* this can go in a default construction so this line wont be required in final version */
   NEXUS_Platform_Join();
   RXPL_RegisterRootX11DisplayPlatform(&rxpl_handle, x_dpy);

   printf("%s %d %p\n", __FUNCTION__, __LINE__, x_dpy);
   if (!x_dpy) {
