...
{
   CARPL_NativeWindowInfo   win_info;

   /* Register this display for exclusive mode access */
   CARPL_RegisterDisplayPlatform(&nxpl_handle);

   *aspect = (float)config->vpW / config->vpH;
   ...
   native_window = CARPL_CreateNativeWindow(&win_info);
...
int main(int argc, char** argv)
{
   ...
   CARPL_DestroyNativeWindow(native_window);
