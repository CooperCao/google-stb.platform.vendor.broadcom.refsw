...
bool processArgs(int argc, const char *argv[], AppConfig *config)
{
   ...
   for (a = 1; a < argc; ++a)
   {
      ...
      {
         if (sscanf(arg, "z=%u", &config->zOrder) != 1)
            return false;
      }
...
int CLIENT_MAIN(int argc, const char** argv)
{
   ...
   if (!processArgs(argc, argv, &config))
   {
      ...
      fprintf(stderr, "Usage: %s [+m] [+p] [+s] [+fps] [d=WxH] [o=XxY] [bpp=16/24/32] [f=frames] [swap=N] [z=zorder] [+secure]\n", progname);
      return 0;
   }
...
int CLIENT_MAIN(int argc, const char** argv)
{
   BSTD_UNUSED(argc);
   BSTD_UNUSED(argv);
   fprintf(stderr, "This platform does not have V3D hardware\n");
