                                 ...
                                 NEXUS_DisplayHandle nexusDisplay)
{
   ...
   {
      stream->videoDecoder = NEXUS_VideoDecoder_Open(0, NULL); /* take default capabilities */
      if (stream->videoDecoder == NULL)
      {
...
static void InitVideo(void)
{
   ...
   for (v = 0; v < config.numVideos; v++)
   {
      ...
      ConfigureVideoStream(&videoStream[v], config.numVideos > 1 ? eMOSAIC : eSINGLE, v, &mediaData, nexus_display);
   }
}
...
int CLIENT_MAIN(int argc, const char** argv)
{
   ...
   {
      fprintf(stderr, "Usage: %s video=<filename> [+m] [+p] [+s] [+fps] [+bench] [d=WxH] [o=XxY] [swap=N] [bpp=16/24/32] [f=frames] [t=WxH]\n", progname);
      return 0;
   }
