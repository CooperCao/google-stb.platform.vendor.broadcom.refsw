...
#define HELP_STR_IPV6_VERSION  "usage: %s [-p listen-port] [-ipv4/6]\n"
#define HELP_STR_IPV4_VERSION  "usage: %s [listen-port]\n"
...
main(int argc, char* argv[])
{
  ...
  if (argc > 1) {
	...
	if ((StrCmp(argv[1] , "-help") == 0) || (StrCmp(argv[1] , "--h") == 0) ) {
	  FPrintf(stdout, HELP_STR_IPV6_VERSION ,
			  argv[0]);
...
#else /* LIB_IPV6 */
	StrCpy(temp, argv[1]);
	for (count = 0 ; count < (int)(StrLen(temp)) ; count++) {
	  if (!IsDigit((int)temp[count])) {
		FPrintf(stdout, HELP_STR_IPV4_VERSION, argv[0]);
		return MNTCPAPP_CMDLINE_ERR;
	  }
	}
  ...
  if (argc == 1) {
	...
	appState.udpSockInfo.localPort = (unsigned short int)ASCIIToInt(argv[1]);
  }
#endif /* LIB_IPV6 */
