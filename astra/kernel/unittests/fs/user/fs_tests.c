...
static void fileCreationTests() {
    ...
    for (int i=0; i<NumTestFiles; i++) {
        char fname[80];
        strcpy(fname, "/unittests/fs/testFile");
...
static void fileScatterWriteTests() {
    ...
    int fd = open("/unittests/fs/scatter_tests", O_CREAT|O_TRUNC|O_RDWR, S_IRUSR|S_IWUSR);
    assert(fd > 0);
...
static void fileWriteTests() {
    ...
    int fd = open("/unittests/fs/write_tests", O_CREAT|O_TRUNC|O_RDWR, S_IRUSR|S_IWUSR);
    assert(fd > 0);
...
static void linkTests() {
    int rc = link("/unittests/musl", "/unittests/libc");
    assert(rc == 0);

    int fd = open("/unittests/libc/init/kernel_init.cpp", O_RDONLY);
    assert(fd > 0);
    close(fd);

    rc = unlink("/unittests/libc/init/system.cpp");
    assert(rc == 0);

    fd = open("/unittests/musl/init/system.cpp", O_RDONLY);
    assert(fd < 0);

    rc = unlink("/unittests/libc");
    assert(rc == 0);

    fd = open("/unittests/libc/init/kernel_init.cpp", O_RDONLY);
    assert(fd < 0);

    fd = open("/unittests/musl/init/kernel_init.cpp", O_RDONLY);
    assert(fd > 0);
    close(fd);
...
static void cwdTests() {
    int rc = chdir("/unittests/fs/init");
    assert(rc == 0);
