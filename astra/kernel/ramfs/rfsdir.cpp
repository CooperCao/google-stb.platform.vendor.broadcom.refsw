...
bool RamFS::Directory::checkPermissions(int permsBit) {
    ...
    //printf("%s: perms 0x%x owner %x group %x uid %x gid %x\n", __PRETTY_FUNCTION__, perms, owner, group, uid, gid);
...
void RamFS::Directory::print(int level) {
   ...
   SpinLocker locker(&lock);
    ...
    while (top >= 0 && top < 16) {
        ...
        if(entry->type == FileEntry)
            printf("%s - f{%x, %x, %x}\n", entry->name,entry->file->owner(),entry->file->group(),entry->file->permissions() );
        if ((entry->type == DirEntry) && (strcmp(entry->name, ".")) && (strcmp(entry->name, "..")))
            printf("%s - d{%x, %x, %x}\n", entry->name,entry->dir->owner(),entry->dir->group(),entry->dir->permissions() );
