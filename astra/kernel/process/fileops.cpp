...
int TzTask::open(char *filePath, int flags, int mode) {
    ...
    bool exec    = (flags & O_EXEC);
    //printf("----> %s \n\tcreateFile %d\n\treadFile %d\n\twriteFile %d\n\tappendFile %d\n\ttruncateFile %d\n\tcloseOnExec %d \n\texec %d\n",
    //          filePath, createFile, readFile, writeFile, appendFile, truncateFile, closeOnExec, exec);
    //printf("Create File %s (%d %d %d) (%x %x)\n",filePath,createFile, truncateFile,exclusive, flags, mode);
    ...
    if(createFile && exec)
        printf("Error: Possible error in flags: createFile & exec");
    ...
    if (file != nullptr) {
        ...
        //printf("File %x %x Permission %x %x %x\n",fileOwner, fileGroup, OWNER_ACCESS_PERMS(perms), GROUP_ACCESS_PERMS(perms),OTHERS_ACCESS_PERMS(perms));
    ...
    if (createFile) {
        ...
        //printf("Create File %s Permission %x %x %x\n",fileName, parentDir->owner(), parentDir->group(), mode);
    ...
    RamFS::File::open((RamFS::File *)file, filePath, flags);
