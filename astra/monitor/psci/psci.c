...
static size_t psci_num_clusters;
static size_t psci_num_cpus;
    ...
    uint32_t *cluster_num_cpus)
{
    ...
    psci_num_clusters = num_clusters;
    psci_num_cpus = num_cpus;
...
int psci_cpu_up(void)
{
    ...
    psci_cluster_t *pcluster = &psci_clusters[MPIDR_AFFLVL1_VAL(curr_cpu)];
    psci_cpu_t *pcpu = pcluster->pcpus[MPIDR_AFFLVL0_VAL(curr_cpu)];
...
int psci_cpu_off(void)
{
    ...
    for (i = 0; i < pcluster->num_cpus; i++) {
        ...
        if (pccpu->state != PSCI_STATE_OFF)
            break;
    }
    if (i == pcluster->num_cpus) {
        pcluster->state = PSCI_STATE_OFF;

        for (i = 0; i < psystem->num_clusters; i++) {
            psci_cluster_t *pscluster = psystem->pclusters[i];
            if (pscluster->state != PSCI_STATE_OFF)
                break;
        }
        if (i == psystem->num_clusters)
...
int psci_system_off(void)
{
    ...
    for (i = 0; i < psci_num_cpus; i++) {
        psci_cpu_t *ppcpu = &psci_cpus[i];

        if (ppcpu != pcpu &&
            ppcpu->state == PSCI_STATE_RUNNING) {
            /* Call plaform power function */
            plat_cpu_power_down(pcpu->index);
        }
    }
