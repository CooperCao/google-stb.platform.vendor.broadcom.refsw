...
eRet CPid::open(CPlaypump * pPlaypump)
{
    ...
    BDBG_MSG(("pid channel %p successful", _pidChannel));
...
eRet CPid::open(CRecord * pRecord)
{
    ...
    if (true != isOpen())
    {
        BDBG_WRN(("trying to record a not open pid channel (pid:0x%lx)", _pid));
        goto error;
    }
...
error:
    BDBG_MSG(("PidChannel close (pid:0x%lu)", _pid));
    _pidChannel = NULL;
    return(ret);
} /* close */
...
error:
    BDBG_MSG(("PidChannel (pid:0x%lu) closed", _pid));
    _pidChannel = NULL;
    return(ret);
} /* close */
...
error:
    BDBG_MSG(("PidChannel (pid:0x%lu) removed record ", _pid));
    return(ret);
} /* close */
