...
uint32_t CParserBand::getMaxDataRate()
{
    ...
    if (0 < GET_INT(_pCfg, MAXDATARATE_PARSERBAND))
    {
        ...
        maxDataRate = pSettings->transportModuleSettings.maxDataRate.parserBand[getNumber()];
    }
    else
    {
        /* default max data rate */
        NEXUS_ParserBandSettings nsettings;
        NEXUS_ParserBand_GetSettings(getBand(), &nsettings);
