        ...
        )
{
    ...
    int           patTimeout     = GET_INT(_pCfg, TUNE_QAM_PAT_TIMEOUT)/10; /* in tsPsi_setTimeout() this is 500msecs */
    int           patTimeoutOrig = 0;
    int           pmtTimeout     = GET_INT(_pCfg, TUNE_QAM_PMT_TIMEOUT)/10; /* in tsPsi_setTimeout() this is 500msecs */
...
void CChannelQam::dump(bool bForce)
{
    CPid *     pPid = NULL;
    BDBG_Level level;
    ...
    if (true == bForce)
    {
        BDBG_GetModuleLevel("atlas_channel_qam", &level);
        BDBG_SetModuleLevel("atlas_channel_qam", BDBG_eMsg);
    }

    BDBG_MSG(("channel - type:%s, number:%d.%d, tuned:%d", boardResourceToString(_type).s(), _major, _minor, _tuned));
    BDBG_MSG(("    freq:%d mode:%d annex:%d", getFrequency(), getMode(), getAnnex()));
