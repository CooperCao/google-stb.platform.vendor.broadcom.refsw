...
{
    _scanThread_name = "CTunerOobNx_Scan";
}

eRet CTunerOobNx::acquire()
{
    eRet ret = eRet_Ok;
    NEXUS_FrontendAcquireSettings frontendAcquireSettings;

    NEXUS_Frontend_GetDefaultAcquireSettings(&frontendAcquireSettings);
    frontendAcquireSettings.capabilities.outOfBand = true;
    _frontend = NEXUS_Frontend_Acquire(&frontendAcquireSettings);
    CHECK_PTR_ERROR_GOTO("Unable to find OOB-capable frontend.  Run nxserver with -frontend option.", _frontend, ret, eRet_NotAvailable, error);

error:
    return(ret);
}

void CTunerOobNx::release()
{
    if (NULL != _frontend)
    {
