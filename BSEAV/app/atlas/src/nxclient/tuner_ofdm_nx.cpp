...
{
    _scanThread_name = "CTunerOfdmNx_Scan";
}

eRet CTunerOfdmNx::acquire()
{
    eRet ret = eRet_Ok;
    ...
    CHECK_PTR_ERROR_GOTO("Unable to find OFDM-capable frontend.  Run nxserver with -frontend option.", _frontend, ret, eRet_NotAvailable, error);

error:
    return(ret);
}

void CTunerOfdmNx::release()
{
    if (NULL != _frontend)
    {
