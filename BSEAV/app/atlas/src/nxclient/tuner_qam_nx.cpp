...
{
    _scanThread_name = "CTunerQamNx_Scan";
}

eRet CTunerQamNx::acquire()
{
    eRet ret = eRet_Ok;
    ...
    CHECK_PTR_ERROR_GOTO("Unable to find QAM-capabale frontend.  Run nxserver with -frontend option.", _frontend, ret, eRet_NotAvailable, error);

error:
    return(ret);
}

void CTunerQamNx::release()
{
    if (NULL != _frontend)
    {
