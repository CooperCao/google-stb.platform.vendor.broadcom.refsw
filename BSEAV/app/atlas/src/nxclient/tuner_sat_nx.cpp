...
CTunerSatNx::CTunerSatNx(
        const char *     name,
        const unsigned   number,
        CConfiguration * pCfg
        ) :
    CTunerSat(name, number, pCfg)
{
    _scanThread_name = "CTunerSatNx_Scan";
}

eRet CTunerSatNx::acquire()
{
    eRet ret = eRet_Ok;
    ...
    CHECK_PTR_ERROR_GOTO("Unable to find Satellite-capable frontend.  Run nxserver with -frontend option.", _frontend, ret, eRet_NotAvailable, error);

error:
    return(ret);
}

void CTunerSatNx::release()
{
    if (NULL != _frontend)
    {
