...
/* Here's the dual-purpose code */
    ...
    ATLAS_DECLARE(AUDIO_DECODER_FADE_DURATION,             int,    "1500",                      "Duration of audio fade between main/pip.  Only applicable whem dual audio decoders are available"),
    ...
    ATLAS_DECLARE(VOLUME_KEY_MODE,                         string, "volume",                    "Determines audio feature that volume up/down remote buttons control (volume | dialog) dialog enhancement option only available for ac4 streams"),
    ...
    ATLAS_DECLARE(RESOURCE_NUM_SIMPLE_PCM_PLAYBACKS,     int,    "-1",                        "Override num of simple audio playback resources reported by Nexus.  -1 disables override."),
    ...
    ATLAS_DECLARE(WPA_SUPPLICANT_IFACE_PATH,               string, "/tmp/run/wpa_supplicant",   "Path to the WPA Supplicant directory containing the interface to use"),
