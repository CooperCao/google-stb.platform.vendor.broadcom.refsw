...
eRet CDma::open()
{
    ...
    if (NEXUS_NUM_DMA_CHANNELS < _number)
    {
        BDBG_ERR((" Not enough DMA channels"));
        ret = eRet_ExternalError;
        goto error;
    }

    _dma = NEXUS_Dma_Open(_number, NULL);
...
void CDma::close()
{
    BDBG_MSG(("Closing DMA %s ", __FILE__));

    if (_dma != NULL)
    {
        NEXUS_Dma_Close(_dma);
    }
}
