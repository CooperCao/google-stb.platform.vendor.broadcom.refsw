...
BDBG_MODULE(channel_oob);

CChannelOob::CChannelOob(
        CConfiguration * pCfg,
        CTunerOob *      pTuner
        ) :
    CChannel("CChannelOob", eBoardResource_frontendOob, pCfg)
{
#if NEXUS_HAS_FRONTEND
    setTuner(pTuner);
#endif
    NEXUS_Frontend_GetDefaultOutOfBandSettings(&_settings);
}

CChannelOob::CChannelOob(const CChannelOob & chOob) :
    CChannel(chOob),
    _settings(chOob._settings)
{
    /* copy constructor will not copy any unique objects like tuner */
}

CChannel * CChannelOob::createCopy(CChannel * pChannel)
{
    CChannelOob * pChNew = NULL;

    pChNew = new CChannelOob(*(CChannelOob *)pChannel);
    return(pChNew);
}
...
eRet CChannelOob::initialize(PROGRAM_INFO_T * pProgramInfo)
{
    eRet ret = eRet_Ok;
        ...
        )
{
    ...
    if (NULL == pTuner)
    {
        pTuner = (CTunerOob *)pBoardResources->checkoutResource(id, eBoardResource_frontendOob, index);
        CHECK_PTR_ERROR_GOTO("unable to checkout OOB tuner", pTuner, ret, eRet_NotAvailable, done);

        ret = pTuner->open(_pWidgetEngine);
        CHECK_ERROR_GOTO("unable to open OOB tuner", ret, done);

        ret = pTuner->acquire();
        CHECK_ERROR_GOTO("unable to acquire OOB tuner", ret, done);
...
bool CChannelOob::operator ==(CChannel &other)
{
    CChannelOob * pOtherOob = (CChannelOob *)&other;

    /* check base class equivalency first */
    if (true == CChannel::operator ==(other))
    {
        if ((pOtherOob->getFrequency() == getFrequency()) &&
            (pOtherOob->getMode() == getMode()))
        {
            return(true);
        }
    }
