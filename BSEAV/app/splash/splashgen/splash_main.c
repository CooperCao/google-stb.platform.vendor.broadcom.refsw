...
int main(int argc, char *argv[])
{
	...
	BMMA_Heap_Handle  hMem[SPLASH_NUM_MEM] = {NULL};
	...
	for (ii=0; ii<NEXUS_MAX_HEAPS; ii++)
	{
		...
		if (!rc)
		{
				...
				stat.size > heap_size[stat.memcIndex])
			{
				...
				hMem[stat.memcIndex] = NEXUS_Heap_GetMmaHandle(g_pCoreHandles->heap[ii].nexus);
			}
	...
	g_stModeHandles.hRulMem = hMem[rulMem];
	...
	g_stModeHandles.surf[0].hMem = hMem[surf0Mem];
	...
	if (surf1Mem != BBOX_MemcIndex_Invalid)
	{
		...
		g_stModeHandles.surf[1].hMem = hMem[surf1Mem];
