...
static bwin_font_glyph *bwin_p_get_glyph(bfont_t font, bwin_char ch);
extern fileio_ctx_t *fsctx_usb0;
...
bfont_t bfont_open(const char *filename)
{
    ...
    f = bsu_fs_open(fsctx_usb0, (char *)filename, FILE_MODE_READ);
    if (!f) {
        printf("error from bsu_fs_open\n");
        return NULL;
    }

    font = bfont_callback_open(f);

    bsu_fs_close(fsctx_usb0, f);
...
bfont_t bfont_callback_open(void *context)
{
    ...
    if (bsu_fs_read(fsctx_usb0, (uint8_t *)font, sizeof(*font), 1, context) != 1) {
    ...
    if (bsu_fs_read(fsctx_usb0, (uint8_t *)font->glyph_cache, s, 1, context) != 1) {
    ...
    if (bsu_fs_read(fsctx_usb0, (uint8_t *)font->char_map, s, 1, context) != 1) {
    ...
    for (i=0;i<font->cache_size;i++) {
        ...
        if (s) { /* space has no pixels */
            glyph->mem = malloc(s);
            if (bsu_fs_read(fsctx_usb0, glyph->mem, s, 1, context) != 1) {
...
static char bwin_p_get_char(const char *text, bwin_char *ch)
{
    char data = text[0];
    ...
    bfont_valign valign, bfont_halign halign)
{
    ...
    rc = bfont_measure_text(font, text, len, &width, &height, &base);
    if (rc) {
        return bfont_draw_text(desc, font, 0, 0, text, len, color, pRect);
    }
    switch (valign) {
    ...
    return bfont_draw_text(desc, font, x, y, text, len, color, pRect);
}
