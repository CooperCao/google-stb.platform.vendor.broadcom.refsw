...
static b_sampleTCP_t sampleTCP;
static b_sampleDejittering_t sampleDejittering;
static b_sampleRTP_t sampleRTP;
static b_sampleMPEG2Stats_t sampleMPEG2TSStats;
static b_sampleVideoDecoderStats_t sampleVideoDecoderStats;
static b_sampleAudioDecoderStats_t sampleAudioDecoderStats;
...
void init_135(void)
{
	...
	BKNI_Memset(&sampleTCP, 0, sizeof(sampleTCP));
	BKNI_Memset(&sampleDejittering, 0, sizeof(sampleDejittering));
	BKNI_Memset(&sampleRTP, 0, sizeof(sampleRTP));
	BKNI_Memset(&sampleMPEG2TSStats, 0, sizeof(sampleMPEG2TSStats));
	BKNI_Memset(&sampleVideoDecoderStats, 0, sizeof(sampleVideoDecoderStats));
	BKNI_Memset(&sampleAudioDecoderStats, 0, sizeof(sampleAudioDecoderStats));
...
int getSampleTCP(b_sampleTCP_t **pSample)
{
	if (pSample == NULL)
		return -1;
	*pSample = &sampleTCP;
...
int setSampleTCP(b_sampleTCP_t *in)
{
	if (in == NULL)
		return -1;
	BKNI_Memcpy(&sampleTCP, in, sizeof(b_sampleTCP_t));
	return 0;
}

int getSampleDejittering(b_sampleDejittering_t **pSample)
{
	if (pSample == NULL)
		return -1;
	*pSample = &sampleDejittering;
	return 0;
}

int setSampleDejittering(b_sampleDejittering_t *in)
{
	if (in == NULL)
		return -1;
	BKNI_Memcpy(&sampleDejittering, in, sizeof(b_sampleDejittering_t));
	return 0;
}

int getSampleRTP(b_sampleRTP_t **pSample)
{
	if (pSample == NULL)
		return -1;
	*pSample = &sampleRTP;
...
int setSampleRTP(b_sampleRTP_t *in)
{
	if (in == NULL)
		return -1;
	BKNI_Memcpy(&sampleRTP, in, sizeof(b_sampleRTP_t));
	return 0;
}

int getSampleMPEG2TSStats(b_sampleMPEG2Stats_t **pSample)
{
	if (pSample == NULL)
		return -1;
	*pSample = &sampleMPEG2TSStats;
	return 0;
}

int setSampleMPEG2TSStats(b_sampleMPEG2Stats_t *in)
{
	if (in == NULL)
		return -1;
	BKNI_Memcpy(&sampleMPEG2TSStats, in, sizeof(b_sampleMPEG2Stats_t));
	return 0;
}

int getSampleVideoDecoderStats(b_sampleVideoDecoderStats_t **pSample)
{
	if (pSample == NULL)
		return -1;
	*pSample = &sampleVideoDecoderStats;
	return 0;
}

int setSampleVideoDecoderStats(b_sampleVideoDecoderStats_t *in)
{
	if (in == NULL)
		return -1;
	BKNI_Memcpy(&sampleVideoDecoderStats, in, sizeof(b_sampleVideoDecoderStats_t));
	return 0;
}

int getSampleAudioDecoderStats(b_sampleAudioDecoderStats_t **pSample)
{
	if (pSample == NULL)
		return -1;
	*pSample = &sampleAudioDecoderStats;
	return 0;
}

int setSampleAudioDecoderStats(b_sampleAudioDecoderStats_t *in)
{
	if (in == NULL)
		return -1;
	BKNI_Memcpy(&sampleAudioDecoderStats, in, sizeof(b_sampleAudioDecoderStats_t));
	return 0;
}
...
const char *toString(char *dest, char *str)
{
	unsigned len = 0;
	char buff[256];
	char *rs;
	...
	if (strlen(rs) <= sizeof(buff)){
		printf("String %s len %d buff %d \n", str, strlen(rs), sizeof(buff));
		len += sprintf(dest + len, "%s%s", sep, str);
		return dest;
		printf("Result string %s len %d \n", (rs + len), strlen(rs));
	}
	else
		BKNI_Memset(rs, 0, sizeof(rs));
	printf("Retur string %s len %d buff %s \n", rs, strlen(rs), buff);
	return dest;
}
