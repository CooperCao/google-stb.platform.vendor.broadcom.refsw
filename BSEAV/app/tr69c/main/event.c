...
void setCallback(void *target, EventHandler func, void *handle)
{
  ...
  if ((cb = lookup_callback(target, func, handle)) != NULL) {
    ...
    slog(LOG_DEBUG,"setCallback(), callback %p/%p aready exists (ignore).\n", func, handle);
    return;
  }
...
void eventLoop(void)
{
	while (timers || listeners)
	{
		...
		{
			slog(LOG_CRIT, "Error in select (%d=%s)", errno, strerror(errno));
			continue; /* ?????????? */
			/* return; */
		}

		for (i = 0; i<res; ++i)  /* this is to keep trying until the number of fd returned from */
		{
...
void cmsTmr_cancel(void *handle, EventHandler func, void *ctxData)
{
	BSTD_UNUSED(ctxData);
    /* call our func */
    /* not using ctxData */
...
CmsRet cmsTmr_set(void *handle, EventHandler func, void *ctxData, unsigned int ms, const char *name)
{
	BSTD_UNUSED(ctxData);
	BSTD_UNUSED(name);
    /* call our func */
    /* not using name */
    int ret;
    ret = setTimer(func, handle, ms);
    /* setTimer returns 0 for error, 1 for timer queued */
    /* this func expects the opposite */
    return !ret;
}

int cmsTmr_replaceIfSooner(void *handle, EventHandler func, void *ctxData, unsigned int ms, const char *name)
{
