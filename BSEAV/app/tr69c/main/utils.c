...
#else
	if ( (netent = getnameinfo( name ))) {
#endif
		*res = ntohl( (int)*(int *)*netent->h_addr_list);
	} else
		*res =0;
	slog (LOG_DEBUG,"dns_lookup(%s) = %s\n", name, writeIp(*res));
	return 1;
}
		...
		while ( (getline( &lineptr, &size, f)>=0) ) {
			...
			if ( sscanf(lineptr, "%s %s %x %x %x %x %x %x %x %x %x",
				...
				if (streq(dest,"00000000")) {
					...
					/*slog(LOG_DEBUG, "WanIF up %s %s", wanState->name, writeIp(wanState->ip));*/
					break;
				}
			}
		}
...
void runScript(const char* name)
{
	...
	system(buf);
	slog(LOG_DEBUG, "runScript %s", name);
}
...
int findProc(const char* str)
{
	...
	if ((d = opendir("/proc")) != NULL) {
		while ((de = readdir(d))) {
			...
			cmd[n] = '\0';
/*#ifdef DEBUG
	  slog(LOG_DEBUG, "findProc() pid=%s cmd=\"%s\" str=\"%s\" match=%d",
		  de->d_name, cmd, str, strcmp(cmd,str) == 0);
			...
			if (strcmp(cmd, str) == 0) {
				...
				closedir(d);
#ifdef DEBUG
				slog(LOG_DEBUG, "findProc(%s) pid=%d", str, pid);
#endif
				return pid;
			}
		}
...
#ifdef DEBUG
	slog(LOG_DEBUG, "findProc(%s) not found", str);
#endif
	return -1;
}
...
static void generateRequestDigest( SessionAuth *sa, char *user, char* pwd)
{
	...
	tr69_md5it(md5buf,(const unsigned char *)md5inbuf);
...
int parseAuthorizationHdr(char *ahdr, SessionAuth *sa, char *username, char *password)
{
	...
	{
#ifdef DEBUG
printf("Authorization is digest\n");
...
static char *formatDigestParamStr( SessionAuth *sa, char *user)
{
	...
	#ifdef DEBUG
	DBGPRINT((stderr, "Authorization header value =%s\n", hdrvalue));
	#endif
	return hdrvalue;
}
/*
