...
#include "bcmtimer.h"
extern volatile unsigned long Timer0IntCount;
extern void TimerISR(void);

void timer_open()
{
   /* set up timer0 for 1 ms */
   NEXUS_Core_ConnectInterrupt((unsigned long)INT1_ID_UPG_TMR_CPU_INTR,
                      (NEXUS_Core_InterruptFunction)TimerISR, (void *)NULL, 0);
   NEXUS_Core_DisableInterrupt(INT1_ID_UPG_TMR_CPU_INTR);
   TIMER->TimerCtl0 = 0x80006978; /* 1ms */
   TIMER->TimerInts = 0x01; /* reset timer0 */
   Timer0IntCount = 0;
   TIMER->TimerMask = 0x01; /* enable timer0 interrupt */
...
SPDP dtime(void)
{
   return ((SPDP)Timer0IntCount / (SPDP)1000.0);
}
#endif 
