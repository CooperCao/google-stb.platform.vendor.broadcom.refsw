...
BDBG_MODULE(mmt_encrypted_playback);
/**
  * mmt_encrypted_playback app decodes and renders AV from 2
  * sample encrypted file source which is in tlv format.
  * Without the sample streams, this example app will not work
  **/
#define TEST_CASE 2

#if TEST_CASE==1
/* Broadcom encrypted stream - encryption_test_derived_iv_scrambled.v1.tlv */
#define MAX_KEYS  3
#define KEY_SIZE 16
static uint8_t odd_key_table[MAX_KEYS][KEY_SIZE] =
{
    { 0x2f, 0x6c, 0xcd, 0xb8, 0xc2, 0x5d, 0xad, 0x9b, 0xea, 0x10, 0x42, 0x91, 0xa4, 0x48, 0xca, 0x5d },
    { 0xa7, 0x77, 0x6d, 0xd2, 0xd8, 0x5a, 0x24, 0xce, 0xf1, 0x09, 0x1f, 0x1a, 0x03, 0xdb, 0x8e, 0x3A },
    { 0x80, 0xd2, 0x86, 0x82, 0x5e, 0xa0, 0x69, 0x30, 0x45, 0xe9, 0x99, 0x4a, 0x8e, 0x6f, 0x2d, 0xb1 }
};
static uint8_t even_key_table[MAX_KEYS][KEY_SIZE] =
{
    ...
    { 0x0a, 0x99, 0xb2, 0xae, 0x2a, 0x8e, 0xfe, 0xf0, 0x67, 0xed, 0x30, 0x80, 0xa0, 0xa8, 0xff, 0x6c },
    { 0xce, 0xb2, 0xc8, 0x75, 0x34, 0x22, 0x1f, 0xf2, 0x81, 0xa9, 0xb7, 0x39, 0x43, 0x38, 0x35, 0x68 }
};
#elif TEST_CASE==2
/* Humax encrypted stream - CAS-J-CA00-03-csc.tlv */
#define MAX_KEYS  2
#define KEY_SIZE 16
static uint8_t odd_key_table[MAX_KEYS][KEY_SIZE] =
{
    { 0x65, 0xA6, 0x68, 0xB1, 0x3E, 0x5D, 0xE7, 0x7B, 0x7F, 0x8D, 0x9B, 0x2C, 0x20, 0x4F, 0x96, 0x48 },
    { 0xBF, 0x8B, 0x8E, 0x5C, 0x1F, 0x96, 0x87, 0x8F, 0x03, 0xDE, 0xCC, 0xD1, 0xFE, 0x15, 0x45, 0x45 }
};
static uint8_t even_key_table[MAX_KEYS][KEY_SIZE] =
{
    { 0xF4, 0xE5, 0xD8, 0xB9, 0xC0, 0x17, 0x3C, 0x9F, 0x11, 0x24, 0xB9, 0x18, 0x01, 0x5E, 0xB3, 0x47 },
    { 0xAD, 0xA1, 0x1D, 0x15, 0x03, 0x52, 0xA0, 0x27, 0xC5, 0x1F, 0xB6, 0xB8, 0xE2, 0x54, 0x12, 0x07}
};
#endif
...
#if TEST_CASE==2
    ip_addr.address.ipv6.addr[15] = 0x2;
    bmmt_set_ip_filter(mmt, &ip_addr);
#endif
    if (msg_r == BMMT_MAX_MSG_BUFFERS)
    {
