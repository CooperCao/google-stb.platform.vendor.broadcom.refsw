...
int platform_media_player_start(PlatformMediaPlayerHandle player, const PlatformMediaPlayerStartSettings * pSettings)
{
    ...
    /*
     * having hold until TSM lock makes it look weird when switching from
     * pig to full because the pig window is scaled first with the previous
     * content before the full content is started.  Same goes the other way.
