    ...
    )
{
    ...
    sprintf( systemCmd, "cat /proc/brcm/core > %s",tempLogFilename );
    PRINTF( "system(%s)\n", systemCmd );
    system( systemCmd );
    usleep( 500 );
    ...
    line_size = (size_t)statbuf.st_size;

    while (getline(&contents, &line_size, fpInput) != -1) {
        /* replace the newline charactor to null charactor */
        contents[strlen(contents)-1]='\0';
        PRINTF( "found (%s", contents );

        if (!isdigit(contents[0]))
            continue;

        sscanf( contents, "%u %lx %u %lx %lu %lx %u%% %u%% %s", &heapIdx, &offset, &memc, &size, &megabytes, &vaddr, &usedPercentage, &peakPercentage, &mapping[0] );
        ...
        if (heapIdx < BHEAP_MAX_HEAP_NUM)
        {
            ...
            pos1 = strstr( contents, mapping );
            if (pos1)
            {
                PRINTF( "pos1 (%s): mapping (%s); strncpy max %u bytes\n", pos1, mapping, sizeof( pheap_info[heapIdx].mapping ));
                strncpy( pheap_info[heapIdx].mapping, pos1, sizeof( pheap_info[heapIdx].mapping ));
            }
        }
        else
        {
