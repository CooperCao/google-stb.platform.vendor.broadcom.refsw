    ...
    )
{
    ...
    p_working_path = (char *) params->uri;
    ...
    b_url_path_is_html_file = Rest_IsHtmlFile( params->uri );

    /* split the URI into two parts ... the plugin name and the arguments to be sent to the plugin */
    {
        char *slash = NULL;
        char *query = NULL;
        ...
        slash = strchr( &p_working_path[1], '/' );
        query = strchr( p_working_path, '?' );
    ...
    switch (pid) {
        case 0:                                            /* this is the child */
        {
            ...
            if (plugin_name[0] == '/')
            {
                firstCharacterOffset = 1; /* if the name starts with a slash, do not include the slash in the name */
            }
            ...
            else if (NULL != p_query_string)
            {
                ...
                sprintf( plugin_name_with_dir, "./plugins/%s", &plugin_name[firstCharacterOffset] ); /* executables are in the subdirectory "plugins" */
        ...
        default:                                           /* this is the parent again */
        {
            ...
            if (size_payload == 0)
            {
                char strGenerateErrorMsg[128];
                int  firstCharacterOffset = 0;

                /* if the plugin name begins with a slash, use the part AFTER the slash */
                if (plugin_name[0] == '/')
                {
                    firstCharacterOffset = 1;
                }

                /* generate a "resource not found" message that includes whatever plugin name was found in the URI */
                snprintf( strGenerateErrorMsg, sizeof( strGenerateErrorMsg ),
                    "%s resource not found. Check the resource name or plugin is not compiled", &plugin_name[firstCharacterOffset] );

                /* initialize cJSON */
                objectRoot = json_Initialize();
                CHECK_PTR_ERROR_GOTO( "Unable to allocate JSON object", objectRoot, rc, -1, json_error1 );

                /* generate failsafe JSON header ... this is done because the forked child did not do it for some reason */
                objectData = json_GenerateHeader( objectRoot, &plugin_name[firstCharacterOffset],
                        "Header Generated by plugin launcher", NULL, PLUGIN_LAUNCHER_VERSION );
                CHECK_PTR_ERROR_GOTO( "Unable to generate JSON header", objectData, rc, -1, json_error2 );

                json_AddNull( objectRoot, NO_FILTER, objectRoot, "data" );
                json_GenerateError( objectRoot, BMON_CALLER_ERR_RES_NOT_FOUND, strGenerateErrorMsg );
...
json_error1:
            ...
            else
            {
                ...
                size_payload = Rest_AppendDurationDeltas( params->p_payload, BUFFER_SIZE*buffers, tv1, tv2, tv3, size_payload );
                RC_CHECK_GOTO(( size_payload > 0 ), ( "Rest_AppendDurationDeltas() Failed" ), size_payload );
