...
/* Flags to control BRCM-defined, customizable PlayReady behavior */
typedef enum OEM_SETTINGS_FLAGS
{
    OEM_SETTINGS_FLAGS_NONE                     = 0x0000,
    OEM_SETTINGS_FLAGS_PROTECT_CONTENT_KEY      = 0x0001,        /* Enables content key protection on SW TEE platforms; has no effect on SAGE platforms as the content key is always protected */

    OEM_SETTINGS_FLAGS_ALL                      = 0xffff
} OEM_SETTINGS_FLAGS;
...
typedef struct OEM_Settings{
    ...
    DRM_WCHAR           *keyHistoryFileName;    /* Optional playready bin file name (full path) */
    DRM_WCHAR           *defaultRWDirName;   /* Optional default directory for all read/write files. Overridden by above file name paths. */
    DRM_VOID            *f_pOEMContext;         /* Returned context by Drm_Platform_Initialize() */
    OEM_SETTINGS_FLAGS   flags;                 /* Optional control flags */
}OEM_Settings;
...
DRM_API
DRM_VOID* DRM_CALL Oem_SharedMemAlloc(
    __in DRM_DWORD f_cbSize );

/* Frees shared memory */
DRM_API_VOID DRM_VOID DRM_CALL Oem_SharedMemFree(
