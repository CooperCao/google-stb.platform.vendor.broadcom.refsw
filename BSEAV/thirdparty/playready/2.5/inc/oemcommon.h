...
typedef enum _OEM_AES_OPERATION{
    OEM_HAL_AES_ENCRYPT = 0,
    OEM_HAL_AES_DECRYPT
}OEM_AES_OPERATION;
...
typedef struct OEM_Settings{
#ifndef PLAYREADY_SAGE_IMPL
    NEXUS_HeapHandle  heap;               /* Optional heap to allocate internal data structures. Must be CPU accessible. */
    uint32_t drmType;
#endif
    DRM_WCHAR        *binFileName;        /* Optional playready bin file name (full path) */
    DRM_WCHAR        *keyFileName;        /* Optional playready key file name (full path) */
    DRM_WCHAR        *keyHistoryFileName; /* Optional key history file name (full path) */
    DRM_WCHAR        *defaultRWDirName;   /* Optional default directory for all read/write files. Overridden by above file name paths. */
}OEM_Settings;


/* Oem Aes Hardware handle */
typedef void * Oem_AesHwHandle;

/* Generic OEM Hal handle */
typedef void * Oem_HalHandle;
...
DRM_RESULT _GetHWID(
    __in uint8_t *pId,
    __in size_t idLength,
    __out DRM_BYTE *shaOut );
...
** For all devices, this function MUST be implemented by the OEM.
**
** Note: For Sage, this functions should not be used by the application.
**       And it should ONLY be used by the Secure Clock feature.
**
***********************************************************************/
...
/**********************************************************************
** Function:    Oem_Clock_SetSystemTimeFromBBContext
** Synopsis:    Sets the System time. It is expressed in UTC.
** Arguments:   [f_pBBXContextHandle]-- A blackbox context.
**              [f_lpSystemTime]--Pointer to DRMSYSTEMTIME structure
**              holding the time.
**
** Notes:
** This function was added for the Broadcom platform in support of the
** platform-independent PlayReady Common DRM API layer.
**
***********************************************************************/
typedef struct __tagDRM_BB_CONTEXT * Oem_BB_ContextHandle;

DRM_API DRM_VOID DRM_CALL Oem_Clock_SetSystemTimeFromBBContext(
    __in     Oem_BB_ContextHandle  f_pBBXContextHandle,
    __in     const DRMSYSTEMTIME  *f_lpSystemTime );

#if (PLAYREADY_STANDALONE_IMPL==1)
/**********************************************************************
** Function:    OEM_TimeInit
** Synopsis:    Initialize the System time. It should be expressed in UTC.
** Arguments:   [f_pOEMContext]--OEM specific data
**              [f_ptime]--Pointer to time_t structure
**              holding the time. If anti-rollback clock is detected, where
**              the given *f_ptime is earlier than the one from the secure
**              data store or the deflault value, then the clock will not be
**              updated and the last good time will be returned. If the
**              *f_ptime is not present, the clock will be initialized
**              based on the last known good time either from the secure
**              data storeif existed or from a hard-coded default value.
**
** Note:
** This is only  for Non-Sage for Anti-rollback clock implementation
**
***********************************************************************/
DRM_RESULT DRM_API DRM_CALL OEM_TimeInit(
    __in      DRM_VOID   *f_pOEMContext,
    __in_opt  time_t     *f_ptime);

/**********************************************************************
** Function:    OEM_TimeUnInit
** Synopsis:    Uninitialize the System time. Upon success, the last
**              known good time will be written to the secure data store.
** Arguments:   [f_pOEMContext]--OEM specific data
**
** Note:
** This is only for Non-Sage for Anti-rollback clock implementation
**
***********************************************************************/
DRM_RESULT DRM_API DRM_CALL OEM_UnTimeInit(
    __in      DRM_VOID   *f_pOEMContext);

#endif

/**********************************************************************
** Function:    Oem_Clock_ResetContext
** Synopsis:    Resets the locally maintained context data.
** Arguments:
**
** Note:
** Needed in support of Secure Clock feature on Sage to
** handle drm runtime uninitialization/initialization.
**
***********************************************************************/
DRM_API DRM_VOID DRM_CALL Oem_Clock_ResetContext(void);

/* OEM platform initialization functions (oemimpl.c). */
DRM_API DRM_VOID * DRM_CALL Oem_Platform_Init(
    __in        OEM_Settings *pSettings);
