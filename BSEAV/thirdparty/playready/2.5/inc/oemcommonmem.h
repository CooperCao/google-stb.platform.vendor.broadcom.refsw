...
static DRMFORCEINLINE DRM_VOID * DRM_CALL DRMCRT_LocalMemset(
    __out_bcount_full( f_cbCount )  DRM_VOID  *f_pOut,
...
static DRMFORCEINLINE DRM_VOID * DRM_CALL DRMCRT_LocalDWORDSetZero(
...
static DRMFORCEINLINE DRM_BOOL DRM_CALL DRMCRT_LocalAreEqual(
...
static DRMFORCEINLINE DRM_VOID * DRM_CALL DRMCRT_LocalDWORDcpy(
...
static DRMFORCEINLINE DRM_VOID * DRM_CALL DRMCRT_LocalDWORDcpy(
    ...
    __in                             DRM_DWORD  f_cdwCount )
{
    uint32_t count = f_cdwCount*4;
    uint8_t *pdout = (uint8_t *)f_pdwOut;
    BKNI_Memmove(pdout, (uint8_t *)f_pdwIn, count);
    return (DRM_VOID *)(pdout + count);
}
