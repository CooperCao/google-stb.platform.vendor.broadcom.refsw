...
DRM_API DRM_RESULT Oem_Keyfile_GetHash(
	__in_opt 									DRM_VOID	*f_pOEMContext,
	__out_bcount( SHA256_DIGEST_SIZE_IN_BYTES ) DRM_BYTE	*f_pbFileHash );

DRM_API DRM_RESULT Oem_Keyfile_Delete( __in_opt DRM_VOID   *f_pOEMContext);
...
DRM_API DRM_RESULT Oem_Keyfile_GetCurrentKFAesKeyIdWrapper(
...
DRM_API DRM_RESULT Oem_Keyfile_GetKFAesKey(
    __in                DRM_BYTE       *pHistoryFile,
    __in                DRM_DWORD       cbHistoryFile,
...
#if(PLAYREADY_SAGE_IMPL==1 || PLAYREADY_STANDALONE_IMPL==1)

DRM_API DRM_RESULT Oem_Keyfile_GetCurrentKFAesKeyId(
    DRM_BYTE    *pHistoryFile,
    DRM_DWORD    cbHistoryFile,
    __out DRM_GUID *f_pguidAesKeyID );
#endif
