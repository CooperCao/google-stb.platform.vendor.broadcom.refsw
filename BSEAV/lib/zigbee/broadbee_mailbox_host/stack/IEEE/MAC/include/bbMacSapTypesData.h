 ...
 *  this option is set to GTS Transmission.
 ...
 *  considered to be set to Direct Transmission; for ZigBee PRO context in the case of
 *  transmission onto the PAN Coordinator with omitted Destination Address this option is
 *  also ignored (indirect transmissions to the PAN Coordinator are forbidden) and direct
 *  (immediate) transmission is performed; the Warning will be logged if this option is
 *  set to Indirect transmission with omitted Destination Address.
 * \par     Documentation
 *  See IEEE 802.15.4-2006, subclause 7.1.1.1.1, table 41.
 */
...
typedef struct _MAC_DataReqParams_t
{
    ...
    SYS_DataPointer_t  payload;         /*!< The set of octets forming the MSDU to be transmitted by the MAC sublayer
                                            entity. */
    /* 16-bit data. */
    MAC_PanId_t        dstPanId;        /*!< The 16-bit PAN identifier of the entity to which the MSDU is being
                                            transferred. */
    /* 8-bit data. */
    MAC_AddrMode_t     dstAddrMode;     /*!< The destination addressing mode for this primitive and subsequent MPDU. */

    MAC_AddrMode_t     srcAddrMode;     /*!< The source addressing mode for this primitive and subsequent MPDU. */

    MAC_MsduHandle_t   msduHandle;      /*!< The handle associated with the MSDU to be transmitted by the MAC sublayer
                                            entity. */
 ...
 *      MCPS-DATA.request being confirmed, for the case of direct data transmission. An
 *      MLME-RESET.request or MCPS-PURGE.request was issued prior to the pending
 ...
 *  - INVALID_GTS               This implementation of the MAC does not support GTS
 *      (guaranteed time slots).
 *  - COUNTER_ERROR, UNAVAILABLE_KEY, UNSUPPORTED_SECURITY      This implementation of the
 *      MAC does not support the MAC Security.
 *
...
{
    /* 32-bit data. */
    HAL_SymbolTimestamp_t  timestamp;       /*!< The time, in symbols, at which the data were transmitted. */
...
typedef struct _MAC_DataIndParams_t
{
    ...
    MAC_Address_t          srcAddr;             /*!< The individual device address of the entity from which the MSDU was
                                                    received. */
    /* 32-bit data. */
    HAL_SymbolTimestamp_t  timestamp;           /*!< The time, in symbols, at which the data were received. */
