...
{
    ZBPRO_NWK_LifeTime_t lifeTime;         /*!< The time of life of a neighbor entry. */
    PHY_LQI_t lqi;                         /*!< The estimated link quality for RF transmissions from this device. */
    ZBPRO_NWK_LinkCost_t linkCost;         /*!< Link cost in two directions. */
    ...
    ZBPRO_NWK_Capability_t capability;     /*!< IEEE 802.15.4-2006 7.3.1.2 Capability Information field. */
    PHY_Channel_t logicalChannel;          /*!< The logical channel on which the network is operating. */
    ZBPRO_NWK_NwkAddr_t networkAddr;       /*!< The 16-bit network address of the neighboring device. */
    ZBPRO_NWK_PanId_t panId;               /*!< The 16-bit Pan Id number. */
    ZBPRO_NWK_ExtAddr_t extAddr;           /*!< 64-bit IEEE address that is unique to every device. */
    ZBPRO_NWK_ExtPanId_t extPanId;         /*!< The 64-bit unique identifier of the network to which the device belongs.*/
    ZBPRO_NWK_Depth_t depth;               /*!< The tree depth of the neighbor device. */
    ZBPRO_NWK_UpdateId_t updateId;         /*!< The value identifying a snapshot of the network settings with which
                                                this node is operating with.*/
    ZBPRO_NWK_TransmitFailure_t transmitFailure; /*!< A value indicating if previous transmissions to the device
                                                      were successful or not. Higher values indicate more failures. */
    Bool8_t rxOnWhenIdle : 1;              /*!< Indicates if neighbor's receiver is enabled during idle periods. */
    Bool8_t permitJoining : 1;             /*!< An indication of whether the device is accepting joining requests. */
    Bool8_t potentialParent : 1;           /*!< An indication of whether the device has been ruled out as a potential
                                                parent. */
    Bool8_t isConflicted : 1;              /*!< Network address of the neighbor is conflict with other address in
                                                network. */
    ZBPRO_NWK_RejoinMethod_t joinMethod : 3; /*!< Attributes which device use to join/leave. Used by zdo */
    Bool8_t waitingJoinResp     : 1;       /*!< Self-descriptive bitfield. */
    Bool8_t waitingAuth         : 1;       /*!< Self-descriptive bitfield. */
    Bool8_t isSecureJoin        : 1;       /*!< Self-descriptive bitfield. */
    Bool8_t isPendingLeave      : 1;       /*!< Self-descriptive bitfield. */
    Bool8_t rejoinAfterLeave    : 1;       /*!< Self-descriptive bitfield. */
    Bool8_t leaveRemovesChilds  : 1;       /*!< Self-descriptive bitfield. */
} ZBPRO_NWK_Neighbor_t;

/**//**
 * \brief New child parameters structure.
 */
...
/************************************************************************************//**
  \brief Returns the first unauthenticated entry.
...
/************************************************************************************//**
  ...
  \return True, if given neighbor is child; otherwise - false.
****************************************************************************************/
...
/*************************************************************************************//**
  ...
  \param[in] rejoinAfterLeave - Should rejoin after leave?
  \param[in] leaveRemovesChilds - Should leave remove children?
  \return Nothing.
*****************************************************************************************/
...
/*************************************************************************************//**
  \brief Returns the first child to leave procedure.
...
/************************************************************************************//**
  ...
  \return NWK Cost.
****************************************************************************************/
NWK_PUBLIC ZBPRO_NWK_Cost_t ZBPRO_NWK_CostFromLqi(const PHY_LQI_t lqi);
...
/************************************************************************************//**
  ...
  \return Pointer to neighbor or NULL if elementIndex is incorrect.
****************************************************************************************/
