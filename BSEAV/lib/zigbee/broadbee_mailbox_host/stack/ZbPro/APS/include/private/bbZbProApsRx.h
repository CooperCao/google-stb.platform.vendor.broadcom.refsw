...
typedef struct _ZbProApsRxBuffer_t
{
    ...
    struct
    {
        ...
        BitField8_t             isExtHeader     : 1;    /*!< TRUE if APS Extended Header is included into this frame. */
        BitField8_t             isExtNonce      : 1;
        BitField8_t             isFragmented    : 1;    /*!< TRUE if this frame is either a Data Fragment or ACK on a
                                                            group (window) of acknowledged data fragment(s). This field
                                                            is forced to FALSE when isExtHeader == FALSE. */
    };

    uint8_t                     fragIdx;        /*!< For a Data frame - Index of this Data Fragment. For an ACK frame -
                                                    Index of the first fragment in the window acknowledged with this ACK
                                                    frame. This field takes values in the range 0..254; it is valid only
                                                    when isFragmented == TRUE. */
    union
    {
        uint8_t                 fragsNum;       /*!< Total number of fragments in the current transaction. This field
                                                    takes values in the range 1..255; it is valid only for Data frames
                                                    received with isFragmented == TRUE and fragIdx == 0. */
        BitField8_t             ackedMask;      /*!< Bitmap of fragments in the current window successfully received and
                                                    acknowledged with this ACK frame. This field is valid only for ACK
                                                    frames received with isFragmented == TRUE. */
    };
