...
{
    /* 64-bit data. */
    ZBPRO_APS_ExtAddr_t  destAddress;       /*!< The extended 64-bit address of the device to which
                                                the request-key command should be sent. */

    ZBPRO_APS_ExtAddr_t  partnerAddress;    /*!< If the \e KeyType parameter indicates an application key,
                                                this parameter shall indicate an extended 64-bit address of a device
                                                that shall receive the same key as the device requesting the key. */
    /* 8-bit data. */
    ZBPRO_APS_KeyType_t  keyType;           /*!< The type of key being requested. */
 ...
 *  APSME-REQUEST-KEY.request. Nevertheless, this function is called with confirmation
 *  status parameter that shows at least successful or unsuccessful result from NWK layer.
 */
typedef void ZBPRO_APS_RequestKeyConfCallback_t(ZBPRO_APS_RequestKeyReqDescr_t         *const reqDescr,
...
typedef struct _ZBPRO_APS_RequestKeyReqDescr_t
{
    ...
    ZBPRO_APS_RequestKeyConfCallback_t *callback;       /*!< Confirm callback function.  */
...
typedef struct _ZBPRO_APS_RequestKeyIndParams_t
{
    ...
    ZBPRO_APS_ExtAddr_t  srcAddress;        /*!< The extended 64-bit address of the device
                                                that sent the request-key command. */
...
} ZBPRO_APS_RequestKeyIndParams_t;
 ...
 */
typedef void ZBPRO_APS_RequestKeyInd_t(ZBPRO_APS_RequestKeyIndParams_t *indParams);
...
/*************************************************************************************//**
  ...
  \note
    ...
    \c callback field to the entry point of its APSME-REQUEST-KEY.confirm
    handler-function.
  \note
...
/*************************************************************************************//**
  ...
  \code
    void ZBPRO_APS_RequestKeyInd(ZBPRO_APS_RequestKeyIndParams_t *indParams) { ... }
  \endcode
  \note
