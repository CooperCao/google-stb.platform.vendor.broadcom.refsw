 ...
 * \param[in]   atomicSectionUid    Global numeric identifier of the atomic section.
 * \note    On ML507 platform this decorator disables only the low-priority interrupts (Level 1) while the high-priority
 *  interrupts (Level 2) stays enabled. The Level 2 interrupts are used by the Radio Driver on ML507. Use the
 *  HAL_ATOMIC_START decorator to disable all interrupts on ML507. Take into account that SoC does not use Level 2
 *  interrupts actually, so this decorator on SoC indeed disables all interrupts.
 */
#if defined(__arc__)
