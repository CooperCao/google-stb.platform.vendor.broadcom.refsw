...
/*****************************************************************************
*
* FILENAME: $Workfile: trunk/stack/common/Mailbox/src/bbMailPowerFilterKey.c $
*
* DESCRIPTION:
*
*
* $Revision: 9365 $
* $Date: 2016-01-04 23:06:12Z $
*
*****************************************************************************************/
...
{
    if(0 == socCpuIsSleep()){
        Mail_Serialize(RF4CE_ZRC2_CONTROL_COMMAND_IND_FID, indParams);
        return;
    }
...
void RF4CE_ZRC1_ControlCommandInd(RF4CE_ZRC1_ControlCommandIndParams_t *const indParams)
{
    if(0 == socCpuIsSleep() && !isHostWakingUp()){
        Mail_Serialize(RF4CE_ZRC1_IND_CONTROLCOMMAND_FID, indParams);
        return;
    }
    // Check if this key could wake up the CPU.
