...
### Files to be excluded from the Common/External component of the Stack.
### Note: all extensions and paths are ignored if they are indicated.


# If Common/External shall be build as a single integrated C-file then exclude all the separated Common/External source
# files and compile only the Mail-Integrated source file that includes all the necessary source parts.
ifeq ($(_EXT_INTEGRATED_),1)
  SRCEXC2 += %
  SRCINC2 += bbExtIntegrated
else
  SRCEXC2 += bbExtIntegrated
endif
