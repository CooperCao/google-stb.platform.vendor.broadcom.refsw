    ...
    union
    {
        ...
        struct
        {
            Bool8_t          isDataConfirmed   : 1;     /*!< TRUE if request to transmit data via the lower-level layer
                                                            was already confirmed; if so, this transaction may be killed
                                                            if there is no other task on it. Used by Client transaction
                                                            only. Set to FALSE during initialization. Service must not
                                                            reassign this field on its own. */

            Bool8_t          isWaitingResponse : 1;     /*!< TRUE if transaction is waiting for Server Response; such
                                                            transaction will be activated each time a response with
                                                            matching transaction sequence number and service identifier
                                                            is received. Used by Client transaction only. Set to FALSE
                                                            during initialization. Service must not reassign this field
                                                            on its own. */
        };
    };
