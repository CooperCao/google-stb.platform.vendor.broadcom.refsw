...
int main(int argc, char *argv[])
{
    ...
    BroadBee_ZHA_Set_Attribute_apsChannelMask(1 << 14);
    ...
    if(BROADBEE_ZHA_STATUS_SUCCESS != (retStatus = BroadBee_ZHA_Bind(myExtAddr, remoteDeviceExtAddr, ZBPRO_ZCL_CLUSTER_ID_DOOR_LOCK, DOOR_LOCK_ENDPOINT_NUM, 1)))
    ...
    while(1){
        ...
        //if(key == 'Y')
        {
            ...
            BroadBee_ZHA_DoorLockCmdLockUnlock_Indirect(1, DOOR_LOCK_ENDPOINT_NUM, 1);
        }
        sleep(15);
        Sys_DbgPrint("Unlock?\n");
        //key = getchar();
        //if(key == 'Y')
        {
           ...
           BroadBee_ZHA_DoorLockCmdLockUnlock_Indirect(0, DOOR_LOCK_ENDPOINT_NUM, 1);
        }
    }
    return 0;
_exit:
...
/* eof zigbee_ha_doorlock_controller.c */
