...
int main(int argc, char *argv[])
{
    ...
    uint32_t permitJoin;
    sscanf(argv[1], "%x", &permitJoin);
    //clearInput();
    permitJoin &= 0xff;
    printf("%02x\n", permitJoin);

    //BroadBee_ZHA_Set_Attribute_zdoPermitJoinDuration(permitJoin);
    //BroadBee_ZHA_StartNetwork();

    BroadBee_ZHA_PermitJoining(permitJoin);

    return 0;
_exit:
    printf("MY_HA_PERMIT_APP:  closing...\n");
