...
int main(int argc, char *argv[])
{
    ...
    BroadBee_ZHA_GetActiveEp(short16bitAddress, short16bitAddress, &activeEpCount, &activeEpList);

    ZBPRO_APS_SimpleDescriptor_t  simpleDescr;
    uint8_t length;
    for(int i = 0; i < activeEpCount; i++){
        Sys_DbgPrint("Endpoint %d:\n", activeEpList[i]);
        BroadBee_ZHA_GetSimpleDesc(short16bitAddress, short16bitAddress, activeEpList[i], &length, &simpleDescr);
    ...
    if(BROADBEE_ZHA_STATUS_SUCCESS != (retStatus = BroadBee_ZHA_Bind(0x0010187623cdUL, smartPlugExtAddr, ZBPRO_ZCL_CLUSTER_ID_ONOFF, 1, 1)))
        Sys_DbgPrint("Failed to call BroadBee_ZHA_Bind %08x:\n", retStatus);
    while(1){
        Sys_DbgPrint("On?\n");
        char key = getchar();
        if(key == 'Y')
            BroadBee_ZHA_OnOffCmdOn_Indirect(1, 1, 1);

        Sys_DbgPrint("Off?\n");
        key = getchar();
        if(key == 'Y')
           BroadBee_ZHA_OnOffCmdOn_Indirect(0, 1, 1);
    }
    return 0;
_exit:
...
/* eof zigbee_ha_dump_descriptor.c */