...
static bool saveCommissioningInfo()
{
    ...
    FILE *f = fopen("/tmp/commission.bin", "wb");
    if(f == NULL){
...
static bool retrieveCommissioningInfo()
{
    ...
    FILE *f = fopen("/tmp/commission.bin", "rb");
    if(f == NULL){
...
#define ONOFF_ATTR_LENGTH  1
    ...
    BroadBee_ZHA_Read_Attributes_Direct(getSrcEndpoint(index), getDstEndpoint(index), getDstExtAddr(index),
...
static void testLoopForOnOff(uint8_t index)
{
    ...
    do{
        ...
        Sys_DbgPrint("On?\n");
        {
            BroadBee_ZHA_OnOffCmdOn_Direct(1, getSrcEndpoint(index), getDstEndpoint(index), getDstExtAddr(index), 5);
        }
        ...
        Sys_DbgPrint("Off?\n");
        {
            BroadBee_ZHA_OnOffCmdOn_Direct(0, getSrcEndpoint(index), getDstEndpoint(index), getDstExtAddr(index), 5);
        }
...
static void testLoopForDoorLock(uint8_t index)
{
    ...
    if(isSleepyDevice(index)){
        ...
        BroadBee_ZHA_Set_Attribute_macTransactionPersistenceTime(520);  // about 8s MAC transaction persistent time
    }
    Sys_DbgPrint("Going to DoorLock test loop, press 'Ctrl - C' to exit from this app\n");
    resetExitflag();
    //installSignalHandler();
    while(1){
        if((BROADBEE_ZHA_STATUS_SUCCESS != BroadBee_ZHA_GetNwkAddr(getDstExtAddr(index), &nwkAddrOfInterest)) || (nwkAddrOfInterest == 0)){
    ...
    do{
        ...
        Sys_DbgPrint("Lock?\n");
        {
            BroadBee_ZHA_DoorLockCmdLockUnlock_DirectByNwkAddr(1, getSrcEndpoint(index), getDstEndpoint(index), getDstNwkAddr(index), 5);
            //BroadBee_ZHA_DoorLockCmdLockUnlock_Direct(1, getSrcEndpoint(index), getDstEndpoint(index), getDstExtAddr(index), 5000);
        }
        ...
        Sys_DbgPrint("Unlock?\n");
        {
           BroadBee_ZHA_DoorLockCmdLockUnlock_DirectByNwkAddr(0, getSrcEndpoint(index), getDstEndpoint(index), getDstNwkAddr(index) ,5);
           //BroadBee_ZHA_DoorLockCmdLockUnlock_Direct(0, getSrcEndpoint(index), getDstEndpoint(index), getDstExtAddr(index) ,5000);
        }
        sleep(1); // wait for the slow action for lock/unlock
        getDoorLockState(index);
...
static void My_BroadBee_ZHA_Read_Temperature_Direct_Async_Callback(ZBPRO_ZCL_ProfileWideCmdReadAttrReqDescr_t *const reqDescr,
...
static BroadBee_ZHA_Status BroadBee_ZHA_Read_Temperature_Direct_Async(uint8_t index)
{
...
static void zigbee_HA_Read_Temperature_thread(void *param)
{
    ...
    do{
        ...
        BroadBee_ZHA_Read_Temperature_Direct_Async(index);
...
static void My_BroadBee_ZHA_Read_Humidity_Direct_Async_Callback(ZBPRO_ZCL_ProfileWideCmdReadAttrReqDescr_t *const reqDescr,
    ZBPRO_ZCL_ProfileWideCmdReadAttrConfParams_t *const confParams)
{
    int payloadSize = 0;
    uint8_t *payload = NULL;
...
static BroadBee_ZHA_Status BroadBee_ZHA_Read_Humidity_Direct_Async(uint8_t index)
{
    ZBPRO_APS_EndpointId_t srcEndpoint = MY_HARD_CODED_ENDPOINT_NUMBER;
...
static void zigbee_HA_Read_Humidity_thread(void *param)
{
    ...
    do{
        ...
        BroadBee_ZHA_Read_Humidity_Direct_Async(index);
...
int main(int argc, char *argv[])
{
    ...
    //BroadBee_ZHA_Get_MyMacExtendedAddress(&myExtAddr);
            ...
            case ZBPRO_ZCL_CLUSTER_ID_TEMPERATURE_MEASUREMENT:
                ...
                */
                if(isSleepyDevice(deviceIndex)){
                    ...
                    */
                    BroadBee_ZHA_Set_Attribute_macTransactionPersistenceTime(30000);
                    /*
                    * There are two ways to get the temperateure from remote sensor continuousely.
                    *
                    * 1. Directly call BroadBee_ZHA_Read_Temperature_Direct_Async and call BroadBee_ZHA_Read_Temperature_Direct_Async again
                    ...
                    *
                    *    BroadBee_ZHA_Read_Temperature_Direct_Async(deviceIndex);
                    *
                    * 2. Create a thread to keep call BroadBee_ZHA_Read_Temperature_Direct_Async once during a specific period.
...
/* eof zigbee_ha_stb_commissioning.c */