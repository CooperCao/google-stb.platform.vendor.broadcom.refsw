...
static void testLoopForOnOff(uint8_t index)
{
    ...
    do{
        ...
        {
            Sys_DbgPrint("ON................\n");
            BroadBee_ZHA_OnOffCmdOn_Direct(1, getSrcEndpoint(index), getDstEndpoint(index), getDstExtAddr(index));
        }
        sleep(5);
        if(shouldExit())
            break;
        ...
        {
            Sys_DbgPrint("OFF................\n");
            BroadBee_ZHA_OnOffCmdOn_Direct(0, getSrcEndpoint(index), getDstEndpoint(index), getDstExtAddr(index));
        }
    }while(1);
...
static void testLoopForDoorLock(uint8_t index)
{
    ...
    do{
        ...
        Sys_DbgPrint("Lock?\n");
        {
            ...
            BroadBee_ZHA_DoorLockCmdLockUnlock_Direct(1, getSrcEndpoint(index), getDstEndpoint(index), getDstExtAddr(index));
        }
        sleep(15);
        if(shouldExit())
            break;
        Sys_DbgPrint("Unlock?\n");
        {
           ...
           BroadBee_ZHA_DoorLockCmdLockUnlock_Direct(0, getSrcEndpoint(index), getDstEndpoint(index), getDstExtAddr(index));
        }
    }while(1);
    restoreSignalHandler();
}
...
/* eof zigbee_ha_normal_operation.c */
