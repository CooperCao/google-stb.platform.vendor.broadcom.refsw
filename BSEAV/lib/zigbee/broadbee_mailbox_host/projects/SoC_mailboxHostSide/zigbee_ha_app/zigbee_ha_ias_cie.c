...
int main(int argc, char *argv[])
{
    ...
    if(ZIGBEE_HA_STATUS_SUCCESS != (retStatus = Zigbee_HA_Bind(myExtAddr, remoteDeviceExtAddr, ZBPRO_ZCL_CLUSTER_ID_IAS_ZONE, 11, 1)))
        Sys_DbgPrint("Failed to call Zigbee_HA_Bind %08x:\n", retStatus);
    Zigbee_HA_Cie_Enroll_Start(remoteDeviceNwkAddr, 5000, 45000, false);

    while(1){
        int size;
        uint8_t *attribute;
        Zigbee_HA_Read_Attributes_Indirect(11, 1, ZBPRO_ZCL_CLUSTER_ID_IAS_ZONE, 2, &size, &attribute);
        printf("%04x\n", *(uint16_t*)attribute);
        sleep(5);
    }
_exit:
