...
#define DOOR_LOCK_ENDPOINT_NUM  2
...
int main(int argc, char *argv[])
{
    ...
    Zigbee_HA_Set_Attribute_apsChannelMask(1 << 14);
    ...
    //Zigbee_HA_Set_Attribute_zdoPermitJoinDuration(0xff);
    Sys_DbgPrint("Zigbee_HA_StartNetwork\n");
    Zigbee_HA_StartNetwork();

    ZBPRO_APS_SimpleDescriptor_t localSimpleDescr;
    zbPro_HA_Init_DoorLock_SimpleDescriptor(DOOR_LOCK_ENDPOINT_NUM, &localSimpleDescr);
    ...
    Zigbee_HA_GetActiveEp(remoteDeviceNwkAddr, remoteDeviceNwkAddr, &activeEpCount, &activeEpList);

    ZbProZdoNodeDescriptor_t nodeDescr;
    Zigbee_HA_GetNodeDesc(remoteDeviceNwkAddr, remoteDeviceNwkAddr, &nodeDescr);

    ZBPRO_APS_SimpleDescriptor_t  simpleDescr;
    uint8_t length;
    for(int i = 0; i < activeEpCount; i++){
        Sys_DbgPrint("Endpoint %d:\n", activeEpList[i]);
        Zigbee_HA_GetSimpleDesc(remoteDeviceNwkAddr, remoteDeviceNwkAddr, activeEpList[i], &length, &simpleDescr);
        dumpSimpleDescr(&simpleDescr);
    }

    free(activeEpList);

    if(ZIGBEE_HA_STATUS_SUCCESS != (retStatus = Zigbee_HA_Bind(myExtAddr, remoteDeviceExtAddr, ZBPRO_ZCL_CLUSTER_ID_DOOR_LOCK, DOOR_LOCK_ENDPOINT_NUM, 1)))
    ...
    while(1){
        ...
        //if(key == 'Y')
        {
            ...
            Zigbee_HA_DoorLockCmdLockUnlock_Indirect(1, DOOR_LOCK_ENDPOINT_NUM, 1);
        }
        sleep(15);
        Sys_DbgPrint("Unlock?\n");
        //key = getchar();
        //if(key == 'Y')
        {
           ...
           Zigbee_HA_DoorLockCmdLockUnlock_Indirect(0, DOOR_LOCK_ENDPOINT_NUM, 1);
        }
    }
    return 0;
_exit:
