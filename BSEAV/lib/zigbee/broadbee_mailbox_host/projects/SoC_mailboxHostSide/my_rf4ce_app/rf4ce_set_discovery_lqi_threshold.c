...
static void rf4ce_Test_Set_Discovery_Lqi_Threshold(uint8_t threshold)
{
    uint8_t statusSetThreshold = 0;

    RF4CE_NWK_SetReqDescr_t req = {0};
    void rf4ce_Test_Set_Discovery_Lqi_Threshold_Callback(RF4CE_NWK_SetReqDescr_t *request, RF4CE_NWK_SetConfParams_t *conf)
    {
        if(0 == conf->status)
    ...
    req.callback = rf4ce_Test_Set_Discovery_Lqi_Threshold_Callback;
...
int main(int argc, char *argv[])
{
    ...
    int times = 1;
    while(times--)
        rf4ce_Test_Set_Discovery_Lqi_Threshold((uint8_t)threshold);
