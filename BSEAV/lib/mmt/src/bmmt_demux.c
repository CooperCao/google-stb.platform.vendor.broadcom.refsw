...
struct b_mmt_timestamp_entry {
    ...
    bmmt_mpu_timestamp_descriptor_entry timestamp;
    bmmt_mpu_extended_timestamp_descriptor_header extended_timestamp_descriptor_header;
    bmmt_mpu_extended_timestamp_descriptor_entry extended_timestamp_descriptor_entry;
};
...
int bmmt_timestamp_queue_add_timestamp_descriptors(bmmt_timestamp_queue queue, const bmmt_mpu_timestamp_descriptor_entry * timestamps, unsigned n_timestamps)
{
    unsigned i;
...
int bmmt_timestamp_queue_add_extended_timestamp_descriptors(bmmt_timestamp_queue queue,
                                            const bmmt_mpu_extended_timestamp_descriptor_header *extended_timestamp_descriptor_header,
                                            const bmmt_mpu_extended_timestamp_descriptor_entry *extended_timestamp_descriptor_entry, unsigned n_extended_timestamps
                                           )
{
    unsigned i;
    BDBG_OBJECT_ASSERT(queue, bmmt_timestamp_queue);
...
int b_mmt_demux_stream_process_payload_aac(bmmt_demux_stream_t stream, const bmmt_mpu_header *mpu, bmmt_timed_mfu_data *mfu)
{
...
int bmmt_demux_stream_process_payload(bmmt_demux_t demux, bmmt_demux_stream_t stream, const bmmt_packet_header *header, batom_cursor *payload)
{
    bmmt_mpu_header mpu;
    int rc;
    ...
    if(bmmt_parse_mpu_payload_header(payload,&mpu)!=0) {
        return 0;
    }
    if(mpu.type==BMMT_MPU_TYPE_MFU && mpu.timed) {
        bmmt_timed_mfu_data mfu[16];
        unsigned parsed_mfu;
        ...
        if(bmmt_parse_mpu_payload_mfu(payload, &mpu, mfu, sizeof(mfu)/sizeof(mfu[0]), &parsed_mfu)==0) {
            unsigned i;
            if(!mpu.aggregation)
            {
...
{
    bmmt_mp_table_header mp_header;
    if(bmmt_parse_mp_table_header(payload, &mp_header)==0) {
        unsigned asset_no;
        for(asset_no=0;asset_no<mp_header.number_of_assets;asset_no++) {
            bmmt_package_table_asset asset;
            bmmt_package_table_asset_init(&asset);
            BDBG_MSG(("asset_no:%u", asset_no));
            if(bmmt_parse_package_table_asset(payload, &asset)==0) {
                unsigned i;
                ...
                for(i=0;i<asset.asset_location.location_count;i++) {
                    ...
                    for(stream=BLST_D_FIRST(&demux->streams);stream;stream=BLST_D_NEXT(stream,link)) {
                        if(
                           (asset.asset_location.locations[0].location_type==bmmt_general_location_type_id &&  asset.asset_location.locations[0].data.packet_id==stream->config.packet_id) ||
                           (asset.asset_location.locations[0].location_type==bmmt_general_location_type_ipv6 &&  asset.asset_location.locations[0].data.mmt_ipv6.packet_id==stream->config.packet_id)
                          ) {
                ...
                if(asset.asset_descriptors.asset_descriptors_length) {
                    ...
                    for(;;) {
                        bmmt_mpu_timestamp_descriptor_header timestamp_descriptor_header;
                        bmmt_mpu_extended_timestamp_descriptor_header extended_timestamp_descriptor_header;
                        bmmt_mpu_extended_timestamp_descriptor_entry extended_timestamp_descriptor_entry[64];
                        ...
                        if(bmmt_parse_mpu_timestamp_descriptor_header(&asset.asset_descriptors.asset_descriptors_bytes, &timestamp_descriptor_header)==0) {
                            unsigned timestamp;
                            for(timestamp=0;timestamp<timestamp_descriptor_header.N;timestamp++) {
                                bmmt_mpu_timestamp_descriptor_entry timestamp_descriptor_entry;
                                if(bmmt_parse_mpu_timestamp_descriptor_entry(&asset.asset_descriptors.asset_descriptors_bytes, &timestamp_descriptor_entry)==0) {
                                    if(timestamp_queue) {
                        ...
                        } else if(bmmt_parse_mpu_extended_timestamp_descriptor(&asset.asset_descriptors.asset_descriptors_bytes, &extended_timestamp_descriptor_header, 64, extended_timestamp_descriptor_entry, &extended_timestamp_descriptor_entry_enries)==0) {
                            if(0) {
                                unsigned i;
                ...
                bmmt_package_table_asset_shutdown(&asset);
                continue;
            } else {
                break;
            }
...
int bmmt_demux_process_signaling_message(bmmt_demux_t demux, batom_cursor *payload, const bmmt_signalling_header *signalling_header)
{
    unsigned parsed_messages;
    bmmt_signalling_message_data message_data[16];
    unsigned i;
    int rc;

    rc = bmmt_parse_signalling_message(payload, signalling_header, message_data, sizeof(message_data)/sizeof(message_data[0]), &parsed_messages);
    if(rc!=0) {
        return BDBG_TRACE_ERROR(rc);
    }
    for(i=0;i<parsed_messages;i++) {
        ...
        rc = bmmt_parse_signalling_message_header(&message_data[i].message,&message_header);
        if(rc!=0) {
            return BDBG_TRACE_ERROR(rc);
        }
        ...
        rc = bmmt_parse_package_access_message(&message_data[i].message, pa_message, sizeof(pa_message)/sizeof(pa_message[0]), &pa_messages);
        if(rc!=0) {
            return BDBG_TRACE_ERROR(rc);
        }
