...
/*
 * VC4 doesn't have GLES 3.2 so we can't #include <GLES3/gl32.h>
 *
 * We also can't determine GPU type here because this file is used
 * by VC4, VC5 and GPUMonitor on the host PC.
 */
#include <GLES2/gl2.h>
#if !GL_ES_VERSION_3_2
   typedef void (GL_APIENTRY  *GLDEBUGPROC)(
...
private:
   ...
   union Data
   {
      ...
      Data(float x)     : f(x) {}
      Data(void *x)     : p(x) {}
      Data(long int x)  : p((void*)(uintptr_t)x) {}
   };
   Data m_data;
   uint32_t    m_numBytes;
};
