...
#else
         GLenum status = Real(glCheckFramebufferStatus(GL_FRAMEBUFFER));
#endif
...
#else
               Real(glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, a, p, &val));
#endif
...
#else
               Real(glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, p, &val));
#endif
               dataPacket.AddItem(PacketItem(p));
               dataPacket.AddItem(PacketItem(val));
            }
...
static void GetGLBuffer(const Packet &packet)
{
   ...
   {
      GLint readBuffer = packet.Item(6).GetInt32();
#if GL_ES_VERSION_3_0
      if (GetESMajorVersion() >= 3)
      {
         GLint curReadBuffer;
...
static void GetPerfData(const Packet &packet)
{
   ...
   if (action == Control::ePerfStart)
   {
      ...
      if (sSetPerfCountingBRCM && sChoosePerfCountersBRCM && sSetPerfCountingBRCM) // VC5
      {
   ...
   else if (action == Control::ePerfNames)
   {
      ...
      if (sGetPerfCounterConstantBRCM && sGetPerfCounterGroupInfoBRCM && sGetPerfCounterInfoBRCM) // VC5
      {
...
static void GetEventData(const Packet &packet)
{
   ...
   if (action == Control::eEventNames)
   {
      ...
      p.AddItem(eventMaxStrLen > 64 ? eventMaxStrLen : 64); // API names are not longer than 64
...
static void PacketizePBOData(void *off, GLint len)
{
#if GL_ES_VERSION_3_0
...
static GLint getPixelUnpackBufferBinding()
{
   GLint buf = 0;
#if GL_ES_VERSION_3_0
      if (GetESMajorVersion() > 2)
...
{ \
   ...
   {\
      ...
      GLint buf = getPixelUnpackBufferBinding();\
      sCurPacket = &p; \
      p.AddItem(cmd_##func); \
      Packetize args; \
      if (sCaptureStream) \
