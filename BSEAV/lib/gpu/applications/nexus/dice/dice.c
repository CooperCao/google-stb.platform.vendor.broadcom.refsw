...
static void* load_image(const char *filename, unsigned int *width, unsigned int *height)
{
   ...
   p = (png_bytep)malloc(h * png_get_rowbytes(png_ptr,info_ptr));
   if (p == NULL)
      goto error4;
...
int CLIENT_MAIN(int argc, const char** argv)
{
   ...
   if (!processArgs(argc, argv, &config))
   {
      ...
      fprintf(stderr, "Usage: %s [+m] [+s] [d=WxH] [o=XxY] [bpp=16/24/32] [f=frames] [+secure] [-c]\n", progname);
      return 0;
   }
