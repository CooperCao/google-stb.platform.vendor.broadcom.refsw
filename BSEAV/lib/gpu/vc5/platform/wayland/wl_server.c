      ...
      uint32_t alignment, uint32_t size)
{
   ...
   if (memory->handle)
   {
      ...
      if (err == NEXUS_SUCCESS)
         memory->settings.physicalOffset = (uint64_t)devPtr;
      else
      ...
      memory->settings.byteSize = size;
      memory->settings.miplevels = 1;
      memory->settings.contiguous = true;
...
static void DestroyWlBufferMemory(WlBufferMemory *memory)
{
   ...
   if (memory->handle)
   {
      if (memory->settings.physicalOffset)
      ...
      uint32_t pitch, uint32_t alignment, uint32_t size, int secure)
{
   ...
   if (!memory->handle || !memory->settings.physicalOffset
   ...
   *offset = memory->settings.physicalOffset;
