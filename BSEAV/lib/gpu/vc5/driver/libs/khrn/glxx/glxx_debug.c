...
static void debug_message_control(GLenum source, GLenum type,
                                  GLenum severity, GLsizei count,
                                  const GLuint *ids, GLboolean enabled)
{
...
GL_APICALL void GL_APIENTRY glDebugMessageControlKHR(GLenum source, GLenum type,
                                                     ...
                                                     const GLuint *ids, GLboolean enabled)
{
   debug_message_control(source, type, severity, count, ids, enabled);
}
...
static void debug_message_insert(GLenum source, GLenum type, GLuint id,
...
GL_APICALL void GL_APIENTRY glDebugMessageInsertKHR(GLenum source, GLenum type, GLuint id,
                                                    ...
                                                    const GLchar *buf)
{
   debug_message_insert(source, type, id, severity, length, buf);
}
...
static void debug_message_callback(GLDEBUGPROCKHR callback, const void *userParam)
{
   GLXX_SERVER_STATE_T *state = glxx_lock_server_state(OPENGL_ES_ANY);
   if (state == NULL)
      return;
...
{
   debug_message_callback(callback, userParam);
}
...
static GLuint get_debug_message_log(GLuint count, GLsizei bufSize,
...
GL_APICALL GLuint GL_APIENTRY glGetDebugMessageLogKHR(GLuint count, GLsizei bufSize,
                                                      ...
                                                      GLsizei *lengths, GLchar *messageLog)
{
   return get_debug_message_log(count, bufSize, sources, types, ids,
                                severities, lengths, messageLog);
}

#if KHRN_GLES32_DRIVER
GL_APICALL GLuint GL_APIENTRY glGetDebugMessageLog(GLuint count, GLsizei bufSize,
                                                      GLenum *sources, GLenum *types,
                                                      GLuint *ids, GLenum *severities,
                                                      GLsizei *lengths, GLchar *messageLog)
{
...
static void push_debug_group(GLenum source, GLuint id, GLsizei length, const GLchar *message)
{
   GLXX_SERVER_STATE_T *state = glxx_lock_server_state(OPENGL_ES_ANY);
   if (state == NULL)
      return;
...
{
   push_debug_group(source, id, length, message);
}
...
static void pop_debug_group(void)
{
   GLXX_SERVER_STATE_T *state = glxx_lock_server_state(OPENGL_ES_ANY);
   if (state == NULL)
      return;
...
static void object_label(GLenum identifier, GLuint name, GLsizei length, const GLchar *label)
{
   GLXX_SERVER_STATE_T *state = glxx_lock_server_state(OPENGL_ES_ANY);
...
{
   object_label(identifier, name, length, label);
}

#if KHRN_GLES32_DRIVER
GL_API void GL_APIENTRY glObjectLabel(GLenum identifier, GLuint name, GLsizei length, const GLchar *label)
{
   object_label(identifier, name, length, label);
}
#endif
...
static void get_object_label(GLenum identifier, GLuint name, GLsizei bufSize,
...
GL_APICALL void GL_APIENTRY glGetObjectLabelKHR(GLenum identifier, GLuint name, GLsizei bufSize,
                                                GLsizei *length, GLchar *label)
{
   get_object_label(identifier, name, bufSize, length, label);
}
...
static void object_ptr_label(const void *ptr, GLsizei length, const GLchar *label)
{
   GLXX_SERVER_STATE_T *state = glxx_lock_server_state(OPENGL_ES_ANY);
...
static void get_object_ptr_label(const void *ptr, GLsizei bufSize,
                                 GLsizei *length, GLchar *label)
{
   GLXX_SERVER_STATE_T *state = glxx_lock_server_state_unchanged(OPENGL_ES_ANY);
   GLchar              *lab = NULL;
...
#if KHRN_GLES32_DRIVER
GL_APICALL void GL_APIENTRY glGetObjectPtrLabel(const void *ptr, GLsizei bufSize,
                                                GLsizei *length, GLchar *label)
{
   get_object_ptr_label(ptr, bufSize, length, label);
}
...
GL_APICALL void GL_APIENTRY glGetPointervKHR(GLenum pname, void **params)
{
   ...
   if (!glxx_get_pointerv(state, pname, params))
      glxx_server_state_set_error(state, GL_INVALID_ENUM);
