...
static bool is_bstc_or_888(const khrn_image *img)
{
   ...
   assert(num_planes == 1); // We don't support multi-plane color attachments
   return gfx_lfmt_is_bstc_family(lfmts[0]) ||
      (gfx_lfmt_get_base(&lfmts[0]) == GFX_LFMT_BASE_C8_C8_C8);
}
                                  ...
                                  bool *color_is_int)
{
   ...
   *color_is_int = is_int_or_uint(src_img->api_fmt);
   ...
   while (glxx_fb_iterate_valid_draw_bufs(dst_fb, &i, &att_index))
   {
      ...
      /* We do not support TMU blitting to/from RGB8/BSTC framebuffers. For
       * simplicity, we just refuse to blit to/from such framebuffers at all.
       * This is not conformant, but that is fine as we only support such
       * framebuffers via non-conformant EGL configs. */
      if (is_bstc_or_888(src_img) || is_bstc_or_888(dst_img))
      {
         error = GL_INVALID_OPERATION;
         goto end;
      }
