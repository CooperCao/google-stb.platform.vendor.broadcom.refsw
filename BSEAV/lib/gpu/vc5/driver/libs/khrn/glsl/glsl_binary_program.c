                                                    ...
                                                    const struct glsl_backend_cfg *key)
{
   ...
   ShaderFlavour f = glsl_ir_program_has_stage(ir, SHADER_COMPUTE) ? SHADER_COMPUTE : SHADER_FRAGMENT;
   if (!glsl_binary_shader_from_dataflow(&ret->fshader, f, false, &ret->vary_map, ir, key))
      goto fail;
