...
def print_header_api(outf, scalar_types):
    ...
    print("PrimitiveTypeIndex glsl_prim_get_image_base_type(PrimitiveTypeIndex idx);", file=outf)
...
""", file=outf)
        ...
        print("   return primitiveTypeImageBase[idx];", file=outf)
        print("}", file=outf)
        print(file=outf)
        print("PrimSamplerInfo *glsl_prim_get_image_info(PrimitiveTypeIndex idx) {", file=outf)
        print("   return &image_gadgets[glsl_prim_get_image_base_type(idx) - %s];" % prim_index(texture_names[0]), file=outf)
        print("}", file=outf)
        print(file=outf)
        print("bool glsl_prim_sampler_is_shadow(PrimitiveTypeIndex idx) {", file=outf)
