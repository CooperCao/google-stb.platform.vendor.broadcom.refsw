...
static void ast_validate_switch_case_nesting(Statement *ast);

static Statement *sprev_validate_switch_case(Statement *s, void *data) {
   (void)data;
   if (s->flavour == STATEMENT_SWITCH) {
      for (StatementChainNode *node = s->u.switch_stmt.stmtChain->first; node; node = node->next) {
         /* Validate the statements inside the case and default blocks */
         if (node->statement->flavour != STATEMENT_CASE && node->statement->flavour != STATEMENT_DEFAULT)
            ast_validate_switch_case_nesting(node->statement);
      }
      /* Now skip this statement. It's just been validated above. */
      return NULL;
   } else if (s->flavour == STATEMENT_CASE || s->flavour == STATEMENT_DEFAULT) {
      glsl_compile_error(ERROR_CUSTOM, 7, s->line_num, "case and default labels may only appear inside switches");
      return s;
   } else
      return s;
}
...
static void ast_validate_switch_case_nesting(Statement *ast) {
   glsl_statement_accept(ast, NULL, sprev_validate_switch_case, NULL, NULL, NULL);
}
...
void glsl_ast_validate(Statement *ast, ShaderFlavour flavour, int version) {
   ...
   ast_validate_switch_case_nesting(ast);
