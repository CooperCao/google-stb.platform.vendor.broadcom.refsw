...
def print_banner(outf, lines, comment_char = None):
    if comment_char == None:
        ...
        print(comment_char + (comment_char * (100//len(comment_char))), file=outf)
        line_start = comment_char
    for line in lines:
        print("%s %s" % (line_start,line), file=outf)
    if comment_char == None:
        print(" " + ("*" * 100) + "*/", file=outf)
    else:
        print(comment_char + (comment_char * (100//len(comment_char))), file=outf)
...
def print_array(outf, elt_type, name, dimension, values=[]):
    if len(values) == 0:
        print("%s %s[1] = {};" % (elt_type, name), file=outf)
        return

    print("%s %s[%s] = {" % (elt_type, name, dimension), file=outf)

    maxlen = max([len(str(v)) for v,k in values])
    for v,k in values:
        v = str(v)
        pad = " " * (maxlen-len(v))
        print("   %s,%s /* %s */" % (v,pad,k), file=outf)
    print("};", file=outf)
    print(file=outf)
...
def print_includes(outf, includes, system=False):
    ...
    for inc in active:
        print(format_string % inc, file=outf)
    print(file=outf)

def print_inclusion_guard_start(outf, filename):
    define = define_name(filename)
    print("#ifndef %s" % define, file=outf)
    print("#define %s" % define, file=outf)
    print(file=outf)

def print_inclusion_guard_end(outf, filename):
    define = define_name(filename)
    print("#endif /* %s */" % define, file=outf)

def print_enum(outf, name, elements):
    print("typedef enum _%s {" % name, file=outf)
    for element in elements:
        print("   %s," % element, file=outf)
...
def print_defines(outf, elements):
    ...
    for m,v in elements:
        pad = " " * (maxlen - len(m))
        print("#define %s%s %s" % (m,pad,v), file=outf)
    print(file=outf)
