...
"   float oomaxabs = 1.0 / maxabs;"
"   vec3 norm_coord = coord * oomaxabs;"
"   return $$texture_cube_lod(sampler, norm_coord, lod);"
"}"

/* Add in some extra functions for games which pass the wrong arguments to built-ins */
"vec4 texture2D(samplerCube sampler, vec3 coord) { return textureCube(sampler, coord); }"
"vec4 texture2D(sampler2D sampler, vec3 coord) { return texture2D(sampler, coord.xy); }"
