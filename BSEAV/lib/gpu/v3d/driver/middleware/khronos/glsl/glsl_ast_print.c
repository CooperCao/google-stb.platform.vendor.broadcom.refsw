...
static void glsl_print_expr(FILE* f, Expr* expr, bool fully_evaluated);
...
static void glsl_print_compile_time_value(FILE* f, SymbolType* type, void* compile_time_value)
{
   if (compile_time_value)
   {
      switch (type->flavour)
      {
         ...
         {
            int prim_index = type->u.primitive_type.index;
            ...
            case PRIM_MATRIX_TYPE:
               ...
               for (int i = 0; i < primitiveTypeSubscriptDimensions[prim_index]; i++)
               {
         ...
         case SYMBOL_STRUCT_TYPE:
            ...
            for (int i = 0; i < type->u.struct_type.member_count; i++)
            {
...
static void print_expr_value(FILE* f, Expr* expr)
{
   glsl_print_compile_time_value(f, expr->type, expr->compile_time_value);
}

static void print_expr_instance(FILE* f, Expr* expr)
{
...
static void glsl_print_expr(FILE* f, Expr* expr, bool fully_evaluated)
{
   ...
   {
      case EXPR_VALUE:
         print_expr_value(f, expr);
         break;
      case EXPR_INSTANCE:
         print_expr_instance(f, expr);
         break;
      case EXPR_SUBSCRIPT:
...
{
   for (StatementChainNode *decl = statement->u.ast.decls->first; decl; decl = decl->next)
   {
...
{
   for (StatementChainNode *decl = statement->u.decl_list.decls->first; decl; decl = decl->next)
   {
      glsl_print_statement(f, decl->statement, fully_evaluated, indent_depth, suppress_semicolon);
      if (decl->next) fprintf(f, "\n");
   }
...
static void print_statement_function_def(FILE* f, Statement* statement, bool fully_evaluated, int indent_depth, bool suppress_semicolon)
{
   ...
   for (int i = 0; i < indent_depth; i++) fprintf(f, "\t");

   fprintf(f, "%s %s(", header->type->u.function_type.return_type->name, header->name);

   for (int i = 0; i < header->type->u.function_type.param_count; i++)
   {
...
static void print_statement_compound(FILE* f, Statement* statement, bool fully_evaluated, int indent_depth, bool suppress_semicolon)
{
   ...
   for (StatementChainNode *s = statement->u.compound.statements->first; s; s = s->next)
   {
...
static void print_statement_null(FILE* f, int indent_depth, bool suppress_semicolon)
{
   for (int i = 0; i < indent_depth; i++) fprintf(f, "\t");
   if (!suppress_semicolon) fprintf(f, ";");
}
...
void glsl_print_statement(FILE* f, Statement* statement, bool fully_evaluated, int indent_depth, bool suppress_semicolon)
{
   switch (statement->flavour)
   {
      ...
      case STATEMENT_RETURN_EXPR:
         ...
         break;
      case STATEMENT_NULL:
         print_statement_null(f, indent_depth, suppress_semicolon);
         break;
      default:
         UNREACHABLE();
         break;
