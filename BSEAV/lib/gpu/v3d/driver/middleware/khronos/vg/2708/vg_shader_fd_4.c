...
FILE DESCRIPTION
Generation of frame draw shaders (plus single clear shader).

vg_shader_fd_4.c is generated from vg_shader_fd_4.c.qasm by:
qasm -minline_c -tb0 vg_shader_fd_4.c.qasm >vg_shader_fd_4.c
So don't edit vg_shader_fd_4.c directly.
=============================================================================*/
...
/* TMU write fifo checking */
static struct {
   uint32_t entry[8];
   uint32_t p;
   uint32_t count;
} tmu_fifo;
#define TMU_WRITE(n) {tmu_fifo.count+=n; vcos_assert (tmu_fifo.count<=4); tmu_fifo.entry[tmu_fifo.p]+=n;}
#define TMU_WRITE_S(n) {tmu_fifo.count+=n; vcos_assert (tmu_fifo.count<=4); tmu_fifo.entry[tmu_fifo.p++]+=n;}
#define TMU_LOAD {vcos_assert(tmu_fifo.p); tmu_fifo.count-=tmu_fifo.entry[--tmu_fifo.p]; tmu_fifo.entry[tmu_fifo.p]=0;}
#define TMU_USED (tmu_fifo.count)
#define TMU_AVAILABLE (4 - tmu_fifo.count)
...
0x01000dc0, 0xde020d27 /* fadd  recip, ra0.8dc, f(0.0) ; nop */ \
         ...
         default: UNREACHABLE(); }
         early_image_load = !(TMU_AVAILABLE > 0);
         if (stencil & early_image_load) {
            APPEND(p, \
...
, 0x009e7000, 0x100009e7 /* nop                          ; nop */ \
         ...
         TMU_WRITE_S(2); TMU_LOAD; TMU_LOAD; TMU_WRITE_S(2);
      } else {
         switch (where) {
         case WHERE_TU: { APPEND(p, \
...
, 0x969e7024, 0x10025e40 /* mov  t0t, f(0.0)             ; mov  ra0, r4 */ \
                          ...
                          TMU_LOAD; TMU_WRITE(1); break; }
         case WHERE_RA0:
         case WHERE_RA0_OLD: {
            early_image_load = !(TMU_AVAILABLE > 0);
            if (stencil && early_image_load) {
               APPEND(p, \
0x00000000, 0xe0020827 /* mov  r0, f(0.0)              ; nop */ \
);
               APPEND_I(p, where == WHERE_TU, \
1, \
{ \
...
MEM_HANDLE_T vg_shader_fd_get(VG_SHADER_FD_KEY_T key)
{
   ...
   if (handle == MEM_INVALID_HANDLE) {
      ...
      */
      ...
      p = (uint32_t *)mem_lock(handle, NULL);
