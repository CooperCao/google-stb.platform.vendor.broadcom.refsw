...
static uint64_t steady_clock_now_us(void)
{
   struct timespec ts;
...
public:
   ...
   uint64_t TimeNow() const { return steady_clock_now_us() + m_timeBaseOffset; }
...
private:
   ...
   uint32_t                    m_nexusTracks;
   uint32_t                    m_nexusEvents;
   int64_t                     m_timeBaseOffset;
};
...
bool PerfGetEventTrackInfo(EventContext *ctx, uint32_t track,
...
bool PerfGetEventInfo(EventContext *ctx, uint32_t event, struct bcm_sched_event_desc *event_desc)
{
   PerfData *pd = (PerfData*)ctx->internals;
...
bool PerfGetEventDataFieldInfo(EventContext *ctx, uint32_t event, uint32_t field,
...
bool PerfSetEventCollection(EventContext *ctx, BEGL_SchedEventState state)
{
   PerfData *pd = (PerfData*)ctx->internals;
   ...
   if (state == BEGL_EventStart)
   {
      ...
      pd->SetTimebaseOffset(ctx->timeSync - steady_clock_now_us());
      pd->SetActive(true);
   }
                  ...
                  bcm_sched_event_type type, ...)
{
   ...
   if (pd->IsActive())
   {
      ...
      pd->AddData(track, event, id, type, steady_clock_now_us(), args);
