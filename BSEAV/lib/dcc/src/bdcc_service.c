	...
	for ( curIndex=0 ; curIndex < NumBytes ; curIndex++ )
	{
		...
		switch ( state )
		{
			case BDCC_SRV_P_State_eLookingForHeader :
				...
				BDBG_MSG(("%s: Service Number = %d", BSTD_FUNCTION,service_number));
			...
			case BDCC_SRV_P_State_eCollectingBlock :
				...
				BDBG_MSG(("%s: CollectingBlock = %d", BSTD_FUNCTION,service_number));
			...
			case BDCC_SRV_P_State_eSkippingBlock :
				...
				BDBG_MSG(("%s: SkippingBlock", BSTD_FUNCTION));
				BytesThisBlock++ ;
		...
		{
			BDBG_MSG(("%s: FinishedHeader", BSTD_FUNCTION));
		...
		{
			BDBG_MSG(("%s: FinishedBlock", BSTD_FUNCTION));
			if ( pOutInfo )
