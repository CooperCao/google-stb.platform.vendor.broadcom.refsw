...
DRM_NO_INLINE DRM_API DRM_RESULT DRM_CALL DRM_TEE_PROXY_AES128CTR_PrepareToDecrypt(
    ...
    __in                 const NEXUS_SecurityKeySlotInfo      *pKeyslotInfo,        /* BRCM: Structure containing the key slot number to configure on sage . */
...
DRM_NO_INLINE DRM_API DRM_RESULT DRM_CALL DRM_TEE_PROXY_AES128CTR_LoadIv(
    ...
    __in                  const DRM_TEE_BYTE_BLOB            *f_pCDKB,
    __in_tee_opt          const DRM_TEE_BYTE_BLOB            *f_pOEMKeyInfo,
    __in                        DRM_UINT64                    f_ui64InitializationVector) DRM_NO_INLINE_ATTRIBUTE;
