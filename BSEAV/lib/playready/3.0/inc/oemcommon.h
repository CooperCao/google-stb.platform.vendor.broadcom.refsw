...
DRM_API DRM_RESULT DRM_CALL Oem_Tee_Module_Initialize(
    __in      DRM_VOID              *f_pOEMContext,
    __in      SRAI_ModuleHandle     *pSraiCtx
);

DRM_API DRM_RESULT DRM_CALL Oem_Tee_Module_Uninitialize(
    __in      DRM_VOID              *f_pOEMContext,
    __in  SRAI_ModuleHandle          sraiCtx
);


#define MAX_PLAYREADY_MODULES 5   /* Must be <= to MAX_PLAYREADY_CONTEXTS defined in Sage Playready 3.0 platform. */

typedef struct OEM_Context{

    CommonCryptoHandle commonCryptoHandle;   /* Nexus common crypto handle */

    OEM_Settings    settings;            /* Settings the module was opened with. */

    NEXUS_ReadOtpIO otpId;

    SRAI_PlatformHandle platformHandle;
    SRAI_ModuleHandle   sraiHandles[MAX_PLAYREADY_MODULES];
