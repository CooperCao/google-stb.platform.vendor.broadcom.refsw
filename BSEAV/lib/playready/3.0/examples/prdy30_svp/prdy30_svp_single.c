        ...
        BKNI_EventHandle event)
{
    ...
    if (videoDecoder) {
        ...
        if(videoPidChannel)NEXUS_Playpump_ClosePidChannel(videoPlaypump, videoPidChannel);
        if(videoPlaypump){
    ...
    if (audioDecoder) {
        ...
        if(audioPidChannel)NEXUS_Playpump_ClosePidChannel(audioPlaypump, audioPidChannel);
        if(audioPlaypump){
    ...
    if(videoPlaypump) NEXUS_Playpump_Close(videoPlaypump);
    if(audioPlaypump) NEXUS_Playpump_Close(audioPlaypump);
...
int main(int argc, char* argv[])
{
    ...
    if (argc < 2) {
        printf("Usage : %s <Input-Stream> [-vc1]", argv[0]);
        return -1;
    }
    ...
    printf("@@@ MSG Check Point Start vc1_stream %d--", vc1_stream);
    ...
    {
        NEXUS_HeapHandle heap = platformConfig.heap[ii];
