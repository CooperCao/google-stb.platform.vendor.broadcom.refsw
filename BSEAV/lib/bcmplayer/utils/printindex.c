...
static unsigned long bcmindex_returnPts2(const BSCT_Entry *p_sct)
{
...
static void print_sct6(int i, const BSCT_SixWord_Entry *entry)
{
    ...
    else
    {
        ...
        {
            if (!skippts)
                printf("PTS=%#lx DTS=%#x\n", bcmindex_returnPts2((const BSCT_Entry *)entry), entry->recordByteCountHi);
        }
        else
        {
            pic = sc == SC_PICTURE ? returnPictureCode(entry->startCodeBytes) : 0;
...
int main(int argc, char **argv) {
    ...
    if (indextype == b_indextype_bcm) {
        ...
        if (fread(&entry, sizeof(entry), 1, file) == 1) {
            ...
            if (!silence)
                printf("Version: %d\n", ver);
            if (ver == BNAV_Version_AVC || ver == BNAV_Version_HEVC) {
