...
b_profile_thread_entry(bprof_threads *threads,  const bprofile_sys_iface *sys_iface, const bprofile_entry *entry)
{
	...
	if (!key) {
		char buf[16];
		BDBG_WRN(("unknown thread for stack %#lx[%s]", stack, sys_iface->get_name(entry->addr, buf, sizeof(buf))));
...
b_profile_report_flat(b_profile_cntx *cntx)
{
	...
	for(thread=&threads->threads[0];thread<&threads->threads[threads->nelem];thread++) {
		...
		unsigned diff;
		unsigned prev_time;
		BDBG_MSG(("[%p] %#x:%#x %s", (void *)thread, thread->stack_min, thread->stack_max, sys_iface->thread_name(thread->thread_key)));
#endif
