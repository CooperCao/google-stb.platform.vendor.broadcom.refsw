 ...
 * This example decrypts the content and playback the video and audio. 
 * 
 * Revision History:
 *
 * $brcm_Log: $
...
/* The following function is not for production. It is available only for testing purpose. */
extern int gen_key_from_seed( uint8_t * kid, uint8_t * key );
...
int decrypt_sample( app_ctx_t app, uint32_t sampleSize, batom_cursor * cursor, bdrm_mp4_box_se_sample *pSample, uint32_t *bytes_processed )
{
    ...
    */
     ...
     */
    BKNI_Memcpy(pBuf, app->contentKey+8, 8);	/* Copy key.  H and L need to be swapped */
    BKNI_Memcpy(pBuf+8, app->contentKey, 8);	
