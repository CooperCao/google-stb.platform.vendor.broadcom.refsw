...
brtp_enqueue_pkt(brtp_t rtp, const void *pkt, size_t pkt_len, void *cookie)
{
    ...
    if(rtp->pipe_out==NULL) {
        BDBG_MSG(("brtp_enqueue_pkt: %#lx %#lx premature pkt %zu ssrc:%#x", (unsigned long)rtp, (unsigned long)pkt, pkt_len, B_RTP_LOAD32(rtp_pkt->data, 8)));
        goto discard_pkt;
    }

    BDBG_MSG_FLOW(("%#x brtp_enqueue_pkt: %#lx packet:%#lx:%zu type:%u ssrc:%#x ts:%u seq:%u(%u)", word0, (unsigned long)rtp, (unsigned long)pkt, pkt_len, B_RTP_GET_BITS(word0, 9, 15), B_RTP_LOAD32(rtp_pkt->data, 8), word1, (unsigned)B_RTP_GET_BITS(word0, 16, 31), (unsigned)rtp->state.next_sequence));
    if (
        ...
        BDBG_MSG(("%#x brtp_enqueue_pkt: %#lx packet:%#lx:%zu filtered out type:%u(%u) ssrc:%#x(%#x)", word0, (unsigned long)rtp, (unsigned long)pkt, pkt_len, B_RTP_GET_BITS(word0, 9, 15), rtp->session_cfg.pt_value, B_RTP_LOAD32(rtp_pkt->data, 8), rtp->session_cfg.ssrc_value));
        goto discard_pkt;
    }
...
brtp_spf_enqueue_pkt(brtp_spf_t rtp, const void *pkt, size_t pkt_len, void *cookie)
{
    ...
    if(rtp->pipe_out==NULL) {
        BDBG_MSG(("brtp_spf_enqueue_pkt: %#lx %#lx premature pkt %zu ssrc:%#x", (unsigned long)rtp, (unsigned long)pkt, pkt_len, B_RTP_LOAD32(rtp_pkt->data, 8)));
        goto discard_pkt;
    }

    BDBG_MSG_FLOW(("%#x brtp_spf_enqueue_pkt: %#lx packet:%#lx:%zu type:%u ssrc:%#x ts:%u seq:%u(%u)", word0, (unsigned long)rtp, (unsigned long)pkt, pkt_len, B_RTP_GET_BITS(word0, 9, 15), B_RTP_LOAD32(rtp_pkt->data, 8), word1, (unsigned)B_RTP_GET_BITS(word0, 16, 31), (unsigned)rtp->state.next_sequence));
    if (
        ((B_RTP_GET_BITS(word0, 9, 15) & rtp->session_cfg.pt_mask) !=  rtp->session_cfg.pt_value) ||
        (B_RTP_LOAD32(rtp_pkt->data, 8) & rtp->session_cfg.ssrc_mask) != rtp->session_cfg.ssrc_value) {
        BDBG_MSG(("%#x brtp_spf_enqueue_pkt: %#lx packet:%#lx:%zu filtered out type:%u(%u) ssrc:%#x(%#x)", word0, (unsigned long)rtp, (unsigned long)pkt, pkt_len, B_RTP_GET_BITS(word0, 9, 15), rtp->session_cfg.pt_value, B_RTP_LOAD32(rtp_pkt->data, 8), rtp->session_cfg.ssrc_value));
        goto discard_pkt;
    }
