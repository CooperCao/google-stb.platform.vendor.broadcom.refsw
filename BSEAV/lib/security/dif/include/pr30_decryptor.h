...
#define DRM_MAX_NUM_DECRYPTCONTEXTS ( 4 )
...
private:
    ...
    struct DecryptContextInfo
    {
        DRM_DECRYPT_CONTEXT m_drmDecryptContext;
        uint8_t             m_KeyId[BMP4_KEY_ID_LENGTH];  /* Network-ordered bytes (aka big endian) */
    };
    ...
    DRM_RESULT DeleteStoredLicenses();
    bool SetDecryptContextKeyId(DecryptContextInfo& decryptContextInfo, const DRM_WCHAR *pKID);
    bool FindSchemeIdForKeyId(const uint8_t *pKID, size_t bcpKID, bmp4_protection_info& protectionInfo, uint32_t *ioSchemeId);
    bool AddDecryptContext(DecryptContextInfo *pDecryptContextInfo, bmp4_protection_info& protectionInfo);
    bool GetDecryptContext(bmp4_protectionSchemeInfo& trackProtectionInfo, DecryptContextInfo** ioDecryptContext);
    bool ReleaseDecryptContextForSchemeId(uint32_t inSchemeId);
    bool ReleaseDecryptContexts();
    ...
    uint32_t             m_drmDecryptContextCount;
    DecryptContextInfo*  m_drmDecryptContexts[DRM_MAX_NUM_DECRYPTCONTEXTS];
