    ...
    : BaseStreamer()
{
    m_dmaHandle = NULL;
    m_dmaJob = NULL;
    LOGD(("SecureStreamer was created: %p", this));
}
...
SecureStreamer::~SecureStreamer()
{
    LOGD(("SecureStreamer::%s %p enter", __FUNCTION__, this));
    if (m_playpump) {
    ...
    if (m_playpumpBuffer) {
        SRAI_Memory_Free(m_playpumpBuffer);
    ...
    NEXUS_PlaypumpOpenSettings &playpumpOpenSettings)
{
    ...
    m_playpumpBuffer = SRAI_Memory_Allocate(
    ...
    playpumpOpenSettings.memory =
        NEXUS_MemoryBlock_FromAddress(m_playpumpBuffer);
