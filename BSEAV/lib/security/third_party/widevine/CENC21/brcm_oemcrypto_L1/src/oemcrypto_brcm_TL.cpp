...
OEMCryptoResult OEMCrypto_Initialize(void)
{
    ...
    if (DRM_WVOemCrypto_Initialize(&WvOemCryptoParamSettings, (int*)&wvRc) != Drm_Success)
    {
...
OEMCryptoResult OEMCrypto_OpenSession(OEMCrypto_SESSION* session)
{
    ...
    if (DRM_WVOemCrypto_OpenSession(&session_id, (int*)&wvRc) != Drm_Success)
    {
...
OEMCryptoResult OEMCrypto_DecryptCTR(OEMCrypto_SESSION session,
    const uint8_t* data_addr, size_t data_length, bool is_encrypted,
    ...
    const OEMCrypto_DestBufferDesc* out_buffer, uint8_t subsample_flags)
{
    ...
    switch (out_buffer->type) {
        ...
        case OEMCrypto_BufferType_Secure:
            if (!is_encrypted) {
                // This case we don't need to call DecryptCTR
                // because the clear data is already DMA transferred
                LOGV(("%s:Clear data in SVP: no actions required", __FUNCTION__));
                return OEMCrypto_SUCCESS;
            }
