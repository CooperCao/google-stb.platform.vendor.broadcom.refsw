...
LOGD(("%s: te_box.type = BMP4_CENC_TE_BOX", __FUNCTION__));
                    ...
                    uint32_t defaultIsEncrypted;
                    uint16_t defaultIVSize;
...
ErrorExit:
    if (found_fmt == true && found_mp4 == true && found_extended == true) {
...
int bmp4_parse_stsd(batom_cursor *cursor, bmp4_box *pBox, bmp4_trackInfo *pTrack)
{
    ...
    uint8_t tmp_data[5+BMP4_MAX_PPS_SPS];
...
static
int bmp4_parse_mp4_pssh(bmp4_mp4_headers *header, batom_cursor *cursor, bmp4_box *pBox)
{
    int     rc = -1;
    NEXUS_Error errCode;
...
int bmp4_parse_sample_enc_extended_box(bmp4_mp4_frag_headers *frag_header, batom_cursor *cursor)
{
    ...
    if (bmp4_parse_fullbox(cursor, &fullbox)) {
        ...
        frag_header->samples_enc.flags = fullbox.flags;
            ...
            BKNI_Memset(pSample, 0, sizeof(bmp4_drm_mp4_box_se_sample));
...
LOGD(("%s: trackId=%d", __FUNCTION__, trackId));
                ...
                if (trackType != BMP4_SAMPLE_ENCRYPTED_VIDEO &&
                    ...
                    trackType != BMP4_SAMPLE_AVC &&
                    trackType != BMP4_SAMPLE_MP4A) {
#else
                if (trackType != BMP4_SAMPLE_ENCRYPTED_VIDEO &&
                    trackType != BMP4_SAMPLE_ENCRYPTED_AUDIO) {
#endif
                    LOGW(("%s: Detected unexpected track type in fragment 0x%x, skip over.\n",
                        __func__, trackType));
int i = 0;
for (; i < BMP4_MAX_NB_OF_TRACKS; i++) {
LOGD(("scheme[trackId=%d]=%p trackType=0x%x",i, &header->scheme[i],header->scheme[i].trackType));
}
                    skip_frag = true;
                } else {
...
LOGD(("%s calling batom_cursor_from_atom", __FUNCTION__));
    ...
    if((box_hdr_size = bmp4_parse_box(&cursor, &moov))){
        ...
        if(moov.type == BMP4_MOVIE)
        {
            ...
            for(i = box_hdr_size; i < moov.size; i += box.size){
                ...
                switch(box.type){
                    case BMP4_EXTENDED:
LOGD(("calling bmp4_parse_mp4_pssh"));
                        rc = bmp4_parse_mp4_pssh(header, &cursor, &box);
                        if(rc != 0) {
                            goto ErrorExit;
                        }
                        break;
...
LOGD(("calling bmp4_parse_track"));
                    ...
                    default :
                        /* Not the box we are looking for. Skip over it.*/
LOGD(("calling batom_cursor_skip"));
                        batom_cursor_skip(&cursor, box.size - box_hdr_size);
                        break;
                }
            }
