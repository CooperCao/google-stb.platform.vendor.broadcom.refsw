...
void SSDTl_Get_Default_Settings(ssdd_Settings *ssdd_settings)
{
    ...
    if (ssdd_settings != NULL) {
        ...
        ssdd_settings->hasRpmb = true;
        ssdd_settings->formatRpmb = false;
        ssdd_settings->formatVfs = false;
    }
...
static BERR_Code SSDTl_Device_Operation(void)
{
    ...
    if (sage_operation->rpmb_partition) {
        ...
        if (rc != BERR_SUCCESS) {
            SSDD_Debug_printf("RPMB ioctl failed\n");
            goto errorExit;
        }
    } else {
        /* Execute VFS op */
        ...
        if (rc != BERR_SUCCESS) {
            SSDD_Debug_printf("VFS operation failed\n");
            goto errorExit;
        }
    }
...
static BERR_Code SSDTl_Perform_Full_Operation_Cycle(int *SSD_TA_rc)
{
    ...
    while (SSDD_Running()) {
        SSDD_Debug_printf("SAGE operation received\n");
        ...
        if ((rc != BERR_SUCCESS) || (*SSD_TA_rc != BERR_SUCCESS)) {
            SSDD_Debug_printf("Error sending result command (%d), TA result (%d)\n", rc, SSD_TA_rc);
            goto errorExit;
        }

        if (sage_operation->blockCount == 0) {
            SSDD_Debug_printf("Operation cycle finished\n");
            break;
        }
    }
...
static void SSDTl_TATerminateCallback (uint32_t platformId) {
    ...
    if (platformId != BSAGE_PLATFORM_ID_SSD) {
        SSDD_Debug_printf("Incorrect platform ID reported in terminate callback\n");
        return;
    }

    SSDD_Debug_printf("SSD TA terminate callback event\n");
...
BERR_Code SSDTl_Init(ssdd_Settings *ssdd_settings)
{
    ...
    SSDD_Debug_printf("Initialising SSD TA\n");

    if (hasInit) {
        rc = BERR_OS_ERROR;
        SSDD_Debug_printf("SSD TA already initialised\n");
        goto errorExit;
    }

    if (ssdd_settings == NULL) {
        SSDD_Debug_printf("%s - Parameter settings are NULL\n", BSTD_FUNCTION);
    ...
    if (rc != BERR_SUCCESS) {
        SSDD_Debug_printf("%s - Error Creating BKNI Event handle", BSTD_FUNCTION);
        goto errorExit;
    }
    ...
    if (container == NULL) {
        SSDD_Debug_printf("%s - Error allocating container\n", BSTD_FUNCTION);
    ...
    if (rc != BERR_SUCCESS) {
        SSDD_Debug_printf("%s - Error initializing SSD TL module (0x%08x)", BSTD_FUNCTION, container->basicOut[0]);
        goto errorExit;
    }
    ...
    if (rc != BERR_SUCCESS) {
        SSDD_Debug_printf("%s - Error registering terminate callback (0x%08x)", BSTD_FUNCTION, rc);
        goto errorExit;
    }
    ...
    if (settings.hasRpmb) {
        ...
        if (rc != BERR_SUCCESS) {
            SSDD_Debug_printf("Error sending SSD initialisation command on RPMB partition [%u]\n",
        ...
        if (rc == BERR_SUCCESS) {
            ...
            SSDD_Debug_printf("RPMB partition already initialised\n");
            rc = BERR_SUCCESS;
        }

        if ((rc != BERR_SUCCESS) || (SSD_TA_rc != BERR_SUCCESS)) {
            SSDD_Debug_printf("Error initialising RPMB partition\n");
            /* Continue running without RPMB device */
            settings.hasRpmb = false;
        }
    }
    ...
    rc = SRAI_Module_ProcessCommand(moduleHandle, SSD_CommandId_eDaemonInit, container);
    if (rc != BERR_SUCCESS) {
        SSDD_Debug_printf("Error sending SSD initialisation command on VFS partition [%u]\n",
                          container->basicOut[0]);
        goto errorExit;
    }
    ...
    if (rc == BERR_SUCCESS) {
        ...
        rc = SSDTl_Perform_Full_Operation_Cycle(&SSD_TA_rc);
        if ((rc != BERR_SUCCESS) || (SSD_TA_rc != BERR_SUCCESS)) {
            SSDD_Debug_printf("Error initialising VFS partition\n");
            rc = BERR_OS_ERROR;
            goto errorExit;
        }
    } else if (rc == BSAGE_ERR_ALREADY_INITIALIZED) {
        SSDD_Debug_printf("VFS partition already initialised\n");
        rc = BERR_SUCCESS;
    }
    ...
    SSDD_Debug_printf("Initialised SSD TA\n");
    hasInit = true;
...
errorExit:
    ...
    if (!hasInit) {
        ...
        SSDD_Debug_printf("%s - Error Exit cleanup\n", BSTD_FUNCTION);
        SSDTl_Uninit();
    }
    return rc;
}
...
void SSDTl_Uninit(void)
{
    ...
    SSDD_Debug_printf("Uninitialising SSD TA\n");
    ...
    if (container != NULL) {
        ...
        SSDD_Debug_printf("%s - Error allocating container used to uninitialise\n", BSTD_FUNCTION);
    }
    ...
    if (rc != BERR_SUCCESS) {
        SSDD_Debug_printf("%s - Error unregistering terminate callback (0x%08x)", BSTD_FUNCTION, rc);
    }
    ...
    SSDD_Debug_printf("Uninitialised SSD TA\n");
    hasInit = false;
}

static BERR_Code SSDTl_Check_Standby_Status(void) {
    BERR_Code rc = BERR_SUCCESS;
    NxClient_StandbyStatus standbyStatus, prevStatus;

    rc = NxClient_GetStandbyStatus(&standbyStatus);
    if (rc != BERR_SUCCESS) {
        SSDD_Debug_printf("ERROR NxClient_GetStandbyStatus %x\n", rc);
        goto errorExit;
    }
    ...
    if(standbyStatus.transition == NxClient_StandbyTransition_eAckNeeded) {
        ...
        while(resumed == false) {
            ...
            resumed = (standbyStatus.settings.mode == NEXUS_PlatformStandbyMode_eOn) &&
        ...
        if (rc != BERR_SUCCESS){
            SSDD_Debug_printf("ERROR SSDTl_Init %x\n", rc);
            goto errorExit;
        }
    }
...
void SSDTl_Wait_For_Operations(void)
{
    ...
    SSDD_Debug_printf("Start waiting for operations\n");
    ...
    while (SSDD_Running()) {
        ...
        if (checkAgain) {
            ...
            rc = BKNI_WaitForEvent(indication_event, SSD_TIMEOUT_MS);
        }
        ...
        if (rc == BERR_SUCCESS) {
            ...
            if (rc != BERR_SUCCESS) {
                SSDD_Debug_printf("Error getting operation\n");
                continue;
            }
            ...
            if (sage_operation->blockCount) {
                rc = SSDTl_Perform_Full_Operation_Cycle(&SSD_TA_rc);
                if (rc != BERR_SUCCESS) {
                    SSDD_Debug_printf("Error occurred completing operation cycle\n");
                    continue;
                }

                if (SSD_TA_rc != BERR_SUCCESS) {
                    SSDD_Debug_printf("Operation cycle failed\n");
                }
            ...
            rc = SSDTl_Check_Standby_Status();
            if (rc != BERR_SUCCESS) {
                SSDD_Debug_printf("Error occurred checking standby status\n");
                goto errorExit;
            }
        } else {
            SSDD_Debug_printf("BKNI_WaitForEvent failed\n");
            goto errorExit;
        }
    }

errorExit:
    SSDD_Debug_printf("Stop waiting for operations\n");
}
...
static void  SSDTl_priv_indication_cb(SRAI_ModuleHandle module, uint32_t arg, uint32_t id, uint32_t value)
{
    ...
    if ((id == SSD_IndicationType_eOperationSend) && (module == moduleHandle)){
        SSDD_Debug_printf("Indication event received, Id: %d Val: %d \n", id, value);
