...
#define VFS_DEFAULT_PATH "/tmp/vfs.raw"
...
BERR_Code SSD_VFS_Init(ssd_vfs_settings *settings)
{
    ...
    SSDD_Debug_printf("VFS filesystem path: %s\n", g_settings.fs_path);
...
BERR_Code SSD_VFS_Operation(ssd_rpmb_frame_t *frames, unsigned int blockCount) {
    ...
    if ((addr + blockCount) > SSD_VFS_NUM_BLOCKS) {
        SSDD_Debug_printf("Request out of range\n");
        rc = NEXUS_INVALID_PARAMETER;
        goto errorExit;
    }

    if (((rpmb_type == MMC_RPMB_WRITE) || (rpmb_type == MMC_RPMB_READ)) && blockCount == 0) {
        SSDD_Debug_printf("Must specify a block count\n");
        rc = NEXUS_INVALID_PARAMETER;
        goto errorExit;
    }
    ...
    if (file_fd == NULL) {
        ...
        if (file_fd == NULL) {
            SSDD_Debug_printf("Error opening VFS partition\n");
            rc = BERR_OS_ERROR;
            goto errorExit;
        }
    }
    ...
    if (rc) {
        SSDD_Debug_printf("Error seeking VFS partition\n");
        goto errorExit;
    }
    ...
    case MMC_RPMB_WRITE:
        for (int i = 0; i < blockCount; i++) {
            ...
            if (size != 1) {
                SSDD_Debug_printf("Error writing to VFS partition\n");
                rc = BERR_OS_ERROR;
                goto errorExit;
            }
        }
    ...
    default:
        SSDD_Debug_printf("Invalid request\n");
        rc = NEXUS_INVALID_PARAMETER;
        goto errorExit;
        break;
    }
