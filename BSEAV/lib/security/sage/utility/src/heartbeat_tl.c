...
BERR_Code HeartbeatTl_Init(HeartbeatTlSettings *pHeartbeatModuleSettings)
{
    ...
    if(rc != BERR_SUCCESS)
    {
        BDBG_ERR(("%s - Error initializing Heartbeat TL module (0x%08x)", __FUNCTION__, container->basicOut[0]));
        goto ErrorExit;
    }
...
BERR_Code HeartbeatTl_ProcessCommand(uint32_t command_id)
{
    ...
    {
        BDBG_ERR(("%s - Invalid command ID (%u)", __FUNCTION__, command_id));
        rc = BERR_INVALID_PARAMETER;
        goto handle_error;
    }
    ...
    {
        BDBG_ERR(("%s - error taking SAGE's heartbeat (sage rc = %u, actual rc = %u)", __FUNCTION__, sage_rc, container->basicOut[0]));
        rc = sage_rc;
        goto handle_error;
    }
