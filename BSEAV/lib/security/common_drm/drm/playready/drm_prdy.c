    ...
    )
{
    ...
    if(pClonedDecryptContext->pKeyContext){
        ...
        if(pDecryptContext->pKeyContext != NULL){
            BKNI_Memcpy(pClonedDecryptContext->pKeyContext, pDecryptContext->pKeyContext, sizeof( *pDecryptContext->pKeyContext));
        ...
        uint16_t               milliseconds)
{
    ...
    BDBG_MSG(("%s:%d - Calling Oem_Clock_SetSystemTime with BBX context %x",__FUNCTION__,__LINE__,&poAppContextInternal->oBlackBoxContext));
    Oem_Clock_SetSystemTimeFromBBContext(&poAppContextInternal->oBlackBoxContext, &systemTime);
