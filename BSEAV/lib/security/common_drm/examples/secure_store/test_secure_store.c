...
int main(void)
{
    ...
    for(jj=0; jj < 10; jj++)
    {
		...
		if(rc != Drm_Success)
		{
			printf("%s - DRM_Common_BasicInitialize() failed (%u)\n", __FUNCTION__, jj);
			return 1;
		}
		...
		if(BKNI_Memcmp(pDmaBuf, data, 16) != 0)
		{
			printf("\n\t###   MAIN - Error comparing decrypted data with original (%u)\n", jj);
			return -3;
		}
