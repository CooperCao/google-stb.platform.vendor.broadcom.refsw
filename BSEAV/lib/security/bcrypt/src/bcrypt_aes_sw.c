...
extern void BCrypt_CMACSw_Xor_128(unsigned char *a, unsigned char *b, unsigned char *out);
...
void AES_128(unsigned char *key, unsigned char *data, unsigned char *ciphertext)
{
    ...
    for (round = 0; round < 11; round++)
    {
        if (round == 0)
        {
            BCrypt_CMACSw_Xor_128(round_key, data, ciphertext);
	...
	else if (round == 10)
        {
            ...
            BCrypt_CMACSw_Xor_128(intermediateb, round_key, ciphertext);
        }
        else    /* 1 - 9 */
        {
            byte_sub(ciphertext, intermediatea);
            ...
            BCrypt_CMACSw_Xor_128(intermediatea, round_key, ciphertext);
            next_key(round_key, round);
        }
    }
}
