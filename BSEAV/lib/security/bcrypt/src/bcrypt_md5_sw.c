                                    ...
                                    BCRYPT_S_MD5SwParam_t *pInputParam     )
{
    ...
    BDBG_MSG(("Inside BCrypt_MD5Sw\n"));
    BDBG_ENTER(BCrypt_MD5Sw);
    BDBG_ASSERT( hBcrypt );
     ...
     */
    MD5_Init (&c);
    MD5_Update (&c, pInputParam->pbInData, pInputParam->cbInData);
    MD5_Final (pInputParam->pbOutData, &c);

BCRYPT_P_DONE_LABEL:

    BDBG_LEAVE(BCrypt_MD5Sw);
    return( errCode );
}
