                                         ...
                                         unsigned int* pLengthDecoded)
{
    ...
    at = d2i_ASN1_TYPE(NULL,ppEncoded,lengthEncoded);

    if(!at)
    {
        printf("Error parsing structure\n");
    }
    else
    {
        ...
        else
        {
            ...
            *pLengthDecoded = at->value.asn1_string->length;
            if (*pLengthDecoded > 0)
            {
                *ppDecoded = malloc(*pLengthDecoded);

                memcpy(*ppDecoded, at->value.asn1_string->data, *pLengthDecoded);
