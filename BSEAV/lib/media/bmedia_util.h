...
unsigned bmedia_waveformatex_pcm_block_size(const bmedia_waveformatex *wf);
...
typedef struct bmedia_bpp_pkt {
    uint32_t data[10];
} bmedia_bpp_pkt;

#define B_MPEG2PES_BPP_LENGTH 184
size_t bmedia_make_bpp_pkt(uint16_t stream_id, const bmedia_bpp_pkt *bpp, void *pkt, size_t length);
