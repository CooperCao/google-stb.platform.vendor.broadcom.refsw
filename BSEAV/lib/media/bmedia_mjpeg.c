/***************************************************************************
 *     Copyright (c) 2012, Broadcom Corporation
 *     All Rights Reserved
 *     Confidential Property of Broadcom Corporation
 *
 *  THIS SOFTWARE MAY ONLY BE USED SUBJECT TO AN EXECUTED SOFTWARE LICENSE
 *  AGREEMENT  BETWEEN THE USER AND BROADCOM.  YOU HAVE NO RIGHT TO USE OR
 *  EXPLOIT THIS MATERIAL EXCEPT SUBJECT TO THE TERMS OF SUCH AN AGREEMENT.
 *
 * $brcm_Workfile: $
 * $brcm_Revision: $
 * $brcm_Date: $
 *
 * Module Description:
 *
 * Media parser library,  MJPEG support
 * 
 * Revision History:
 *
 * $brcm_Log: $
 * 
 *******************************************************************************/
#include "bstd.h"
#include "bmedia_util.h"

/* Default DHT Segment */
 
static const uint8_t b_media_jpeg_stream_header[20]=
{
 0xFF, 0xD8,    /* SOI */
 0xFF, 0xE0,    /* APP0 */
 0x00, 16,      /* APP0 header size (including this field, but excluding preceding */
 'J', 'F', 'I', 'F', 0, /* ID string 'JFIF\0' */
 1, 1,          /* version */
 0,             /* bits per type */
 0, 0,          /* X density */
 0, 0,          /* Y density */
 0,             /* X thumbnail size */
 0,             /* Y thumbnail size */
};
const batom_vec bmedia_jpeg_stream_header = BATOM_VEC_INITIALIZER((void *)b_media_jpeg_stream_header, sizeof(b_media_jpeg_stream_header)); 

static const uint8_t b_media_jpeg_default_dht[4+0x1A0] =
{
/* JPEG DHT Segment for YCrCb omitted from MJPG data */
 
};

const batom_vec bmedia_jpeg_default_dht = BATOM_VEC_INITIALIZER((void *)b_media_jpeg_default_dht, sizeof(b_media_jpeg_default_dht)); 


static const uint8_t b_media_jpeg_dqt[]={0xff, 0xdb};
const batom_vec bmedia_jpeg_dqt = BATOM_VEC_INITIALIZER((void *)b_media_jpeg_dqt, sizeof(b_media_jpeg_dqt)); 

static const uint8_t b_media_jpeg_dht[]={0xff, 0xc4};
const batom_vec bmedia_jpeg_dht = BATOM_VEC_INITIALIZER((void *)b_media_jpeg_dht, sizeof(b_media_jpeg_dht)); 

static const uint8_t b_media_jpeg_sof[]={0xff, 0xc0};
const batom_vec bmedia_jpeg_sof = BATOM_VEC_INITIALIZER((void *)b_media_jpeg_sof, sizeof(b_media_jpeg_sof)); 

static const uint8_t b_media_jpeg_sos[]={0xff, 0xda};
const batom_vec bmedia_jpeg_sos = BATOM_VEC_INITIALIZER((void *)b_media_jpeg_sos, sizeof(b_media_jpeg_sos)); 

static const uint8_t b_media_jpeg_eoi[]={0xff, 0xd9};
const batom_vec bmedia_jpeg_eoi = BATOM_VEC_INITIALIZER((void *)b_media_jpeg_eoi, sizeof(b_media_jpeg_eoi)); 

