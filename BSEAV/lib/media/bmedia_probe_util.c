...
bh265_video_sps_to_string(const bh265_video_sps *sps, char *buf, size_t size)
{
    char profile_number[8];
    const char *profile;
    /* ITU-T Rec. H.265 (04/2015) High efficiency video coding.  Annex A  Profiles, tiers and levels */
    switch(sps->general_profile_idc) {
    case 1: profile = "Main";break;
    case 2: profile = "Main 10";break;
    case 3: profile = "Main Still";break;
    case 5: profile = "High";break;
    case 9: profile = "Screen";break;
    default:
        profile = profile_number;
        BKNI_Snprintf(profile_number, sizeof(profile_number),"%u", sps->general_profile_idc);
        break;
    }
    return BKNI_Snprintf(buf, size, "P:%s L:%u.%u T:%s", profile, sps->general_level_idc/30, (sps->general_level_idc/3)%10, sps->general_tier_flag?"High":"Main");
}
