...
int cdl_sec_brcm_check_integrity_from_buffer(unsigned char * data, int data_len,
    ...
    unsigned long key4_flash_addr = 0;
    /* 
     * calculate sha digest for data 
     */
    flush_d_cache((unsigned int) data, data_len);
...
int cdl_sec_brcm_image_validation(int bank) {
    ...
    /* image validation */
	xprintf_prod("[%d]Validating image signature\n", bank);
    ...
    /* check the checksum  */
    if (cfe_check_scratch_reg_bit_0()) {
        /*
 ...
 out:
    ...
    if (ret < 0) {
         ...
         * validation failed.
         */
        xprintf_prod("[%d]Image verification failed\n", bank);
        image_validated = -1;
