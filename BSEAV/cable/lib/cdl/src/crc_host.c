...
unsigned long FastCrc32_host(const void *pData, unsigned int numberOfBytes)
{
	...
	/*
	    If there are less than 4 bytes, just return the data itself. fill the high bytes with 0.
		This might voilate the crc algorithm, but we can meet the basic purpose of checksum,
		guarante the data integraty if the checksum is the same.
    */
    if (numberOfBytes < 4)
    {
                                       ...
                                       unsigned long prev_crc, int flag)
{
	...
	/*
		...
		guarante the data integraty if the checksum is the same.
    */
    if (numberOfBytes < 4 && (flag & FIRST_BLOCK))
    {
        const unsigned char *pDataUint8 = (const unsigned char *) pData;
        result  = 0;
        while (numberOfBytes> 0) {
