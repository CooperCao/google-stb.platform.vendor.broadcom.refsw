...
static void     tree_from_node(struct tree *tp, struct nodex *np);
static void     do_subtree(struct tree *, struct nodex **);
static void     do_linkup(struct module *, struct nodex *);
...
static void     init_node_hash(struct nodex *);
...
static void     free_node(struct nodex *);
...
static struct nodex *parse_objectid(FILE *, char *);
...
static struct nodex *parse_asntype(FILE *, char *, int *, char *);
static struct nodex *parse_objecttype(FILE *, char *);
static struct nodex *parse_objectgroup(FILE *, char *, int,
                                      struct objgroup **);
static struct nodex *parse_notificationDefinition(FILE *, char *);
static struct nodex *parse_trapDefinition(FILE *, char *);
static struct nodex *parse_compliance(FILE *, char *);
static struct nodex *parse_capabilities(FILE *, char *);
static struct nodex *parse_moduleIdentity(FILE *, char *);
static struct nodex *parse_macro(FILE *, char *);
static void     parse_imports(FILE *);
static struct nodex *parse(FILE *, struct nodex *);
...
static struct nodex *merge_parse_objectid(struct nodex *, FILE *, char *);
...
static void
init_node_hash(struct nodex *nodes)
{
    struct nodex    *np, *nextp;
    int             hash;
...
{
    struct nodex    *np;
    np = (struct nodex *) calloc(1, sizeof(struct nodex));
    if (np) {
        np->tc_index = -1;
        np->modid = modid;
...
#ifdef TEST
static void
print_nodes(FILE * fp, struct nodex *root)
{
 ...
 */
static void
do_subtree(struct tree *root, struct nodex **nodes)
{
    struct tree    *tp, *anon_tp = NULL;
    struct tree    *xroot = root;
    struct nodex    *np, **headp;
    struct nodex    *oldnp = NULL, *child_list = NULL, *childp = NULL;
    int             hash;
    int            *int_p;
...
static void
do_linkup(struct module *mp, struct nodex *np)
{
    struct module_import *mip;
    struct nodex    *onp, *oldp, *newp;
    struct tree    *tp;
    int             i, more;
    /*
...
parse_objectid(FILE * fp, char *name)
{
    ...
    struct nodex    *np, *root = NULL, *oldnp = NULL;
    struct tree    *tp;
...
{
    int             type, nexttype;
    struct nodex    *np = alloc_node(current_module);
    char            nexttoken[MAXTOKEN];
...
static void
scan_objlist(struct nodex *root, struct objgroup *list, const char *error)
{
    int             oLine = mibLine;
 ...
 */
static struct nodex *
parse(FILE * fp, struct nodex *root)
{
    char            token[MAXTOKEN];
    char            name[MAXTOKEN];
    int             type = LABEL;
...
{
printf("add_mibdir - not_for_ecos\n");
#ifdef NOT_FOR_ECOS
    FILE           *fp, *ip;
    DIR            *dir, *dir2;
    const char     *oldFile = File;
 ...
 */
static struct nodex *
merge_parse_objectid(struct nodex *np, FILE * fp, char *name)
{
    struct nodex    *nnp;
    /*
 ...
 */
static void
tree_from_node(struct tree *tp, struct nodex *np)
{
