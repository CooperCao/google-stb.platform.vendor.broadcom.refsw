...
//  $Id$
//
//  Filename:       NullTerminalConsoleThread.cpp
//  Author:         Maurice Turcotte
//  Creation Date:  01 MAY 2007
//
//****************************************************************************
//  Description:
//      This is the implementation of a Terminal Console Thread that does
//      absolutely nothing. Use this if you want a silent console, but need
//      to have the thread running.
//
//****************************************************************************
...
// Returns:  N/A
//
BcmNullTerminalConsoleThread::BcmNullTerminalConsoleThread(bool startSuspended,
    ...
    BcmTerminalConsoleThread(startSuspended, initialPriority)
{
    ...
    fMessageLogSettings.SetModuleName("BcmNullTerminalConsoleThread");
    ...
    {
        gErrorMsg(fMessageLogSettings, "BcmNullTerminalConsoleThread")
...
// Returns:  N/A
//
BcmNullTerminalConsoleThread::~BcmNullTerminalConsoleThread()
{
#ifdef NOTDEF
    // Reset the original terminal settings.
    tcsetattr(STDIN_FILENO, TCSANOW, &fOriginalTerminalIO);

    gInfoMsg(fMessageLogSettings, "~BcmNullTerminalConsoleThread")
...
//
bool BcmNullTerminalConsoleThread::ReadCharacter(char &character)
{
#ifdef NOTDEF
    long result;
    ...
    if (result > 0)
    {
        gAppMsg(fMessageLogSettings, "ReadCharacter", 0x80000000)
            << "User typed character code " << (void *) character << endl;
...
//
BcmNullTerminalConsoleThread::EscapeKeySequence BcmNullTerminalConsoleThread::ReadEscapeSequence(void)
{
#ifdef NOTDEF
    bool keyRead;
    char character;
...
//
bool BcmNullTerminalConsoleThread::WriteCharacter(char character)
{
#ifdef NOTDEF
//    long result;
    ...
    // Temporary fix.  Currently STDIN_FILENO gives EBADF error on
	// uclibc rootfs, but not glibc.  Hack is to use gLogMessageRaw
	// instead of write().

	gLogMessageRaw << character << flush;

/* 
    result = write(STDIN_FILENO, &character, 1);
...
//      false if a problem occurred.
//
bool BcmNullTerminalConsoleThread::WriteBackspace(void)
{
#ifdef NOTDEF
    long result;
    ...
    // Write it out.
    result = write(STDIN_FILENO , backspace, 3);
