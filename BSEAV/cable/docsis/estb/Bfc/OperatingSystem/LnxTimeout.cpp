...
#if NPTL_SUPPORT
long BcmLinuxTimeout::nTimeout = 0;
#else                                            
...
BcmLinuxTimeout::BcmLinuxTimeout(pthread_t tid)
{
	...
	memset(pString, 0, 32);
	sprintf(pString, "Timeout(0x%x)", tid);
...
//		<< pfName << " tid = " << hex << (void *)pthread_self() << endl;
    ...
    // Make sure not use out signal.
    if (nTimeout >= (SIGRTMAX - SIGRTMIN))
    {
        // looked at all objects without a match
        cout << "BcmLinuxtimeout: err! in tid: ==== " << pthread_self() << endl;
    }

    timer_event.sigev_notify = SIGEV_SIGNAL;
    timer_event.sigev_signo = SIGRTMIN + nTimeout;
    timer_event.sigev_value.sival_ptr = (void *)&myTimer;
    if (timer_create(CLOCK_REALTIME, &timer_event, &myTimer) < 0)
    {
        cout << "StopTimer: error. tid: " << pthread_self() << endl; 
    }
#endif 
...
#if NPTL_SUPPORT
    // Unmask the signals that this thread want to receive.
    sigemptyset(&set);
    sigaddset(&set, SIGUSR1);
    sigaddset(&set, SIGRTMIN + nTimeout);
    pthread_sigmask(SIG_UNBLOCK, &set, NULL);

    // Register per-thread timer handler 
    struct sigaction action;
    action.sa_sigaction = BcmLinuxTimeout::LTOAlarmHandler;
    // Real time signal
    action.sa_flags = SA_SIGINFO; 
    sigemptyset (&action.sa_mask);
    sigaddset (&action.sa_mask, SIGUSR1);
    sigaddset (&action.sa_mask, SIGRTMIN + nTimeout);
    sigaction (SIGRTMIN + nTimeout, &action, NULL);
    nTimeout++;
#else
    if (!fSignalInitted)
	{
...
//			<< "BcmLinuxTimeout::StopTimer" << endl;
    ...
    if (timer_settime(myTimer,0,&mytime,NULL) < 0)
    {
        cout << "StopTimer: error. tid: " << pthread_self() << endl; 
    }
#else 
    ...
    struct itimerval mytime;
    timerclear(&(mytime.it_value));
    timerclear(&(mytime.it_interval));
...
{
//    cout << "BcmLinuxTimeout::StartTimer " << duration_ms << endl;
//    cout << "StartTimer: pid = " << getpid() 
//         << " tid = " << pthread_self() << endl;
    // (re)establish alarm handler
//    signal( SIGALRM, BcmLinuxTimeout::LTOAlarmHandler ); 
#if NPTL_SUPPORT
    // fill the timer details.
    struct itimerspec mytime;
    mytime.it_value.tv_sec = duration_ms/1000;
    mytime.it_value.tv_nsec = (duration_ms%1000)*1000;
    mytime.it_interval.tv_sec = 0;
    mytime.it_interval.tv_nsec = 0;
    if (timer_settime(myTimer,0,&mytime,NULL) < 0)
    {
        cout << "StartTimer: error. tid: " << pthread_self() << endl; 
    }
#else 
    // set the timer duration & start it
    struct itimerval mytime;
    mytime.it_value.tv_sec = duration_ms/1000; // get the seconds
    mytime.it_value.tv_usec = (duration_ms%1000)*1000; // get the ms, convert to us
    timerclear(&(mytime.it_interval));
    setitimer (ITIMER_REAL, &mytime, NULL);
#endif
    if (record_start)
        ...
        while (!foundit && !checked_nodes)
        {
            ...
            while (!foundit && !checked_eles)
            {
                if (eleptr->timerid == victim)
                {
                    ...
                    root_removed = RemoveElement (nodeptr, prevnode, eleptr, prevele);
                }
                else
                {
                    prevele = eleptr;
...
//----------------------------------------------------------------------------
#if NPTL_SUPPORT
void BcmLinuxTimeout::LTOAlarmHandler (int signal_number, siginfo_t *extra, void *cruft)
#else
...
#if NPTL_SUPPORT
long BcmLinuxTimeout::GetnTimeout (void)
{
    return nTimeout;
}
#endif  
