 ...
 ******************************************************************************/
//****************************************************************************
//  $Id: CommandTable.cpp 1.53 2007/05/23 13:41:02Z dpullen Release $
//
...
{
    list<Instance>::iterator myIterator;
    unsigned int instanceNumber, nameLength;
    ...
    // Get the length of the instance name once.
    nameLength = strlen(pInstanceName);
    ...
    for (myIterator = fInstances.begin(); myIterator != fInstances.end(); ++myIterator)
    {
        ...
        if (strnicmp((*myIterator).pfInstanceName, pInstanceName, nameLength) == 0)
        {
            outputStream << instanceNumber << ")  " 
                         << (*myIterator).pfInstanceName << "  ("
                         << (*myIterator).pfInstanceValue << ")\n";
