...
# UTF Framework Object Definition for a ControlChart using an n=2
# moving range.  To be used for data which cannot be logically
# clustered, such as memory tests.
#
# With a little work on the cache file format it should be possible to
# roll this back into ControlChant.tcl
#
# $Id: c4a93aeb081d5473d04814cf1eacc0c90192492c $
# $Copyright Broadcom Corporation$
#

package provide UTF::MemChart 2.0

package require snit
package require UTF::doc

UTF::doc {
    # [manpage_begin UTF::MemChart n 1.0]
    # [moddesc {HND Wireless Test Framework}]
    # [titledesc {UTF MemChart}]
    # [copyright {2008 Broadcom Corporation}]
    # [require UTF::MemChart]
    # [description]
    # [para]

    # UTF::MemChart takes a data point (usually from a memory test)
    ...
    # MemChart can optionally use gnuplot to provide graphical plots
    # for visual reporting in addition to the autimated analysis.

    # MemChart is currently called by Test/memchart.test.  There is a
    # related self test utility Test/randomchart2.test. See either of
    # these scripts for practical examples of setting up the
    # appropriate object and calling the addsample method.

    # [list_begin definitions]
}

snit::type UTF::MemChart {

    UTF::doc {
        # [call [cmd UTF::MemChart] [arg name]
	...
	# [opt_def [option -s] [arg integer]]
        ...
        # value, range is 2 to 10.  Default is 2.  Values other than 2
        # may work, but have not been extensively tested.  For tests
        # that provide several datapoints per sample, see [uri
        # APdoc.cgi?UTF::ControlChart.tcl UTF::ControlChart]
	...
	# [opt_def [option -norangecheck] [arg boolean]]

	# By default, zero and negative values are assumed to be an
	# immediate failure and are not reported performance cache.
	# Setting [option -norangecheck] [arg true] disables this and
	# allows zero and negative values to be treated normally.

        # [list_end]

        # [para]
        # MemChart objects have the following methods:
        # [list_begin definitions]
    }
    ...
    option -title "Memory"
    option -ylabel ""
    option -konvert -readonly true -type boolean
	...
	if {$options(-perfcache) eq ""} {
		...
		error "MemChart: Unable to find default for -perfcache.  Please use -perfcache or set UTF::Perfcache or UTF::SummaryDir"
	    }
	}
	...
	set ibase [expr {$num - ([llength $mmmlist]/2)}]
	...
	puts $G {plot \
		     "-" using 1:2 axes x1y1 title "x" with linespoints lt 1, \
	...
	if {[UTF::GnuplotVersion] > 4.0} {
	    puts $G "set terminal png size 640,480 notransparent"
	    puts $G {set tmargin -1; set bmargin -1}
	    puts $G {set lmargin -1; set rmargin -1}
	} else {
	    puts $G "set terminal png notransparent medium"
	    puts $G "set size 1"
	}
	...
	if {[catch {open $cache r} f]} {
	    ...
	    # option
	    if {!$options(-konvert) && [info exists ::UTF::PerfCacheMigrate]} {
		set options(-oldkey) \
		...
		if {$options(-konvert)} {
		    set konvert 1
		    UTF::Message INFO "" \
			"Converting cache: $options(-oldkey) -> $options(-key)"
		} else {
		    UTF::Message INFO "" \
	...
	while {[gets $f line] >= 0} {
	    switch [llength $line] {
		1 {
		    # Legacy format - add dummy session
		    lappend mmmlist 0
		    set v $line
		}
		2 {
		    lappend mmmlist [lindex $line 0]
		    set v [lindex $line 1]
		    if {$konvert} {
			set v [expr {$v / 1024.0}]
		    }
	    ...
	    # Hack - discard extreme values from MFGC error codes
	    if {$v > -800} {
		lappend mmmlist $v
	    }
	}
    ...
    UTF::doc {
        ...
        # Store the session mean data in a disk file for future use.
	...
	foreach {session mean} $mmmlist {
	    # Record if within cache plausibility limits
	    if {($options(-cachemin) eq "" || $mean > $options(-cachemin)) &&
		($options(-cachemax) eq "" || $mean < $options(-cachemax))} {
		puts $f "$session $mean"
	    }
	}
	close $f
    ...
    UTF::doc {
        ...
        # which is a flat list of session mean pairs.  session is not
        # used.
	...
	set last ""
 	foreach {- mean} $mmmlist {
	    if {$last ne ""} {
		set meansum [expr {$meansum + $mean}]
		set R [expr {abs($mean - $last)}]
	...
	# Force into permitted range
	if {$options(-cachemax) ne ""} {
	    if {$Ux > $options(-cachemax)} {
		set Ux $options(-cachemax)
	    }
	    if {$Lx > $options(-cachemax)} {
		set Lx $options(-cachemax)
	    }
	}
	if {$options(-cachemin) ne ""} {
	    if {$Lx < $options(-cachemin)} {
		set Lx $options(-cachemin)
	    }
	    if {$Ux < $options(-cachemin)} {
		set Ux $options(-cachemin)
	    }
	}
	...
	# Compare the current datapoint against the controlchart
	# bounds determined by the the historical data.
	...
	set R [expr {abs($mean - [lindex $mmmlist end-2])}]
	...
	if {$options(-cachemin) eq "0" && $mean == 0} {
	    append msg " ZERO"
	} elseif {$mean < $Lx} {
	    append msg " LOW"
	} elseif {$mean > $Ux} {
    ...
    UTF::doc {
        ...
        # [arg mmm] is list of numeric datapoints. [para]
	...
	foreach m $mmm {
	    lappend mmmlist $UTF::SessionID $m
	}

        # Remove earlier values, keeping last (-history) datapoints.
	set mmmlist [lreplace $mmmlist 0 end-[expr {2*$options(-history)}]]
	...
	if {[llength $mmmlist] < 4} {
	    # need at least 2 entries
	    set mmmlist [concat $mmmlist $UTF::SessionID $mmm]
	}
