...
set ::UTF::SetupTestBed {
	...
	#
	foreach S {4360ref 4360aci 43217t} {
	catch {$S wl down}
	$S deinit
    }
	G1 attn default
...
4360ref configure -ipaddr 192.168.1.101 -attngrp G1 -hasdhcpd 1 -ap 1 \
...
4360aci configure -ipaddr 192.168.2.105 -attngrp G2 -hasdhcpd 1 -ap 1 \
...
UTF::Linux blr05tst1 \
        ...
        -brand linux-internal-wl \
        -tag "BISON_BRANCH_7_10" \
        -wlinitcmds {wl down; wl country US;} \
...
# Adapter        - Adex PCIe Half  Miniflex
# RF Enclosure 3 - Ramsey STE 5000
# Power          - npc12 port 2    172.1.1.21
################################################################################


UTF::Linux blr05tst2 \
        -lan_ip 10.132.116.47 \
        -sta {43217 eth0} \
        -power "npc12 2"\
        -power_button "auto" \
        -tcpwindow 4M \
...
# Adapter        - Adex PCIe Miniflex
# RF Enclosure 3 - Ramsey STE 5000
# Power          - npc12 port 1      172.1.1.21
################################################################################

# UTF::Linux blr05tst4 \
        # -lan_ip 10.132.116.49 \
        # -sta {4331 eth0} \
        # -tcpwindow 4M \
        # -power "npc12 1"\
        # -power_button "auto" \
...
UTF::Linux blr12tst2 -lan_ip 10.132.116.97 -sta {43602l enp1s0} \
    ...
    -yart {-pad 23 -attn2g 40-95 -frameburst 1} \
     ...
     -pre_perf_hook {{%S wl ampdu_clear_dump} {%S wl amsdu_clear_counters} {%S wl rssi} {%S wl dump rssi}} \
     -post_perf_hook {{%S wl rssi} {%S wl rate} {%S wl nrate} {%S wl dump phycal} {%S wl dump rssi} {%S wl chanspec} } \
	  

# 43602l configure -ipaddr 192.168.1.103 \

43602l clone 43602lt -tag trunk \

43602l clone 43602tdls -type obj-debug-apdef-stadef-p2p-mchan-tdls \

43602tdls clone 43602dual -sta {43602l eth0 43602l.2 wl0.2} \
    -wlinitcmds {wl down;wl vht_features 3; wl bss -C 2 sta} \
    

43602l.2 configure -dualband {4360aci 43602l -c1 36/80 -c2 52/80 -b1 800m -b2 800m} \

43602tdls clone 43602.1 -sta {43602.1 eth0 _43602.2 wl0.2} \
    -wlinitcmds {wl down; wl vht_features 3;wl interface_create sta}\
    -perfchans {36/80 36l 36 3}  -nocustom 1 \

_43602.2 clone 43602.2 -sta {_43602.1 eth0 43602.2 wl0.2} \
    -nocustom 1 \
	-perfchans {36/80 36l 36 3} \
	
43602.1 configure -dualband {4360aci 43602.2 -c1 36/80 -c2 161/80 -b1 800m -b2 800m} 
...
set UTF::StaNightlyCustom {
	...
	MultiSTANightly -ap1 $Router -ap2 $(ap2) -sta $STA -mode1 "sta" -mode2 "ap"  \
