...
void add_iQosRules(char *pcWANIF)
{
	char *buf;
	char *g;
	char *p;
	...
	bcount_enable = inuse = sticky_enable = 0;
		...
		*/
	{
		...
		/* End of the rule */
		{
			...
			if (sticky_enable)
				class_num &= 0xFF;
			snprintf(acClass, sizeof(acClass), "0x%x/0xFF", class_num);
	...
	*/

	{
		...
		class_num = i + 1;
		if (method == 1) class_num |= 0x200;
		snprintf(acClass, sizeof(acClass), "0x%x", class_num);
