...
if [[ "${WIFI_WL_FD[0]}" = "1" && "${WIFI_WL_FD[1]}" = "1" ]]; then
        if [[ ! -f /lib/firmware/brcm/${WIFI_WL_CHIP[0]}-00-00-0000_${WIFI_WL_PCINAME[0]}.nvm || \
                -z $(sed -n '/wlunit/p' /lib/firmware/brcm/${WIFI_WL_CHIP[0]}-00-00-0000_${WIFI_WL_PCINAME[0]}.nvm) ]]; then
                echo "wlunit=0" >> /lib/firmware/brcm/${WIFI_WL_CHIP[0]}-00-00-0000_${WIFI_WL_PCINAME[0]}.nvm
        fi
        if [[ ! -f /lib/firmware/brcm/${WIFI_WL_CHIP[1]}-00-00-0000_${WIFI_WL_PCINAME[1]}.nvm || \
                -z $(sed -n '/wlunit/p' /lib/firmware/brcm/${WIFI_WL_CHIP[1]}-00-00-0000_${WIFI_WL_PCINAME[1]}.nvm) ]]; then
                echo "wlunit=1" >> /lib/firmware/brcm/${WIFI_WL_CHIP[1]}-00-00-0000_${WIFI_WL_PCINAME[1]}.nvm
        fi
...
if [[ "${WIFI_WL_FD[0]}" = "1" || "${WIFI_WL_FD[1]}" = "1" ]]; then
        insmod ${WIFI_DRIVER_DIR}/dhd.ko instance_base=0
        sleep 2

		if [ "${WIFI_WL_CHIP[0]}" = "bcm43602a3" ]; then
			dhd -i ${WL0_IF} download /lib/firmware/brcm/bcm43602a3-firmware.bin
			sleep 1
		fi
