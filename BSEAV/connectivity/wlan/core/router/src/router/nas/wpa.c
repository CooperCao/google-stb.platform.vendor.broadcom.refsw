 ...
 *
 * $Id: wpa.c 691236 2017-03-21 11:27:46Z $
 */
...
static int wpa_new_ptk_callback(bcm_timer_id td, nas_sta_t *nas);
...
/* remove the PTK rekey timer */
void
wpa_stop_ptk_rekey_timer(nas_sta_t *sta)
{
	if (sta->ptk_rekey_timer) {
		TIMER_DELETE(sta->ptk_rekey_timer);
	}
}
...
static int
wpa_new_ptk_callback(bcm_timer_id td, nas_sta_t *sta)
{
	nas_t *nas = sta->nas;
	wpa_t *wpa = nas->wpa;
	itimer_status_t ts;
...
#ifdef WLHOSTFBT
	if (CHECK_FBT(sta->mode)) {
		if (wpa_ft_validate_ies(wpa, body->data, ntohs(body->data_len), &parse) < 0) {
...
/* Create the PTK rekey timer */
static int wpa_create_ptk_rekey_timer(wpa_t *wpa, nas_sta_t *sta)
{
	itimer_status_t ts;

	if (wpa->ptk_rekey_secs <= 0) {
		return 0;
	}

	/* If the timer created already, Set the new expiry timeout */
	if (sta->ptk_rekey_timer) {
		wpa_set_timeout(sta->ptk_rekey_timer, wpa->ptk_rekey_secs, 0);
	} else {
...
wpa_ptkinitdone2(wpa_t *wpa, nas_sta_t *sta)
{
	...
	if ((nas->flags & NAS_FLAG_WDS) ||
		CHECK_WPA2(sta->mode) || CHECK_FBT(sta->mode)) {
		if (sta->wds_td)
		...
		/* Create PTK rekey timer */
		wpa_create_ptk_rekey_timer(wpa, sta);
		return 0;
	}
...
wpa_gen_igtk(wpa_t *wpa)
{
	...
	bcopy((char*)&wpa->nas->ea, (char*)&data[data_len], ETHER_ADDR_LEN);
	data_len += ETHER_ADDR_LEN;
	memcpy(&(data[data_len]), &(wpa->igtk.ipn_lo), sizeof(uint32));
	memcpy(&(data[data_len+sizeof(uint32)]), &(wpa->igtk.ipn_hi), sizeof(uint16));
	data_len += 6;
	wpa->igtk.ipn_lo++;
...
#ifdef MFP
	/* PMKR1Name has to be inserted before Group Management Cipher Suite(GMCS) */
	if (((wpa)->cap[0] & RSN_CAP_MFPC) || ((wpa)->cap[0] & RSN_CAP_MFPR))   {
		/* Update RSNIE length to include PMKR1Name */
		body->data[1] += WPA_PMK_NAME_LEN;
		/* Shift GMCS after PMKID */
		bcopy(&body->data[offset - WPA_SUITE_LEN],
			&body->data[offset - WPA_SUITE_LEN + WPA_PMK_NAME_LEN],
			WPA_SUITE_LEN);
