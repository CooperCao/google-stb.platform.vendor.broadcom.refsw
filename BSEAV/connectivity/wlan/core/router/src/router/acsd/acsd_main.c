 ...
 *
 * $Id: acsd_main.c 687783 2017-03-02 18:52:37Z $
 */
...
acs_upgrade_downgrade_opermode(acs_chaninfo_t * c_info)
{
	...
	if (ACS_11H(c_info) && c_info->is160_upgradable &&
		...
		CHSPEC_CHANNEL(c_info->cur_chspec) == ACS_DYN160_CENTER_CH_80 &&
		ACS_OP_BW_IS_80(c_info->oper_mode)) {
		ACSD_INFO("%s Upgrading to 160 Mhz by oper_mode \n", c_info->name);
		/* Upgrade to 160Mhz done in two steps, 0x112 and then to 0x113 */
