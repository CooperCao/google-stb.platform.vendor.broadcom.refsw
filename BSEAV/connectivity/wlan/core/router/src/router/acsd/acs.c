 ...
 *
 * $Id: acs.c 686645 2017-02-23 12:56:58Z $
 */
...
/* is chanspec a passive or quite channel */
static bool acs_is_passive_chanspec(acs_chaninfo_t *c_info, chanspec_t chspec);
...
acs_run_escan(acs_chaninfo_t *c_info, uint8 scan_type)
{
	...
	while (c_info->acs_escan->acs_escan_inprogress) {
		acsd_main_loop(&tv);
	}
exit2:
	free(params);
...
acs_build_candidates(acs_chaninfo_t *c_info, int bw)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (acs_is_dfs_chanspec(c_info, candi[i].chspec) &&
			acs_is_passive_chanspec(c_info, candi[i].chspec)) {
			candi[i].is_dfs = TRUE;
			if (!rsi->reg_11h) {
...
/* is chanspec a passive or quite channel */
static bool
acs_is_passive_chanspec(acs_chaninfo_t *c_info, chanspec_t chspec)
{
	return (acs_channel_info(c_info, chspec) & WL_CHAN_PASSIVE) ? TRUE : FALSE;
}
...
acs_prep_dfs_forced_chspec_list(acs_chaninfo_t *c_info, wl_dfs_forced_t **dfs_frcd)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (!excl && ACS_FCS_MODE(c_info) && excl_chans &&
				excl_chans->count) {
			for (j = 0; j < excl_chans->count; j++) {
				if (c == excl_chans->clist[j]) {
					excl = TRUE;
...
void
acs_set_chspec(acs_chaninfo_t * c_info, bool update_dfs_params)
{
	int ret = 0;
...
void
acs_wbd_set_chspec(acs_chaninfo_t * c_info, chanspec_t chspec)
{
	int ret = 0;
	c_info->selected_chspec = chspec;
	c_info->cur_chspec = chspec;
	acs_set_chspec(c_info, FALSE);
	ret = acs_update_driver(c_info);
	if (ret)
		ACSD_ERROR("update driver failed\n");
