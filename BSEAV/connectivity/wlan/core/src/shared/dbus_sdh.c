...
static const bcm_iovar_t dbus_sdio_iovars[] = {
	{"intr",	IOV_INTR,	0, 0,	0, IOVT_BOOL,	0 },
	{"sleep",	IOV_SLEEP,	0, 0,	0, IOVT_BOOL,	0 },
	{"pollrate",	IOV_POLLRATE,	0, 0,	0, IOVT_UINT32,	0 },
	{"idletime",	IOV_IDLETIME,	0, 0,	0, IOVT_INT32,	0 },
	{"idleclock",	IOV_IDLECLOCK,	0, 0,	0, IOVT_INT32,	0 },
	{"sd1idle",	IOV_SD1IDLE,	0, 0,	0, IOVT_BOOL,	0 },
	{"membytes",	IOV_MEMBYTES,	0, 0,	0, IOVT_BUFFER,	2 * sizeof(int) },
	{"ramsize",	IOV_RAMSIZE,	0, 0,	0, IOVT_UINT32,	0 },
	{"dwnldstate",	IOV_SET_DOWNLOAD_STATE,	0, 0,	0, IOVT_BOOL,	0 },
	{"vars",	IOV_VARS,	0, 0,	0, IOVT_BUFFER,	0 },
	{"sdiod_drive",	IOV_SDIOD_DRIVE, 0, 0,	0, IOVT_UINT32,	0 },
	{"readahead",	IOV_READAHEAD,	0, 0,	0, IOVT_BOOL,	0 },
	{"sdrxchain",	IOV_SDRXCHAIN,	0, 0,	0, IOVT_BOOL,	0 },
	{"alignctl",	IOV_ALIGNCTL,	0, 0,	0, IOVT_BOOL,	0 },
	{"sdalign",	IOV_SDALIGN,	0, 0,	0, IOVT_BOOL,	0 },
	{"devreset",	IOV_DEVRESET,	0, 0,	0, IOVT_BOOL,	0 },
#ifdef BCMDBG
	{"sdreg",	IOV_SDREG,	0, 0,	0, IOVT_BUFFER,	sizeof(sdreg_t) },
	{"sbreg",	IOV_SBREG,	0, 0,	0, IOVT_BUFFER,	sizeof(sdreg_t) },
	{"sd_cis",	IOV_SDCIS,	0, 0,	0, IOVT_BUFFER,	DHD_IOCTL_MAXLEN },
	{"forcealign",	IOV_FORCEEVEN,	0, 0,	0, IOVT_BOOL,	0 },
...
dbus_sdio_bussleep(sdio_info_t *sdio_info, bool sleep)
{
	...
	if (sleep) {
		...
		if (sdio_info->dpc_sched || sdio_info->rxskip || pktq_n_pkts_tot(&sdio_info->txq))
			return BCME_BUSY;
...
dbus_sdio_txpkt(sdio_info_t *sdio_info, void *pkt, uint chan)
{
	...
	*(((uint16*)frame) + 1) = htol16(~len);
	...
	do {
		ret = bcmsdh_send_buf(sdh, SI_ENUM_BASE(sdio_info->sih), SDIO_FUNC_2, F2SYNC,
...
#ifdef BCMDBG
	if (pktqprec_n_pkts(&sdio_info->txq, prec) > qcount[prec])
		qcount[prec] = pktqprec_n_pkts(&sdio_info->txq, prec);
#endif
...
dbus_sdio_read_control(sdio_info_t *sdio_info, uint8 *hdr, uint len, uint doff)
{
	...
	sdret = bcmsdh_recv_buf(sdh, SI_ENUM_BASE(sdio_info->sih), SDIO_FUNC_2, F2SYNC,
...
dbus_sdio_rxglom(sdio_info_t *sdio_info, uint8 rxseq)
{
	...
	if (sdio_info->glom) {
		 ...
		 */
		if (usechain) {
			errcode = bcmsdh_recv_buf(sdio_info->sdh, SI_ENUM_BASE(sdio_info->sih),
				SDIO_FUNC_2, F2SYNC, (uint8*)PKTDATA(osh, pfirst),
				dlen, pfirst, NULL, NULL);
		} else if (sdio_info->dataptr) {
			errcode = bcmsdh_recv_buf(sdio_info->sdh, SI_ENUM_BASE(sdio_info->sih),
				SDIO_FUNC_2, F2SYNC, sdio_info->dataptr,
				dlen, NULL, NULL, NULL);
...
dbus_sdio_readframes(sdio_info_t *sdio_info, uint maxframes, bool *finished)
{
	...
	*finished = FALSE;
	...
	for (rxseq = sdio_info->rx_seq, rxleft = maxframes;
		...
		if (dhd_readahead && sdio_info->nextlen) {
			...
			if (!(pkt = dbus_sdcb_pktget(sdio_info, rdlen + SDALIGN, FALSE))) {
				if (sdio_info->bus == SPI_BUS) {
					...
					sdret = bcmsdh_recv_buf(sdh, SI_ENUM_BASE(sdio_info->sih),
						SDIO_FUNC_2, F2SYNC,
						rxbuf, rdlen, NULL, NULL, NULL);
					sdio_info->f2rxdata++;
					ASSERT(sdret != BCME_PENDING);
				...
				/* Read the entire frame */
				sdret = bcmsdh_recv_buf(sdh, SI_ENUM_BASE(sdio_info->sih),
					SDIO_FUNC_2, F2SYNC, rxbuf, rdlen, pkt, NULL, NULL);
				sdio_info->f2rxdata++;
				ASSERT(sdret != BCME_PENDING);
		...
		sdret = bcmsdh_recv_buf(sdio_info->sdh, SI_ENUM_BASE(sdio_info->sih), SDIO_FUNC_2,
			F2SYNC, sdio_info->rxhdr, firstread, NULL, NULL, NULL);
		sdio_info->f2rxhdrs++;
		ASSERT(sdret != BCME_PENDING);
...
xfer_done:
	...
	if (bcmsdhsdio_set_sbaddr_window(sdh, SI_ENUM_BASE(sdh->sih), FALSE)) {
                ...
                void *params, int plen, void *arg, int len, int val_size)
{
	...
	case IOV_GVAL(IOV_SBREG):
	{
		...
		addr = SI_ENUM_BASE(sdio_info->sih) + sd_ptr->offset;
		size = sd_ptr->func;
		int_val = (int32)bcmsdh_reg_read(sdio_info->sdh, addr, size);
		if (bcmsdh_regfail(sdio_info->sdh))
			bcmerror = BCME_SDIO_ERROR;
	...
	void *sdh)
{
	...
	ASSERT((uintptr)regsva == si_enum_base(0));
	...
	if (devid == 0)
		devid = bcmsdh_reg_read(sdh, si_enum_base(0), 4) & CID_ID_MASK;
...
dbus_sdio_probe_init(probe_sdh_info_t *pinfo)
{
...
#ifdef BCMDBG
	/* Return the window to backplane enumeration space for core access */
	if (bcmsdhsdio_set_sbaddr_window(sdh, si_enum_base(0), TRUE)) {
		DBUSERR(("%s: FAILED to return to SI_ENUM_BASE\n", __FUNCTION__));
		goto fail;
	}
	...
	if (CHIPTYPE(pinfo->sih->socitype) != SOCI_AI) {
		printf("F1 signature read @0x18000ffc=0x%4x\n",
			bcmsdh_reg_read(sdh, si_enum_base(0) + 0xffc, 4));
	}

	printf("F1 signature read @0x18000000=0x%4x\n",
	       bcmsdh_reg_read(sdh, si_enum_base(0), 4));
