/** @file dbus_sdio.c
 *
 * Dongle BUS interface. In practice, DBUS is not used for the SDIO interface, so the code below can
 * be considered inactive (Jan 2014).
 *
...
#define MAX_CTRL_BUF (12 * 1024)

#define DBUS_SDIO_DNGL_MEM_SIZE	(300 * 1024)
...
#undef MAX_HDR_READ
#define MAX_HDR_READ	8
#define MAX_RX_DATASZ	2048

/* Maximum milliseconds to wait for F2 to come up */
#define DHD_WAIT_F2RDY	4000

#define DHD_WAIT_SDIO_RDY	10
...
typedef struct {
	...
	uint		rxctl_len; /* Length of valid data in ctrl buf */
	uint		rxctl_doff; /* Data offset of the ctrl buf */
	...
	uint	f2ctrldata;	/* Number of f2 control frame data reads */
	uint	f2ctrlpkts;	/* Number of f2 control pkts received */
...
extern int dbus_sdos_pnp(void *bus, int event);
 ...
 */
static void *dbus_sdio_attach(dbus_pub_t *pub, void *cbarg, dbus_intf_callbacks_t *cbs);
static void dbus_sdio_detach(dbus_pub_t *pub, void *info);
static int dbus_sdio_send_irb(void *bus, dbus_irb_tx_t *txirb);
static int dbus_sdio_send_ctl(void *bus, uint8 *buf, int len);
static int dbus_sdio_recv_ctl(void *bus, uint8 *buf, int len);
static int dbus_sdio_up(void *bus);
#ifdef BCMDBG
static void dbus_sdio_dump(void *bus, struct bcmstrbuf *b);
#endif
static bool dbus_sdio_device_exists(void *bus);
static bool dbus_sdio_dlneeded(void *bus);
static int dbus_sdio_dlstart(void *bus, uint8 *fw, int len);
static int dbus_sdio_dlrun(void *bus);
static int dbus_sdio_down(void *bus);
static int dbus_sdio_get_attrib(void *bus, dbus_attrib_t *attrib);
...
dbus_sdio_probe_cb(void *handle, const char *desc, uint32 bustype, uint32 hdrlen)
{
	...
	if (probe_cb) {
		...
		if (g_dbusintf != NULL) {
			 ...
			 */
			dbus_sdio_intf.attach = dbus_sdio_attach;
			dbus_sdio_intf.detach = dbus_sdio_detach;
			dbus_sdio_intf.send_irb = dbus_sdio_send_irb;
			/* SDIO does not need pre-submitted IRBs like USB
			 * so set recv_irb() to NULL so dbus.c would not call
			 * this function.
			 */
			dbus_sdio_intf.send_ctl = dbus_sdio_send_ctl;
			dbus_sdio_intf.recv_ctl = dbus_sdio_recv_ctl;
			dbus_sdio_intf.up = dbus_sdio_up;
#ifdef BCMDBG
			dbus_sdio_intf.dump = dbus_sdio_dump;
#endif
			dbus_sdio_intf.device_exists = dbus_sdio_device_exists;
			dbus_sdio_intf.dlneeded = dbus_sdio_dlneeded;
			dbus_sdio_intf.dlstart = dbus_sdio_dlstart;
			dbus_sdio_intf.dlrun = dbus_sdio_dlrun;
			dbus_sdio_intf.down = dbus_sdio_down;
			dbus_sdio_intf.get_attrib = dbus_sdio_get_attrib;
		}
...
dbus_bus_fw_get(void *bus, uint8 **fw, int *fwlen, int *decomp)
{
	...
	*fw = sdio_info->dlarray;
	*fwlen = sdio_info->dlarray_size;
}
...
dbus_sdio_probe_device(sdio_info_t *sdio_info)
{
	...
	if (sdio_info->devid == 0) {
		if (dbus_sdos_reg_read(sdio_info->sdos_info,
			SI_ENUM_BASE(sdio_info->sih), 4, &sdio_info->devid) == BCME_OK)
...
dbus_sdio_device_init(sdio_info_t *sdio_info)
{
	...
	if (!(sdio_info->sih = si_attach(sdio_info->devid, sdio_info->pub->osh,
		(void *)si_enum_base(0), SD_BUSTYPE, sdio_info->sdos_info,
...
void
dbus_sdio_detach(dbus_pub_t *pub, void *info)
{
	...
	if (sdio_info->rxbuf) {
		MFREE(osh, sdio_info->rxbuf, MAX_CTRL_BUF);
		sdio_info->rxctl = sdio_info->rxbuf = NULL;
		sdio_info->rxctl_len = 0;
	}
...
void *
dbus_sdio_attach(dbus_pub_t *pub, void *cbarg, dbus_intf_callbacks_t *cbs)
{
	int32 fnum;
	sdio_info_t *sdio_info;
	...
	if (!(sdio_info->rxbuf = MALLOC(pub->osh, MAX_CTRL_BUF))) {
		DBUSERR(("%s: MALLOC of %d-byte rxbuf failed\n",
			__FUNCTION__, MAX_CTRL_BUF));
		goto err;
	}

	sdio_info->rxctl = sdio_info->rxbuf;
	if (dhd_alignctl) {
		sdio_info->rxctl += MAX_HDR_READ;
		if ((uintptr)sdio_info->rxctl % SDALIGN)
			sdio_info->rxctl += (SDALIGN - ((uintptr)sdio_info->rxctl % SDALIGN));
		sdio_info->rxctl -= MAX_HDR_READ;
	}
...
err:
	dbus_sdio_detach(pub, sdio_info->sdos_info);
	return NULL;

}

static bool
dbus_sdio_device_exists(void *bus)
{
	sdio_info_t *sdio_info = BUS_INFO(bus, sdio_info_t);
	if (sdio_info->pub->busstate == DBUS_STATE_SLEEP)
		dbus_sdos_pnp(sdio_info->sdos_info, DBUS_PNP_RESUME);
	if (!dbus_sdio_probe_device(sdio_info))
		return FALSE;
...
dbus_sdio_init(sdio_info_t *sdio_info)
{
	...
	OSL_DELAY(DHD_WAIT_SDIO_RDY * 1000);
exit:
	return ret;
}
static int
...
dbus_sdio_write_vars(sdio_info_t *sdio_info)
{
	...
	char *varbuf;
	uint32 varsizew;
	ALIGNED_LOCAL_VARIABLE(avar, SDALIGN)
	...
	vbuffer = (char*)MALLOC(sdio_info->pub->osh, (varsize + SDALIGN));
	if (!vbuffer)
		return BCME_NOMEM;

	varbuf = (char *)ALIGN_ADDR(vbuffer, SDALIGN);
	bzero(varbuf, varsize);
	bcopy(sdio_info->vars, varbuf, sdio_info->varsz);

	/* Write the vars list */
	bcmerror = dbus_sdio_membytes(sdio_info, TRUE, varaddr, varbuf, varsize);

	MFREE(sdio_info->pub->osh, vbuffer, (varsize + SDALIGN));
	...
	if (bcmerror) {
		*((uint32 *)avar) = 0;
		...
		dbus_sdio_membytes(sdio_info, TRUE,
			(sdio_info->orig_ramsize - 4), (uint8*)avar, 4);
	} else {
...
dbus_sdio_download_state(sdio_info_t *sdio_info, bool enter)
{
	...
	if (enter) {
		...
		if (sdio_info->ramsize) {
			ALIGNED_LOCAL_VARIABLE(zeros, SDALIGN)
			*((uint32 *)zeros) = 0;
			dbus_sdio_membytes(sdio_info, TRUE,
...
dbus_sdio_download_image_array(sdio_info_t *sdio_info, uint8 *fw, int len)
{
	...
	buf =  (uint8 *)MALLOC(sdio_info->pub->osh, MEMBLOCK + SDALIGN);
	abuf = (uint8 *)ALIGN_ADDR(buf, SDALIGN);

	bzero(abuf, MEMBLOCK);

	while (offset < DBUS_SDIO_DNGL_MEM_SIZE) {
		bcmerror = dbus_sdio_membytes(sdio_info, TRUE, offset, abuf, MEMBLOCK);
		if (bcmerror) {
...
err:
	MFREE(sdio_info->pub->osh, buf, MEMBLOCK + SDALIGN);
...
static void
dbus_sdio_dump(void *bus, struct bcmstrbuf *b)
{
	int bcmerror = -1;
	sdio_info_t *sdio_info = BUS_INFO(bus, sdio_info_t);
	ALIGNED_LOCAL_VARIABLE(addr, SDALIGN)
	uint32 offset;
	uint32 length, index, i;
	uint8 *buf;
	uint8 *tmp_buf[256];
	sdio_params *sparams;
	struct pktq *txq;

	bcmerror = dbus_sdio_membytes(sdio_info, FALSE, sdio_info->active_cons, addr, 4);
	if (bcmerror) {
		DBUSERR(("%s: error %d on reading %d membytes at 0x%08x\n",
			__FUNCTION__, bcmerror, 4, sdio_info->active_cons));
		bcmerror = dbus_sdos_abort(sdio_info->sdos_info, SDIO_FUNC_1);
		if (bcmerror) {
			DBUSERR(("%s: error %d on sending abort\n",
				__FUNCTION__, bcmerror));
		}
	}

	offset = ltoh32(*((uint32 *)addr)) + FIELD_OFFSET(hnd_cons_t, log);

	bcmerror = dbus_sdio_membytes(sdio_info, FALSE, offset, addr, 12);
	if (bcmerror) {
		DBUSERR(("%s: error %d on reading %d membytes at 0x%08x\n",
			__FUNCTION__, bcmerror, 8, offset));
		bcmerror = dbus_sdos_abort(sdio_info->sdos_info, SDIO_FUNC_1);
		if (bcmerror) {
			DBUSERR(("%s: error %d on sending abort\n",
				__FUNCTION__, bcmerror));
	...
	offset = ltoh32(*((uint32 *)addr));
	length = ltoh32(*((uint32 *)&addr[4]));
	index = ltoh32(*((uint32 *)&addr[8]));

	bcm_bprintf(b, "Dongle Log index = %d\n", index);

	buf = (uint8 *)(((uintptr)b->buf + SDALIGN) & ~(SDALIGN - 1));

	if ((uintptr)buf != (uintptr)b->buf) {
		for (i = 0; i < (((uintptr)buf) - (uintptr)b->buf); i++)
			b->buf[i] = '\n';
		b->size -= i;
	}

	bcmerror = dbus_sdio_membytes(sdio_info, FALSE, offset, buf, length);
	if (bcmerror) {
		DBUSERR(("%s: error %d on reading %d membytes at 0x%08x\n",
			__FUNCTION__, bcmerror, length, offset));
		bcmerror = dbus_sdos_abort(sdio_info->sdos_info, SDIO_FUNC_1);
		if (bcmerror) {
			DBUSERR(("%s: error %d on sending abort\n",
				__FUNCTION__, bcmerror));
	...
	sparams = (sdio_params *)
		(((uintptr)tmp_buf + SDALIGN) & ~(SDALIGN - 1));

	bcmerror = dbus_sdio_membytes(sdio_info, FALSE, sdio_info->debug_params,
		(uint8 *)sparams, sizeof(sdio_params));
	if (bcmerror) {
		DBUSERR(("%s: error %d on reading %d membytes at 0x%08x\n",
			__FUNCTION__, bcmerror, sizeof(sdio_params), sdio_info->debug_params));
		bcmerror = dbus_sdos_abort(sdio_info->sdos_info, SDIO_FUNC_1);
		if (bcmerror) {
			DBUSERR(("%s: error %d on sending abort\n",
				__FUNCTION__, bcmerror));
	...
	bcmerror = dbus_sdio_membytes(sdio_info, FALSE, ltoh32(sparams->tx_seq), addr, 4);
	if (bcmerror) {
		DBUSERR(("%s: error %d on reading %d membytes at 0x%08x\n",
			__FUNCTION__, bcmerror, 1, (uintptr)sparams->tx_seq));
		bcmerror = dbus_sdos_abort(sdio_info->sdos_info, SDIO_FUNC_1);
		if (bcmerror) {
			DBUSERR(("%s: error %d on sending abort\n",
				__FUNCTION__, bcmerror));
	...
	bcm_bprintf(b, "tx_seq = %d\n", ((uint8 *)addr)[0]);

	bcm_bprintf(b, "rx_seq = %d\n", ((uint8 *)addr)[1]);

	bcm_bprintf(b, "rx_lim = %d\n", ((uint8 *)addr)[2]);

	bcm_bprintf(b, "rx_hlim = %d\n", ((uint8 *)addr)[3]);

	bcmerror = dbus_sdio_membytes(sdio_info, FALSE, ltoh32(sparams->txavail), addr, 4);
	if (bcmerror) {
		DBUSERR(("%s: error %d on reading %d membytes at 0x%08x\n",
			__FUNCTION__, bcmerror, 4, (uintptr)sparams->txavail));
		bcmerror = dbus_sdos_abort(sdio_info->sdos_info, SDIO_FUNC_1);
		if (bcmerror) {
			DBUSERR(("%s: error %d on sending abort\n",
				__FUNCTION__, bcmerror));
	...
	bcm_bprintf(b, "txavail = %d\n", ltoh32(*((uint32 *)addr)));

	bcmerror = dbus_sdio_membytes(sdio_info, FALSE, ltoh32(sparams->txq), addr, 40);
	if (bcmerror) {
		DBUSERR(("%s: error %d on reading %d membytes at 0x%08x\n",
			__FUNCTION__, bcmerror, 40, (uintptr)sparams->txq));
		bcmerror = dbus_sdos_abort(sdio_info->sdos_info, SDIO_FUNC_1);
		if (bcmerror) {
			DBUSERR(("%s: error %d on sending abort\n",
				__FUNCTION__, bcmerror));
	...
	bcm_bprintf(b, "txq length = %d\n", ltoh16(*((uint16 *)&addr[8])));

	bcm_bprintf(b,
		"Host txq length = %d, flowcontrol = %d, tx_seq = %d, tx_max = %d, rx_seq = %d\n",
		sdio_info->txq.len, sdio_info->flowcontrol, sdio_info->tx_seq,
		sdio_info->tx_max, sdio_info->rx_seq);
...
static int
dbus_sdio_dlstart(void *bus, uint8 *fw, int len)
{
	sdio_info_t *sdio_info = BUS_INFO(bus, sdio_info_t);
	int err = DBUS_ERR;
...
static int
dbus_sdio_get_attrib(void *bus, dbus_attrib_t *attrib)
{
	sdio_info_t *sdio_info = BUS_INFO(bus, sdio_info_t);
...
dbus_sdio_txpkt(sdio_info_t *sdio_info, void *pkt, uint chan)
{
	...
	*(((uint16*)frame) + 1) = htol16(~len);
	...
	do {
		ret = dbus_sdos_send_buf(sdio_info->sdos_info, SI_ENUM_BASE(sdio_info->sih),
 ...
 */
static int
dbus_sdio_send_ctl(void *bus, uint8 *buf, int len)
{
	sdio_info_t *sdio_info = BUS_INFO(bus, sdio_info_t);
	int err = DBUS_OK;
...
static int
dbus_sdio_send_irb(void *bus, dbus_irb_tx_t *txirb)
{
	sdio_info_t *sdio_info = BUS_INFO(bus, sdio_info_t);
	int err;
	...
	if (sdio_info->glom) {
		...
		if (usechain) {
			errcode = dbus_sdos_recv_buf(sdio_info->sdos_info,
				SI_ENUM_BASE(sdio_info->sih), SDIO_FUNC_2, F2SYNC,
				(uint8*)PKTDATA(osh, pfirst), dlen);
		} else if (sdio_info->dataptr) {
			errcode = dbus_sdos_recv_buf(sdio_info->sdos_info,
				SI_ENUM_BASE(sdio_info->sih), SDIO_FUNC_2,
...
dbus_sdio_readframes(sdio_info_t *sdio_info, bool *finished)
{
	...
	uint16 maxlen;		/* Total number of bytes to read */
	uint8 *rxbuf;
	...
	sdret = dbus_sdos_recv_buf(sdio_info->sdos_info, SI_ENUM_BASE(sdio_info->sih),
		SDIO_FUNC_2, F2SYNC, sdio_info->rxctl, MAX_HDR_READ);
	sdio_info->f2rxhdrs++;
	...
	len = ltoh16_ua(sdio_info->rxctl);
	check = ltoh16_ua(sdio_info->rxctl + sizeof(uint16));
	...
	if (len < SDPCM_HDRLEN) {
		...
		DBUSERR(("%s : mismatch, len %d rnd %d; "
			"expected rxseq %d\n", __FUNCTION__, len,
			ROUNDUP(len, 16), sdio_info->rx_seq));
		return dbus_sdio_rxfail(sdio_info, TRUE, TRUE);
	}

	if ((chan = SDPCM_PACKET_CHANNEL(&sdio_info->rxctl[SDPCM_FRAMETAG_LEN])) ==
		...
		if (!(pkt = dbus_sdio_pktget(sdio_info, PKTBUFSZ, FALSE))) {
			...
			bcopy(sdio_info->rxctl, rxbuf, MAX_HDR_READ);

			maxlen = MAX_RX_DATASZ;
		}
	}

	/* Length to read */
	rdlen = len - MAX_HDR_READ;
	...
	sdret = dbus_sdos_recv_buf(sdio_info->sdos_info, SI_ENUM_BASE(sdio_info->sih),
		SDIO_FUNC_2, F2SYNC, &rxbuf[MAX_HDR_READ], rdlen);
	sdio_info->f2rxdata++;
	ASSERT(sdret != BCME_PENDING);
	...
	seq = SDPCM_PACKET_SEQUENCE(&rxbuf[SDPCM_FRAMETAG_LEN]);
	doff = SDPCM_DOFFSET_VALUE(&rxbuf[SDPCM_FRAMETAG_LEN]);
	txmax = SDPCM_WINDOW_VALUE(&rxbuf[SDPCM_FRAMETAG_LEN]);
	...
	/* Handle Flow Control */
	fcbits = SDPCM_FCMASK_VALUE(&rxbuf[SDPCM_FRAMETAG_LEN]);
	...
	if (chan == SDPCM_CONTROL_CHANNEL) {
		sdio_info->f2ctrldata++;	/* Number of f2 frame writes */
		sdio_info->f2ctrlpkts++;

		sdio_info->rxctl_len = len - doff;
		sdio_info->rxctl_doff = doff;
		...
		if (chan == SDPCM_GLOM_CHANNEL) {
			if (SDPCM_GLOMDESC(&rxbuf[SDPCM_FRAMETAG_LEN])) {
...
dbus_sdio_rxctl(sdio_info_t *sdio_info, uchar *msg, uint msglen)
{
	 ...
	 */
	rxlen = sdio_info->rxctl_len;
	bcopy(&sdio_info->rxctl[sdio_info->rxctl_doff], msg, MIN(msglen, rxlen));
	sdio_info->rxctl_len = 0;
...
static int
dbus_sdio_recv_ctl(void *bus, uint8 *buf, int len)
{
	sdio_info_t *sdio_info = BUS_INFO(bus, sdio_info_t);
