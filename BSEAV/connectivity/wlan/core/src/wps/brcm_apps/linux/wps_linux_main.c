...
wps_osl_build_conf()
{
	...
	for (i = 0; i < 256; i ++) {
		...
		foreach(name, ifnames, next) {
			...
			if (value == NULL || strcmp(value, "disabled") == 0)
				continue;

			/* check "wlx.y_wps_mode_override" addtionally
			 * for temperally wps configuration
			 */
			value = nvram_get(strcat_r(prefix, "wps_mode_override", tmp));
			if (value && !strcmp(value, "disabled"))
				continue;
