...
static char g_Eapol_Buf[WPS_EAP_DATA_MAX_LENGTH];
...
registration_loop(unsigned long start_time)
{
	...
	while (1) {
		...
		retVal = wait_for_eapol_packet(g_Eapol_Buf, &len, WPS_EAP_READ_DATA_TIMEOUT);

		if (retVal == WPS_SUCCESS) {
			/* Show receive message */
			msg_type = wps_get_msg_type(g_Eapol_Buf, len);
			DBGPRINT(("Receive EAP-Request%s\n", wps_get_msg_string((int)msg_type)));

			/* process ap message */
			retVal = wps_process_ap_msg(g_Eapol_Buf, len);
...
enroll_device(int mode, uint8 *bssid, char *ssid, char *pin)
{
	...
	if (wps_osl_init((char *)bssid_ptr) != WPS_SUCCESS) {
		DBGPRINT(("Initializing OSL failed. \n"));
	...
	DBGPRINT(("Starting WPS enrollment. Start time is %u\n", start_time));
	...
	while (1) {
		...
		*/
		 ...
		 */
			...
			/* We allow 3 attemps of protocol registration,
			 * so the maximum process time
			 * is 6 minutes as each protocol timeout is 2 minutes
			*/
			if (nRegAttempts > 3) {
...
static void get_random_credential(WpsEnrCred *credential)
{
	/* ssid */
	uint8 mac[6] = {0};
	unsigned short ssid_length = 0, key_length = 0;
	...
	wps_osl_get_mac(mac);
	sprintf(macString, "%02x:%02x:%02x:%02x:%02x:%02x", mac[0], mac[1], mac[2],
	                                                    mac[3], mac[4], mac[5]);
...
static int get_pbc_ap(wps_ap_list_info_t *list_inout, int count, int *nAP)
{
	...
	*nAP = 0;
	...
	{
		/* Search the wps ap list and set the pbc ap (only one is allowed currently) to the
		 *first one in this given ap list
		*/
		while (i < count)
		{
			if (stricmp((char*)list_inout[i].ssid, ssid) == 0)
			{
...
/*
	...
	be called repeatidly with small timeouts in seconds (say 4 or 5 secs) to
	allow for UI updates and user
	cancelation. If multiple PBC APs are found, this is an error condition
	and FALSE is returned.
	nAP will contain the number of PBC APs found (will be greater than 1).

	The value of *nAP is updated with the number of APs found. For PBC APs,
	it will be always 1 on success (or
...
bool wps_findAP(int *nAP, int mode, int timeout)
{
	...
	*nAP = 0;
	...
	while ((start_time+timeout) > get_current_time())
	{
		...
		if (wpsaplist)
		{
			/* After this call, the first wps_ap_total elements in
			 *the ap_list are wps ones
			*/
			...
			if (wps_ap_total > 0)
			{
				if (mode == STA_ENR_JOIN_NW_PBC)
				{
					...
					if (get_pbc_ap(wpsaplist, wps_ap_total, nAP) !=
					           PBC_NOT_FOUND)
						break;
				}
				else
...
/*
	...
	that will be called periodically by the WPS API. When called, this
	callback function will be provided with the current status.
	If the calling process wants to cancel the WPS protocol, it
	should return FALSE (upon the user pressing a Cancel button, for example).

	If the calling process does not want to be called back,
	it should send NULL as a function pointer.

	GUI applications should use the asynchronous version of this function
	so as not to block or slow down a UI's message loop.
*/

bool wps_get_AP_info(int wps_mode, uint8 *bssid, char *ssid, char *pin,
                               wps_credentials *credentials)
{
	bool bRet = false;
	WpsEnrCred cred;
...
/*
	This function creates a preferred network profile that can be used
	by Windows Zero Config (WZC)
	to connect to the network. Call this function with the results
	of the last WPS exchange.

	This function will return WPS_STATUS_ERROR if WZC does not control the adapter or if
	the creation of profile failed.

*/

bool wps_create_profile(const wps_credentials *credentials)
{
	DBGPRINT(("wps_create_profile API not supported by upper stack\n"));
	return false;
}
...
bool wps_configureAP(uint8 *bssid, const char *pin, const wps_credentials *credentials)
{
	...
	else
	{
		...
		strncpy(credNew.nwKey, credentials->nwKey, SIZE_64_BYTES);
