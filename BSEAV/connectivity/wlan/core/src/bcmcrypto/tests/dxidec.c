...
void usage(const char* p)
{
	...
	fprintf(stderr, "		tkip ; key is 64 hex chars; tk.amick.smick\n");
	...
	uchar* data, uint data_len, uchar* key, uint ksz)
{
	...
	rc = aes_ccm_encrypt(rk, ksz, nonce, aad_len, aad, edata_len, edata, 
						 edata, &edata[edata_len], DOT11_ICV_AES_LEN);
	return !rc;
}
	...
	uchar* data, uint data_len, uchar* key, uint ksz)
{
	...
	edata_len = data_len - DOT11_IV_AES_CCM_LEN;
	rijndaelKeySetupEnc(rk, key, AES_KEY_BITLEN(ksz));
	rc = aes_ccm_decrypt(rk, ksz, nonce, aad_len, aad,
		edata_len, edata, edata, DOT11_ICV_AES_LEN);
	return !rc;
}
