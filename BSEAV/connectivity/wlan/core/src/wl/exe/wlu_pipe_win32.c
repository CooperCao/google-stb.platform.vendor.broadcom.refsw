...
{
	int SockDes;
	if ((SockDes = (int)socket(AddrFamily, Type, Protocol)) == INVALID_SOCKET) {
...
rwl_acceptconnection(int SocketDes, struct sockaddr* ClientAddr, int *SizeOfAddr)
{
	...
	if ((NewSockDes = (int)accept(SocketDes, ClientAddr, SizeOfAddr)) == SOCKET_ERROR) {
...
#if defined(RWL_SERIAL) || defined(RWL_DONGLE)
static HANDLE
rwl_open_serial(char *port, int ReadTotalTimeout, int debug, int baud)
{
	DCB dcb;
...
#ifdef RWL_SERIAL
	case REMOTE_SERIAL:
		if ((irh = rwl_open_serial(port_str, ReadTotalTimeout, debug,
...
#ifdef RWL_DONGLE
	case REMOTE_DONGLE:
		if ((irh = rwl_open_serial(g_rwl_device_name_serial, 0, 0,
				g_rwl_device_baud)) == NULL) {
