...
void
wlc_set_icmp_offload(wlc_info_t *wlc, wl_icmp_ip_cfg_t *host_ip)
{
	int32 i;
	uint32 icmp_enable = 1, mem_needed;
	wl_icmp_ipv6_cfg_t *cfg;

	/*  Clear out any stored addresses */
	wlc_clear_icmp_offload(wlc);

	/* Program the host addresses and enable */
	for (i = 0; i < host_ip->num_ipv4; i++) {
#ifdef BCMDBG
		uint8 *v4 = host_ip->ipv4[i].addr;

		WL_ERROR(("%s: Adding v4[%d]: %d.%d.%d.%d\n", __FUNCTION__, i,
			v4[0], v4[1], v4[2], v4[3]));
#endif /* BCMDBG */
		wlc_iovar_op(wlc, "icmp_hostip",
			&icmp_enable, sizeof(icmp_enable), host_ip->ipv4[i].addr,
			IPV4_ADDR_LEN, IOV_SET, NULL);
	}
	mem_needed = WL_ICMP_CFG_IPV6_LEN(host_ip->num_ipv6);
	cfg = (wl_icmp_ipv6_cfg_t *) MALLOC(wlc->osh, mem_needed);
	if (!cfg) {
		WL_ERROR(("%s: Cannot malloc %d bytes\n", __FUNCTION__, mem_needed));
		return;
	}
#ifdef BCMDBG
	for (i = 0; i < host_ip->num_ipv6; i++) {
		uint16 *v6 = (uint16 *)host_ip->ipv6[i].addr;
		int z;
	...
	cfg->version = WL_ICMP_IPV6_CFG_VERSION;
	cfg->fixed_length = WL_ICMP_CFG_IPV6_FIXED_LEN;
	cfg->length = mem_needed;
	cfg->flags = 0;
	cfg->num_ipv6 = host_ip->num_ipv6;
	memcpy(cfg->host_ipv6, host_ip->ipv6,
		host_ip->num_ipv6 * sizeof(struct ipv6_addr));
	wlc_iovar_op(wlc, "icmp_hostipv6",
		NULL, 0, cfg,
		mem_needed, IOV_SET, NULL);
	MFREE(wlc->osh, cfg, mem_needed);
	return;
}

void
wlc_clear_icmp_offload(wlc_info_t *wlc)
{
	uint32 icmp_enable = 0;
	uint8 null_addr[IPV4_ADDR_LEN];
	wl_icmp_ipv6_cfg_t cfg;

	memset(null_addr, 0, sizeof(null_addr));
	/* No ipv6 addresses need to be sent to clear */
	memset(&cfg, 0, sizeof(wl_icmp_ipv6_cfg_t));
	cfg.version = WL_ICMP_IPV6_CFG_VERSION;
	cfg.flags = WL_ICMP_IPV6_CLEAR_ALL;

	wlc_iovar_op(wlc, "icmp_hostip",
		&icmp_enable, sizeof(icmp_enable),
		null_addr, IPV4_ADDR_LEN, IOV_SET, NULL);
	wlc_iovar_op(wlc, "icmp_hostipv6",
		NULL, 0, &cfg, sizeof(wl_icmp_ipv6_cfg_t), IOV_SET, NULL);
	return;
}
