...
BCMATTACHFN(bwte_gpio_init)(bwte_info_t *bwte_info, si_t *sih)
{
	...
	if (BCM4349_CHIP(sih->chipid)) {
		fnsel = CC4349_FNSEL_SAMEASPIN;
...
BCMATTACHFN(wlc_bwte_attach)(wlc_info_t* wlc)
{
	...
	WL_TRACE(("sb(%p), b_g(%p), w_g(%p), b2w_i(%p), w2b_i(%p)\n",
		...
		OSL_OBFUSCATE_BUF(&bwte_info->stats->bt2wlan_intrcnt),
		OSL_OBFUSCATE_BUF(&bwte_info->stats->wlan2bt_intrcnt)));
