...
#define WLC_TBOW_CFG_MATCH(tbow_info, bsscfg)	((tbow_info)->p2p_cfg == bsscfg)
...
struct tbow_info {
	...
	tbow_datachan_state_t *datachan_state;

	struct wl_timer *teardown_timer;
	wlc_bsscfg_t *p2p_cfg;
	bool is_iovar_triggered;
};
...
typedef struct tbow_ho_setupmsg {
	uint8 type; /* first byte is control msgid */
	uint8 version;
	uint8 opmode;
...
bool tbow_is_supported(tbow_info_t *tbow_info);
void tbow_send_wlan_status(tbow_info_t *tbow_info, uint8 status);
