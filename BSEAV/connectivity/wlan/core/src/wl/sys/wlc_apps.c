...
wlc_apps_ps_flush(wlc_info_t *wlc, struct scb *scb)
{
	...
	WL_PS(("wl%d.%d: wlc_apps_ps_flush: flushing %d packets for %s aid %d\n",
	       wlc->pub->unit, WLC_BSSCFG_IDX(scb->bsscfg), pktq_n_pkts_tot(&scb_psinfo->psq),
	       bcm_ether_ntoa(&scb->ea, eabuf), AID2PVBMAP(scb->aid)));

	pktq_init(&tmp_q, 1, PSQ_PKTQ_LEN_DEFAULT);
	/* Move all packets in SCB PS queue to tmp_q. This is being done to ensure we don`t run
	 * into a race condition where SCB free happens as part of Packet Free callback
	 *
