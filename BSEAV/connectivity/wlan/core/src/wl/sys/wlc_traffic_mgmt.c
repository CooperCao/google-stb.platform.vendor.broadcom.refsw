	...
	if (trf_mgmt_rssi_needed(tm)) {
		...
		FOREACHSCB(wlc->scbstate, &scbiter, scb) {
			...
			if ((rssi_worst > rssi) && (rssi > -100)) {
				...
				*/
				selected_scb = psta_prim ? psta_prim : scb;
			}
			scb_count++;
		}
...
trf_mgmt_do_rssi_processing(wlc_trf_mgmt_ctxt_t *tm, struct scb *scb, void *pkt)
{
	 ...
	 */
	if (trf_mgmt_rssi_needed(tm) &&
	    (tm->rssi_info.rssi_scb == (psta_prim ? psta_prim : scb))) {
		...
		case TRF_MGMT_RSSI_DOWNGRADE_LOWEST:
			...
			WL_RSSI(("%s: Packet for scb %p/%p (RSSI %d) downgraded to BE\n",
			         __FUNCTION__, OSL_OBFUSCATE_BUF(scb), OSL_OBFUSCATE_BUF(psta_prim),
...
/* BSSCFG cubby */
static int
trf_mgmt_bss_init(void *ctx, wlc_bsscfg_t *cfg)
{
	wlc_trf_mgmt_ctxt_t *trf_mgmt_ctxt = (wlc_trf_mgmt_ctxt_t *)ctx;
	 ...
	 */
	if (TRAFFIC_MGMT_ENAB(trf_mgmt_ctxt->wlc->pub))
		return wlc_trf_mgmt_bsscfg_allocate(trf_mgmt_ctxt, cfg);

	return BCME_OK;
}

static void
trf_mgmt_bss_deinit(void *ctx, wlc_bsscfg_t *cfg)
{
	wlc_trf_mgmt_ctxt_t *trf_mgmt_ctxt = (wlc_trf_mgmt_ctxt_t *)ctx;
	 ...
	 */
	if (TRAFFIC_MGMT_ENAB(trf_mgmt_ctxt->wlc->pub))
		wlc_trf_mgmt_bsscfg_free(trf_mgmt_ctxt, cfg);
}

static int
wlc_trf_mgmt_dump_clr(void *ctx)
{
	wlc_trf_mgmt_ctxt_t *trf_mgmt_ctxt = ctx;
	wlc_bsscfg_t *cfg;
	int idx;
	int i;

	FOREACH_BSS(trf_mgmt_ctxt->wlc, idx, cfg) {
		if (cfg->trf_mgmt_info == NULL)
			continue;
		for (i = 0; i < TRF_MGMT_MAX_PRIORITIES; i++) {
			bzero(&cfg->trf_mgmt_info->tx_queue[i].stats, sizeof(trf_mgmt_stats_t));
			bzero(&cfg->trf_mgmt_info->rx_queue[i].stats, sizeof(trf_mgmt_stats_t));
		}
	}
...
BCMATTACHFN(wlc_trf_mgmt_attach)(wlc_info_t *wlc)
{
	...
	if (wlc_bsscfg_cubby_reserve(wlc, 0, trf_mgmt_bss_init, trf_mgmt_bss_deinit, NULL,
	                             trf_mgmt_ctxt) < 0) {
		wlc_module_unregister(wlc->pub, "trf_mgmt", trf_mgmt_ctxt);
...
wlc_trf_mgmt_bsscfg_down(wlc_trf_mgmt_ctxt_t *trf_mgmt_ctxt, wlc_bsscfg_t *bsscfg)
{
	...
	if (trf_mgmt_ctxt->rssi_info.rssi_policy == TRF_MGMT_RSSI_NONE) {
	    ...
	    WL_RSSI(("%s: No RSSI policy, unconfiguring txmod for SCBs in BSS %p\n",
	        __FUNCTION__, OSL_OBFUSCATE_BUF(trf_mgmt_info->bsscfg)));
...
wlc_trf_mgmt_txpktcnt(void *context)
{
	...
	FOREACH_BSS(trf_mgmt_ctxt->wlc, i, bsscfg) {
		...
		for (j = 0; j < TRF_MGMT_MAX_PRIORITIES; j++) {
		    ...
		    pktcnt  += spktq_n_pkts(&tx_queue->pkt_queue);
		}
	    }
	}
#endif  /* TRAFFIC_SHAPING */
...
wlc_trf_mgmt_allocate_info_block(wlc_trf_mgmt_ctxt_t *trf_mgmt_ctxt, wlc_bsscfg_t *bsscfg)
{
	...
	for (i = 0; i < TRF_MGMT_MAX_PRIORITIES; i++) {
	    spktqinit(&trf_mgmt_info->tx_queue[i].pkt_queue, MAX_TX_QUEUE_LEN);
	    spktqinit(&trf_mgmt_info->rx_queue[i].pkt_queue, MAX_RX_QUEUE_LEN);
	}
	...
	uint32                  cmd_length)
{
	...
	if (cmd_length <
		...
		WL_ERROR(("wl%d: %s: Invalid cmd_length %d, buffer size %d \n",
			...
			(uint)(OFFSETOF(trf_mgmt_filter_list_t, filter) +
			trf_mgmt_filter_list->num_filters * sizeof(trf_mgmt_filter_t))));
		return BCME_ERROR;
	}
...
queue_it:
	 ...
	 */
	 if (spktq_full(&queue->pkt_queue)) {
	     wlc_trf_mgmt_handle_full_queue(trf_mgmt_info, queue);
	}

	spktenq(&queue->pkt_queue, pkt);
...
wlc_trf_mgmt_flush_tx_queue(wlc_trf_mgmt_info_t *trf_mgmt_info, bool free_pkts)
{
	...
	for (i = trf_mgmt_priority_high; i >= trf_mgmt_priority_low; i--) {
	    ...
	    while (spktq_n_pkts(&tx_queue->pkt_queue) > 0) {
		pkt = spktdeq(&tx_queue->pkt_queue);
		ASSERT(pkt != NULL);
...
wlc_trf_mgmt_flush_rx_queue(wlc_trf_mgmt_info_t *trf_mgmt_info, bool free_pkts)
{
	...
	for (i = trf_mgmt_priority_high; i >= trf_mgmt_priority_low; i--) {
	    ...
	    while (spktq_n_pkts(&rx_queue->pkt_queue) > 0) {
		pkt = spktdeq(&rx_queue->pkt_queue);
		ASSERT(pkt != NULL);
...
wlc_trf_mgmt_handle_full_queue(wlc_trf_mgmt_info_t *trf_mgmt_info, trf_mgmt_queue_t *queue)
{
	...
	pkt     = spktq_peek(&queue->pkt_queue);
	pkt_len = pkttotlen(WLCOSH(trf_mgmt_info), pkt);
...
wlc_trf_mgmt_process_queue(wlc_trf_mgmt_info_t *trf_mgmt_info, trf_mgmt_queue_t *queue)
{
	...
	while (spktq_n_pkts(&queue->pkt_queue) > 0) {
	    pkt = spktq_peek(&queue->pkt_queue);
	    ASSERT(pkt != NULL);
...
void wlc_trf_mgmt_scb_txfail_detect(wlc_trf_mgmt_ctxt_t *trf_mgmt_ctxt, struct scb *scb)
{
	...
	wl_intfer_event_t intfer_event;
	uint8 *stats;
	...
	if ((intfer_params->txfail_thresh && (!is_tcp_stream) &&
		...
		memset(&intfer_event, 0, sizeof(intfer_event));
