...
static int wlc_rrm_doiovar(void *hdl, uint32 actionid, void *p, uint plen, void *a,
...
static const char BCMATTACHDATA(rstr_rrm)[] = "rrm";
wlc_rrm_info_t *
BCMATTACHFN(wlc_rrm_attach)(wlc_info_t *wlc)
{
	...
	/* register module */
	if (wlc_module_register(wlc->pub, rrm_iovars, rstr_rrm, rrm_info, wlc_rrm_doiovar,
	                        NULL, NULL, NULL)) {
	...
	wlc_rrm_req_t *req)
{
	...
	if ((rrm_info->framereq =
		(rrm_framereq_t *)MALLOC(wlc->osh, sizeof(rrm_framereq_t))) == NULL) {
		WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		req->flags |= DOT11_RMREP_MODE_REFUSED;
		return;
	...
	bzero(rrm_info->framereq, sizeof(rrm_framereq_t));
	...
	dot11_rmreq_chanload_t *rmreq_chload, wlc_rrm_req_t *req)
{
	...
	if ((rrm_info->chloadreq =
		(rrm_chloadreq_t *)MALLOC(wlc->osh, sizeof(rrm_chloadreq_t))) == NULL) {
		WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		req->flags |= DOT11_RMREP_MODE_REFUSED;
		return;
	...
	bzero(rrm_info->chloadreq, sizeof(rrm_chloadreq_t));
	...
	WL_RRM(("<802.11K Rx Req> %s (%zd bytes): token=%d reg=%d channel=0x%x dur=%d "
	...
	wlc_rrm_req_t *req)
{
	...
	if ((rrm_info->noisereq =
		(rrm_noisereq_t *)MALLOC(wlc->osh, sizeof(rrm_noisereq_t))) == NULL) {
		WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		req->flags |= DOT11_RMREP_MODE_REFUSED;
		return;
	...
	bzero(rrm_info->noisereq, sizeof(rrm_noisereq_t));
	...
	dot11_rmreq_tx_stream_t *rmreq_txstrm, wlc_rrm_req_t *req)
{
	...
	rrm_info->txstrmreq = (rrm_txstrmreq_t *)MALLOC(wlc->osh, sizeof(rrm_txstrmreq_t));
	...
	bzero(rrm_info->txstrmreq, sizeof(*rrm_info->txstrmreq));
...
wlc_rrm_rep_err(wlc_rrm_info_t *rrm_info, uint8 type, uint8 token, uint8 reason)
{
	...
	if ((p = wlc_frame_get_action(wlc, &rrm_info->da,
		...
		(DOT11_RM_ACTION_LEN + DOT11_RM_IE_LEN), &pbody,
		DOT11_ACTION_CAT_RRM)) != NULL) {
...
wlc_rrm_send_lmreq(wlc_rrm_info_t *rrm_info, wlc_bsscfg_t *cfg, struct ether_addr *da)
{
	...
	if ((p = wlc_frame_get_action(wlc, da,
		&cfg->cur_etheraddr, &cfg->BSSID, buflen, &pbody,
		DOT11_ACTION_CAT_RRM)) == NULL) {
		return;
	}
	...
	struct ether_addr *da, unsigned int buflen, uint8 **pbody)
{
	...
	if ((p = wlc_frame_get_action(wlc, da,
		&cfg->cur_etheraddr, &cfg->BSSID, buflen,
		pbody, DOT11_ACTION_CAT_RRM)) == NULL) {
		return NULL;
	}
	...
	int body_len)
{
	...
	if ((rrm_cfg = RRM_BSSCFG_CUBBY(rrm_info, cfg)) == NULL)
		return;
	...
	int body_len, int8 rssi, ratespec_t rspec)
{
	...
	p = wlc_frame_get_action(wlc, &scb->ea,
		&cfg->cur_etheraddr, &cfg->BSSID, DOT11_LMREP_LEN, &pbody, DOT11_ACTION_CAT_RRM);
	if (p == NULL) {
		WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		return;
...
wlc_rrm_start(wlc_info_t *wlc)
{
	...
	for (i = 0; i < rrm_state->req_count; i++) {
		...
		WL_ERROR(("RM REQ token 0x%02x (%2d), type %2d: %s, channel %3d, "
			"tsf 0x%x:%08x dur %4d TUs random interval: %4d TUs\n",
			req->token, req->token, req->type, name,
			CHSPEC_CHANNEL(req->chanspec), req->tsf_h,
			req->tsf_l, req->dur, req->intval));
	}
#endif /* BCMDBG */
