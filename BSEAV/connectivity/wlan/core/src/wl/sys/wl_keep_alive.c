...
BCMATTACHFN(wl_keep_alive_attach)(wlc_info_t *wlc)
{
	...
	if (RSDB_ENAB(wlc->pub)) {
		...
		wlc_bsscfg_cubby_reserve_ext(wlc, NULL, &cubby_params);
	}
#endif
	...
	for (i = 0; i < keepalive->ka_cmn->keep_alive_count; i++) {
		...
		if (indiv->bsscfg == bsscfg) {
			...
			period = indiv->override_period_msec ?
				...
				else if (!BSSCFG_IS_RSDB_CLONE(bsscfg) &&
						(!(oldstate & ASSOCIATED) && SCB_ASSOCIATED(scb)) &&
						period != 0) {
...
#ifdef BCMINTDBG
	case IOV_GVAL(IOV_KEEP_ALIVE):
		/* Check if the memory provided is sufficient */
		if (alen < (int)(WL_KEEP_ALIVE_FIXED_LEN + ka_cmn->mkeep_alive[0].len_bytes)) {
			err = BCME_BUFTOOSHORT;
			break;
		}

		keep_alive_pkt = a;
		bcopy(&ka_cmn->mkeep_alive[0].len_bytes,
		      &keep_alive_pkt->len_bytes,
		      sizeof(keep_alive_pkt->len_bytes));

		bcopy(&ka_cmn->mkeep_alive[0].period_msec,
		      &keep_alive_pkt->period_msec,
		      sizeof(keep_alive_pkt->period_msec));

		bcopy(ka_cmn->mkeep_alive[0].pkt_data,
		      keep_alive_pkt->data,
		      ka_cmn->mkeep_alive[0].len_bytes);
