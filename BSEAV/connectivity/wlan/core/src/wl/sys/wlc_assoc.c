...
#endif /* defined(BCMCCX) && defined(BCMINTSUP) */
#ifdef WLRCC
#include <wlc_okc.h>
#endif /* WLRCC */
...
wlc_cook_join_targets(wlc_bsscfg_t *cfg, bool for_roam, int cur_rssi)
{
	...
	if (for_roam &&
...
#ifdef BCMCCX
		} else if (CCX_ENAB(wlc->pub) && CCX_ASSOCIATION(wlc->ccx, cfg) &&
			wlc_bss_connected(cfg) && (current_bss->ccx_version >= 4) &&
			(wlc_ccx_get_orig_reason(wlc->ccx, cfg) == WLC_E_REASON_LOW_RSSI) &&
		...
		if ((roam->reason == WLC_E_REASON_TSPEC_REJECTED) ||
			...
			if (WLWNM_ENAB(wlc->pub)) {
				if (wlc_wnm_bsstrans_zero_assoc_bss_score(wlc->wnm_info, cfg)) {
					cur_pref_score = cur_pref_rssi = 0;
				}
			} else
#endif /* WLWNM */
	 ...
	 */
	if (CCX_ENAB(wlc->pub) && wlc->as->cmn->join_targets_last > 1) {
		uint32 max, threshold;
		max = join_pref[wlc->as->cmn->join_targets_last - 1].score;
		...
		for (i = 0; i < (int)wlc->as->cmn->join_targets_last; i++) {
		...
		for (j = (int)wlc->as->cmn->join_targets_last; --j >= 0;) {
			int k;
			bool done = TRUE;
			done = TRUE;
			for (k = i; k < j; k++) {
	...
	const struct ether_addr *bssid, const chanspec_t *chanspec_list, int channel_num)
{
	...
	if (assoc) {
		...
		if (BSS_CCX_ENAB(wlc, cfg) &&
			wlc_ccx_get_fast_roam(wlc->ccx, cfg) &&
			!channel_num) {
			WL_ASSOC(("wl%d: ROAM: CCX FAST, using AP channel list (%d channels)\n",
				WLCWLUNIT(wlc), wlc_ccx_get_ap_channel_num(wlc->ccx, cfg)));
			chanspec_list = wlc_ccx_get_ap_chanspec_list(wlc->ccx, cfg);
			channel_num = wlc_ccx_get_ap_channel_num(wlc->ccx, cfg);
...
wlc_roam_scan(wlc_bsscfg_t *cfg, uint reason, chanspec_t *list, uint32 channum)
{
	...
	uint cur_roam_reason = roam->reason;
		...
		if (BSS_CCX_ENAB(wlc, cfg) &&
			wlc_ccx_get_ap_channel_num(wlc->ccx, cfg) &&
			reason == WLC_E_REASON_FAST_ROAM_FAILED) {
	...
	if (BSS_CCX_ENAB(wlc, cfg)) {
		...
		WL_ROAM(("ROAM: Start roamscan reason %d fast_roam %d\n",
			reason, wlc_ccx_get_fast_roam(wlc->ccx, cfg)));
		...
		if (reason != WLC_E_REASON_FAST_ROAM_FAILED)
			wlc_read_tsf(wlc,
				wlc_ccx_get_roam_start_time(wlc->ccx, cfg), &tsf_h);
	} else {
...
wlc_join_wsec_filter(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
	...
	if (WLFBT_ENAB(wlc->pub)) {
		/* Check if FT AKM suites need to be set due to FBT enabled through CCX Cap IE */
		wlc_fbt_set_akm(wlc->fbt, cfg, bi);
...
wlc_join_attempt(wlc_bsscfg_t *cfg)
{
	...
	for (; (wlc->as->cmn->join_targets_last > 0 && wlc->as->cmn->join_targets_last < MAXBSS);
		...
		if (WLEXTSTA_ENAB(wlc->pub)) {
			...
			if (wlc->allow_mode == 1) {
				if (bcmp(&bi->BSSID.octet, &cfg->target_bss->BSSID.octet,
				         ETHER_ADDR_LEN)) {
...
#ifdef BCMCCX
			if (BSS_CCX_ENAB(wlc, cfg) && bi->aironet_ie_rx &&
				(status = wlc_ccx_prune(wlc->ccx, bi, cfg))) {
	...
	if (WLWNM_ENAB(wlc->pub) &&
		(wlc->as->cmn->join_targets_last > 0)) {
		wlc_bss_list_t *join_targets = wlc->as->cmn->join_targets;
...
#ifdef BCMCCX
		if (BSS_CCX_ENAB(wlc, cfg)) {
			wlc_ccx_set_ccx_network(wlc->ccx, cfg, bi->aironet_ie_rx);
			WL_ASSOC(("wl%d: JOIN: join a %s network\n", WLCWLUNIT(wlc),
				CCX_ASSOCIATION(wlc->ccx, cfg) ? "CCX" : "non-CCX"));
		}
...
wlc_join_bss_start(wlc_bsscfg_t *cfg)
{
	...
	if (as->type == AS_ROAM && wlc_bss_connected(cfg) &&
		...
		WL_ASSOC(("ROAM: waiting for %d tx packets drained"" out before roam\n",
			pktq_n_pkts_tot(q) + TXPKTPENDTOT(wlc)));
	} else {
		wlc_join_bss(cfg);
	}
}
	...
	struct scb *scb, uint8 mcsallow)
{
	...
	if (CCX_ENAB(wlc->pub) && CCX_ASSOCIATION(wlc->ccx, cfg))
		auth = wlc_ccx_set_auth(wlc->ccx, cfg);
#endif /* BCMCCX */
...
/* apply BSS configuration/capabilities tp scb */
	...
	*/
	if (WLEXTSTA_ENAB(wlc->pub)) {
		wlc_bss_mac_event(wlc, cfg, WLC_E_ASSOC_START, &bi->BSSID, as->type,
			((as->type == AS_ROAM) || (as->type == AS_RECREATE) ? roam->reason : 0),
			bi->bss_type, NULL, 0);
	}
#endif /* EXT_STA */
...
wlc_join_recreate(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
	...
	if (bsscfg->BSS) {
		...
		if (scb == NULL ||
#ifdef EXT_STA
		    (WLEXTSTA_ENAB(wlc->pub) && wlc_ndis_ar_disassoc_st(wlc->ndis, bsscfg)) ||
#endif
		    FALSE) {
...
wlc_sta_assoc_upd(wlc_bsscfg_t *cfg, bool state)
{
	...
	/* Move the cfg to IDLE if it is being
	* disassociated. Otherwise core may
	* not go down in MPC path.
	*/
	if (!state) {
		wlc_assoc_change_state(cfg, AS_IDLE);
	}
...
wlc_join_adopt_bss(wlc_bsscfg_t *cfg)
{
	...
	if (BSS_CCX_ENAB(wlc, cfg) &&
		...
		wlc_ccx_on_join_adopt_bss(wlc->ccx, cfg);
#endif /* BCMCCX */
	...
	struct dot11_bcn_prb *bcn, int bcn_len)
{
	...
	if (CCX_ENAB(wlc->pub) && as->type != AS_ASSOCIATION) {
		wlc_ccx_update_roaming_delay_tu(wlc->ccx, cfg);
	}
#endif /* BCMCCX */
...
#ifdef BCMCCX
	if (CCX_ENAB(wlc->pub) && CCX_ASSOCIATION(wlc->ccx, cfg))
		wlc_ccx_tx_pwr_ctl(wlc->ccx, tlvs, tlvs_len, cfg);
#endif /* BCMCCX */
...
#ifdef BCMCCX
	if (CCX_ENAB(wlc->pub) && CCX_ASSOCIATION(wlc->ccx, cfg))
		cfg_auth_alg = wlc_ccx_set_auth(wlc->ccx, cfg);
	else
#endif
...
_wlc_join_start_ibss(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int bss_type)
{
	...
	bi = (wlc_bss_info_t *)MALLOCZ(wlc->osh, sizeof(wlc_bss_info_t));
	if (bi == NULL)
		return BCME_NOMEM;
	/* set up default BSS params */
	bcopy(cfg->target_bss, bi, sizeof(wlc_bss_info_t));
...
#ifdef EXT_STA
	if (WLEXTSTA_ENAB(wlc->pub) && (as->disassoc_tx_retry == 0)) {
		/* Cancel the suppress-ssid-after-hibernate flag for Vista */
		wlc->scan_suppress_ssid = FALSE;
	}
#endif /* EXT_STA */
...
#ifdef BCMCCX
	if (BSS_CCX_ENAB(wlc, cfg) && cfg->BSS)
		wlc_ccx_on_leave_bss(wlc->ccx, cfg);
#endif /* BCMCCX */
...
wlc_assoc_success(wlc_bsscfg_t *cfg, struct scb *scb)
{
	...
	if (WL11K_ENAB(wlc->pub)) {
		...
		if (cfg->BSS && cfg->associated && cfg->current_bss &&
			...
			ssid.SSID_len = cfg->SSID_len;
			bcopy(cfg->SSID, ssid.SSID, ssid.SSID_len);
			err = wlc_iovar_op(wlc, "rrm_nbr_req", NULL, 0, &ssid, sizeof(wlc_ssid_t),
			      IOV_SET, cfg->wlcif);
			if (err) {
...
#if defined(BCMCCX) && defined(BCMINTSUP)
		if (SUP_ENAB(wlc->pub) && BSS_CCX_ENAB(wlc, cfg) &&
...
wlc_roam_complete(wlc_bsscfg_t *cfg, uint status, struct ether_addr *addr, uint bss_type)
{
	...
	if (status != WLC_E_STATUS_SUCCESS && status != WLC_E_STATUS_ABORT) {
		if (BSS_CCX_ENAB(wlc, cfg) &&
			...
			wlc_ccx_set_orig_reason(wlc->ccx, cfg, roam->reason);
...
do_event:
	...
	if (ASSOC_RECREATE_ENAB(wlc->pub) && assoc_recreate) {
		if (status != WLC_E_STATUS_SUCCESS) {
			 ...
			 */
			if (WLEXTSTA_ENAB(wlc->pub)) {
				wlc_ndis_disassoc_complete(wlc->ndis, cfg);
			}
#endif /* EXT_STA */
		}
...
wlc_roamscan_start(wlc_bsscfg_t *cfg, uint roam_reason)
{
	...
	if (CCX_ENAB(wlc->pub) &&
		...
		wlc_ccx_on_roam_start(wlc->ccx, cfg, roam_reason == WLC_E_REASON_DEAUTH ?
...
#ifdef BCMCCX
		 /* issue a CCX fast roam scan request only if Fast Roam has not failed */
		if (BSS_CCX_ENAB(wlc, cfg) &&
			wlc_ccx_get_ap_channel_num(wlc->ccx, cfg) &&
			roam_reason != WLC_E_REASON_FAST_ROAM_FAILED) {
			wlc_ccx_set_fast_roam(wlc->ccx, cfg, TRUE);
			roam->cache_valid = FALSE;
		} else
#endif /* BCMCCX */
...
#ifdef BCMCCX
				/* issue a CCX roam scan request only if Fast Roam has not failed */
				if (BSS_CCX_ENAB(wlc, cfg) &&
					wlc_ccx_get_ap_channel_num(wlc->ccx, cfg) &&
					roam_reason != WLC_E_REASON_FAST_ROAM_FAILED)
					wlc_ccx_set_fast_roam(wlc->ccx, cfg, TRUE);
...
#ifdef BCMCCX
	if (BSS_CCX_ENAB(wlc, cfg) && wlc_ccx_is_droam(wlc->ccx, cfg))
		return;
#endif
	/* Check for motion */
	if (!roam->motion &&
...
#ifdef BCMCCX
		else if (BSS_CCX_ENAB(wlc, bc) &&
			wlc_ccx_get_fast_roam(wlc->ccx, bc)) {
...
#ifdef EXT_STA
static int
wlc_get_link_quality_parms(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, ndis_link_parms_t *link_qual)
{
	ratespec_t rspec;

	memset(link_qual, 0, sizeof(*link_qual));

	/* return raw datarate in units of 500 Kbit/s, other parameters are translated */
	rspec = wlc_get_rspec_history(bsscfg);
	link_qual->link_speed = RSPEC2KBPS(rspec) / 500;

	/* return max raw datarate in units of 500 Kbit/s */
	rspec = wlc_get_current_highest_rate(bsscfg);
	link_qual->max_link_speed = RSPEC2KBPS(rspec) / 500;

	bcopy(bsscfg->BSSID.octet, link_qual->peer_mac.octet, ETHER_ADDR_LEN);
	link_qual->rssi = bsscfg->link->rssi;
	link_qual->chanspec = bsscfg->current_bss->chanspec;
...
#ifdef EXT_STA
	if (WLEXTSTA_ENAB(wlc->pub)) {
		if (wlc->link_report > 0)
			wlc->link_report--;
		else
			wlc->link_report = SW_TIMER_LINK_REPORT;
	}
#endif /* EXT_STA */
...
#ifdef EXT_STA
		/* periodically update link speed and quality */
		if (WLEXTSTA_ENAB(wlc->pub) && (wlc->link_report == SW_TIMER_LINK_REPORT)) {
			ndis_link_parms_t   link_quality;
			wlc_get_link_quality_parms(wlc, cfg, &link_quality);
			wlc_bss_mac_event(wlc, cfg, WLC_E_NDIS_LINK, NULL, 0, 0, 0,
...
#ifdef EXT_STA
				/* Due to limitation on number of interrupts in
				* NDIS code, need to emulate tbtt passing
				* according to missed beacon time
				* if extsta and bcn_interval under count, then
				* update; if bcn_interval_cnt is greater or equal
				* the bcn lost msg event already got sent and we shouldn't send it
				* again
				*/
				if (WLEXTSTA_ENAB(wlc->pub) && bp != 0) {

					uint32 missed_beacons;
					missed_beacons = roam->time_since_bcn*1000u/bp;

					wlc_roam_handle_missed_beacons(cfg, missed_beacons);
				}
#endif /* EXT_STA */
...
typedef struct {
	uint state;
	uint type;
	uint8 *ie;		/* During CCX re-assoc, assoc ie is re-used. If STA moves
					 * to other core, same needs to be copied to new cfg
					 */
	uint ie_len;		/* length of assoc ie */
...
wlc_assoc_bss_set(void *ctx, wlc_bsscfg_t *cfg, const uint8 *data, int len)
{
	...
	/* first free any existing ie_info on cfg */
		if (cfg->assoc->ie) {
			MFREE(cfg->wlc->osh, cfg->assoc->ie, cfg->assoc->ie_len);
			cfg->assoc->ie = NULL;
		}
	/* Allocate and copy ie_info on cfg */
		if (cp->ie_len && cp->ie) {
			cfg->assoc->ie = MALLOC(cfg->wlc->osh, cp->ie_len);
			if (!cfg->assoc->ie) {
				WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
				cfg->wlc->pub->unit, __FUNCTION__, MALLOCED(cfg->wlc->osh)));
				return BCME_NOMEM;
			}
			cfg->assoc->ie_len = cp->ie_len;
			memcpy(cfg->assoc->ie, cp->ie, cp->ie_len);
		}
...
wlc_assoc_bss_state_upd(void *ctx, bsscfg_state_upd_data_t *evt)
{
	...
	/* cleanup when disabling the bsscfg */
		...
		if (BSSCFG_STA(cfg) && cfg->associated) {
			if (wlc->pub->up && !BSSCFG_AWDL(wlc, cfg)) {
...
wlc_assoc_get_join_target(wlc_info_t *wlc, int offset)
{
	...
	if ((int)asc->join_targets_last + offset >= 0 &&
