	...
	void *p, uint plen, void *a, uint alen, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_RSSI_ANT): {
		...
		if (!wlc->pub->associated && MONITOR_ENAB(wlc)) {
			...
			ant_rssi->count = WL_RSSI_ANT_MAX;
			break;
		}
#endif
	...
	wlc_rx_pkt_type_t type)
{
	...
	for (k = WL_ANT_IDX_1, j = 0; k < WL_RSSI_ANT_MAX; k ++) {
		...
		if (type == PKT_TYPE_DATA) {
			row = slqi->rssi_chain_window + j * MA_WIN_SZ(blqi->rssi_window_sz);
			window_sz = MA_WIN_SZ(blqi->rssi_window_sz);
			memset(row, rssi, window_sz);
		}
#ifdef STA
		else {
			row = slqi->rssi_chain_bcn_window + j * MA_WIN_SZ(blqi->rssi_bcn_window_sz);
...
int8 BCMFASTPATH
wlc_lq_rssi_bss_sta_ma_upd(wlc_info_t *wlc, wlc_bsscfg_t *cfg,
	int band, int8 rssi, uint8 qdb, bool ucast)
{
	...
	if ((scb = wlc_scbfindband(wlc, cfg, &cfg->BSSID, band)) != NULL) {
		return wlc_lq_rssi_ma_upd(wlc, cfg, scb, rssi, qdb, ucast);
	}
...
*/
uint8 BCMFASTPATH
wlc_lq_recv_snr_compute(wlc_info_t *wlc, wlc_d11rxhdr_t *wrxh, int8 noise)
{
...
{
	wlc_lq_info_t *lqi = wlc->lqi;
	int8 rssi = WLC_RSSI_INVALID, rssi_avg = WLC_RSSI_INVALID;
	uint8 snr = WLC_SNR_INVALID, snr_avg = WLC_SNR_INVALID;

	/* rssi */
	if (PER_SCB_RSSI_SAMP(cfg) &&
	    (rssi = wlc_lq_recv_rssi_compute(wlc, wrxh)) != WLC_RSSI_INVALID) {
	...
	if (PER_SCB_SNR_SAMP(cfg) &&
	    (snr = wlc_lq_recv_snr_compute(wlc, wrxh, lqi->noise)) != WLC_SNR_INVALID) {
		/* collect the sample & calculate per-scb snr moving average */
		if (PER_SCB_SNR_MOVE_AVG(cfg)) {
...
void BCMFASTPATH
wlc_lq_bss_sta_sample(wlc_info_t *wlc, wlc_bsscfg_t *cfg,
	wlc_d11rxhdr_t *wrxh, int band, bool ucast, rx_lq_samp_t *samp)
{
	...
	if ((scb = wlc_scbfindband(wlc, cfg, &cfg->BSSID, band)) != NULL) {
		wlc_lq_sample(wlc, cfg, scb, wrxh, ucast, samp);
	}
}
	...
	rx_lq_samp_t *samp)
{
	...
	if ((rssi = wlc_lq_recv_rssi_compute(wlc, wrxh)) != WLC_RSSI_INVALID) {
		wlc_lq_info_t *lqi = wlc->lqi;
		...
		for (k = WL_ANT_IDX_1, j = 0; k < WL_RSSI_ANT_MAX; k ++) {
			if ((wlc->stf->rxchain & (1 << k)) == 0)
				continue;
			row = win->rssi_chain_window + j * MA_WIN_SZ(win->rssi_chain_window_sz);
...
wlc_lq_ant_rssi_monitor_get(wlc_info_t *wlc, int chain)
{
	...
	for (k = WL_ANT_IDX_1, j = 0; k < WL_RSSI_ANT_MAX; k ++) {
		if ((wlc->stf->rxchain & (1 << k)) == 0)
			continue;
		row = monitor->rssi_chain_window + j * MA_WIN_SZ(monitor->rssi_chain_window_sz);
		if (k == (uint)chain) {
...
wlc_lq_monitor_dump(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	for (k = WL_ANT_IDX_1, j = 0; k < WL_RSSI_ANT_MAX; k ++) {
		if ((wlc->stf->rxchain & (1 << k)) == 0)
			continue;
		row = monitor->rssi_chain_window + j * rssi_window_sz;
...
wlc_lq_scb_secsz(void *ctx, struct scb *scb)
{
	...
	if (PER_ANT_RSSI_SAMP(lqi, cfg)) {
		extra += rssi_window_sz * RX_ANT_NUM(lqi);	/* per-ant raw rssi samples */
	}
#ifdef STA
	/* per-ant raw bcn rssi samples */
	if (PER_ANT_BCN_RSSI_SAMP(lqi, cfg)) {
		extra += rssi_bcn_window_sz * RX_ANT_NUM(lqi);
	}
#endif /* STA */
	if (PER_SCB_SNR_SAMP(cfg)) {
...
wlc_lq_scb_init(void *ctx, struct scb *scb)
{
	...
	*pslqi = slqi;
	...
	if (PER_ANT_RSSI_SAMP(lqi, cfg)) {
		...
		extra += rssi_window_sz * RX_ANT_NUM(lqi);
	}
#ifdef STA
	if (PER_ANT_BCN_RSSI_SAMP(lqi, cfg)) {
...
wlc_lq_chanspec_to_chanim_stats(chanim_info_t *c_info, chanspec_t chanspec)
{
	...
	if ((stats = wlc_lq_chanim_chanspec_to_stats(c_info, chanspec)) != NULL) {
...
{
	wlc_chanim_stats_t *chanim_stats = wlc_lq_chanim_chanspec_to_stats(c_info, chanspec);
...
wlc_lq_chanim_find_stats(wlc_info_t *wlc, chanspec_t chanspec)
{
	...
	wlc_chanim_stats_t *stats = wlc_lq_chanim_chanspec_to_stats(c_info, chanspec);
...
wlc_lq_chanim_meas(wlc_info_t *wlc, chanim_cnt_t *chanim_cnt)
{
	...
	for (i = 0; i < CCASTATS_MAX; i++) {
		...
		if (i == CCASTATS_MYRX) {
			/* TODO: SWWLAN-108287 */
			/* TODO: ucode changes for M_CCA_MYRX are missing in trunk */
			ccastats_cnt[i] = 0;
		} else
#endif /* WLCHANIM_V2 */
		{
	...
	chanspec_t selected, uint8 trigger)
{
	...
	wlc_chanim_stats_t *cur_stats = wlc_lq_chanim_chanspec_to_stats(c_info, home_chspc);
