...
wlc_stf_get_pwrperrate(wlc_info_t *wlc, ratespec_t rspec, uint16 spatial_map)
{
	...
	if (RSPEC_ISHT(rspec) || RSPEC_ISVHT(rspec)) {
		if (IS_STBC(rspec)) {
			...
			/* TX chains can't be < TX streams (nss) */
			if ((uint)chains < (uint)nss) {
				WL_TXBF(("wl%d: %s: nss(%d) > tx chains(%d). Bump chains\n",
					wlc->pub->unit, __FUNCTION__, nss, chains));
				switch (nss) {
					case WL_TX_NSS_4:
					...
					case WL_TX_NSS_1:
						...
						break;
					default:
						WL_ERROR(("wl%d: %s:%d chains:%d, nss:%d",
	...
	wl_tx_chains_t txbf_chains)
{
	...
	if (RSPEC_ISHT(rspec) || RSPEC_ISVHT(rspec)) {
		...
		/* TX chains can't be < TX streams (nss) */
		if ((uint)txbf_chains < (uint)nss) {
			WL_TXBF(("wl%d: %s: nss(%d) > txbf_chains(%d). Bump txbf_chains\n",
				wlc->pub->unit, __FUNCTION__, nss, chains));
			switch (nss) {
				case WL_TX_NSS_4:
					txbf_chains = WL_TX_CHAINS_4;
					break;
				case WL_TX_NSS_3:
					txbf_chains = WL_TX_CHAINS_3;
					break;
				case WL_TX_NSS_2:
					txbf_chains = WL_TX_CHAINS_2;
					break;
				case WL_TX_NSS_1:
					txbf_chains = WL_TX_CHAINS_1;
					break;
				default:
					WL_ERROR(("wl%d: %s:%d txbf_chains:%d, nss:%d",
						wlc->pub->unit, __FUNCTION__, __LINE__,
						txbf_chains, nss));
					break;
			}
		...
		/* TX chains can't be < TX streams (nss) */
		if ((uint)chains < (uint)nss) {
			WL_TXBF(("wl%d: %s: nss(%d) > chains(%d). Bump chains\n",
				wlc->pub->unit, __FUNCTION__, nss, chains));
			switch (nss) {
				case WL_TX_NSS_4:
					chains = WL_TX_CHAINS_4;
