...
#else
#define WNM_COPY_SIZE 0
#define wlc_wnm_bss_get NULL
#define wlc_wnm_bss_set NULL
#endif /* WLRSDB */
...
BCMATTACHFN(wlc_wnm_detach)(wlc_wnm_info_t *wnm)
{
	...
	/* Free orphan tclas */
	wnm_tclas_free(wlc, wnm->tclas_head);
...
wlc_wnm_is_blacklisted_bss(wlc_wnm_info_t *wnm, wlc_bsscfg_t *cfg, wlc_bss_info_t *bip)
{
	...
	if ((bsi->disassoc_imnttmr_active) &&
		...
		(CHSPEC_WLCBANDUNIT(bip->chanspec) == bsi->blklisted_bssid_bandunit)) {
		retval = TRUE;
	}
	return retval;
}
	...
	uint8 *body, int body_len)
{
	...
	case DOT11_WNM_ACTION_BSSTRANS_RESP: {
		if (body_len < DOT11_BSSTRANS_RESP_LEN) {
			WL_WNM(("WNM BSS TRANS RESP length %d is invalid\n", body_len));
			return BCME_BADLEN;
		}
...
wlc_wnm_dms_amsdu_on(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	if (AMSDU_TX_ENAB(wlc->pub) &&
	    WLWNM_ENAB(wlc->pub) &&
	    wnm_cfg && WNM_DMS_ENABLED(wnm_cfg->cap) &&
...
#ifdef WLCHANIM_V2
		cu_self += chanim_stats->ccastats[CCASTATS_MYRX];
#endif /* WLCHANIM_V2 */
		wnm_cfg->load_current_cu_self = cu_self;
	}
#endif /* WLCHANIM */
...
wlc_wbtext_dump(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	FOREACH_AS_STA(wlc, idx, cfg) {
		...
		bcm_bprintf(b, "RSSI(cur:%d, trig:%d)\n",
				cfg->link->rssi, wlc->bandstate[band]->roam_trigger);
