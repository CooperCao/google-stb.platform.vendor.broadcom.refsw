...
/*
 * API choices:
 *  UEFI_API: UEFI 2.5 API, Native UEFI  + BRCM ioctl/iovars
 *  OLYMPIC_API: Apple Proprietary API + BRCM ioctl/iovars
 *  Niether:  No interface except for BRCM ioctl/iovars
 */
#ifdef UEFI_API
...
#ifdef OLYMPIC_API
EFI_GUID gBrcmwlprotoGuid = APPLE_80211_PROTOCOL_GUID;
#else
EFI_GUID gBrcmwlprotoGuid = BCMWL_GUID;
#endif
...
#define EFI_WLINFO_80211_FROM_THIS(a) CR (a, wl_info_t, proto, BCMWL_DRIVER_SIGNATURE)
...
#else  /* EFI_WINBLD */
#define WL_TIMER_TPL	TPL_CALLBACK
#endif /* EFI_WINBLD */
#define WL_ISR_TPL	(WL_TIMER_TPL + 2) /* Run at HIGHER TPL to prevent fifo overflows */
...
#ifdef BCMDBG
#define WL_LOCK(_wl, _tpl) do { \
		if (!(_wl)->locked) {		  \
			(_wl)->lock_line_no = __LINE__; \
			bzero((_wl)->lock_function, 256); \
			strncpy((_wl)->lock_function, __FUNCTION__, \
			        MIN(255, strlen(__FUNCTION__)));    \
			(_tpl) = 0; \
			EfiAcquireLock(&(_wl)->lock); \
			(_wl)->locked = TRUE; \
		} else { \
			WL_ERROR(("locked already by %s %d\n", (_wl)->lock_function, \
			          (_wl)->lock_line_no));		\
			ASSERT(0); \
		} \
	} while (0)

#define WL_UNLOCK(_wl, _tpl)  ASSERT((_wl)->locked); (_wl)->locked = FALSE; \
	EfiReleaseLock(&(_wl)->lock)
#else
...
STATIC EFI_STATUS EFIAPI BCMWL_ImageUnloadHandler(EFI_HANDLE ImageHandle)
{
	EFI_STATUS  Status;
	...
	WL_TRACE(("%s: %p\n\r", __FUNCTION__, OSL_OBFUSCATE_BUF(ImageHandle)));
                       ...
                       IN EFI_SYSTEM_TABLE     *SystemTable)
{
	...
	wl_msg_level =	WL_ERROR_VAL	/*  errors */
	        /* 	|	WL_TRACE_VAL // function trace */
	        /*	        | WL_PORT_VAL */
	        /*  | WL_PRHDRS_VAL */
		/* 	|	0x0004	// one-line frame tx/rx summary */
		/* 	|	0x0008	// complete frame tx/rx in hex */
		/* 	|	WL_INFORM_VAL	// informational */
		/* 	|	0x0020	// detailed debugging crap */
		/* 	|	WL_OID_VAL	// OID indications */
		/* 	| WL_MPC_VAL */
		/* 	| WL_DUAL_VAL */
		/* 	|	0x0080	// rate change */
		/* 	|	WL_ASSOC_VAL	// association */
		...
		| WL_APSTA_VAL
	;
	wl_apsta_dbg = WL_APSTA_UPDN_VAL;
#endif /* BCMDBG */
                      ...
                      IN EFI_DEVICE_PATH_PROTOCOL *RemainingDevicePath)
{
	...
	if (!wlc_chipmatch(PCI_VENDOR(VidDid), PCI_DEVID(VidDid)))
		...
		WL_TRACE(("%s: viddid:0x%x status: 0x%llx\n\r",
...
wl_free(wl_info_t *wl, EFI_DRIVER_BINDING_PROTOCOL *This)
{
...
#ifdef BCMDBG
		if (timer->name)
			MFREE(wl->osh, timer->name, strlen(timer->name) + 1);
#endif /* BCMDBG */
	...
	while ((efi_pkt = spktdeq(&wl->rxsendup))) {
	...
	while ((efi_pkt = spktdeq(&wl->txdone)))
		MFREE(wl->osh, efi_pkt, sizeof(struct EFI_PKT));
...
wl_cur_etheraddr_set(wl_info_t *wl, uint8 *addr)
{
	...
	if (Status != EFI_SUCCESS)
		bcopy(addr, &wl->proto.mode->HwAddress, ETHER_ADDR_LEN);
                  ...
                  IN EFI_DEVICE_PATH_PROTOCOL *RemainingDevicePath)
{
...
#else  /* EFI_WINBLD */
	wl = AllocateZeroPool(sizeof(wl_info_t));
#endif /* EFI_WINBLD */
	 ...
	 */
	spktqinit(&wl->txdone, PKTQ_LEN_MAX);
	wl->txpendcnt = 0;
	spktqinit(&wl->rxsendup, PKTQ_LEN_MAX);
	...
	wl->proto.Ioctl = BCMWL_80211Ioctl;
	wl->proto.mode = MALLOC(wl->osh, sizeof(BRCM_MODE));

	bcopy(addr, &wl->proto.mode->HwAddress, ETHER_ADDR_LEN);

	if (wl->proto.mode == NULL) {
		WL_ERROR(("wl%d: wl->proto allocation failed\n",
		          wl->pub->unit));
		goto Error;
	}
	...
	if (EFI_ERROR (Status)) {
		EFI_ERRMSG(Status, "InstallMultipleProtocolInterfaces failed\r\n");
		goto Error;
	}
                 ...
                 IN EFI_HANDLE *ChildHandleBuffer)
{
	...
	if (EFI_ERROR(Status))
		EFI_ERRMSG(Status, "Uninstall MultipleProtocolInterfaces failed\r\n");

	if (wl->proto.mode != NULL) {
		MFREE(wl->osh, wl->proto.mode, sizeof(BRCM_MODE));
		wl->proto.mode = NULL;
	}
...
EFI_STATUS EFIAPI BCMWL_ComponentNameGetDriverName(IN  EFI_COMPONENT_NAME_PROTOCOL *This,
                                                   IN  CHAR8 *Language,
                                                   OUT CHAR16 **DriverName)
{
#if defined(EFI_WINBLD)
...
EFI_STATUS EFIAPI BCMWL_ComponentNameGetControllerName(IN EFI_COMPONENT_NAME_PROTOCOL *This,
                                                       IN EFI_HANDLE ControllerHandle,
                                                       IN EFI_HANDLE ChildHandle OPTIONAL,
                                                       IN  CHAR8 *Language,
                                                       OUT CHAR16 **ControllerName)
{
	...
	return EfiLibLookupUnicodeString(Language,
#else  /* EFI_WINBLD */
	UnicodeSPrint(BCMWL_ControllerNameTable[0].UnicodeString,
	       (StrLen(BCMWL_ControllerNameTable[0].UnicodeString)+1) * 2,
	       L"BCM%x Wireless Controller", gWlDevice->devid);
...
#ifdef OLYMPIC_API
static void
update_state(wl_info_t *wl, int state)
{
	switch (state) {
	case BRCM_80211_S_UNDEF:
		wl->apple80211state = APPLE80211_S_UNDEF;
		break;
	case BRCM_80211_S_IDLE:
		wl->apple80211state = APPLE80211_S_IDLE;
		break;
	case BRCM_80211_S_SCAN:
		wl->apple80211state = APPLE80211_S_SCAN;
		break;
	case BRCM_80211_S_ASSOC:
		wl->apple80211state = APPLE80211_S_ASSOC;
		break;
	case BRCM_80211_S_AUTH:
		wl->apple80211state = APPLE80211_S_AUTH;
		break;
	case BRCM_80211_S_RUN:
		wl->apple80211state = APPLE80211_S_RUN;
		break;
	case BRCM_80211_S_BADKEY:
		wl->apple80211state = APPLE80211_S_BADKEY;
		break;
	case BRCM_80211_S_BADAUTH:
		wl->apple80211state = APPLE80211_S_BADAUTH;
		break;
	case BRCM_80211_S_AUTHFAIL:
		wl->apple80211state = APPLE80211_S_AUTHFAIL;
		break;
	case BRCM_80211_S_ASSOCFAIL:
		wl->apple80211state = APPLE80211_S_ASSOCFAIL;
		break;
	case BRCM_80211_S_NONETWORK:
		wl->apple80211state = APPLE80211_S_NONETWORK;
		break;
	case BRCM_80211_S_OTHER:
...
void wl_event(struct wl_info *wl, char *ifname, wlc_event_t *evt)
{
	...
	case WLC_E_LINK:
		if ((evt->event.flags & WLC_EVENT_MSG_LINK) != WLC_EVENT_MSG_LINK) {
		        update_state(wl, BRCM_80211_S_IDLE);
	...
	case WLC_E_SET_SSID:
		if (status != WLC_E_STATUS_SUCCESS)
			update_state(wl, BRCM_80211_S_NONETWORK);

		/*  Handle IBSS create/join */
		if (wlc->default_bss->bss_type == DOT11_BSSTYPE_INDEPENDENT)
			if (status != WLC_E_STATUS_SUCCESS)
				wl_setInfraMode(wl, WL_BSSTYPE_ANY);
		break;
	case WLC_E_AUTH:
		if (status != WLC_E_STATUS_SUCCESS) {
			...
			if (wsec && wl_code2applebadkey_map(reason, apple80211_bad_key_sc))
				update_state(wl, BRCM_80211_S_BADKEY);
			else
				update_state(wl, BRCM_80211_S_AUTHFAIL);
		}
		break;
	case WLC_E_ROAM:
	...
	case WLC_E_ASSOC:
		if (status != WLC_E_STATUS_SUCCESS)
			update_state(wl, BRCM_80211_S_ASSOCFAIL);
		break;
	case WLC_E_DEAUTH_IND: {
		...
		PRINT_BANNER(("DeAuthenticated by %a, reason #%d\n", eabuf, reason));
#endif
		if (wl_iovar_getint(wl, "wsec", &wsec) != EFI_SUCCESS)
			wsec = 0;
		...
		if (wsec && wl_code2applebadkey_map(reason, apple80211_bad_key_rc))
			...
			update_state(wl, BRCM_80211_S_OTHER);
		else
			update_state(wl, BRCM_80211_S_IDLE);
#endif  /* OLYMPIC_API */
		break;
              ...
              const char *name)
{
...
#ifdef BCMDBG
	timer->name = NULL;
	if ((timer->name = (char *)MALLOCZ(wl->osh, (uint)(strlen(name) + 1))) != NULL)
		strncpy(timer->name, name, strlen(name) + 1);
#endif /* BCMDBG */
	...
	WL_NONE(("%s: Success: %p, %s\n\r", __FUNCTION__, OSL_OBFUSCATE_BUF(timer), name));
...
void wl_add_timer(struct wl_info *wl, struct wl_timer *timer, uint ms, int periodic)
{
	...
	WL_TRACE(("%s: 0x%p %s ms:%d periodic:%d\n\r", __FUNCTION__,
...
#ifdef BCMDBG
	if (Status != EFI_SUCCESS) {
		WL_ERROR(("wl%d: Failed to delete timer %s\n", wl->pub->unit, timer->name));
		EFI_ERRMSG(Status, " Delete timer failed\n");
	}
#endif /* BCMDBG */
...
void wl_sendup(struct wl_info *wl, struct wl_if *wlif, void *p, int numpkt)
{
	...
	if (efi_pkt == NULL) {
		...
		spktenq(&wl->rxsendup, (void*)efi_pkt);
}

void wl_sendup_event(struct wl_info *wl, struct wl_if *wlif, void *pkt)
{
...
#ifndef EFI_WINBLD
void wl_monitor(struct wl_info *wl, struct wl_rxsts *rxsts, void *p)
#else
                       ...
                       uint8 *addr)
{
	...
	base_path_len = (UINT16) (end_node - basedev_p);
...
BCMWL_UNDIAPIEntry(UINT64 cdb, wl_info_t *wl)
{
	...
	if (tmp_cdb.CPBsize == PXE_CPBSIZE_NOT_USED) {
		if (tmp_cdb.CPBaddr != PXE_CPBADDR_NOT_USED) {
			WL_ERROR(("BCMWL_UNDIAPIEntry()): Bad CPBaddr! Expected=%lld\n\r",
	...
	if (tmp_cdb.DBsize == PXE_DBSIZE_NOT_USED) {
		if (tmp_cdb.DBaddr != PXE_DBADDR_NOT_USED) {
			WL_ERROR(("BCMWL_UNDIAPIEntry()): Bad DBaddr! Expected %lld\n\r",
			          PXE_CPBADDR_NOT_USED));
			goto badcdb;
		}
...
/* or should we define EDK2_NAMES? */
#if defined(EDK2_NAMES) || !defined(EFI_WINBLD)
...
STATIC int
wl_db_ptr_data_update(void *ctx, const uint8 *data, uint16 type, uint16 len)
{
	...
	switch (type) {
		...
		case WL_CNT_XTLV_GE40_UCODE_V1:
		{
			 ...
			 */
			const wl_cnt_lt40mcst_v1_t *cnt = (const wl_cnt_lt40mcst_v1_t *)data;
                     ...
                     struct wl_info *wl)
{
	...
	cntbuf = MALLOCZ(wl->osh, WLC_IOCTL_MEDLEN);
	if (cntbuf == NULL) {
		return;
	}
	...
	Status = wl_iovar_op(wl, "counters", NULL, 0,
	                     cntbuf, WLC_IOCTL_MEDLEN, FALSE, NULL);
	WL_UNLOCK(wl, tpl);
	 ...
	 */
	Status = wl_cntbuf_to_xtlv_format(wl->osh, cntinfo, WLC_IOCTL_MEDLEN, 0);
	if (Status != EFI_SUCCESS) {
		WL_ERROR(("\twl_cntbuf_to_xtlv_format failed! %llu\n", Status));
		cdb_ptr->StatFlags = PXE_STATFLAGS_COMMAND_FAILED;
		cdb_ptr->StatCode = PXE_STATCODE_INVALID_CDB;
		goto exit;
	}
...
exit:
	MFREE(wl->osh, cntbuf, WLC_IOCTL_MEDLEN);
	return;
}
	...
	struct wl_info *wl)
{
	...
	if (!wlc_scan_inprog(wl->wlc)) {
		...
		while (wl->txpendcnt != (uint)spktq_n_pkts(&wl->txdone) &&
			i < (WL_PKTTX_TIMEOUT/100)) {
			OSL_DELAY(100);
			wl_isr(wl);
			i++;
		}

		if (wl->txpendcnt != (uint)spktq_n_pkts(&wl->txdone))
			WL_ERROR(("wl%d: TX pkt timed out. %d pkts pending\n",
			          wl->pub->unit, wl->txpendcnt - spktq_n_pkts(&wl->txdone)));
	}

	if (spktq_n_pkts(&wl->rxsendup) != 0) {
		struct EFI_PKT *pkt = spktq_peek(&wl->rxsendup);
	...
	if (cdb_ptr->OpFlags & PXE_OPFLAGS_GET_TRANSMITTED_BUFFERS) {
		...
		i = 0;
		while (!spktq_empty(&wl->txdone) &&
		      tx_buffer_size >= sizeof(UINT64)) {
			struct EFI_PKT *pkt = spktdeq(&wl->txdone);
			wl->txpendcnt--;
                   ...
                   struct wl_info *wl)
{
	...
	if (spktq_empty(&wl->rxsendup)) {
		cdb_ptr->StatCode = PXE_STATCODE_NO_DATA;
		WL_UNLOCK(wl, tpl);
		return;
	}

	pxe_pkt = spktdeq(&wl->rxsendup);
	WL_UNLOCK(wl, tpl);
...
wl_setInfraMode(wl_info_t *wl, int mode)
{
	...
	Status = wl_iovar_setint(wl, "infra_configuration", mode);
...
BCMWL_ScanConvertResult(wl_info_t *wl, wlc_bss_info_t * bi, wl_80211_scanresults_t *wl_sr)
{
	...
	if (WL_PORT_ON()) {
		...
		WL_PORT(("Scan result for \"%s\" -- Advcap 0x%x\n", ssidbuf, asr->advCap));
	}
#endif /* BCMDBG */
}
...
#else  /* EFI_WINBLD */
	sr = (wl_80211_scanresults_t *)AllocateZeroPool(sizeof(wl_80211_scanresults_t) *
	                                               wlc->scan_results->count);
#endif /* EFI_WINBLD */
...
BCMWL_JoinIoctl(wl_info_t *wl, APPLE_80211_JOINPARAMS *joinParams)
{
	...
	case APPLE80211_CIPHER_AES_CCM:
		if (strlen(Key->Password) == 0 && Key->KeyLen == PMK_LEN) {
...
BCMWL_IBSSIoctl(wl_info_t *wl, APPLE_80211_IBSSPARAMS *ibssParams)
{
	...
	wl_setInfraMode(wl, WL_BSSTYPE_INDEP);
                 ...
                 IN OUT UINT32 *OutSize)
{
	...
	wl = EFI_WLINFO_80211_FROM_THIS((BCMWL_PROTO *)this);
