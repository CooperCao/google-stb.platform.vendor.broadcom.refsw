...
/* auto country 11d sharing */
typedef struct wlc_11d_shared {
...
BCMATTACHFN(wlc_11d_attach)(wlc_info_t *wlc)
{
	...
	m11d->cmn = (wlc_11d_shared_t *) obj_registry_get(wlc->objr, OBJR_11D_CMN_INFO);
	if (m11d->cmn == NULL) {
		if (!(m11d->cmn = (wlc_11d_shared_t *)MALLOCZ(wlc->osh,
			sizeof(wlc_11d_shared_t)))) {
			WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
				wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
			goto fail;
		}
		obj_registry_set(wlc->objr, OBJR_11D_CMN_INFO, m11d->cmn);
	}
	...
	bzero(&m11d->cmn->supported_channels, sizeof(chanvec_t));
	for (i = 0; i < NBANDS(wlc); i++, band = wlc->bandstate[OTHERBANDUNIT(wlc)]) {
		...
		for (j = 0; j < sizeof(chanvec_t); j++)
			m11d->cmn->supported_channels.vec[j] |= channels.vec[j];
...
BCMATTACHFN(wlc_11d_detach)(wlc_11d_info_t *m11d)
{
	...
	if ((m11d->cmn) && (obj_registry_unref(m11d->wlc->objr, OBJR_11D_CMN_INFO) == 0)) {
		obj_registry_set(m11d->wlc->objr, OBJR_11D_CMN_INFO, NULL);
		MFREE(m11d->wlc->osh, m11d->cmn, sizeof(wlc_11d_shared_t));
		m11d->cmn = NULL;
	}
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_AUTOCOUNTRY_DEFAULT):
		if ((uint)len <= strlen(m11d->cmn->autocountry_default) + 1) {
			err = BCME_BUFTOOSHORT;
			break;
		}
		strncpy(arg, m11d->cmn->autocountry_default, len - 1);
		break;
	...
	case IOV_SVAL(IOV_AUTOCOUNTRY_DEFAULT): {
		...
		strncpy(m11d->cmn->autocountry_default, country_abbrev, WLC_CNTRY_BUF_SZ - 1);
		break;
	}
	...
	case IOV_SVAL(IOV_AUTOCOUNTRY): {
		...
		if (err) {
			...
			m11d->cmn->awaiting_cntry_info = awaiting_cntry_info;
			break;
		}

		m11d->cmn->autocountry_adopted_from_ap = FALSE;

		break;
	}
#ifdef CNTRY_DEFAULT
	case IOV_GVAL(IOV_CNTRY_DEFAULT):
		*ret_int_ptr = (int)m11d->cmn->_cntry_default;
		break;
	case IOV_SVAL(IOV_CNTRY_DEFAULT):
		m11d->cmn->_cntry_default = bool_val;
		break;
#endif /* CNTRY_DEFAULT */
#ifdef LOCALE_PRIORITIZATION_2G
	case IOV_GVAL(IOV_CCODE_PR_2G):
		*ret_int_ptr = (int)m11d->cmn->ccode_pr_2g;
		break;
	case IOV_SVAL(IOV_CCODE_PR_2G):
		m11d->cmn->ccode_pr_2g = bool_val;
		break;
#endif /* LOCALE_PRIORITIZATION_2G */
#endif /* STA */
...
{
	bzero(m11d->cmn->best_abbrev, sizeof(m11d->cmn->best_abbrev));
	m11d->cmn->best_chans = 0;
}
                         ...
                         uint8 *bcn_prb, uint bcn_prb_len)
{
	...
	if (!strncmp(m11d->cmn->best_abbrev, country_abbrev, WLC_CNTRY_BUF_SZ))
		return;
	...
	if (result == CLM_RESULT_OK) {
		for (i = 0; i < NBANDS(wlc); i++, band = wlc->bandstate[OTHERBANDUNIT(wlc)]) {
			wlc_locale_get_channels(&locale_new,
			                        ((band->bandtype == WLC_BAND_2G)
			                         ? CLM_BAND_2G : CLM_BAND_5G),
			                         &channels, &unused);

			for (j = 0; j < sizeof(chanvec_t); j++)
				channels.vec[j] &= m11d->cmn->supported_channels.vec[j];
	...
	if (x_band > m11d->cmn->best_chans) {
		if (m11d->cmn->best_abbrev[0])
			WL_REGULATORY(("wl%d: %s: Country IE \"%s\" with "
			               "%d channels preferred over "
			               "\"%s\" with %d channels.\n",
			               wlc->pub->unit, __FUNCTION__,
			               country_abbrev, x_band,
			               m11d->cmn->best_abbrev, m11d->cmn->best_chans));
		m11d->cmn->best_chans = x_band;
                    ...
                    uint8 *bcn_prb, uint bcn_prb_len)
{
	...
	if (m11d->cmn->autocountry_mode_flags & AUTOCOUNTRY_MODE_FLAGS_DEFER_EVAL)
		return;
...
wlc_11d_scan_complete(wlc_11d_info_t *m11d, int status)
{
	...
	if ((status != WLC_E_STATUS_SUCCESS) &&
	    (m11d->cmn->autocountry_mode_flags & AUTOCOUNTRY_MODE_FLAGS_SUCCESS_ONLY)) {
		return;
	}
	...
	if (wlc->pub->associated &&
		...
		WL_REGULATORY(("wl%d: %s: Not adopting best choice Country \"%s\" from "
		               ...
		               wlc->pub->unit, __FUNCTION__, m11d->cmn->best_abbrev,
		               bcm_ether_ntoa(&m11d->cmn->best_bssid, eabuf),
		               wf_chspec_ctlchan(m11d->cmn->best_chanspec),
	...
	wlc_set_countrycode(wlc->cmi, m11d->cmn->best_abbrev);
	m11d->cmn->awaiting_cntry_info = FALSE;
...
wlc_11d_adopt_country(wlc_11d_info_t *m11d, char *country_str, bool adopt_country)
{
	...
	if (!(m11d->cmn->awaiting_cntry_info ||
	      adopt_country ||
	      !m11d->cmn->autocountry_adopted_from_ap))
		return;
		...
		*/
		if (WLC_CNTRY_DEFAULT_ENAB(wlc)) {
			strncpy(country_abbrev,
				m11d->cmn->autocountry_default, sizeof(country_abbrev) - 1);
			country_abbrev[2] = '\0';
		}
	}
	...
	if (adopt_country) {
		m11d->cmn->autocountry_adopted_from_ap = TRUE;
...
{
	m11d->cmn->awaiting_cntry_info = FALSE;
	bzero(m11d->cmn->autocountry_scan_learned, WLC_CNTRY_BUF_SZ - 1);
}
...
{
	return m11d->cmn->_cntry_default;
}
#endif /* CNTRY_DEFAULT */
...
{
	return m11d->cmn->autocountry_adopted_from_ap;
}
...
{
	m11d->cmn->autocountry_adopted_from_ap = FALSE;
	bzero(m11d->cmn->autocountry_adopted, WLC_CNTRY_BUF_SZ - 1);
}

void
wlc_11d_set_autocountry_adopted(wlc_11d_info_t *m11d, const char *country_abbrev)
{
	strncpy(m11d->cmn->autocountry_adopted, country_abbrev, WLC_CNTRY_BUF_SZ - 1);
}

const char *
wlc_11d_get_autocountry_adopted(wlc_11d_info_t *m11d)
{
	return m11d->cmn->autocountry_adopted;
}

bool
wlc_11d_autocountry_scan_learned(wlc_11d_info_t *m11d)
{
	if (!m11d->cmn->awaiting_cntry_info && m11d->cmn->autocountry_scan_learned[0] != '\0')
		return TRUE;
	return FALSE;
...
{
	return m11d->cmn->autocountry_scan_learned;
}

void
wlc_11d_set_autocountry_scan_learned(wlc_11d_info_t *m11d, const char *country_abbrev)
{
	strncpy(m11d->cmn->autocountry_scan_learned, country_abbrev, WLC_CNTRY_BUF_SZ - 1);
}
...
{
	return m11d->cmn->autocountry_default;
}
#endif /* WL11D */
