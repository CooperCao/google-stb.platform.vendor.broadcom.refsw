...
#define CONST_MCNX_BSSCFG_CUBBY_LOC(mcnx, cfg)\
	((const bss_mcnx_info_t * const *) CONST_BSSCFG_CUBBY((cfg), (mcnx)->cfgh))
#define BSS_MCNX_INFO(mcnx, cfg) MCNX_BSSCFG_CUBBY(mcnx, cfg)
#define CONST_BSS_MCNX_INFO(mcnx, cfg) (*(CONST_MCNX_BSSCFG_CUBBY_LOC(mcnx, cfg)))

/* local prototypes */

/* module */
static int wlc_mcnx_doiovar(void *ctx, uint32 actionid,
...
#define INVALID_RCMTA_IDX -2 /* Default value of rcmta_alt_ra_idx. used only fore corerev >40 */

/* Local functions */
static void wlc_mcnx_unset_rcmta_bss_info(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg);
...
wlc_mcnx_rcmta_type_set(wlc_mcnx_info_t *mcnx, int idx, uint16 mask, uint16 val)
{
	...
	if (D11REV_GE(wlc->pub->corerev, 40)) {
		...
		new_val = orig_val =
			wlc_mcnx_read_shm(mcnx, M_ADDR_BMP_BLK(wlc, idx - mcnx->p2p_start_idx));
	}
	...
	if (D11REV_GE(wlc->pub->corerev, 40)) {
		...
		wlc_mcnx_write_shm(mcnx, M_ADDR_BMP_BLK(wlc, idx - mcnx->p2p_start_idx), new_val);
	}
}
...
static int
wlc_mcnx_doiovar(void *ctx, uint32 actionid,
	void *p, uint plen, void *a, uint alen, uint vsize, struct wlc_if *wlcif)
{
...
wlc_mcnx_up(void *ctx)
{
	...
	WL_TRACE(("%s\n", __FUNCTION__));
	/* SHM location */
	mcnx->d11shm = wlc_read_shm(wlc, M_P2P_BLK_PTR(wlc)) << 1;
	...
	for (b = 0; b < M_P2P_BSS_MAX; b ++) {
		for (c = 0; c < M_P2P_BSS_BLK_SZ; c ++)
			wlc_mcnx_write_shm(mcnx, M_P2P_BSS(wlc, b, c), 0);
		wlc_mcnx_write_shm(mcnx, M_P2P_PRE_TBTT(wlc, b), 0);
	}

	/* clear interrupt blocks */
	for (b = 0; b < M_P2P_BSS_MAX; b ++) {
		for (c = 0; c < M_P2P_I_BLK_SZ; c ++)
			wlc_mcnx_write_shm(mcnx, M_P2P_I(wlc, b, c), 0);
	}

	/* clear TSF blocks */
	for (b = 0; b < M_P2P_BSS_MAX; b ++) {
		for (c = 0; c < M_P2P_TSF_BLK_SZ; c ++)
			wlc_mcnx_write_shm(mcnx, M_P2P_TSF(wlc, b, c), 0);
	}

	/* clear HPS blocks */
	wlc_mcnx_write_shm(mcnx, M_P2P_HPS_OFFSET(wlc), 0);
...
uint16
wlc_mcnx_read_dtim_prd(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *bsscfg)
{
	uint bss = wlc_mcnx_BSS_idx(mcnx, bsscfg);
	return wlc_mcnx_read_shm(mcnx, M_P2P_BSS_DTIM_PRD(mcnx->wlc, bss));
}

void
wlc_mcnx_write_dtim_prd(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *bsscfg, uint16 dtim)
{
	uint bss = wlc_mcnx_BSS_idx(mcnx, bsscfg);
	wlc_mcnx_write_shm(mcnx, M_P2P_BSS_DTIM_PRD(mcnx->wlc, bss), dtim);
}
...
wlc_mcnx_hps_get(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg)
{
	...
	return wlc_mcnx_read_shm(mcnx,
		M_P2P_HPS_OFFSET(wlc)) & (M_P2P_HPS_NOA(bss) | M_P2P_HPS_CTW(bss));
}
...
wlc_p2p_int_proc(wlc_info_t *wlc, uint8 *p2p_interrupts, uint32 tsf_l, uint32 tsf_h)
{
	...
	for (b = 0; b < M_P2P_BSS_MAX; b ++) {
		...
		for (i = 0; i < M_P2P_I_BLK_SZ; i ++) {
			 ...
			 */
			if (i == M_P2P_I_PRE_TBTT) {
				WLCNTINCR(wlc->pub->_cnt->p2p_tbtt);
				wlc_bss_tbtt(cfg);
			}
		}
	}
...
wlc_mcnx_bss_upd(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool is_up)
{
	...
	wlc_mcnx_st_upd(mcnx, cfg, is_up);
...
_wlc_mcnx_ps_enab(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool enab)
{
	...
	state = wlc_mcnx_read_shm(mcnx, M_P2P_BSS_ST(wlc, bss));
	if (enab) {
	...
	wlc_mcnx_write_shm(mcnx, M_P2P_BSS_ST(wlc, bss), state);
}
...
_wlc_mcnx_st_upd(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool set)
{
	...
	state = set ?
	        (P2P_GO(wlc, cfg) ? M_P2P_BSS_ST_GO :
	         ...
	         BSSCFG_NAN_MGMT(cfg) ? M_P2P_BSS_ST_NAN | M_P2P_BSS_ST_GC :
	         M_P2P_BSS_ST_STA) :
	        0;
	...
	go_ind_bmp = wlc_mcnx_read_shm(mcnx, M_P2P_GO_IND_BMP_OFFSET(wlc));
	...
	wlc_mcnx_write_shm(mcnx, M_P2P_GO_IND_BMP_OFFSET(wlc), go_ind_bmp);
}
...
_wlc_mcnx_tbtt_upd(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool set)
{
	...
	if (set) {
		...
		if (cfg->associated ||
			...
			if (bmi->unaligned_tbtt_tsfo > 0) {
				wlc_uint64_add(&rtbtt_h, &rtbtt_l, 0, bmi->unaligned_tbtt_tsfo);
				/* add bcnint if already passed unaligned_tbtt_tsfo */
				if (bmi->unaligned_tbtt_tsfo < offset) {
		...
		wlc_mcnx_write_shm(mcnx, M_P2P_PRE_TBTT(wlc, bss), (uint16)pretbtt);
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_N_PRE_TBTT(wlc, bss),
			(uint16)(n_pretbtt >> P2P_UCODE_TIME_SHIFT));
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_BCN_INT(wlc, bss),
		...
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_BCN_INT(wlc, bss), 0);
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_N_PRE_TBTT(wlc, bss), 0);
		wlc_mcnx_write_shm(mcnx, M_P2P_PRE_TBTT(wlc, bss), 0);
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_ST(wlc, bss), 0);
	}
}
...
_wlc_mcnx_dtim_upd(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool set)
{
	...
	if (set) {
		...
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_DTIM_PRD(wlc, bss), dtimprd);
	}
	else {
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_DTIM_PRD(wlc, bss), 0);
	}
}
...
wlc_mcnx_reset_bss(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg)
{
	...
	wlc_mcnx_mac_suspend(mcnx);
	for (c = 0; c < M_P2P_BSS_BLK_SZ; c ++)
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS(wlc, bss, c), 0);
	wlc_mcnx_mac_resume(mcnx);
}
	...
	uint32 ltsf_h, uint32 ltsf_l, uint32 rtsf_h, uint32 rtsf_l)
{
	...
	if (tsf_upd) {
		...
		wlc_mcnx_write_shm(mcnx, M_P2P_TSF(wlc, bss, 3), (uint16)(off_h >> 16));
		wlc_mcnx_write_shm(mcnx, M_P2P_TSF(wlc, bss, 2), (uint16)off_h);
		wlc_mcnx_write_shm(mcnx, M_P2P_TSF(wlc, bss, 1), (uint16)(off_l >> 16));
		wlc_mcnx_write_shm(mcnx, M_P2P_TSF(wlc, bss, 0), (uint16)off_l);
	}
...
wlc_mcnx_info_init(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	*pbmi = bmi;
	...
	bmi->bcn_prd = cfg->current_bss->beacon_period;
	bmi->rcmta_alt_ra_idx = INVALID_RCMTA_IDX;
...
wlc_mcnx_info_deinit(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	wlc_mcnx_unset_rcmta_bss_info(mcnx, cfg);
...
static void
wlc_mcnx_unset_rcmta_bss_info(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg)
{
	bss_mcnx_info_t *bmi;
...
#if defined(WLAIBSS)
	if (AIBSS_ENAB(mcnx->wlc->pub) && BSSCFG_IBSS(cfg))
		wlc_set_bssid_hw(mcnx->wlc, idx, &cfg->BSSID, 0);
	else
#endif /* WLAIBSS */
...
#ifdef WLRSDB
/* For RSDB, set ra on other wlc using this wlc's bsscfg */
int
wlc_mcnx_alt_ra_set(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg)
{
	bss_mcnx_info_t *bmi;

	ASSERT(cfg != NULL);

	/* This API is supported only for cfg which doesn't belong to this mcnx. */
	if (mcnx->wlc == cfg->wlc) {
	       return BCME_BADARG;
	}
	...
	wlc_mcnx_rcmta_alloc(mcnx, &bmi->rcmta_alt_ra_idx);
...
/* For RSDB, unset ra on other wlc using this wlc's bsscfg */
int
wlc_mcnx_alt_ra_unset(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg)
{
	bss_mcnx_info_t *bmi;
	...
	wlc_mcnx_rcmta_free(mcnx, bmi->rcmta_alt_ra_idx);
	bmi->rcmta_alt_ra_idx = INVALID_RCMTA_IDX;
...
{
	const bss_mcnx_info_t *bmi;

	ASSERT(cfg != NULL);
	bmi = CONST_BSS_MCNX_INFO(mcnx, cfg);
	ASSERT(bmi != NULL);
...
wlc_mcnx_rcmta_ra_idx(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg)
{
	...
	/* If requested ra idx is for current wlc, send ra_idx, otherwise alt_ra_idx. */
#ifdef WLRSDB
	if (RSDB_ENAB(mcnx->wlc->pub) && mcnx->wlc != cfg->wlc) {
		ASSERT(bmi->rcmta_alt_ra_idx != INVALID_RCMTA_IDX);
		return bmi->rcmta_alt_ra_idx;
...
_wlc_mcnx_ctw_upd(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool enab, uint32 ctw)
{
	...
	if (enab) {
		 ...
		 */
		st = wlc_mcnx_read_shm(mcnx, M_P2P_BSS_ST(wlc, bss));
		st &= ~M_P2P_BSS_ST_CTW;
		...
		if (!cfg->associated) {
			...
			if (offset >> P2P_UCODE_TIME_SHIFT < ctw >> P2P_UCODE_TIME_SHIFT) {
				/* race condition??? */
				if (wlc_mcnx_read_shm(mcnx, M_P2P_BSS_CTW(wlc, bss)) == 0) {
					...
					wlc_mcnx_write_shm(mcnx, M_P2P_BSS_N_CTW_END(wlc, bss),
						ctwend);
				}
				st |= M_P2P_BSS_ST_CTW;
			}
		...
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_CTW(wlc, bss),
			(uint16)(ctw >> P2P_UCODE_TIME_SHIFT));

		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_ST(wlc, bss), st);
	}
	else {
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_CTW(wlc, bss), 0);
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_N_CTW_END(wlc, bss), 0);
	}
}
...
_wlc_mcnx_abs_upd(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool enab, wlc_mcnx_abs_t *sched)
{
	...
	if (enab) {
		...
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_N_NOA(wlc, bss),
			(uint16)(start >> P2P_UCODE_TIME_SHIFT));
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_NOA_DUR(wlc, bss),
			(uint16)(duration >> P2P_UCODE_TIME_SHIFT));
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_NOA_TD(wlc, bss),
			(uint16)(presence >> P2P_UCODE_TIME_SHIFT));
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_NOA_OFS(wlc, bss), (uint16)remain);
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_NOA_CNT(wlc, bss), (uint16)count);
	}
	else {

		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_NOA_CNT(wlc, bss), 0);
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_N_NOA(wlc, bss), 0);
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_NOA_DUR(wlc, bss), 0);
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_NOA_TD(wlc, bss), 0);
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_NOA_OFS(wlc, bss), 0);
	}
}
...
/* Update BSS block to enable capable of multi-DTIM sleep */
static int
_wlc_mcnx_multidtim_upd(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool enab)
{
	wlc_info_t *wlc = mcnx->wlc;
	uint16 state;
	int bss;
	...
	/* Only supported for STA mode i.e dual sta case */
	ASSERT(!P2P_CLIENT(wlc, cfg));
	...
	/* Explicitly forcing as infra-BSS STA to enable Multi-DTIM */
	if (enab) {
		state |= M_P2P_BSS_ST_MULTIDTIM;
	} else {
		state &= ~M_P2P_BSS_ST_MULTIDTIM;
	}

	WL_MCNX(("wl%d.%d: enable bss %d multi-dtim support\n",
	        wlc->pub->unit, WLC_BSSCFG_IDX(cfg), bss));
...
int
wlc_mcnx_multidtim_upd(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool enab)
{
	wlc_info_t *wlc = mcnx->wlc;
	int ret = BCME_OK;
	...
	if (D11REV_GE(wlc->pub->corerev, 40)) {
		wlc_mcnx_mac_suspend(mcnx);
		ret = _wlc_mcnx_multidtim_upd(mcnx, cfg, enab);
		wlc_mcnx_mac_resume(mcnx);
	}
...
/*
Set BSS Index in bits 10:8 of SHM addr - M_P2P_GO_IND_BMP_OFFSET.
...
wlc_mcnx_shm_bss_idx_set(wlc_mcnx_info_t *mcnx, int bss_idx)
{
	...
	go_ind_bmp = wlc_mcnx_read_shm(mcnx, M_P2P_GO_IND_BMP_OFFSET(mcnx->wlc));
	go_ind_bmp &= ~M_P2P_BSS_INDEX_MASK;
	go_ind_bmp |= ((bss_idx << M_P2P_BSS_INDEX_SHIFT_BITS) & M_P2P_BSS_INDEX_MASK);
	wlc_mcnx_write_shm(mcnx, M_P2P_GO_IND_BMP_OFFSET(mcnx->wlc), go_ind_bmp);
}
...
static void write_dts_shm(wlc_bsscfg_t *cfg, uint32 dts)
{
	wlc_mcnx_info_t *mcnx = cfg->wlc->mcnx;
	int bss = wlc_mcnx_BSS_idx(mcnx, cfg);

	if (dts) {
		// in order for dts to work, lower shm can't be zero
		// so make it >= 1us.
		dts |= 1;
	}

	// write TX stop deadline timestamp, hi 1st
	wlc_mcnx_write_shm(mcnx,  M_P2P_TX_STOP_TS(bss, 1), (uint16)(dts >> 16));
	wlc_mcnx_write_shm(mcnx,  M_P2P_TX_STOP_TS(bss, 0), (uint16)(dts));

	// readback
	DTS_INFO(("\t\t wrote dts_shm(bss_idx:%d):%08x, readback:%04x:%04x\n\n",
		wlc_mcnx_BSS_idx(mcnx, cfg), dts,
		wlc_mcnx_read_shm(mcnx, M_P2P_TX_STOP_TS(bss, 1)),
		wlc_mcnx_read_shm(mcnx, M_P2P_TX_STOP_TS(bss, 0))));
}

/*	--- smart ampdu TX dts (Dedline Time Stamp) utility function ---
	wlc modules that register time-slots with msch scheduler must periodically
	call this function with a non zero "dts" argument as follows:
	for Flex-Flex timeslots (STA) 	-from MSCH_CT_ON_CHAN type callback,
	for Fix-Fix timeslots (NAN, AIBSS) -from SLOT_START type callback
	and pass the slot duration "dts" into this function.
	The "dts" is expressed in microseconds and instructs d11 ucode to stop
	all TX operations, specifically AMPDU aggregation and framebursting
	"dts" microseconds from now. All frames that are already in d11 memory
	but haven't been transmitted will be suppressed.
		NOTE: NULL-QOS frames are excluded from this rule.
	If "dts" parameter is -1, time limit for TX operations will be disabled
	for a single bsscfg passed in "cfg" parameter.
	If "cfg" parameter is NULL, dts will be disabled for all bsscfgs.
*/
void wlc_set_cfg_tx_stop_time(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int32 dts)
{
	uint32 tsf_l, tsf_h, dts_h = 0, dts_l = 0;

	// if dts -1 clear bsscfg SHM
	...
	if (dts > 0) {
		...
		dts_h = tsf_h; dts_l = tsf_l;
		wlc_uint64_add(&dts_h, &dts_l, 0, dts);

		if (dts_h != tsf_h) {
			/* handle dts_l overflow */
			 dts_l = 0; // don't use dts
		}

		DTS_INFO(("\t\t dts:%uus, add to local_tsf:%08x%08x, + dts -> %08x%08x\n",
		dts, tsf_h, tsf_l, dts_h, dts_l));
	}

	// if cfg is NULL clear DTS SHMs for all bsscfgs
	if (!cfg) {
		int i;
		dts_l = 0; // ignore dts value
		FOREACH_BSS(wlc, i, cfg) {
			write_dts_shm(cfg, dts_l);
		}
	} else {
		write_dts_shm(cfg, dts_l);
	}
