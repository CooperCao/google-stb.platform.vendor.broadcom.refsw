...
#ifdef MACOSX_DHD
// FD doesn't need any head or tail room
#define TXOFF 0
#define TXTAIL 0
...
/// APPLE APPLE APPLE - corecapture
#include <IOKit/corecapture/CCDataPipe.h>
#include <IOKit/corecapture/CCDataPipeShared.h>
#include <IOKit/corecapture/CCDataStream.h>
#include <IOKit/corecapture/CCLogPipe.h>
#include <IOKit/corecapture/CCLogPipeShared.h>
#include <IOKit/corecapture/CCLogStream.h>
#include <IOKit/corecapture/CCLogStreamShared.h>
#include <IOKit/corecapture/CCPipe.h>
#include <IOKit/corecapture/CCPipeShared.h>
#include <IOKit/corecapture/CCStream.h>
#include <IOKit/corecapture/CCStreamShared.h>
...
//
// DEBUGLOG:
//   Send message out to corecapture logs unconditionally AND:
//
//   Bit postion
//   -----------
//             0  Send message out to IOLog with timestamp
//             1  Send message out to kprintf
//
//   To enable add to the boot-args "bcom.logging"
//
//   For example:
//     Added to boot-args "bcom.logging=0x3", sends out to corecapture, IOLog, and kprintf.
//
#define DEBUGLOG(args...)                                               \
do {                                                                    \
	if( !g_brcm_controller ) break;                                 \
                                                                        \
	AirPort_Brcm43xx *_obj = (AirPort_Brcm43xx *)g_brcm_controller; \
                                                                        \
	/* send to corecapture logs */                                  \
	WIFICC_LOGDEBUG(args);                                          \
                                                                        \
	if ((_obj->_logging & (1 << 0)) )                               \
	{                                                               \
		osl_iolog_withtimestamp args;                           \
	}                                                               \
                                                                        \
	if ((_obj->_logging & (1 << 1)) )                               \
	{                                                               \
		osl_fwlog_kprintf args;                                 \
	}                                                               \
} while(0);

//
// RELEASELOG:
//   Send message out to corecapture logs/system.log (IOLog) unconditionally AND:
//
//   Bit postion
//   -----------
//             1  Send message out to kprintf
...
//
//   For example:
//     Added to boot-args "bcom.logging=0x2", sends out to corecapture/IOLog, and kprintf.
//
#define RELEASELOG(args...)                                             \
do {                                                                    \
	if( !g_brcm_controller ) break;                                 \
                                                                        \
	AirPort_Brcm43xx *_obj = (AirPort_Brcm43xx *)g_brcm_controller; \
...
#define WL_POWERCYCLE_OFF_DURATION             300             // minimum delay in between off and on
#define WL_POWERCYCLE_ON_DURATION              300             // minimum delay after power on
#ifdef SCAN_JOIN_TIMEOUT
#define JOIN_FAIL_TIMEOUT						9
#define SCAN_TIMEOUT_FAIL						18
#endif
...
struct wl_link_down_reason {
    uint8_t                             isDisconnectInVoluntary;
    int			reason;
    uint32_t	subreason;
};

struct wl_link_up_reason {//currently not used
};
...
typedef struct intf_info_blk {
	...
	bool isLinkDown;				// cached value for setLinkState
	union {
		struct wl_link_up_reason link_up_reason;
		struct wl_link_down_reason link_down_reason;
	}link_changed_reason;			// cached value for setLinkState
} intf_info_blk_t;
...
{
    u_int32_t   version;
    char        countryCode[APPLE80211_MAX_CC_LEN];
    u_int16_t   numChanSpecs;
    u_int16_t   chanSpec[APPLE80211_MAX_CHANNELS];
    u_int8_t    chanNum[APPLE80211_MAX_CHANNELS];
    u_int8_t    indoorRestricted[APPLE80211_MAX_CHANNELS];
    u_int8_t    radarDFS[APPLE80211_MAX_CHANNELS];
    u_int8_t    passive[APPLE80211_MAX_CHANNELS];
    u_int8_t    support40MHz[APPLE80211_MAX_CHANNELS];
    u_int8_t    support80MHz[APPLE80211_MAX_CHANNELS];
    u_int32_t   perChanInfo[APPLE80211_MAX_CHANNELS];
    u_int32_t   chanBitmap[APPLE80211_MAX_CHANNELS];
};
#endif
#if VERSION_MAJOR > 9
...
extern "C" {
extern void wl_macos_watchdog(void *hdl);
}
extern SInt32 version_check(u_int32_t version, const char* ioname);
...
extern const struct name_entry apple80211_ionames[];
extern const struct name_entry apple80211_cipher_type_names[];
extern const struct name_entry apple80211_authtype_lower_names[];
extern const struct name_entry apple80211_authtype_upper_names[];
extern const struct name_entry apple80211_apmode_names[];
extern const struct name_entry apple80211_scan_type_names[];
extern const struct name_entry apple80211_msgnames[];
extern const struct name_entry apple80211_statenames[];
extern const struct name_entry apple80211_power_state_names[];
extern const struct name_entry apple80211_locale_names[];
extern const struct name_entry apple80211_unit_names[];
extern const struct name_entry dot11_status_names[];
extern const struct name_entry IO80211LinkState_names[];

extern const bcm_bit_desc_t apple80211_phymode_flags[];

/*
 * Debug Name lookup utility.
 * Given a name table and an ID, will return the name matching the ID, or a string with the raw ID.
 */
extern const char* lookup_name(const struct name_entry* tbl, int id);
...
#ifdef MACOSX_DHD
/**
 * BCMDBG is the flag which has to be enabled for Debug builds in MACOS NIC Mode
 * DHD_DEBUG is the flag which has to be enabled for Debug builds in DHD for Linux..etc
 * MACOSDHD is leveraging the code from both the NIC mode(wl_macosx.cpp) and the DHD non OSL code.
 * So both the flags(BCMDBG, DHD_DEBUG) must be defined for debug builds.
 */
#if ((defined(BCMDBG) && !defined(DHD_DEBUG)) || (!defined(BCMDBG) && \
	defined(DHD_DEBUG)))
#error "Should define both BCMDBG and DHD_DEBUG."
#endif /* BCMDBG && !DHD_DEBUG || !BCMDBG && DHD_DEBUG */
#endif /* MACOSX_DHD */
...
#define LINKDEBUGFLAGS_WATCHDOG                             ( 1 <<   0)
#define LINKDEBUGFLAGS_TXPACKET                             ( 1 <<   4)
#define LINKDEBUGFLAGS_TXPACKETVERBOSE                      ( 1 <<   5)
#define LINKDEBUGFLAGS_RXPACKET                             ( 1 <<   8)
#define LINKDEBUGFLAGS_RXPACKETVERBOSE                      ( 1 <<   9)


#define LINKDEBUGENABLED( _flags, _mask )                   (((_flags) & (_mask)) ? 1 : 0)
#define LINKDEBUGPRINT( _flags, _mask, _format, ...)        \
do {                                                        \
	if( LINKDEBUGENABLED( _flags, _mask ) ) {           \
...
public:
...
#ifdef MACOSX_DHD
	void getOsIfaceName(char *ifname);
	OSObject *getPrimaryIface(void);
	virtual bool wlSendPkt(void *sdu, void *iface) = 0;
	virtual bool isAssociated() = 0;
	virtual bool isUp() = 0;
	virtual bool isBusUp() = 0;
	// IOVAR and IOCTL API functions
	virtual int wlIovarOp(const char *name, void * params, size_t p_len, void *buf, size_t len, bool set,
				OSObject *iface) = 0;
	virtual int wlIovarGetInt(const char *name, int *arg) const = 0;
	virtual int wlIoctl(uint cmd, void *arg, size_t len, bool set, OSObject *iface) = 0;
	virtual int wlIoctlGet(uint cmd, void *arg, size_t len, OSObject *iface) const = 0;
#else
	virtual int wlIovarOp(const char *name, void * params, size_t p_len, void *buf, size_t len, bool set,
				OSObject *iface);
...
#if defined(ENABLE_CORECAPTURE)
	CCLogPipe	*_ccLogPipe;
	CCLogStream	*_ccLogStream;

	CCDataPipe	*_ccDataPipe;
	CCDataStream    *_ccDataStream;
	OSData          *_FWData;
	OSData          *_UCMData;
	bool		initializeCoreCapture();

	static void	cc_callback_capture(const OSObject *target, const char* reason, const CCTimestamp *timestamp);
	static void	cc_callback_setLogLevel(const OSObject *target, enum CCStreamLogLevel level);
	static void	cc_callback_setLogFlags(const OSObject *target, CCStreamLogFlag flags);
	static void	cc_callback_setConsoleLogLevel(const OSObject *target, enum CCStreamLogLevel level);
	static void	cc_callback_setConsoleLogFlags(const OSObject *target, CCStreamLogFlag flags);

	char _ccReason[CC_REASON_STR_LEN];
	bool _ccCapturePending;
#endif /* ENABLE_CORECAPTURE */

#ifdef CCA_STATS
	bool		setupCCAChannelQualityEvent(bool start);
#endif /* CCA_STATS */
	...
	uint64		_powerStateWillChangeToTimeStamp;
	uint64		_powerStateDidChangeToTimeStamp;
	uint64		_setPowerStateTimeStamp;
	uint64		_macosWatchdogTimeStamp;
	uint64		_configHandlerTimeStamp;

	uint32		_linkdebug;  /* Helpful boot-args "io80211.linkdebug" link-level connectivity issues, netboot/etc */
...
struct ether_addr configured_bssid;	// setBSSID() value for association restrictions
...
#ifdef MACOSX_DHD
	si_t    *sih;
	struct wl_timer	*watchdog_timer;
#endif /* MACOSX_DHD */
#ifndef MACOSX_DHD
	...
	SInt32		getHW_SUPPORTED_CHANNELS( OSObject * interface, struct apple80211_sup_channel_data  * sd );
#endif
	...
	SInt32		getMIMO_POWERSAVE( OSObject * interface, struct apple80211_powersave_data * pd )
			{ BCM_REFERENCE(interface); BCM_REFERENCE(pd); return EOPNOTSUPP; }
	...
	SInt32		getRIFS( OSObject * interface, struct apple80211_rifs_data * rd )
			{ BCM_REFERENCE(interface); BCM_REFERENCE(rd); return EOPNOTSUPP; }
	...
	SInt32		setCOUNTRY_CODE( OSObject * interface, struct apple80211_country_code_data * ccd );
	bool		isCntryDefaultSupported(uint boardid);
#ifdef CNTRY_DEFAULT
	bool		isCntryRestricted(char *country);
	SInt32		resetAutoCountry(char *country);
	SInt32		setRestrictedCntryCode(char *country, bool restricted);
#endif /* CNTRY_DEFAULT */
#ifdef APPLE80211_IOC_MCS_VHT
	...
	SInt32		btc_profile_copy( wlc_btc_select_profile_t *btc_profile, int band, int profile_index );
#endif /* WLBTCPROF && APPLE80211_IOC_BTCOEX_PROFILES */
	...
	SInt32		get_channels_by_country(struct channels_info *param);
	SInt32		is_channel_supported(UInt32 sel_channel, char *country);
#endif // VERSION_MAJOR > 10
#endif // IO80211P2P
#ifdef APPLE80211_IOC_TX_CHAIN_POWER
...
#endif
	virtual void	macosWatchdog();
#ifdef APPLE80211_IOC_LINK_CHANGED_EVENT_DATA
	SInt32	getLINK_CHANGED_EVENT_DATA( OSObject * interface, struct apple80211_link_changed_event_data * ed);
#endif
#ifdef APPLE80211_IOC_BTCOEX_OPTIONS
	SInt32	setBTCOEX_OPTIONS(OSObject *  interface, struct apple80211_btc_options_data* btc_opt );
	SInt32	getBTCOEX_OPTIONS(OSObject *  interface, struct apple80211_btc_options_data* btc_opt );
#endif
#ifdef APPLE80211_IOC_SUPPORTED_COUNTRY_CODES
	SInt32 getSUPPORTED_COUNTRY_CODES(OSObject *  interface, struct apple80211_supported_country_codes_data *sccd);
#endif /* APPLE80211_IOC_SUPPORTED_COUNTRY_CODES */
#ifdef APPLE80211_IOC_COUNTRY_TX_POWER_LIMITS
	SInt32 getCOUNTRY_TX_POWER_LIMITS(OSObject *  interface, struct apple80211_country_tx_power_limits_data *cpld);
...
#ifdef WL_LOCK_CHECK
	int wl_lock_count;
	void * wl_lock_thread_id;

	/* Spin lock to protect the lock operation */
#ifdef USE_SPIN_LOCK
	lck_grp_attr_t *wl_spin_lock_grp_attr;
	lck_grp_t *wl_spin_lock_grp;
	lck_attr_t *wl_spin_lock_attr;
	lck_spin_t *wl_spin_lock;
#endif /* USE_SPIN_LOCK */

	/* debug data to trace lock failures */
#if defined(LOCK_DEBUG)
	void * prev_lock_backtrace[MAX_BACKTRACE_DEPTH];
	int prev_lock_backtrace_depth;

	void * prev_unlock_backtrace[MAX_BACKTRACE_DEPTH];
	int prev_unlock_backtrace_depth;
#endif /* LOCK_DEBUG */

#endif /* WL_LOCK_CHECK */

#ifdef WL_BSSLISTSORT
	bool wlSortBsslist(wlc_bss_info_t **bip, uint nbip);
#endif
	...
	IOReturn callPlatformFunction( const char * funcName, bool val );
#endif /* AAPLPWROFF */

#ifdef NEED_HARD_RESET
	int wl_fault_powercycle(void * wl);
	void powerCycleOffOnThread( void * arg0, void * arg1, void * arg2, void * arg3 );
	static IOReturn powerCycleOffOnThreadGated( OSObject * owner, void * arg0, void * arg1, void * arg2, void * arg3 );

	volatile bool   _powerCycleOffOnThreadRunning;
	volatile bool   _powerCycleInProgress;
	bool            _powerCycleThreadWaiting;
	bool            _powerOffOnThreadBlockedForWake;
	thread_call_t   _pwrCycleOffOnThreadCall;
...
protected:
	...
	void		stop_local();
	virtual bool		wlcStart();
	virtual void		wlcStop();
	...
	IOPCIEventSource * pcieEventSource;
	void    receivePCIEvent(IOPCIEventSource * es, const IOPCIEvent * event);
#endif
...
private:
...
#ifdef BCMDBG
	void	dump_apple80211_channels(uint count, struct apple80211_channel *channels);
	void	dump_apple80211_network_data(struct apple80211_network_data * nd);
	void	dump_apple80211_key(struct apple80211_key * key);
#endif
	...
	SInt32 appendProhibited_channels(wl_uint32_list_t *list);
#if defined(WLBTCPROF) && defined(APPLE80211_IOC_BTCOEX_PROFILES)
...
#ifdef MACOSX_DHD
	bool	scanresult_list_filter(struct ether_addr *bssid);
	void	scanAddResult(wl_bss_info_t *scan_results, uint count);
	void	scanComplete(wl_bss_info_t *scan_results, uint count);
	void	ratesetModulations(wl_rateset_t *rs, bool onlyBasic,
			bool *cck, bool *ofdm, bool *other);
	void	scanConvertResult(wl_bss_info_t * bi, struct apple80211_scan_result *asr);
#else
	...
	int	applechannel2chanspec(const struct apple80211_channel *channel, chanspec_t *chanspec);
	int	setAP(bool ap);
...
public:
	...
	void bssinfoToScanresult(wl_bss_info_t * bi, struct apple80211_scan_result *asr);

	void wl_compose_postmessage_tlv(wlc_event_t *evt,
...
public:
	int _platformCheckSkip;
	int _panicOnBPT;
	int _checkAllWrappers;
	int32_t _fatalErrRecovery;
	uint32_t _logging;

	static int getIOPMPowerSourceKey( const char *key, char *buffer, int len, const char *fmt = NULL );
	static int getIOPMSystemSleepType( char *buffer = NULL, int len = 0 );
	static int logIOPMPowerSourceInformation( char *prefix );

};

/* APPLE APPLE APPLE - this is a temporary change
 * To port over the APPLE80211_M_DRIVER_AVAILABLE events from iOS,
 * The following definitions are copied from AppleBCMWLANReturn.h for now.
 * The AppleBCMWLANReturn* definitions should be moved here.
 */
enum AppleBCMWLANReturnOffset
{
    kAppleBCMWLANReturnCodeOffset    = 0,
    kAppleBCMWLANReturnTypeOffset    = 8,
};

enum AppleBCMWLANReturnMask
{
    kAppleBCMWLANReturnCodeMask    = (0xFF << kAppleBCMWLANReturnCodeOffset),
    kAppleBCMWLANReturnTypeMask    = (0x3F << kAppleBCMWLANReturnTypeOffset),
};

/** Identifies the category of an AppleBCMWLAN return code. */
enum AppleBCMWLANReturnType
{
    kAppleBCMWLANReturnTypeGeneric   = (0x0 << kAppleBCMWLANReturnTypeOffset),
    kAppleBCMWLANReturnTypeFirmware  = (0x1 << kAppleBCMWLANReturnTypeOffset),
    kAppleBCMWLANReturnTypeChip      = (0x2 << kAppleBCMWLANReturnTypeOffset),
    kAppleBCMWLANReturnTypeBus       = (0x3 << kAppleBCMWLANReturnTypeOffset),
    kAppleBCMWLANReturnTypeCommand   = (0x4 << kAppleBCMWLANReturnTypeOffset),
    kAppleBCMWLANReturnTypeCore      = (0x5 << kAppleBCMWLANReturnTypeOffset),
    kAppleBCMWLANReturnTypeScan      = (0x6 << kAppleBCMWLANReturnTypeOffset),
    kAppleBCMWLANReturnTypeFWLoading = (0x7 << kAppleBCMWLANReturnTypeOffset),
    kAppleBCMWLANReturnTypeNet       = (0x8 << kAppleBCMWLANReturnTypeOffset),
    kAppleBCMWLANReturnTypeJoin      = (0x9 << kAppleBCMWLANReturnTypeOffset),
    kAppleBCMWLANReturnTypeDatapath  = (0xa << kAppleBCMWLANReturnTypeOffset),
    kAppleBCMWLANReturnTypeH5        = (0xb << kAppleBCMWLANReturnTypeOffset),
    kAppleBCMWLANReturnTypeDownload  = (0xc << kAppleBCMWLANReturnTypeOffset),
};

/** AppleBCMWLAN error codes have the following structure:
 *
 *  [ sys_iokit | sub_iokit_vendor_specific | AppleBCMWLANReturnType | error_code ]
 * 31          26                          14                        8            0
 *
 * This results in an error code that starts with 0xE3FF8000.
 */
#define APPLEBCMWLAN_RETURN(type, /*UInt8*/ code) \
  iokit_vendor_specific_err((((enum AppleBCMWLANReturnType)type) & kAppleBCMWLANReturnTypeMask) | (((UInt8)code) & kAppleBCMWLANReturnCodeMask))

enum AppleBCMWLANCoreReturn
{
    kAppleBCMWLANCoreReturnWatchdog              = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 1),
    kAppleBCMWLANCoreReturnBootedImage           = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 2),
    kAppleBCMWLANCoreReturnUserInduced           = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 3),
    kAppleBCMWLANCoreReturnUserInducedWatchdog   = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 4),
    kAppleBCMWLANCoreReturnUserInducedDump       = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 5),
    kAppleBCMWLANCoreReturnWatchdogFailed        = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 6),
    kAppleBCMWLANCoreReturnPowerAlert            = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 7),
    kAppleBCMWLANCoreReturnSuperclassInduced     = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 8),
    kAppleBCMWLANCoreTxStatusOverflow            = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 9),
    kAppleBCMWLANCoreTxStatusBeforeWriteComplete = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 10),
    kAppleBCMWLANCoreDissassocFailed             = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 11),
    kAppleBCMWLANCoreEventMissed                 = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 12),
    kAppleBCMWLANCoreModuleInstanceMissed        = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 13),
    kAppleBCMWLANCoreBeaconLossWatchdog          = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 14),
    kAppleBCMWLANCoreReturnUnknown               = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 15),
    kAppleBCMWLANCoreReturnPSSync                = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 16),
    kAppleBCMWLANCoreReturnPsmWD                 = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 17),
    kAppleBCMWLANCoreReturnMacWake               = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 18),
    kAppleBCMWLANCoreReturnMacSuspend            = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 19),
    kAppleBCMWLANCoreReturnSpinWait              = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 20),
    kAppleBCMWLANCoreReturnBusError              = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 21),
    kAppleBCMWLANCoreReturnDeviceRemoved         = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 22),
    kAppleBCMWLANCoreReturnPcieAER               = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 23),
    kAppleBCMWLANCoreReturnFwTrap                = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 24),
    kAppleBCMWLANCoreReturnFifoErr               = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 25),
    kAppleBCMWLANCoreReturnTxStatus              = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 26),
    kAppleBCMWLANCoreReturnMqError               = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 27),
    kAppleBCMWLANCoreReturnThyTxErrThresh        = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 28),
    kAppleBCMWLANCoreReturnUserForced            = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 29),
    kAppleBCMWLANCoreReturnFullReset             = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 30),
    kAppleBCMWLANCoreReturnApBeacon              = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 31),
    kAppleBCMWLANCoreReturnPMExcessWake          = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 32),
    kAppleBCMWLANCoreReturnNoClock               = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 33),
    kAppleBCMWLANCoreReturnSwAssert              = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 34),
    kAppleBCMWLANCoreReturnScanTimeout           = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 35),
    kAppleBCMWLANCoreReturnJoinTimeout           = APPLEBCMWLAN_RETURN(kAppleBCMWLANReturnTypeCore, 36)
};
