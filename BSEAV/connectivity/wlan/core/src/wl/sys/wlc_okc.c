...
/* iovar table */
enum wlc_okc_iov {
	IOV_OKC_ENABLE = 1,        /* enable/disable okc feature */
	IOV_OKC_INFO_PMK = 2,       /* pmk for specfic AP  */
	IOV_RCC_MODE = 3,
	IOV_RCC_CHANNELS = 4,
	IOV_OKC_LAST
};
...
#ifdef WL_OKC
	{"okc_enable", IOV_OKC_ENABLE, 0, 0, IOVT_BOOL, 0},
	{"okc_info_pmk", IOV_OKC_INFO_PMK, 0, 0, IOVT_BUFFER, 0},
#endif
#ifdef WLRCC
	{"roamscan_mode", IOV_RCC_MODE, 0, 0, IOVT_BOOL, 0},
	{"roamscan_channels", IOV_RCC_CHANNELS, 0, 0, IOVT_BUFFER, 0},
#endif
	{NULL, 0, 0, 0, 0, 0}
};
...
static int
wlc_okc_rcc_doiovar(void *context, uint32 actionid,
...
/* in case of WLRCC only */
	...
	switch (actionid) {
		...
		case IOV_GVAL(IOV_OKC_ENABLE):
			*((uint32*)a) = wlc->pub->cmn->_okc;
			break;

		case IOV_SVAL(IOV_OKC_ENABLE):
			if (BSSCFG_STA(bsscfg))
				wlc->pub->cmn->_okc = (int_val  == 1);
			break;
		...
		case IOV_SVAL(IOV_OKC_INFO_PMK): {
			...
			if (SUP_ENAB(wlc->pub) && BSSCFG_STA(bsscfg) &&
				...
				for (i = 0; i < (WSEC_MAX_PSK_LEN/2); i++) {
					c += snprintf(c, sizeof(pmkstr), "%02x", ((uint8 *)a)[i]);
				}
	...
	wl_join_assoc_params_t *assoc_param)
{
	...
	/* clear the existing rcc channel list. */
	memset(roam->rcc_channels, 0, sizeof(chanspec_t) * MAX_ROAM_CHANNEL);
