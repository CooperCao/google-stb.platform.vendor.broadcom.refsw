...
static const bcm_iovar_t lpc_iovars[] = {
	...
	(0), 0, IOVT_BUFFER, sizeof(lpc_params_t)
	},
...
static int wlc_lpc_doiovar(void *hdl, uint32 actionid,
...
#if defined(WL11AC) && defined(WL_LPC_DEBUG)
...
static int
wlc_lpc_doiovar(void *hdl, uint32 actionid,
	void *p, uint plen, void *a, uint alen, uint val_size, struct wlc_if *wlcif)
{
...
#endif /* WL11AC && WL_LPC_DEBUG */

/* Extern functions */
void
wlc_lpc_scb_update(lpc_info_t *lpci, lcb_t *state)
{
	ASSERT(state);
	ASSERT(lpci);
...
wlc_lpc_capable_chip(wlc_info_t *wlc)
{
	if (WLCISACPHY(wlc->band) || WLCISLCN20PHY(wlc->band)) {
		...
		if ((CHIPID(wlc->pub->sih->chip) == BCM4345_CHIP_ID) ||
			...
			(CHIPID(wlc->pub->sih->chip) == BCM43012_CHIP_ID) ||
