...
static int mux_doiovar(void *hdl, uint32 actionid,
                       void *p, uint plen,
                       void *a, uint alen,
                       uint vsize,
                       struct wlc_if *wlcif);
 ...
 */
wlc_mux_info_t*
BCMATTACHFN(wlc_mux_module_attach)(wlc_info_t *wlc)
{
	wlc_mux_info_t *muxi;
	osl_t *osh = wlc->osh;
 ...
 */
static int
mux_doiovar(void *hdl, uint32 actionid,
            void *p, uint plen,
            void *a, uint alen,
            uint vsize,
            struct wlc_if *wlcif)
...
wlc_mux_output(wlc_mux_t *mux, uint request_time, struct spktq *output_q)
{
	...
	if (src == NULL) {
		WL_NONE(("%s: MUX %p req %d, exit with no active sources\n",
		         __FUNCTION__, OSL_OBFUSCATE_BUF(mux), request_time));
		return 0;
	}
	...
	while (supplied_time < request_time) {
		...
		WL_NONE(("%s: MUX %p req %d sup %d src %p\n", __FUNCTION__,
		         OSL_OBFUSCATE_BUF(mux), request_time, supplied_time,
		...
		if (m->next == NULL || m->prev == NULL) {
			...
			if (m->prev->next != m) {
				bcm_bprintf(b, "Prev (%p) next ptr (%p) does not match cur (%p)\n",
					...
					OSL_OBFUSCATE_BUF(m->prev->next), OSL_OBFUSCATE_BUF(m));
			}
			if (m->next->prev != m) {
				bcm_bprintf(b, "Next (%p) prev ptr (%p) does not match cur (%p)\n",
					OSL_OBFUSCATE_BUF(m->prev),
					OSL_OBFUSCATE_BUF(m->next->prev), OSL_OBFUSCATE_BUF(m));
			}
		}
		...
		if (m == head) {
			if (i != count) {
				bcm_bprintf(b, "ERR: back at head pointer %p, but count %d did not "
				            "reach expected\n",
				            OSL_OBFUSCATE_BUF(head), i, count);
			}
			break;
		}
...
mux_source_dump(mux_source_t *m, struct bcmstrbuf *b)
{
	bcm_bprintf(b, "ptr %p flags 0x%x time_bal %d\n",
	            OSL_OBFUSCATE_BUF(m), m->flags, m->time_balance);
	bcm_bprintf(b, "output_fn %p ctx %p\n",
		OSL_OBFUSCATE_BUF(m->output_fn), OSL_OBFUSCATE_BUF(m->ctx));
