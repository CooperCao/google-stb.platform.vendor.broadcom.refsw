...
BCMATTACHFN(wlc_pkt_filter_attach)(void *context)
{
	...
	info = obj_registry_get(wlc->objr, OBJR_PKT_FILTER);
	...
	if (info == NULL) {
		...
		obj_registry_set(wlc->objr, OBJR_PKT_FILTER, info);
	}
...
BCMATTACHFN(wlc_pkt_filter_detach)(wlc_pkt_filter_info_t *info)
{
	...
	if (info == NULL)
		return;
	/* unreference object registry */
	ref_cnt = obj_registry_unref(info->wlc->objr, OBJR_PKT_FILTER);
	if (ref_cnt == 0) {
		obj_registry_set(info->wlc->objr, OBJR_PKT_FILTER, NULL);
	}
	/* De-register this module. */
	wlc_module_unregister(info->wlc->pub, "pkt_filter", info);

	/* Free allocated packet filter engine state. */
	if (NULL != info && !ref_cnt) {
...
bool wlc_pkt_filter_recv_proc(wlc_pkt_filter_info_t *info, const void *sdu)
{
	...
	if (info->toss_icmp) {
		...
		if (WLEXTSTA_ENAB(info->wlc->pub) && !PKT_DOT3(WLCOSH(info), sdu)) {
			if ((ltoh16(((struct dot11_header *)pkt)->fc)
				...
				/* 802.11 QOS DATA packet */
				offs = DOT11_MAC_HDR_LEN + DOT11_QOS_LEN + SNAP_HDR_LEN;
			} else {
				/* 802.11 type packet */
	...
	if (APF_ENAB(info->wlc->pub)) {
		/*
		* In case of APF, the filters are executed one after the other, and
		* the packet is accepted only if none of the filters drop it.
		*/
		if (info->apf_program_tot_size && match) {
			filter = info->enabled_list;
			while (NULL != filter) {
	...
	while (NULL != filter) {
		if (filter->type == WL_PKT_FILTER_TYPE_PATTERN_MATCH_TIMEOUT &&
			...
			if (time_since_last_pkt >= (pattern_timeout->timeout * 1000)) {
				/* send timeout event */
				wlc_bss_mac_event(info->wlc, info->wlc->cfg, WLC_E_PKT_FILTER,
					NULL, WLC_E_PKT_FILTER_TIMEOUT, filter->id, 0, 0, 0);
...
#if defined(APF)
	if (APF_ENAB(info->wlc->pub)) {
		if (removed_filter->type == WL_PKT_FILTER_TYPE_APF_MATCH) {
