...
enum {
	...
	IOV_IBSS_ROUTE_TABLE	/* Update route table */
};

static const bcm_iovar_t ipfo_iovars[] = {
	{"ipfo", IOV_IPFO, (IOVF_SET_DOWN), 0, IOVT_BOOL, 0},
	{"ip_route_table", IOV_IP_ROUTE_TABLE,
	(0), 0, IOVT_BUFFER, 0
	},
	/* This need to be removed after DHD modification */
	{"ibss_route_tbl", IOV_IBSS_ROUTE_TABLE,
	(0), 0, IOVT_BUFFER, 0
	},
	{NULL, 0, 0, 0, 0, 0}
};
...
static int wlc_ipfo_doiovar(void *hdl, uint32 actionid,
...
static int
wlc_ipfo_doiovar(void *hdl, uint32 actionid,
	void *p, uint plen, void *a, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	switch (actionid) {
		...
		case IOV_GVAL(IOV_IBSS_ROUTE_TABLE):
		{
		...
		case IOV_SVAL(IOV_IBSS_ROUTE_TABLE):
		{
