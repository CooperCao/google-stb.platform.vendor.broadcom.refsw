...
#ifdef WLAMPDU_HW
	pkt = MALLOCZ(ampdu_tx->wlc->osh, AGGFIFO_CAP);
#else
	pkt = MALLOCZ(ampdu_tx->wlc->osh, AMPDU_MAX_PKTS);
#endif
	if (pkt == NULL)
		return BCME_NOMEM;
...
#ifdef WLAMPDU_HW
	MFREE(ampdu_tx->wlc->osh, pkt, AGGFIFO_CAP);
#else
	MFREE(ampdu_tx->wlc->osh, pkt, AMPDU_MAX_PKTS);
#endif
	return err;
...
free_and_next:
	...
	if (update_ratesel) {
		...
		if (rs_txs.tx_cnt[0] == 0 && fbr_cnt == 0) {
			 ...
			 */
			if (SCB_PS_PRETEND(scb) && !SCB_PS_PRETEND_THRESHOLD(scb) && p != NULL) {
