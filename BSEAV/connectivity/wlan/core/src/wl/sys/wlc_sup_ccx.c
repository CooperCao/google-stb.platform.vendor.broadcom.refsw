...
typedef struct bss_ccxsup_info {
	wlc_info_t *wlc;		/* pointer to main wlc structure */
	wlc_pub_t *pub;			/* pointer to wlc public portion */
	void *wl;			/* per-port handle */
	osl_t *osh;			/* PKT* stuff wants this */
...
#define CCXSUP_BSSCFG_CUBBY_LOC(ccxsup, cfg) \
	((bss_ccxsup_info_t **)BSSCFG_CUBBY(cfg, (ccxsup)->cfgh))
...
static void wlc_leap_rogue_clear(bss_ccxsup_info_t *ccxsup_bss);
static void wlc_leap_ddp_rogue_rpt(bss_ccxsup_info_t *ccxsup_bss);
static void wlc_leapsup_sendeap(bss_ccxsup_info_t *ccxsup_bss);
static void wlc_leapsup_timer(void *arg);
static void wlc_leapsup_timer_callback(bss_ccxsup_info_t *ccxsup_bss);
...
static int wlc_ccxsup_doioctl(void *ctx, uint cmd, void *arg, uint len, struct wlc_if *wlcif);
/* Handle WLC_[GS]ET_LEAP_LIST ioctls */
static int wlc_ccx_set_leap_list(bss_ccxsup_info_t *ccxsup_bss, wlc_bsscfg_t *cfg, void *pl);
static int wlc_ccx_get_leap_list(bss_ccxsup_info_t *ccxsup_bss, wlc_bsscfg_t *cfg, void *pl);
...
BCMATTACHFN(wlc_ccxsup_attach)(wlc_info_t *wlc)
{
	...
	ccxsup_info->wlc = wlc;
	ccxsup_info->pub = wlc->pub;
	ccxsup_info->wl = wlc->wl;
	ccxsup_info->osh = wlc->osh;

	/* reserve cubby in the bsscfg container for per-bsscfg private data */
	if ((ccxsup_info->cfgh = wlc_bsscfg_cubby_reserve(wlc, sizeof(bss_ccxsup_info_t *),
		NULL /* wlc_sup_init */, wlc_ccxsup_deinit, NULL,
		(void *)ccxsup_info)) < 0) {
		WL_ERROR(("wl%d: %s: wlc_bsscfg_cubby_reserve() failed\n",
			UNIT(ccxsup_info), __FUNCTION__));
			goto err;
	}
...
err:
	MODULE_DETACH(ccxsup_info, wlc_ccxsup_detach);
	return NULL;
}
...
BCMATTACHFN(wlc_ccxsup_detach)(wlc_ccxsup_info_t *ccxsup_info)
{
	...
	(void)wlc_module_remove_ioctl_fn(ccxsup_info->wlc->pub, ccxsup_info);
	wlc_bsscfg_updown_unregister(ccxsup_info->wlc, wlc_ccxsup_bss_updn, ccxsup_info);
	wlc_sup_up_down_unregister(ccxsup_info->wlc, wlc_ccxsup_up_down_callbk, ccxsup_info);
	/* wlc_module_unregister(ccxsup_info->pub, "idsup", ccxsup_info); */
	MFREE(ccxsup_info->osh, ccxsup_info, sizeof(wlc_ccxsup_info_t));
}

static int
wlc_ccxsup_doioctl(void *ctx, uint cmd, void *arg, uint len, struct wlc_if *wlcif)
{
	...
	case WLC_SET_LEAP_LIST:
		err = wlc_ccx_set_leap_list(ccxsup_bss, bsscfg, pval);
		break;
	case WLC_GET_LEAP_LIST:
		ASSERT(pval != NULL);
		err = wlc_ccx_get_leap_list(ccxsup_bss, bsscfg, pval);
		break;
	default:
...
wlc_ccxsup_init(void *ctx, sup_init_event_data_t *evt)
{
	...
	if (BSSCFG_AP(evt->bsscfg))
		return BCME_NOTSTA;
	if (!(ccxsup_bss = (bss_ccxsup_info_t *)MALLOC(ccxsup_info->osh,
		sizeof(bss_ccxsup_info_t)))) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
			UNIT(ccxsup_info), __FUNCTION__, MALLOCED(ccxsup_info->osh)));
			goto err;
	}
	bzero(ccxsup_bss, sizeof(bss_ccxsup_info_t));
	*pccxsup_bss = ccxsup_bss;

	ccxsup_bss->m_handle = wlc->ccxsup;
	ccxsup_bss->cfg = evt->bsscfg;
	ccxsup_bss->wlc = wlc;
	ccxsup_bss->osh = ccxsup_info->osh;
	ccxsup_bss->wl = ccxsup_info->wl;
	ccxsup_bss->pub = ccxsup_info->pub;
	ccxsup_bss->wpa = evt->wpa;
	ccxsup_bss->wpa_info = evt->wpa_info;

	if (CCX_ENAB(wlc->pub)) {
		if (!(ccxsup_bss->leap = MALLOC(ccxsup_bss->osh, sizeof(leap_sup_t)))) {
			WL_ERROR(("wl%d: wlc_ccxsup_init: out of memory, malloced %d bytes\n",
				UNIT(ccxsup_bss), MALLOCED(ccxsup_bss->osh)));
			goto err;
		}
		bzero(ccxsup_bss->leap, sizeof(leap_sup_t));
		if (!(ccxsup_bss->leap_info =
			MALLOC(ccxsup_bss->osh, sizeof(wl_leap_info_t)))) {
			WL_ERROR(("wl%d: wlc_sup_init: out of memory, malloced %d bytes\n",
				UNIT(ccxsup_bss), MALLOCED(ccxsup_bss->osh)));
			goto err;
		}
		bzero(ccxsup_bss->leap_info, sizeof(wl_leap_info_t));
		if (!(ccxsup_bss->leap->timer =
			wl_init_timer(ccxsup_bss->wl, wlc_leapsup_timer,
			ccxsup_bss, "leap"))) {
...
wlc_ccxsup_deinit(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	if (ccxsup_bss != NULL) {
		...
		if (ccxsup_bss->leap) {
			if (ccxsup_bss->leap->timer) {
				wl_free_timer(ccxsup_bss->wl, ccxsup_bss->leap->timer);
			}
			MFREE(ccxsup_bss->osh, ccxsup_bss->leap,
				sizeof(leap_sup_t));
		}

		if (ccxsup_bss->leap_info)
			MFREE(ccxsup_bss->osh, ccxsup_bss->leap_info,
				sizeof(wl_leap_info_t));

		/* toss the leap list if there is one */
		if (ccxsup_bss->leap_list != NULL) {
			MFREE(ccxsup_bss->osh, ccxsup_bss->leap_list,
				ccxsup_bss->leap_list->buflen);
		}
#endif	/* BCMINTSUP */
		MFREE(ccxsup_bss->osh, ccxsup_bss, sizeof(bss_ccxsup_info_t));
		*pccxsup_bss = NULL;
	}
}
...
wlc_ccxsup_bss_updn(void *ctx, bsscfg_up_down_event_data_t *evt)
{
	...
	if (!evt->up) {
		if (ccxsup_bss != NULL) {
			if (CCX_ENAB(ccxsup_bss->pub) && (ccxsup_bss->leap != NULL))
				wl_del_timer(ccxsup_bss->wl, ccxsup_bss->leap->timer);
		}
	}
}
...
{
	bss_ccxsup_info_t *ccxsup_bss = (bss_ccxsup_info_t *)arg;
	wlc_info_t *wlc = ccxsup_bss->wlc;
	...
	if (DEVICEREMOVED(wlc)) {
		WL_ERROR(("wl%d: %s: dead chip\n", UNIT(ccxsup_bss), __FUNCTION__));
		wl_down(wlc->wl);
		return;
	}
	wlc_leapsup_timer_callback(ccxsup_bss);
}

static void
wlc_leap_rogue_clear(bss_ccxsup_info_t *ccxsup_bss)
{
	uint j;

	WL_TRACE(("wl%d: wlc_leap_rogue_clear: clear rogue list\n", UNIT(ccxsup_bss)));
	for (j = 0; j < LEAP_ROGUE_NUM; j++) {
		if (ccxsup_bss->leap->rogue_ap[j] != NULL) {
			MFREE(OSH(ccxsup_bss), ccxsup_bss->leap->rogue_ap[j],
				sizeof(wlc_ccx_rogue_t));
			ccxsup_bss->leap->rogue_ap[j] = NULL;
		}
	}
}

static void
wlc_leap_ddp_rogue_rpt(bss_ccxsup_info_t *ccxsup_bss)
{
	uint len;
	int idx;
	...
	osh = OSH(ccxsup_bss);
	for (idx = LEAP_ROGUE_NUM - 1; idx >= 0; idx--) {
		rogue = ccxsup_bss->leap->rogue_ap[idx];
		if ((rogue == NULL) || (!rogue->valid))
			continue;
		len = CCX_DDP_PKT_LEN;
		if ((p = PKTGET(osh, len + TXOFF, TRUE)) == NULL) {
			WL_ERROR(("wl%d: %s: pktget error for len %d\n",
			          UNIT(ccxsup_bss), __FUNCTION__, len));
			WLCNTINCR(ccxsup_bss->pub->_cnt->txnobuf);
			return;
		}
		...
		bcopy(&BSS_EA(ccxsup_bss), &ddp_hdr->eth.ether_dhost, ETHER_ADDR_LEN);
		bcopy(&CUR_EA(ccxsup_bss), &ddp_hdr->eth.ether_shost, ETHER_ADDR_LEN);
		...
		bcopy(&BSS_EA(ccxsup_bss), &ddp_hdr->dest_mac, ETHER_ADDR_LEN);
		bcopy(&CUR_EA(ccxsup_bss), &ddp_hdr->src_mac, ETHER_ADDR_LEN);
		...
		WL_WSEC(("wl%d: wlc_leap_ddp_rogue_rpt: reporting rogue AP %s\n",
			UNIT(ccxsup_bss), bcm_ether_ntoa(&rogue->ap_mac, eabuf)));
		/* send it out */
		rogue->valid = FALSE;
		wlc_sendpkt(ccxsup_bss->wlc, p, ccxsup_bss->cfg->wlcif);
	}
}
...
wlc_ccx_leap_ssid(wlc_ccxsup_info_t *ccxsup_info, wlc_bsscfg_t *cfg, uchar SSID[], int len)
{
	...
	if ((ccxsup_bss->leap_list == NULL) || (ccxsup_bss->leap_list->count == 0))
	    return FALSE;

	/* If SSID matches one in the leap list, the matching list
	 * item is current leap info.
	 */
	li = ccxsup_bss->leap_list->leap_info;
	do {
		if ((len == (int) li->ssid.SSID_len) && !bcmp(SSID, li->ssid.SSID, len)) {
			 ...
			 */
			bcopy(li, ccxsup_bss->leap_info, sizeof(wl_leap_info_t));

			/* Update the supplicant structure with the user info as well */
			/* Don't use domain if it isn't furnished. */
			if (ccxsup_bss->leap_info->domain_len != 0) {
				bcopy(ccxsup_bss->leap_info->domain,
					ccxsup_bss->leap->name,
					ccxsup_bss->leap_info->domain_len);
				ccxsup_bss->leap->name[ccxsup_bss->leap_info->domain_len]
					= '\\';
				i = ccxsup_bss->leap_info->domain_len + 1;
			}
			bcopy(ccxsup_bss->leap_info->user,
				&ccxsup_bss->leap->name[i],
				ccxsup_bss->leap_info->user_len);
			ccxsup_bss->leap->name_len = ccxsup_bss->leap_info->user_len + i;
			return TRUE;
		}
	} while (++li < (ccxsup_bss->leap_list->leap_info +
		ccxsup_bss->leap_list->count));
	return FALSE;
}
 ...
 */
static void
wlc_leapsup_sendeap(bss_ccxsup_info_t *ccxsup_bss)
{
	uint16 len;
	void *p = NULL;
	...
	osl_t *osh = OSH(ccxsup_bss);

	len = (uint16) ccxsup_bss->leap->name_len;
	switch (ccxsup_bss->leap->status) {
	case CCX_STARTED:
		if ((p = wlc_eapol_pktget(ccxsup_bss->wlc, ccxsup_bss->cfg,
			&BSS_EA(ccxsup_bss), EAPOL_HEADER_LEN)) == NULL)
			break;
		eapol_hdr = (eapol_header_t *) PKTDATA(osh, p);
		eapol_hdr->type = EAPOL_START;
		eapol_hdr->length = 0;
		WL_WSEC(("wl%d: wlc_leapsup_sendeap: sending EAPOL START %d\n",
			UNIT(ccxsup_bss), ccxsup_bss->leap->start_count));
		timeout = LEAP_TIMER_MSECS;	/* send START again when timeout */
		ccxsup_bss->leap->start_count ++;
		break;

	case CCX_IDED:
		if (ccxsup_bss->leap->type == EAP_IDENTITY)
			len += EAP_HEADER_LEN + 1;
		else
			len += EAP_HEADER_LEN + 2;
		if ((p = wlc_eapol_pktget(ccxsup_bss->wlc, ccxsup_bss->cfg,
			&BSS_EA(ccxsup_bss), EAPOL_HEADER_LEN + len)) == NULL)
			break;
		...
		eap_hdr->id = (uchar) ccxsup_bss->leap->id;
		eap_hdr->length = hton16(len);
		if (ccxsup_bss->leap->type == EAP_IDENTITY) {
			eap_hdr->type = EAP_IDENTITY;
			bcopy(ccxsup_bss->leap->name, eap_hdr->data,
				ccxsup_bss->leap->name_len);
	...
	case CCX_CHALLENGED:
		...
		bcm_ccx_hashpwd(ccxsup_bss->leap_info->password,
		                ccxsup_bss->leap_info->password_len,
		                ccxsup_bss->leap->pwhash, ccxsup_bss->leap->pwhashhash);

		/* cobble challenge response */
		bcm_ccx_leap_response(ccxsup_bss->leap->pwhash,
			ccxsup_bss->leap->peer_challenge,
			leap_resp->response);
		bcopy(leap_resp->response, ccxsup_bss->leap->peer_response, LEAP_RESPONSE_LEN);
		bcopy(ccxsup_bss->leap->name, leap_resp->username,
			ccxsup_bss->leap->name_len);
	...
	case CCX_CHALLENGING:
		...
		/* cobble a challenge */
		wlc_getrand(ccxsup_bss->wlc, ccxsup_bss->leap->net_challenge,
		            LEAP_CHALLENGE_LEN);
		bcopy(ccxsup_bss->leap->net_challenge,
			leap_chall->challenge, LEAP_CHALLENGE_LEN);
		bcopy(ccxsup_bss->leap->name, leap_chall->username,
			ccxsup_bss->leap->name_len);
		WL_WSEC(("wl%d: wlc_leapsup_sendeap: sending LEAP challenge request %d\n",
		         UNIT(ccxsup_bss), ccxsup_bss->leap->challenge_count));
		timeout = LEAP_TIMER_MSECS;	/* send challenge again when timeout */
		ccxsup_bss->leap->challenge_count ++;
		break;
	...
	default:
		WL_ERROR(("wl%d: %s: unexpected CCX status %d\n",
		          UNIT(ccxsup_bss), __FUNCTION__, ccxsup_bss->leap->status));
		break;
	}
	if (p != NULL) {
		wlc_sendpkt(ccxsup_bss->wlc, p, ccxsup_bss->cfg->wlcif);
		if (timeout)
			wl_add_timer(ccxsup_bss->wl, ccxsup_bss->leap->timer, timeout, 0);
	}
}

static void
wlc_leapsup_timer_callback(bss_ccxsup_info_t *ccxsup_bss)
{
	if (ccxsup_bss == NULL)
		return;
	...
	case CCX_STARTED:
	case CCX_HELD:
		ccxsup_bss->leap->status = CCX_STARTED;
		/* IEEE Std 802.1X-2001 Sect 8.5.10 */
		if (ccxsup_bss->leap->start_count < LEAP_MAX_RETRY)
			wlc_leapsup_sendeap(ccxsup_bss);
		else {
			WL_WSEC(("wl%d: wlc_leapsup_timer_callback: reach max # EAPOL START count,"
			         " move to state %d\n",
			         UNIT(ccxsup_bss), CCX_AUTHENTICATED));
			ccxsup_bss->leap->status = CCX_AUTHENTICATED;
			ccxsup_bss->leap->auth_pending = FALSE;
			/* bump it to trigger timeout status */
			ccxsup_bss->leap->start_count = LEAP_MAX_RETRY + 1;
		}
		break;
	case CCX_IDED:
	case CCX_CHALLENGED:
		...
		WL_WSEC(("wl%d: wlc_leapsup_timer_callback: timeout in state %d, move to state"
		         " %d\n",
		         UNIT(ccxsup_bss), ccxsup_bss->leap->status, CCX_STARTED));
		ccxsup_bss->leap->status = CCX_STARTED;
		wlc_leapsup_sendeap(ccxsup_bss);
		break;
	case CCX_CHALLENGING:
		/* RFC2284 2.2.1 */
		if (ccxsup_bss->leap->challenge_count < LEAP_MAX_RETRY)
			wlc_leapsup_sendeap(ccxsup_bss);
		else {
			WL_WSEC(("wl%d: wlc_leapsup_timer_callback: reach max # LEAP challenge"
			         " count, move to state %d\n",
			         UNIT(ccxsup_bss), CCX_IDLE));
			ccxsup_bss->leap->auth_pending = FALSE;
			ccxsup_bss->leap->status = CCX_IDLE;
		}
		break;
	default:
...
wlc_ccx_authenticated(wlc_ccxsup_info_t *ccxsup_info, wlc_bsscfg_t *cfg)
{
	...
	return ccxsup_bss->leap->status == CCX_KEYED;
}
 ...
 */
static void
wlc_leapsup_recveap(bss_ccxsup_info_t *ccxsup_bss, eap_header_t *rx_eap_hdr)
{
	wlc_ccxsup_info_t *ccxsup_info = ccxsup_bss->m_handle;
	uint16 len;
	leap_challenge_t *leap_chall;
	leap_response_t *leap_resp;
	leapsup_status_t status;
	...
	len = ccxsup_bss->leap->name_len;

	/* What kind of EAP is this? */
	switch (rx_eap_hdr->code) {
	case EAP_REQUEST:

		ccxsup_bss->leap->type = rx_eap_hdr->type;
		...
		if (rx_eap_hdr->type == EAP_IDENTITY) {
			 ...
			 */
			if ((ccxsup_bss->leap->status > CCX_IDED) &&
			    (ccxsup_bss->leap->status < CCX_KEYED)) {
				WL_ERROR(("wl%d: %s: unexpected EAP identity"
				          " request\n",
				          UNIT(ccxsup_bss), __FUNCTION__));
				break;
			}
			(void) wl_del_timer(ccxsup_bss->wl, ccxsup_bss->leap->timer);
			ccxsup_bss->leap->id = rx_eap_hdr->id;
			/* Check if we have the identity info to respond */
			if (ccxsup_bss->leap_info->user_len) {
				ccxsup_bss->leap->status = CCX_IDED;
				/*
				 * make sure this counter doesn't get wrapped around but
				 * still reflects 'timeout' condition
				 */
				if (ccxsup_bss->leap->retry_count <= LEAP_MAX_RETRY)
					ccxsup_bss->leap->retry_count ++;
				wlc_leapsup_sendeap(ccxsup_bss);
			}
			else
				ccxsup_bss->leap->status = CCX_IDREQD;
			/* reset START counter per IEEE 802.1X Std Sect 8.5.10 */
			ccxsup_bss->leap->start_count = 0;
			...
			(void) wl_del_timer(ccxsup_bss->wl, ccxsup_bss->leap->timer);
			/* save the challenge */
			bcopy(leap_chall->challenge, ccxsup_bss->leap->peer_challenge,
				LEAP_CHALLENGE_LEN);
			ccxsup_bss->leap->id = rx_eap_hdr->id;
			ccxsup_bss->leap->status = CCX_CHALLENGED;
			wlc_leapsup_sendeap(ccxsup_bss);
			...
			if (rx_eap_hdr->id != ccxsup_bss->leap->id) {
				WL_ERROR(("wl%d: %s: EAP notication id %d,"
				          " expected id %d\n",
				          UNIT(ccxsup_bss), __FUNCTION__, rx_eap_hdr->id,
				          ccxsup_bss->leap->id));
			}
			...
			status = ccxsup_bss->leap->status;
			ccxsup_bss->leap->status = CCX_NOTIFIED;
			wlc_leapsup_sendeap(ccxsup_bss);
			ccxsup_bss->leap->status = status;
			...
			if ((len > (EAP_HEADER_LEN + EAP_REJECT_LEN)) &&
				...
				WL_WSEC(("wl%d: %s: EAP reject notice\n",
				         UNIT(ccxsup_bss), __FUNCTION__));
				wlc_ccx_rogueap_update(ccxsup_info, ccxsup_bss->cfg,
			...
			WL_WSEC(("wl%d: %s: received EAP type %d request\n",
				UNIT(ccxsup_bss), __FUNCTION__, rx_eap_hdr->type));
	...
	case EAP_RESPONSE:
		...
		if ((ntoh16(rx_eap_hdr->length) < EAP_HEADER_LEN + LEAP_RESPONSE_HDR_LEN + len) ||
		    ...
		    (leap_resp->version != LEAP_VERSION) ||
		    (leap_resp->resp_len != LEAP_RESPONSE_LEN) ||
		    (bcmp(leap_resp->username, ccxsup_bss->leap->name, len))) {
			WL_ERROR(("wl%d: %s: unexpected EAP response",
			          UNIT(ccxsup_bss), __FUNCTION__));
			wlc_ccx_rogueap_update(ccxsup_info, ccxsup_bss->cfg,
				CCX_ROGUE_CHAN_TO_AP, &BSS_EA(ccxsup_bss));
			ccxsup_bss->leap->auth_pending = FALSE;
			break;
		}
		(void) wl_del_timer(ccxsup_bss->wl, ccxsup_bss->leap->timer);
		/* check the response text */
		bcm_ccx_leap_response(ccxsup_bss->leap->pwhashhash,
		                      ccxsup_bss->leap->net_challenge,
		                      ccxsup_bss->leap->net_response);
		if (bcmp(ccxsup_bss->leap->net_response, leap_resp->response,
			LEAP_RESPONSE_LEN)) {
			WL_WSEC(("wl%d: %s: challenge response failure; reporting"
				" as rogue AP\n",
				UNIT(ccxsup_bss), __FUNCTION__));
			wlc_ccx_rogueap_update(ccxsup_info, ccxsup_bss->cfg,
				(uint16)CCX_ROGUE_CHAN_TO_AP, &BSS_EA(ccxsup_bss));
			ccxsup_bss->leap->status = CCX_IDLE;
			ccxsup_bss->leap->auth_pending = FALSE;
			break;
		}

		bcm_ccx_session_key(ccxsup_bss->leap->pwhashhash, CCX_PW_HASH_LEN +
		                    LEAP_CHALLENGE_LEN + LEAP_RESPONSE_LEN +
		                    LEAP_CHALLENGE_LEN + LEAP_RESPONSE_LEN,
		                    ccxsup_bss->leap->session_key);
		if (ccxsup_bss->leap->status < CCX_AUTHENTICATED)
			ccxsup_bss->leap->status = CCX_AUTHENTICATED;
#ifdef	BCMSUP_PSK
		if (BSS_SUP_TYPE(ccxsup_bss->wlc->idsup, ccxsup_bss->cfg)
			== SUP_LEAP_WPA) {
			wsec_pmk_t pmk;

			pmk.key_len = CCX_SESSION_KEY_LEN;
			pmk.flags = 0;
			bcopy(ccxsup_bss->leap->session_key, pmk.key, CCX_SESSION_KEY_LEN);
			wlc_ioctl(ccxsup_bss->wlc, WLC_SET_WSEC_PMK, &pmk, sizeof(wsec_pmk_t),
			          ccxsup_bss->cfg->wlcif);
		}
#endif /* BCMSUP_PSK */
		break;
	...
	case EAP_SUCCESS:
		...
		(void) wl_del_timer(ccxsup_bss->wl, ccxsup_bss->leap->timer);
		ccxsup_bss->leap->id = rx_eap_hdr->id;
		ccxsup_bss->leap->status = CCX_CHALLENGING;
		ccxsup_bss->leap->challenge_count = 0;
		ccxsup_bss->leap->retry_count = 0;
		ccxsup_bss->leap->start_count = 0;
		wlc_leapsup_sendeap(ccxsup_bss);
		break;
	...
	default:
		WL_ERROR(("wl%d: %s: unexpected EAP code %d\n",
		          UNIT(ccxsup_bss), __FUNCTION__, (int) rx_eap_hdr->code));
		break;
	}
	return;
}
...
wlc_leapsup_start(wlc_ccxsup_info_t *ccxsup_info, wlc_bsscfg_t *cfg)
{
	...
	WL_TRACE(("wl%d: wlc_leapsup_start: EAPOL start for %s\n",
	          UNIT(ccxsup_bss), bcm_ether_ntoa(&BSS_EA(ccxsup_bss), eabuf)));

	/* Make a clean start */
	wl_del_timer(ccxsup_bss->wl, ccxsup_bss->leap->timer);
	/* Save last failure status for same ssid */
	if (ccxsup_bss->leap->status != CCX_INIT &&
		ccxsup_bss->leap->status < CCX_AUTHENTICATED &&
		ccxsup_bss->leap->last_ssid_len == BSS_SSID_LEN(ccxsup_bss) &&
		!bcmp(BSS_SSID(ccxsup_bss), ccxsup_bss->leap->last_ssid,
		ccxsup_bss->leap->last_ssid_len))
		last_failed = TRUE;
	...
	if (bcmp(&ccxsup_bss->leap->last_bssid, &BSS_EA(ccxsup_bss),
		sizeof(struct ether_addr))) {
		bcopy(&BSS_EA(ccxsup_bss), &ccxsup_bss->leap->last_bssid,
			sizeof(struct ether_addr));
		bcopy(BSS_SSID(ccxsup_bss), ccxsup_bss->leap->last_ssid,
			BSS_SSID_LEN(ccxsup_bss));
		ccxsup_bss->leap->last_ssid_len = BSS_SSID_LEN(ccxsup_bss);
		ccxsup_bss->leap->start_count = 0;
		ccxsup_bss->leap->retry_count = 0;
	}
	if (last_failed) {
		ccxsup_bss->leap->status = CCX_HELD;
		delay_time = LEAP_HELD_DELAY;
		WL_WSEC(("wl%d: wlc_leapsup_start: hold a while after failure\n",
			UNIT(ccxsup_bss)));
	}
	wl_add_timer(ccxsup_bss->wl, ccxsup_bss->leap->timer, delay_time, 0);
	ccxsup_bss->leap->auth_pending = (ccxsup_bss->leap->status > CCX_IDLE);

	return (ccxsup_bss->leap->auth_pending);
}
...
wlc_ccx_leapsup(wlc_ccxsup_info_t *ccxsup_info, wlc_bsscfg_t *cfg, eapol_header_t *rx_hdr)
{
	...
	if (rx_hdr->type == EAP_PACKET) {
		...
		if (ccxsup_bss->leap->status != CCX_HELD)
			wlc_leapsup_recveap(ccxsup_bss, eap_hdr);
		else {
		...
		if (!(key = MALLOC(ccxsup_bss->osh, sizeof(wl_wsec_key_t)))) {
			WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
				UNIT(ccxsup_bss), __FUNCTION__,
				MALLOCED(ccxsup_bss->osh)));
			return FALSE;
		}
		...
		default:
			WL_WSEC(("wl%d: wlc_ccx_leapsup: ignoring EAPOL key with unexpected length"
			         " %d\n",
			         UNIT(ccxsup_bss), key->len));
			MFREE(ccxsup_bss->osh, key, sizeof(wl_wsec_key_t));
			goto done;
			break;
		}
		...
		if (eapol_key->index & EAPOL_KEY_UNICAST) {
			 ...
			 * which index to pick.
			 */
			bcopy(ccxsup_bss->leap->session_key, key->data, key->len);
			key->flags |= WL_PRIMARY_KEY;
			/* LEAP complete */
			report_rogues = ccxsup_bss->leap->auth_pending;
			ccxsup_bss->leap->auth_pending = FALSE;
			ccxsup_bss->leap->status = CCX_KEYED;
			...
			bcopy(eapol_key->iv, ccxsup_bss->leap->key_iv, EAPOL_KEY_IV_LEN);
			prepare_key(ccxsup_bss->leap->key_iv,
			            EAPOL_KEY_IV_LEN+CCX_SESSION_KEY_LEN, &ks);
			rc4(key->data, key->len, &ks);
		}
		wlc_ioctl(ccxsup_bss->wlc, WLC_SET_KEY, key, sizeof(wl_wsec_key_t),
			ccxsup_bss->cfg->wlcif);
		MFREE(ccxsup_bss->osh, key, sizeof(wl_wsec_key_t));

		/* Authorize scb for data */
#ifdef BCMINTSUP
		wlc_ioctl(ccxsup_bss->wlc, WLC_SCB_AUTHORIZE, &BSS_EA(ccxsup_bss),
		          ETHER_ADDR_LEN, ccxsup_bss->cfg->wlcif);
#else
		AUTHORIZE(ccxsup_bss->cfg);
#endif

		/* leap is done, unicast key is in place, report Rogue AP list */
		if (report_rogues) {
			wlc_leap_ddp_rogue_rpt(ccxsup_bss);
			wlc_leap_rogue_clear(ccxsup_bss);
		}
	}
done:
...
wlc_ccx_rogue_timer(wlc_ccxsup_info_t *ccxsup_info, wlc_bsscfg_t *cfg, struct ether_addr *ap_mac)
{
	...
	if ((ccxsup_bss == NULL) ||
		(BSS_SUP_TYPE(ccxsup_bss->wlc->idsup, cfg) != SUP_LEAP) ||
	    (ccxsup_bss->leap->status >= CCX_AUTHENTICATED))
		return;
	...
	struct ether_addr *ap_mac)
{
	...
	WL_TRACE(("wl%d: wlc_ccx_rogueap_update: add rogue AP %s, reason %d\n",
		UNIT(ccxsup_bss), bcm_ether_ntoa(ap_mac, eabuf), reason));

	/* if found, return; if not found, add to the list */
	for (indx = 0; indx < LEAP_ROGUE_NUM; indx++) {
		rogue = ccxsup_bss->leap->rogue_ap[indx];
		if ((rogue == NULL) || (!rogue->valid))
			break;
...
static int
wlc_ccx_set_leap_list(bss_ccxsup_info_t *ccxsup_bss, wlc_bsscfg_t *cfg, void *pl)
{
	...
	ccxsup_bss->leap_list = (wl_leap_list_t *) MALLOC(ccxsup_bss->osh, ll->buflen);
	if (ccxsup_bss->leap_list == NULL) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
		          UNIT(ccxsup_bss), __FUNCTION__, MALLOCED(ccxsup_bss->osh)));
		return BCME_NOMEM;
	}
	bcopy(ll, ccxsup_bss->leap_list, ll->buflen);

	/* Check if we are waiting for user identity information */
	if (ccxsup_bss->leap->status == CCX_IDREQD) {
		if (wlc_ccx_leap_ssid(ccxsup_bss->m_handle, ccxsup_bss->cfg,
			ccxsup_bss->leap_info->ssid.SSID,
			ccxsup_bss->leap_info->ssid.SSID_len)) {
			if (ccxsup_bss->leap_info->user_len) {
				ccxsup_bss->leap->status = CCX_IDED;
				wlc_leapsup_sendeap(ccxsup_bss);
			}
...
static int
wlc_ccx_get_leap_list(bss_ccxsup_info_t *ccxsup_bss, wlc_bsscfg_t *cfg, void *pl)
{
	wl_leap_list_t *ll = (wl_leap_list_t *)pl;

	/* Must have a leapsup context with a leap list. */
	if ((ccxsup_bss == NULL) || (ccxsup_bss->leap_list == NULL) ||
	    (ll->buflen < ccxsup_bss->leap_list->buflen))
		return BCME_BADARG;

	/* special case zero-count list */
	ll->count = ccxsup_bss->leap_list->count;
	if (ccxsup_bss->leap_list->count == 0)
		return 0;

	/* Caller must provide enough buffer for the list */
	if (ll->buflen <  sizeof(wl_leap_list_t) +
	    ((ccxsup_bss->leap_list->count - 1) * sizeof(wl_leap_info_t)))
		return BCME_BUFTOOSHORT;

	ll->buflen = ccxsup_bss->leap_list->buflen;
	ll->version = ccxsup_bss->leap_list->version;
	bcopy(&ccxsup_bss->leap_list->leap_info, &(ll->leap_info),
	      ccxsup_bss->leap_list->count * sizeof(wl_leap_info_t));
	return 0;
}
...
#endif /* BCMSUP_PSK */
	bzero(&ccxsup_bss->leap->last_bssid, sizeof(struct ether_addr));
	/* delete leap timer */
	wl_del_timer(ccxsup_bss->wl, ccxsup_bss->leap->timer);
	/* reset leap status */
	ccxsup_bss->leap->status = CCX_INIT;
}

#if defined(BCMSUP_PSK)
static void
wlc_cckm_calc_krk_btk(bss_ccxsup_info_t *ccxsup_bss)
{
	uchar *data, *prf_buff;
	const char prefix[] = "Fast-Roam Generate Base Key";
	int data_len = 0;

	if (!(data = MALLOC(ccxsup_bss->osh, WPA_KEY_DATA_LEN_128))) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
			UNIT(ccxsup_bss), __FUNCTION__,  MALLOCED(ccxsup_bss->osh)));
		return;
	}
	if (!(prf_buff = MALLOC(ccxsup_bss->osh, PRF_OUTBUF_LEN))) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
			UNIT(ccxsup_bss), __FUNCTION__,  MALLOCED(ccxsup_bss->osh)));
	...
	bcopy(&CUR_EA(ccxsup_bss), &data[data_len], ETHER_ADDR_LEN);
	data_len += ETHER_ADDR_LEN;
	bcopy(&ccxsup_bss->wpa->snonce, &data[data_len], EAPOL_WPA_KEY_NONCE_LEN);
	...
	fPRF(ccxsup_bss->wpa_info->pmk, ccxsup_bss->wpa_info->pmk_len,
	...
	PUSH_KRK_TO_WLDRIVER(ccxsup_bss->cfg, ccxsup_bss->key_refresh_key, CCKM_KRK_LEN);
#endif
	MFREE(ccxsup_bss->osh, data, WPA_KEY_DATA_LEN_128);
...
static void
wlc_cckm_calc_ptk(bss_ccxsup_info_t *ccxsup_bss)
{
	uchar *data, *prf_buff;
	int data_len = 0;
	uint32 RN = htol32(ccxsup_bss->rn);
	...
	bcopy(&RN, &data[data_len], sizeof(ccxsup_bss->rn));
	...
	uint32 tsf_l, struct ether_addr *bssid, wpa_ie_fixed_t *rsnie)
{
	...
	wlc_cckm_calc_reassocreq_MIC(cckmie, bssid, rsnie,
		&ccxsup_bss->cfg->cur_etheraddr, ccxsup_bss->rn,
		ccxsup_bss->key_refresh_key, ccxsup_bss->wpa->WPA_auth);
}
...
wlc_sup_getleapauthpend(wlc_ccxsup_info_t *ccxsup_info, wlc_bsscfg_t *cfg)
{
	...
	if (ccxsup_info) {
		...
		if (ccxsup_bss && ccxsup_bss->leap)
			auth_pending = ccxsup_bss->leap->auth_pending;
	}
	return auth_pending;
}
...
static bool
wlc_cckm_reassoc(bss_ccxsup_info_t *ccxsup_bss, cckm_reassoc_resp_ie_t *cckm_ie)
{
	uint32 rn;
	uint len;
	...
	if (bcmp(cckm_ie->oui, CISCO_AIRONET_OUI, DOT11_OUI_LEN) ||
		...
		WL_WSEC(("wl%d: wlc_cckm_reassoc: OUI (%02x:%02x:%02x:%02x) mismatch in CCKM"
		         " reassoc IE\n",
		         UNIT(ccxsup_bss), cckm_ie->oui[0], cckm_ie->oui[1],
	...
	if (len != (uint)(cckm_ie->len + TLV_HDR_LEN)) {
		WL_WSEC(("wl%d: wlc_cckm_reassoc: CCKM reassoc IE length = %d, should be %d\n",
		         UNIT(ccxsup_bss), cckm_ie->len, len - TLV_HDR_LEN));
		return FALSE;
	}
	...
	if (rn != ccxsup_bss->rn) {
		WL_WSEC(("wl%d: wlc_cckm_reassoc: CCKM reassociation request number = %d, should be"
		         " %d\n",
		         UNIT(ccxsup_bss), rn, ccxsup_bss->rn));
		return FALSE;
	}
...
static void
wlc_cckm_calc_reassocresp_MIC(bss_ccxsup_info_t *ccxsup_bss,
	cckm_reassoc_resp_ie_t *cckmie, uint8 *obuf)
{
	...
	bcopy(&CUR_EA(ccxsup_bss), &data[data_len], ETHER_ADDR_LEN);
	data_len += ETHER_ADDR_LEN;
	bcopy(ccxsup_bss->wpa->auth_wpaie, &data[data_len],
		ccxsup_bss->wpa->auth_wpaie_len);
	data_len += ccxsup_bss->wpa->auth_wpaie_len;
	...
	/* generate the MIC */
	if (ccxsup_bss->wpa->WPA_auth == WPA2_AUTH_CCKM)
		hmac_sha1(data, data_len, ccxsup_bss->wpa->eapol_mic_key,
			WPA_MIC_KEY_LEN, hash_buf);
	else
		hmac_md5(data, data_len, ccxsup_bss->wpa->eapol_mic_key,
			WPA_MIC_KEY_LEN, hash_buf);
	bcopy(hash_buf, obuf, CCKM_MIC_LEN);
}

static bool
wlc_cckm_decr_gtk(bss_ccxsup_info_t *ccxsup_bss, uint8 *egtk, uint16 egtk_len)
{
	...
	if (ccxsup_bss->wpa->WPA_auth == WPA2_AUTH_CCKM) {
		if (aes_unwrap(WPA_ENCR_KEY_LEN, ccxsup_bss->wpa->eapol_encr_key,
		...
		bcopy(&ccxsup_bss->rn, encrkey, sizeof(ccxsup_bss->rn));
		encrkeylen += sizeof(ccxsup_bss->rn);
		bcopy(ccxsup_bss->wpa->eapol_encr_key, &encrkey[encrkeylen], WPA_ENCR_KEY_LEN);
	...
	bcopy(egtkbuf, ccxsup_bss->wpa->gtk, ccxsup_bss->wpa->gtk_len);
	return TRUE;
}
...
wlc_cckm_reassoc_resp(wlc_ccxsup_info_t *ccxsup_info, wlc_bsscfg_t *cfg)
{
	...
	if (!(cckmie = (cckm_reassoc_resp_ie_t *)
		...
		WL_WSEC(("wl%d: wlc_cckm_reassoc_resp: no CCKM reassoc IE.  Association"
			" response IEs length %d bytes\n", UNIT(ccxsup_bss), IEs_len));
		return FALSE;
	}

	/* validate the CCKM IE */
	if (!wlc_cckm_reassoc(ccxsup_bss, cckmie))
		return FALSE;

	/* process the CCKM IE */

	/* derive the PTK */
	wlc_cckm_calc_ptk(ccxsup_bss);

	/* validate cckmie->MIC */
	wlc_cckm_calc_reassocresp_MIC(ccxsup_bss, cckmie, MIC);
	...
	wlc_wpa_plumb_tk(wlc, cfg, (uint8*)ccxsup_bss->wpa->temp_encr_key,
		ccxsup_bss->wpa->tk_len,
		ccxsup_bss->wpa->ucipher, &cfg->current_bss->BSSID);

	/* decrypt and plumb the group key */
	if (!wlc_cckm_decr_gtk(ccxsup_bss, cckmie->egtk, cckmie->gtklen))
		return FALSE;
	/* plumb GTK */
	wlc_wpa_plumb_gtk(wlc, cfg, ccxsup_bss->wpa->gtk, ccxsup_bss->wpa->gtk_len,
		cckmie->mcast_idx, ccxsup_bss->wpa->mcipher, cckmie->rsc, FALSE);

	bzero(ccxsup_bss->wpa->replay, EAPOL_KEY_REPLAY_LEN);
...
{
	struct dot11_assoc_resp *dst = ccxsup_bss->wlc->assoc_resp;
	...
	if (len < sizeof(struct dot11_assoc_resp) || src == NULL) {
		DEBUG_PRINTF(("%s: bad args:  assoc_resp %p len %d\n",
			__FUNCTION__, OSL_OBFUSCATE_BUF(src), len));
		return;
	}
	if (dst != NULL)
		MFREE(ccxsup_bss->osh, dst, ccxsup_bss->wlc->assoc_resp_len);

	dst = MALLOC(ccxsup_bss->osh, len);
	if (dst == NULL) {
		return;
	}
	bcopy(src, dst, len);
	ccxsup_bss->wlc->assoc_resp_len = len;
	ccxsup_bss->wlc->assoc_resp = dst;
}
...
bool
wlc_cckm_get_rn(wlc_ccxsup_info_t *ccxsup_info, wlc_bsscfg_t *cfg, int* rn)
{
	bss_ccxsup_info_t *ccxsup_bss;
...
wlc_ccxsup_get_auth_status(wlc_ccxsup_info_t *ccxsup_info, wlc_bsscfg_t *cfg)
{
	...
	*/
	if ((ccxsup_bss->leap->start_count > LEAP_MAX_RETRY &&
		ccxsup_bss->leap->status == CCX_AUTHENTICATED) ||
		(ccxsup_bss->leap->retry_count > LEAP_MAX_RETRY &&
		ccxsup_bss->leap->status < CCX_AUTHENTICATED &&
		ccxsup_bss->leap->status > CCX_IDLE)) {
		WL_WSEC(("wl%d: TIMEOUT, start %d retry %d status %d\n",
			UNIT(ccxsup_bss),
			ccxsup_bss->leap->start_count, ccxsup_bss->leap->retry_count,
			ccxsup_bss->leap->status));
...
wlc_ccxsup_handle_joinstart(wlc_ccxsup_info_t *ccxsup_info, wlc_bsscfg_t *cfg, uint16 sup_type)
{
	...
	if (BSS_CCX_ENAB(cfg->wlc, cfg) && (wlc_ccx_set_leap_on(cfg->wlc->ccx,
		cfg, wlc_ccx_leap_ssid(ccxsup_info, cfg, cfg->SSID, cfg->SSID_len)))) {
...
wlc_ccxsup_handle_wpa_eapol_msg1(wlc_ccxsup_info_t *ccxsup_info, wlc_bsscfg_t *cfg, uint16 key_info)
{
	...
	wlc_cckm_calc_krk_btk(ccxsup_bss);
	...
	bcopy(ccxsup_bss->key_refresh_key, wpa->eapol_mic_key, CCKM_KRK_LEN);

	/* send message 2 */
	if (wlc_wpa_sup_sendeapol(ccxsup_bss->wlc->idsup, ccxsup_bss->cfg,
		...
		WL_WSEC(("wl%d: wlc_wpa_sup_eapol: send message 2 failed"
				 "(ccx)\n", UNIT(ccxsup_bss)));
		wlc_wpa_send_sup_status(ccxsup_bss->wlc->idsup, ccxsup_bss->cfg,
			WLC_E_SUP_SEND_FAIL);
	}
...
wlc_ccxsup_send_leap_rogue_report(wlc_ccxsup_info_t *ccxsup_info, wlc_bsscfg_t *cfg)
{
	...
	if (ccxsup_bss->leap->auth_pending) {
		ccxsup_bss->leap->auth_pending = FALSE;
		wlc_leap_ddp_rogue_rpt(ccxsup_bss);
		wlc_leap_rogue_clear(ccxsup_bss);
	}
...
wlc_ccxsup_start_negotimer(wlc_ccxsup_info_t *ccxsup_info, wlc_bsscfg_t *cfg)
{
	...
	if (BSS_CCX_ENAB(wlc, cfg) &&
		wlc_ccx_get_leap_on(wlc->ccx, cfg)) {
		/* start LEAP negotiation timer */
		wlc_ccx_set_leap_start(wlc->ccx, cfg, wlc->pub->now);
		ccxsup_bss->leap->auth_pending = TRUE;
	}
}
