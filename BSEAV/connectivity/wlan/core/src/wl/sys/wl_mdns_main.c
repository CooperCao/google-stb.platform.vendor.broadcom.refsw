...
/* memobj to track memory allocated by mDNSPlatformMemAllocate() */
typedef struct memobj {
	struct memobj	*next;
	mDNSu32		size;		/* size of buf in bytes */
	char		buf[];		/* buffer for new malloc */
} memobj_t;

/* mdns private info structure */
struct wlc_mdns_info {
	wlc_info_t		*wlc;
	uint32			magic;
	...
	u32 			seed;
	memobj_t		*memobj_list;	/* memory object list to track mallocs */
	int32			memobj_count;	/* count of memory objects */
	int32			memobj_bytes;	/* bytes consumed by buffers of memory objects */
};
...
static void wl_mdns_wake(wlc_mdns_info_t *mdnsi, uint32 reason);
static void *mDNSPlatformMemAllocate(wlc_mdns_info_t *mdnsi, mDNSu32 inSize);
static void mDNSPlatformMemFree(wlc_mdns_info_t *mdnsi, void *addr, mDNSu32 inSize);
...
static mStatus mDNS_Register_internal(wlc_mdns_info_t *mdnsi, AuthRecord *rr);
static mStatus mDNS_RegisterService(wlc_mdns_info_t *mdns,
	domainlabel *name, domainname *type, domainname *domain,
	mDNSIPPort port, u8 txtinfo[], u16 txtlen, u32 ttl, domainname *SubTypes, u32 NumSubTypes);
static mStatus AdvertiseInterface(wlc_mdns_info_t *mdnsi, NetworkInterfaceInfo *set);
mDNSOpaque16 mDNSOpaque16fromIntVal(mDNSu16 v);
static mStatus mDNSPlatformInit(wlc_mdns_info_t *mdns, proxy_services_t *proxy_services);
...
static void mDNS_Lock(wlc_mdns_info_t *mdnsi);
static s32 GetNextScheduledEvent(wlc_mdns_info_t *mdnsi);
static void mDNS_Unlock(wlc_mdns_info_t *mdnsi);
static u32 mDNSPlatformRandomSeed(void);
static u32 mDNSRandom(wlc_mdns_info_t *mdnsi, u32 max);
...
#ifdef ICMP
static bool mdns_dbase_extract_ip(uint8 *buffer, int32 buflen, wl_icmp_ip_cfg_t *host_ip);
#endif /* ICMP */

#define MDNS_IS_ENABLE(mdnsi)		(mdnsi->wlc->pub->_bdo)
#define MDNS_SET_ENABLE(mdnsi, state)	(mdnsi->wlc->pub->_bdo = state)

/* Send a character array out */
static int
send_packet(wlc_info_t *wlc, uchar *params, uint p_len)
{
	bool status;
	osl_t *osh = wlc->osh;
	void *pkt;
	...
	/* WL_LOCK(); */
	status = wlc_sendpkt(wlc, pkt, NULL);
	/* W_UNLOCK(); */
...
static void
wl_mdns_wake(wlc_mdns_info_t *mdnsi, uint32 reason)
{

	if (mdnsi->sent_wakeup) {
		return;
	}
	...
	if (mdnsi->rx_dataptr && mdnsi->rx_datalen) {

		mdnsi->sent_wakeup = TRUE;
		/* stop mdns processing */
		MDNS_SET_ENABLE(mdnsi, FALSE);
	}
...
/*
 * Different memory manamgement for Mac and Dongle solution:
 * Mac: No memory free needed cause system will restart the driver;
 * Dongle: Need extra memory management to release the allocated memory
 *         for different cases, cause Dongle can't afford to restart.
 */
void *mDNSPlatformMemAllocate(wlc_mdns_info_t *mdnsi, mDNSu32 inSize)
{
	memobj_t *newmalloc;

	newmalloc = MALLOC(mdnsi->wlc->osh, inSize + OFFSETOF(memobj_t, buf));
	if (!newmalloc) {
		WL_ERROR(("%s: MALLOC of %d bytes failed\n", __FUNCTION__, inSize));
		return NULL;
	}

	/* add new malloc to the head of malloc list */
	newmalloc->next = mdnsi->memobj_list;
	newmalloc->size = inSize;
	mdnsi->memobj_list = newmalloc;
	mdnsi->memobj_count++;
	mdnsi->memobj_bytes += newmalloc->size;

	return ((void *)newmalloc->buf);
}

static void
free_memobj(wlc_mdns_info_t *mdnsi, memobj_t *memobj)
{
	if (memobj) {
		mdnsi->memobj_bytes -= memobj->size;
		MFREE(mdnsi->wlc->osh, memobj, memobj->size + OFFSETOF(memobj_t, buf));
		mdnsi->memobj_count--;
	}
}
...
static void mDNSPlatformMemFree(wlc_mdns_info_t *mdnsi, void *addr, mDNSu32 inSize)
{
	memobj_t **current = &mdnsi->memobj_list;
	memobj_t *found = mDNSNULL;

	/* find target in malloc list */
	while (*current) {
		if ((*current)->buf == addr) {
			found = *current;
	...
	if (found) {
		free_memobj(mdnsi, found);
	}
}

static void
mDNSPlatformMemFreeAll(wlc_mdns_info_t *mdns)
{
	memobj_t *current = mdns->memobj_list;

	while (current) {
		memobj_t *next;
		next = current->next;

		free_memobj(mdns, current);

		current = next;
	}

	mdns->memobj_list = mDNSNULL;

#ifdef BCMDBG
	WL_ERROR(("(%s) memory obj count: %d size(bytes): %d\n",
			__FUNCTION__, mdns->memobj_count, mdns->memobj_bytes));
#endif /* BCMDBG */

	if (mdns->memobj_count || mdns->memobj_bytes) {
		WL_ERROR(("(%s) memory objs not freed properly! memory obj count: %d size(bytes): %d\n",
			__FUNCTION__, mdns->memobj_count, mdns->memobj_bytes));
	}
}
...
mStatus mDNS_Register_internal(wlc_mdns_info_t *mdnsi, AuthRecord *rr)
{
	mdns_share_mem_block_t *mdns_sb = &mdnsi->mdns_share_blk_struct;
	AuthRecord **p = &mdns_sb->m->ResourceRecords;
	...
	*/
	...
	if (!mdns_sb->m->NewLocalRecords)
		mdns_sb->m->NewLocalRecords = rr;
 ...
 */
mStatus mDNS_RegisterService(wlc_mdns_info_t *mdns, domainlabel *name,
	domainname *type, domainname *domain, mDNSIPPort port, u8 txtinfo[], u16 txtlen,
	u32 ttl, domainname *SubTypes, u32 NumSubTypes)
{
	ServiceRecordSet	*sr;
	domainname		buffer;
	u32			size;
	...
	mdns_share_mem_block_t *mdns_sb = &(mdns->mdns_share_blk_struct);

	sr = (ServiceRecordSet *)mDNSPlatformMemAllocate(mdns, sizeof(ServiceRecordSet));
	if (!sr) {
		return mStatus_NoMemoryErr;
	}
	memset(sr, 0, sizeof(ServiceRecordSet));
	sr->next = mdns_sb->mdns_sr;
	mdns_sb->mdns_sr = sr;
	...
	sr->RR_SRV.resrec.rdata->u.srv = (rdataSRV *)mDNSPlatformMemAllocate(mdns, sizeof(rdataSRV));
	if (!sr->RR_SRV.resrec.rdata->u.srv) {
		return mStatus_NoMemoryErr;
	}
	sr->RR_SRV.resrec.rdata->MaxRDLength = sizeof(rdataSRV_t);

	sr->RR_TXT.resrec.rdata->u.txt = (UTF8str255 *)mDNSPlatformMemAllocate(mdns, txtlen+1);
	if (!sr->RR_TXT.resrec.rdata->u.txt) {
		return mStatus_NoMemoryErr;
	}
	...
	sr->RR_ADV.resrec.name = mDNSPlatformMemAllocate(mdns, size);
	if (!sr->RR_ADV.resrec.name) {
		return mStatus_NoMemoryErr;
	}
	...
	if (ConstructServiceName(&buffer, mDNSNULL, type, domain) == mDNSNULL)
		return (mStatus_BadParamErr);
	size = DomainNameLength(&buffer);
	sr->RR_PTR.resrec.name = mDNSPlatformMemAllocate(mdns, size);
	if (!sr->RR_PTR.resrec.name) {
		return mStatus_NoMemoryErr;
	}
	...
	if (ConstructServiceName(&buffer, name, type, domain) == mDNSNULL)
		return (mStatus_BadParamErr);
	size = DomainNameLength(&buffer);
	sr->RR_SRV.resrec.name = mDNSPlatformMemAllocate(mdns, size);
	if (!sr->RR_SRV.resrec.name) {
		return mStatus_NoMemoryErr;
	}
	...
	if (NumSubTypes)
	{
		...
		sr->SubTypes =
			(AuthRecord *)mDNSPlatformMemAllocate(mdns, NumSubTypes * sizeof(AuthRecord));
		if (!sr->SubTypes) {
			return mStatus_NoMemoryErr;
		}
		...
		for (i = 0; i < NumSubTypes; i++)
		{
			...
			size = DomainNameLength(&buffer);
			sr->SubTypes[i].resrec.name = mDNSPlatformMemAllocate(mdns, size);
			if (!sr->SubTypes[i].resrec.name) {
				return mStatus_NoMemoryErr;
			}
	...
	sr->RR_SRV.resrec.rdata->u.srv->target = &mdns_sb->m->MulticastHostname;
	...
	err = mDNS_Register_internal(mdns, &sr->RR_PTR);
	if (!err) err = mDNS_Register_internal(mdns, &sr->RR_SRV);
	if (!err) err = mDNS_Register_internal(mdns, &sr->RR_TXT);
	if (!err) err = mDNS_Register_internal(mdns, &sr->RR_ADV);
	for (i = 0; i < NumSubTypes; i++)
		if (!err)
			err = mDNS_Register_internal(mdns, &sr->SubTypes[i]);

	return (err);
}

mStatus AdvertiseInterface(wlc_mdns_info_t *mdnsi, NetworkInterfaceInfo *set)
{
	...
	*/
	...
	set->RR_A.resrec.name = &mdns_sb->m->MulticastHostname;
	set->RR_PTR.resrec.rdata->u.name = &mdns_sb->m->MulticastHostname;

	if (set->ip.type == mDNSAddrType_IPv4)
	{
		set->RR_A.resrec.rrtype = kDNSType_A;
		set->RR_A.resrec.rdata->u.ipv4 =
			(mDNSv4Addr *)mDNSPlatformMemAllocate(mdnsi, sizeof(mDNSv4Addr));
		if (!set->RR_A.resrec.rdata->u.ipv4) {
			return mStatus_NoMemoryErr;
		}
	...
	else if (set->ip.type == mDNSAddrType_IPv6)
	{
		...
		set->RR_A.resrec.rdata->u.ipv6 =
			(mDNSv6Addr *)mDNSPlatformMemAllocate(mdnsi, sizeof(mDNSv6Addr));
		if (!set->RR_A.resrec.rdata->u.ipv6) {
			return mStatus_NoMemoryErr;
		}
	...
	set->RR_PTR.resrec.name = mDNSPlatformMemAllocate(mdnsi, nameLen);
	if (!set->RR_PTR.resrec.name) {
		return mStatus_NoMemoryErr;
	}
	...
	mDNS_Register_internal(mdnsi, &set->RR_A);
	mDNS_Register_internal(mdnsi, &set->RR_PTR);
 ...
 */
mStatus mDNSPlatformInit(wlc_mdns_info_t *mdns, proxy_services_t *proxy_services)
{
	...
	UNUSED_PARAMETER(tot_ports);
	UNUSED_PARAMETER(tot_recs);
	UNUSED_PARAMETER(pSRVOffsets);

	WL_TRACE(("Enter %s\n", __FUNCTION__));

	memcpy(mdns->NicMacAddr, &mdns->wlc->cfg->cur_etheraddr, ETHER_ADDR_LEN);
	...
	WL_ERROR(("%s: numSRVR %d, numTXT %d numUDPWakeup %d numTCPWakeup %d totPorts %d totRecs %d\n",
		__FUNCTION__, nSRV_Rec, nTXT_Rec, nUDP_Wake, nTCP_Wake, tot_ports, tot_recs));
	...
	for (n_serv = 0; n_serv < m->numServices; n_serv++)
	{
		...
		if (m->MulticastHostname.c[0] == 0)
		{
			...
			for (j = 0; j < pOffloadAddrInfo->numIPv4; j++)
			{
				...
				intf = (NetworkInterfaceInfo *)
					mDNSPlatformMemAllocate(mdns, sizeof(NetworkInterfaceInfo));
				if (!intf) {
					return mStatus_NoMemoryErr;
				}
			...
			if (pOffloadAddrInfo->numIPv6Sol)
			{
				m->ipv6SolAddr = mDNSPlatformMemAllocate
					(mdns, pOffloadAddrInfo->numIPv6Sol * sizeof(mDNSv6Addr));
				if (!m->ipv6SolAddr) {
					return mStatus_NoMemoryErr;
				}
			...
			if (pSecondaryAddrInfo->numIPv4)
			{
				...
				m->secondaryIpv4Addr =
					...
					(mdns, pSecondaryAddrInfo->numIPv4 * sizeof(mDNSv4Addr));
				if (!m->secondaryIpv4Addr) {
					return mStatus_NoMemoryErr;
				}
			...
			if (pSecondaryAddrInfo->numIPv6)
			{
				...
				m->secondaryIpv6Addr = (u8 *)mDNSPlatformMemAllocate
					(mdns, pSecondaryAddrInfo->numIPv6 * sizeof(mDNSv6Addr));
				if (!m->secondaryIpv6Addr) {
					return mStatus_NoMemoryErr;
				}
			...
			if (pData->numUDPWakeupPorts)
			{
				...
				m->udpWakeUpPorts = (u16 *)mDNSPlatformMemAllocate
					(mdns, pData->numUDPWakeupPorts * sizeof(mDNSu16));
				if (!m->udpWakeUpPorts) {
					return mStatus_NoMemoryErr;
				}
			...
			if (pData->numTCPWakeupPorts)
			{
				...
				m->tcpWakeUpPorts = (u16 *)mDNSPlatformMemAllocate
					(mdns, pData->numTCPWakeupPorts * sizeof(mDNSu16));
				if (!m->tcpWakeUpPorts) {
					return mStatus_NoMemoryErr;
				}
		...
		if ((err = mDNS_RegisterService(mdns, &name, &type, &domain,
			port,
			mDNSNULL, 0, m->rec.r.resrec.rroriginalttl,
			subTypes, numOfSubTypes)) != mStatus_NoError) {
			return err;
		}
	}
	...
	for (n_txt = 0; n_txt < m->numTxt; n_txt++) {
		...
		if (found) {
			...
			rr->resrec.rdata->u.txt =
				(UTF8str255 *)mDNSPlatformMemAllocate(mdns, m->rec.r.resrec.rdlength);
			if (!rr->resrec.rdata->u.txt) {
				return mStatus_NoMemoryErr;
			}
				...
				(m->DeviceInfo.resrec.RecordType == kDNSRecordTypeUnregistered))
			{
				...
				m->DeviceInfo.resrec.name = mDNSPlatformMemAllocate
					(mdns, DomainNameLength(m->rec.r.resrec.name));
				if (!m->DeviceInfo.resrec.name) {
					return mStatus_NoMemoryErr;
				}
				AssignDomainName(m->DeviceInfo.resrec.name, m->rec.r.resrec.name);
				m->DeviceInfo.resrec.rdata->u.txt = (UTF8str255 *)
					mDNSPlatformMemAllocate(mdns, m->rec.r.resrec.rdlength);
				if (!m->DeviceInfo.resrec.rdata->u.txt) {
					return mStatus_NoMemoryErr;
				}
				...
				mDNS_Register_internal(mdns, &m->DeviceInfo);
			} else {
				pRR_TXT = (AuthRecord *)mDNSPlatformMemAllocate(mdns, sizeof(AuthRecord));
				if (!pRR_TXT) {
					return mStatus_NoMemoryErr;
				}
				...
				mDNS_Register_internal(mdns, pRR_TXT);
			}
		}
	}

	if (!m->numServices && !m->numTxt) {
		MDNS_SET_ENABLE(mdns, FALSE);
		WL_ERROR(("%s: No TXT or SRV records, not starting mDNS\n", __FUNCTION__));
		err = mStatus_NotInitializedErr;
	} else {
		MDNS_SET_ENABLE(mdns, TRUE);

		m->HostInterfaces = intfList;
		intf = intfList;
		while (intf) {
			if ((err = AdvertiseInterface(mdns, intf)) != mStatus_NoError) {
				break;
			}
			intf = intf->next;
		}
#if defined(BDO) && defined(BCMDBG)
		WL_ERROR(("(%s) memory obj count: %d size(bytes): %d\n",
			__FUNCTION__, mdns->memobj_count, mdns->memobj_bytes));
#endif
	}
...
#ifdef ICMP
#define GETN(to, from, n, rem) { memcpy(to, from, n); from += n; rem -= n; }
#define MDNS_HDR_DBASE_LEN    11

static bool
mdns_dbase_extract_ip(uint8 *buffer, int32 buflen, wl_icmp_ip_cfg_t *host_ip)
{
	uint8 *p = buffer;
	int32 i, remlen = buflen;

	/* Is there at least one ip address? */
	if (remlen < (MDNS_HDR_DBASE_LEN + IPV4_ADDR_LEN)) {
		return FALSE;
	}

	/* Skip:Signature 4 bytes followed by 4 byte flag */
	p += 8;
	/* local ipv4 */
	host_ip->num_ipv4= *p++;
	/* local ipv6 */
	host_ip->num_ipv6 = *p++;
	/* Skip: local solicited */
	p++;
	remlen -= MDNS_HDR_DBASE_LEN;

	/* IPv4 addresses first */
	for (i = 0 ; (i < host_ip->num_ipv4) && (remlen >= IPV4_ADDR_LEN); i++) {
		GETN(host_ip->ipv4[MIN(i, MAX_HOST_IPV4 - 1)].addr, p, IPV4_ADDR_LEN, remlen);
		/* Skip: mask */
		p += 4;
		remlen -= 4;
	}
	if (host_ip->num_ipv4 > MAX_HOST_IPV4) {
		WL_ERROR(("%s: Too many IPv4 addrs (%d), max = %d. Keeping last.\n",
			__FUNCTION__, host_ip->num_ipv4, MAX_HOST_IPV4));
		host_ip->num_ipv4 = MAX_HOST_IPV4;
	}

	/* IPv6 addresses and prefix lens */
	for (i = 0 ; (i < host_ip->num_ipv6) && (remlen >= IPV6_ADDR_LEN); i++) {
		GETN(host_ip->ipv6[MIN(i, MAX_HOST_IPV6 - 1)].addr, p, IPV6_ADDR_LEN, remlen);
		/* Skip: prefix */
		p++;
		remlen--;
	}
	if (host_ip->num_ipv6 > MAX_HOST_IPV6) {
		WL_ERROR(("%s: Too many IPv6 addrs (%d), max = %d. Keeping last.\n",
			__FUNCTION__, host_ip->num_ipv6, MAX_HOST_IPV6));
		host_ip->num_ipv6 = MAX_HOST_IPV6;
	}
	return TRUE;
}
#endif /* ICMP */
 ...
 * Returns TRUE on successful mDNS initialization.
 */
bool
wl_mDNS_Init(wlc_mdns_info_t *mdnsi, uint8 *dbase, uint32 dbase_len)
{
	...
	MDNS_SET_ENABLE(mdnsi, FALSE);
	mdnsi->rx_dataptr = NULL;
	mdnsi->rx_datalen = 0;
	mdnsi->sent_wakeup = FALSE;
	memset(&mdnsi->mdns_share_blk_struct, 0, sizeof(mdnsi->mdns_share_blk_struct));
	mdnsi->mDnsGratuitousArpSent = FALSE;
	mdnsi->seed = 0;
	mdnsi->memobj_list = mDNSNULL;
	mdnsi->memobj_count = 0;
	mdnsi->memobj_bytes = 0;

	if (!mdnsi->wlc->pub->associated) {
	...
	/* Init all fields of mdns_sb */
	memset(mdns_sb, 0, sizeof(*mdns_sb));
	mdns_sb->signature = BONJOUR_PROXY_SIG;
	mdns_sb->mdns_sr = mDNSNULL;
	mdns_sb->mdns_intf = mDNSNULL;

	mdns_sb->m = (mDNS *)mDNSPlatformMemAllocate(mdnsi, sizeof(mDNS));
	if (!mdns_sb->m) {
		return FALSE;
	}
	memset(mdns_sb->m, 0, sizeof(mDNS));
	mdns_sb->m->signature = BONJOUR_PROXY_SIG;
	...
	if (mDNSPlatformInit(mdnsi, proxy_services) != mStatus_NoError) {
		return FALSE;
	}
#ifdef ICMP
	if (ICMP_ENAB(mdnsi->wlc->pub)) {
		wl_icmp_ip_cfg_t host_ip;

		memset(&host_ip, 0, sizeof(wl_icmp_ip_cfg_t));
		/* Extract the ipv4 and ipv6 addresses from the db */
		if (mdns_dbase_extract_ip(dbase, dbase_len, &host_ip)) {
			wlc_set_icmp_offload(mdnsi->wlc, &host_ip);
		} else {
			WL_ERROR(("Unable to extract ipv4/6 addresses from mdns dbase\n"));
		}
	}
#endif /* ICMP */

	if (MDNS_IS_ENABLE(mdnsi)) {
		wl_add_timer(mdnsi->wlc->wl, mdnsi->mdns_execute_timer, 20, FALSE);
	}
...
{
	/* return time as signed 32-bit */
	return (OSL_SYSUPTIME() & 0x7fffffff);
}
...
static void mDNS_Lock(wlc_mdns_info_t *mdnsi)
{
	mDNS *m = mdnsi->mdns_share_blk_struct.m;
...
static void mDNS_Unlock(wlc_mdns_info_t *mdnsi)
{
	...
	if (m->mDNS_busy == 0)
	{
		m->NextScheduledEvent = GetNextScheduledEvent(mdnsi);
		m->timenow = 0;
	}
...
static u32 mDNS_send_packet(wlc_mdns_info_t *mdnsi,
	void *pHdr, u8 hdrLen, void *pBuffer, u16 bufferLen)
{
	int ret = 0;
	uint8 *pnew_pkt = NULL;
	...
	if (hdrLen) {
		pnew_pkt = mDNSPlatformMemAllocate(mdnsi, hdrLen + bufferLen);
		if (!pnew_pkt) {
			...
			ret = send_packet(mdnsi->wlc, pnew_pkt, hdrLen + bufferLen);
		}
	} else {
		ret = send_packet(mdnsi->wlc, pBuffer, bufferLen);
	}
	if (ret)
		WL_ERROR(("Error mdns_send_packet returns %d\n", ret));

	if (hdrLen && pnew_pkt) {
		mDNSPlatformMemFree(mdnsi, pnew_pkt, hdrLen + bufferLen);
	}
	return ret;
}
...
static u32 mDNSRandom(wlc_mdns_info_t *mdnsi, u32 max)
{
	...
	{
		int i;
		mdnsi->seed = mDNSPlatformRandomSeed();		/* Pick an initial seed */
		for (i= 0; i<100; i++)
			mdnsi->seed = mdnsi->seed * 21 + 1;	/* And mix it up a bit */
	}
	while (mask < max)
		mask = (mask << 1) | 1;

	do
		mdnsi->seed = mdnsi->seed * 21 + 1;
	while ((mdnsi->seed & mask) > max);

	return (mdnsi->seed & mask);
}
...
mDNSBool AddressIsLocalSubnet(wlc_mdns_info_t *mdnsi, mDNSAddr *addr);
mDNSBool AddressIsLocalSubnet(wlc_mdns_info_t *mdnsi, mDNSAddr *addr)
{
	NetworkInterfaceInfo *intf;
	mDNS *m = mdnsi->mdns_share_blk_struct.m;
...
void AddAdditionalsToResponseList(wlc_mdns_info_t *mdnsi,
	AuthRecord *ResponseRecords, AuthRecord ***nrpp);
void AddAdditionalsToResponseList(wlc_mdns_info_t *mdnsi,
	AuthRecord *ResponseRecords, AuthRecord ***nrpp)
{
	AuthRecord  *rr, *rr2;
...
u8 *GenerateUnicastResponse(wlc_mdns_info_t *mdnsi,
	DNSMessage *query, mDNSu8 *end, bool LegacyQuery, AuthRecord *ResponseRecords);
u8 *GenerateUnicastResponse(wlc_mdns_info_t *mdnsi,
	DNSMessage *query, mDNSu8 *end, bool LegacyQuery, AuthRecord *ResponseRecords)
{
	mDNS *m = mdnsi->mdns_share_blk_struct.m;
...
mStatus mDNSPlatformSendUDP(wlc_mdns_info_t *mdnsi,
	void *msg, u8 *end, u8 *dstMacAddr, mDNSAddr *dst, mDNSIPPort dstPort);
mStatus mDNSPlatformSendUDP(wlc_mdns_info_t *mdnsi,
	void *msg, u8 *end, u8 *dstMacAddr, mDNSAddr *dst, mDNSIPPort dstPort)
{
	...
	if (dst->type == mDNSAddrType_IPv4)
	{
		...
		mDNS_send_packet(mdnsi, &mdns_ipv4_pkt, sizeof(TXPORT_HDR), msg, len);
	}
	else if (dst->type == mDNSAddrType_IPv6)
	{
		...
		mDNS_send_packet(mdnsi, &mdns_ipv6_pkt, sizeof(TXPORT_IPV6_UDP_HDR), msg, len);
	}

	return mStatus_NoError;
}

mStatus mDNSSendDNSMessage(wlc_mdns_info_t *mdnsi,
	DNSMessage *msg, u8 *end, mDNSu8 *dstMacAddr, mDNSAddr *dst, mDNSIPPort dstport);
mStatus mDNSSendDNSMessage(wlc_mdns_info_t *mdnsi,
	DNSMessage *msg, u8 *end, mDNSu8 *dstMacAddr, mDNSAddr *dst, mDNSIPPort dstport)
{
	mStatus status;
	...
	*ptr++ = (u8)(numAdditionals &  0xFF);
	...
	status = mDNSPlatformSendUDP(mdnsi, msg, end, dstMacAddr, dst, dstport);
...
u8 *ProcessQuery(wlc_mdns_info_t *mdnsi,
	DNSMessage *query, u8 *end, mDNSu8 *srcMacAddr, mDNSAddr *srcaddr,
	...
	bool LegacyQuery, bool QueryWasMulticast, bool QueryWasLocalUnicast)
{
	mdns_share_mem_block_t *mdns_sb = &(mdnsi->mdns_share_blk_struct);
	mDNS *m = mdns_sb->m;
	bool        FromLocalSubnet	= AddressIsLocalSubnet(mdnsi, srcaddr);
...
/* same name, then mark it to generate an NSEC record on this interface */
	...
	/* *** */
	AddAdditionalsToResponseList(mdnsi, ResponseRecords, &nrp);
 ...
 */
	...
	*/
		...
		m->SuppressSending =
			...
			(s32)mDNSRandom(mdnsi, (u32)mDNSPlatformOneSecond*5) + 49, 50);
	...
	*/
	if (SendLegacyResponse) {
		...
		responseptr = GenerateUnicastResponse(mdnsi, query, end, LegacyQuery, ResponseRecords);
	}
...
void mDNSCoreReceiveQuery(wlc_mdns_info_t *mdnsi,
	DNSMessage *msg, mDNSu8 *end, mDNSu8 *srcMacAddr, mDNSAddr *srcaddr,
	...
	mDNSIPPort srcport, mDNSAddr *dstaddr, mDNSIPPort dstport)
{
	...
	bool QueryWasLocalUnicast = !mDNSAddrIsDNSMulticast(dstaddr) &&
		AddressIsLocalSubnet(mdnsi, srcaddr);

	responseend = ProcessQuery(mdnsi, msg, end, srcMacAddr, srcaddr,
	...
	if (responseend) {
		...
		mDNSSendDNSMessage(mdnsi, m->omsg, responseend, srcMacAddr, srcaddr, srcport);
	}
}
...
mDNSBool MatchDependentOn(wlc_mdns_info_t *mdnsi,
	CacheRecord *pktrr, AuthRecord *master);
mDNSBool MatchDependentOn(wlc_mdns_info_t *mdnsi,
	CacheRecord *pktrr, AuthRecord *master)
{
	mDNS *m = mdnsi->mdns_share_blk_struct.m;
...
*/
mDNSBool PacketRRConflict(wlc_mdns_info_t *mdnsi,
	AuthRecord *our, CacheRecord *pktrr);
mDNSBool PacketRRConflict(wlc_mdns_info_t *mdnsi,
	AuthRecord *our, CacheRecord *pktrr)
{
	...
	if (our->DependentOn || MatchDependentOn(mdnsi, pktrr, our)) return (mDNSfalse);
...
void mDNSWakeUpConflictResolution(wlc_mdns_info_t *mdnsi);
void mDNSWakeUpConflictResolution(wlc_mdns_info_t *mdnsi)
{
	WL_TRACE(("%s\n", __FUNCTION__));

	/* force wake up system */
	wl_mdns_wake(mdnsi, WL_WOWL_MDNS_CONFLICT);
...
*/
void mDNSCoreReceiveResponse(wlc_mdns_info_t *mdnsi,
	DNSMessage *response, mDNSu8 *end, mDNSu8 *srcMacAddr,
...
*/
	...
	*/
	if (!mDNSAddrIsDNSMulticast(dstaddr))
	{
		...
		if (!AddressIsLocalSubnet(mdnsi, srcaddr))
			return;
				...
				if (PacketRRMatchesSignature(&m->rec.r, rr))
				{
					...
					*/
					if (m->rec.r.resrec.rroriginalttl > 0 &&
						PacketRRConflict(mdnsi, rr, &m->rec.r))
					{
					...
					WL_ERROR(("%s: Wakeup due to kDNSRecordTypeVerified\n",
						__FUNCTION__));
							mDNSWakeUpConflictResolution(mdnsi);
						}
...
bool mDNSAddrIsSecondary(wlc_mdns_info_t *mdnsi, mDNSAddr *ip);
bool mDNSAddrIsSecondary(wlc_mdns_info_t *mdnsi, mDNSAddr *ip)
{
	int i;
	mDNS *m = mdnsi->mdns_share_blk_struct.m;
...
void mDNSCoreReceive(wlc_mdns_info_t *mdnsi,
	void *pkt, u8 *end, u8 *srcMacAddr, mDNSAddr *srcaddr,
	mDNSIPPort srcport, mDNSAddr *dstaddr, mDNSIPPort dstport)
{
	mdns_share_mem_block_t *mdns_sb = &(mdnsi->mdns_share_blk_struct);
	mDNS *m = mdns_sb->m;
	...
	u8 StdR	= kDNSFlag0_QR_Response | kDNSFlag0_OP_StdQuery;
	u8 *ptr	= mDNSNULL;
	u8 QR_OP;
	...
	if (mDNSAddrIsSecondary(mdnsi, srcaddr)) {
	...
	if (QR_OP == StdQ)
		mDNSCoreReceiveQuery(mdnsi, msg, end, srcMacAddr, srcaddr, srcport, dstaddr, dstport);
	else if (QR_OP == StdR)
		mDNSCoreReceiveResponse(mdnsi, msg, end, srcMacAddr, srcaddr, srcport, dstaddr, dstport);

	mDNS_Unlock(mdnsi);
}

void SendDelayedUnicastResponse(wlc_mdns_info_t *mdnsi, mDNSu8 *destMacAddr, mDNSAddr *dest);
void SendDelayedUnicastResponse(wlc_mdns_info_t *mdnsi, mDNSu8 *destMacAddr, mDNSAddr *dest)
{
	mdns_share_mem_block_t *mdns_sb = &(mdnsi->mdns_share_blk_struct);
	mDNS *m = mdns_sb->m;
	...
	while (ResponseRecords)
	{
		...
		if (m->omsg->h.numAnswers) {
			...
			mDNSSendDNSMessage(mdnsi, m->omsg, responseptr, destMacAddr, dest,
				MulticastDNSPort);
		}
	}
}
...
void SendResponses(wlc_mdns_info_t *mdnsi);
void SendResponses(wlc_mdns_info_t *mdnsi)
{
	mdns_share_mem_block_t *mdns_sb = &(mdnsi->mdns_share_blk_struct);
	mDNS *m = mdns_sb->m;
	...
	for (rr = m->ResourceRecords; rr; rr = rr->next) {
		if (rr->ImmedUnicast) {
			...
			if (!mDNSIPv4AddressIsZero(rr->v4Requester))
				SendDelayedUnicastResponse(mdnsi, macAddr, &v4);
			if (!mDNSIPv6AddressIsZero(rr->v6Requester))
				SendDelayedUnicastResponse(mdnsi, macAddr, &v6);
			if (rr->ImmedUnicast) {
	...
	while (1)
	{
		...
		if (m->omsg->h.numAnswers > 0 || m->omsg->h.numAdditionals)
		{
			if (m->IPv4Available) {
				...
				sent_one++;
				mDNSSendDNSMessage(mdnsi, m->omsg, responseptr, MulicastV4EthAddr.b,
			...
			if (m->IPv6Available) {
				...
				sent_one++;
				mDNSSendDNSMessage(mdnsi, m->omsg, responseptr, MulicastV6EthAddr.b,
...
s32 mDNS_Execute(wlc_mdns_info_t *mdnsi)
{
	mdns_share_mem_block_t *mdns_sb = &(mdnsi->mdns_share_blk_struct);
	mDNS *m = mdns_sb->m;
	int32 nexttime;
	...
	if ((m->timenow_last > 100) && !mdnsi->mDnsGratuitousArpSent) {
		u8 bcast[6];
		u8 *pkt = mdnsi->glob_txPktBuff;
		...
		mDNS_send_packet(mdnsi, NULL, 0, pkt, sizeof(MAC_HDR) + sizeof(ARP_PKT));
		mdnsi->mDnsGratuitousArpSent = mDNStrue;
	}

	mDNS_Lock(mdnsi);	/* Must grab lock before trying to read m->timenow */
	...
	mDNS_Unlock(mdnsi);	/* Calling mDNS_Unlock is what gives m->NextScheduledEvent its new value */
...
void mdns_sd_main_init(wlc_mdns_info_t *mdnsi)
{
	mdnsi->seed = 0;
	mdnsi->mDnsGratuitousArpSent = mDNSfalse;
}
...
bool matchOffloadIpAddr(wlc_mdns_info_t *mdnsi, u8 *ipAddr, u32 addrType);
bool matchOffloadIpAddr(wlc_mdns_info_t *mdnsi, u8 *ipAddr, u32 addrType)
{
	mDNS *m = mdnsi->mdns_share_blk_struct.m;
...
uint16 wakeupTCPPort(wlc_mdns_info_t *mdnsi, pTcpHdr  tcpHdr);
uint16 wakeupTCPPort(wlc_mdns_info_t *mdnsi, pTcpHdr  tcpHdr)
{
...
uint16 wakeupUDPPort(wlc_mdns_info_t *mdnsi, pUdpHdr udp);
uint16 wakeupUDPPort(wlc_mdns_info_t *mdnsi, pUdpHdr udp)
{
	bool wake = FALSE;
	u16 udp_len;
...
uint32 mdns_rx(wlc_mdns_info_t *mdnsi, void *pkt, uint16 len)
{
	...
	case PROTOCOL_IP: /* IPV4 */
		...
		if (!is_multi(hdr->ip.dest_addr, mDNSAddrType_IPv4)) {  /* ! multicast */
			if (!matchOffloadIpAddr(mdnsi, hdr->ip.dest_addr, mDNSAddrType_IPv4)) { /*! ours */
...
#ifdef MCAST_FILTER
		if (matchOffloadIpAddr(mdnsi, hdr->ip.src_addr, mDNSAddrType_IPv4)) { /* came from us! */
		...
		case PROTOCOL_TCP: {
			...
			wake = wakeupTCPPort(mdnsi, tcpHdr);
			if (wake) {
		...
		case PROTOCOL_UDP:
			...
			if (hdr->txp.udp.dest_port == hton16(UDP_PORT_MDNS)) {
				...
				mDNSCoreReceive(mdnsi, &mdns_ipv4_pkt->h, (u8 *)pkt + len,
				...
				wake = wakeupUDPPort(mdnsi, &hdr->txp.udp);
				if (wake) {
	...
	case PROTOCOL_IPV6: {
		...
		if (!ismulti) {
			if (!matchOffloadIpAddr(mdnsi, pIpv6Hdr->dest_addr, mDNSAddrType_IPv6)) {
...
#ifdef MCAST_FILTER
		if (matchOffloadIpAddr(mdnsi, pIpv6Hdr->src_addr, mDNSAddrType_IPv6)) { /* came from us! */
		...
		switch (pIpv6Hdr->next_hdr) {
			...
			case PROTOCOL_TCP: {
				...
				*/
				...
				wake = wakeupTCPPort(mdnsi, &pTcp->tcp);
	...
	if (wake) {
		...
		/* force wake up system */
		wl_mdns_wake(mdnsi, WL_WOWL_MDNS_SERVICE);
	}

	/* return ERR_NO_ERROR for a valid MDNS packet */
	if (mdnsi->sent_wakeup) {
		/* return ERR_WAKEUP for service match or conflict
		 * and allow packet to be sent up to host
		 */
		rc = ERR_WAKEUP;
	} else if (!is_mdns_packet) {
		/* return ERR_PKT_IGNORED for all non-MDNS packets
...
bool
wl_mDNS_Exit(wlc_mdns_info_t *mdns)
{
	MDNS_SET_ENABLE(mdns, FALSE);

	wl_del_timer(mdns->wlc->wl, mdns->mdns_execute_timer);

#ifdef ICMP
	if (ICMP_ENAB(mdns->wlc->pub)) {
		wlc_clear_icmp_offload(mdns->wlc);
	}
#endif /* ICMP */
	mDNSPlatformMemFreeAll(mdns);
	return TRUE;
}
...
{
	wlc_mdns_info_t *mdnsi = (wlc_mdns_info_t *)arg;
	s32 nexttime = mDNS_Execute(mdnsi);

	if (MDNS_IS_ENABLE(mdnsi)) {
		wl_add_timer(mdnsi->wlc->wl,
			mdnsi->mdns_execute_timer, nexttime, FALSE);
	} else {
...
wlc_mdns_info_t *
wlc_mdns_attach(wlc_info_t *wlc)
{
	wlc_mdns_info_t *mdnsi;

	WL_ERROR(("%s\n", __FUNCTION__));
	/* allocate mdns private info struct */
	mdnsi = MALLOCZ(wlc->osh, sizeof(wlc_mdns_info_t));
	if (!mdnsi) {
	...
	/* init mdns private info struct */
	mdnsi->magic = BONJOUR_PROXY_SIG;
	mdnsi->wlc = wlc;

	if ((mdnsi->mdns_execute_timer =
		wl_init_timer(wlc->wl, wl_mdns_timer, mdnsi, "mdns")) == NULL) {
		MFREE(wlc->osh, mdnsi, sizeof(wlc_mdns_info_t));
...
void
BCMATTACHFN(wl_mdns_detach)(wlc_mdns_info_t *mdnsi)
{
	WL_ERROR(("%s\n", __FUNCTION__));
	...
	wl_free_timer(mdnsi->wlc->wl, mdnsi->mdns_execute_timer);
	MFREE(mdnsi->wlc->osh, mdnsi, sizeof(wlc_mdns_info_t));
}
