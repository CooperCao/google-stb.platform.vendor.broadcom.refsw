...
wl_pfn_sendonenet(wl_pfn_info_t *pfn_info, wlc_bss_info_t * bi, uint32 now)
{
	...
	*/
	...
	if (wlc_bss2wl_bss(wlc, bi, &result->bss_info, evt_data_size, TRUE) !=
			BCME_OK) {
		...
		wlc_bss_info_t *bi, uint32 now)
{
	...
	if (pfn_internal->wpa_auth != (uint16)WPA_AUTH_PFN_ANY) {
