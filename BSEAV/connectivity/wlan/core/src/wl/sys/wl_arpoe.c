...
static const bcm_iovar_t arp_iovars[] = {
	{"arpoe", IOV_ARPOE, (IOVF_RSDB_SET), 0, IOVT_BOOL, 0},
	{"arp_ol", IOV_ARP_OL,
	(0), 0, IOVT_UINT32, 0
	},
...
arp_reply_peer(wl_arp_info_t *arpi, struct bcmarp *arp_req, bool snap)
{
	...
	/* Allocate extra TXOFF bytes headroom */
	if (!(pkt = PKTGET(WLCOSH(arpi), TXOFF + pktlen, TRUE))) {
	...
	err = wlc_sendpkt(arpi->wlc, pkt, arpi->wlcif);
	/* Force to forward frame to handle it at host side */
	if (err == TRUE) {
             ...
             struct bcmarp **arp_out, bool *snap_out)
{
	...
	if (WLEXTSTA_ENAB(arpi->wlc->pub) && !lb_dot3_pkt(LBP(sdu))) {
		if (((ltoh16(((struct dot11_header *)frame)->fc))
				& FC_KIND_MASK) == FC_QOS_DATA) {
			pt = frame + DOT11_MAC_HDR_LEN + DOT11_QOS_LEN + SNAP_HDR_LEN;
		} else {
	...
	*/
	...
	if (arplen > ARP_DATA_LEN) {
		...
		WL_INFORM(("wl%d: %s: truncated ARP packet (%d)\n",
