...
_wlc_pio_com_dump(pio_info_t *pt, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "pio%d: qlen %d txfrmcnt %d txdatacnt %d txfifodepth %d\n",
		pt->_fifo, pktq_n_pkts_tot(&pt->txpioq), pt->txfrmcnt, pt->txdatacnt,
		pt->txfifolimit);
...
_wlc_pio4_rx(pio_info_t *pt)
{
	...
	va32 = (uint32 *)va;
	*va32++ = w;
	i = (WL_RXHDR_LEN(pt->pub->corerev) / 4) - 1;
	{
		while (i--) {
	...
	rxh = (d11rxhdr_t*)va;
	rxstatus = ltoh16(rxh->lt80.RxStatus1);
