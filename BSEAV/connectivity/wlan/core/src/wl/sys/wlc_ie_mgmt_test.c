...
static uint
wlc_iem_test_calc_raps_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	return TLV_HDR_LEN + 8;
}

static int
wlc_iem_test_write_raps_ie(void *ctx, wlc_iem_build_data_t *data)
{
	uint8 raps[] = {255, 8, 11, 0x50, 0xf2, 0xff, 0x1, 0x2, 0x3, 0x4};
	bcopy(raps, &data->buf[TLV_TAG_OFF], sizeof(raps));
	return BCME_OK;
}
...
static int
wlc_iem_test_parse_raps_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	if (data->ie != NULL)
		prhex("raps", data->ie, TLV_HDR_LEN + data->ie[TLV_LEN_OFF]);
	return BCME_OK;
}
...
BCMATTACHFN(wlc_iem_test_register_fns)(wlc_iem_info_t *iem)
{
	...
	if ((err = wlc_iem_add_build_fn(iem, WLC_IEM_FC_TEST, DOT11_MNG_SSID_ID,
		...
		WL_ERROR(("%s: wlc_iem_add_build_fn failed, err %d, ssid in test\n",
		          __FUNCTION__, err));
		return err;
	}
	if ((err = wlc_iem_add_build_fn(iem, WLC_IEM_FC_TEST, DOT11_MNG_RATES_ID,
		...
		WL_ERROR(("%s: wlc_iem_add_build_fn failed, err %d, sup in test\n",
		          __FUNCTION__, err));
		return err;
	}
	if ((err = wlc_iem_add_build_fn(iem, WLC_IEM_FC_TEST, DOT11_MNG_EXT_RATES_ID,
		...
		WL_ERROR(("%s: wlc_iem_add_build_fn failed, err %d, ext in test\n",
		          __FUNCTION__, err));
		return err;
	}
	if ((err = wlc_iem_vs_add_build_fn(iem, WLC_IEM_FC_TEST, WLC_IEM_VS_IE_PRIO_WPA,
		...
		WL_ERROR(("%s: wlc_iem_vs_add_build_fn failed, err %d, sup in test\n",
		          __FUNCTION__, err));
		return err;
	}
	if ((err = wlc_iem_vs_add_build_fn(iem, WLC_IEM_FC_TEST, 111,
		...
		WL_ERROR(("%s: wlc_iem_vs_add_build_fn failed, err %d, vs111 in test\n",
		          __FUNCTION__, err));
		return err;
	}
	if ((err = wlc_iem_add_build_fn(iem, WLC_IEM_FC_TEST, DOT11_MNG_RAPS_ID,
	    wlc_iem_test_calc_raps_ie_len, wlc_iem_test_write_raps_ie, iem)) != BCME_OK) {
		WL_ERROR(("%s: wlc_iem_add_build_fn failed, err %d, raps in test\n",
		          __FUNCTION__, err));
		return err;
	}
	...
	if ((err = wlc_iem_add_parse_fn(iem, WLC_IEM_FC_TEST, DOT11_MNG_SSID_ID,
		...
		WL_ERROR(("%s: wlc_iem_add_parse_fn failed, err %d, ssid in test\n",
		          __FUNCTION__, err));
		return err;
	}
	if ((err = wlc_iem_add_parse_fn(iem, WLC_IEM_FC_TEST, DOT11_MNG_RATES_ID,
	                                wlc_iem_test_parse_sup_rates_ie, iem)) != BCME_OK) {
		WL_ERROR(("%s: wlc_iem_add_parse_fn failed, err %d, sup in test\n",
		          __FUNCTION__, err));
		return err;
	}
	if ((err = wlc_iem_add_parse_fn(iem, WLC_IEM_FC_TEST, DOT11_MNG_EXT_RATES_ID,
	                                wlc_iem_test_parse_ext_rates_ie, iem)) != BCME_OK) {
		WL_ERROR(("%s: wlc_iem_add_parse_fn failed, err %d, ext in test\n",
		          __FUNCTION__, err));
		return err;
	}
	if ((err = wlc_iem_vs_add_parse_fn(iem, WLC_IEM_FC_TEST, WLC_IEM_VS_IE_PRIO_WPA,
	                                wlc_iem_test_parse_wpa_ie, iem)) != BCME_OK) {
		WL_ERROR(("%s: wlc_iem_vs_add_parse_fn failed, err %d, wpa in test\n",
		          __FUNCTION__, err));
		return err;
	}
	if ((err = wlc_iem_add_parse_fn(iem, WLC_IEM_FC_TEST, DOT11_MNG_RAPS_ID,
	                                wlc_iem_test_parse_raps_ie, iem)) != BCME_OK) {
		WL_ERROR(("%s: wlc_iem_add_parse_fn failed, err %d, raps in test\n",
		          __FUNCTION__, err));
		return err;
	}
...
static bool
wlc_iem_test_req_ins_cb(void *ctx, wlc_iem_ins_data_t *data)
{
	BCM_REFERENCE(ctx);
	if (data->is_tag &&
...
static bool
wlc_iem_test_req_mod_cb(void *ctx, wlc_iem_mod_data_t *data)
{
...
wlc_iem_test_req_vsie_cb(void *ctx, wlc_iem_cbvsie_data_t *data)
{
...
wlc_iem_test_build_frame(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	uiel.ins_fn = wlc_iem_test_req_ins_cb;
	uiel.mod_fn = wlc_iem_test_req_mod_cb;
	uiel.vsie_fn = wlc_iem_test_req_vsie_cb;
...
static void
wlc_iem_test_nhdlr_cb(void *ctx, wlc_iem_nhdlr_data_t *data)
{
...
static wlc_iem_tag_t
wlc_iem_test_vsie_cb(void *ctx, wlc_iem_pvsie_data_t *data)
{
	return wlc_iem_vs_get_id(data->ie);
}
...
wlc_iem_test_parse_frame(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	uint8 buf[] = {
		DOT11_MNG_SSID_ID, 3, 'u', 'i', 'e',
		DOT11_MNG_EXT_CAP_ID, 3, 0xaa, 0xaa, 0xaa,
		/* DOT11_MNG_RAPS_ID */ 255, 8, 11, 0x50, 0xf2, 0xff, 0x1, 0x2, 0x3, 0x4,
		DOT11_MNG_VS_ID, 4, 0x00, 0x50, 0xf2, 0xff,
		DOT11_MNG_VS_ID, 6, 0x00, 0x50, 0xf2, 0x01, 0x00, 0x01
	};
	...
	upp.notif_fn = wlc_iem_test_nhdlr_cb;
	upp.vsie_fn = wlc_iem_test_vsie_cb;
	upp.ctx = wlc->iemi;
