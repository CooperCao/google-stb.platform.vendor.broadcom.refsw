...
static const bcm_iovar_t p2po_iovars[] = {
	...
	(0), 0, IOVT_BUFFER, sizeof(wl_p2po_listen_t)
	},
	{"p2po_find", IOV_P2PO_FIND,
	(0), 0, IOVT_VOID, 0
	},
	{"p2po_stop", IOV_P2PO_STOP,
	(IOVF_RSDB_SET), 0, IOVT_VOID, 0
	},
	...
	(0), 0, IOVT_BUFFER, sizeof(wl_p2po_find_config_t)
	},
	{"p2po_gas_config", IOV_P2PO_GAS_CONFIG,
	(0), 0, IOVT_BUFFER, sizeof(wl_gas_config_t)
	},
	{"p2po_wfds_seek_add", IOV_P2PO_WFDS_SEEK_ADD,
	(0), 0, IOVT_BUFFER, sizeof(int32)
	},
...
static int p2po_doiovar(void *hdl, uint32 actionid,
...
#endif /* BCMDBG */
#else
static void wl_p2po_ext_listen_count_cb(void *ext_listen_count_ctx);
#endif /* P2POELO */
	...
	if (event_type == WLC_E_ESCAN_RESULT && wl_event->status == WLC_E_STATUS_PARTIAL) {
		...
		if (length >= sizeof(*escan_data)) {
			...
			WL_P2PO(("%s:Instance %s gas %p\n",
				__FUNCTION__,
				bcm_ether_ntoa(&instance->addr, eabuf),
				OSL_OBFUSCATE_BUF(instance->gas)));
...
static int
p2po_doiovar(void *hdl, uint32 actionid,
...
static void
wl_p2po_ext_listen_count_cb(void *ext_listen_count_ctx)
{
	wl_p2po_info_t *p2po = (wl_p2po_info_t *)ext_listen_count_ctx;
	wl_p2po_stop(p2po);
}
...
wl_p2po_listen(wl_p2po_info_t *p2po, void *arg, int len)
{
	...
	if (listen->period && listen->interval) {
		...
		p2po->listen.count = listen->count;
	}
	else {
		p2po->listen.period = P2PO_LISTEN_PERIOD;
		p2po->listen.interval = P2PO_LISTEN_INTERVAL;
	...
	wlc_eventq_set_all_ind(wlc, wlc->eventq,
			WLC_E_P2P_DISC_LISTEN_COMPLETE, 0);
#ifndef P2POELO
	...
	if (p2po->listen.count != 0) {
		bcm_p2p_ext_listen_count_callback(p2po->disc,
				wl_p2po_ext_listen_count_cb, (void *)p2po);
	}
...
{
	wlc_info_t *wlc = p2po->wlc;
	wlc_bsscfg_t *cfg = wl_disc_bsscfg(p2po->disci);

	WL_P2PO(("%s: Entry p2po->disc %p\n", __FUNCTION__, OSL_OBFUSCATE_BUF(p2po->disc)));
	...
	if (p2po->disc) {
		...
		wlc_eventq_set_all_ind(wlc, wlc->eventq,
				WLC_E_P2P_DISC_LISTEN_COMPLETE, 1);

		if ((p2po->listen.count != 0) && !bcm_p2p_is_listening(p2po->disc)) {
			wlc_bss_mac_event(wlc, cfg, WLC_E_P2P_DISC_LISTEN_COMPLETE, NULL,
...
wl_p2po_set_find_config(wl_p2po_info_t *p2po, void *arg, int len)
{
	...
	for (i = 0; i < fc->num_social_channels; i++) {
		/* When ULB Mode is enaled need to check for a valid ULB chanspec */
	...
	for (i = 0; i < MAX_WFDS_SEEK_SVC; i++) {
		...
		if (dest + P2P_WFDS_HASH_LEN >
			...
			WL_P2PO_TRACE(("%s: dst > src! dst=%p #=%u vndr=%p vndrlen=%u\n",
				__FUNCTION__, OSL_OBFUSCATE_BUF(dest), num_hashes,
				OSL_OBFUSCATE_BUF(p2po->prbreq_svc_hash_vndr_ie),
				p2po->prbreq_svc_hash_vndr_ie_len));
		}
	}
...
wl_p2po_wfds_seek_dump(wl_p2po_info_t *p2po)
{
	...
	for (i = 0; i < MAX_WFDS_SEEK_SVC; i++) {
		seek = p2po->seek_svcs[i];
		seek_len = p2po->seek_svcs_len[i];
		WL_PRINT((" %u: seek=%u,%p", i, seek_len, OSL_OBFUSCATE_BUF(seek)));
		if (seek) {
		WL_PRINT((" hdl=%u query=%u,%p\n",
		       seek->seek_hdl, seek->query_data_len, OSL_OBFUSCATE_BUF(seek->query_data)));
...
wl_p2po_wfds_advert_dump(wl_p2po_info_t *p2po)
{
	...
	for (i = 0; i < MAX_WFDS_ADVERT_SVC; i++) {
		advert = p2po->advert_svcs[i];
		advert_len = p2po->advert_svcs_len[i];
		WL_PRINT(("%u: len=%u ptr=%p", i, advert_len, OSL_OBFUSCATE_BUF(advert)));
		if (advert) {
			uint8 *hash;
