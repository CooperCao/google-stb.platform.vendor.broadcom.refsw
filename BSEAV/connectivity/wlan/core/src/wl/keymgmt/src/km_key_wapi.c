	...
	uint8 *body, int body_len, const key_pkt_info_t *pkt_info)
{
	...
	if (!ETHER_ISMULTI(&hdr->a1) && ((rx_iv->PN[0] - wapi_key->rx_seq[0]) & 0x1)) {
		err = BCME_DECERR;
		goto done;
	}
...
km_key_wapi_rotate_key(wlc_key_t *cur_key, wlc_key_t *prev_key, wlc_key_info_t *key_info)
{
	...
	/* update expiration */
	prev_key->exp = KEY_PUB(cur_key)->now + SMS4_OLD_KEY_MAXVALIDTIME;
	cur_key->exp = 0;

	/* update prev key flag */
	cur_key->info.flags |= (WLC_KEY_FLAG_WAPI_HAS_PREV_KEY | WLC_KEY_FLAG_TX);
	cur_key->info.flags &= ~WLC_KEY_FLAG_WAPI_PREV_KEY;
	if (prev_key->info.flags & WLC_KEY_FLAG_HW_MIC)
		cur_key->info.flags |= WLC_KEY_FLAG_HW_MIC;

	prev_key->info.flags &= ~(WLC_KEY_FLAG_WAPI_HAS_PREV_KEY | WLC_KEY_FLAG_TX);
	prev_key->info.flags |= WLC_KEY_FLAG_WAPI_PREV_KEY;

	if (key_info)
		*key_info = cur_key->info;

	KM_RESET_KEY_CACHE(KEY_KM(cur_key));
	return BCME_OK;
}
