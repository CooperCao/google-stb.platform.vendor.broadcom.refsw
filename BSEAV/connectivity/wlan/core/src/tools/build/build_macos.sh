...
BUILD_SYSTEMS="LEOPARD SNOWLEOPARD BAROLO LION ZIN ML CAB"

BUILD_SERVER_LEOPARD=
BUILD_SERVER_SNOWLEOPARD=hndmacbuild2.sj.broadcom.com
BUILD_SERVER_BAROLO=hndmacbuild4.sj.broadcom.com
BUILD_SERVER_LION=hndmacbuild4.sj.broadcom.com
BUILD_SERVER_ZIN=hndmacbuild6.sj.broadcom.com
BUILD_SERVER_ML=hndmacbuild7.sj.broadcom.com
BUILD_SERVER_CAB=hndmacbuild8.sj.broadcom.com
BUILD_SERVER_65=hndcoveritymac2.sj.broadcom.com
BUILD_SERVER_75=hndcoveritymac3.sj.broadcom.com
BUILD_SERVER_YOSEMITE=hndcoveritympro1.sj.broadcom.com
BUILD_SERVER_GALA=hndmacpro4.sj.broadcom.com
BUILD_SERVER_FUJI=hndmacpro7.sj.broadcom.com
...
while getopts 'b:c:d:e:f:hjm:p:r:s:t:x:' OPT ; do
    case "$OPT" in
       ...
       f)
           ...
           BUILD_ARGS="$BUILD_ARGS -f '$OPTARG'"
           ;;
       g)
           GMAKE=1
           ;;
...
for s in $BUILD_SYSTEMS ; do
    eval "echo \${BUILD_SERVER_$s}"
done | grep -qiE "^$(hostname)(\\..*)?\$"
if [ $? -eq 0 ]; then
...
for i in $(${ITERCMD} ${ITERLOOP}) ; do
    ...
    _OS=$(echo $BRAND | sed 's,^.*-,,' | tr '[:lower:]' '[:upper:]')
    echo "_OS: '${_OS}'"
    eval "_HN=\${BUILD_SERVER_${_OS}}"
    echo "_HN: '${_HN}' HOSTNAME: $(hostname)"
    if [ -z "${_HN}" ]; then
        echo "ERROR: No known host builds targets for '${_OS}'"
        exit 1
    fi

    if ! echo "${_HN}" | grep -qiE "^$(hostname | sed -e 's,.local,,g')(\\..*)?\$" ; then
        # Signals like Ctrl-C don't get propogated correctly in this model
        echo "INFO: "
        echo "INFO: Brand '${BRAND}' (OS: ${_OS}) detected, redirecting"
        echo "INFO: the build to server '${_HN}'"
        echo "INFO: "
        build_script=$(echo $0 | sed "/^\//!s,^,$PWD/,")
        _DO="ssh -l $LOGNAME ${_HN} $build_script $BUILD_ARGS -b $BRAND"
        echo "${_DO}"
        (${_DO}) &
        sleep 10
        continue
    ...
    if echo ${TAG:-NIGHTLY} | grep -i -q "_BRANCH_\|_TWIG_\|NIGHTLY"; then
       if [ "${LOGNAME}" == "hwnbuild" -a -d "${SERVER_BASE}/PRESERVED" -a -f ${PRESERVE_LIST} ]; then
          ...
          if [ "$preserved_brand_flag" != "0" ]; then
              ...
              [ -f "$PLACELOG" ] && cp -v ${PLACELOG} $preservedir
