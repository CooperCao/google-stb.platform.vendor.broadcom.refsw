...
MISC_DIR=misc
RLSLOG=,release.log
PLACELOG=,placeholder_release.log
...
SIGNTS_ERRORLOG=$winpfx/projects/hnd_swbuild/build_admin/logs/signing/timestamp_errors.log
...
PRESERVE_LIST="$winpfx/home/hwnbuild/src/tools/release/preserve_built_objects.txt"
PRESERVE_CTIME=120
...
PRESERVE_LOGS="$PRESERVE_LOGS $PLACELOG"
...
function search_util {
	...
	net_path1=$winpfx/projects/hnd/software/gallery/src/tools/build
	net_path2=$winpfx/home/hwnbuild/src/tools/build
...
cat <<EOF
    ...
    * You may want to refer to usage examples below
    ...
    -m email_address or "none"
              ...
              If the -m option is not provided, defaults to hnd-software-scm-list
...
NOTE:It is preferable to use build_windows.bat script instead using this
  ...
  1. Build OR respin win_external_wl for MY_BRANCH_3_120 tag and notify 
     prakashd only instead of hnd-software-scm-list:
...
function uniquedir () {
    ...
    while [ -d ${base}${j} ]
      ...
      then
        ...
        echo "${FUNCNAME} Giving up: cannot create unique directory in ${base}" >&2
        return 1;
      fi;
    done
    echo ${base}${j};
...
function copy_to_server {
    ...
    (
     ...
     # Check for robocopy. If it doesn't exist, use xcopy
      export COPY_TOOL="robocopy"
      export IS_ROBOCOPY=1
      export COPY_OPTS=" /S /E /R:1 /W:5 /np "
      where_rc=`where robocopy`
      if [ $? != 0 ]; then
          export COPY_TOOL="xcopy"
          export IS_ROBOCOPY=0
          export COPY_OPTS=" /E /H /I /Q /Y "
      fi
      echo "COPY_TOOL is $COPY_TOOL"
      ...
      echo "$COPY_TOOL ${COPY_OPTS} ${copysrc//\//\\} ${serverdir//\//\\}"
      $COPY_TOOL ${COPY_OPTS} ${copysrc//\//\\} ${serverdir//\//\\}
      COPYRC=$?

      echo "${COPY_TOOL} finished (copyrc ${COPYRC})"
      echo "Local $copysrc disk-usage: ${du}%"
      ls ${serverdir}
      echo "END:   copy build to server, `date '+%m/%d/%y %H:%M:%S'`"
     
      # Robocopy exit codes are more complex.
      #    * 0x1 means "copy successful"
      #    * 0x2 means "found files/dirs in destination that are not in source"
      #    * (there are other flags, which mean failure)
      #   These flags are OR'ed together for a result, so we accept 1 or 3.
      # xcopy return code nonzero means failure
      COPY_FAIL=0
      if [ $IS_ROBOCOPY == 1 -a $COPYRC != 1 -a $COPYRC != 3 ]; then
          COPY_FAIL=1
      fi
      if [ $IS_ROBOCOPY != 1 -a $COPYRC != 0 ]; then
          COPY_FAIL=1
      fi

      if [ $COPY_FAIL == 1 -o ! -d "${serverdir}" ]; then
         echo "" >> ${RLSLOG}
         echo "ERROR: ${COPY_TOOL} from ${copysrc} to ${serverdir} failed" >> \
              ${RLSLOG}
         echo "ERROR: ${COPY_TOOL} exit code: $COPYRC" >> \
              ${RLSLOG}
    ...
    ## Since all the stdout is directed to ${MOVELOG}, ${COPY_TOOL} it over again
...
MAILSTATUS=true
MAILTO="hnd-software-scm-list@broadcom.com";
...
SERVER_BASE_WINDOW=$winpfx/projects/hnd_swbuild/build_window
...
SERVER_BASE_WINDOW_MOUNT=$winpfx/projects/hnd/swbuild/build_window

# If the SVN repository path pointed to 'users' area, the builds
# are redirected to following path
SERVER_BASE_WINDOW_USERS=$winpfx/projects/hnd_swbuild/build_window/USERS
SERVER_BASE_WINDOW_USERS_RE=$winpfx/projects.hnd.swbuild.build_window.USERS
...
PATH=/cygdrive/${BUILD_DRV}/tools/Python:/cygdrive/${BUILD_DRV}/tools/Subversion/cygdrive/z/projects/hnd/tools/win/Python:/cygdrive/z/projects/hnd/tools/win/Subversion:/usr/bin:/bin:/usr/local/bin:$(cygpath -u ${HNDEMAKE_DIR}/i686_win32/bin):/cygdrive/${BUILD_DRV}/tools/build:${PATH}
...
#
# A note about -f <whatever> and GMAKEFLAGS.
#
# Though the documentation for these scripts claims that "-f <whatever>" is
# for passing information through the script and into the invocation of the
# make program (be it GNU make or Electric), that is NOT the entirety of the
# function of these flags.
#
# These flags are interpreted by the various build_* scripts, and information
# encoded in these flags is used to configure parts of the build scripts.
# These are NOT simply flags and values passed through to make.
#
for arg in "${GMAKEFLAGS[@]}" ; do
  case "$arg" in
     VCTOOL=cvs)
           ...
           export CVSROOT=$(echo "$arg" | fmt -1 | grep CVSROOT | awk -F= '{print $2}')

           if echo $CVSROOT | grep -qi -v ":pserver:"; then
              CVSROOT_OLD=$CVSROOT
              export CVSROOT=":pserver:hwnbuild@cvsps-sj1-1.sj.broadcom.com:$CVSROOT"
              idx=0
              while [ $idx -lt ${#GMAKEFLAGS[@]} ]; do
                  if [ "${GMAKEFLAGS[$idx]}" = "CVSROOT=$CVSROOT_OLD" ]; then
                      GMAKEFLAGS[$idx]="CVSROOT=$CVSROOT"
                      break
                  fi
                  idx=$(($idx + 1))
              done
     ...
     http*)
           ...
           echo "ERROR: Did you miss REPO_URL or DEPS_URL variable name"
...
#if echo "$GMAKEFLAGS" | egrep -q "[[:space:]]+=|=[[:space:]]+"; then
#   # Catch <var> = <value> mistakes, with white-space in them
#   # Do not try to auto-correct
#   echo "ERROR: "
#   echo "ERROR: Erroneous -f '$GMAKEFLAGS' specified"
#   echo "ERROR: White space around '=' make variables not allowed"
#   echo "ERROR: Can't continue, Exiting"
#   echo "ERROR: "
#   exit 1;
#fi
...
if [ "${CVSCUTOFF}" == "midnight" ]; then
   CVSCUTOFF="`date '+%Y-%m-%d 00:00'`"
   SVNCUTOFF="`date '+%Y-%m-%d 00:00'`"
fi

if [ "${CVSCUTOFF}" == "noon" ]; then
   CVSCUTOFF="`date '+%Y-%m-%d 12:00'`"
   SVNCUTOFF="`date '+%Y-%m-%d 12:00'`"
fi
...
# Strip off {} and timezone for compatibility with GUB which includes them.
if [[ "$SVNCUTOFF" = {*-*-*T*:*:*-*} ]]; then
    SVNCUTOFF=$(echo "$SVNCUTOFF" | sed 's@^{\(.*\)T\(.*\)-..00}$@\1 \2@')
elif [[ "$SVNCUTOFF" = *-*-*T*:*:*-* ]]; then
    SVNCUTOFF=$(echo "$SVNCUTOFF" | sed 's@^\(.*\)T\(.*\)-..00$@\1 \2@')
fi
...
if [ "$RELDIR" == "" ]; then
   ...
   if [[ "$COMPUTERNAME" == *NT-SJCA-051* ]]; then
      ...
      RELDIR=${TAG:-"NIGHTLY"};
   elif [[ "$COMPUTERNAME" == *WC-SJCA-0517 ]]; then
      RELDIR=${TAG:-"NIGHTLY"};
   elif [[ "$COMPUTERNAME" == *SJC-WLAN-E* ]]; then
      RELDIR=${TAG:-"NIGHTLY"};
   else
...
if [ "${SKIP_SERVER_COPY}" != "1" -a ! -d "${serverdir}" ]; then
	...
	if ! mkdir -pv "${serverdir}" ; then
		...
		echo "INFO: Open this URL in a browser to see status: file://///$(hostname)/$(cygpath -m ${PWD}|cut -d ':' -f2-|cut -d '/' -f2-)/,release.log" >> $serverdir/$RLSLOG
		echo "INFO: Build in progress on $(hostname)"    >> $serverdir/$RLSLOG
		echo "INFO: at $(cygpath -m ${PWD})"             >> $serverdir/$RLSLOG
		if [ -n "${EXTRA_LOG_STATUS}" ]; then
		   echo "INFO: ${EXTRA_LOG_STATUS}"              >> $serverdir/$RLSLOG
                fi
                if [ "${EMAKE}" != "" ]; then
		   echo "INFO: Following URL shows status"       >> $serverdir/$RLSLOG
		   echo "INFO: ${EA_URL}"                        >> $serverdir/$RLSLOG
                fi

		# Want to preserve this logfile for post-mortem.
		rm -f $serverdir/$PLACELOG
        cp -fp $serverdir/$RLSLOG $serverdir/$PLACELOG
	fi
fi
...
if [ -z "$NTICE" ]; then
   export NTICE="$winpfx/projects/hnd_tools/win/numega/SoftICE/"
fi;
...
if [ "$PVT_MAKEFILE" -a -s "$PVT_MAKEFILE" ]; then
	...
	if [[ -n "$SVNCUTOFF" ]]; then
	    atpath=$MISC_DIR/AT_$(echo $SVNCUTOFF$SVNTZ | tr -d ': -')
	    touch $atpath ||:
	fi
        ...
        if [ "$VCTOOL" == "svn" ]; then
          ...
          # Checkout checkout rules before release makefile is checked out
          $VCTOOL -Q -d $CVSROOT_PROD co -p src/tools/release/${CHECKOUT_DEFS} > ${CHECKOUT_DEFS} 2> $NULL
          $VCTOOL -Q -d $CVSROOT_PROD co -p src/tools/release/${CHECKOUT_RULES} > ${CHECKOUT_RULES} 2> $NULL
...
(
    export SERVER_PATH="${SERVER_PATH:-$serverdir}"
    ...
    if [[ $vctoolrc == 0 ]]; then
        ...
        MAKEOPTS="${TAG:+TAG=$TAG} ${BRAND:+BRAND=$BRAND}"
        MAKEOPTS="${MAKEOPTS} ${COTOOL:+COTOOL=$COTOOL}"
        MAKEOPTS="${MAKEOPTS} ${SVNREV:+SVNREV=$SVNREV}"
        MAKEOPTS="${MAKEOPTS} ${GMAKEFLAGS[@]}"
        MAKEOPTS="${MAKEOPTS} ${SERVER_PATH:+SERVER_PATH=${SERVER_PATH}}"
...
if [ -n "$MAILSTATUS" ]; then
   ...
   if [ -s "${MISC_DIR}/serverdir.txt" ]; then
      serverdir=`cat ${MISC_DIR}/serverdir.txt | col -b`
