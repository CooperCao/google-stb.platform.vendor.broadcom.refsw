...
# 
# $Id: build_linux.sh 291617 2011-10-24 18:22:08Z $
#
# SVN: $HeadURL$
#
...
# Default options
SCRIPT_VERSION='$Id: build_linux.sh 291617 2011-10-24 18:22:08Z $'
SCRIPT_NAME=$0
BUILD_BASE=
BUILDS_TO_KEEP=
...
-c cvs_cutoff_time
        Cvs cutoff time (yyyy-mm-dd hh:mm format or other cvs formats)
...
-f makeflags
        ...
        Remember to enclose these flags in a double quotes.
        Typically uses are: force dongle build or force cvs/hndcvs checkout.
        or specifying a custom SVN URL to pick sources from, like /users
        See examples below
...
---------------
  ...
  5. Build a full brand build from a private work-space (as-is with no
  ...
  6. Build DHD builds forcing dongle build (say linux-external-dongle-sdio)
  ...
  7. Fetch sources from CVS instead of SVN (that's build with HNDCVS)
       build_linux.sh -b linux-external-wl -f "CVSROOT=/projects/cvsroot" -r KIRIN_BRANCH_5_100 -d /projects/hnd/swbuild/build_linux/

  8. Fetch sources from custom wlansvn/users/<user> area instead of $SVNROOT
  ...
  9. Build linux-external-wl on trunk using hnd_gclient
...
function usefreedisk
{
   ...
   if echo "${BUILD_BASE}" | egrep -qvi "${BUILD_BASE_LINUX_PREBUILD}"; then
      ...
      if [ "${BUILD_BASE}" == "${BUILD_BASE_LINUX}" -o "${BUILD_BASE}" == "${BUILD_BASE_LINUX_MOUNT}" -o "${BUILD_BASE}" == "${BUILD_BASE_MOUNT}" ]; then
         ...
         shift
         DISK_POOL="$@"
         if [ ! -h "${BUILD_BASE}/${TAG}" -a ! -n "$OVERRIDE" ]; then
            ...
            if [ "$mindisk" != "${BUILD_BASE_LINUX}" ]; then
               [ -n "$VERBOSE" ] && echo "Picked Disk: ${mindu}% $mindisk"
               mkdir -pv ${mindisk}/${TAG} > $NULL 2>&1
               chmod 775 ${mindisk}/${TAG} > $NULL 2>&1
               ln -sv ${mindisk}/${TAG} ${BUILD_BASE}/${TAG} > $NULL 2>&1
...
# Available k,w,y,z
while getopts 'ab:c:d:e:f:ghi:jlm:no:p:q:r:s:t:uvx:' OPT ; do
    case "$OPT" in
	a)
	...
	b)
	    ...
	    ;;
	c)
	    # Cutoff time to use during checkout
	...
	p)
	    ...
	    ;;
	q)
	    # Used not to submit to lsf, but build locally on current node
	    LSF=${OPTARG}
	    ;;
	r)
...
case "$GMAKEFLAGS" in
     *VCTOOL=cvs*)
           ...
           export CVSROOT=$(echo "$GMAKEFLAGS" | fmt -1 | awk -F= '/CVSROOT/{print $2}')
...
if [ "${LSF}" != "none" ] ; then
    ...
    r64args="${args} \"-o\" \"R64BIT\" \"-b\" \"${R64BUILDS[*]}\""
    r64args_dslcpe="${args} \"-b\" \"${DSLCPEBRANDS[*]}\""
    r32cmd="/tools/bin/bsub -R r32bit ${LSFSLOTS:+ -n $LSFSLOTS} -L /bin/bash ${LSFOUTPUT} -q ${LSF} -J\"${LSFUSER}[1-${#R32BUILDS[*]}]\" \"$build_script\" ${r32args} -q none"
    r64cmd="/tools/bin/bsub -R rhel50 ${LSFSLOTS:+ -n $LSFSLOTS} -L /bin/bash ${LSFOUTPUT} -q ${LSF} -J\"${LSFUSER}[1-${#R64BUILDS[*]}]\" \"$build_script\" ${r64args} -q none"
    r64cmd_dslcpe="/tools/bin/bsub -R r64bit -L /bin/bash ${LSFOUTPUT} -q ${LSF} -J\"${LSFUSER}[1-${#DSLCPEBRANDS[*]}]\" \"$build_script_dslcpe\" ${r64args_dslcpe} -q none"
    ...
    submitec=0; submitec32=0; submitec64=0; submitecdsl=0
...
for i in $(seq 0 $((${#BRANDS[*]}-1))) ; do
    ...
    # If any new build dirs are created for custom svn url or users area
    # make them group writeable, otherwise they are owned by build user
    if [ -n "$BUILD_USERDIR" ]; then
       chmod ug+w $BRAND_DIR
    fi
    ...
    if [ "$VCTOOL" == "svn" ]; then
       ...
       $SVNCMD export ${REPO_URL_TRUNK}/src/tools/release/${CHECKOUT_RULES} \
               ${CHECKOUT_RULES}
       echo "$SVNCMD export ${SVNCUTOFF:+-r \{'$SVNCUTOFF $SVNTZ'\}} ${REPO_URL}/src/tools/release/${BRAND}.mk release.mk"
       $SVNCMD export ${SVNCUTOFF:+-r \{"$SVNCUTOFF $SVNTZ"\}} ${REPO_URL}/src/tools/release/${BRAND}.mk \
               release.mk
       vctoolrc=$?
    else
       # Checkout checkout rules before release makefile is checked out
    ...
    if [ "$VCTOOL" == "svn" -a "${SVNCUTOFF}" != "" ]; then
       ...
       echo "SVN CUTOFF      = ${SVNCUTOFF}"           >> ${RLSLOG}
    fi
    if [ "${GMAKEFLAGS}" != "" ]; then
...
then
        ...
        if [ "${errors}" != "0" ] ; then
           ...
           if [ "${BRAND}" == "hndrte" -o "${BRAND}" == "hndrte-dongle-wl" ]; then
              rm -f ,succeeded ${IGNORE_ERRORLOG}
           else
    ...
    if echo ${TAG:-NIGHTLY} | grep -i -q "_BRANCH_\|_TWIG_\|NIGHTLY"; then
       if [ "${LSFUSER}" == "hwnbuild" -a -d "${BUILD_BASE}/PRESERVED" -a -f "${PRESERVE_LIST}" ]; then
          ...
          do
              ...
              ## PRESERVERLIST format build_linux : BRAND : file_to_preserve
    ...
    if [ -f "${MISC_DIR}/emake.dlog" -a -f ",succeeded" ]; then
