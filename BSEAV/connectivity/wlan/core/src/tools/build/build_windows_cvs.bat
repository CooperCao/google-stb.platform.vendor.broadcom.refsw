...
$build_script     = "${SYSTEMDRIVE}/tools/build/build_windows_brand_cvs.sh";
# Local instance of windows build script, to allow seemless svn updates
$build_script_name= "temp_build_windows_brand_cvs_${nowdate}.sh";
...
sub doBuild
{
   ...
   if ( $emake ) {
      ...
      print BLDCMD "\@echo on\n";
      print BLDCMD "\@REM Auto-generated at $nowts\n";
      print BLDCMD "\@REM on $thishost and scheduled on $bldSvr\n";
      print BLDCMD "set log=${brand_log}\n";
      print BLDCMD "chmod -v 755 $build_script_loc                    >> %log%\n";
      # See the permission on local instance of build script
      print BLDCMD "ls -l $build_script_loc                           >> %log%\n";
      print BLDCMD "echo '[%date% %time%] Build Command = $build_cmd'\n";
      print BLDCMD "echo '[%date% %time%] Build Command = $build_cmd' >> %log%\n";
      print BLDCMD "bash -c '${build_cmd}'                            >> %log% 2>&1\n";
      print BLDCMD "echo '[%date% %time%] Exit Code = %errorlevel%'   >> %log%\n";
      print BLDCMD "echo '==========================================' >> %log%\n";
      close(BLDCMD);
     ...
     # gmake builds are run in sequence
      ...
      # Symbian build server has a mix of cvs repositories fetch sources from.
      # It uses custom cygwin variables to set cygwin/cvs home directories and
      # needs shell logon session explicitly
