...
LSFUSER=hwnbuild
BLDUSER=hwnbuild
XLINUX="xlinux5.sj.broadcom.com"
...
if [ "${LSF}" != "none" ] ; then
    ...
    r32cmd="/tools/bin/bsub -R r32bit ${LSFSLOTS:+ -n $LSFSLOTS} -L /bin/bash ${LSFOUTPUT} -q ${LSF32BIT} -J\"${LSFUSER}[1-${#R32BUILDS[*]}]\" \"$build_script\" ${r32args} -q none"
    r64cmd="/tools/bin/bsub -R r64bit ${LSFSLOTS:+ -n $LSFSLOTS} -L /bin/bash ${LSFOUTPUT} -q ${LSF} -J\"${LSFUSER}[1-${#R64BUILDS[*]}]\" \"$build_script\" ${r64args} -q none"
