...
my $COVSERVER = "wlansw-coverity.sj.broadcom.com";


if (! $logdir ) {
	$logdir = "${BLDSERVER_BASE}/build_admin/logs/coverity650";
}
...
my $HTML_DRIVER_URL = 'http://wlansw-coverity.sj.broadcom.com:8080/query/defects.htm?project=DRIVER';
my $HTML_WL_EXE_URL = 'http://wlansw-coverity.sj.broadcom.com:8080/query/defects.htm?project=WL-EXE';
my $HTML_ROUTER_URL = 'http://wlansw-coverity.sj.broadcom.com:8080/query/defects.htm?project=ROUTER';
my $HTML_IHV_URL    = 'http://wlansw-coverity.sj.broadcom.com:8080/query/defects.htm?project=IHV';
...
sub storeNewDefects
{
	...
	# Get New Defects.
	$component_arg = "--component=router.Other" if ( $component =~ m/router/i );
	$component_arg = "--component=ihv.Other" if ( $component =~ m/ihv/i );
...
sub scanRuns
{
	...
	foreach my $schedrun ( sort keys %$rAllRuns ) {
		...
		if (($bldtarget =~ /preco_/) || ($runstring =~ /preco/i)) {
			...
			$streamname  = "6.5.1_${branch}__${bldtarget}";
		}
