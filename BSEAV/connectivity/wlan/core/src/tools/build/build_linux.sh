...
MAILTO=
# Default LSF queue to use for linux software builds is sj-wlanswbuild
# 'sj-wlanswbuild' has rhel6 64bit resources, limited rhel5 64bit resources and very
# limited rhel5 32bit resources
WLAN_LSF_QUEUE=wlanswbuild
...
RLSLOG=,release.log
## This is the log file which allows waiting builds to wait successfully
## on builds run on node-local disk.
...
# RHEL6 == 2.6.32-358.23.2.el6
RHEL6BUILD=${MISC_DIR}/_RHEL6_BUILD
...
## /projects/hnd/swbuild is symlink to /projects/hnd_swbuild
...
LOCAL_BUILD=1
# Allow up to 50 unique local temporary build dirs for a given branch
# and brand on a given node. In practice more than 10 are never used
...
# Search utils used by this script in multiple network path hierarchy
...
function setenviron {
    ...
    SunOS)
         ...
         ;;
    Linux)
         export PATH=/home/hwnbuild/bin:${UNIX_PATH}:/tools/bin:/projects/hnd/tools/linux/bin:${CROSSGCCLINKS}
         if [ -z "$CVSROOT" ]; then
            export CVSROOT=$CVSROOT_PROD
         fi
         ;;
...
if [ "$VCTOOL" == "cvs" -a "${CVSCUTOFF}" != "" -a "${TAG}" != "" ]; then
...
for i in $(seq 0 $((${#BRANDS[*]}-1))) ; do
    ...
    if   [ "$OSFLAG" == "R32BIT" ]; then
         ...
         continue
    elif [ "$OSFLAG" == "RHEL50" ]; then
         R64BUILDS=(${R64BUILDS[*]} $BRAND)
         continue
    elif [ "$OSFLAG" == "RHEL60" ]; then
         R64BUILDS=(${R64BUILDS[*]} $BRAND)
         continue
...
if [ "${LSF}" != "none" ] ; then
    ...
    r64args="${args} \"-o\" \"RHEL50\" \"-b\" \"${R64BUILDS[*]}\""
    ubuntuargs="${args} \"-o\" \"UBUNTU\" \"-b\" \"${UBUNTUBUILDS[*]}\""
    r64args_dslcpe="${args} \"-b\" \"${DSLCPEBRANDS[*]}\""

    # If local build flag is specified, look for node with at-least
    # BUILD_LOCAL_MINDU MB free local space (for now)
    if [ -n "$LOCAL_BUILD" ]; then
       r32res="r32bit && tmp>=$BUILD_LOCAL_MINDU"
       r64res="rhel50 && tmp>=$BUILD_LOCAL_MINDU"
       ubuntures="ubuntu"
       r64res_dslcpe="rhel50 && tmp>=$BUILD_LOCAL_MINDU"
    else
       r32res="r32bit"
       r64res="rhel50"
       ubuntures="ubuntu"
       r64res_dslcpe="rhel50"
    fi

    if [ "$OSFLAG" == "RHEL60" ]; then
       r64res="rhel60 && tmp>=$BUILD_LOCAL_MINDU"
       r64res_dslcpe="rhel60 && tmp>=$BUILD_LOCAL_MINDU"
    fi
...
if [ -n "$OVERRIDE" ]; then
    ...
    # This entire check is unnecessary. Feel free to remove it in future
    # if it causes trouble. See http://wlan-rb.sj.broadcom.com/r/12826/.
    cvs status -l $OVERRIDE > ${NULL} 2>&1; cvs_ovstatus=$?
    svn info $OVERRIDE > ${NULL} 2>&1; svn_ovstatus=$?
    test -f $OVERRIDE/.gclient; gclient_ovstatus=$?

    if [ "$cvs_ovstatus" != "0" -a "$svn_ovstatus" != "0" -a "$gclient_ovstatus" != "0" ]; then
	echo "ERROR: $(hostname): $OVERRIDE not an SCM checkout?"
	exit 1
    fi
...
for i in $(seq 0 $((${#BRANDS[*]}-1))) ; do
    ...
    if [ -n "$LOCAL_BUILD" ]; then
       ...
       if [ "$wlanlsfnode" != "" -o "$LSF" == "none" ]; then
          ...
          if [ "$tmpdu" -ge "$BUILD_LOCAL_MINDU" ]; then
             let i=0; let j=0
             LOCAL_BASE=$LOCAL_BUILD_ROOT/$USER/${TAG:-TRUNK}/${BRAND}_$(date '+%y%m%d%S')_${RANDOM}
             ...
             if [ -d "$LOCAL_BASE" ]; then
                ...
                echo "[`date`]"                                            >> ${RLSLOG}
                test -f $WIP_FILE || touch $WIP_FILE
                ...
                if [ -n "$EXTRA_LOG_STATUS" ]; then
                    echo "INFO: $EXTRA_LOG_STATUS" >> ${RLSLOG}
                fi
                echo "INFO: Run 'bjobs $LSB_BATCH_JID' to query status or" >> ${RLSLOG}
                echo "INFO: rsh $HOSTNAME tail -f $LOCAL_BASE/,release.log"   >> ${RLSLOG}
    ...
    if [ -z "$LOCAL_BUILD" ]; then
       # For a non-local Linux build, the release log should be the same as
       # the placeholder.  Waiting builds can still find the info they need.
       ln -sf ${RLSLOG} ${PLACELOG}
    else
       # Ensure that we place the placeholder release log for waiting builds.
       cp -f ${RLSLOG} ${PLACELOG}
    ...
    case "`uname -r`" in
         ...
         *2.6.18*)
             ...
             ;;
         *2.6.32*)
             echo "Build Machine OS: $(uname -r)" > ${RHEL6BUILD}
             ;;
    esac
