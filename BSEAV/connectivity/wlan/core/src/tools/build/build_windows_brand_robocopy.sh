#!/bin/bash
#
# build_windows_brand.sh - Do a checkout/release for bcm43xx drivers & tools
#
...
#
# $Id: build_windows_brand.sh 376870 2013-01-03 02:40:18Z $
#
# SVN: $HeadURL$
#

SCRIPT_VERSION='$Id: build_windows_brand.sh 376870 2013-01-03 02:40:18Z $'
SCRIPT_NAME=$0
NULL=/dev/null
HOSTOS=$(uname -s)
SERVERNAME=$(hostname)
...
function copy_to_server {
    ...
    (
     ...
     # Check for robocopy. If it doesn't exist, use xcopy
      ...
      # Robocopy exit codes are more complex. Basically 0x1 means "copy successful"
      # xcopy return code nonzero means failure
      COPY_FAIL=0
      if [ $IS_ROBOCOPY == 1 -a $COPYRC != 1 ]; then
          COPY_FAIL=1
      fi
...
case "$GMAKEFLAGS" in
     ...
     *DEPS_URL=*)
     ...
     http*)
           ...
           # or DEPO_URL prefix
           echo "ERROR: "
           echo "ERROR: Erroneous -f '$GMAKEFLAGS' gmakeflags specified"
...
if [[ "$COMPUTERNAME" == *WC-SJCA-E* -o "$COMPUTERNAME" == *SJC-WLAN-E*]]; then
   if [ "${GMAKE}" != "" ]; then
      MAKE=gmake
   else
      EMAKE=1
