...
# Arg processing wipes ARGV, keep for special miracast build
local @MIR_ARGV = @ARGV;

## -branch or -revision or -tagname are synonyms
## Multiple -brands run serial in gmake, parallel in emake
## Command line option similar to build_windows_brand.sh
%scan_args = (
...
$BUILD_LOGS_DIR_DOS = $BUILD_LOGS_DIR;
$BUILD_LOGS_DIR_DOS =~ s#/#\\#g;
if ( ! -d "$BUILD_LOGS_DIR" ) {
   &Cmd("mkdir -p $BUILD_LOGS_DIR");
}
...
$build_script     = "${SYSTEMDRIVE}/tools/build/build_windows_brand.sh";
# Local instance of windows build script, to allow seemless svn updates
$build_script_name= "temp_build_windows_brand_${nowdate}.sh";
...
if ( grep { m/^$thishost$/gi } @aGmakeServers ) {
   ...
   if (( ! $forcebuild )) {
       ...
       &Warn("Please use new faster WC-SJCA-EB04");
...
sub doBuild
{
   my($brand, $bldSvr, $delay, $locbld) = @_;
   my($result) = 0;
   ...
   # Temp edit for winblue e108-gold server
   # For some perplexing reason the schtasks tool pauses for up to 5
   # min scheduling a trivial task and fails with "success" status
   # if launch time passes while still connecting to remote box.
   if ( $brand =~ m/winblue_/ ) {
      $delay += 5;
   }

   ###############################################################
   ## Miracast builds cannot use taskscheduler or remote login.
   ## Miracast can only be built as -localbuild on the miracast
   ## box so use both miracast brand marker and localbuild flag
   ## to determine whether this is a user requesting a build
   ## (no -localbuild) or a build machine performing a build.
   ## (has -localbuild).  Build machine runs build_miracast.bat
   ## via task scheduler every few minutes looking for requests.
   ###############################################################
   if ( $brand =~ m/_miracast$/ && !$locbld )
   {
      # keep it simple. If we are miracast and not localbuild then
      # we are a user's dispatch request so write the script file.
      my $rand6dig = sprintf('%-0.6d', int(rand(1000000)));
      my $jobname = 'z:/home/hwnbuild/polling/' . $rand6dig . '.bat';

      # Original request could have had multiple brands on one line
      # so nuke all brand args and push our brand back for writing.
      my @arglist = @MIR_ARGV;
      ...
      # never allow emake for miracast
      @arglist = grep(!/^-j/, @arglist);

      # strip any brands we blanked out
      @arglist = grep(!/^\s*$/, @arglist);

      push @arglist, "-localbuild";
      push @arglist, "-brand";
      push @arglist, $brand;

      open(BLDCMD, ">${jobname}") || die "Can't create ${jobname}";
      print BLDCMD "\@echo off\n";
      print BLDCMD 'c:\tools\build\build_windows.bat ' .
                   '"' . join('"  "', @arglist) . '"' . "\n";
      print BLDCMD "exit";
      close(BLDCMD);
   ...
   ###############################################################
   ## End Miracast
   ## for mircast -locabuild or other non-mir type run as usual...
   ###############################################################
   ...
   if ( $emake ) {
      ...
      my $temp_script="${SYSTEMDRIVELETTER}:/temp/${build_script_name}";

      if ( $locbld ) {
         if ( ! -s "$temp_script") {
            &Info("Creating Local build script instance $temp_script");
            &CmdQL("cp -v $build_script $temp_script");
            if ( ! -f "$temp_script" ) {
               # This assumes that default_bldserver is at-least up, if not alert
               # admin users
               $errmsgs .= "ERROR: Temp $temp_script couldn't be created\n";
               $errmsgs .= "ERROR: Ensure that ${SYSTEMDRIVELETTER}: is accessible from $thishost\n";
            }
         }
      }
...
sub Main
{
   ...
   foreach my $srv (@aEmakeServers) {
	push(@aEmakeServersTemp, $srv) unless ($srv=~/SJC-WLAN-VMLSF9/i);
   }
   ...
   my @win8ServersWdk8161=qw(
				WC-SJCA-EB101
				WC-SJCA-E104
			);

   my @win8ServersWdk9200=qw(
				SJC-WLAN-EVW13b
				SJC-WLAN-EVW14b
				SJC-WLAN-EVW15b
				SJC-WLAN-EVW16b
   ...
   # WinBlue/WinNext WDK support during initial devel.
   # The lack of a %tags...93XX means we cannot support building Win8 with 93XX at this time
   my @winblueServersWdk93XX=qw(
				WC-SJCA-EV11
				WC-SJCA-EV10
			);

   my @tagsWdk8161=qw(
				AARD118RC34_TWIG_6_30_160
				AARDVARK_REL_6_30_59_127
				AARDVARK_REL_6_30_59_87
				AARDVARK_REL_6_30_145_29
				AARDVARK_REL_6_30_95_47
				AARDVARK_REL_6_30_95_46
				AARDVARK_REL_6_30_145_30
				AARDVARK_REL_6_30_171_34
				AARDVARK_REL_6_30_59_91
			);

   my %tagsWdk8161Hash = map { $_ => 1 } @tagsWdk8161;


   my @win7ServersWdk7600=qw(
				WC-SJCA-EV02
				WC-SJCA-EV04
				WC-SJCA-EV05
				WC-SJCA-EV06
   ...
   foreach $brand ( grep { /win8_/ } @brands ) {
      my $is_8161build = exists ( $tagsWdk8161Hash{$tagname} );

      # less used feature of user builds using REPO_URL.*tagname instead of -r
      for my $tag8161 ( @tagsWdk8161 ) {
          $is_8161build ||= ($gmakeflags =~ m/REPO_URL=\S+$tag8161/ );
      }

      my @winServersTemp = ( $brand =~ /win8_slate_/ || !$is_8161build ) ? @win8ServersWdk9200 : @win8ServersWdk8161;
      $psrv{$brand} = $winServersTemp[int(rand(@winServersTemp))];
      $rand = ( $rand > 0 ) ? $rand - 1 : @winServersTemp - 1 ;
   }

   foreach $brand ( grep { /win8x_/ } @brands ) {
      my @winServersTemp = @win8xServersWdk9600;
      $psrv{$brand} = $winServersTemp[int(rand(@winServersTemp))];
      $rand = ( $rand > 0 ) ? $rand - 1 : @winServersTemp - 1 ;
   }

   foreach $brand ( grep { /winblue_/ } @brands ) {
      # If brand is a winblue_ then we force selection of 93XX build machines without exception
      # If Win8+93XX builds are desired then a new tagsWdk93XX must be made
      my @winServersTemp = @winblueServersWdk93XX;
      $psrv{$brand} = $winServersTemp[int(rand(@winServersTemp))];
      $rand = ( $rand > 0 ) ? $rand - 1 : @winServersTemp - 1 ;
   }

   foreach $brand ( grep { /win_/ } @brands ) {
      my @winServersTemp = @win7ServersWdk7600;
      $psrv{$brand} = $winServersTemp[int(rand(@winServersTemp))];
      $rand = ( $rand > 0 ) ? $rand - 1 : @winServersTemp - 1 ;
   }
   ...
   ## same as number of parallel build launch servers(wc-sjca-eb/03/04)
   ...
   foreach $brand ( @brands ) {
      ...
      if ( $localbuild ) {
         ...
         &doBuild($brand, $thishost, $ldelay, $localbuild);
         $ldelay += 15;
      } else {
         ...
         &doBuild($brand, $psrv{$brand}, $delay, $localbuild);
      }
   }
