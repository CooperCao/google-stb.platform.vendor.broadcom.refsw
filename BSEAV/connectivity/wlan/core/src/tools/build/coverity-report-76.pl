...
my $reportdate;
my $help;
my $bldadmin="hnd-software-scm-list.pdl";
my $mailto;
...
my $COVERITY_VERSION_76 = "7.6";
my $COVERITY_VERSION = "$COVERITY_VERSION_76";
...
my $COVSERVER_76 = "wlansw-cov75.sj.broadcom.com";


if (! $logdir ) {
	$logdir = "${BLDSERVER_BASE}/build_admin/logs/coverity76";
}
...
my %BrandsByPlatform = (
	...
	windows => qw(win-coverity-76),
	window  => qw(win-coverity-76),	
	linux   => qw(linux-coverity),
	linux   => qw(linux-coverity-65),
	linux   => qw(linux-coverity-76),
	macos   => qw(macos-coverity),
	macos   => qw(macos-coverity-65),
	macos   => qw(macos-coverity-76),
...
$rCatInfo = {
	...
	sta     => {
		owners  => [qw(hnd-coverity-list.pdl@broadcom.com)],
	},
	router  => {
		owners  => [qw(hnd-coverity-list.pdl@broadcom.com)],
	},
...
# 7.6
my $HTML_DRIVER_URL = "http://${COVSERVER_76}:8080/query/defects.htm?project=DRIVER&outstanding=true";
my $HTML_WL_EXE_URL = "http://${COVSERVER_76}:8080/query/defects.htm?project=WL-EXE&outstanding=true";
my $HTML_ROUTER_URL = "http://${COVSERVER_76}:8080/query/defects.htm?project=ROUTER&outstanding=true";
my $HTML_IHV_URL    = "http://${COVSERVER_76}:8080/query/defects.htm?project=IHV&outstanding=true";
