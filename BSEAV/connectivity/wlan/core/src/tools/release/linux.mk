#
# This Common makefile is used to build the following
...
# other brands like 'linux-external-media.mk --> linux-media.mk'
#
...
## --------------------------------------------------------------------------------
all: build_start checkout filelists prebuild_prep build_host_drv build_apps build_hsl release build_end
release: release_host_drv release_apps release_hsl release_src verify_pkg_apps verify_pkg_src release_cleanup
...
build_end: | build_host_drv build_apps build_hsl release
	@$(MARKEND_BRAND)
...
$(foreach oem,$(OEM_LIST),prebuild_$(oem)_prep): | filelists
	...
	mkdir -p build/$(oem)
	$(FIND) src components $(FIND_SKIPCMD) -print | $(SRCFILTER) -d -v $(SRCFILELIST_FINAL)_$(oem) | pax -rw build/$(oem)
...
$(foreach oem,$(OEM_LIST),$(foreach drv_os,$(HOST_DRV_OS_$(oem)),$(foreach drv_type,$(HOST_DRV_TYPE_$(oem)),build_host_drv_$(oem)_$(drv_type)_$(drv_os))))::
	@$(MARKSTART)
	@if [ "$(opts)" == "" ]; then \
...
$(foreach oem,$(OEM_LIST),$(foreach apps_os,$(APP_DRV_OS_$(oem)),build_apps_$(oem)_$(apps_os)))::
	@$(MARKSTART)
	@echo "opts ---> $(opts) ---> $($(opts))"
	...
	if [ "$$oem" != "android" ]; then \
		...
		$(MAKE) -C build/$(oem)/src/usbdev/usbdl $(if $(BCM_MFGTEST), WLTEST=1) $($(opts)) \
			ObjPfx=$(subst TARGETARCH=,,$(filter TARGETARCH%, $($(opts))))/; \
	fi
...
$(foreach oem,$(OEM_LIST),$(foreach hsl_os,$(APP_HSL_OS_$(oem)),build_hsl_$(oem)_$(hsl_os)))::
	@$(MARKSTART)
	@echo "opts ---> $(opts) ---> $($(opts))"
...
## --------------------------------------------------------------------------------
release_host_drv: $(foreach oem,$(OEM_LIST),release_$(oem)_host_drv) | build_host_drv
...
$(foreach oem,$(OEM_LIST),release_$(oem)_host_drv):
	@$(MARKSTART)
	...
	# Copying WPA Supplicant
	@if [ "$(oem)" == "bcm" ]; then \
		mkdir -p -v $(RELDIR)/BCG_STB/wpa_supp ; \
...
#comment#	for mdir in /projects/hnd/swbuild/build_linux/$(WPA_SUPP_TAG)/android-panda-wpa-supp/$${LATEST_BUILD}/release/external/base* ; do \
		for mdir in /projects/hnd/swbuild/build_linux/$(WPA_SUPP_TAG)/ubuntu-external-wpa-supp/$${LATEST_BUILD}/release/base* ; do \
			cp -a $${mdir} $(RELDIR)/BCG_STB/wpa_supp ; \
		done ; \
	fi
...
## --------------------------------------------------------------------------------
release_apps: $(foreach oem,$(OEM_LIST),release_$(oem)_apps) | build_apps
...
$(foreach oem,$(OEM_LIST),release_$(oem)_apps):
	@$(MARKSTART)
	install -d -m 755 $(RELDIR)
	$(call WARN_PARTIAL_BUILD,$(RELDIR))
...
## --------------------------------------------------------------------------------
release_hsl: $(foreach oem,$(OEM_LIST),release_$(oem)_hsl) | build_hsl
...
$(foreach oem,$(OEM_LIST),release_$(oem)_hsl):
	@$(MARKSTART)
	install -d -m 755 $(RELDIR)
	$(call WARN_PARTIAL_BUILD,$(RELDIR))
...
$(foreach oem,$(OEM_LIST),release_$(oem)_fw):
	@$(MARKSTART)
...
## --------------------------------------------------------------------------------
release_src: $(foreach oem,$(OEM_LIST),$(foreach pkg,$(RLS_SRC_PKG_$(oem)),release_$(oem)_src_pkg_$(pkg))) | release_host_drv release_apps release_hsl
...
$(foreach oem,$(OEM_LIST),$(foreach pkg,$(RLS_SRC_PKG_$(oem)),release_$(oem)_src_pkg_$(pkg))):
	@$(MARKSTART)
	mkdir -p $(RELDIR)
	$(call WARN_PARTIAL_BUILD,$(RELDIR))
...
## --------------------------------------------------------------------------------
verify_pkg_src: $(foreach oem,$(OEM_LIST),$(foreach pkg,$(RLS_SRC_PKG_VERIFY_DRV_$(oem)),$(foreach drv_os,$(HOST_OS_PKG_VERIFY_$(oem)),verify_host_drv_$(oem)_$(pkg)_pkg_$(drv_os)))) | release_src
...
$(foreach oem,$(OEM_LIST),$(foreach pkg,$(RLS_SRC_PKG_VERIFY_DRV_$(oem)),$(foreach drv_os,$(HOST_OS_PKG_VERIFY_$(oem)),verify_host_drv_$(oem)_$(pkg)_pkg_$(drv_os)))):
	@$(MARKSTART)
	mkdir -p $(RELDIR)
	$(call WARN_PARTIAL_BUILD,$(RELDIR))
	@if [ "$($(opts))" == "" ]; then \
...
## --------------------------------------------------------------------------------
verify_pkg_apps: $(foreach oem,$(OEM_LIST),$(foreach pkg,$(RLS_SRC_PKG_VERIFY_APPS_$(oem)),$(foreach apps_os,$(HOST_OS_PKG_VERIFY_$(oem)),verify_apps_$(oem)_$(pkg)_pkg_$(apps_os)))) | release_apps
...
$(foreach oem,$(OEM_LIST),$(foreach pkg,$(RLS_SRC_PKG_VERIFY_APPS_$(oem)),$(foreach apps_os,$(HOST_OS_PKG_VERIFY_$(oem)),verify_apps_$(oem)_$(pkg)_pkg_$(apps_os)))):
	@$(MARKSTART)
	@echo "opts ---> $(opts) ---> $($(opts))"
	if [ "$$pkg" == "dhd" ]; then \
...
.PHONY: FORCE checkout build_include prebuild_prep build_host_drv $(foreach oem,$(OEM_LIST),build_host_drv_$(oem)) release_build_clean build_apps $(foreach oem,$(OEM_LIST),$(APP_DRIVERS_$(oem))) build_hsl release release_bins release_apps release_hsl release_src
