...
#
# $Id: win8x_dhdsdio_external_wl.mk 366774 2012-11-05 06:46:14Z $
#
...
MAKE_MODE          := unix
NULL               := /dev/null
BRAND              ?= win8x_dhdsdio_internal_wl
RETRYCMD           ?= $(firstword \
                      $(wildcard \
...
# On top-level invocation win8x-dhdsdio-config.mk not yet checked out, so it doesn't included here
...
define RELEASE_SIGN_WIN8X_DRIVERS
	...
	$(MAKE) -C src/dhd/wdm release_sign_driver \
		...
		SIGN_DRIVERCAT="$(strip $3)" WINOS=$(SIGN_OS) \
...
# Arg listing:
# $1 Destinaton directory
# $2 Binary subdirectory (relative to bcmfn2\src\bcmfn2)
...
define BUILD_MSBUILD
	...
	cmd /c "call $(call INVOKE_VCVARS,$6,$5) && \
	    cd $2 && set OEM=$1 && \
		msbuild /t:build $3 /p:Configuration=$4 /p:Platform=$5 /p:UseEnv=true"
endef #BUILD_MSBUILD
...
# after checkoit by recursively call this makefile with build_firmware2 target
#
...
build_fn2: checkout mogrify
	@$(MARKSTART)
	$(MAKE) -C $(FN2_CHECKOUT_DIR)/$(FN2_PATH) VS_VER=$(VS_VER) WDK_VER=$(WIN8X_WDK_VERSION) BUILD_ARCHS="$(BUILD_ARCHS)" CONFIGURATION="$(FN2_BUILD_CONFIG)"
	@$(MARKEND)

build_driver: build_firmware
build_driver: pre_build
	@$(MARKSTART)
	$(MAKE) -C src/dhd/wdm VS_VER=$(VS_VER) WDK_VER=$(WIN8X_WDK_VERSION) BUILD_TYPES=checked BUS=sdio BUILD_ARCHS="$(BUILD_ARCHS)" -f win8xdriver.mk all
	$(MAKE) -C src/dhd/wdm VS_VER=$(VS_VER) WDK_VER=$(WIN8X_WDK_VERSION) BUILD_TYPES=checked BUS=pcie BUILD_ARCHS="$(BUILD_ARCHS)" -f win8xdriver.mk all
	$(MAKE) -C src/wl/exe VS_VER=$(VS_VER) WDK_VER=$(WIN8X_WDK_VERSION) BUILD_CONFS=Debugwb -f wl_vs13.mk all
	$(MAKE) -C src/dhd/exe VS_VER=$(VS_VER) WDK_VER=$(WIN8X_WDK_VERSION) BUILD_CONFS=Debug -f dhd_vs13.mk all
	@$(MARKEND)

build_ihv: pre_build
	@$(MARKSTART)
	$(MAKE) -C components/apps/windows/ihvfrm/ihv VS_VER=$(VS_VER) WDK_VER=$(WIN8X_WDK_VERSION) BUILD_CONFS=Debugwb BUILD_OEMS=Bcm -f build_vs13.mk
	@$(MARKEND)
...
build_app_libraries: pre_build
	@$(MARKSTART)
	$(call BUILD_MSBUILD,bcm,components\apps\windows\libraries,WLWin8_VS13.sln,Debugwb,Win32,2013)
	$(call BUILD_MSBUILD,bcm,components\apps\windows\libraries,WLWin8_VS13.sln,Debugwb,x64,2013)
	@$(MARKEND)
...
$(RELEASEDIR)/$(VERSION_NAME)/checked/DriverOnly: FORCE
	...
	$(call INSTALL_DRIVER_FILES, $@/x86/, DRIVER_FILES_INTEL DRIVER_FILES_OTHER, Bcm, , $(DRIVER_NAME), inf, win8x_sdio_checked/Win32, Debugwb, map pdb)
	$(call INSTALL_DRIVER_FILES, $@/x64/, DRIVER_FILES_INTEL DRIVER_FILES_OTHER, Bcm, 64, $(DRIVER_NAME), inf, win8x_sdio_checked/x64, x64/Debugwb, map pdb)
	$(call INSTALL_DRIVER_FILES, $@/arm/, DRIVER_FILES_ARM DRIVER_FILES_OTHER, Bcm, , $(DRIVER_NAME), inf, win8x_sdio_checked/arm, arm/Debugwb, map pdb)
	$(call INSTALL_BINARIES, $@/x86/, src/wl/exe/Debugwb/, wl.exe, pdb)
	$(call INSTALL_BINARIES, $@/x86/, src/dhd/exe/Debug/, dhd.exe, pdb)
	$(call INSTALL_BINARIES, $@/x64/, src/wl/exe/x64/Debugwb/, wl.exe, pdb)
	$(call INSTALL_BINARIES, $@/x64/, src/dhd/exe/x64/Debug/, dhd.exe, pdb)
#	$(call INSTALL_BINARIES, $@/arm/, src/wl/exe/arm/Debug/, wl.exe, pdb)
	$(call INSTALL_BINARIES, $@/x86/, components/apps/windows/ihvfrm/ihv/Debugwb/, wapiutil.exe, pdb)
	$(call INSTALL_BINARIES, $@/x64/, components/apps/windows/ihvfrm/ihv/x64/Debugwb/, wapiutil.exe, pdb)
	$(call INSTALL_BINARIES, $@/arm/, components/apps/windows/ihvfrm/ihv/arm/Debugwb/, wapiutil.exe, pdb)
	...
	$(call INSTALL_FN2_DRIVER_FILES, $@/arm/, ARM/$(subst $(space),$(empty),$(FN2_BUILD_CONFIG)), pdb)
	$(call RELEASE_SIGN_WIN8X_DRIVERS, $@/x86, X86, $(DRIVER_NAME).sys@$(DRIVER_NAME).cat $(FN2_DRIVER_NAME).sys@$(FN2_DRIVER_NAME).cat)
	$(call RELEASE_SIGN_WIN8X_DRIVERS, $@/x64, X64, $(DRIVER_NAME).sys@$(DRIVER_NAME).cat $(FN2_DRIVER_NAME).sys@$(FN2_DRIVER_NAME).cat)
#InfBroken	$(call RELEASE_SIGN_WIN8X_DRIVERS, $@/arm, ARM, $(DRIVER_NAME).sys@$(DRIVER_NAME).cat $(FN2_DRIVER_NAME).sys@$(FN2_DRIVER_NAME).cat)
...
#disabled#	$(call INSTALL_INF, $(INF_DIR)/bcm/$(WIN8X_WLINF), $@/Drivers/$(VERSION_NAME)/WL)
	$(call INSTALL_DRIVER_FILES, $@/Drivers/$(VERSION_NAME)/WL/, DRIVER_FILES_INTEL, Bcm, , $(DRIVER_NAME), inf, win8x_sdio_checked/Win32, Debugwb, map pdb)
	$(call INSTALL_BINARIES, $@/Drivers/$(VERSION_NAME)/WL/, components/apps/windows/ihvfrm/ihv/Debugwb/, wapiutil.exe, pdb)
	$(call INSTALL_FN2_DRIVER_FILES, $@/Drivers/$(VERSION_NAME)/WL/, $(subst $(space),$(empty),$(FN2_BUILD_CONFIG)), pdb)
	$(call RELEASE_SIGN_WIN8X_DRIVERS, $@/Drivers/$(VERSION_NAME)/WL, X86, $(DRIVER_NAME).sys@$(DRIVER_NAME).cat $(FN2_DRIVER_NAME).sys@$(FN2_DRIVER_NAME).cat)

	$(call INSTALL_DRIVER_FILES, $@/Drivers/$(VERSION_NAME)/WL/x64/, DRIVER_FILES_INTEL, Bcm, 64, $(DRIVER_NAME), inf, win8x_sdio_checked/x64, x64/Debugwb, map pdb)
	$(call INSTALL_BINARIES, $@/Drivers/$(VERSION_NAME)/WL/x64/, components/apps/windows/ihvfrm/ihv/x64/Debugwb/, wapiutil.exe, pdb)
	$(call INSTALL_FN2_DRIVER_FILES, $@/Drivers/$(VERSION_NAME)/WL/x64/, x64/$(subst $(space),$(empty),$(FN2_BUILD_CONFIG)), pdb)
	$(call RELEASE_SIGN_WIN8X_DRIVERS, $@/Drivers/$(VERSION_NAME)/WL/x64, X64, $(DRIVER_NAME).sys@$(DRIVER_NAME).cat $(FN2_DRIVER_NAME).sys@$(FN2_DRIVER_NAME).cat)

	$(call INSTALL_BINARIES, $@/, components/apps/windows/libraries/WlAdapter/DLL/Debugwb/, bcmwlapiu.dll,)
	$(call INSTALL_BINARIES, $@/x64/, components/apps/windows/libraries/WlAdapter/DLL/Debugwb/x64/, bcmwlapiu.dll,)

	$(call INSTALL_INSTALLSHIELD_FILES,bcm,checked,BCMAppInstaller)
	$(INSTALL) $(INS_DIR)/ini_win8x_dhdsdio/bcm/checked/driver.ini $@/bcmwls.ini
	$(INSTALL) $(INS_DIR)/ini_win8x_dhdsdio/bcm/checked/driver.ini $@/bcmwlsd.ini
	$(INSTALL) $(INS_DIR)/installshield/BCMAppInstaller/is_bin/bcm/Setup_win8x_dhdsdio.ini $@/Setup.ini
	unix2dos $@/Setup.ini

	$(INSTALL) $(INS_DIR)/Utils/PackageVersion.dll $@/

	$(call INSTALL_VCREDIST, $@/vs13,     13, checked, 32)
	$(call INSTALL_VCREDIST, $@/vs13/x64, 13, checked, 64)
