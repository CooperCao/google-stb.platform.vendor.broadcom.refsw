...
MAKE_MODE          := unix
NULL               := /dev/null
BRAND              ?= win8x_dhdsdio_external_wl
RETRYCMD           ?= $(firstword \
                      $(wildcard \
...
#OEM_LIST_WIN8X_RLSPKG   ?=  Bcm Dell
#OEM_LIST                ?=  bcm dell
...
FN2_BUILD_CONFIG = WinBlue Release
...
build_driver: pre_build
	@$(MARKSTART)
	$(MAKE) -C src/dhd/wdm VS_VER=$(VS_VER) WDK_VER=$(WIN8X_WDK_VERSION) BUILD_TYPES=free BUS=sdio BUILD_ARCHS="$(BUILD_ARCHS)" -f win8xdriver.mk all
...
build_ihv: pre_build
	@$(MARKSTART)
	$(MAKE) -C components/apps/windows/ihvfrm/ihv VS_VER=$(VS_VER) WDK_VER=$(WIN8X_WDK_VERSION) BUILD_CONFS=Releasewb BUILD_OEMS=Bcm -f build_vs13.mk
	@$(MARKEND)
...
build_app_libraries: pre_build
	@$(MARKSTART)
	$(call BUILD_MSBUILD,bcm,components\apps\windows\libraries,WLWin8_VS13.sln,Releasewb,Win32,2013)
	$(call BUILD_MSBUILD,bcm,components\apps\windows\libraries,WLWin8_VS13.sln,Releasewb,x64,2013)
	@$(MARKEND)
...
$(RELEASEDIR)/$(VERSION_NAME)/Internal: build FORCE
	...
	$(call INSTALL_DRIVER_FILES, $@/x86/, DRIVER_FILES_INTEL DRIVER_FILES_OTHER, Bcm, , $(DRIVER_NAME), inf, win8x_sdio_free/Win32, Releasewb, map pdb)
	$(call INSTALL_DRIVER_FILES, $@/x64/, DRIVER_FILES_INTEL DRIVER_FILES_OTHER, Bcm, 64, $(DRIVER_NAME), inf,win8x_sdio_free/x64, x64/Releasewb, map pdb)
	$(call INSTALL_DRIVER_FILES, $@/arm/, DRIVER_FILES_ARM DRIVER_FILES_OTHER, Bcm, , $(DRIVER_NAME), inf, win8x_sdio_free/arm, arm/Releasewb, map pdb)
...
#	$(call INSTALL_BINARIES, $@/arm/, src/wl/exe/arm/Releasewb/, wl.exe, pdb)
	$(call INSTALL_BINARIES, $@/x86/, components/apps/windows/ihvfrm/ihv/Releasewb/, wapiutil.exe, pdb)
	$(call INSTALL_BINARIES, $@/x64/, components/apps/windows/ihvfrm/ihv/x64/Releasewb/, wapiutil.exe, pdb)
	$(call INSTALL_BINARIES, $@/arm/, components/apps/windows/ihvfrm/ihv/arm/Releasewb/, wapiutil.exe, pdb)
...
#disabled#	$(call INSTALL_INF, $(INF_DIR)/bcm/$(WIN8X_WLINF), $@/Drivers/$(VERSION_NAME)/WL)
	...
	$(call INSTALL_DRIVER_FILES, $@/Drivers/$(VERSION_NAME)/WL/x64/, DRIVER_FILES_INTEL, Bcm, 64, $(DRIVER_NAME), inf, win8x_sdio_free/x64, x64/Releasewb,)
	...
	$(call INSTALL_BINARIES, $@/, components/apps/windows/libraries/WlAdapter/DLL/Releasewb/, bcmwlapiu.dll,)
	$(call INSTALL_BINARIES, $@/x64/, components/apps/windows/libraries/WlAdapter/DLL/Releasewb/x64/, bcmwlapiu.dll,)

	$(call INSTALL_INSTALLSHIELD_FILES,bcm,free,BCMAppInstaller)
	$(INSTALL) $(INS_DIR)/ini_win8x_dhdsdio/bcm/free/driver.ini $@/bcmwls.ini
	$(INSTALL) $(INS_DIR)/ini_win8x_dhdsdio/bcm/free/driver.ini $@/bcmwlsd.ini
	...
	$(call INSTALL_VCREDIST, $@/vs13,     13, free, 32)
	$(call INSTALL_VCREDIST, $@/vs13/x64, 13, free, 64)
...
$(RELEASEDIR)/$(VERSION_NAME)/Dell/Dell_DriverOnly: build FORCE
	...
	mkdir -pv $@
	$(call WARN_PARTIAL_BUILD)
	$(call INSTALL_DRIVER_FILES, $@/x86/, DRIVER_FILES_INTEL, Bcm, , $(DRIVER_NAME), inf, win8x_sdio_free/Win32, Releasewb,)
	$(call INSTALL_DRIVER_FILES, $@/x64/, DRIVER_FILES_INTEL, Bcm, 64, $(DRIVER_NAME), inf, win8x_sdio_free/x64, x64/Releasewb,)
	$(call INSTALL_DRIVER_FILES, $@/arm/, DRIVER_FILES_ARM, Bcm, , $(DRIVER_NAME), inf, win8x_sdio_free/arm, arm/Releasewb,)
	...
	$(call INSTALL_BINARIES, $@/arm/, components/apps/windows/ihvfrm/ihv/arm/Releasewb/, wapiutil.exe,)
...
#disabled#	$(call INSTALL_INF, $(INF_DIR)/dell/$(WIN8X_WLINF), $@/Drivers/$(VERSION_NAME)/WL)
	$(call INSTALL_DRIVER_FILES, $@/Drivers/$(VERSION_NAME)/WL/, DRIVER_FILES_INTEL, Bcm, , $(DRIVER_NAME), inf, win8x_sdio_free/Win32, Releasewb,)
	$(call INSTALL_BINARIES, $@/Drivers/$(VERSION_NAME)/WL/, components/apps/windows/ihvfrm/ihv/Releasewb/, wapiutil.exe,)
	$(call INSTALL_FN2_DRIVER_FILES, $@/Drivers/$(VERSION_NAME)/WL/, $(subst $(space),$(empty),$(FN2_BUILD_CONFIG)),)
	$(call RELEASE_SIGN_WIN8X_DRIVERS, $@/Drivers/$(VERSION_NAME)/WL, X86, $(DRIVER_NAME).sys@$(DRIVER_NAME).cat $(FN2_DRIVER_NAME).sys@$(FN2_DRIVER_NAME).cat)
	...
	$(INSTALL) $(INS_DIR)/ini_win8x_dhdsdio/dell/free/driver.ini $@/bcmwls.ini
	$(INSTALL) $(INS_DIR)/ini_win8x_dhdsdio/dell/free/driver.ini $@/bcmwlsd.ini
	$(INSTALL) $(INS_DIR)/installshield/BCMAppInstaller/is_bin/dell/Setup_win8x_dhdsdio.ini $@/Setup.ini
	unix2dos $@/Setup.ini
