...
#
# $Id: win10_external_wl.mk 594886 2015-10-24 01:12:19Z $
#
...
WIN10_x86_WLDRIVER   = $(subst $(space),$(empty),src/wl/sys/wdm/obj/win10_nic/free/x86/bcmwl63.sys)
WIN10_x64_WLDRIVER   = $(subst $(space),$(empty),src/wl/sys/wdm/obj/win10_nic/free/x64/bcmwl63a.sys)
WIN10_WLINF          = $(subst $(space),$(empty),bcmwl63.inf)

# Microsft driver included in the OS also requires a debug enabled copy without BCM INTERNAL CPP macros
# so we build it only for microsoft OEM drops. Other vendors do not need this
WIN10_x86_WLDEBUG   = src/wl/sys/wdm/obj/win10_nic/checkedexternal/x86/bcmwl63.sys
WIN10_x64_WLDEBUG   = src/wl/sys/wdm/obj/win10_nic/checkedexternal/x64/bcmwl63a.sys)

X86_WLCAT           = $(subst $(space),$(empty),bcm43xx.cat)
X64_WLCAT           = $(subst $(space),$(empty),bcm43xx64.cat)
...
UNDEFS = CONFIG_BRCM_VJ CONFIG_BCRM_93725 CONFIG_BCM93725_VJ \
	...
	CONFIG_MIPS_BRCM POCKET_PC BCMINTERNAL DSLCPE LINUXSIM BCMSIM       \
	BCMSIMUNIFIED WLNINTENDO WLNINTENDO2
...
build_driver: mogrify build_include
	...
	$(MAKE) -C src/wl/sys/wdm WIN10_WDK_VER=$(WIN10_WDK_VERSION) \
                                  WINOS=$(UPPER_VERSION_NAME) BUILD_TYPES=free \
                                  $(if $(strip ${SERVER_PATH}),SERVER_PATH=$(strip ${SERVER_PATH})) \
                                  build_win10_driver

	$(MAKE) -C src/wl/exe -f wl_vs13.mk BUILD_CONFS=Releasewb all
	@$(MARKEND)
...
pre_release:
	...
	@[ -d release/$(VERSION_NAME)/Bcm ] || mkdir -pv release/$(VERSION_NAME)/Bcm
	@[ -d release/$(VERSION_NAME)/Internal ] || mkdir -pv release/$(VERSION_NAME)/Internal
	@$(MARKEND)

release: release_win10
release_win10: pre_release
release_win10: $(RELEASEDIR)/$(VERSION_NAME)/Bcm

##############################################################################
# release_win10 for Win10 - Win10 only DriverOnly and Internal are supported
##############################################################################
...
# Win10
$(RELEASEDIR)/$(VERSION_NAME)/Bcm/Bcm_DriverOnly: FORCE
	@$(MARKSTART)
	mkdir -pv $@
	$(call WARN_PARTIAL_BUILD)
	$(INSTALL) $(WIN10_x86_WLDRIVER) $@/
	$(INSTALL) $(WIN10_x64_WLDRIVER) $@/
	$(call INSTALL_INF,$(INF_DIR)/bcm/$(WIN10_WLINF),$@/)
	$(call RELEASE_SIGN_WIN10_DRIVER,$@,X86)
	$(call RELEASE_SIGN_WIN10_DRIVER,$@,X64)
	$(call REMOVE_WARN_PARTIAL_BUILD)
	@$(MARKEND)
#	# End of $(RELEASEDIR)/Win10/Bcm/Bcm_DriverOnly

################################################################################
# Release the Bcm Internal misc folder
################################################################################

$(RELEASEDIR)/$(VERSION_NAME)/Internal: FORCE
	@$(MARKSTART)
	$(INSTALL) src/wl/exe/Releasewb/wl.exe $@/
	$(call INSTALL_UNSIGNED_DRIVER_CAT,$@)
	$(INSTALL) $(WIN10_x86_WLDRIVER) $@/
	$(INSTALL) $(subst .sys,.pdb,$(WIN10_x86_WLDRIVER)) $@/free_$(basename $(notdir $(WIN10_x86_WLDRIVER))).pdb
	$(INSTALL) $(subst .sys,.map,$(WIN10_x86_WLDRIVER)) $@/free_$(basename $(notdir $(WIN10_x86_WLDRIVER))).map
	$(INSTALL) $(WIN10_x64_WLDRIVER) $@/
	$(INSTALL) $(subst .sys,.pdb,$(WIN10_x64_WLDRIVER)) $@/free_$(basename $(notdir $(WIN10_x64_WLDRIVER))).pdb
	$(INSTALL) $(subst .sys,.map,$(WIN10_x64_WLDRIVER)) $@/free_$(basename $(notdir $(WIN10_x64_WLDRIVER))).map
	$(call INSTALL_INF,$(INF_DIR)/bcm/$(WIN10_WLINF),$@/)
	@$(MARKEND)
#	# End of $(RELEASEDIR)/WIN10/Internal
