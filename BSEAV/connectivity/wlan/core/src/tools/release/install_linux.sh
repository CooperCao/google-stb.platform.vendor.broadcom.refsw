...
if [ -f hndtools-*.tar.gz ] ; then
    echo -n "Tools installation path [/opt]: "
    read tools
    [ "${tools}" = "" ] && tools="/opt"

    echo -n "Installing tools to ${tools}..."
    (cd ${tools} && tar xzf hndtools-*.tar.gz)
    echo "done"
fi
