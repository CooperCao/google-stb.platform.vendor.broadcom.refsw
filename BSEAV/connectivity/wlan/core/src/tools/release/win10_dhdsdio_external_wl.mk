...
MAKE_MODE       := unix
NULL            := /dev/null
BRAND           ?= win10_dhdsdio_external_wl
RETRYCMD        ?= $(firstword \
                     $(wildcard \
...
FN2_BUILD_CONFIG = WinThresh Release
#FN2_BUILD_CONFIG = Win10 Release
...
# is started in secondary make, when all prerequisites are already built
HNDRTE_DEPS :=

# On top-level invocation win10-dhdsdio-config.mk is not yet checked out, so it is not included here
...
#build_end: package
#package: build $(OEM_LIST_WIN10_RLSPKG:%=$(RELEASEDIR)/$(VERSION_NAME)/%)
...
build: pre_build \
	...
	build_install \
	build_app_libraries \
	$(OEM_LIST_WIN10_RLSPKG:%=$(RELEASEDIR)/$(VERSION_NAME)/%)
...
build_driver: pre_build
	@$(MARKSTART)
	$(MAKE) -C src/dhd/wdm WINOS=$(WINOS) VS_VER=$(VS_VER) WDK_VER=$(WIN10_WDK_VERSION) BUILD_TYPES=free BUS=sdio BUILD_ARCHS="$(BUILD_ARCHS)" -f win8xdriver.mk all
	$(MAKE) -C src/dhd/wdm WINOS=$(WINOS) VS_VER=$(VS_VER) WDK_VER=$(WIN10_WDK_VERSION) BUILD_TYPES=free BUS=pcie BUILD_ARCHS="$(BUILD_ARCHS)" -f win8xdriver.mk all
	$(MAKE) -C src/wl/exe WINOS=$(WINOS) VS_VER=$(VS_VER) WDK_VER=$(WIN10_WDK_VERSION) BUILD_CONFS=Releasewb -f wl_vs13.mk all
	$(MAKE) -C src/dhd/exe WINOS=$(WINOS) VS_VER=$(VS_VER) WDK_VER=$(WIN10_WDK_VERSION) BUILD_CONFS=Release -f dhd_vs13.mk all
	@$(MARKEND)

build_ihv: pre_build
	@$(MARKSTART)
	$(MAKE) -C components/apps/windows/ihvfrm/ihv VS_VER=$(VS_VER) WDK_VER=$(WIN10_WDK_VERSION) BUILD_CONFS=Releasewb BUILD_OEMS=Bcm -f build_vs13.mk
	@$(MARKEND)
...
build_app_libraries: pre_build
	@$(MARKSTART)
	$(call BUILD_MSBUILD,bcm,components\apps\windows\libraries,WLWin8_VS15.sln,Releasew15,Win32,2015)
	$(call BUILD_MSBUILD,bcm,components\apps\windows\libraries,WLWin8_VS15.sln,Releasew15,x64,2015)
	@$(MARKEND)

pre_build: checkout
	@$(MARKSTART)
	@mkdir -pv release
	@for oem in $(OEM_LIST_WIN10_RLSPKG); do \
...
$(RELEASEDIR)/$(VERSION_NAME)/Bcm: \
	...
	$(RELEASEDIR)/$(VERSION_NAME)/Bcm/Bcm_InstallShield/Disk1
#	$(RELEASEDIR)/$(VERSION_NAME)/DebugSymbols/Bcm

$(RELEASEDIR)/$(VERSION_NAME)/Internal: build FORCE
	@$(MARKSTART)
	mkdir -pv $@
	$(call WARN_PARTIAL_BUILD)
	$(call INSTALL_DRIVER_FILES, $@/x86/, DRIVER_FILES_INTEL DRIVER_FILES_OTHER, Bcm, , $(DRIVER_NAME), inf, wdi_dhd_sdio_free/Win32, Releasew15, map pdb)
	$(call INSTALL_DRIVER_FILES, $@/x64/, DRIVER_FILES_INTEL DRIVER_FILES_OTHER, Bcm, 64, $(DRIVER_NAME), inf, wdi_dhd_sdio_free/x64, x64/Releasew15, map pdb)
#	$(call INSTALL_DRIVER_FILES, $@/arm/, DRIVER_FILES_ARM DRIVER_FILES_OTHER, Bcm, , $(DRIVER_NAME), inf, wdi_dhd_sdio_free/arm, arm/Releasew15, map pdb)
#	$(call INSTALL_BINARIES, $@/x86/, src/wl/exe/Releasewb/, wl.exe, map pdb)
#	$(call INSTALL_BINARIES, $@/x86/, src/dhd/exe/Release/, dhd.exe, map pdb)
#	$(call INSTALL_BINARIES, $@/x64/, src/wl/exe/x64/Releasewb/, wl.exe, map pdb)
#	$(call INSTALL_BINARIES, $@/x64/, src/dhd/exe/x64/Release/, dhd.exe, map pdb)
#	$(call INSTALL_BINARIES, $@/arm/, src/wl/exe/arm/Releasewb/, wl.exe, map pdb)
#	$(call INSTALL_BINARIES, $@/x86/, components/apps/windows/ihvfrm/ihv/Releasewb/, wapiutil.exe, map pdb)
#	$(call INSTALL_BINARIES, $@/x64/, components/apps/windows/ihvfrm/ihv/x64/Releasewb/, wapiutil.exe, map pdb)
	$(call INSTALL_FN2_DRIVER_FILES, $@/x86/, $(subst $(space),$(empty),$(FN2_BUILD_CONFIG)), map pdb)
	$(call INSTALL_FN2_DRIVER_FILES, $@/x64/, x64/$(subst $(space),$(empty),$(FN2_BUILD_CONFIG)), map pdb)
...
$(RELEASEDIR)/$(VERSION_NAME)/Bcm/Bcm_InstallShield/Disk1: build FORCE
	@$(MARKSTART)
	mkdir -pv $@
	rm -rf $@/*
	$(call WARN_PARTIAL_BUILD)
...
#disabled#	$(call INSTALL_INF, $(INF_DIR)/bcm/$(WIN10_WLINF), $@/Drivers/$(VERSION_NAME)/WL)
	$(call INSTALL_DRIVER_FILES, $@/Drivers/$(VERSION_NAME)/WL/, DRIVER_FILES_INTEL, Bcm, , $(DRIVER_NAME), inf, wdi_dhd_sdio_free/Win32, Releasewb,)
#	$(call INSTALL_BINARIES, $@/Drivers/$(VERSION_NAME)/WL/, components/apps/windows/ihvfrm/ihv/Releasewb/, wapiutil.exe,)
#	$(call INSTALL_DRIVER_FILES, $@/Drivers/$(VERSION_NAME)/WL/, DRIVER_FILES_INTEL, Bcm, , $(DRIVER_NAME), inf, win8x_sdio_free/Win32, Releasewb,)
#	$(call INSTALL_BINARIES, $@/Drivers/$(VERSION_NAME)/WL/, components/apps/windows/ihvfrm/ihv/Releasewb/, wapiutil.exe,)
	$(call INSTALL_FN2_DRIVER_FILES, $@/Drivers/$(VERSION_NAME)/WL/, $(subst $(space),$(empty),$(FN2_BUILD_CONFIG)),)
	...
	$(call INSTALL_DRIVER_FILES, $@/Drivers/$(VERSION_NAME)/WL/x64/, DRIVER_FILES_INTEL, Bcm, 64, $(DRIVER_NAME), inf, wdi_dhd_sdio_free/x64, x64/Releasewb,)
#	$(call INSTALL_BINARIES, $@/Drivers/$(VERSION_NAME)/WL/x64/, components/apps/windows/ihvfrm/ihv/x64/Releasewb/, wapiutil.exe,)
	$(call INSTALL_FN2_DRIVER_FILES, $@/Drivers/$(VERSION_NAME)/WL/x64/, x64/$(subst $(space),$(empty),$(FN2_BUILD_CONFIG)),)
#	$(call RELEASE_SIGN_WIN10_DRIVERS, $@/Drivers/$(VERSION_NAME)/WL/x64, X64, $(DRIVER_NAME).sys@$(DRIVER_NAME).cat $(FN2_DRIVER_NAME).sys@$(FN2_DRIVER_NAME).cat)
	...
	$(INSTALL) $(INS_DIR)/ini_win10_dhdsdio/bcm/free/driver.ini $@/bcmwls.ini
	$(INSTALL) $(INS_DIR)/ini_win10_dhdsdio/bcm/free/driver.ini $@/bcmwlsd.ini
...
$(RELEASEDIR)/$(VERSION_NAME)/Dell/Dell_DriverOnly: OEM=Dell

$(RELEASEDIR)/$(VERSION_NAME)/Bcm/Bcm_DriverOnly \
$(RELEASEDIR)/$(VERSION_NAME)/Dell/Dell_DriverOnly: build FORCE
	@$(MARKSTART)
	mkdir -pv $@
	$(call WARN_PARTIAL_BUILD)
	$(call INSTALL_DRIVER_FILES, $@/x86/, DRIVER_FILES_INTEL, Bcm, , $(DRIVER_NAME), inf, wdi_dhd_sdio_free/Win32, Releasewb,)
	$(call INSTALL_DRIVER_FILES, $@/x64/, DRIVER_FILES_INTEL, Bcm, 64, $(DRIVER_NAME), inf, wdi_dhd_sdio_free/x64, x64/Releasewb,)
#	$(call INSTALL_DRIVER_FILES, $@/arm/, DRIVER_FILES_ARM, Bcm, , $(DRIVER_NAME), inf, wdi_dhd_sdio_free/arm, arm/Releasewb,)
#	$(call INSTALL_BINARIES, $@/x86/, components/apps/windows/ihvfrm/ihv/Releasewb/, wapiutil.exe,)
#	$(call INSTALL_BINARIES, $@/x64/, components/apps/windows/ihvfrm/ihv/x64/Releasewb/, wapiutil.exe,)

	$(call INSTALL_FN2_DRIVER_FILES, $@/x86/, $(subst $(space),$(empty),$(FN2_BUILD_CONFIG)),)
	$(call INSTALL_FN2_DRIVER_FILES, $@/x64/, x64/$(subst $(space),$(empty),$(FN2_BUILD_CONFIG)),)
#	$(call INSTALL_FN2_DRIVER_FILES, $@/arm/, ARM/$(subst $(space),$(empty),$(FN2_BUILD_CONFIG)),)
	$(call RELEASE_SIGN_WIN10_DRIVERS, $@/x86, X86, $(DRIVER_NAME).sys@$(DRIVER_NAME).cat $(FN2_DRIVER_NAME).sys@$(FN2_DRIVER_NAME).cat)
	$(call RELEASE_SIGN_WIN10_DRIVERS, $@/x64, X64, $(DRIVER_NAME).sys@$(DRIVER_NAME).cat $(FN2_DRIVER_NAME).sys@$(FN2_DRIVER_NAME).cat)
...
$(RELEASEDIR)/$(VERSION_NAME)/Dell: \
	$(RELEASEDIR)/$(VERSION_NAME)/Internal \
	$(RELEASEDIR)/$(VERSION_NAME)/Dell/Dell_DriverOnly \
	$(RELEASEDIR)/$(VERSION_NAME)/Dell/Dell_InstallShield/Disk1
#	$(RELEASEDIR)/$(VERSION_NAME)/DebugSymbols/Dell

$(RELEASEDIR)/$(VERSION_NAME)/Dell/Dell_InstallShield/Disk1: build FORCE
	@$(MARKSTART)
	mkdir -pv $@
	rm -rf $@/*
	$(call WARN_PARTIAL_BUILD)
...
#disabled#	$(call INSTALL_INF, $(INF_DIR)/dell/$(WIN10_WLINF), $@/Drivers/$(VERSION_NAME)/WL)
	$(call INSTALL_DRIVER_FILES, $@/Drivers/$(VERSION_NAME)/WL/, DRIVER_FILES_INTEL, Bcm, , $(DRIVER_NAME), inf, wdi_dhd_sdio_free/Win32, Releasewb,)
#	$(call INSTALL_BINARIES, $@/Drivers/$(VERSION_NAME)/WL/, components/apps/windows/ihvfrm/ihv/Releasewb/, wapiutil.exe,)
	$(call INSTALL_FN2_DRIVER_FILES, $@/Drivers/$(VERSION_NAME)/WL/, $(subst $(space),$(empty),$(FN2_BUILD_CONFIG)),)
	$(call RELEASE_SIGN_WIN10_DRIVERS, $@/Drivers/$(VERSION_NAME)/WL, X86, $(DRIVER_NAME).sys@$(DRIVER_NAME).cat $(FN2_DRIVER_NAME).sys@$(FN2_DRIVER_NAME).cat)
...
#	$(call INSTALL_BINARIES, $@/Drivers/$(VERSION_NAME)/WL/x64/, components/apps/windows/ihvfrm/ihv/x64/Releasewb/, wapiutil.exe,)
	...
	$(INSTALL) $(INS_DIR)/ini_win10_dhdsdio/dell/free/driver.ini $@/bcmwls.ini
	$(INSTALL) $(INS_DIR)/ini_win10_dhdsdio/dell/free/driver.ini $@/bcmwlsd.ini
	$(INSTALL) $(INS_DIR)/installshield/BCMAppInstaller/is_bin/dell/Setup_win10_dhdsdio.ini $@/Setup.ini
	unix2dos $@/Setup.ini
	$(INSTALL) $(INS_DIR)/Utils/PackageVersion.dll $@/
	$(call REMOVE_WARN_PARTIAL_BUILD)
	@$(MARKEND)
