...
ifneq ($(filter DHD_TWIG_1_201_88,$(TAG)),)
  LINUX_DHD_OPTS :=
  LINUX_DHD_OPTS += LINUXVER=3.11.1-200.fc19.x86_64
  LINUX_DHD_OPTS += LINUXDIR=/tools/linux/src/linux-3.11.1-200.fc19.x86_64
  LINUX_DHD_OPTS += CROSS_COMPILE=/tools/bin/
  LINUX_DHD_OPTS += GCCVER=4.8.1
  LINUX_DHD_OPTS += TARGET_OS=fc19
  LINUX_DHD_OPTS += TARGET_ARCH=x86_64

  LINUX_DHD_OPTS_ANDROID_ARM :=
  LINUX_DHD_OPTS_ANDROID_ARM += LINUXVER=3.14.13-1.2pre
  LINUX_DHD_OPTS_ANDROID_ARM += LINUXDIR=/projects/hnd/tools/linux/BCG/stblinux-3.14-1.2/linux
  LINUX_DHD_OPTS_ANDROID_ARM += CROSS_COMPILE=/projects/hnd/tools/linux/BCG/stbgcc-4.8-1.0/bin/arm-linux-

  LINUX_DHD_OPTS_ANDROID_MIPS :=
  LINUX_DHD_OPTS_ANDROID_MIPS += LINUXVER=3.3.8-3.3
  LINUX_DHD_OPTS_ANDROID_MIPS += LINUXDIR=/projects/hnd/tools/Linux/BCG/stblinux-3.3-3.3/linux
  LINUX_DHD_OPTS_ANDROID_MIPS += CROSS_COMPILE=/projects/hnd/tools/Linux/BCG/stbgcc-4.5.4-2.8/bin/mipsel-linux-
endif # DHD_TWIG_1_201_88
...
ifeq ($(findstring Darwin,$(UNAME)),Darwin)
  ...
  HNDSVN_BOMS_GCLIENT         += wl-build
  TEMP                        := /tmp
  PREVENTDIR                  ?= $(TOOLSDIR)/macos/Coverity/cov-sa-macosx-7.5.1
  AUTH                        ?= /home/$(COVADMIN)/.restricted/passwd
  COV_PWD                     ?= $(shell cat $(AUTH))
endif # Darwin
...
ifdef COV-LINUXBUILD
	...
	$(INSTALL) $(BUILD_TREE)$(ROUTER_TREE)/config/$(ROUTERCFG) $(BUILD_TREE)$(ROUTER_TREE)/.config
	$(LINUX_ROUTER_MIPS_PATH) $(COVMAKE) -j 4 LINUX_VERSION=$(LINUX_VERSION) -C $(BUILD_TREE)$(ROUTER_TREE) oldconfig
	@echo "$(LINUX_ROUTER_MIPS_PATH) $(COVBUILD) $(COVMAKE) -j 4 LINUX_VERSION=$(LINUX_VERSION) -C $(BUILD_TREE)$(ROUTER_TREE)"; \
	       $(LINUX_ROUTER_MIPS_PATH) $(COVBUILD) $(COVMAKE) -j 4 LINUX_VERSION=$(LINUX_VERSION) -C $(BUILD_TREE)$(ROUTER_TREE); \
	       $(CHECK_COVBUILD_STATUS)
	$(call RUN_COVERITY_ANALYZE);
	$(call RUN_COVERITY_COMMIT,$@)
	$(call RUN_COVERITY_CUSTOM_CHECKERS,$@)
