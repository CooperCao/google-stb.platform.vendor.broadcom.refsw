 ...
 * rempci_read() and rempci_write() plus a dedicated thread processing
 * responses and interrupt msgs arriving on the client-side socket,
 * plus a dedicated interrupt thread.
 *
 * The server-side supports a single-threaded rempci_recv()/rempci_reply() loop
 * plus the occational rempci_sendinterrupt().
 */
...
int rempci_recv(pci_transaction_t *t, int s);


int
rempci_client_init(char *host, int port)
{
	struct hostent *hp;
...
rempci_server_init(int *port)
{
	...
	if ((pci_ssocket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0)
		return (-1);
	...
	rc = bind(pci_ssocket, (struct sockaddr *)&sin, sizeof(struct sockaddr));
	if (rc < 0)
		syserr("bind");
	...
	listen(pci_ssocket, 1);

	addrlen = sizeof(sin);
	if ((fd = accept(pci_ssocket, (struct sockaddr *)&sin, &addrlen)) < 0)
		syserr("accept");
...
int
rempci_read(uint pci_addr, uchar *dst, uint nbytes, uint cfgread)
{
	struct pciop *op;
	int cmd;
	int seq;
	...
	if (pciverbose)
		printf("rempci_read: addr 0x%x nbytes %d: ", pci_addr, nbytes);
...
int
rempci_write(uint pci_addr, uchar *src, uint nbytes, uint cfgwrite)
{
	struct pciop *op;
	int seq;
	int cmd;
	...
	if (pciverbose)
		printf("rempci_write: addr 0x%x nbytes %d: ", pci_addr, nbytes);
...
void
rempci_copyin(uchar *src, uint32 dst, uint n)
{
	int size;
	uint resid;
	int rc;
	...
	while (resid > 0) {
		...
		rc = rempci_write(dst, src, size, 0);
		ASSERT(rc == 0);
...
void
rempci_copyout(uint32 src, uchar *dst, uint n)
{
	int size;
	uint resid;
	int rc;
...
/* copy from host memory to pci memory */
void
rempci_cfg_copyin(uchar *src, uint32 dst, uint n)
{
	int size;
	uint resid;
	int rc;
	...
	while (resid > 0) {
		size = MIN(resid, (PCI_MAX_BURST_SIZE - (dst & 3)));
		rc = rempci_write(dst, src, size, TRUE);
		ASSERT(rc == 0);
...
void
rempci_cfg_copyout(uint32 src, uchar *dst, uint n)
{
	int size;
	uint resid;
	int rc;
	...
	while (resid > 0) {
		...
		rc = rempci_read(src, dst, size, TRUE);
		ASSERT(rc == 0);
...
int
rempci_reply(pci_transaction_t *t)
{
	if (pciverbose) {
		printf("pci_reply: ");
		pci_print(t, 0);
...
pci_loop()
{
	 ...
	 */
	while (1) {
		(void) rempci_recv(&t, pci_csocket);
 ...
 */
int
rempci_recv(pci_transaction_t *t, int s)
{
	int bytes;
	...
	/* sanity check */
	ASSERT((t->command == pci_cmd_read) ||
	       (t->command == pci_cmd_write) ||
...
int
rempci_sendinterrupt(uint value)
{
	pci_transaction_t t;
	int seq;

	if (pciverbose)
		printf("pci_sendinterrupt\n");
...
rempci_kill(void)
{
	...
	op->t.dest = PCI_DEST_MASTER;
	op->t.command = htonl(pci_cmd_kill);
...
pci_print(pci_transaction_t *t, uint request)
{
	...
	printf("sequence %d %s pci_addr 0x%x size %d result %d", t->sequence,
	       cmdname(t->command), t->pci_addr, t->size, t->result);
...
pci_htol(pci_transaction_t *t)
{
	...
	t->first_byte_en = htol32(t->first_byte_en);
	t->last_byte_en = htol32(t->last_byte_en);
