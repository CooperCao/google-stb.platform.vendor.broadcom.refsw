...
{
	LPTSTR prohibited[] = { _T("WAN"), _T("AsyMac"), NULL };
	LPTSTR *pw;

	for (pw = &prohibited[0]; *pw; pw++) {
		if (_tcsstr(Data, *pw) != NULL) 
			return FALSE;
	}
	return TRUE;
}
...
)
{
	...
	Status = RegQueryValueEx(hKey, _T("Description"), 0, NULL, 
							 (LPBYTE)Buffer, &ResultLength);
...
)
{
	...
	Status = RegQueryValueEx(hKey, _T("ServiceName"), 0, NULL, 
							 (LPBYTE)pdev->adaptername, &ResultLength);
	return Status;
}



#define REG_NETCARDS  _T("Software\\Microsoft\\Windows NT\\CurrentVersion\\NetWorkCards")
