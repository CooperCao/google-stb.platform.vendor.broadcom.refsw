...
--*/
{
	...
	/* BCMDL: start a task to do the download */
...
--*/
{
   ...
   BulkUsb_DbgPrint(1, ("SelectInterfaces: %d interfaces\n", numberOfInterfaces));
   tmp = interfaceList =
        ExAllocatePool(
               NonPagedPool, 
    ...
    while(interfaceNumber < numberOfInterfaces) {
        ...
        if(interfaceDescriptor) {
			...
			BulkUsb_DbgPrint(1, ("USBD_ParseConfigurationDescriptorEx Failed for interface %d\n",
					interfaceindex));
		}
    ...
    if(urb) {
        ...
        if(NT_SUCCESS(ntStatus)) {
			...
			//
			// urb->UrbSelectConfiguration.Interface is a concatenation of 
			// interface descriptor data structures.
			// The last interface is the one we want with the bulk IN/OUT pipes
			//
			intfIndex =0;
			for ( ; pCur < pEnd; pCur += pIface->Length, intfIndex++) {
				pIface = (PUSBD_INTERFACE_INFORMATION) pCur;
				ASSERT(pIface->NumberOfPipes <= 0xFF);
                BulkUsb_DbgPrint(1, ("Interface %d; num pipes %d\n", intfIndex, 
					(UCHAR) pIface->NumberOfPipes));
			}
            ...
            BulkUsb_DbgPrint(1, ("---------\n"));
            BulkUsb_DbgPrint(1, ("NumberOfPipes 0x%x\n", 
                                 Interface->NumberOfPipes));
            BulkUsb_DbgPrint(1, ("Length 0x%x\n", 
                                 Interface->Length));
            BulkUsb_DbgPrint(1, ("Alt Setting 0x%x\n", 
                                 Interface->AlternateSetting));
            BulkUsb_DbgPrint(1, ("Interface Number 0x%x\n", 
                                 Interface->InterfaceNumber));
            BulkUsb_DbgPrint(1, ("Class, subclass, protocol 0x%x 0x%x 0x%x\n",
                                 Interface->Class,
                                 Interface->SubClass,
                                 Interface->Protocol));
            //
            ...
            for(i=0; i<Interface->NumberOfPipes; i++) {
                ...
                BulkUsb_DbgPrint(1, ("---------\n"));
                BulkUsb_DbgPrint(1, ("PipeType 0x%x\n", 
                                     Interface->Pipes[i].PipeType));
                BulkUsb_DbgPrint(1, ("EndpointAddress 0x%x\n", 
                                     Interface->Pipes[i].EndpointAddress));
                BulkUsb_DbgPrint(1, ("MaxPacketSize 0x%x\n", 
                                    Interface->Pipes[i].MaximumPacketSize));
                BulkUsb_DbgPrint(1, ("Interval 0x%x\n", 
                                     Interface->Pipes[i].Interval));
                BulkUsb_DbgPrint(1, ("Handle 0x%x\n", 
                                     Interface->Pipes[i].PipeHandle));
                BulkUsb_DbgPrint(1, ("MaximumTransferSize 0x%x\n", 
                                    Interface->Pipes[i].MaximumTransferSize));
            }
