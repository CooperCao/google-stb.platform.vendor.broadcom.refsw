...
class signed_magnitude {
 ...
 public:
	...
	operator const int() const { return( get_mantissa() ); }
	...
	signed_magnitude& operator =(const long x) {
		set_mantissa(x);
		return *this;
	}
	...
	int get_msb_index(void) const {
		int m=0;
		int i=Bits-1;
		if(umantissa == 0) return(0);
		while (m==0 && i>0) {
			if (umantissa & (0x1 << i)) m = i;
			i--;
		}
		return(m);
	}
	...
	void set_mantissa(const int64_t x) { 
		signbit=(x<0) ? 1 : 0;
	...
	friend std::ostream& 
		operator <<(std::ostream& os, const signed_magnitude &r) {
		return os <<  r.signbit << " " << r.umantissa;
	}
