...
class signed_mantissa {
 ...
 public:
	...
	operator const long() const { return( get_mantissa() ); }
	...
	int get_msb_index(void) const {
		int m=0;
		int i=63;
		uint64_t mant = get_unsigned_mantissa();
		if(mant == 0) return(0);
		while (m==0 && i>0) {
			if (mant & ((uint64_t)0x1 << i)) m = i;
			i--;
		}
		return(m);
	}
	int get_shift_msbs(void) const {return(get_msb_index()-(Bits-2)); }
	...
	void set_mantissa(const int64_t x) { 	mantissa = LIMIT_BITS64(x,Bits-1);	}
	// Multiplication
	signed_mantissa operator *(const signed_mantissa& x) const {
		signed_mantissa tmp;
		tmp.Bits      = Bits + x.Bits;
	...
	// Left Shifting
	template <typename T> signed_mantissa operator <<(const T x) const {
		signed_mantissa tmp(*this);
		tmp.Bits += (int)x;
		tmp <<= (int)x;
		return tmp;
