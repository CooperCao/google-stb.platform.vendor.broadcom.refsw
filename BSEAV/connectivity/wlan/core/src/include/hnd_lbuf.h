...
#ifndef MAXPKTFRAGSZ
#define MAXPKTFRAGSZ (338 - RATE_CACHE_HDRSZ)	/**< enough to fit TxHdrs + lbuf_frag overhead */
#endif

#ifndef MAXPKTRXFRAGSZ
#define MAXPKTRXFRAGSZ  256	/**< enough to fit management pkt */
...
#define LBF_PKT_NODROP	0x0400	/**< for PROP_TXSTATUS */
#define LBF_PKT_EVENT	0x0800	/**< for PROP_TXSTATUS */
#define LBF_CHAINED	0x1000
#define LBF_RX_FRAG	0x2000
#define LBF_METADATA	0x4000
#define LBF_NORXCPL	0x8000
#define LBF_EXEMPT_MASK	0x30000
#define LBF_EXEMPT_SHIFT 16
#define LBF_PKTFETCHED	0x40000	/* indicates pktfetched packet */
#define LBF_FRWD_PKT	0x80000
#ifdef WL_MONITOR
#define LBF_MON_PKT		0x100000
#endif
#define LBF_SFD_FRAME	0x200000	/**< Indicates Short frame descriptor based frame */
#define LBF_SFD_TXC	0x400000	/**< Indicates Short frame descriptor associated with TXC */
...
#define lb_is_sfd_frame(lb) ({ \
	ASSERT(lb_sane(lb)); \
	(((lb)->flags & LBF_SFD_FRAME) != 0); \
})

#define lb_set_sfd_frame(lb) ({ \
	ASSERT(lb_sane(lb)); \
	(lb)->flags |= LBF_SFD_FRAME; \
})

#define lb_reset_sfd_frame(lb) ({ \
	ASSERT(lb_sane(lb)); \
	(lb)->flags &= ~LBF_SFD_FRAME; \
})

#define lb_is_sfd_txc(lb) ({ \
	ASSERT(lb_sane(lb)); \
	(((lb)->flags & LBF_SFD_TXC) != 0); \
})

#define lb_set_sfd_txc(lb) ({ \
	ASSERT(lb_sane(lb)); \
	(lb)->flags |= LBF_SFD_TXC; \
})

#define lb_reset_sfd_txc(lb) ({ \
	ASSERT(lb_sane(lb)); \
	(lb)->flags &= ~LBF_SFD_TXC; \
})
...
static inline int
lb_is_sfd_frame(struct lbuf *lb)
{
	ASSERT(lb_sane(lb));
	return ((lb->flags & LBF_SFD_FRAME) != 0);
}

static inline void
lb_set_sfd_frame(struct lbuf *lb)
{
	ASSERT(lb_sane(lb));
	lb->flags |= LBF_SFD_FRAME;
}
...
static inline void
lb_reset_sfd_frame(struct lbuf *lb)
{
	ASSERT(lb_sane(lb));
	lb->flags &= ~LBF_SFD_FRAME;
}
...
static inline int
lb_is_sfd_txc(struct lbuf *lb)
{
	ASSERT(lb_sane(lb));
	return ((lb->flags & LBF_SFD_TXC) != 0);
}

static inline void
lb_set_sfd_txc(struct lbuf *lb)
{
	ASSERT(lb_sane(lb));
	lb->flags |= LBF_SFD_TXC;
}
...
static inline void
lb_reset_sfd_txc(struct lbuf *lb)
{
	ASSERT(lb_sane(lb));
	lb->flags &= ~LBF_SFD_TXC;
}
