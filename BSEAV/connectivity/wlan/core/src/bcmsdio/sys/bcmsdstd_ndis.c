/*
 *  'Standard' SDIO HOST CONTROLLER driver - Windows port
 *
...
uint32 *
sdstd_reg_map(osl_t *osh, ulong pa, int size)
{
	return osl_reg_map(osh, (uint32) pa, (uint) size);
}

void
sdstd_reg_unmap(osl_t *osh, ulong va, int size)
{
	osl_reg_unmap(osh, (void *) va, (uint) size);
}
