...
sdioh_attach(osl_t *osh, void *regsva, uint irq)
{
	...
	OSL_PCI_WRITE_CONFIG(osh, PCI_BAR0_WIN, sizeof(uint32), (SI_ENUM_BASE(sd->sih) +
	                                                         SDIOH_SB_ENUM_OFFSET));

	if (!(sd->sih = si_attach(SDIOH_FPGA_ID, osh, (volatile void *)sd->sdioh_regs,
	                          PCI_BUS, NULL, NULL, 0))) {
	...
	if (sd->sd_use_dma) {
		...
		if ((sd->di = dma_attach(osh, "sdbcm", sd->sih,
		        (volatile void *)&sd->sdioh_regs->dmaregs.xmt,
				(volatile void *)&sd->sdioh_regs->dmaregs.rcv,
...
int sdbrcm_abort(sdioh_info_t *sd, uint fnum)
{
	...
	cmd_reg = SFIELD(cmd_reg, SDIOH_CMD_EXP_BSY, 1);
...
sdbrcm_start_power(sdioh_info_t *sd)
{
	...
	cmd_rsp = 0;
	sdbrcm_get_ocr(sd, &cmd_arg, &cmd_rsp);
	sd_info(("OCR = 0x%x\n", GFIELD(cmd_rsp, RSP4_IO_OCR)));
	return TRUE;
}
