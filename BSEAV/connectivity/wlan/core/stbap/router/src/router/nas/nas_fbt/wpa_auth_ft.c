 ...
 *
 * $Id: wpa_auth_ft.c 742089 2018-01-19 05:47:04Z $
 */
...
/* Cache to hold PMKR0 for each STA's */
typedef struct wpa_ft_pmk_r0_sa {
	struct wpa_ft_pmk_r0_sa *next;		/* Next PMKR0 */
	uint8 pmk_r0[PMK_LEN];			/* PMKR0 key */
	uint8 pmk_r0_name[WPA_PMK_NAME_LEN];	/* PMKR0 Name */
	uint8 spa[ETH_ALEN];			/* STA MAC */
	int pairwise;				/* Pairwise cipher suite, WPA_CIPHER_* */
	/* TODO: expiration, identity, radius_class, EAP type, VLAN ID */
	int pmk_r1_pushed;			/* PMK is pushed or not */
} wpa_ft_pmk_r0_sa_t;

/* Cache to hold PMKR1 for each STA's */
typedef struct wpa_ft_pmk_r1_sa {
	struct wpa_ft_pmk_r1_sa *next;		/* Next PMKR1 */
	uint8 pmk_r1[PMK_LEN];			/* PMKR1 key */
	uint8 pmk_r1_name[WPA_PMK_NAME_LEN];	/* PMKR1 Name */
	uint8 spa[ETH_ALEN];			/* STA MAC */
	int pairwise;				/* Pairwise cipher suite, WPA_CIPHER_* */
	bcm_timer_id rekey_timer;		/* Timer handle for rekey expire */
	/* TODO: expiration, identity, radius_class, EAP type, VLAN ID */
	fbt_t *fbt_info;			/* Pointer to FBT info */
} wpa_ft_pmk_r1_sa_t;

/* PMK R0 and R1 cache */
typedef struct wpa_ft_pmk_cache {
	wpa_ft_pmk_r0_sa_t *pmk_r0;	/* PMKR0 for all the STA's */
	wpa_ft_pmk_r1_sa_t *pmk_r1;	/* PMKR1 for all the STA's */
} wpa_ft_pmk_cache_t;

static void wpa_ft_new_pmkr1_callback(bcm_timer_id td, wpa_ft_pmk_r1_sa_t *r1);
static int wpa_ft_push_pmk_r1_of_sta(wpa_t *wpa, wpa_ft_pmk_r0_sa_t *r0);
...
static int wpa_ft_rrb_send(wpa_t *wpa, uint8 *dst, uint8 *data, size_t data_len)
{
	...
	eth = malloc(len);
	if (eth == NULL) {
		dbg(wpa->nas, "FT: Failed to allocate ethernet header memory\n");
		return -1;
	}
	...
	const uint8 *pmkid)
{
	...
	memcpy(mcast->oui, WPA2_OUI, WPA_OUI_LEN);
	...
	if (WSEC_AES_ENABLED(wsec)) {
		memcpy(ucast->list[count].oui, WPA2_OUI, WPA_OUI_LEN);
	...
	if (mode & WPA2) {
		memcpy(auth->list[count].oui, WPA2_OUI, WPA_OUI_LEN);
...
int wpa_write_mdie(wpa_t *wpa, nas_sta_t *sta, uint8 *buf, size_t len)
{
	...
	/* Check for buf length. It should not be less than sizeof rsn_mdie + type(1 byte)
	 * and length(1 byte)
	 */
	if ((pos == NULL) || (len < (2 + sizeof(struct rsn_mdie)))) {
		dbg(wpa->nas, "FT: Invalid buffer\n");
		return -1;
	}

	/* Mobile domain ID TLV format */
	...
	size_t subelem_len)
{
	...
	if ((pos == NULL) || (len < (2 + sizeof(*hdr) + 2 + FT_R1KH_ID_LEN + 2 + r0kh_id_len +
		subelem_len))) {
		dbg(wpa->nas, "FT: Invalid buffer\n");
		return -1;
	}
...
void wpa_ft_pmk_cache_deinit(struct wpa_ft_pmk_cache *cache)
{
	...
	/* Free all STA's PMKR1 cache */
	r1 = cache->pmk_r1;
	while (r1) {
		if (r1->rekey_timer) {
			TIMER_DELETE(r1->rekey_timer);
		}
		r1prev = r1;
		r1 = r1->next;
...
/* Create the timer to delete the expired PMKR1 key */
static int wpa_ft_create_pmkr1_timer(wpa_t *wpa, wpa_ft_pmk_r1_sa_t *r1)
{
	itimer_status_t ts;

	if (wpa->fbt_info.r0_key_lifetime <= 0) {
		return 0;
	}

	/* If the timer created already, Set the new expiry timeout */
	if (r1->rekey_timer) {
		wpa_set_timeout(r1->rekey_timer, wpa->fbt_info.r0_key_lifetime, 0);
	} else {
...
/* Callback to delete the expired PMKR1 key. This function will get called only if the key is
 * expired because if we get the PMKR1 we update the timer there
 */
static void wpa_ft_new_pmkr1_callback(bcm_timer_id td, wpa_ft_pmk_r1_sa_t *pmk_r1)
{
	fbt_t *fbt_info = pmk_r1->fbt_info;
	wpa_ft_pmk_cache_t *cache = fbt_info->ft_pmk_cache;
	wpa_ft_pmk_r1_sa_t *r1, *r1prev;

	if (!cache) {
		return;
	}

	/* Search the STA in cache */
	r1 = cache->pmk_r1;
	while (r1) {
		if (memcmp(r1->spa, pmk_r1->spa, ETH_ALEN) == 0) {
			/* Delete the timer */
			if (r1->rekey_timer) {
				TIMER_DELETE(r1->rekey_timer);
			}
	...
	const uint8 *pmk_r0_name, int pairwise)
{
	...
	while (r0) {
		if (memcmp(r0->spa, spa, ETH_ALEN) == 0) {
			...
			/* Needs to push the new PMKR1 for all R1KH */
			r0->pmk_r1_pushed = 0;
			goto end;
		}
		r0 = r0->next;
	...
	uint8 *pmk_r0, int *pairwise)
{
	...
	while (r0) {
		if (memcmp(r0->spa, spa, ETH_ALEN) == 0 &&
			memcmp(r0->pmk_r0_name, pmk_r0_name, WPA_PMK_NAME_LEN) == 0) {
	...
	const uint8 *pmk_r1_name, int pairwise)
{
	...
	while (r1) {
		if (memcmp(r1->spa, spa, ETH_ALEN) == 0) {
			...
			wpa_ft_create_pmkr1_timer(wpa, r1);
			goto end;
		}
		r1 = r1->next;
	}
	...
	uint8 *pmk_r1, int *pairwise)
{
	...
	while (r1) {
		if (memcmp(r1->spa, spa, ETH_ALEN) == 0 &&
			memcmp(r1->pmk_r1_name, pmk_r1_name, WPA_PMK_NAME_LEN) == 0) {
	...
	size_t r0kh_id_len, const uint8 *pmk_r0_name)
{
	...
	/* Search in R0kh list for the given r0kh ID */
	...
	if (r0kh == NULL) {
		dbg(wpa->nas, "FT: Peer R0KH not found in the list");
		return -1;
	}
	...
	dbg(wpa->nas, "FT: Sending rrb request");
	...
	unsigned char *ptk, size_t ptk_len)
{
	...
	memcpy(wpa->fbt_info.pmk_r1name, pmk_r1_name, WPA_PMK_NAME_LEN);
	if (!psk_local) {
	...
	int *pairwise, struct wpa_ft_ies *parse)
{
	...
	ret = wpa_ft_validate_ies(wpa, data, len, parse);
	if (ret) {
	...
	dbg(nas, "FT: R1KH_ID: %s", (uint8 *)ether_etoa(wpa->fbt_info.r1_key_holder, eabuf));

	dbg(nas, "FT: S1KH_ID: %s", (uint8 *)ether_etoa(sta->ea.octet, eabuf));
	...
	uint32 resp_ies_len, uint16 status)
{
	...
	wpa_auth_ft_build_auth_resp(wpa, sta, sta->ea.octet,
		status, resp_ies, resp_ies_len, buf, &auth_resp_len);
	...
	const uint8 *dest_bssid, uint8 *resp_ies, uint32 resp_ies_len, uint16 status)
{
	...
	dbg(wpa->nas, "FT: RRB authentication response: STA= %s"
			...
			ether_etoa(sta_addr, eabuf_tmp1), ether_etoa(current_ap, eabuf_tmp2),
			status);
	...
	*pos++ = 2; /* Action: Response */
	...
	if (err >= 0) {
		...
		if (nas_set_key(wpa->nas, &sta->ea, wpa->PTK.tk1,
			sizeof(wpa->PTK.tk1), 0, 1, 0, 0) < 0) {
			dbg(wpa->nas, "FT: unicast key rejected by driver, "
				"assuming too many associated STAs");
			cleanup_sta(wpa->nas, sta, DOT11_RC_BUSY, 0);
			free(frame);
			free(sta);
			return err;
...
static int wpa_ft_rrb_rx_resp(wpa_t *wpa,
	uint8 *src_addr, uint8 *data, size_t data_len)
{
	...
	/* Search R0KH based on the source address to decrypt the message to get PMKR1 */
	r0kh = wpa->fbt_info.r0kh_list;
	while (r0kh) {
	...
	if (r0kh == NULL) {
		dbg(wpa->nas, "FT: No matching R0KH address found for "
			"PMK-R1 pull response source address %s",
...
int
wpa_ft_rrb_rx(wpa_t *wpa, const uint8 *src_addr, const uint8 *dest_addr,
	const uint8 *data, size_t data_len)
{
	...
	if (memcmp(dest_addr, own_addr, ETH_ALEN) != 0) {
		dbg(wpa->nas, "FT: RRB received frame with destination MAC %s not matching"
			"own address dropping", eabuf_tmp2);
		return -1;
	}

	if (data_len < sizeof(*frame)) {
		dbg(wpa->nas, "FT: Too short RRB frame (data_len=%lu)\n", (unsigned long) data_len);
		return -1;
	}
	...
	dbg(wpa->nas, "FT: RRB frame - frame_type=%d packet_type=%d "
		"action_length=%d from ap_address=%s to ap_address=%s\n",
	...
	if (frame->packet_type == FT_PACKET_R0KH_R1KH_RESP)
		return wpa_ft_rrb_rx_resp(wpa, (uint8 *)src_addr, (uint8 *)data, data_len);
	if (frame->packet_type == FT_PACKET_R0KH_R1KH_PUSH)
		return wpa_ft_rrb_rx_push(wpa, (uint8 *)src_addr, (uint8 *)data, data_len);
...
/* Generate PMKR1 and push it to R1KH */
static int
wpa_ft_generate_and_push_pmk_r1(wpa_t *wpa, ft_remote_r1kh_t *r1kh_list, wpa_ft_pmk_r0_sa_t *r0)
{
	int res = -1;
	struct ft_r0kh_r1kh_push_frame frame, f;
	struct timeval now;
	...
	if (aes_wrap(strlen((const char *)r1kh_list->key), (uint8 *)r1kh_list->key,
		FT_R0KH_R1KH_PUSH_DATA_LEN, f.timestamp, frame.timestamp) != 0) {
		dbg(wpa->nas, "FT: Failed to encrypt message to "
			"destination "MACF"", ETHERP_TO_MACF(r1kh_list->br_addr));
		return -1;
	}
	...
	res = wpa_ft_rrb_send(wpa, r1kh_list->br_addr, (uint8 *) &frame, sizeof(frame));

	return res;
}

/* Generate and push PMK R1 for the STA to all the r1kh_list */
static int
wpa_ft_push_pmk_r1_of_sta(wpa_t *wpa, wpa_ft_pmk_r0_sa_t *r0)
{
	int res = 1;
	ft_remote_r1kh_t *r1kh_list;

	dbg(wpa->nas, "FT: Push PMKR1 for STA "MACF" to all R1KH\n", ETHERP_TO_MACF(r0->spa));
	/* For all the R1KHs */
	...
	while (r1kh_list) {
		...
		if (wpa_ft_generate_and_push_pmk_r1(wpa, r1kh_list, r0) < 0) {
			res = 0;
		}
		r1kh_list = r1kh_list->next;
	}

	r0->pmk_r1_pushed = res;

	return res;
}

/* Calculate and push all cached PMKR1s to all R1KHs using RRB protocol */
void
wpa_ft_push_pmk_r1(wpa_t *wpa, int pushall)
{
	wpa_ft_pmk_r0_sa_t *r0;
	...
	/* For each STA's PMK R0 generate PMKR1's for each AP(R1KH) and send */
	r0 = wpa->fbt_info.ft_pmk_cache->pmk_r0;
	while (r0) {
		/* Generate PMKR1 for this STA for all the AP and send */
		if (pushall || !r0->pmk_r1_pushed) {
			wpa_ft_push_pmk_r1_of_sta(wpa, r0);
		}
		r0 = r0->next;
	}
