 ...
 *
 * $Id: wpa.c 742089 2018-01-19 05:47:04Z $
 */
...
void wpa_reset_countermeasures(wpa_t *wpa)
{
	if (wpa && wpa->countermeasures_timer)
		wpa_countermeasures_cb((timer_t)((uintptr_t)wpa->countermeasures_timer), wpa->nas);
}
...
static int wpa_create_ptk_rekey_timer(wpa_t *wpa, nas_sta_t *sta)
{
	...
	if (sta->ptk_rekey_timer) {
		...
		ts = wpa_set_itimer(wpa->nas->timer, &sta->ptk_rekey_timer,
			...
			(uintptr_t)sta, wpa->ptk_rekey_secs, 0);
		if (ts != ITIMER_OK) {
...
wpa_request(wpa_t *wpa, nas_sta_t *sta)
{
	...
	*/
	...
	if (bcm_timer_create(nas->timer, &sta->wds_td))
		ret = ITIMER_CREATE_ERROR;
	else if (bcm_timer_connect(sta->wds_td, request_pkinit, (uintptr_t)sta))
		ret = ITIMER_CONNECT_ERROR;
...
wpa_start(wpa_t *wpa, nas_sta_t *sta)
{
	...
	if (bcm_timer_create(nas->timer, &sta->wds_td))
		ret = ITIMER_CREATE_ERROR;
	else if (bcm_timer_connect(sta->wds_td, start_pkinit, (uintptr_t)sta))
		ret = ITIMER_CONNECT_ERROR;
	else if (bcm_timer_settime(sta->wds_td, &its))
		ret = ITIMER_SET_ERROR;
	if (ret)
