...
#ifdef WL_DSI
		ulp_fcbs_add_dynamic_seq(seq_ptr,
...
#ifndef USE_FCBS_ROM
	if (ds_idx == FCBS_DS0) {
		seq_ptr = (seq_en_flags[RADIO_PD_OFF] == TRUE) ? radio_pd_seq :
				(fcbs_input_data_t *)FCBS_DYN_SEQ_SKIP;
		ulp_fcbs_add_dynamic_seq(seq_ptr,
...
ds0_radio_seq_update(phy_info_t *pi)
{
	...
	uint32 cmd_ptr = FCBS_DS0_BM_CMDPTR_BASE;
	uint32 data_ptr = FCBS_DS0_BM_DATPTR_BASE;
	...
	err = fcbs_reset_cmd_dat_ptrs(FCBS_DS0, cmd_ptr, data_ptr);
