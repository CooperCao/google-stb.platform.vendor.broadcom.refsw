...
#if !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_32) || defined(PHY_ACMAJORREV_4) || defined(PHY_ACMAJORREV_3)))
static void wlc_tiny_setup_coarse_dcc(phy_info_t *pi);
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_32) || defined(PHY_ACMAJORREV_4) || defined(PHY_ACMAJORREV_3))) */
...
static void wlc_phy_low_pwr_logen_settting_4347(phy_info_t *pi, bool mimo_bf_enable);
...
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_40)) */
void wlc_phy_low_rate_adc_enable_acphy(phy_info_t *pi, bool enable);
...
#if !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_40))
	if (ACMAJORREV_40(pi->pubpi->phy_rev) && !phy_ac_chanmgr_attach_paprr(ac_info)) {
...
static const uint32 clip2_tbl[6] =
	{0xaa98766, 0xaa98766, 0xaa99877, 0xaaaa988, 0xaaaa988, 0xaaaaa99};
...
static const uint64 rfseqext_rev37[2][11] = {
	{
		0x0074000698C3, 0x003400068C63, 0x001400068843,
		0x001400068843, 0x001400068843, 0x003400068C63,
		0x0074000498C3, 0x003400048C63, 0x001400048843,
		0x000000000000, 0x000000000000}, /* normal ADC + DACx2 mode */
	{
		0x0070020698F3, 0x003002068CF3, 0x0010020688F3,
		0x0010020688F3, 0x0010020688F3, 0x003002068CF3,
		0x0074000498C3, 0x003400048C63, 0x001400048843,
		0x000000000000, 0x000000000000}  /* low ADC + DACx2 mode */
};
...
uint16 rfseq_majrev40_tx2rx_cal_dly[] =
	{0x8, 0x4, 0x2, 0x2, 0x1, 0x3, 0x4, 0x6, 0x4, 0xa, 0x4, 0x2, 0x1, 0x1, 0x1, 0x1};
/* Rx2Tx */
static const uint16 rfseq_majrev40_rx2tx_cmd[] =
	{0x0, 0x1, 0x2, 0x8, 0x5, 0x3d, 0x3e, 0x6, 0x3, 0xf, 0x4, 0x35, 0xf, 0x0, 0x36, 0x1f};
static const uint16 rfseq_majrev40_rx2tx_dly[] =
	{0x18, 0x6, 0x6, 0x4, 0x4, 0x2, 0x1, 0x10, 0x26, 0x2, 0x1, 0x4, 0xfa, 0xfa, 0x2, 0x1};
...
#if !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_3) || defined(PHY_ACMAJORREV_2) || defined(PHY_ACMAJORREV_1)))
...
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_3) || defined(PHY_ACMAJORREV_2) || defined(PHY_ACMAJORREV_1))) */
...
#if !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_40))
static void wlc_phy_set_fdss_table_4347(phy_info_t *pi);
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_40)) */
#endif
...
int8 papr_final_clipping, int8 papr_final_scaling)
{
	...
	if (enable) {
		...
		if (ACMAJORREV_40(pi->pubpi->phy_rev)) {
			...
			uint16 gammaOffset_4347[4] = {0, 0, 0, 0};
...
#if !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_40))
static void
wlc_phy_set_fdss_table_4347(phy_info_t *pi)
{
	uint8 core;
	uint8 nbkpts;
	uint8 *fdss_tbl = NULL;
...
wlc_phy_set_reg_on_reset_acphy(phy_info_t *pi)
{
	...
	*/
...
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_1) || defined(PHY_ACMAJORREV_3))) */
#if !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_36))
	if (ACMAJORREV_36(pi->pubpi->phy_rev) && (CCT_INIT(pi->u.pi_acphy))) {
	...
	if (ACMAJORREV_36(pi->pubpi->phy_rev)) {
		...
		if (ACMINORREV_0(pi)) {
			...
			*/
			...
			/* Target var 502 (ie 10db backoff) is good for11mbps humpreduction */
			/* compared to 511. Ideally 12db is req as there is SAT for 10bit */
			/* after DVG but 5.5senloss with this */
			WRITE_PHYREG(pi, Core0_BPHY_TargetVar_log2_pt8us, 502);
			WRITE_PHYREG(pi, cckshiftbitsRefVar, 0x409c);
			MOD_PHYREG(pi, Core0MinMaxGain, maxGainValue, 0x63);
			/* With Predect Enabled, reducing Warmup helps to save some */
			/* 5us (22mhz) time budget */
			MOD_PHYREG(pi, SyncControl, WarmupDurationM1, 1);
			/* Keeping sync wait below 5us effects 1/2mbps across boards */
			MOD_PHYREG(pi, OptionalModes, WaitStateTime, 5);
			WRITE_PHYREG(pi, CCKLMSStepSize, 0x2);
		}
	...
	if (ACMAJORREV_40(pi->pubpi->phy_rev)) {
		...
		ACPHY_REG_LIST_EXECUTE(pi);
		if (CHSPEC_IS80(pi->radio_chanspec)) {
			MOD_PHYREG(pi, mu_a_mod_zf_4, mu_a_mod_zf_long_4, 0);
			MOD_PHYREG(pi, mu_a_mod_zf_4, mu_a_mod_zf_long_5, 0);
			MOD_PHYREG(pi, mu_a_mod_zf_5, mu_a_mod_zf_extra_long_4, 4);
			MOD_PHYREG(pi, mu_a_mod_zf_5, mu_a_mod_zf_extra_long_5, 4);
		}
		MOD_PHYREG(pi, overideDigiGain1, cckdigigainEnCntValue, 119);
		...
		MOD_PHYREG(pi, dot11acConfig, bphyPreDetTmOutNegEdgEn, 0);
		WRITE_PHYREG(pi, bphy_pre_detection_timeout_interval, 0x3e8);
...
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_3)) */
#if !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_33) || defined(PHY_ACMAJORREV_33)))
	if (ACMAJORREV_32(pi->pubpi->phy_rev) || ACMAJORREV_33(pi->pubpi->phy_rev)) {
		data = rfseq_majrev32_reset2rx_cmd;
		dly = rfseq_majrev32_reset2rx_dly;
	} else
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_33) || defined(PHY_ACMAJORREV_33))) */
#if !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_36))
	if (ACMAJORREV_36(pi->pubpi->phy_rev)) {
...
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_36)) */
#if !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_2))
	if (ACMAJORREV_2(pi->pubpi->phyrev) && !(PHY_IPA(pi))) {
		wlc_phy_table_write_acphy(pi, ACPHY_TBL_ID_RFSEQ, 16, 0x00,
...
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_4)) */
#if !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_33) || defined(PHY_ACMAJORREV_32)))
	if (ACMAJORREV_32(pi->pubpi->phy_rev) || ACMAJORREV_33(pi->pubpi->phy_rev)) {
		wlc_phy_table_write_acphy(pi, ACPHY_TBL_ID_RFSEQ, 16, 0x10, 16,
...
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_33) || defined(PHY_ACMAJORREV_32))) */
#if !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_36))
	if (ACMAJORREV_36(pi->pubpi->phy_rev)) {
		wlc_phy_table_write_acphy(pi, ACPHY_TBL_ID_RFSEQ, 16, 0x10, 16,
...
wlc_phy_set_regtbl_on_band_change_acphy(phy_info_t *pi)
{
	...
	if (!(ACMAJORREV_40(pi->pubpi->phy_rev) && HW_MINREV_IS(pi, 0))) {
		wlc_phy_bphymrc_hwconfig(pi);
	} else {
		pi->sh->bphymrc_en = 0;
		WRITE_PHYREG(pi, bphymrcCtrl, 0x0);
	}
		...
		FOREACH_CORE(pi, core) {
			for (idx = 0; idx < RFSEQEXT_TBL_SZ_PER_CORE_28NM; idx++) {
				wlc_phy_table_write_acphy(pi, ACPHY_TBL_ID_RFSEQEXT,
					1, core*RFSEQEXT_TBL_SZ_PER_CORE_28NM+idx, 60,
						(const void*)(&rfseqext_rev37[mode][idx]));
			}
		}
		//7271 needs to set AFE overrides off explicitely
		//to be able to join in 2g without phy_forcecal
		if (pi->u.pi_acphy->sromi->srom_low_adc_rate_en) {
			wlc_phy_low_rate_adc_enable_acphy(pi, TRUE);
		}
	}
	...
	if (ACMAJORREV_40(pi->pubpi->phy_rev)) {
		...
		uint sicoreunit;
		uint32 read_val[2], write_val[2];
		uint8 phy_coremask = pi->pubpi->phy_coremask;
	...
	if (ACMAJORREV_36(pi->pubpi->phy_rev) || ACMAJORREV_5(pi->pubpi->phy_rev) ||
		...
		!(ACMINORREV_0(pi) || ACMINORREV_1(pi))) || (ACMAJORREV_40(pi->pubpi->phy_rev))) {
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
	...
	*/
	...
	if (ACMAJORREV_40(pi->pubpi->phy_rev) &&
		...
		wlc_phy_low_pwr_logen_settting_4347(pi, 0);
	}
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_40)) */
...
#if !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_40))
		if (ACMAJORREV_40(pi->pubpi->phy_rev)) {
			wlc_phy_set_fdss_table_4347(pi);
		} else
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_40)) */
		{
...
#endif /* WL_FDSS_DISABLED */
#if !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_40))
	/* For 4347A0, set the notch filter per bandwidth */
	if (ACMAJORREV_40(pi->pubpi->phy_rev)) {
		FOREACH_CORE(pi, core) {
			if (CHSPEC_IS20(pi->radio_chanspec)) {
...
#if !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_2) || defined(PHY_ACMAJORREV_0) || defined(PHY_ACMAJORREV_5)))
...
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_2) || defined(PHY_ACMAJORREV_0) || defined(PHY_ACMAJORREV_5))) */
...
#ifndef WLTEST        /* use in place of BCM_MFGTEST */
...
chanspec_setup_regtbl_on_chan_change(phy_info_t *pi)
{
	...
	if (TINY_RADIO(pi)) {
		...
		if ((ACMAJORREV_25(pi->pubpi->phy_rev)) ||
				(ACMAJORREV_40(pi->pubpi->phy_rev))) {
			wlc_phy_farrow_setup_20694(pi, radio_data->ulp_tx_mode,
				radio_data->ulp_adc_mode);
			/* Configure AFE div */
			wlc_phy_radio20694_afe_div_ratio(pi, radio_data->ulp_tx_mode,
			radio_data->ulp_adc_mode);
		} else
			...
			for (ctr = 0; ctr < max_modes; ctr++) {
				if ((mode_mask >> ctr) & 1) {
					...
					if (ACMAJORREV_37(pi->pubpi->phy_rev) ||
						...
						// control. The biquad control fields for Rx are
						// in the Rx2Tx table entries and are initialized
						// here too.
...
wlc_phy_set_reg_on_bw_change_acphy(phy_info_t *pi)
{
	...
	if (ACMAJORREV_40(pi->pubpi->phy_rev)) {
		...
		MOD_PHYREG(pi, lesiFstrClassifierEqualizationFactor0_0, subBand0Factor,
			CHSPEC_IS20(pi->radio_chanspec) ?
			0x20: CHSPEC_IS40(pi->radio_chanspec) ? 0x20 : 0x30);
		MOD_PHYREG(pi, lesiFstrClassifierEqualizationFactor1_0, subBand3Factor,
			CHSPEC_IS20(pi->radio_chanspec) ?
			0x20: CHSPEC_IS40(pi->radio_chanspec) ? 0x20 : 0x30);
		MOD_PHYREG(pi, lesiFstrClassifierEqualizationFactor0_1, subBand0Factor,
...
#if !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_3) || defined(PHY_ACMAJORREV_1) || defined(PHY_ACMAJORREV_40)))
	if (CHSPEC_IS2G(pi->radio_chanspec)) {
	  if (ACMAJORREV_1(pi->pubpi->phy_rev) || ACMAJORREV_3(pi->pubpi->phy_rev) ||
	  ACMAJORREV_40(pi->pubpi->phy_rev)) {
...
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_3) || defined(PHY_ACMAJORREV_1) || defined(PHY_ACMAJORREV_40))) */
...
#if !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_37))
		if (ACMAJORREV_37(pi->pubpi->phy_rev)) {
			/* Override the Av/Vmid as it is a static value - no need to toggle */
			MOD_RADIO_REG_20696(pi, AUXPGA_OVR1, core, ovr_auxpga_i_sel_vmid, 0x1);
			WRITE_RADIO_REG_20696(pi, AUXPGA_VMID, core, vmid[core] & 0x3ff);
			MOD_RADIO_REG_20696(pi, AUXPGA_OVR1, core, ovr_auxpga_i_sel_gain, 0x1);
			MOD_RADIO_REG_20696(pi, AUXPGA_CFG1, core, auxpga_i_sel_gain,
				av[core] & 0x7);
		} else
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_37)) */
...
wlc_phy_set_tx_iir_coeffs(phy_info_t *pi, bool cck, uint8 filter_type)
{
	if (cck == FALSE) {
		 ...
		 */
#if !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_1) || defined(PHY_ACMAJORREV_3) || defined(PHY_ACMAJORREV_4) || defined(PHY_ACMAJORREV_0)))
...
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_1) || defined(PHY_ACMAJORREV_3) || defined(PHY_ACMAJORREV_4) || defined(PHY_ACMAJORREV_0)))	 */
	if ((ACMAJORREV_1(pi->pubpi->phy_rev) && ACMINORREV_2(pi)) ||
...
wlc_phy_rx_farrow_setup_28nm(phy_info_t *pi)
{
	...
	uint64 numerator;
#ifdef REGULATORY_DEBUG
	uint8 p_afediv_lut[] = {48, 48, 32, 96, 96, 64};
#else
	uint8 p_afediv_lut[] = {96, 48, 32, 192, 96, 64};
#endif
...
wlc_phy_populate_recipcoeffs_acphy(phy_info_t *pi)
{
...
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_0) || defined(PHY_ACMAJORREV_5))) */
#if !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_40) || defined(PHY_ACMAJORREV_2) || defined(PHY_ACMAJORREV_4)))
...
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_40) || defined(PHY_ACMAJORREV_2) || defined(PHY_ACMAJORREV_4))) */
	if (ACMAJORREV_32(pi->pubpi->phy_rev) ||
	           ACMAJORREV_33(pi->pubpi->phy_rev) ||
	           ACMAJORREV_37(pi->pubpi->phy_rev)) {
		if (!PHY_AS_80P80(pi, pi->radio_chanspec)) {
	...
	if (ACMAJORREV_0(pi->pubpi->phy_rev) || ACMAJORREV_5(pi->pubpi->phy_rev)) {
		...
		uint16 nwords_start = 12, nwords_pad = 4;
...
wlc_phy_smth(phy_info_t *pi, int8 enable_smth, int8 dump_mode)
{
	...
	if ((ACMAJORREV_1(pi->pubpi->phy_rev) && ACMINORREV_2(pi)) ||
		...
		else if (ACMAJORREV_36(pi->pubpi->phy_rev) || ACMAJORREV_40(pi->pubpi->phy_rev)) {
...
phy_ac_chanmgr_core2core_sync_setup(phy_ac_chanmgr_info_t *chanmgri, bool enable)
{
	...
	FOREACH_CORE(pi, core) {
		...
		if ((enable && (PHY_BITSCNT(phy_stf_get_data(pi->stfi)->phytxchain) < 4)) ||
		    (PHY_BITSCNT(phy_stf_get_data(pi->stfi)->phytxchain) >= 4)) {
...
chanspec_tune_phy_ACMAJORREV_37(phy_info_t *pi)
{
	...
	if (CCT_INIT(pi_ac)) {
		...
		/* TODO Need to implement DC cals. Commenting out printf */
	}
...
#if !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_40))
static void
wlc_phy_low_pwr_logen_settting_4347(phy_info_t *pi, bool mimo_bf_enable)
{
...
bool
phy_ac_chanmgr_get_pay_decode_war(phy_info_t *pi)
{
	return pi->phy_pay_decode_war;
}

void
phy_ac_chanmgr_set_pay_decode_war(phy_info_t *pi, bool val)
{
	pi->phy_pay_decode_war = val;
}
...
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_1) || defined(PHY_ACMAJORREV_2))) */
#if !defined(RADIO_ID)  || (defined(RADIO_ID) && defined(PHY_ACMAJORREV_2))
		if (!PHY_IPA(pi) && ACMAJORREV_2(pi->pubpi->phy_rev) && (ACMINORREV_1(pi))) {
			int freq;
			uint8 ch = CHSPEC_CHANNEL(pi->radio_chanspec);
			const void *chan_info = NULL;
...
#endif /* !defined(RADIO_ID)  || (defined(RADIO_ID) && defined(PHY_ACMAJORREV_2)) */
#if !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_40))
		if (!PHY_IPA(pi) && ACMAJORREV_40(pi->pubpi->phy_rev)) {
			enable = TRUE;
		} else
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && defined(PHY_ACMAJORREV_40)) */
		{
