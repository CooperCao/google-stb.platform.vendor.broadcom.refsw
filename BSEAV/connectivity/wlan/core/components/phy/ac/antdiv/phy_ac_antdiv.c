...
{
	if (txant == 1) {
		MOD_PHYREG(pi, AntSelConfig, AntCfg_Override, 1);
		...
		switch (swdiv->swdiv_swctrl_en) {
			...
			case SWDIV_SWCTRL_1:
				...
				if (ACMAJORREV_40(pi->pubpi->phy_rev))
				{
					...
					MOD_PHYREG(pi, RfctrlCoreGlobalPus, muxTxVlinOnFemCtrl,
						1 << 3);
					/* set '0' on core0 as a default value */
					MOD_PHYREG(pi, RfctrlCoreAuxTssi10, tx_vlin, 0);
					MOD_PHYREG(pi, RfctrlOverrideAuxTssi0, tx_vlin_ovr, 1);
					/* to keep routing the core1 tx path to AUX2G */
					MOD_PHYREG(pi, RfctrlCoreAuxTssi11, tx_vlin, 1);
					MOD_PHYREG(pi, RfctrlOverrideAuxTssi1, tx_vlin_ovr, 1);
				}
				break;
...
phy_acphy_swdiv_set_ant(phy_type_antdiv_ctx_t *ctx, uint8 new_ant)
{
	...
	switch (swdiv->swdiv_swctrl_en) {
		case SWDIV_SWCTRL_0:
		{
			...
			W_REG(pi->sh->osh, &pi->regs->psm_gpio_out,
...
{
	uint8 band = 0, ovr_en = 0, ovr_ant = 0, elna = 0, ant = 0, nswctrl = 8;
