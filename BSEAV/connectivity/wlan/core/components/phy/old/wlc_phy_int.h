...
/* validation macros */
#define VALID_PHYTYPE(pi)	(ISNPHY(pi) || ISACPHY(pi) || ISHTPHY(pi) || ISLCN20PHY(pi))

#define VALID_N_RADIO(radioid)  ((radioid == BCM2057_ID) || (radioid == BCM20671_ID))
...
#if defined(BCMINTPHYDBG) || defined(EXT_CBALL) || defined(BCMQT)
...
struct phy_info
{
	...
	int8	phy_spuravoid;	/* spur avoidance: NPHY, HT
							 * 0: disable, 1: auto, 2: on, 3: on2
							 */
	int8	phy_spuravoid_mode;		/* Prev chann spur mode: NPHY, HT */
	uint8	phynoise_state;			/* phy noise sample state NPHY, HT */
	uint16  phy_classifier_state;	/* NPHY, HT */
	uint16	phy_clip_state[2];		/* NPHY, HT */
	uint16	old_bphy_test;			/* NPHY, HT */
	uint16	old_bphy_testcontrol;	/* NPHY, HT */
	uint8	ldo_voltage;			/* NPHY, HT */
	bool	edcrs_threshold_lock;	/* lock the edcrs detection threshold NPHY, HT */
	uint16	phy_gpiosel;			/* NPHY, HT */
	int		cur_interference_mode;	/* Track interference mode of phy NPHY, HT */
	...
	int8	carrier_suppr_disable;	/* disable carrier suppression HT, LCN20 */
	...
	int8	fdss_level_2g_ch1[2];
	int8	fdss_level_2g_ch13[2];
...
extern void wlc_phy_clip_det_htphy(phy_info_t *pi, uint8 write, uint16 *vals);
extern void wlc_phy_resetcca_htphy(phy_info_t *pi);
