...
BCMATTACHFN(phy_ht_register_impl)(phy_info_t *pi, phy_type_info_t *ti, int bandtype)
{
	...
	if (pi->dbgi != NULL &&
	    (hti->dbgi = phy_ht_dbg_register_impl(pi, hti, pi->dbgi)) == NULL) {
		PHY_ERROR(("%s: phy_ht_dbg_register_impl failed\n", __FUNCTION__));
		goto fail;
	}
	...
	if (pi->cachei != NULL &&
	    (hti->cachei = phy_ht_cache_register_impl(pi, hti, pi->cachei)) == NULL) {
		PHY_ERROR(("%s: phy_ht_cache_register_impl failed\n", __FUNCTION__));
		goto fail;
	}

	/* Register with MISC module */
	if (pi->misci != NULL &&
		(hti->misci = phy_ht_misc_register_impl(pi, hti, pi->misci)) == NULL) {
		PHY_ERROR(("%s: phy_ht_misc_register_impl failed\n", __FUNCTION__));
		goto fail;
	}

	/* Register with Channel Manager module */
	if (pi->chanmgri != NULL &&
		(hti->chanmgri = phy_ht_chanmgr_register_impl(pi, hti, pi->chanmgri)) == NULL) {
		PHY_ERROR(("%s: phy_ht_chanmgr_register_impl failed\n", __FUNCTION__));
		goto fail;
	}

	/* Register with Bluetooth Coexistence module */
	if (pi->btcxi != NULL &&
		(hti->btcxi = phy_ht_btcx_register_impl(pi, hti, pi->btcxi)) == NULL) {
		PHY_ERROR(("%s: phy_ht_btcx_register_impl failed\n", __FUNCTION__));
		goto fail;
	}
...
BCMATTACHFN(phy_ht_unregister_impl)(phy_info_t *pi, phy_type_info_t *ti)
{
	...
	if (hti->btcxi != NULL)
		phy_ht_btcx_unregister_impl(hti->btcxi);

	/* Unregister from Channel Manager module */
	if (hti->chanmgri != NULL)
		phy_ht_chanmgr_unregister_impl(hti->chanmgri);

	/* Unregister from MISC module */
	if (hti->misci != NULL)
		phy_ht_misc_unregister_impl(hti->misci);

	/* Unregister from Cache module */
	if (hti->cachei != NULL)
		phy_ht_cache_unregister_impl(hti->cachei);
	...
	if (hti->dbgi != NULL)
		phy_ht_dbg_unregister_impl(hti->dbgi);
}
