...
if [ $# -ne 4 ]
then
  echo "Duplicates a string within a locale file"	
  ...
  echo "This will go thru all TrayappStrs.txt files in all locales and copy the string STR_FOO"
  echo "within each file with a new tag STR_BAR. Both STR_FOO and STR_BAR will remain in each"
  echo "file."
  exit 1
fi
...
do
    echo "Duplicating $fromString to $toString in $localeDir/$language/$fileName"
    ...
    cat $localeDir/$language/$fileName | sed "s/\xEF\xBB\xBF//g" | sed -e "/$fromString/p" -e "s/^$fromString\(?*\)/$toString\1/" > $tempName
    if [ $? -ne 0 ]
    then
