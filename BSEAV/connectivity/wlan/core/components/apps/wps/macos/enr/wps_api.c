...
extern uint32 wps_generatePin(char c_devPwd[8], IN bool b_display);
	...
	while (1) {
		...
		*/
		 ...
		 */
			...
			while(CallClientCallback(WPS_STATUS_IDLE,NULL) && i--) WpsSleep(100);
...
bool CallClientCallback(unsigned int uiStatus, void *data)
{
	...
	if(g_wps_join_callback!=NULL) 
	{
		...
		if((uiStatus==WPS_STATUS_SUCCESS || uiStatus==WPS_STATUS_CANCELED || uiStatus==WPS_STATUS_ERROR))
		{
			g_wps_join_callback=NULL; // Disable any more notification to the client at this point
		}
	}
...
*/
bool wps_open(void *context, fnWpsProcessCB callback, char if_name[])
{
	struct ifreq ifr;
...
void * StartThreadGetInfo(void *lpParam) 
{
	...
	if(lpParam==NULL) return ((void *) -1);
	if(!gIsOpened) return ((void *)-1);
...
bool wps_generate_pin(char *pin)
{
