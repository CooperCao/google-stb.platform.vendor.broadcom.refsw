 ...
 */
int
wps_main_args(int argc, char **argv)
{
	/* set pin to default */
	char *pin = NULL;
	char start_ok = 0;
	...
	while (argc) {
		...
		if (!strcmp(cmd, "-help")) {
			...
			ARGC_CHECK();
			val = argv[index++]; argc--;
			strncpy((char *)ssid, val, SIZE_SSID_LENGTH - 1);
			ssid[SIZE_SSID_LENGTH - 1] = 0;
			user_ssid = true;
		}
		else if (!strcmp(cmd, "-if")) {
			ARGC_CHECK();
			val = argv[index++]; argc--;
			strncpy(if_name, val, sizeof(if_name) - 1);
	...
	while (start_ok) {
		...
		printf("Start enrollment for BSSID:%x:%x:%x:%x:%x:%x\n", bssid[0],
		                                                         bssid[1],
		                                                         bssid[2],
		                                                         bssid[3],
		                                                         bssid[4],
		                                                         bssid[5]);
		ret = enroll_device(pin, ssid, wsec, bssid_ptr);

		/* done when WPS successful or no more APs */
		if (ret == WPS_SUCCESS || ap_index == apcount)
			break;
...
wps_main_str(char *str)
{
	...
	return (wps_main_args(argc, &argv[0]));
}
