...
#
# XXX hack: must be run from the src/wl/exe directory.
#

"""
Filename: ClmCurpowerDump.py
Date: 2011-07-01
Author: tommcd
Description: Reads the available country code/regrevs from the driver, then
             ...
             the regulatory, board limit and target power for the various rates for 20MHz,
...
for k in range(lstart, lend):
    cc_reg = countries[k]
    print "# country %s (%d, %d, %d)" % (cc_reg, lstart, k, lend)
    if cc_reg in countries and cc_reg not in exceptions:
        ...
        if 'Bad' not in result and 'Could not' not in result:
            ...
            for ch in channels:
                print "# country %s ch %s" % (cc_reg, ch)
                ...
                #        cleaned_output = commands.getoutput('wl dump txpwr_reg')
                    ...
                    pipe = os.popen("./wl curpower -v")
                    cleaned_output = [x.strip() for x in pipe.readlines()]
                    status = pipe.close()
                    if status:
                        sys.exit("ERROR: Failed to read wl curpower -v")
