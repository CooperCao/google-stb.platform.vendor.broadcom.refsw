...
_msch_write_profiler_tlv(wlc_msch_profiler_info_t *profiler_info, uint type, int len)
{
	...
	if (cmn->dump_status & (MSCH_EVENT_LOG_BIT | MSCH_EVENT_BIT)) {
		_msch_dump_data(profiler_info, cmn->edata, type);
	}
#endif /* DONGLEBUILD */
}
...
#ifdef EVENT_LOG_COMPILE
				event_log_tag_start(EVENT_LOG_TAG_MSCHPROFILE, EVENT_LOG_SET_WL,
					EVENT_LOG_TAG_FLAG_LOG);
				event_log_tag_start(EVENT_LOG_TAG_MSCHPROFILE_TLV, EVENT_LOG_SET_WL,
					EVENT_LOG_TAG_FLAG_LOG);
#endif /* EVENT_LOG_COMPILE */
...
#endif /* MSCH_PROFILER */
			if (cmn->edata) {
				MFREE(wlc->osh, cmn->edata, WL_MSCH_PROFILER_BUFFER_SIZE);
			}
