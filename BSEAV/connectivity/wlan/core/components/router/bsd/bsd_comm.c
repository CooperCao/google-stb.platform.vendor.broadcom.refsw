...
static int bsd_helper_proc_rpc(bsd_info_t*info)
{
	...
	BSD_RPCD("\n\nRecv ioctl rpc_ioctlfd[%d]: bytes:%d"
		...
		info->rpc_ioctlfd, bytes, (int)sizeof(ret_buf), pkt->id);
	...
	struct ether_addr *bssid)
{
	...
	int fdmax;
	int width, status = 0, bytes;
	char buf_ptr[BSD_BUFSIZE_1K], *pkt;
	...
	if (status <= 0) {
		BSD_ERROR("err from select: %s", strerror(errno));
		printf("err from select: %s", strerror(errno));
		return BSD_BSS_RESPONSE_UNKNOWN;
	}
	...
	if (info->event_fd2 !=  BSD_DFLT_FD && FD_ISSET(info->event_fd2, &fdset)) {
		...
		if (event_id != WLC_E_BSSTRANS_RESP) {
			BSD_EVENT("BSS Transit Response: wrong event_id %d\n", event_id);
			continue;
		}

		BSD_EVENT("BSS Transit Response: bytes=%d, sta="MACF", ifname=%s\n",
			bytes, ETHERP_TO_MACF(&(dpkt->event.addr)), ifname);
		...
		BSD_EVENT("BSS Transit Response: token=%x, status=%d, mac="MACF"\n",
		...
		if (bsstrans_resp->token != token) {
			/* not for the requested interface */
			BSD_EVENT("BSS Transit Response: received token %x not for token %x\n",
				bsstrans_resp->token, token);
			continue;
		}
		...
		if (bsstrans_resp->status) {
			...
			return BSD_BSS_RESPONSE_REJECT;
		}
#if 0
...
#endif
		return BSD_BSS_RESPONSE_ACCEPT;
	}
	} /* while */
