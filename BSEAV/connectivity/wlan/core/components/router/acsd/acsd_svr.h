...
#define ACS_FLAGS_CI_SCAN_DISABLED	0x80 /* Disable ci scan; overrides ACS_FLAGS_CI_SCAN  */
...
#define CI_SCAN_DISABLED(c_info) ((c_info)->flags & ACS_FLAGS_CI_SCAN_DISABLED)
...
#define ACS_TRAFFIC_INFO_UPDATE_INTERVAL	120	/* update traffic info every few mins */
#define ACS_RECENT_CHANSWITCH_TIME		240	/* Recent channel switch time */
#define ACS_CHANIM_BUF_LEN			(2*1024)

#define ACS_11H_AND_BGDFS(CI)	((CI)->rs_info.reg_11h && (CI)->acs_bgdfs != NULL)
...
#ifdef WL_MEDIA_ACSD
	/*
	 * Media uses RAW socket, when wl event is received, only source address is present
	 * in the event struct and no ifname. Need to maintain source address to idx mapping
	 */
...
typedef struct acs_chaninfo {
	...
	chanspec_t cur_chspec, dfs_forced_chspec, non_dfs_forced_chspec, recent_prev_chspec;
	bool cur_is_dfs;
...
#define ACSD_DATA_FROM_LISTEN_FD 0x100
#define ACSD_DATA_FROM_EVENT_FD 0x101
...
extern int acsd_escan_open_socket(char *name);
extern int acsd_escan_wait_for_event(int fd);
extern int acsd_escan_receive_event(int fd, char **data);
#else
extern int acs_run_cs_scan(acs_chaninfo_t *c_info);
#endif /* WL_ACSD_ESCAN */
extern void acsd_close_socket(int fd);
...
extern int acsd_open_event_socket();
extern int acsd_open_listen_socket(unsigned int port, acsd_wksp_t *d_info);
extern int acsd_check_for_data_in_event_socket(acsd_wksp_t *d_info, int *bytes);
extern int acsd_wait_for_events(struct timeval *tv, acsd_wksp_t *d_info);
extern int acsd_daemonize();
extern void acsd_set_event_fd(int fd);

#ifdef WL_MEDIA_ACSD
extern int acs_idx_from_sa(uint8 *sa);
extern int acsd_open_rawsocket(acs_chaninfo_t* c_info, int *event_fd);

extern int acsd_get_add_from_ifname(char *name, uint8 *mac);
extern int acsd_media_register_wl_event(acs_chaninfo_t* c_info);
extern int wl_ioctl(char *name, int cmd, void *buf, int len);
extern bool select_rand_value();
extern void dcs_select_rand_chspec(acs_chaninfo_t *c_info);
extern int acsd_read_config(acsd_hashtable_t *hashtable);
extern void acsd_ht_set(acsd_hashtable_t *hashtable, char *key, char *value);
#endif /* WL_MEDIA_ACSD */
#endif  /* _acsd_srv_h_ */
