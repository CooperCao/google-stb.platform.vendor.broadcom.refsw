...
#define acs_request_data acs_request_escan_data
#define acs_run_cs_scan(c_info) acs_run_escan(c_info, ACS_SCAN_TYPE_CS)
#define acs_run_ci_scan(c_info) acs_run_escan(c_info, ACS_SCAN_TYPE_CI)
#endif
...
#define ACSD_FCS(fmt, arg...) \
		do { if (acsd_debug_level & ACSD_DEBUG_FCS) \
			printf("ACSD_FCS >>%s(%d): "fmt, __FUNCTION__, __LINE__, ##arg); } while (0)
...
/* avoid entering prev chan before ACS_RECENT_CHANSWITCH_TIME  time */
#define ACS_INVALID_AVOID_PREV		0x4000
...
extern int acsd_open_socket();
extern void acsd_close_socket(int fd);
extern int acsd_ioctl(int s, char *name, wl_ioctl_t *ioc);
extern int acsd_write(int fd, char *buf, unsigned int size);
extern int acsd_read(int fd, char *buf, unsigned int size);
#endif /*  _acsd_h_ */
