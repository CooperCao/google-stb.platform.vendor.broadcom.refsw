 ...
 *
 * $Id: emfu.c 527353 2015-01-17 01:00:08Z $
 */
#include <stdio.h>
#include <sys/types.h>
#include <typedefs.h>
...
static int emf_cfg_log_level_set(char *argv[]);
static int emf_cfg_log_level_show(char *argv[]);
...
static emf_cmd_arg_t cmd_args[] =
{
	...
	},
	{
		"set",
		"loglevel",
		emf_cfg_log_level_set,
		EMF_ARGC_SET_LOG_LEVEL
	},
	{
		"show",
		"loglevel",
		emf_cfg_log_level_show,
		EMF_ARGC_SHOW_LOG_LEVEL
	}
...
static int
emf_cfg_log_level_set(char *argv[])
{
	emf_cfg_request_t req;
	uint32 *log_level = (uint32 *) req.arg;

	bzero((char *)&req, sizeof(emf_cfg_request_t));
	req.command_id = EMFCFG_CMD_LOG_LEVEL;
	req.oper_type  = EMFCFG_OPER_TYPE_SET;

	*log_level = (uint32)strtoul(argv[3], NULL, 0);
...
static int
emf_cfg_log_level_show(char *argv[])
{
	emf_cfg_request_t req;
	uint32 *loglevel = (uint32 *)&req.arg[0];
	...
	fprintf(stdout, "EMF log level=0x%x\n", *loglevel);
	return (SUCCESS);
}
