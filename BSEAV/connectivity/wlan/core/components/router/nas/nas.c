 ...
 *
 * $Id: nas.c 691236 2017-03-21 11:27:46Z kugaurav $
 */
...
cleanup_sta(nas_t *nas, nas_sta_t *sta, int reason, int driver_signal)
{
	 ...
	 */
	if (!driver_signal) {
		dbg(nas, "deauthorizing and deauthenticating %s", ether_etoa((uchar *)&sta->ea,
			eabuf));
...
driver_message_dispatch(nas_t *nas, bcm_event_t *dpkt)
{
	...
	case WLC_E_FBT:
		...
		uint16 data_len = (fbt_event->length - fbt_event->data_offset);
		uint8 *data = (uint8*)fbt_event + fbt_event->data_offset;

		if (fbt_event->type == WL_E_FBT_TYPE_FBT_OTD_AUTH) {
