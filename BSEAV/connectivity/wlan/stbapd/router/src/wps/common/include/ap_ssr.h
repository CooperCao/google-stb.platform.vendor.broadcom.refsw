...
typedef struct wps_ssr_scb_s {
	...
	unsigned char authorizedMacs[SIZE_MAC_ADDR * SIZE_AUTHORIZEDMACS_NUM]; /* <= 30 B */
...
int ap_ssr_get_authorizedMacs(unsigned char *authorizedMacs_buf);
int ap_ssr_get_union_attributes(CTlvSsrIE *ssrmsg, unsigned char *authorizedMacs_buf,
	int *authorizedMacs_len);
