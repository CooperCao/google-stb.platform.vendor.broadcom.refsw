...
static int
_add_mac_in_list(unsigned char *authorizedMacs_buf, int authorizedMacs_len, unsigned char *new_mac)
{
	int i;
	int duplicated = 0;
	unsigned char *mac;
...
int
ap_ssr_get_authorizedMacs(unsigned char *authorizedMacs_buf)
{
	int i;
	int full = 0;
...
int
ap_ssr_get_union_attributes(CTlvSsrIE *ssrmsg, unsigned char *authorizedMacs_buf,
