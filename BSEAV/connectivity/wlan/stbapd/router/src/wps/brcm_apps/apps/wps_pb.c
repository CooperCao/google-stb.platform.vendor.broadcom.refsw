 ...
 *
 * $Id: wps_pb.c 712376 2017-07-24 11:40:27Z $
 */
...
wps_pb_find_next()
{
	...
	char *wlnames, *wps_ifnames;

	int target_id = -1;
	int target_instance = 0, wps_per_bss = 0, change_bss = 0;
	int i, imax;
...
refind:
	...
	if (wps_per_bss) {
		if ((pb_last_led_id >= 0) && (pb_last_led_id != target_id)) {
			change_bss = 1;
		}
	}
	...
	if (wps_per_bss) {
		if ((change_bss) || (!*pb_ifname)) {

			/* Find next BSS WPS interface name in that list */
			find_next_in_list(wps_ifnames, pb_ifname, ifname, sizeof(ifname));

			/* Change the current WPS Push Button interface name to that BSS */
