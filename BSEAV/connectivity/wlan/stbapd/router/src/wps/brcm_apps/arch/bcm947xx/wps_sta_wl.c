 ...
 *
 * $Id: wps_sta_wl.c 715747 2017-08-14 10:47:34Z $
 */
...
void wps_fill_aplist(wl_bss_info_t *bi, uint wps_ap_count)
{
	if ((bi->ie_length) &&
		...
		ap_list[wps_ap_count].chanspec = bi->chanspec;
#ifdef MAP_TEMP_DEBUG
		printf("wps_fill_aplist ssid [%s]\n", ap_list[wps_ap_count].ssid);
#endif
	}
...
#ifdef MAP_TEMP_DEBUG
	printf("create_aplist_escan start\n");
#endif
	if (is_scan_done != WPS_ESCAN_DONE) {
...
int
join_network_with_bssid(char* ssid, uint32 wsec, unsigned char *bssid)
{
...
int
wps_get_bssid(unsigned char *bssid)
{
...
do_map_wps_escan(char *ssid)
{
...
#ifdef MAP_TEMP_DEBUG
	printf("do_map_wps_escan is going on\n");
#endif
	...
	if (ssid != NULL && (strlen(ssid) < sizeof(params->params.ssid.SSID))) {
		wps_strncpy((char*)params->params.ssid.SSID, ssid, sizeof(params->params.ssid.SSID));
...
int
do_map_wps_scan(char *ssid)
{
	int ret;
	wl_scan_params_t* params;
	int params_size = WL_SCAN_PARAMS_FIXED_SIZE + WL_NUMCHANNELS * sizeof(uint16);
#ifdef MAP_TEMP_DEBUG
	printf("do_map_wps_scan start\n");
#endif
	params = (wl_scan_params_t*)malloc(params_size);
	if (params == NULL) {
		TUTRACE((TUTRACE_ERR, "Error allocating %d bytes for scan params\n", params_size));
	...
	if (ssid != NULL && (strlen(ssid) < sizeof(params->ssid.SSID))) {
		wps_strncpy((char*)params->ssid.SSID, ssid, sizeof(params->ssid.SSID));
...
#ifdef _TUDEBUGTRACE
	if (ret) {
		TUTRACE((TUTRACE_INFO, "do_map_wps_scan : do multiap wps scan command failed\n"));
	}
#endif
#ifdef MAP_TEMP_DEBUG
	if (ret) {
		printf("do_map_wps_scan : do multiap wps scan command failed\n");
	}
#endif
