 ...
 *
 * $Id: wps_libupnp.c 551899 2015-04-24 11:55:46Z $
 */
	...
	char *databuf, int datalen, int init, char event_type)
{
	...
	WPS_HexDumpAscii("==> To UPnP", UPNP_WPS_TYPE_WE, (unsigned char*)databuf, datalen);
...
void
wps_upnp_update_init_wlan_event(int if_instance, unsigned char *mac, int init)
{
...
wps_upnp_set_ssr(int ess_id, void *data, int len, char *ssr_client_ip)
{
	...
	CTlvSsrIE ssrmsg;
	unsigned char authorizedMacs_buf[SIZE_MAC_ADDR * SIZE_AUTHORIZEDMACS_NUM] = {0};
...
wps_upnp_send_msg(int if_instance, char *buf, int len, int type)
{
	...
	WPS_HexDumpAscii("==> To UPnP", type, (unsigned char*)buf, len);
 ...
 */
static int
wps_upnp_if_init(char *lan_ifname, char *ifname, unsigned char *mac, int ess_id, int instance)
{
...
wps_upnp_init()
{
	...
	unsigned char wl_mac[SIZE_MAC_ADDR];
	char *wl_hwaddr;
...
wps_libupnp_ProcessMsg(char *ifname, char *upnpmsg, int upnpmsg_len)
{
	...
	WPS_HexDumpAscii("<== From UPnP", type, (unsigned char*)data, data_len);
	...
	case UPNP_WPS_TYPE_GDIR:
		...
		if (upnp_if->m1_buf == NULL || (now - upnp_if->m1_built_time) >= 5) {
			...
			if (upnp_if->enr_nonce == NULL) {
				upnp_if->enr_nonce = (unsigned char *)malloc(SIZE_128_BITS);
				if (upnp_if->enr_nonce == NULL)
					break;
			}

			if (upnp_if->private_key == NULL) {
				upnp_if->private_key = (unsigned char *)malloc(SIZE_PUB_KEY);
				if (upnp_if->private_key == NULL)
					break;
			}
