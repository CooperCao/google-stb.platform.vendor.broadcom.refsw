...
static uint32 wpssta_reg_config_init(wpssta_wksp_t *wksp, char *ifname, unsigned char *bssid, char oob);
...
static uint32
wpssta_reg_config_init(wpssta_wksp_t *sta_wksp, char *ifname, unsigned char *bssid, char oob)
{
	DevInfo info;
	char *value, *next;
	int auth = 0;
	...
	BCM_REFERENCE(wep_key);
	BCM_REFERENCE(wep_index);
...
wpssta_enr_config_init(char *ifname)
{
	...
	if (value[0] != '\0') {
		int val = atoi(value);
		info.map_attr = val & 0x4 ? WPS_WFA_MAP_BHSTA : 0;
	}
#endif	/* MULTIAP */
#ifdef WPS_NFC_DEVICE
	if (pre_privkey) {
...
#if defined(MULTIAP)
	if (wps_map_is_onboarding(ifname)) {
...
#ifdef BCMDBG
	{
	/* for debugging prupose, print credentials received after wps session (ie. M8) */
		WpsEnrCred *creds = (WpsEnrCred*)map_settings->data;
		char keymgmt[SIZE_20_BYTES+2] = {0};
		char nwkey[SIZE_64_BYTES+2] = {0};
		keymgmt[SIZE_20_BYTES+1] = '\0';
		nwkey[SIZE_64_BYTES+1] = '\0';

		memcpy(keymgmt, creds->keyMgmt, SIZE_20_BYTES+1);
		memcpy(nwkey, creds->nwKey, SIZE_64_BYTES+1);

		printf("%s %d ssid %s ess_id %d mode %d nwKeyLen %d encrType %d wepIndex %d nwKeyShareable %d\n",
			__func__, __LINE__, creds->ssid, map_settings->ess_id, map_settings->mode, creds->nwKeyLen,
			creds->encrType, creds->wepIndex, creds->nwKeyShareable);
		printf("keymgmt %s\n", keymgmt);
		printf("nwkey %s\n", nwkey);
	}
#endif
...
wps_map_do_configure()
{
	...
	printf("%s Backhaul ifnames %s \n", __func__, ifnames);
#endif
	if (wps_map_eap_init()) {
		TUTRACE((TUTRACE_ERR, "Multiap eap init failed \n"));
#ifdef MAP_TEMP_DEBUG
		printf("%s Multiap eap init failed\n", __func__);
#endif
		goto end;
	}

	/* first try regular scan; if back haul ssid is not found, do escan */
	foreach(ifname, ifnames, next) {
		...
		/* do regular scan and wait the scan results */
		if ((err = do_map_wps_scan(creds->ssid))) {
			TUTRACE((TUTRACE_INFO, "scan failed for ifr %s err %d.", ifname, err));
		}
		...
		if (ap_list) {
			...
			if (wpssta_get_chspec_from_scan_results(ap_list, creds->ssid, &chanspec) &&
				...
				TUTRACE((TUTRACE_INFO, "For ssid %s selected ifname is %s\n",
					creds->ssid, ifname));
				/* scan succeeded and found back haul ssid; skip escan attempt */
				goto end;
			}
		}
	}

	/* regular scan failed, try escan */
	foreach(ifname, ifnames, next) {
		int err = 0;
		time_t start, end;
	...
	if (ifname[0] != '\0') {
		...
		printf("**** Backhaul ifr not found for ssid %s \n****", creds->ssid);
	}
