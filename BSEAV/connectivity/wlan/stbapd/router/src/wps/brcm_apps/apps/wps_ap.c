 ...
 *
 * $Id: wps_ap.c 738664 2018-01-03 05:04:52Z $
 */
...
wpsap_process_msg(wpsap_wksp_t *wps_wksp, char *buf, uint32 len, uint32 msgtype)
{
	...
	case TRANSPORT_TYPE_EAP:
		WPS_HexDumpAscii("<== From EAPOL", -1, (unsigned char*)buf, len);
	...
	case TRANSPORT_TYPE_UPNP_DEV:
	{
		...
		WPS_HexDumpAscii("<== From UPnP", type, (unsigned char*)buf, len);
...
wpsap_readBackhaulConfig(wpsap_wksp_t *wps_wksp, DevInfo *ap_devinfo)
{
	...
	if (strlen(wps_wksp->backhaul_ifname)) {
		...
		if (value[0] != '\0') {
			int val = atoi(value);
			ap_devinfo->map_attr = (val & 0x2) ? WPS_WFA_MAP_BHBSS : 0;
		}
...
wpsap_wksp_init(wpsap_wksp_t *ap_wksp)
{
	...
	if (ap_eap_sm_init(ap_wksp->mc, ap_wksp->mac_sta, wps_eapol_parse_msg,
		wps_eapol_send_data, ap_wksp->eap_frag_threshold,
		ap_wksp->wps_delay_deauth_ms) != WPS_SUCCESS) {
...
wpsap_open_session(wps_app_t *wps_app, int sc_mode, unsigned char *mac, unsigned char *mac_sta,
	char *osifname, unsigned char *enr_nonce, unsigned char *pre_privkey, uint8 *authorizedMacs,
...
found:
	...
	ap_wksp->pre_privkey = pre_privkey;
