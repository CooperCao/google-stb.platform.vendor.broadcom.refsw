 ...
 *
 * $Id: wps_linux_osl.c 547005 2015-04-07 09:00:24Z $
 */
...
wps_osl_arp_get(char *client_ip, unsigned char mac[6])
{
	...
	*/
	while (fgets(line, 100, arpfile))
	{
		...
		if (ipaddrstr && hwtypestr && flagstr && hwaddrstr && maskstr && devicestr) {
...
wps_hndl_t *
wps_osl_wait_for_all_packets(char *dataBuffer, int32 *dataLen, wps_hndl_t *hndl_list)
{
	wps_hndl_t *hndl;
