 ...
 *
 * $Id: eapd.c 726453 2017-10-12 17:02:32Z $
 */
...
eapd_dump(eapd_wksp_t *nwksp)
{
	...
	cb = lte_u->cb;
	if (cb) {
		EAPD_PRINT("	 lte_u appSocket %d for %s", lte_u->appSocket, cb->ifname);
		/* print each cb ifname */
		cb = cb->next;
		while (cb) {
			EAPD_PRINT(" %s", cb->ifname);
	...
	cb = airiq->cb;
	if (cb) {
		EAPD_PRINT("	 airiq appSocket %d for %s", airiq->appSocket, cb->ifname);
		/* print each cb ifname */
		cb = cb->next;
		while (cb) {
			EAPD_PRINT(" %s", cb->ifname);
...
#endif /* BCM_DCS */
#ifdef WL_AIR_IQ
			if (lte_u_app_enabled(name))
				needStart |= eapd_add_interface(nwksp, name, EAPD_APP_LTE_U, &cb);
			if (airiq_app_enabled(name))
				needStart |= eapd_add_interface(nwksp, name, EAPD_APP_AIRIQ, &cb);
#endif /* WL_AIR_IQ */
	...
	bool bridge_interface)
{
	...
	*cbp = NULL;
	...
	switch (mode) {
		...
		case EAPD_APP_LTE_U:
			app = &nwksp->lte_u;
			app_set_eventmask = (void*) lte_u_app_set_eventmask;
			break;
		case EAPD_APP_AIRIQ:
			app = &nwksp->airiq;
			app_set_eventmask = (void*) airiq_app_set_eventmask;
			break;
#endif /* WL_AIR_IQ */
#ifdef BCM_MEVENT
...
#endif /* BCM_DCS */
#ifdef WL_AIR_IQ
		else if (!strncmp(argv[i], "-lte_u", 4)) {
			current_mode = EAPD_APP_LTE_U;
		}
		else if (!strncmp(argv[i], "-airiq", 4)) {
			current_mode = EAPD_APP_AIRIQ;
		}
#endif /* WL_AIR_IQ */
#ifdef BCM_MEVENT
...
#endif /* BCM_DCS */
#ifdef WL_AIR_IQ
	nwksp->lte_u.appSocket = -1;
	nwksp->airiq.appSocket = -1;
#endif /* WL_AIR_IQ */
#ifdef BCM_MEVENT
...
#ifdef WL_AIR_IQ
	/* initial lte_u */
	if (lte_u_app_init(nwksp)) {
		EAPD_ERROR("lte_u_app_init fail...\n");
		return -1;
	}
	/* initial airiq */
	if (airiq_app_init(nwksp)) {
		EAPD_ERROR("airiq_app_init fail...\n");
		return -1;
	}
#endif /* WL_AIR_IQ */
...
#endif /* BCM_DCS */
#ifdef WL_AIR_IQ
	lte_u_app_deinit(nwksp);
	airiq_app_deinit(nwksp);
#endif /* WL_AIR_IQ */
#ifdef BCM_MEVENT
...
eapd_wksp_find_cb(eapd_app_t *app, char *wlifname, uint8 *mac)
{
	...
	if (wl_wlif_is_psta(wlifname) ||
#ifdef __CONFIG_GMAC3__
		find_in_list(fwd_wlandevs, wlifname) ||
#endif
		FALSE)
		ifname = wlifname;
	else
...
eapd_wksp_dispatch(eapd_wksp_t *nwksp)
{
	...
	/* add lte_u appSocket */
	lte_u = &nwksp->lte_u;
	if (lte_u->appSocket >= 0) {
		FD_SET(lte_u->appSocket, &nwksp->fdset);
		if (lte_u->appSocket > nwksp->fdmax)
			nwksp->fdmax = lte_u->appSocket;
	}
	/* add airiq appSocket */
	airiq = &nwksp->airiq;
	if (airiq->appSocket >= 0) {
		FD_SET(airiq->appSocket, &nwksp->fdset);
		if (airiq->appSocket > nwksp->fdmax)
			nwksp->fdmax = airiq->appSocket;
	}
#endif /* WL_AIR_IQ */
	...
	if (status > 0) {
...
#ifdef WL_AIR_IQ
		/* check lte_u appSocket */
		if (lte_u->appSocket >= 0 &&
		     FD_ISSET(lte_u->appSocket, &fdset)) {
			bytes = recv(lte_u->appSocket, pkt, len, 0);
			if (bytes > ETHER_HDR_LEN) {
				char *ifname = (char *) pkt;
				struct ether_header *eth;
#ifdef BCMDBG
				HEXDUMP_ASCII("EAPD:: data from LTE_U app", pkt, bytes);
#endif
				/* ether header */
				eth = (struct ether_header*)(ifname + IFNAMSIZ);
				bytes -= IFNAMSIZ;
				...
				cb = eapd_wksp_find_cb(lte_u, ifname, eth->ether_shost);
				/* send message data out. */
				if (cb)
					lte_u_app_recv_handler(nwksp, cb, (uint8 *)eth, &bytes);
			}
		}
		/* check airiq appSocket */
		if (airiq->appSocket >= 0 &&
		     FD_ISSET(airiq->appSocket, &fdset)) {
			bytes = recv(airiq->appSocket, pkt, len, 0);
			if (bytes > ETHER_HDR_LEN) {
				char *ifname = (char *) pkt;
				struct ether_header *eth;
#ifdef BCMDBG
				HEXDUMP_ASCII("EAPD:: data from airiq app", pkt, bytes);
#endif
				/* ether header */
				eth = (struct ether_header*)(ifname + IFNAMSIZ);
				bytes -= IFNAMSIZ;
				...
				cb = eapd_wksp_find_cb(airiq, ifname, eth->ether_shost);
				/* send message data out. */
				if (cb)
					airiq_app_recv_handler(nwksp, cb, (uint8 *)eth, &bytes);
			}
		}
#endif /* WL_AIR_IQ */
		...
		if (ssd->appSocket >= 0 &&
			...
			if (bytes > ETHER_HDR_LEN) {
				...
				cb = eapd_wksp_find_cb(ssd, ifname, eth->ether_shost);
				/* send message data out. */
				if (cb)
			...
			if (eventd->appSocket >= 0 &&
				...
				if (bytes > ETHER_HDR_LEN) {
					...
					cb = eapd_wksp_find_cb(eventd, ifname, eth->ether_shost);
					/* send message data out. */
					if (cb) {
		...
		if (aspm->appSocket >= 0 &&
			...
			if (bytes > ETHER_HDR_LEN) {
				...
				cb = eapd_wksp_find_cb(aspm, ifname, eth->ether_shost);
				/* send message data out. */
				if (cb) {
			...
			if (visdcoll->appSocket >= 0 &&
				...
				if (bytes > ETHER_HDR_LEN) {
					...
					eth = (struct ether_header*)(ifname + IFNAMSIZ);
					bytes -= IFNAMSIZ;
					cb = eapd_wksp_find_cb(visdcoll, ifname, eth->ether_shost);
					/* send message data out */
					if (cb) {
		...
		if (wbd->appSocket >= 0 &&
			...
			if (bytes > ETHER_HDR_LEN) {
				...
				cb = eapd_wksp_find_cb(wbd, ifname, eth->ether_shost);
				/* send message data out. */
				if (cb) {
...
#ifdef WL_AIR_IQ
	/* check  lte_u application */
	lte_u_app_handle_event(nwksp, pData, Len, from->ifname);
	/* check  airiq application */
	airiq_app_handle_event(nwksp, pData, Len, from->ifname);
#endif /* WL_AIR_IQ */
...
event_init(eapd_wksp_t *nwksp)
{
	...
	uchar buf_param[WLC_IOCTL_SMLEN], buf[WLC_IOCTL_SMLEN];
...
#ifdef WL_AIR_IQ
	memset(name, 0, sizeof(name));
	foreach(name, lte_u->ifnames, next) {
		add_to_list(name, all_ifnames, sizeof(all_ifnames));
	}
	memset(name, 0, sizeof(name));
	foreach(name, airiq->ifnames, next) {
		add_to_list(name, all_ifnames, sizeof(all_ifnames));
	}
#endif /* WL_AIR_IQ */
#ifdef BCM_MEVENT
...
#ifdef WL_AIR_IQ
		/* is lte_u have this name */
		if (find_in_list(lte_u->ifnames, name)) {
			for (i = 0; i < EAPD_WL_EVENTING_MASK_LEN; i++) {
				bitvec[i] |= lte_u->bitvec[i];
			}
		}
		/* is airiq have this name */
		if (find_in_list(airiq->ifnames, name)) {
			for (i = 0; i < EAPD_WL_EVENTING_MASK_LEN; i++) {
				bitvec[i] |= airiq->bitvec[i];
			}
		}
#endif /* WL_AIR_IQ */
