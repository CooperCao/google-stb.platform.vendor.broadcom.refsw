...
WL0_IF=wlan0                    # first wifi interface name
WL1_IF=wlan1                    # second wifi interface name
...
# don't load wl drivers from kernel init scripts (rcS)
sed -i -e '/wl.ko/{/^#/!s/^/#/g}' -e '/wlplat.ko/{/^#/!s/^/#/g}' /etc/init.d/rcS
...
if [[ "${WIFI_WL_FD[0]}" = "1" || "${WIFI_WL_FD[1]}" = "1" ]]; then
        insmod ${WIFI_DRIVER_DIR}/dhd_secdma.ko iface_name=wlan secdma_addr=0x60000000 secdma_size=0x10b0000
        sleep 2
...
if [[ "${WIFI_WL_FD[0]}" = "1" || "${WIFI_WL_FD[1]}" = "1" ]]; then
	...
	if [[ "${WIFI_WL_FD[0]}" = "0" || "${WIFI_WL_FD[1]}" = "0" ]]; then
                cd ${WIFI_DRIVER_DIR}; insmod wlnic.ko ${WL1_IF} instance_base=1; cd -
        fi
fi
sleep 2

# Load module for ebtables

#this may be built into kernel
if [ -f /lib/modules/${LINUXVER}/kernel/net/netfilter/x_tables.ko ]; then
	insmod /lib/modules/${LINUXVER}/kernel/net/netfilter/x_tables.ko
fi
