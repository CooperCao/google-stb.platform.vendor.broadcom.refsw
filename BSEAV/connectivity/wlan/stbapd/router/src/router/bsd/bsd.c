...
void bsd_steer_sta(bsd_info_t *info, bsd_sta_info_t *sta, bsd_bssinfo_t *to_bss)
{
	...
	BSD_APPEVENT("Update stats for STA "MACF" from %s to %s "
		...
		steer_bssinfo->ifnames, sta->mcs_phyrate, sta->at_ratio,
		sta->rssi, intf_info->steering_flags);

	bsd_update_sta_stats(info, &(sta->addr), bssinfo->ifnames, steer_bssinfo->ifnames,
		0, sta->mcs_phyrate, sta->at_ratio, sta->rssi, intf_info->steering_flags,
		sta->steerflag);
...
bsd_process_bss_trans_resp(void *data, int response)
{
	...
	if (response == BSD_BSS_RESPONSE_REJECT) {
		callback_data->sta->steerflag |= BSD_STA_BSS_REJECT;
		BSD_STEER("Skip STA:"MACF" reject BSSID\n",
			ETHER_TO_MACF(callback_data->sta->addr));
		goto end;
	}

	if (response == BSD_BSS_RESPONSE_UNKNOWN) {
		if (bssinfo->intf_info->band != callback_data->steer_bssinfo->intf_info->band) {
			if (!bsd_is_sta_dualband(callback_data->info, &callback_data->sta->addr)) {
				BSD_STEER("Skip STA "MACF" no response for single-band sta\n",
					ETHER_TO_MACF(callback_data->sta->addr));
				goto end;
			}
		} else if (!bsd_qualify_sta_rf(callback_data->info, callback_data->sta->bssinfo,
			callback_data->sta->to_bssinfo, &callback_data->sta->addr)) {
			BSD_STEER("Skip STA "MACF" no response for not-at-target sta\n",
				ETHER_TO_MACF(callback_data->sta->addr));
			goto end;
		}

		if (callback_data->info->steer_no_deauth) {
			callback_data->sta->steerflag |= BSD_STA_BSSTRANS_UNSUPPORTED;
			BSD_STEER("NOT steer STA "MACF" from %s by steer_no_deauth\n",
				ETHER_TO_MACF(callback_data->sta->addr), bssinfo->ifnames);
			goto end;
		}
	}

	/* adding to maclist */
	for (intfidx = 0; intfidx < callback_data->info->max_ifnum; intfidx++) {
		/* search bands other than steer-ed intf */
		if (intfidx != callback_data->steer_bssinfo->intf_info->idx) {
			tmp_intf_info = &(callback_data->info->intf_info[intfidx]);
			...
			for (bssidx = 0; bssidx < WL_MAXBSSCFG; bssidx++) {
				...
				if (!strcmp(tmp_bssinfo->ssid,
					...
					if (tmp_bssinfo == bssinfo) {
						bsd_addto_maclist(tmp_bssinfo,
							&(callback_data->sta->addr),
							callback_data->steer_bssinfo);
					} else {
						bsd_addto_maclist(tmp_bssinfo,
							&(callback_data->sta->addr), NULL);
					}

					/* block the sta at current intf.  */
					wl_wlif_block_mac(tmp_bssinfo->wlif_hdl,
						callback_data->sta->addr, -1);
				}
			}
		}
	}
	...
	if (callback_data->info->enable_flag & BSD_FLAG_WBD_ENABLED) {
		bsd_wbd_send_block_client_req(callback_data->info, callback_data->steer_bssinfo,
			callback_data->sta, &wbd_ret);
	}
#endif /* BCM_WBD */

	/* remember the rssi at this steering point */
	bsd_log_rssi(callback_data->info, callback_data->sta, TRUE);

	bsd_update_steering_record(callback_data->info, &(callback_data->sta->addr),
		bssinfo, callback_data->steer_bssinfo, intf_info->steering_flags);
