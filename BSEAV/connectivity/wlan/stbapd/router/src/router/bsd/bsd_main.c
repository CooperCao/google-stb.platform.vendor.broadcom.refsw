...
static void bsd_query_sta_stats_hdlr(int sig)
{
	bsd_query_sta_stats(bsd_info);
	return;
}

static void bsd_query_radio_stats_hdlr(int sig)
{
...
static void bsd_tty_hdlr(int sig)
{
	char tty[32] = "";
	FILE* ff;
	int fd;
...
static void bsd_sta_config_hdlr(int sig)
{
...
int main(int argc, char *argv[])
{
	...
	if (!BSD_WBD_DISABLED(map_mode))
		...
		printf("WBD(%s) is not enabled: %s=%d\n", BSD_WBD_NVRAM_MAP_MODE, wbd_val, map_mode);
#endif /* BCM_WBD */
...
/* If WBD is Enabled, and BSD is not, & if there is no ifnames for BSD, WBD sets it */
	...
	signal(SIGPWR, bsd_tty_hdlr);
	signal(SIGIO, bsd_sta_config_hdlr);

	signal(SIGVTALRM, bsd_query_sta_stats_hdlr);
	signal(SIGINT, bsd_query_radio_stats_hdlr);
