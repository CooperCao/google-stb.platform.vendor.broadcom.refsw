 ...
 *
 * $Id: wlconf.c 736262 2017-12-14 07:37:52Z $
 */
...
wlconf_get_wbd_slave_mode(char *ifname)
{
	...
	int wbd_mode = atoi(nvram_safe_get("wbd_mode"));

	/* If not Downstream AP return 0 */
	if (!IS_WBD_DEV_DAP(wbd_mode)) {
		WLCONF_DBG("WBD mode[%d] not Downstream AP\n", atoi(nvram_safe_get("wbd_mode")));
		return 0;
	}
...
#define NVRAM_WBD_MODE		"wbd_mode"
...
{
	int i, nvval, ret, wbd_mode = 0, wbd_fbt = 0, fbt = 0;
	...
	for (i = 0; i < bclist->count; i++) {
		...
		/* Get wbd_mode, wbd_fbt and FBT NVRAM */
		wbd_mode = atoi(nvram_safe_get(NVRAM_WBD_MODE));
		...
		WLCONF_DBG("i[%d] name[%s] prefix[%s] wbd_mode[%d] wbd_fbt[%d] fbt[%d]\n",
			i, bsscfg->ifname, bsscfg->prefix, wbd_mode, wbd_fbt, fbt);
...
wlconf(char *name)
{
	...
	int rxchain_pwrsave = 0, radio_pwrsave = 0, map = 0;
	...
	map = atoi(nvram_safe_get(strcat_r(prefix, "map", tmp)));

#ifdef MULTIAP
	/* For multiap agents wireless onboarding intial mode is sta to do wps */
	if (map == 4 && atoi(nvram_safe_get("multiap_mode")) == 2 &&
	...
	for (i = 0; i < bclist->count; i++) {
		...
		if (ap || sta || psta || psr || (apsta && !wet)) {
			...
			map_val = atoi(nvram_safe_get(tmp));
		}
	...
	if (atoi(nvram_safe_get("wbd_mode")) > 0) {
