 ...
 * $Id: nas_radius.c 724936 2017-10-04 14:53:54Z $
 */
...
radius_dispatch(nas_t *nas, radius_header_t *response)
{
	...
	int nfrags = 0;
	bool gtk_is_being_plumbed = FALSE;
#ifdef BCMDBG
		...
		if (mppe_recv) {
			...
			if (nas_set_key(nas, &sta->ea, &mppe_recv[1], length, index, 1, 0, 0) < 0) {
				dbg(nas, "unicast key rejected by driver, assuming too many"
					" associated STAs");
				/* If GTK was plumbed now, clear the GTK plumbed flag */
				if (gtk_is_being_plumbed) {
					dbg(nas, "Clear GTK plumbed flag");
					nas->flags &= ~NAS_FLAG_GTK_PLUMBED;
				}
...
radius_forward(nas_t *nas, nas_sta_t *sta, eap_header_t *eap)
{
	...
	unsigned char buf[DOT11_MAX_SSID_LEN+23], *ptr;
	long val;
	int left;
