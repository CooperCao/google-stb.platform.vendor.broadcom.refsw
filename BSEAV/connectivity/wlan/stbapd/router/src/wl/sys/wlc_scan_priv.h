...
#define IS_COEX_ENAB(scan)		COEX_ENAB(SCAN_WLC(scan)->pub)
#define IS_COEX_ACTIVE(cfg)		COEX_ACTIVE((cfg))
...
#define SCAN_BLOCK_DATAFIFO_SET(scan, bit)	(SCAN_WLC(scan)->block_datafifo |= (bit))
#define SCAN_BLOCK_DATAFIFO_CLR(scan, bit)	(SCAN_WLC(scan)->block_datafifo &= ~(bit))
