 ...
 *
 * $Id: blanket.c 766982 2018-08-23 12:11:37Z tp888369 $
 *
 *  @author
 *  @bug No known bugs.
 */
...
#define BKT_DEBUG_SOCKET	0x0010
#define BKT_DEBUG_JSON		0x0020
#define BKT_DEBUG_TRACE		0X0080
#define BKT_DEBUG_TBSS		0x0100
#define BKT_DEBUG_BOUNCE	0x0200
#define BKT_DEBUG_STEER		0x0400
#define BKT_DEBUG_DS		0x0800
...
#define BKT_PRINT(prefix, fmt, arg...) \
	printf(prefix"WBD-%s >> (%lu) %s: "fmt, g_bkt_process_name, (unsigned long)time(NULL), \
	__FUNCTION__, ##arg)
	...
	bool use_ctrl_chan)
{
	...
	do {
		...
		int txpwr = 0;
		bool update_cntrl_chan_for_80_160_bw = FALSE;
		uint8 chan_bitmap[21] = {0};
		rc = rc_map[in_idx++];
		nrc = rc_map[in_idx++];
		start = in_idx;
		endpos = start + nrc;
		...
		for (k = start; k < endpos && k < rc_max; k++) {
			if (!isset(bitmap, k)) {
				...
				if (CHSPEC_IS80(rc_map[k]) || CHSPEC_IS160(rc_map[k])) {
					/* Prepare bitmap for each subband in chan_bitmap, after
					 * complete iteration of valid chanspecs for 80/160 bw,
					 * update list of control channels and count respectively
					 * in radiocap for particular opclass.
					 */
					/* Use control channel for 80/160 depending on variable */
					if (use_ctrl_chan) {
						setbit(chan_bitmap, wf_chspec_ctlchan(rc_map[k]));
					} else {
						setbit(chan_bitmap, CHSPEC_CHANNEL(rc_map[k]));
					}
					update_cntrl_chan_for_80_160_bw = TRUE;
				} else {
		...
		if (update_cntrl_chan_for_80_160_bw) {
			/* fill channels for regclass 128, 129 and 130 based on bitmap*/
			for (k = 0; k < (sizeof(chan_bitmap) *8); k++) {
				if (isset(chan_bitmap, k)) {
					out_radio_caps->list[out_idx++] = k;
					ch_count++;
				}
			}
		}
		update_cntrl_chan_for_80_160_bw = FALSE;
...
blanket_get_config_val_uint(char* prefix, const char *nvram, uint16 def)
{
	...
	if (val && (val[0] != '\0')) {
		...
		BKT_WARNING("NVRAM is not defined: %s%s \n", (prefix ? prefix : ""), nvram);
	}
...
blanket_get_chanim_stats(char* ifname, uint32 count, char* data_buf, int buflen)
{
	...
	if (list->buflen == 0) {
		...
		BKT_ERROR("Sorry, your driver has wl_chanim_stats version %d "
			"but this program supports only version %d.\n",
...
/* is interface a backhaul bss? */
bool
blanket_is_iface_backhaulbss(char *ifname)
{
	uint8 map_attr = 0;
	wl_iovar_get(ifname, "map", &map_attr, sizeof(map_attr));
	return map_attr ? ((map_attr & MAP_EXT_ATTR_BACKHAUL_BSS) != 0): FALSE;
}
