 ...
 *
 * $Id: wbd_master_com_hdlr.c 766982 2018-08-23 12:11:37Z tp888369 $
 */
...
#if 0
/* Get the CLIENTLIST CLI command data dump */
static int
wbd_master_get_clientlist_dump(wbd_master_info_t *master_info, wbd_slave_item_t *parent_slave,
	...
	i5_dm_clients_type *i5_assoc_sta, wbd_cmd_weak_client_resp_t *cmdweakclientresp)
{
	...
	if (i5_topology->DevicesNumberOfEntries <= 1) {
		WBD_WARNING("BKTID[%d] Slave["MACDBG"] For STA["MACDBG"]. %s\n",
			master_info->bkt_id, MAC2STRDBG(i5_bss->BSSID),
			MAC2STRDBG(i5_assoc_sta->mac), wbderrorstr(WBDE_NO_SLAVE_TO_STEER));
	...
	/* Add STA in all peer Slaves' Monitor STA List */
...
/* Prepares the beacon request fields */
static int
wbd_master_prepare_beacon_request(wbd_master_info_t *master_info,
	i5_dm_clients_type *i5_assoc_sta, ieee1905_beacon_request *bcn_req)
{
	...
	bcn_req->ap_chan_report_count = master_info->ap_chan_report.count;

	/* Traverse br0 AP channel report */
	foreach_glist_item(rclass_item_p, master_info->ap_chan_report) {
	...
	i5_dm_clients_type *i5_assoc_sta)
{
	...
	if (wbd_bcn_rpt && (!eacmp(&(i5_device->DeviceId), &(wbd_bcn_rpt->neighbor_al_mac))) &&
		...
		ret = wbd_master_prepare_beacon_request(master_info, i5_assoc_sta,
			&bcn_req);
		if (ret == WBDE_OK) {
...
wbd_master_process_weak_cancel_cmd(wbd_master_info_t *master, i5_dm_clients_type *i5_assoc_sta)
{
	...
	if (wbd_ds_remove_sta_fm_peer_devices_monitorlist(
		...
		(struct ether_addr*)i5_assoc_sta->mac) != WBDE_OK) {
	...
	unsigned char *tlv_data, unsigned char tlv_data_len)
{
	...
	foreach_glist_item(fbt_item_p, fbt_config_req.entry_list) {
		...
		WBD_INFO("From DEVICE["MACDBG"] BRDG["MACDBG"] BSS["MACDBG"] "
			...
			"R0KH_ID[%s] LEN_R0KH_ID[%ld] "
			"R0KH_Key[%s] LEN_R0KH_Key[%ld] "
			"MDID[%d] FT_CAP[%d] FT_REASSOC[%d]\n", MAC2STRDBG(neighbor_al_mac),
			MAC2STRDBG(bss_vndr_data->br_addr), MAC2STRDBG(i5_bss->BSSID),
			bss_vndr_data->r0kh_id, (long int)strlen(bss_vndr_data->r0kh_id),
			bss_vndr_data->r0kh_key, (long int)strlen(bss_vndr_data->r0kh_key),
	...
	unsigned char *tlv_data, unsigned char tlv_data_len)
{
	...
	WBD_INFO("Data Received VNDR_ASSOC_STA_LINK_METRIC : "
		...
		"Idle_rate[%d] Tx_failures[%d] STA Cap[0x%02x]\n",
		MAC2STRDBG(cmd.src_bssid), MAC2STRDBG(cmd.sta_mac),
		cmd.vndr_metrics.idle_rate, cmd.vndr_metrics.tx_failures, cmd.vndr_metrics.sta_cap);
	...
	unsigned char *radio_mac)
{
	...
	uint8 chan_first, chan_last, channel = 0;
	ieee1905_chan_pref_rc_map_array chan_pref;
	ieee1905_chan_pref_rc_map *rc_map;
	i5_dm_interface_type *interface, *self_intf;
	...
	int rcaps_rc_count;
	int rcidx;
	uint8 *control_chan_array_5g = NULL;
	uint8 control_chan_array_5g_size = 0;
	...
	foreach_i5glist_item(interface, i5_dm_interface_type, device->interface_list) {
		...
		control_chan_array_5g = i5DmGet5gCntrlChanArray(&control_chan_array_5g_size);
		...
		if (blanket_get_config_val_int(NULL, WBD_NVRAM_ENAB_SINGLE_CHAN_BH, 0) == 0) {
			if (wbd_ds_is_interface_dedicated_backhaul(interface)) {
		...
		for (i = 0; (i < REGCLASS_MAX_COUNT) &&
			...
			for (j = 0; j < rc_chan_map[i].count; j++) {
				/* For 128, 129 and 130 Regulatory classes, control channels info
				 * not present in rc_chan_map.
				 * rc_chan_map object contains list of centre channel for 80 mhz.
				 * Store control_chan_array elements excluding the control channel
				 * at which Controller operating.
				 *
				 */
				...
				if ((rc_chan_map[i].regclass > REGCLASS_40MHZ_LAST)) {
					...
					rc_map->regclass = rc_chan_map[i].regclass;
					chan_pref.rc_count++;
					for (k = 0; k < control_chan_array_5g_size; k++) {
						if (channel != control_chan_array_5g[k]) {
							if ((control_chan_array_5g[k] >
								REGCLASS_160MHZ_MAX_CNTRL_CHAN) &&
								(rc_chan_map[i].regclass ==
								REGCLASS_160MHZ)) {
								/* Invalid channel for 160 Mhz */
								break;
							}
							rc_map->channel[rc_map->count] =
								control_chan_array_5g[k];
							WBD_DEBUG("%d\n",
								rc_map->channel[rc_map->count]);
							rc_map->count++;
						}
...
validate:
		...
		for (i = 0; i < REGCLASS_MAX_COUNT; i++) {
			...
			/* Got a regulatory class in Radio capabilities (rc_chan_map[i].regclass),
			 * now check if it has invalid channels
			 */
			if (!WBD_MCHAN_ENAB(info->flags) &&
				rc_chan_map[i].regclass > REGCLASS_40MHZ_LAST) {
				count = control_chan_array_5g_size;
				chan_list = control_chan_array_5g;
			} else {
	...
	unsigned char *bssid, unsigned char *sta_mac, ieee1905_sta_link_metric *metric)
{
	...
	if (metric) {
		...
		sta_stats.tx_rate = metric->uplink_rate;
		assoc_sta->stats.sta_tm = time(NULL);
	}
	sta_stats.idle_rate = assoc_sta->stats.idle_rate;
	sta_stats.tx_failures = assoc_sta->stats.tx_failures;
	...
	WBD_INFO("For STA["MACDBG"] idle_rate[%d] t_idle_rate[%d] rssi[%d] t_rss[%d] "
	...
	if (sta_is_weak) {
		...
		if (I5_IS_BSS_STA(i5_assoc_sta->flags)) {
			WBD_INFO("Backhaul sta ["MACF"], skip processing\n",
				ETHERP_TO_MACF(i5_assoc_sta->mac));
			goto end;
		}

		/* Its weak now */
		/* If the TBSS timer is not created for this STA. create it */
		if (!(assoc_sta->flags & WBD_FLAGS_ASSOC_ITEM_TBSS)) {
			args = (wbd_tbss_timer_args_t *)wbd_malloc(sizeof(*args), &ret);
			WBD_ASSERT_MSG("Malloc falied for tbss timer creation\n");

			memset(args, 0, sizeof(*args));
			args->master_info = master;
			memcpy(args->al_mac, al_mac, MAC_ADDR_LEN);
			memcpy(args->bssid, bssid, MAC_ADDR_LEN);
			memcpy(args->sta_mac, sta_mac, MAC_ADDR_LEN);
			wbd_master_create_identify_target_bss_timer(master, args);
			assoc_sta->flags |= WBD_FLAGS_ASSOC_ITEM_TBSS;
		}

		/* If not weak process the weak client data */
		if (assoc_sta->status != WBD_STA_STATUS_WEAK) {
			wbd_master_process_weak_client_cmd(master, i5_assoc_sta, &sta_stats,
				metric_policy);
		}
	} else {
...
wbd_master_add_to_monitor_list(i5_dm_bss_type *i5_bss, unsigned char *sta_mac)
{
	...
	/* No need to add it to non fronthaul BSS */
	...
	/* Add to monitor list only if its weak */
	if (assoc_sta && assoc_sta->status == WBD_STA_STATUS_WEAK) {
		...
		WBD_WARNING("BSS["MACDBG"] STA["MACDBG"] Is not weak. So Don't add\n",
			MAC2STRDBG(i5_bss->BSSID), MAC2STRDBG(i5_sta->mac));
	}
...
wbd_master_unassoc_sta_metric_resp(unsigned char *al_mac, ieee1905_unassoc_sta_link_metric *metric)
{
	...
	foreach_glist_item(unassoc_sta_p, metric->sta_list) {
		...
		WBD_DS_FIND_I5_FHBSS_IN_DEVICE_FOR_BAND(i5_device, band, i5_bss, &ret);
...
void
wbd_master_update_ap_chan_report(wbd_master_info_t *master_info, i5_dm_interface_type *i5_ifr)
{
	int ret = WBDE_OK;
	...
	rclass_list = wbd_ds_find_rclass_in_ap_chan_report(master_info, i5_ifr->opClass, &ret);
	if (rclass_list == NULL) {
		ret = wbd_ds_add_rclass_in_ap_chan_report(master_info, i5_ifr->opClass,
			&rclass_list);
		WBD_ASSERT();
	}
	...
	wbd_master_info_t *master_info)
{
	...
	wbd_ds_ap_chan_report_cleanup(master_info);
	...
	foreach_i5glist_item(i5_iter_device, i5_dm_device_type, i5_topology->device_list) {
		...
		foreach_i5glist_item(i5_iter_ifr, i5_dm_interface_type,
			...
			wbd_master_update_ap_chan_report(master_info, i5_iter_ifr);
		}
	}
end:
	WBD_EXIT();
