 ...
 *
 * $Id: wbd_wl_utility.c 766238 2018-07-27 08:15:08Z sp888952 $
 */
...
int
wbd_wl_is_sta_type_dwds(char* ifname, struct ether_addr* addr, int *outerr, uint8 *data, int len)
{
	sta_info_t sta_info;
	brcm_ie_t *vndr_ie;
	int isdwds = FALSE;
	*outerr = WBDE_OK;
	...
	/* Find if its DWDS slave using vendor IE if the IE data provided else use sta_info.
	 * This fixes the issue where DWDS STA was also found in assoclist of slave in case
	 * of security is enabled. If security is enabled the "WL_STA_DWDS" flag
	 * takes some 1 seconds to get updated in the sta_info result.
	 */
	if (data != NULL) {
		/* Find vendor IE with BRCM_OUI */
		vndr_ie = (brcm_ie_t*)wbd_wl_find_ie(DOT11_MNG_PROPR_ID, data, len,
			BRCM_OUI, NULL, 0);
		if ((vndr_ie != NULL) && (vndr_ie->flags1 & BRF1_DWDS)) {
			WBD_DEBUG("ifname[%s] Found STA["MACF"] as DWDS STA using vendor IE\n",
	...
	/* Get STA stats from "sta_info" */
	*outerr = blanket_get_sta_info(ifname, addr, &sta_info);
	if (*outerr != WBDE_OK) {
		goto end;
	}

	/* Check for DWDS flag */
	if (sta_info.flags & WL_STA_DWDS) {
		WBD_DEBUG("ifname[%s] Found STA["MACF"] as DWDS STA using sta_info\n",
			ifname, ETHERP_TO_MACF(addr));
		isdwds = TRUE;
	}
	...
	struct ether_addr *sta_mac, uint32 evt_type)
{
	...
	printf("%s: addr("MACF") association %s \n", ifname, ETHERP_TO_MACF(sta_mac),
		dc->assoc_approved ? "ACCEPT" : "REJECT");
#endif
