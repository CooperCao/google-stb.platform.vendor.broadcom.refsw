 ...
 *
 * $Id: wbd_slave_control.c 618311 2016-02-10 15:06:06Z chintanb $
 */
...
#if 0
			ret = wbd_slave_send_weak_client_cmd(slave_item, sta_item);
#endif /* #if 0 */
...
#if 0
				ret = wbd_slave_send_weak_cancel_cmd(slave_item, sta_item);
#endif /* #if 0 */
			}
		}
	}
	...
	uint8 is_retry)
{
	...
	if (!i5DmIsMacWildCard(bss_info->bssid)) {
		...
		/* Block the STAs in all the BSS except current and target */
	...
	if (IEEE1905_IS_STEER_MANDATE(steer_req->request_flags)) {
		...
		ioctl_data.disassoc_timer = steer_req->dissassociation_timer;
	} else {
		ioctl_data.flags |= WLIFU_BSS_TRANS_FLAGS_BTM_ABRIDGED;
	...
	ieee1905_chan_pref_rc_map_array *cp)
{
	...
	for (i = 0; (i < rcaps_rc_count) && (bytes_rd < bytes_to_rd); i++) {
		...
		uint16 chan_cnt;
		uint16 rclass_cnt;
		uint8 pref = IEEE1905_CHAN_PREF_15;
		uint8 reason = I5_REASON_UNSPECFIED;
		...
		for (chan_cnt = 0; chan_cnt < rc_chan_map[rclass_cnt].count; chan_cnt++) {
			...
			if (bitmap & WL_CHAN_INACTIVE) {
				pref = IEEE1905_CHAN_PREF_NON_OP;
				reason = I5_REASON_RADAR;
			} else if (bitmap & (WL_CHAN_RADAR | WL_CHAN_PASSIVE)) {
				pref = IEEE1905_CHAN_PREF_1;
				reason = I5_REASON_DFS_PASSIVE;
			}
			/* update channnel and count in case prefererence for any channel
			 * changed due to radar
			 */
			if ((reason == I5_REASON_RADAR) ||
				(reason == I5_REASON_DFS_PASSIVE)) {
				...
				if (reason == I5_REASON_RADAR) {
					...
					/* Dont override RADAR channel reason */
					if (rc_map->reason == I5_REASON_RADAR) {
						continue;
					}
					rc_map->pref = pref;
					rc_map->reason = I5_REASON_DFS_PASSIVE;
				}
			}
		}
	...
	unsigned char rclass_local_count, ieee1905_chan_pref_rc_map *local_chan_pref)
{
	 ...
	 * Exception: even in singl channel mode, dedicated backhaul can
	 * operate in Multi-channel if it is not explicitly disabled setting nvram
	 */
	if (MAP_IS_AGENT(info->map_mode) && !MAP_IS_CONTROLLER(info->map_mode) &&
		(!WBD_MCHAN_ENAB(info->flags))) {

		int is_dedicated_bh = wbd_ds_is_interface_dedicated_backhaul(pdmif);
		if (!is_dedicated_bh || (is_dedicated_bh &&
			blanket_get_config_val_int(NULL, WBD_NVRAM_ENAB_SINGLE_CHAN_BH, 0))) {
			/* Set mode of ACS running on Repeater to Fixed Chanspec */
			wbd_slave_set_acsd_mode_to_fixchspec(slave);
		}
	}
...
setchannel:
	...
	for (i = chlist->count - 1; i >= 0; i--) {
		...
		if (is_valid) {
			...
			ret = wbd_slave_set_chanspec_through_acsd(slave,
				chlist->element[i], WBD_REASON_OTHER);
			if (ret == WBDE_OK) {
				int delay = 0;
				chanspec_t chanspec;
				...
				while (delay <= WBD_MAX_ACSD_SET_CHSPEC_DELAY) {
					chanspec = 0;
					blanket_get_chanspec(pdmif->ifname, &chanspec);
					if (chanspec != chlist->element[i]) {
						usleep(WBD_GET_CHSPEC_GAP * 1000);
						delay += WBD_GET_CHSPEC_GAP;
						continue;
					}
					/* Add static neighbor entry of this interface bss's
					 * with updated channel
					*/
					wbd_slave_add_ifr_nbr(pdmif, TRUE);
					break;
				}
				WBD_INFO("Current chanspec: [0x%x] acsd set delay: %dms\n",
					chanspec, delay);
				slave->wbd_ifr.chanspec = pdmif->chanspec = chanspec;
				pdmif->opClass = rclass;
	...
	unsigned char *interface_mac, ieee1905_backhaul_link_metric *metric)
{
	...
	if (is_dwds) {
		...
		/* XXX: Recieved Packet errors to be done */
...
wbd_slave_1905AuthTypeToAKM(char *prefix, unsigned short AuthType, char *akm, uint8 akm_len)
{
	...
	if (AuthType == IEEE1905_AUTH_WPA) {
		snprintf(akm, akm_len, "wpa");
	} else if (AuthType == IEEE1905_AUTH_WPA2) {
		snprintf(akm, akm_len, "wpa2");
	} else {
		/* Handle single and mixed mode akm values */
...
static int wbd_slave_same_bss_setting(char *ifname, char *prefix, ieee1905_client_bssinfo_type *bss)
{
	...
	if (!find_in_list(blanket_nvram_safe_get(NVRAM_LAN_IFNAMES), ifname)) {
		goto no_match;
	}
...
no_match:
	WBD_INFO("prefix[%s] ssid[%s] key[%s] AuthType[%x] akm[%s] EncryptType[%x] crypto[%s] "
		"map[%s] ret[%d]\n",
		prefix, bss->ssid.SSID, bss->NetworkKey.key, bss->AuthType, akm,
		bss->EncryptType, crypto, strmap, ret);
	...
	ieee1905_client_bssinfo_type *bss, unsigned char restart)
{
	...
	if (wbd_slave_same_bss_setting(ifname, prefix, bss)) {
		if (wbd_slave_add_ifname_to_list(ifname, WBD_NVRAM_IFNAMES) == 0 ||
			wbd_slave_make_bh_bss_hidden(prefix, bss) == 0) {
			ret = 0;
		} else {
			ret = -1;
		}
	...
	for (idx = 1; idx < WBD_MAX_POSSIBLE_VIRTUAL_BSS; idx++) {
		...
		if (wbd_slave_same_bss_setting(tmpvif, vif_prefix, bss)) {
			if (wbd_slave_add_ifname_to_list(tmpvif, WBD_NVRAM_IFNAMES) == 0 ||
				wbd_slave_make_bh_bss_hidden(vif_prefix, bss) == 0) {
				ret = 0;
			} else {
				ret = -1;
			}
	...
	if (erase) {
		...
		if (nvram_match(strcat_r(prefix, NVRAM_MODE, tmp), "sta") != 1) {
			if (atoi(blanket_nvram_safe_get("map_onboarded")) == 0) {
				blanket_nvram_prefix_unset(prefix, NVRAM_MAP);
				blanket_nvram_prefix_set(prefix, NVRAM_MODE, "ap");
			}

			wbd_slave_apply_default_ap_nvrams(prefix, bss);
	...
	for (idx = 1; idx < WBD_MAX_POSSIBLE_VIRTUAL_BSS; idx++) {
		...
		if (!find_in_list(lan_ifnames, tmpvif)) {
			...
			wbd_slave_apply_default_ap_nvrams(vif_prefix, bss);
