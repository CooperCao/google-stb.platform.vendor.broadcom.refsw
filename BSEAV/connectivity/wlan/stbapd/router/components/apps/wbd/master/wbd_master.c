 ...
 *
 * $Id: wbd_master.c 766984 2018-08-23 12:15:49Z tp888369 $
 */
...
wbd_master_retrieve_blanket_nvram_config(wbd_blanket_master_t *wbd_master)
{
	...
	WBD_DEBUG("%s window[%d] cnt[%d] dwell_time[%d]\n",
...
wbd_ieee1905_process_vendor_specific_msg_cb(ieee1905_vendor_data *msg_data)
{
	...
	wbd_master_process_vendor_specific_msg(msg_data);
}
...
main(int argc, char *argv[])
{
	...
	if (!MAP_IS_CONTROLLER(g_wbdinfo->map_mode)) {
		WBD_DEBUG("MultiAP Mode (%d) is not controller. Aborting Master...\n",
			g_wbdinfo->map_mode);
		goto end;
	}
	...
	ret = blanket_init(g_wbdinfo, MAP_MODE_FLAG_CONTROLLER,
