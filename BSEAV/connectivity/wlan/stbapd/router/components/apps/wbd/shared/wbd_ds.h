 ...
 *
 * $Id: wbd_ds.h 767284 2018-09-04 06:24:05Z sp888952 $
 */
...
#define WBD_TBSS_MIN_STA_THLD		0
...
#define WBD_FLAGS_ASSOC_ITEM_TBSS	0x0001	/* TBSS timer created */

/* Print the slave type. TODO: Add MOCA */
#define WBD_SLAVE_TYPE_NAME(t) ((t == WBD_SLAVE_TYPE_MASTER) ? "Master" : \
		((t == WBD_SLAVE_TYPE_ETHERNET) ? "Eth" : ((t == WBD_SLAVE_TYPE_DWDS) ? "DWDS" : \
		((t == WBD_SLAVE_TYPE_PLC) ? "PLC" : "Invalid"))))
...
/* Information of each DWDS sta in blanket */
typedef struct wbd_dwds_sta_item {
	dll_t node;		/* self referencial (next,prev) pointers of type dll_t */
	struct ether_addr mac;	/* MAC address of the DWDS slave */
	int rssi;		/* RSSI of the DWDS STA */
	float tx_rate;		/* unit Mbps */
} wbd_dwds_sta_item_t;
...
/* Information for Slave's STA_ASSOC/STA_DISASSOC Event & timer callback */
typedef struct wbd_event_item {
	dll_t node; /* self referencial (next,prev) pointers of type dll_t */
	wbd_slave_item_t *parent_slave;	/* ptr to Parent Slave */
	struct ether_addr sta_mac; /* STA mac address */
	struct {
		uint32 type; /* event rcvd from WL */
		int status; /* status rcvd from WL */
		uchar bitvec[1]; /* set bit for events */
	} evt;
} wbd_event_item_t;
...
struct wbd_slave_item {
	...
	void* config_info;		/* Slave Running in Master, keeps NVRAMs as JSON String */
	void* chanspec_list;		/* List of chanspecs supported by driver, will be used for
					 * Slave running in Master mode
					 */
	wbd_slave_type_t slave_type;	/* Slave type, master, ethernet or dwds */
	struct ether_addr uplink_mac;	/* If Slave Type is DWDS, MAC of uplink DWDS STA Ifr */
...
#endif /* PLC_WBD */
	wbd_glist_t event_list;		/* List of wbd_event_item_t type objects */
...
struct wbd_blanket_slave {
	...
	wbd_glist_t dwds_sta_list;	/* List of wbd_dwds_sta_item_t type objects */
	char dwds_ifname[IFNAMSIZ];	/* DWDS Interface name */
...
/* Traverse I5's Device to find a Fronthaul BSS in any Interface based on band,
 * without Error check
 */
#define WBD_DS_FIND_I5_FHBSS_IN_DEVICE_FOR_BAND(i5_device, band, i5_bss, find_ret) \
		do { \
			(i5_bss) = wbd_ds_get_i5_fhbss_in_device_for_band((i5_device), \
					(band), (find_ret)); \
		} while (0)
...
/* Remove all DWDS slave items on this Blanket */
extern int wbd_ds_slave_item_cleanup_dwdslist(wbd_blanket_slave_t *blanket_slave);
#ifdef PLC_WBD
/* Remove all PLC STA items on this Slave */
extern int wbd_ds_slave_item_cleanup_plc_sta_list(wbd_slave_item_t *slave_item);
#endif /* PLC_WBD */
/* Remove all events managed by the Slave */
extern int wbd_ds_slave_item_cleanup_eventlist(wbd_slave_item_t *slave_item);
...
wbd_slave_item_t* wbd_ds_find_dwds_slave_in_master(wbd_master_info_t *master_info,
	struct ether_addr *find_slave_addr, int* error);
...
/* Traverse slave blanket's DWDS STA List to find an DWDS STA */
extern wbd_dwds_sta_item_t* wbd_ds_find_sta_in_slave_dwdslist(wbd_blanket_slave_t* blanket_slave,
	struct ether_addr *find_mac, int* error);
/* Traverse Slave Item's Event List to find an Event Entry */
extern wbd_event_item_t* wbd_ds_find_event_in_slave_eventlist(wbd_slave_item_t* slave_item,
	struct ether_addr *find_sta_mac, int* error);
...
/* Add a DWDS STA item in blankets dwdslist */
extern int wbd_ds_add_sta_in_slave_dwdslist(wbd_blanket_slave_t* blanket_slave,
	struct ether_addr *mac, wbd_dwds_sta_item_t **out_dwds_sta);
/* Add an Event item in a Slave's eventlist */
extern int wbd_ds_add_event_in_slave_eventlist(wbd_slave_item_t* slave_item,
	struct ether_addr* mac, uint32 type, uchar bit, int status, wbd_event_item_t** out_event);
...
/* Remove a DWDS slave item from dwdslist */
extern int wbd_ds_remove_sta_fm_slave_dwdslist(wbd_blanket_slave_t* blanket_slave,
	struct ether_addr *mac);
/* Remove a Event item from a Slave's event List */
extern int wbd_ds_remove_event_fm_slave_eventlist(wbd_slave_item_t* slave_item,
	struct ether_addr *mac);
...
extern void wbd_ds_update_dwds_sta_item(wbd_dwds_sta_item_t *dst, wbd_dwds_sta_item_t *src);
...
i5_dm_bss_type* wbd_ds_get_i5_fhbss_in_device_for_band(i5_dm_device_type *i5_device, int band,
	int *error);
...
extern wbd_bcn_req_rclass_list_t* wbd_ds_find_rclass_in_ap_chan_report(
	wbd_master_info_t *master_info, unsigned char rclass, int* error);
...
extern int wbd_ds_add_rclass_in_ap_chan_report(wbd_master_info_t *master_info,
...
extern int wbd_ds_ap_chan_report_cleanup(wbd_master_info_t *master_info);
