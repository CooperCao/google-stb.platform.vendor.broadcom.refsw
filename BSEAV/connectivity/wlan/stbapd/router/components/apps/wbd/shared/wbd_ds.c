 ...
 *
 * $Id: wbd_ds.c 765031 2018-06-14 09:05:49Z sp888952 $
 */
...
wbd_ds_blanket_slave_init(wbd_info_t *info)
{
	...
	/* Initialize DWDS list for this AP */
	wbd_ds_glist_init(&info->wbd_slave->dwds_sta_list);
...
wbd_ds_blanket_slave_cleanup(wbd_info_t *info)
{
	...
	/* Remove all DWDS STA items */
	wbd_ds_glist_cleanup(&(info->wbd_slave->dwds_sta_list));
...
wbd_ds_slave_item_init(int band, int *error)
{
	...
	/* Initialize CONFIG NVRAMs JSON String, CONFIG cmd will INIT the same */
	slave_item->config_info = NULL;
...
#endif /* PLC_WBD */
	wbd_ds_glist_init(&slave_item->event_list);
...
wbd_ds_slave_item_cleanup(wbd_slave_item_t *slave_item, bool free_slave_mem)
{
	...
	/* Remove all pending event items on this Slave */
	ret = wbd_ds_slave_item_cleanup_eventlist(slave_item);

	if (slave_item->chanspec_list) {
		free(slave_item->chanspec_list);
		slave_item->chanspec_list = NULL;
	}

	if (slave_item->config_info) {
		free(slave_item->config_info);
		slave_item->config_info = NULL;
	}
...
/* Remove all DWDS STA items on this Slave */
int
wbd_ds_slave_item_cleanup_dwdslist(wbd_blanket_slave_t *blanket_slave)
{
	int ret = WBDE_OK;
	WBD_ENTER();

	/* Validate arg */
	WBD_ASSERT_ARG(blanket_slave, WBDE_INV_ARG);

	ret = wbd_ds_glist_cleanup(&(blanket_slave->dwds_sta_list));
	WBD_DS("DWDSlist Cleanup %s\n", wbderrorstr(ret));
end:
	WBD_EXIT();
	return ret;
}
...
/* Remove all pending event items on this Slave */
int
wbd_ds_slave_item_cleanup_eventlist(wbd_slave_item_t *slave_item)
{
	int ret = WBDE_OK;
	WBD_ENTER();
	...
	ret = wbd_ds_glist_cleanup(&(slave_item->event_list));
	WBD_DS("Band[%d] Slave["MACF"] Eventlist Cleanup %s\n", slave_item->band,
		ETHER_TO_MACF(slave_item->wbd_ifr.mac), wbderrorstr(ret));
end:
	WBD_EXIT();
	return ret;
...
/* Traverse Master Info's Slave List to find a DWDS Slave */
wbd_slave_item_t*
wbd_ds_find_dwds_slave_in_master(wbd_master_info_t *master_info,
	struct ether_addr *find_slave_mac, int* error)
{
	wbd_slave_item_t *slave_item = NULL, *ret_slave_item = NULL;
	dll_t *slave_item_p;
	*error = WBDE_OK;
	...
	foreach_glist_item(slave_item_p, master_info->slave_list) {
		...
		/* Slave Checking Condition */
		if ((slave_item->slave_type == WBD_SLAVE_TYPE_DWDS) &&
			(eacmp(&slave_item->uplink_mac, find_slave_mac) == 0)) {
			ret_slave_item = slave_item;
			goto end;
		} else {
			continue;
	...
	WBD_DS("BKTID[%d] DWDS Slave["MACF"]. %s\n", master_info->bkt_id,
		ETHERP_TO_MACF(find_slave_mac), wbderrorstr(WBDE_DS_UNKWN_SLV));
	*error = WBDE_DS_UNKWN_SLV;
end:
	WBD_EXIT();
	return ret_slave_item;
...
/* Traverse Slave Item's DWDS STA List to find an DWDS STA */
wbd_dwds_sta_item_t*
wbd_ds_find_sta_in_slave_dwdslist(wbd_blanket_slave_t* blanket_slave,
	struct ether_addr *find_mac, int* error)
{
	int ret = WBDE_OK;
	wbd_dwds_sta_item_t *dwds_sta = NULL, *ret_dwds_sta = NULL;
	dll_t *sta_item_p;
	*error = WBDE_OK;
	WBD_ENTER();

	WBD_ASSERT_ARG(find_mac, WBDE_INV_ARG);

	/* Travese DWDS STA items in this slave */
	foreach_glist_item(sta_item_p, blanket_slave->dwds_sta_list) {

		dwds_sta = (wbd_dwds_sta_item_t*)sta_item_p;

		/* DWDS STA Checking Condition */
		if (eacmp(&dwds_sta->mac, find_mac) == 0) {
			ret_dwds_sta = dwds_sta;
			goto end;
		} else {
			continue;
		}
	...
	WBD_DS("DWDS STA["MACF"]. %s\n", ETHERP_TO_MACF(find_mac),
		wbderrorstr(WBDE_DS_UN_DWDSSTA));
	*error = WBDE_DS_UN_DWDSSTA;
end:
	WBD_EXIT();
	return ret_dwds_sta;
}
...
wbd_event_item_t*
wbd_ds_find_event_in_slave_eventlist(wbd_slave_item_t* slave_item,
	struct ether_addr *find_sta_mac, int* error)
{
	int ret = WBDE_OK;
	wbd_event_item_t *event_item = NULL, *ret_event_item = NULL;
	dll_t *event_item_p;
	WBD_ENTER();
	...
	foreach_glist_item(event_item_p, slave_item->event_list) {

		event_item = (wbd_event_item_t*)event_item_p;

		/* STA Checking Condition */
		if (eacmp(&event_item->sta_mac, find_sta_mac) == 0) {
			ret_event_item = event_item;
			goto end;
		} else {
			continue;
		}
	...
	WBD_DS("Band[%d] Slave["MACF"] EVENT STA["MACF"]. %s\n", slave_item->band,
		ETHER_TO_MACF(slave_item->wbd_ifr.mac), ETHERP_TO_MACF(find_sta_mac),
		wbderrorstr(WBDE_DS_UN_EVENT));
	*error = WBDE_DS_UN_EVENT;
end:
	WBD_EXIT();
	return ret_event_item;
}
	...
	wbd_trigger_t *out_trigger)
{
	...
	if (slave_item->slave_type == WBD_SLAVE_TYPE_MASTER) {
		/* Free NVRAM values json string, fm Slave running in Master */
		if (master_info->pm_slave->config_info) {
			free(master_info->pm_slave->config_info);
		}
		/* Free list of chanspecs values from Slave running in Master */
		if (master_info->pm_slave->chanspec_list) {
			free(master_info->pm_slave->chanspec_list);
		}
...
/* Add a DWDS STA item in a Slave's dwdslist */
int
wbd_ds_add_sta_in_slave_dwdslist(wbd_blanket_slave_t* blanket_slave, struct ether_addr *mac,
	wbd_dwds_sta_item_t **out_dwds_sta)
{
	int ret = WBDE_OK;
	wbd_dwds_sta_item_t *new_dwds_sta = NULL;
	WBD_ENTER();

	/* Check if the DWDS STA is already there in dwdslist or not */
	if (wbd_ds_find_sta_in_slave_dwdslist(blanket_slave, mac, &ret)) {
		WBD_DS("STA["MACF"] %s in dwdslist \n",	ETHERP_TO_MACF(mac),
			wbderrorstr(WBDE_DS_STA_EXST));
		ret = WBDE_DS_STA_EXST;
		goto end;
	}
	ret = WBDE_OK;

	/* Create a DWDS STA item */
	new_dwds_sta = (wbd_dwds_sta_item_t*)wbd_malloc(sizeof(*new_dwds_sta), &ret);
	WBD_ASSERT();

	/* Fill & Initialize DWDS STA item data */
	memcpy(&new_dwds_sta->mac, mac, sizeof(*mac));

	/* In the end, Add this new DWDS STA item to dwdslist of this Slave */
	wbd_ds_glist_append(&blanket_slave->dwds_sta_list, (dll_t *) new_dwds_sta);

	if (out_dwds_sta) {
		*out_dwds_sta = new_dwds_sta;
	}
	WBD_DS(" DWDS STA Added["MACF"] In DWDSlist\n",	ETHERP_TO_MACF(mac));
end:
	WBD_EXIT();
	return ret;
}

/* Add an Event item in a Slave's eventlist */
int
wbd_ds_add_event_in_slave_eventlist(wbd_slave_item_t *slave_item, struct ether_addr* mac,
	uint32 type, uchar bit, int status, wbd_event_item_t** out_event)
{
	int ret = WBDE_OK;
	wbd_event_item_t* new_event = NULL;
	WBD_ENTER();

	/* Create a Event item */
	new_event = (wbd_event_item_t*)wbd_malloc(sizeof(*new_event), &ret);
	WBD_ASSERT();

	/* Fill & Initialize Event item data */
	new_event->parent_slave = slave_item;
	memcpy(&(new_event->sta_mac), mac, sizeof(new_event->sta_mac));
	new_event->evt.type = type;
	new_event->evt.status = status;
	setbit(new_event->evt.bitvec, bit);

	/* In the end, Add this new Event item to eventlist of this Slave */
	wbd_ds_glist_append(&slave_item->event_list, (dll_t *) new_event);

	if (out_event) {
		*out_event = new_event;
	}
	WBD_DS("Band[%d] STA Event Added["MACF"] In Slave["MACF"]'s Eventlist. Type[0x%x] "
		"Status[%d] Bit[0x%x]\n", slave_item->band,
		ETHERP_TO_MACF(mac), ETHER_TO_MACF(slave_item->wbd_ifr.mac),
		new_event->evt.type, new_event->evt.status, bit);
end:
	WBD_EXIT();
	return ret;
}

/* Add a STA item in all peer Devices' Monitor STA List */
int
...
#if 0 /* TO DO */
	/* Update the Slaves' last active time */
	WBD_DS_UPDATE_LAST_ACTIVE_TIME(parent_slave);
#endif /* 0 */
...
wbd_ds_add_monitorlist_fm_peer_devices_assoclist(i5_dm_bss_type *i5_bss)
{
	...
	/* Only add if its a fronthaul BSS */
	...
	foreach_i5glist_item(i5_device, i5_dm_device_type, i5_topology->device_list) {
		...
		foreach_i5glist_item(i5_ifr, i5_dm_interface_type, i5_device->interface_list) {
			foreach_i5glist_item(i5_peer_bss, i5_dm_bss_type, i5_ifr->bss_list) {
				...
				/* Add it from fronthaul BSS only */
				if (!I5_IS_BSS_FRONTHAUL(i5_peer_bss->mapFlags)) {
					continue;
				}
...
/* Remove a DWDS STA item from a Slave's dwdslist */
int
wbd_ds_remove_sta_fm_slave_dwdslist(wbd_blanket_slave_t* blanket_slave,
	struct ether_addr *mac)
{
	int ret = WBDE_OK;
	wbd_dwds_sta_item_t* dwds_sta = NULL;
	WBD_ENTER();

	/* Travese DWDS STA List to find DWDS STA */
	if ((dwds_sta = wbd_ds_find_sta_in_slave_dwdslist(blanket_slave, mac, &ret)) != NULL) {

		/* In the end, Remove DWDS STA item */
		wbd_ds_glist_delete(&blanket_slave->dwds_sta_list, &dwds_sta->node);
		free(dwds_sta);
		WBD_DS(" DWDS STA Removed["MACF"] from DWDSlist\n", ETHERP_TO_MACF(mac));
	}
...
/* Remove a Event item from a Slave's event List */
int
wbd_ds_remove_event_fm_slave_eventlist(wbd_slave_item_t* slave_item,
	struct ether_addr *mac)
{
	int ret = WBDE_OK;
	...
	/* Travese event List of this Slave to find event */
	if ((event = wbd_ds_find_event_in_slave_eventlist(slave_item, mac, &ret)) != NULL) {

		/* In the end, Remove event from Slave */
		wbd_ds_glist_delete(&slave_item->event_list, &event->node);
		free(event);
		WBD_DS("Band[%d] Event STA Removed["MACF"] from Slave["MACF"]'s eventlist\n",
			slave_item->band, ETHERP_TO_MACF(mac),
			ETHER_TO_MACF(slave_item->wbd_ifr.mac));
	}
...
/* Update DWDS STA info in slaves dwdssta Item */
void
wbd_ds_update_dwds_sta_item(wbd_dwds_sta_item_t *dst, wbd_dwds_sta_item_t *src)
{
	WBD_ENTER();
	dst->rssi = src->rssi;
...
{
	return wbd_ds_get_i5_device(ieee1905_get_al_mac(), error);
}
...
{
	return wbd_ds_get_i5_interface(ieee1905_get_al_mac(), mac, error);
}
...
{
	return wbd_ds_get_i5_bss(ieee1905_get_al_mac(), bssid, error);
}
...
i5_dm_bss_type*
wbd_ds_get_i5_fhbss_in_device_for_band(i5_dm_device_type *i5_device, int in_band, int *error)
{
...
wbd_ds_find_rclass_in_ap_chan_report(wbd_master_info_t *master_info,
	unsigned char rclass, int* error)
{
	...
	*error = WBDE_OK;
	...
	foreach_glist_item(rclass_list_p, master_info->ap_chan_report) {
...
int
wbd_ds_add_rclass_in_ap_chan_report(wbd_master_info_t *master_info, unsigned char rclass,
	wbd_bcn_req_rclass_list_t** out_rclass_list)
{
	...
	wbd_ds_glist_append(&master_info->ap_chan_report, (dll_t *)new_rclass_list);

	if (out_rclass_list) {
		*out_rclass_list = new_rclass_list;
	}
	WBD_DS("BKTID[%d] rclass Added[%d] in ap channel report\n", master_info->bkt_id, rclass);
end:
	WBD_EXIT();
	return ret;
}
...
int
wbd_ds_ap_chan_report_cleanup(wbd_master_info_t *master_info)
{
	int ret = WBDE_OK;
	...
	foreach_safe_glist_item(rclass_list_item_p, master_info->ap_chan_report,
		rclass_list_next_p) {
		...
		wbd_ds_glist_delete(&master_info->ap_chan_report, rclass_list_item_p);
		free(rclass_list);
	}

end:
	WBD_DS("BKTID[%d] Blanket rclass_list Cleanup %s\n", master_info->bkt_id, wbderrorstr(ret));
	WBD_EXIT();
	return ret;
}
