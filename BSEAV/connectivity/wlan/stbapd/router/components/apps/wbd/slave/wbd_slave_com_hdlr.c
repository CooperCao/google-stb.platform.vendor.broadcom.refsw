 ...
 *
 * $Id: wbd_slave_com_hdlr.c 767101 2018-08-28 09:17:12Z pn952104 $
 */
...
#define ASSOC_BIT				0x01
#define DISASSOC_BIT				0x02
...
/* Sanitize occurred event, by checking if same/opposite event has occurred during grace period */
static int
wbd_slave_sanitize_event(wbd_slave_item_t *slave_item, struct ether_addr* sta_mac,
	uchar event_bit_to_validate);
/* Callback to send STA_ASSOC/STA_DISASSOC event to Master after grace period is over */
static void
wbd_slave_event_grace_timer_cb(bcm_usched_handle *hdl, void *arg);
...
wbd_slave_process_weak_client_cmd_resp(unsigned char *neighbor_al_mac,
	unsigned char *tlv_data, unsigned char tlv_data_len);
...
int
wbd_slave_fill_dwds_info(wbd_info_t *info)
{
	int ret = WBDE_OK;
	int iter_sta;
	char backhaul_if[IFNAMSIZ] = {0};
	uint8 *assoclist = NULL;
	int assoclistlen = 0;
	struct maclist* list = NULL;
	WBD_ENTER();

	/* Get the interface name of backhaul */
	wbd_find_backhaul_primif_on_device(backhaul_if, sizeof(backhaul_if), NULL);
	WBDSTRNCPY(info->wbd_slave->dwds_ifname, backhaul_if, sizeof(backhaul_if));

	/* Get Max Assoclist len */
	ret = blanket_get_max_assoc(backhaul_if, &assoclistlen);
	WBD_ASSERT();
	...
	ret = blanket_get_assoclist(backhaul_if, list, assoclistlen);
	WBD_ASSERT();

	for (iter_sta = 0; iter_sta < list->count; iter_sta++) {
		/* check if the sta is DWDS client or not */
		if (wbd_wl_is_sta_type_dwds(backhaul_if, &list->ea[iter_sta],
			&ret, NULL, 0)) {
			WBD_INFO("DWDS STA["MACF"] FOUND\n",
				ETHER_TO_MACF(list->ea[iter_sta]));

			/* Add sta to dwds list */
			ret = wbd_ds_add_sta_in_slave_dwdslist(info->wbd_slave,
				&list->ea[iter_sta], NULL);
			WBD_CHECK_MSG("DWDS STA["MACF"] ERROR : %s\n",
				ETHER_TO_MACF(list->ea[iter_sta]),
				wbderrorstr(ret));
...
wbd_slave_get_chan_info(char* ifname, wbd_interface_chan_info_t* wbd_chan_info, int index_size)
{
	...
	for (channel = 0; channel <= MAXCHANNEL; channel++) {
		...
		if (iter < index_size) {
			wbd_chan_info->chinfo[iter].channel = channel;
...
wbd_slave_steer_retry_timer_cb(bcm_usched_handle *hdl, void *arg)
{
	...
	/* Check if is there any Death Event from driver */
	event = wbd_ds_find_event_in_slave_eventlist(slave, (struct ether_addr*)&i5_assoc_sta->mac,
		&ret);
	if (event && isset(event->evt.bitvec, DISASSOC_BIT)) {
		/* No need to try BSS transition because already the STA is disassociated */
		goto end;
	}
	...
	uchar event_bit_to_validate)
{
	...
	/* Traverse Slave Item's Event List to find an Event Entry */
	event = wbd_ds_find_event_in_slave_eventlist(slave_item, sta_mac, &ret);

	/* if any matching event entry for that sta is found */
	if (event) {
		if (isset(event->evt.bitvec, event_bit_to_validate)) {
			/* if same event occured, no need to process again */
			ret = WBDE_SAME_EVT_OCRRD;
			goto end;
		} else {
			/* stop the timer */
			bcm_usched_remove_timer(slave_item->parent->parent->hdl,
				wbd_slave_event_grace_timer_cb, event);

			/* Remove event entry from eventlist */
			ret = wbd_ds_remove_event_fm_slave_eventlist(slave_item, sta_mac);
			WBD_ASSERT_MSG("Band[%d] Slave["MACF"] STA["MACF"]. Failed to remove "
				"event. Error : %s\n", slave_item->band,
				ETHER_TO_MACF(slave_item->wbd_ifr.mac), ETHERP_TO_MACF(sta_mac),
				wbderrorstr(ret));

			/* if opposite old event already present, no need to process both */
			ret = WBDE_IGNORE_OPPO_EVT;
			goto end;
		}
	} else {
		/* if no matching event entry for that sta is found, add new event */
		ret = WBDE_NEW_EVT_OCRRD;
		goto end;
	}

end:
	WBD_INFO("Band[%d] Slave["MACF"] STA["MACF"] event_bit_to_validate[0x%x]. %s\n",
		slave_item->band, ETHER_TO_MACF(slave_item->wbd_ifr.mac),
		ETHERP_TO_MACF(sta_mac), event_bit_to_validate, wbderrorstr(ret));
	WBD_EXIT();
	return ret;
}
...
wbd_slave_event_grace_timer_cb(bcm_usched_handle *hdl, void *arg)
{
	...
	wbd_event_item_t* param = (wbd_event_item_t*)arg;
	wbd_slave_item_t* slave = param->parent_slave;

	switch (param->evt.type) {
		case WLC_E_DEAUTH:
		case WLC_E_DEAUTH_IND:
		case WLC_E_DISASSOC_IND:
		{
			/* Check and Remove if it is a DWDS STA item from a Slave's dwdslist */
			ret = wbd_ds_remove_sta_fm_slave_dwdslist(slave->parent, &(param->sta_mac));
			if (ret == WBDE_OK) {
				WBD_INFO("Band[%d] Slave["MACF"] Disassociated DWDS STA["MACF"]\n",
					slave->band, ETHER_TO_MACF(slave->wbd_ifr.mac),
					ETHER_TO_MACF(param->sta_mac));
				break;
			}

			/* TODO : Remove a STA item from a Slave's Assoc STA List */
			//ret = wbd_ds_remove_sta_fm_slave_assoclist(slave, &(param->sta_mac));

			/* Remove entry from eventlist, if any */
			wbd_ds_remove_event_fm_slave_eventlist(slave, &(param->sta_mac));
		}
		break;
		...
		{
			/* TODO: Remove Add a STA item in a Slave's Assoc STA List */
			/* ret = wbd_ds_add_sta_in_slave_assoclist(slave, &(param->sta_mac), NULL);
			if (ret != WBDE_OK) {
				WBD_DEBUG("Band[%d] Slave["MACF"] STA["MACF"] Event[%u] Failed to "
					"add to assoclist. Error : %s\n", slave->band,
					ETHER_TO_MACF(slave->wbd_ifr.mac),
					ETHER_TO_MACF(param->sta_mac), param->evt.type,
					wbderrorstr(ret));
				break;
			}
			*/

			/* Remove STA from stamon list */
			wbd_slave_remove_sta_fm_stamon(slave, &(param->sta_mac), NULL);

			/* Remove entry from eventlist */
			wbd_ds_remove_event_fm_slave_eventlist(slave, &(param->sta_mac));
		}
		break;

		default:
			WBD_DEBUG("Band[%d] Slave["MACF"] Event[%u]. Unknown Event.\n",
				slave->band, ETHER_TO_MACF(slave->wbd_ifr.mac),
				param->evt.type);
		break;
	}
...
wbd_slave_check_bh_join_timer_cb(bcm_usched_handle *hdl, void *arg)
{
	...
	blanket_try_get_bssid(param->ifname, WBD_MAX_GET_BSSID_TRY,
		WBD_GET_BSSID_USECOND_GAP, &cur_bssid);
	if (eacmp(&cur_bssid, &param->bh_steer_msg->trgt_bssid) == 0) {
		param->bh_steer_msg->resp_status_code = 0; /* Success */
	} else if (param->check_cnt <= 0) {
		param->bh_steer_msg->resp_status_code = 3; /* Auth or Assoc failed */
	} else {
		goto end;
	}
...
/* Process event on dedicated backhaul interface */
static int
wbd_slave_process_dedicated_backhaul_evt(wbd_blanket_slave_t *blanket_slave,
	char *ifname, struct ether_addr *sta_mac, uint32 evt_type,
	uint8 *data, int len)
{
	int ret = WBDE_OK;
	WBD_ENTER();
	...
	switch (evt_type) {
		...
		{
			WBD_INFO("Ifname[%s] disassoc/deauth Event[0x%x] for STA["MACF"]\n",
				ifname,	evt_type, ETHERP_TO_MACF(sta_mac));
			ret = wbd_ds_remove_sta_fm_slave_dwdslist(blanket_slave, sta_mac);
			WBD_INFO("Ifname[%s] DWDS STA["MACF"] Removed\n",
				ifname, ETHERP_TO_MACF(sta_mac));
			WBD_CHECK_MSG("Ifname[%s] Failed to remove DWDS STA["MACF"]."
				"Error : %s\n", ifname,	ETHERP_TO_MACF(sta_mac),
				wbderrorstr(ret));
			break;
		}
		...
		case WLC_E_ASSOC_IND:
		{
			WBD_INFO("Ifname[%s] Assoc Event[0x%x] for STA["MACF"]\n", ifname,
				evt_type, ETHERP_TO_MACF(sta_mac));
			/* Check if the STA is DWDS STA or not */
			if (wbd_wl_is_sta_type_dwds(ifname, sta_mac, &ret, data, len)) {

				/* If DWDS STA, add to dwdslist */
				ret = wbd_ds_add_sta_in_slave_dwdslist(blanket_slave,
					sta_mac, NULL);
				WBD_INFO("Ifname[%s] DWDS STA["MACF"] JOINED\n",
					ifname, ETHERP_TO_MACF(sta_mac));
				WBD_CHECK_MSG("Ifname[%s] Failed to add DWDS STA["MACF"]."
					"Error : %s\n", ifname,	ETHERP_TO_MACF(sta_mac),
					wbderrorstr(ret));
				break;
			}
			WBD_CHECK_MSG("Ifname[%s] Failed to get DWDS STA["MACF"] Flag."
				"Error : %s\n", ifname, ETHERP_TO_MACF(sta_mac),
				wbderrorstr(ret));
			break;
		}
...
wbd_slave_process_event_msg(wbd_info_t* info, char* pkt, int len)
{
	...
	*/
	...
	if (!slave) {
		 ...
		 * event occured. This event can be on a dedicated backhaul.
		 * Check if this event is on dedicated backhaul.
		 */
		if (!strcmp(ifname, info->wbd_slave->dwds_ifname)) {
			wbd_slave_process_dedicated_backhaul_evt(info->wbd_slave,
				ifname, sta_mac, evt_type, data, len);
			WBD_INFO("Event[0x%x] ifname[%s]\n", evt_type, ifname);
			ret = WBDE_EAPD_ERROR;
			goto end;
		} else {
			/* WAR till virtual interfaces are handled */
			if (!blanket_is_iface_backhaulbss(ifname)) {
				WBD_INFO("Event[0x%x] ifname[%s]\n", evt_type, ifname);
				ret = WBDE_EAPD_ERROR;
				goto end;
			}
	...
	switch (evt_type) {
...
#if 0 /* TODO : Remove */
			/* sanity test to make sure, disassoc event for existing
			 * associated client
			 */
			WBD_INFO("Band[%d] Slave["MACF"] Ifname[%s] Deauth Event[0x%x] for "
				"STA["MACF"]\n", in_band, ETHER_TO_MACF(slave->wbd_ifr.mac), ifname,
				evt_type, ETHERP_TO_MACF(sta_mac));
			if (!wbd_ds_find_sta_in_slave_assoclist(slave, sta_mac, &ret)) {
				WBD_WARNING("Band[%d] Slave["MACF"] STA["MACF"]. Failed to find "
					"STA in assoclist. Error : %s\n", in_band,
					ETHER_TO_MACF(slave->wbd_ifr.mac),
...
#endif
			/* Sanitize occured event */
			ret = wbd_slave_sanitize_event(slave, sta_mac, DISASSOC_BIT);

			/* if no matching event entry for that sta is found, add new event */
			if (ret == WBDE_NEW_EVT_OCRRD) {
				ret = wbd_ds_add_event_in_slave_eventlist(slave, sta_mac, evt_type,
					DISASSOC_BIT, ntoh32(pvt_data->event.status), &event);
				WBD_STEER("In Slave["MACF"] STA["MACF"] Disassociated\n",
					ETHER_TO_MACF(slave->wbd_ifr.mac),
					ETHERP_TO_MACF(sta_mac));
			}
		}
		...
		case WLC_E_ASSOC_IND:
		{
			...
			if (backhaul_sta) {
				...
				i5_assoc_sta->flags |= IEEE1905_MAP_FLAG_STA;
			}
			WBD_INFO("Band[%d] Slave["MACF"] Ifname[%s] Assoc Event[0x%x] for "
				"STA["MACF"]\n", in_band, ETHER_TO_MACF(slave->wbd_ifr.mac), ifname,
				evt_type, ETHERP_TO_MACF(sta_mac));
			/* Check if the STA is DWDS STA or not */
			if (wbd_wl_is_sta_type_dwds(ifname, sta_mac, &ret, data, len)) {

				/* If DWDS STA, no need to process ASSOC, just add to dwdslist */
				ret = wbd_ds_add_sta_in_slave_dwdslist(slave->parent,
					sta_mac, NULL);
				WBD_INFO("Band[%d] Slave["MACF"] Ifname[%s] DWDS STA["MACF"] "
					"JOINED\n", in_band, ETHER_TO_MACF(slave->wbd_ifr.mac),
					ifname, ETHERP_TO_MACF(sta_mac));
				WBD_CHECK_MSG("Band[%d] Slave["MACF"] Ifname[%s] Failed to add "
					"DWDS STA["MACF"]. Error : %s\n", in_band,
					ETHER_TO_MACF(slave->wbd_ifr.mac), ifname,
					ETHERP_TO_MACF(sta_mac), wbderrorstr(ret));
				break;
			}
			WBD_CHECK_MSG("Band[%d] Slave["MACF"] Ifname[%s] Failed to get DWDS "
				"STA["MACF"] Flag. Error : %s\n", in_band,
				ETHER_TO_MACF(slave->wbd_ifr.mac), ifname, ETHERP_TO_MACF(sta_mac),
				wbderrorstr(ret));

			/* Sanitize occured event */
			ret = wbd_slave_sanitize_event(slave, sta_mac, ASSOC_BIT);

			/* if no matching event entry for that sta is found, add new event */
			if (ret == WBDE_NEW_EVT_OCRRD) {
				ret = wbd_ds_add_event_in_slave_eventlist(slave, sta_mac, evt_type,
					ASSOC_BIT, 0x00, &event);
				WBD_STEER("Band[%d] Slave["MACF"] STA["MACF"] Associated\n",
					in_band, ETHER_TO_MACF(slave->wbd_ifr.mac),
					ETHERP_TO_MACF(sta_mac));
			}
		}
	...
	/* Schedule a timer for New Event */
	if (event) {
		ret = wbd_add_timers(slave->parent->parent->hdl, event,
			WBD_SEC_MICROSEC(slave->parent->parent->max.tm_evt_grace),
			wbd_slave_event_grace_timer_cb, 0x00);

		if (ret != WBDE_OK) {
			WBD_WARNING("Band[%d] Slave["MACF"] Interval[%d] Failed to create event "
				"grace timer\n", slave->band, ETHER_TO_MACF(slave->wbd_ifr.mac),
				slave->parent->parent->max.tm_evt_grace);
			/* Remove entry from eventlist */
			wbd_ds_remove_event_fm_slave_eventlist(slave, sta_mac);
		}
	}
...
wbd_slave_send_weak_client_cmd(i5_dm_bss_type *i5_bss, i5_dm_clients_type *i5_assoc_sta)
{
	...
	/* Retrieve fresh stats from driver for the associated WEAK STAs and update locally */
	memset(&metric, 0, sizeof(metric));
	memset(&traffic_stats, 0, sizeof(traffic_stats));
	ret = wbd_slave_process_get_assoc_sta_metric(i5_bss->ifname, i5_bss->BSSID,
		i5_assoc_sta->mac, &metric, &traffic_stats, NULL, NULL);
	WBD_CHECK_ERR_MSG(WBDE_COM_ERROR, "BSS["MACDBG"] STA["MACDBG"] Failed to process "
		"WEAK_CLIENT, Error : %s\n", MAC2STRDBG(i5_bss->BSSID),
...
wbd_slave_send_weak_cancel_cmd(i5_dm_bss_type *i5_bss, i5_dm_clients_type *i5_assoc_sta)
{
	...
	memset(&metric, 0, sizeof(metric));
	memset(&traffic_stats, 0, sizeof(traffic_stats));
	wbd_slave_process_get_assoc_sta_metric(i5_bss->ifname, i5_bss->BSSID, i5_assoc_sta->mac,
		&metric, &traffic_stats, NULL, NULL);
...
int
wbd_slave_set_acsd_mode_to_fixchspec(wbd_slave_item_t *slave_item)
{
	...
	if (MAP_IS_CONTROLLER(slave_item->parent->parent->map_mode)) {
		goto end;
	}
	...
	snprintf(tmpbuf, sizeof(tmpbuf), "set&ifname=%s&param=mode&value=%d", nvval, mode);
	...
	if (!com_hndl) {
		WBD_ERROR("Band[%d] Slave["MACF"] Failed to initialize the communication module "
			"for setting Fixed Chanspec[0x%x] Mode to ACSD\n", slave_item->band,
			ETHER_TO_MACF(slave_item->wbd_ifr.mac), slave_item->wbd_ifr.chanspec);
		ret = WBDE_COM_ERROR;
		goto end;
	}
	...
	WBD_CHECK_MSG("Band[%d] Slave["MACF"] Failed to set Fixed Chanspec[0x%x] Mode to ACSD. "
		"Error : %s\n", slave_item->band, ETHER_TO_MACF(slave_item->wbd_ifr.mac),
		slave_item->wbd_ifr.chanspec, wbderrorstr(ret));
	...
	unsigned char *tlv_data, unsigned char tlv_data_len)
{
	...
	struct ether_addr *sta_mac = NULL;
...
#if 0 /* TO DO */
	/* Revive Me, update My last_active timestamp */
	WBD_DS_UPDATE_LAST_ACTIVE_TIME(slave);
#endif /* 0 */
	...
	sta_mac = (struct ether_addr*)i5_assoc_sta->mac;
	...
	ret = blanket_sta_assoc_disassoc(parent_bssid, sta_mac, 0, 0, 1, NULL, 0);
	if (ret == WBDE_OK) {
	...
	ieee1905_unassoc_sta_link_metric_query *query)
{
	...
	for (i = 0; i < query->chCount; i++) {
		...
		/* Get the band, then get the fronthaul BSS in the local device. From the fronthaul
		 * BSSID, get the slave item
		 */
		band = WBD_BAND_FROM_CHANNEL(channel);
		WBD_DS_FIND_I5_FHBSS_IN_DEVICE_FOR_BAND(self_dev, band, i5_bss, &ret);
		...
		for (count = 0; count < n_sta; count++) {
			/* if passed sat mac address belongs to assoclist in slave,
			 * skip the operation and send action frame to this sta. Let
			 * other agent listen to the response from sta and measure
			 * the rssi
			 */
			i5_assoc_sta = wbd_ds_find_sta_in_device(self_dev,
				(unsigned char*)&mac_addr[count], &ret);
			if (i5_assoc_sta) {
				assoc_sta = (wbd_assoc_sta_item_t*)i5_assoc_sta->vndr_data;
				if (assoc_sta == NULL) {
					WBD_WARNING("STA["MACDBG"] NULL Vendor Data\n",
	...
	i5_dm_bss_type *i5_bss, struct ether_addr* mac_addr)
{
	...
	bcm_stamon_config_t *stamon_config = NULL;
	int sta_load_freq = 0;
	int offchan_time = 0;
	...
	stamon_config = &slave->stamon_config;
	if (stamon_config) {
		sta_load_freq = stamon_config->sta_load_freq;
		offchan_time = info->max.tm_offchan_actframe;
	}
	if (actframe_arg && info->max.tm_offchan_actframe > 0) {
		actframe_arg->af_count = stamon_config->interval *
			sta_load_freq / offchan_time;
		ret = wbd_add_timers(info->hdl, actframe_arg,
			WBD_MSEC_USEC(info->max.tm_offchan_actframe),
	...
	i5_dm_device_type *sdev, bool noselfbss)
{
	...
	memset(&nbr_ie, 0, sizeof(nbr_ie));
	eacopy((struct ether_addr *)(nbss->BSSID), &(nbr_ie.bssid));
	nbr_ie.channel = wf_chspec_ctlchan(nchanspec);
	...
	foreach_i5glist_item(sifr, i5_dm_interface_type, sdev->interface_list) {
		...
		foreach_i5glist_item(sbss, i5_dm_bss_type, sifr->bss_list) {
			...
			/* Get rclass */
			blanket_get_rclass(sbss->ifname, nchanspec, &(nbr_ie.reg));

			/* Add neighbor */
			blanket_add_nbr(sbss->ifname, &nbr_ie);
		}
	}
