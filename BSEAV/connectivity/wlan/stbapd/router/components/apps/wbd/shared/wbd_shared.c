 ...
 *
 * $Id: wbd_shared.c 766982 2018-08-23 12:11:37Z tp888369 $
 */
...
wbd_parse_cli_args(int argc, char *argv[])
{
	...
	if (argc > 1) {
		if ((c = getopt(argc, argv, "hHfFtTvV")) != -1) {
			switch (c) {
				case 'f':
				case 'F':
					foreground = TRUE;
...
wbd_info_init(int *error)
{
	...
	if (MAP_IS_DISABLED(info->map_mode)) {
		WBD_ERROR("WiFi Blanket Disabled. Aborting...\n");
		ret = WBDE_INV_MODE;
		goto end;
	}
...
int
wbd_add_timers(bcm_usched_handle *hdl, void *arg, unsigned long timeout,
	...
	int repeat_flag)
{
	...
	if (status != BCM_USCHEDE_OK) {
		WBD_WARNING("Timeout[%ld]Msec arg[%p] cbfn[%p] Failed to add Timer. Error : %s\n",
...
wbd_retrieve_common_config(wbd_info_t *info)
{
	...
	/* Get NVRAM : Timeout for Slave to send an event to Master */
	info->max.tm_evt_grace = blanket_get_config_val_int(NULL,
		WBD_NVRAM_TM_SLV_EVENT_GRACE, WBD_TM_SLV_EVENT_GRACE);
	...
	WBD_DEBUG("%s=%d\n %s=%d\n"
		...
		WBD_NVRAM_TM_SLV_EVENT_GRACE, info->max.tm_evt_grace,
		...
		info->steer_retry_config.retry_count,
		WBD_NVRAM_TM_PROBE, info->max.tm_probe, info->flags);
...
blanket_get_ieee1905_msglevel(ieee1905_msglevel_t *msglevel)
{
	...
	msglevel->module = blanket_get_config_val_int(NULL, WBD_NVRAM_MULTIAP_MSG_MODULE, 0);
	msglevel->level = blanket_get_config_val_int(NULL, WBD_NVRAM_MULTIAP_MSGLEVEL, 0);
}

/* Initialize the blanket */
int
blanket_init(wbd_info_t *info, unsigned int supServiceFlag, int isRegistrar)
{
	...
	return ieee1905_init(info->hdl, supServiceFlag, isRegistrar, &msglevel, &config);
}
...
/* Routine to send higher layer data event from wbd to appeventd. */
void
wbd_higher_layer_data_send(struct ether_addr *al_mac, unsigned char protocol, unsigned int length,
	unsigned char *data, unsigned int hdr_len)
{
	unsigned int app_data_len = 0;
	unsigned char *app_data = NULL;
	wbd_higher_layer_data_t *hl_data = NULL;
	int sockfd = INVALID_SOCKET;

	app_data_len = sizeof(*hl_data) + length;
	app_data = (unsigned char*)malloc(app_data_len);
	if (!app_data) {
		return;
	}

	/* Prepare event data. */
	memset(app_data, 0, app_data_len);
	hl_data = (wbd_higher_layer_data_t*)app_data;
	hl_data->cmd = 0x001E;
	hl_data->length = app_data_len;
	eacopy(al_mac, &hl_data->al_mac);
	hl_data->msg_type = 0x8018;
	hl_data->protocol = protocol;
	hl_data->data_length = length;
	hl_data->hdr_len = hdr_len;
	memcpy(hl_data->data, data, length);

	/* Connect to the server */
	sockfd = wbd_connect_to_server(WBD_LOOPBACK_IP, EAPD_WKSP_WBD_EVENT_PORT);
	if (sockfd == INVALID_SOCKET) {
		WBD_WARNING("Failed to connect\n");
		goto end;
	}

	/* Send the data */
	if (wbd_socket_send_data(sockfd, (char*)hl_data, app_data_len) <= 0) {
		WBD_WARNING("Failed to send\n");
		goto end;
	}
...
/* prints release version of the apps */
void
wbd_print_app_version(const char* cmd_name)
{
	printf("%s ver %d.%d.%d.%d\n", cmd_name, EPI_MAJOR_VERSION, EPI_MINOR_VERSION,
