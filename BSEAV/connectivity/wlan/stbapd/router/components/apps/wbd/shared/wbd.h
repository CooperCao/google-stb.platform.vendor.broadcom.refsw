 ...
 *
 * $Id: wbd.h 766117 2018-07-25 02:45:53Z pp888947 $
 */
...
#define WBD_TM_SLV_EVENT_GRACE		4	/* Timeout for Slave to send an event to Master */
...
#define WBD_TM_MAP_SEND_BCN_REPORT	200	/* Timeout for sending beacon report (in ms) */
...
#define WBD_DEF_PER_CHAN_BCN_REQ	0	/* Default for per chan bcn request */
...
#define WBD_MAX_BSS_SUPPORTED		2	/* Maximum BSS Supported per radio */
...
#define WBD_NVRAM_TM_SLV_EVENT_GRACE		"wbd_tm_evt_grace"
...
#define WBD_SEC_MICROSEC(x) ((x) * 1000 * 1000)
...
typedef struct wbd_timeouts {
	...
	int tm_evt_grace;		/* Timeout for Slave to send an event to Master */
...
/* MAP Higher Layer event data. */
typedef struct wbd_higher_layer_data {
	int cmd;			/* Higher Layer Data Command ID(0x001E) */
	unsigned int length;		/* Length of whole structure,
					 * sizeof(app_event_wbd_higher_layer_data_t) + data_length
					 */
	struct ether_addr al_mac;	/* AL MAC address. Source AL MAC when it is received
					 * and Destination AL MAC when it has to be sent.
					 */
	unsigned int msg_type;		/* Higher Layer Data Message type(0x8018) */
	unsigned char protocol;		/* Protocol of higher layer data */
	unsigned int hdr_len;		/* Length of the header(Ethernet + IEEE1905 message header).
					 * data+hdr_len is where the TLV starts
					 */
	unsigned int data_length;	/* Length of Higher layer data excluding protocol while
					 * sending. While receiving, its the length of whole data
					 * including ethernet header and ieee1905 message header
					 */
	unsigned char data[1];		/* Higher layer data while sending without protocol. While
					 * receiving, its the whole data including ethernet
					 * header and ieee1905 message header
					 */
} wbd_higher_layer_data_t;
/* --------------------------------- Structure Definitions ---------------------------------- */
