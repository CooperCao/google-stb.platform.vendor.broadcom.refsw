...
void i5CheckForBackhaulStaUpdateFlag(unsigned char *nbr_al_mac, unsigned char *mac)
{
  ...
  i5_dm_device_type *pDevice_nbr = NULL;

  if (!(pDevice_nbr = i5DmDeviceFind(nbr_al_mac))) {
    i5TraceError("Neighbor Device["I5_MAC_DELIM_FMT"] Device Not Found\n",
      I5_MAC_PRM(nbr_al_mac));
    return;
  }

  if (!(pdmif = i5DmInterfaceFind(pDevice_nbr, mac))) {
    i5TraceError("Invalid interface["I5_MAC_DELIM_FMT"] \n",
      I5_MAC_PRM(mac));
    return;
  }

  if (!(pdmif->MediaSpecificInfo[6] & I5_MEDIA_INFO_ROLE_STA)) {
    return;
  }
  while(pDevice) {
    ...
    pDevice = (i5_dm_device_type *)(pDevice->ll.next);
  }
}
int i5TlvDeviceInformationTypeExtract(i5_message_type *pmsg, unsigned char *neighbor_al_mac_address, unsigned char *deviceHasGenericPhy)
{
  ...
  *deviceHasGenericPhy = 0;
  ...
  while (i5MessageTlvExtract(pmsg, i5TlvDeviceInformationType, &length, &pvalue, i5MessageTlvExtractWithoutReset) == 0) {
	...
	if (i5DmIsInterfaceWireless(if_media_type)) {
	  i5CheckForBackhaulStaUpdateFlag(neighbor_al_mac_address, interface_id);
	}
      }
      retval = 0;
    }
...
int i5TlvLegacyNeighborDeviceTypeInsert(i5_message_type *pmsg)
{
	...
	i5_dm_interface_type *pdmif;
	int rc = 0, len = 0, is_consumed = FALSE;
	unsigned int remaining, fullPacket;
	...
	if (remaining > I5_PACKET_BUF_LEN) {
		i5TraceError("short buffer: len %d\n", remaining);
		free(pmem);
		return -1;
	}
	...
	if ((pdmdev = i5DmDeviceFind(i5_config.i5_mac_address)) != NULL) {
		...
		while (pdmif) {
			...
			while (pdmnbor) {
				if (memcmp(pdmif->InterfaceId, pdmnbor->LocalInterfaceId, MAC_ADDR_LEN) == 0) {
					if (remaining >= MAC_ADDR_LEN) {
						...
						rc = i5MessageInsertTlv(pmsg, pmem, len);
						if (!rc) {
							break;
						}
						is_consumed = TRUE;
						...
						len = sizeof(i5_tlv_t) + MAC_ADDR_LEN;
						remaining = fullPacket - sizeof(i5_tlv_t) - MAC_ADDR_LEN;
						/* reuses contents from pdmif */
			...
			/* don't insert tlv if no neighbor device */
			if (!is_consumed && (len > (sizeof(i5_tlv_t) + MAC_ADDR_LEN))) {
				ptlv = (i5_tlv_t *)pmem;
				ptlv->type = i5TlvLegacyNeighborDeviceType;
				ptlv->length = htons(len - sizeof(i5_tlv_t));
				rc = i5MessageInsertTlv(pmsg, pmem, len);
				...
				/* rewind remaining also */
				remaining += sizeof(i5_tlv_t) + MAC_ADDR_LEN;
			}
			pdmif = pdmif->ll.next;
		}
	}
  ...
  while (i5MessageTlvExtract(pmsg, i5Tlv1905NeighborDeviceType, &length, &pvalue, i5MessageTlvExtractWithoutReset) == 0) {
    ...
    if ((length >= MAC_ADDR_LEN) && ((length - MAC_ADDR_LEN) % (MAC_ADDR_LEN + 1) == 0)) {
      ...
      while (length >= pos + MAC_ADDR_LEN + 1) {
        ...
        /* In the local device create the nieghbor if the neighbor_al_mac_address pointing to us
         * and if the neighbor is not created at all, due to unavailability of the discovery
         */
        if (i5DmDeviceIsSelf(neighbor_al_mac_address)) {
          i5Trace("Neighbor AL["I5_MAC_DELIM_FMT"] Local["I5_MAC_DELIM_FMT"] "
            "neighbor ifr["I5_MAC_DELIM_FMT"] Ifname %s and Indec %d\n",
            I5_MAC_PRM(pdevid), I5_MAC_PRM(pmsg->psock->u.sll.mac_address),
            I5_MAC_PRM(local_interface), i5SocketGetIfName(pmsg->psock),
            i5SocketGetIfIndex(pmsg->psock));
          i5DmFillSelfNeighborFromTopologyResp(pmsg->psock->u.sll.mac_address, pdevid,
            local_interface, i5SocketGetIfName(pmsg->psock), i5SocketGetIfIndex(pmsg->psock));
        }
      }
    } else {
...
int i5TlvAssocaitedClientsTypeInsert(i5_message_type *pmsg)
{
  ...
  bss_count = i5DmCountBSSWithAssociatedClients(pdmdev);
  if (bss_count <= 0) {
    rc = 0;
    goto end;
  }

  *pbuf = (unsigned char)bss_count;
  pbuf++;
  ...
  i5HigherLayerProtocolField_Values protocol, unsigned char *data, unsigned short data_len)
{
  unsigned char *pbuf;
  i5_tlv_t *ptlv;
  int rc = 0, index = 0, len = 1 + data_len;
  ...
  index++;
  if (data_len > 0) {
    memcpy(&pbuf[index], data, data_len);
    index += data_len;
  }
...
/* Get the length of all the TLVs in a message */
static unsigned int i5TlvGetAllTLVLen(i5_message_type *pmsg)
{
  i5_packet_type *ppkt = pmsg->packet_list.ll.next;
  unsigned int len = 0, hdr_sz = (sizeof(struct ethhdr) + sizeof(i5_message_header_type));

  /* Go through each packet and calculate the length of the TLV */
  while (ppkt != NULL) {
    len += (ppkt->length - hdr_sz);
    ppkt = ppkt->ll.next;
  }
...
static unsigned int i5TlvGetAllTLVData(i5_message_type *pmsg, unsigned char *complete_data)
{
  i5_packet_type *ppkt = pmsg->packet_list.ll.next;
  i5_packet_type *pprev = ppkt;
  unsigned int index = 0, hdr_sz = (sizeof(struct ethhdr) + sizeof(i5_message_header_type));
  unsigned char *pbuf = complete_data;

  /* Fill header in the end */
  pbuf += hdr_sz;

  /* Go thorugh each packet and aopy only the TLVs */
  while (ppkt != NULL) {
    memcpy(pbuf + index, &ppkt->pbuf[hdr_sz], ppkt->length - hdr_sz);
    index += (ppkt->length - hdr_sz);
    pprev = ppkt;
    ppkt = ppkt->ll.next;
  }

  /* Now copy the last header */
  if (pprev) {
    memcpy(complete_data, &pprev->pbuf[0], hdr_sz);
  }
...
{
  unsigned char *pvalue, *complete_data = NULL;
  unsigned int length, tot_len = 0;
  unsigned int hdr_len = sizeof(struct ethhdr) + sizeof(i5_message_header_type);
  unsigned char protocol;

  if (i5MessageTlvExtract(pmsg, i5TlvHigherLayerDataType, &length, &pvalue,
    i5MessageTlvExtractWithReset) == 0) {
    if (length >= 1) {
      protocol = pvalue[0];
      i5Trace("Higher Layer Data Protocol %d\n", protocol);

      tot_len = i5TlvGetAllTLVLen(pmsg);
      /* Add one ethernet header and message header */
      tot_len += hdr_len;
      complete_data = (unsigned char*)malloc(tot_len);
      if (complete_data == NULL) {
        return -1;
      }

      i5TlvGetAllTLVData(pmsg, complete_data);

      if (i5_config.cbs.higher_layer_data) {
        i5_config.cbs.higher_layer_data(i5MessageSrcMacAddressGet(pmsg), protocol,
          complete_data, tot_len, hdr_len);
      }
      if (complete_data) {
        free(complete_data);
      }
...
int i5TlvChannelPreferenceTypeExtract(i5_message_type *pmsg, int isAgent)
{
  ...
  uint8 *cntrl_chan_array_5g = NULL;
  uint8 cntrl_chan_array_size = 0;
  uint8 chan_count = 0;
  ...
  cntrl_chan_array_5g = i5DmGet5gCntrlChanArray(&cntrl_chan_array_size);
  ...
  while ((i5MessageTlvExtract(pmsg, i5TlvChannelPreferenceType,
    ...
    for (i = 0; (i < cp->rc_count) && (idx + 1 < length); i++) {
      ...
      /* In case of non multi channel, handle regclass 128, 129 and 130 seperately as they have
       * different set of control channels as compared to centre channel
       * in rc_chan_map for 128, 129 and 130
       */
      if (!I5_IS_MCHAN_ENAB(i5_config.flags) && cp->rc_map[i].regclass > REGCLASS_40MHZ_LAST) {
        chan_count = cntrl_chan_array_size;
        chan_list = cntrl_chan_array_5g;
        if (!chan_list) {
          i5TraceError("5G control chan list NULL: %d\n", pvalue[idx]);
          goto end;
        }
      } else {
      ...
      for (j = 0; (j < cp->rc_map[i].count) && (idx + 1 < length) ; j++) {
        for (k = 0; k < chan_count; k++) {
          if (pvalue[idx] == chan_list[k]) {
            if ((cp->rc_map[i].regclass == REGCLASS_160MHZ) &&
              (pvalue[idx] > REGCLASS_160MHZ_MAX_CNTRL_CHAN)) {
              /* It is invalid control chan for regclass 129, exit */
