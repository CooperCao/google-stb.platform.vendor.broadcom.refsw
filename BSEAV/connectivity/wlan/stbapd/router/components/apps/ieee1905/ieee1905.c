 ...
 *
 * $Id: ieee1905.c 766986 2018-08-23 12:19:20Z tp888369 $
 */
  ...
  ieee1905_msglevel_t *msglevel, ieee1905_config *config)
{
  ...
  if (msglevel) {
    i5TraceSet(msglevel->module, msglevel->level, msglevel->ifindex, msglevel->ifmacaddr);
  }
  ...
  unsigned char ssid_len, unsigned short chanspec, char *ifname, unsigned char mapFlags)
{
  ...
  if ((pdmbss = i5DmBSSNew(pdmif, bssid, ssid, ssid_len)) == NULL) {
  ...
  ieee1905_sta_link_metric *metric)
{
  ...
  /* Copy the link metric to the data model */
  memcpy(&pclient->link_metric, metric, sizeof(pclient->link_metric));
...
ieee1905_get_band_from_radiocaps(ieee1905_radio_caps_type *RadioCaps)
{
    ...
    /* If multi channel is not enabled, for 80/160 get control channels */
		if (!I5_IS_MCHAN_ENAB(i5_config.flags) && map_ptr->regclass > REGCLASS_40MHZ_LAST) {
			chan_list = i5DmGet5gCntrlChanArray(&count);
			if (!chan_list || !count) {
				i5TraceError(" Invalid 5g control chan parameters, exit ..\n");
				band_flag = BAND_INV;
				goto end;
			}
		} else {
		...
		for (i = 0; i < count; i++) {
			invalid = FALSE;
			if ((chan_list[i] > REGCLASS_160MHZ_MAX_CNTRL_CHAN) &&
				(map_ptr->regclass == REGCLASS_160MHZ)) {
				i5TraceInfo("max control channel possible in 160 bw reached \n");
				break;
			}
