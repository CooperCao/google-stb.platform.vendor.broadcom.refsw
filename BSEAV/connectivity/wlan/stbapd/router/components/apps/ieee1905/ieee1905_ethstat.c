...
int i5EthStatGetBackhaulLinkMetric(void *ctx, ieee1905_backhaul_link_metric *metric)
{
  ...
  if (!ifCtx->isReadBytesValid || deltaTime > I5_ETHSTAT_MIN_DELTA_TIME) {
    ...
    ifCtx->lastReadBytes = getEthInterfaceStat(ifCtx->name, "statistics/rx_bytes");
    i5TraceInfo("ifCtx->lastReadBytes=%ld\n", ifCtx->lastReadBytes);
    ...
    ifCtx->capacity = speed;

    if (ifCtx->isReadBytesValid) {
      float drate = (float)((float)((ifCtx->lastReadBytes * 8) - (oldBytes * 8))/(float)deltaTime);
      ifCtx->dataRate = min((drate * MBit), speed);
      i5TraceInfo("ifCtx->dataRate=%ld (oldBytes=%ld, ifCtx->lastReadBytes=%ld, "
        "ifCtx->capacity=%ld, deltaTime=%ld)\n",
        ifCtx->dataRate, oldBytes, ifCtx->lastReadBytes, ifCtx->capacity, deltaTime );
        ifCtx->isDataRateValid = 1;
    } else {
      ifCtx->isReadBytesValid = 1;
    ...
    metric->linkAvailability = (unsigned short)(((ifCtx->capacity - ifCtx->dataRate) / (float)ifCtx->capacity) * 100);
