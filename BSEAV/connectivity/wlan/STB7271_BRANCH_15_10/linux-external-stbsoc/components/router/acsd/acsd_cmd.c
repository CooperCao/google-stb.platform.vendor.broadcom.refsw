...
acsd_pass_candi(ch_candidate_t * candi, int count, char* buf, uint* r_size)
{
	...
	for (i = 0; i < count; i++) {
		...
		cptr->chspec = hton16(candi->chspec);
		cptr->valid = candi->valid;
		cptr->is_dfs = candi->is_dfs;
		cptr->reason = hton16(candi->reason);

		if (candi->valid) {
			for (j = 0; j < CH_SCORE_MAX; j++) {
				cptr->chscore[j].score = hton32(candi->chscore[j].score);
				cptr->chscore[j].weight = hton32(candi->chscore[j].weight);
			}
		}
...
acs_policy_name(acs_policy_index i)
{
	switch (i) {
		...
		case ACS_POLICY_MEDIA:		return "Media";
	...
	if (!strcmp(buf, "dump")) {
		...
		if (!strcmp(param, "help")) {
			...
			*r_size = acs_dfsr_dump(ACS_DFSR_CTX(c_info), buf, ACSD_BUFSIZE_4K);
			...
			tmp_stats.version = hton32(chanim_stats->version);
			tmp_stats.buflen = hton32(chanim_stats->buflen);
			tmp_stats.count = hton32(chanim_stats->count);
			...
			*r_size = WL_CHANIM_STATS_FIXED_LEN;
			...
			for (i = 0; i < count; i++) {
				...
				loc.glitchcnt = hton32(stats->glitchcnt);
				loc.badplcp = hton32(stats->badplcp);
				loc.chanspec = hton16(stats->chanspec);
				loc.timestamp = hton32(stats->timestamp);
			...
			for (i = 0; i < CHANIM_ACS_RECORD; i++) {
				if (ch_info->record[idx].valid) {
					...
					record.selected_chspc =
						hton16(record.selected_chspc);
					record.glitch_cnt =
						hton32(record.glitch_cnt);
					record.timestamp =
						hton32(record.timestamp);
