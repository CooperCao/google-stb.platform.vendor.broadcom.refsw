...
static int wpa_ft_rrb_send(wpa_t *wpa, uint8 *dst, uint8 *data, size_t data_len)
{
	...
	memcpy(eth->h_source, bssid, ETH_ALEN);
...
void wpa_ft_r0kh_r1kh_init(wpa_t *wpa)
{
	...
	wpa->fbt_info.r0kh_list = NULL;
	wpa->fbt_info.r1kh_list = NULL;
	...
	foreach(name, fbt_aps, next)
	{
		...
		if (r0kh != NULL)
		{
			...
			sprintf(nv_name, "%s_r0kh_id_len", tar_name);
			printf("nvname1 = %s\n", nv_name);
			...
			memset(nv_name, 0, sizeof(nv_name));
			sprintf(nv_name, "%s_r0kh_id", tar_name);
			printf("nvname2 = %s\n", nv_name);
			...
			memset(nv_name, 0, sizeof(nv_name));
			sprintf(nv_name, "%s_addr", tar_name);
			printf("nvname3 = %s\n", nv_name);
			...
			memset(nv_name, 0, sizeof(nv_name));
			sprintf(nv_name, "%s_r0kh_key", tar_name);
			printf("nvname4 = %s\n", nv_name);
		...
		if (r1kh != NULL)
		{
			...
			memset(nv_name, 0, sizeof(nv_name));
			sprintf(nv_name, "%s_r1kh_id", tar_name);
			printf("nvname1 = %s\n", nv_name);
			...
			memset(nv_name, 0, sizeof(nv_name));
			sprintf(nv_name, "%s_r1kh_key", tar_name);
			printf("nvname3 = %s\n", nv_name);
		...
		size_t r0kh_id_len, const uint8 *pmk_r0_name)
{
	...
	bcopy(wpa->global_key_counter, f.nonce, sizeof(f.nonce));
	wpa_incr_gkc(wpa);
	...
	wpa_ft_rrb_send(wpa, r0kh->addr, (uint8 *) &frame, sizeof(frame));
		...
		uint32 resp_ies_len, uint16 status)
{
	...
	if (nas_set_key(wpa->nas, &sta->ea, wpa->PTK.tk1,
		...
		dbg(wpa->nas, "unicast key rejected by driver, assuming too many associated STAs");
		...
		uint8 *data, size_t data_len)
{
	...
	while (r1kh) {
		if (memcmp(r1kh->addr, src_addr, ETH_ALEN) == 0)
			break;
		r1kh = r1kh->next;
	}
	if (r1kh == NULL) {
	...
	wpa_ft_rrb_send(wpa, src_addr, (uint8 *) &resp, sizeof(resp));
		...
		uint8 *src_addr, uint8 *data, size_t data_len)
{
	...
	r0kh = wpa->fbt_info.r0kh_list;
	while (r0kh) {
		if (memcmp(r0kh->addr, src_addr, ETH_ALEN) == 0)
			break;
		r0kh = r0kh->next;
	}
	if (r0kh == NULL) {
		...
		const uint8 *data, size_t data_len)
{
	...
	if (memcmp(src_addr, bssid, ETH_ALEN) == 0) {
		dbg(wpa->nas, "FT: RRB received frame from remote AP %s matching own"
				"address dropping\n", eabuf_tmp1);
		return -1;
	}

	if (memcmp(dest_addr, bssid, ETH_ALEN) != 0)
	{
...
void wpa_ft_push_pmk_r1(wpa_t *wpa)
{
	...
	while (r1kh_list) {
		...
		while (r0) {
			...
			wpa_ft_rrb_send(wpa, r1kh_list->addr, (uint8 *) &frame, sizeof(frame));
			r0 = r0->next;
		}
