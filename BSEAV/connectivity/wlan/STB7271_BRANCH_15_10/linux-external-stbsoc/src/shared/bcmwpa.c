...
#endif	/* BCMCCX */
#if defined(BCMSUP_PSK) || defined(BCMSUPPL) || defined(WLTDLS)
...
{
	uchar data[128], prf_buff[PRF_OUTBUF_LEN];
	const unsigned char prefix[] = "Pairwise key expansion";
	uint data_len = 0;
	...
	/* generate the PTK */
	ASSERT(sizeof(prefix) + data_len <= PRF_MAX_I_D_LEN);
	KDF(pmk, (int)pmk_len, prefix, sizeof(prefix) - 1, data, data_len,
...
#endif	/* BCMSUP_PSK || GTKOE || BCMAUTH_PSK */
