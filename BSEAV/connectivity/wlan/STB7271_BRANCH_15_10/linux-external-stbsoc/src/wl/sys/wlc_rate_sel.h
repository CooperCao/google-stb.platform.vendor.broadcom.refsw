...
extern void wlc_ratesel_init(ratesel_info_t *rsi, rcb_t *state, void *scb, wlc_rateset_t *rateset,
	uint8 bw, int8 sgi_tx, int8 ldpc_tx, int8 vht_ldpc_tx, uint8 vht_ratemask,
	uint8 active_antcfg_num, uint8 antselid_init, uint32 max_rate, uint32 min_rate);
...
#ifdef WLMESH
uint32 wlc_ratesel_get_curpsr(rcb_t *state);
#endif

#ifdef WL_FRAGDUR
#define WLC_RATE_FRAG_TXAGGTHRESHOLD	2 /* threshold to turn on tx aggregation on fragmentation */

extern ratespec_t wlc_ratesel_getfbrspec(rcb_t *state);
extern bool wlc_ratesel_hit_txaggthreshold(rcb_t *state);
extern void wlc_ratesel_calc_txaggthreshold(wlc_info_t *wrsi, rcb_t *state);
#endif /* WL_FRAGDUR */
