...
#ifdef STB_SOC_WIFI
/* Used to indicate the different types of VSDB case */
typedef enum {
	WLC_MCHAN_NON_BAND_SWITCH = 0,
	WLC_MCHAN_BAND_SWITCH,
	WLC_MCHAN_BAND_SWITCH_STA_GO
} wlc_mchan_vsdb_case_t;
#endif /* STB_SOC_WIFI */
...
#ifdef STB_SOC_WIFI
static uint16
wlc_mchan_proc_time_us_7271(wlc_info_t *wlc, wlc_mchan_vsdb_case_t type);
#endif /* STB_SOC_WIFI */
...
wlc_mchan_watchdog(void *context)
{
	...
	int idx;
	wlc_bsscfg_t *cfg;
	uint32 now = R_REG(wlc->osh, &wlc->regs->tsf_timerlow);
...
wlc_mchan_multichannel_upd(wlc_info_t *wlc, mchan_info_t *mchan)
{
	...
	int idx;
#ifdef STB_SOC_WIFI
	wlc_bsscfg_t *trigger_cfg;
	wlc_bsscfg_t *other_cfg;
	bool bandswitch = FALSE;
	wlc_mchan_vsdb_case_t vsdbtype;
#endif /* STB_SOC_WIFI */
	...
	if (!wlc->pub->_mchan_active) {
...
#ifdef STB_SOC_WIFI
		/* initialize mchan proc time/channel switch time  */
		mchan->proc_time_us = WLC_MCHAN_PROC_TIME_US(wlc);
		mchan->chan_switch_time_us = WLC_MCHAN_CHAN_SWITCH_TIME_US(wlc);
#endif /* STB_SOC_WIFI */
	}
#ifdef STB_SOC_WIFI
	else {
		trigger_cfg = WLC_BSSCFG(wlc, mchan->trigger_cfg_idx);
		other_cfg = WLC_BSSCFG(wlc, mchan->other_cfg_idx);
		bandswitch = !(CHSPEC_WLCBANDUNIT(trigger_cfg->current_bss->chanspec)
					== CHSPEC_WLCBANDUNIT(other_cfg->current_bss->chanspec));

		if (bandswitch) {
			if (P2P_GO(wlc, trigger_cfg))
				vsdbtype = WLC_MCHAN_BAND_SWITCH_STA_GO;
			else
				vsdbtype = WLC_MCHAN_BAND_SWITCH;
		}
		else
			vsdbtype = WLC_MCHAN_NON_BAND_SWITCH;

		mchan->proc_time_us = wlc_mchan_proc_time_us_7271(wlc, vsdbtype);
		mchan->chan_switch_time_us = wlc_mchan_proc_time_us_7271(wlc, vsdbtype);
	}
...
wlc_mchan_proc_time_us_7271(wlc_info_t *wlc, wlc_mchan_vsdb_case_t type)
{
	...
	default:
		/* Default value */
		proc_time = WLC_MCHAN_PROC_TIME_US(wlc);
		break;
	}
	return proc_time;
}
