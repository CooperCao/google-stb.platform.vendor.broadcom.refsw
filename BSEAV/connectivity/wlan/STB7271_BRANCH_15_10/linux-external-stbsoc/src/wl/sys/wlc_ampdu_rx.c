...
wlc_ampdu_release_n_ordered(wlc_info_t *wlc, scb_ampdu_rx_t *scb_ampdu, uint8 tid, uint16 offset)
{
	...
	for (; offset; offset--) {
		...
		if (AMPDU_IS_PKT_PENDING(wlc, resp, indx)) {
			...
			WL_INFORM(("%s: rxholes(%d)\n",
				__FUNCTION__, wlc->ampdu_rx->cnt->rxholes));
		}
		resp->exp_seq = NEXT_SEQ(resp->exp_seq);
	}
...
BCMATTACHFN(wlc_ampdu_rxcfg_init)(wlc_info_t *wlc, ampdu_rx_config_t *ampdu_rx_cfg)
{
	...
	} else if (D11REV_IS(wlc->pub->corerev, 39) && WLC_LDPC_CAP_PHY(wlc)) {
		/* SW43430-243 : 43430 LDPC latency issue WAR */
...
BCMATTACHFN(wlc_ampdu_rx_attach)(wlc_info_t *wlc)
{
	...
	if (!(ampdu_rx->dngl_reorder_bufs = (struct ampdu_reorder_info **)MALLOC(wlc->osh,
	...
	case IOV_SVAL(IOV_AMPDU_HOSTREORDER):
		{
			...
			if (wlc->pub->_ampdu_hostreorder ^ bool_val) {
				FOREACH_BSS(wlc, idx, bsscfg) {
					if (bsscfg->up) {
						wlc_ampdu_rx_cleanup(ampdu_rx, bsscfg,
...
wlc_ampdu_recvdata(ampdu_rx_info_t *ampdu_rx, struct scb *scb, struct wlc_frminfo *f)
{
...
#ifdef BCMCCX
	    (CCX_ENAB(wlc->pub) && !IS_CCKM_AUTH(f->WPA_auth)) &&
#endif
	...
	wlc_ampdu_update_rxcounters(ampdu_rx, (D11RXHDR_ACCESS_VAL(f->rxh,
		...
		PRXS_FT_MASK(wlc->pub->corerev)), scb, f->h, f->p, tid);
			...
			*/
			if (update_host) {
				...
				if (BCMPCIEDEV_ENAB()) {
					...
					AMPDU_CONSOLIDATE_AMSDU_RXCHAIN(wlc->osh, f->p, TRUE);
	...
	if (((offset < resp->ba_wsize) && AMPDU_IS_PKT_PENDING(wlc, resp, indx)) ||
		...
		WL_AMPDU_RX(("wl%d: %s: duplicate seq 0x%x(dropped) flags %x\n",
			wlc->pub->unit, __FUNCTION__, seq, WLPKTTAG(f->p)->flags));
		PKTFREE(wlc->osh, f->p, FALSE);
...
#ifdef WL_RX_STALL
	wlc_rx_healthcheck_update_counters(wlc, f->ac, scb, scb->bsscfg, toss_reason, 1);
#endif
	...
	dot11_addba_req_t *addba_req, int body_len)
{
	...
	btcx_wsize =  wlc_btcx_get_ba_rx_wsize(ampdu_rx->wlc);
	if (btcx_wsize)
	...
	if (!(D11REV_GE(ampdu_rx->wlc->pub->corerev, 40) && (ampdu_rx->wlc->_rx_amsdu_in_ampdu))||
		(!AMSDU_2G_ENAB(ampdu_rx->wlc->pub) &&
	...
	uint8 token, uint16 timeout, uint16 param_set)
{
	...
	p = wlc_frame_get_action(wlc, &scb->ea, &scb->bsscfg->cur_etheraddr,
...
wlc_ampdu_clear_rx_dump(ampdu_rx_info_t *ampdu_rx)
{
	...
	if (ampdu_rx->amdbg) {
		...
		if (AMPDU_MAC_ENAB(ampdu_rx->wlc->pub)) {
			...
			if (ampdu_rx->wlc->clk) {
				wlc_write_shm(ampdu_rx->wlc, MACSTAT_ADDR(ampdu_rx->wlc,
					MCSTOFF_RXBACK), 0);
			}
		}
#endif /* WLAMPDU_MAC */
...
wlc_ampdu_release_frwd_pkts(ampdu_rx_info_t *ampdu_rx, void *p)
{
	...
	while (p != NULL) {
		...
		PKTSETLINK(p, NULL);
		wlc_send_fwdpkt(wlc, p, bsscfg->wlcif);
		p = next_p;
	}
}
...
wlc_ampdu_frwd_handle_host_reorder(ampdu_rx_info_t *ampdu_rx, void *pkt, bool forward)
{
	...
	if (ptr == NULL) {
		...
		if (pkttag->_scb != NULL) {
			ptr = (ampdu_reorder_info_t *)MALLOCZ(wlc->osh,
				sizeof(ampdu_reorder_info_t));
			if (ptr == NULL) {
				WL_ERROR((WLC_MALLOC_ERR, WLCWLUNIT(wlc), __FUNCTION__,
					(int)sizeof(ampdu_reorder_info_t), MALLOCED(wlc->osh)));
				return NULL;
			}
