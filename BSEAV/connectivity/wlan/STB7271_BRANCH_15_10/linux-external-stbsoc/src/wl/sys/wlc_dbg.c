...
wlc_recv_print_rxh(wlc_info_t *wlc, wlc_d11rxhdr_t *wrxh)
{
	...
	uint16 len = rxh->lt80.RxFrameSize;
	uint16 phystatus_0 = rxh->lt80.PhyRxStatus_0;
	uint16 phystatus_1 = rxh->lt80.PhyRxStatus_1;
	uint16 phystatus_2 = rxh->lt80.PhyRxStatus_2;
	uint16 phystatus_3 = rxh->lt80.PhyRxStatus_3;
	uint16 macstatus1 = rxh->lt80.RxStatus1;
	uint16 macstatus2 = rxh->lt80.RxStatus2;
	char flagstr[64];
	char lenbuf[20];
	...
	if ((RXS_SHORT_ENAB(wlc->pub->corerev)) && (rxh->lt80.dma_flags & RXS_SHORT_MASK)) {
#ifdef WL11AX
		if (D11REV_GE(wlc->pub->corerev, 80)) {
			/* TODO : Need to handle 11ax RXH format */
		} else
#endif /* WL11AX */
	...
	prhex("Raw RxDesc", (uchar *)rxh, sizeof(rxh->lt80));

	bcm_format_flags(macstat_flags, macstatus1, flagstr, 64);

	snprintf(lenbuf, sizeof(lenbuf), "0x%x", len);
	printf("RxFrameSize:   %6s (%d)%s\n", lenbuf, len,
		(rxh->lt80.PhyRxStatus_0 & PRXS0_SHORTH) ? " short preamble" : "");
	printf("dma_flags:     %#x\n", rxh->lt80.dma_flags);
	printf("fifo:          %u\n", rxh->lt80.fifo);
	...
	printf("RxTSFTime:     %04x\n", rxh->lt80.RxTSFTime);
	printf("RxChanspec:    %s (%04x)\n", wf_chspec_ntoa(rxh->lt80.RxChan, chanbuf),
	       rxh->lt80.RxChan);
}
