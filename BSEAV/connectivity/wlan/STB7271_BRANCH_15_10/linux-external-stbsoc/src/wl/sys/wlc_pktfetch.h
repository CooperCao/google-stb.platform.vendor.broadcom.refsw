...
#endif
#ifdef BCMCCX
#define CCX_IAPP_PKTFETCH_REQUIRED(wlc, bsscfg, body_len, lsh)	\
	(CCX_ENAB(wlc->pub) && BSSCFG_STA(bsscfg) && bsscfg->BSS &&	\
		    (body_len >= (DOT11_LLC_SNAP_HDR_LEN + CCX_IAPP_HDR_LEN) && \
		    !bcmp(lsh, CISCO_AIRONET_SNAP, DOT11_LLC_SNAP_HDR_LEN)) && \
		    bsscfg->associated)
#endif
