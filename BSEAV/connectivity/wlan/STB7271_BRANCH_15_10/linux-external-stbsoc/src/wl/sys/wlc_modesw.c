...
wlc_modesw_process_ap_backoff_expiry(wlc_modesw_info_t *modesw, wlc_bsscfg_t *bsscfg)
{
	...
	modesw_bsscfg_cubby_t *pmodesw_bsscfg = NULL, *ipmode_cfg = NULL;
	wlc_bsscfg_t *icfg;
	int idx;
	...
	if (pmodesw_bsscfg->action_sendout_counter == 0) {
		...
		if (wlc_modesw_oper_mode_complete(wlc->modesw, bsscfg,
			...
			if (CTRL_FLAGS_HAS(pmodesw_bsscfg->ctrl_flags,
				...
				* done.
				*/
				wlc_modesw_notif_cb_notif(modesw, bsscfg,
				BCME_OK, bsscfg->oper_mode, pmodesw_bsscfg->oper_mode_old,
		...
		if (CTRL_FLAGS_HAS(pmodesw_bsscfg->ctrl_flags,
			...
			if (wlc_modesw_oper_mode_complete(wlc->modesw, bsscfg,
				...
				if (CTRL_FLAGS_HAS(pmodesw_bsscfg->ctrl_flags,
					...
					* done.
					*/
					wlc_modesw_notif_cb_notif(modesw, bsscfg, BCME_OK,
...
wlc_modesw_change_state(wlc_modesw_info_t *modesw_info, wlc_bsscfg_t *cfg, int8 new_state)
{
	...
	if (pmodesw_bsscfg->state != new_state) {
		WL_MODE_SWITCH(("wl%d: cfg = %d MODESW STATE Transition: %d -> %d\n",
	...
	int signal)
{
	...
	ASSERT(pmodesw_bsscfg->timer_ctx->notif == 0);
	pmodesw_bsscfg->timer_ctx->notif = MODESW_TIMER_EXEC_CALLBACKS;
...
void wlc_modesw_oper_mode_timer(void *arg)
{
	...
	switch (notif) {
		...
		{
			int ret = 0;
			WL_MODE_SWITCH(("Start BSSCFG Clone\n"));
		...
		case MODESW_TIMER_EXEC_CALLBACKS:
		{
			...
			if (other_wlc->block_datafifo & DATA_BLOCK_TXCHAIN) {
				other_wlc->block_datafifo &= ~DATA_BLOCK_TXCHAIN;
			}
	...
	wlc_bsscfg_t *bsscfg, uint8 oper_mode, uint8 enabled, uint32 ctrl_flags)
{
	...
	if (BSSCFG_AP(bsscfg) || (BSSCFG_IBSS(bsscfg) && !(BSSCFG_AWDL(wlc, bsscfg) ||
		BSSCFG_NAN_MGMT(bsscfg)))) {
...
wlc_modesw_updown_cb(void *ctx, bsscfg_up_down_event_data_t *updown_data)
{
	...
	modesw_bsscfg_cubby_t *pmodesw_bsscfg = NULL;
	modesw_info = (wlc_modesw_info_t *)ctx;
	pmodesw_bsscfg = MODESW_BSSCFG_CUBBY(modesw_info, updown_data->bsscfg);
	...
	if (updown_data->up == FALSE) {
		WL_MODE_SWITCH(("wl%d.%d: %s: got callback from updown. intf up resetting modesw\n",
			WLCWLUNIT(updown_data->bsscfg->wlc), WLC_BSSCFG_IDX(updown_data->bsscfg),
			__FUNCTION__));
		/* Clear PHY context during wl down */
		wlc_modesw_clear_phy_chanctx(ctx, updown_data->bsscfg);
	}

	wlc_modesw_restore_defaults(ctx, updown_data->bsscfg, updown_data->up);
}
	...
	const uint8 *buf, int len)
{
	...
	/* Validate cubby info */
	if (!pmodesw_bsscfg) {
		WL_ERROR(("%s : modeswitch cubby not enabled modesw info %p cfg %p \n",
			__FUNCTION__, modesw_info, cfg));
		return BCME_ERROR;
	}
	...
	pmodesw_bsscfg->ctrl_flags = ((modesw_bsscfg_cubby_t *)buf)->ctrl_flags;
