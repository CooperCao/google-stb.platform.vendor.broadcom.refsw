...
#endif
#ifdef CCX_SDK
	{"ccx_ihv", IOV_CCX_IHV, (0), 0, IOVT_BOOL, 0},
#endif /* CCX_SDK */
...
#ifdef CCX_SDK
	if (ccx_info->pubci.ccx_ihv && cfgi->ccx_network) {
		/* initialize counters */
		WLCNTSET(ccx_info->pubci.mgmt_cnt.bcastmgmtdisassoc, 0);
		WLCNTSET(ccx_info->pubci.mgmt_cnt.bcastmgmtdeauth, 0);
		WLCNTSET(ccx_info->pubci.mgmt_cnt.bcastmgmtaction, 0);

		/* set key if cached and is for current bssid */
		if (ccx_info->cckm_p_key_cached &&
		    !bcmp(&cfg->target_bss->BSSID, &ccx_info->cckm_p_key.ea, ETHER_ADDR_LEN)) {
			if (wlc_ioctl(ccx_info->wlc, WLC_SET_KEY, &ccx_info->cckm_p_key,
				sizeof(ccx_info->cckm_p_key), NULL))
				WL_ERROR(("wl%d: wlc_ccx_on_join_adopt_bss: error set"
				" pairwise key\n", ccx_info->pub->unit));
			else if (ccx_info->cckm_mfp_key)
				wlc_ccx_mfp_set_key((ccx_t *)ccx_info, &ccx_info->cckm_p_key);
		}
		ccx_info->cckm_p_key_cached = FALSE;
		ccx_info->cckm_mfp_key = FALSE;
	}
#endif /* CCX_SDK */
	...
	case CCX_IAPP_TYPE_RM:
		if (ci->pubci.rm && (iapp->subtype == CCX_IAPP_SUBTYPE_REQ)) {
			...
			if (data_len >= (CCX_RM_REQ_LEN + CCX_RM_REQ_IE_FIXED_LEN)) {
				...
				if (IHV_ENAB(&ci->pubci, cfg) &&
					rm_ie->type == CCX_RM_TYPE_STATISTICS)
					goto ccx_ihv;
			}
#endif /* CCX_SDK */
...
ccx_ihv:
	/* indicate the IAPP frame to ihv */
	wlc_ccx_ind_iapp_pkt(ci, f->h, PKTLEN(ci->pub->osh, f->p), PKTTAG(f->p), cfg);
	return -1;
#endif /* CCX_SDK */
}
...
#ifdef CCX_SDK
	if (!(IHV_ENAB(&ccx->pubci, cfg) && ccx->pubci.ccx_ihv_ies)) {
#endif /* CCX_SDK */
...
wlc_ccx_indicate_event(wlc_ccx_info_t *ci, int event, PTLV_STRUCT tlvs, uint tlv_cnt)
{
	...
	*(uint32*)indication->oui = OID_BCM_CCX;
	...
	shared_indicate_status(wl, wl_get_sh_adapterhandle(wl), 0,
	...
	uint d11_pkt_len, wlc_pkttag_t *pkttag)
{
	...
	rx.uPhyId = wl_ccx_get_cur_phy_id(ci->wlc->wl);
		...
		if (!bcmp((uint8*)snap, CISCO_AIRONET_SNAP, DOT11_LLC_SNAP_HDR_LEN)) {
			...
			if (p[2] == CCXv2_IAPP_TYPE_ROAM) {
				...
				if (cfg->current_bss->ccx_version >= 4 &&
					...
					reason_ie->reason = wlc_ccx_get_roam_reason(ci, cfg);
				}
			}
		}
	}
...
wlc_ccx_log_rx_frame(ccx_t *ccxh, uint8 *d11_pkt, uint d11_pkt_len, struct wl_rxsts *rxsts)
{
	...
	rx_ctxt.uPhyId = wl_ccx_get_cur_phy_id(ci->wlc->wl);
...
wlc_ccx_query_oid(wlc_ccx_info_t *ci, void *wl_ctx, PIHV_QUERY_INFO req, uint len)
{
	...
	*bytes_needed = 0;
	...
	case OID_CCX_FW_VERSION: {
		...
		(void)snprintf(buf, sizeof(buf), "%d.%d", (revinfo.ucoderev >> 16) & 0xffff,
	...
	case OID_CCX_RADIO_CHANNELS: {
		uint phy_type = wl_ccx_get_phy_type(wlc->wl);
		uint band_type = wl_ccx_get_band_type(wlc->wl);
...
wlc_ccx_set_oid(wlc_ccx_info_t *ci, void *wl_ctx, PIHV_SET_INFO req, uint len)
{
	...
	*bytes_needed = 0;
	...
	case OID_CCX_ASSOC_INFO: {
		...
		bi = wlc_assoc_get_join_target(ci->wlc, 0);
		if (bcmp((char*)&assoc->ccxBSSID, &bi->BSSID, ETHER_ADDR_LEN)) {
...
const static struct {
	...
	{ CCX_EVENT_STATUS_INDICATION, "CCX_EVENT_STATUS_INDICATION" },
	{ CCX_EVENT_PACKET_RECEIVED, "CCX_EVENT_PACKET_RECEIVED" },
	{ CCX_EVENT_PACKET_TRANSMITTED, "CCX_EVENT_PACKET_TRANSMITTED" },
	{ CCX_EVENT_OID, "CCX_EVENT_OID" },
	{ CCX_EVENT_OK_TO_ASSOCIATE, "CCX_EVENT_OK_TO_ASSOCIATE" },
	{ CCX_EVENT_SEND_PACKET, "CCX_EVENT_SEND_PACKET" },
	{ OID_CCX_ASSOC_INFO, "OID_CCX_ASSOC_INFO" },
	{ OID_CCX_RM_REQUEST, "OID_CCX_RM_REQUEST" },
	{ OID_CCX_NEIGHBOR_LIST, "OID_CCX_NEIGHBOR_LIST" },
	{ OID_CCX_ROAM, "OID_CCX_ROAM" },
	{ OID_CCX_KEEP_ALIVE_REFRESH, "OID_CCX_KEEP_ALIVE_REFRESH" },
	{ OID_CCX_FRAME_LOGGING_MODE, "OID_CCX_FRAME_LOGGING_MODE" },
	{ OID_CCX_DIAGNOSTICS_MODE, "OID_CCX_DIAGNOSTICS_MODE" },
	{ OID_CCX_STA_STATISTICS_2, "OID_CCX_STA_STATISTICS_2" },
	{ OID_CCX_FW_VERSION, "OID_CCX_FW_VERSION" },
	{ OID_CCX_ANTENNA_DATA, "OID_CCX_ANTENNA_DATA" },
	{ OID_CCX_MANUFACTURER_SERIAL_NUM, "OID_CCX_MANUFACTURER_SERIAL_NUM" },
	{ OID_CCX_MANUFACTURER_MODEL, "OID_CCX_MANUFACTURER_MODEL" },
	{ OID_CCX_DRIVER_VERSION, "OID_CCX_DRIVER_VERSION" },
	{ OID_CCX_LAST_BCN_TIME, "OID_CCX_LAST_BCN_TIME" },
	{ OID_CCX_RADIO_CHANNELS, "OID_CCX_RADIO_CHANNELS" },
	{ OID_CCX_SERVICE_CAPABILITY, "OID_CCX_SERVICE_CAPABILITY" },
	{ OID_CCX_TSF, "OID_CCX_TSF" },
	{ OID_CCX_NUM_TX_BUFFER, "OID_CCX_NUM_TX_BUFFER" }
};
...
const static struct {
	...
	{ NDIS_STATUS_DOT11_ASSOCIATION_COMPLETION, "NDIS_STATUS_DOT11_ASSOCIATION_COMPLETION" },
	{ NDIS_STATUS_DOT11_ASSOCIATION_START, "NDIS_STATUS_DOT11_ASSOCIATION_START" },
	{ NDIS_STATUS_DOT11_CONNECTION_COMPLETION, "NDIS_STATUS_DOT11_CONNECTION_COMPLETION" },
	{ NDIS_STATUS_DOT11_CONNECTION_START, "NDIS_STATUS_DOT11_CONNECTION_START" },
	{ NDIS_STATUS_DOT11_DISASSOCIATION, "NDIS_STATUS_DOT11_DISASSOCIATION" },
	{ NDIS_STATUS_DOT11_LINK_QUALITY, "NDIS_STATUS_DOT11_LINK_QUALITY" },
	{ NDIS_STATUS_DOT11_PMKID_CANDIDATE_LIST, "NDIS_STATUS_DOT11_PMKID_CANDIDATE_LIST" },
	{ NDIS_STATUS_DOT11_PHY_STATE_CHANGED, "NDIS_STATUS_DOT11_PHY_STATE_CHANGED" },
	{ NDIS_STATUS_DOT11_ROAMING_COMPLETION, "NDIS_STATUS_DOT11_ROAMING_COMPLETION" },
	{ NDIS_STATUS_DOT11_ROAMING_START, "NDIS_STATUS_DOT11_ROAMING_START" },
	{ NDIS_STATUS_DOT11_TKIPMIC_FAILURE, "NDIS_STATUS_DOT11_TKIPMIC_FAILURE" },
	{ CCX_TLV_802_11_PACKET, "CCX_TLV_802_11_PACKET" },
	{ CCX_TLV_802_11_FRAME, "CCX_TLV_802_11_FRAME" },
	{ CCX_TLV_BEACON_IHV_DATA, "CCX_TLV_BEACON_IHV_DATA" },
	{ CCX_TLV_RM_REPORT, "CCX_TLV_RM_REPORT" },
	{ CCX_TLV_RX_CHARACTERISTICS, "CCX_TLV_RX_CHARACTERISTICS" },
	{ CCX_TLV_TX_CHARACTERISTICS, "CCX_TLV_TX_CHARACTERISTICS" },
	{ CCX_TLV_QUERY_DATA, "CCX_TLV_QUERY_DATA" },
	{ CCX_TLV_SET_DATA, "CCX_TLV_SET_DATA" },
	{ CCX_TLV_METHOD_DATA, "CCX_TLV_METHOD_DATA" },
	{ CCX_TLV_ADAPTER_RESET, "CCX_TLV_ADAPTER_RESET" }
};
#endif /* BCMINTDBG */ 
...
static const char *wlc_ccx_oid2str(UINT oidval)
{
	...
	for (i = 0; i < ARRAYSIZE(ccx_oid_map); i++) {
		if (oidval == ccx_oid_map[i].oidval)
			return ccx_oid_map[i].oidstr;
	}
#endif /* BCMINTDBG */

	(void)snprintf(buffer, sizeof(buffer), "<unknown> 0x%x", (uint)oidval);
	return buffer;
}
...
static const char *wlc_ccx_tlvcode2str(uint tlv_code)
{
	...
	for (i = 0; i < ARRAYSIZE(ccx_tlv_code_map); i++) {
		if (tlv_code == ccx_tlv_code_map[i].tlv_code)
			return ccx_tlv_code_map[i].status_str;
	}
#endif /* BCMINTDBG */

	(void)snprintf(buffer, sizeof(buffer), "<unknown> 0x%x", (uint)tlv_code);
	return buffer;
}
#endif /* BCMDBG */
#endif /* CCX_SDK */
    ...
    const struct ether_addr *bssid, uint body_len, uint8 **pbody)
{
	...
	if (CCX_ENAB(wlc->pub) && IHV_ENAB(wlc->ccx, wlc->cfg) &&
