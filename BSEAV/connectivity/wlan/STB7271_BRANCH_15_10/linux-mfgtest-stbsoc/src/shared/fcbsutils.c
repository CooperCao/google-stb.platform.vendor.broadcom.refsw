...
fcbs_phy_tbl(void *osh, fcbs_input_data_t *input)
{
	 ...
	 *	dat: hdr: 1, ((tbl_id: 1, tbl_offset: 1, tbl_len: 1) * number of tables)
	 *	cmd: (table_len * table_width / 8) of all the tables
	 */
...
fcbs_bp_reg_nocompress(void *osh, fcbs_input_data_t *input)
{
	...
	/* Calculating the tuple buffer size
	 *	dat: hdr: 1, addr_hi: 1, ((addr_lo :1) * data_size)
	 *	cmd: (data_lo: 1 + data_hi: 1) * data_size
	 */
...
fcbs_initvals_nocompress(void *osh, fcbs_input_data_t *input)
{
	...
	/* Calculating the tuple buffer size
	 *	dat: (hdr: 1, addr_hi: 1, addr_lo :1) * (data_size + 2 * delay_tpl_cnt)
	 *	cmd: (data_lo: 1 + data_hi: 1) * (data_size + 2 * delay_tpl_cnt)
	 */
...
fcbs_delay_tuple(void *osh, fcbs_input_data_t *input)
{
	 ...
	 *	cmd: (data_lo: 1 + data_hi: 1) * (data_size + 2 * delay_tpl_cnt)
	 */
	ft->cmd_buf_size = ((data_size + 2) * FCBS_TUPLE_NUM_BYTES * 3);
	ft->dat_buf_size = ((data_size + 2) * FCBS_TUPLE_NUM_BYTES * 2);
...
fcbs_phy_rad_reg_nocompress(void *osh, fcbs_input_data_t *input)
{
	...
	/* Calculating the tuple buffer size
	 *	dat: (one header for each FCBS_MAX_LEN block) + ((addr_lo: 1) * data_size)
	 *	cmd: (data_lo: 1) * data_size
	 */
	ft->cmd_buf_size =
