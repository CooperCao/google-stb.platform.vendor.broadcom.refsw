...
wl_iov_qualify(wlc_iov_trx_t *var, struct iov_control_info *cinfo)
{
	...
	if (var_qualifies) { /* matched variable */
		if (local_var_idx < 0) {  /* Not found case */
			rv = &iov_unknown_info;
		}
...
wl_iov_check_show(FILE *fid, wlc_iov_trx_t *var, struct iov_control_info *cinfo)
{
	...
	if ((local_var = wl_iov_qualify(var, cinfo)) != NULL) {
		...
		if (cinfo->driver_not_ui && local_var != &iov_unknown_info) {
			return;	/* Don't print out UI matched vars */
		}
		if (NAMES_ONLY(cinfo)) {
			if (cinfo->one_col)
				fprintf(fid, "%s\n", var->name);
			else
				fprintf(fid, "%-35s%s", var->name,
					(cinfo->out_count + 1) % 2 ? " " : "\n");
		} else {
			buf_p = line_buf;
	...
	if (cinfo->ui_not_driver && cinfo->var_found != NULL) {
		for (out_idx = idx = 0; idx < wlu_iov_info_count; idx++) {
			if (!cinfo->var_found[idx]) {
				...
				if (cinfo->one_col) {
					fprintf(fid, "%s\n", wlu_iov_info[idx].name);
				} else {
					fprintf(fid, "%-35s%s", wlu_iov_info[idx].name,
						(++out_idx % 2) ? " " : "\n");
				}
				cinfo->out_count++;
			}
