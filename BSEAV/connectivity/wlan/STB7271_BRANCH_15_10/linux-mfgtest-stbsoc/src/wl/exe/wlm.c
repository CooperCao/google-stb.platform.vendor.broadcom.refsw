...
#if defined(WIN7)
extern DWORD Initialize();
extern DWORD SelectAdapter(HANDLE irh, char * ifname);
extern WINERR ir_vista_init(HANDLE *irh);
#endif
	...
	switch (rwl_get_remote_type()) {
		...
		case NO_REMOTE:
			...
			if (adapter == 0)
				adapter = -1;
#ifdef WIN7
			if (ir_vista_init(&irh) != 0) {
				printf("wlmSelectInterface: initialize failed (win7)\n");
				return FALSE;
			}
			/* Try to Initialize through BCM42RLY.sys for advanced functionality */
			/* Failure of this call will lead to only physical wireless adapter */
			/* support functionality So no use of checking for failure */
			Initialize();
			if (SelectAdapter(irh, interfaceName) != 0) {
				printf("wlmSelectInterface: No Broadcom Wireless Adapter found\n");
				return FALSE;
			}
#else /* Not WIN7, But WINXP */
...
wlm_dump_bss_info(wl_bss_info_t *bi, char *scan_dump_buf, int *len)
{
	...
	*len = 0;
	...
	offset += wlm_print_vs_ie(scan_dump_buf + offset,
...
static int wlm_print_vs_ie(char *scan_dump_buf, uint8 *parse, int len)
{
	bcm_tlv_t *ie;
	int offset = 0;
	...
	while ((ie = bcm_parse_tlvs(parse, (int)len, DOT11_MNG_VS_ID))) {
		int len_tmp = 0;
		offset += sprintf(scan_dump_buf + offset, "VS_IE:");
		offset += sprintf(scan_dump_buf + offset, "%02x%02x", ie->id, ie->len);
		while (len_tmp < ie->len) {
			offset += sprintf(scan_dump_buf + offset, "%02x", ie->data[len_tmp]);
			len_tmp++;
		}
		offset += sprintf(scan_dump_buf + offset, "\n");
...
wlmOlpcAnchorIndexSet(int core, int channel, int olpc_idx, int tempsense)
{
	...
	if ((version_err != BCME_OK) || (*(int*)ptr == 0)) {
		...
		if (wlu_var_getbuf_med(irh, "olpc_anchoridx", olpc_pwr_ptr,
				sizeof(wl_olpc_pwr_t), &ptr) < 0) {
			printf("wlmOlpcAnchorIndexGet: failed to"
