...
BCMATTACHFN(wlc_iocv_high_reg_iovt)(wlc_iocv_info_t *ii, wlc_iovt_desc_t *iovd)
{
	...
	if (idx < high->iovt_cnt) {
		memmove(&high->iovt[idx + 1], &high->iovt[idx],
		        (high->iovt_cnt - idx) * sizeof(wlc_iovt_ent_t));
	}

	high->iovt_wlc ++;

	high->iovt[idx].iovt = iovd->iovt;
#ifdef WLC_PATCH_IOCTL
	high->iovt[idx].patch_iovt = iovd->patch_iovt;
#endif
...
BCMATTACHFN(wlc_iocv_high_reg_ioct)(wlc_iocv_info_t *ii, wlc_ioct_desc_t *iocd)
{
	...
	if (idx < high->ioct_cnt) {
		memmove(&high->ioct[idx + 1], &high->ioct[idx],
		        (high->ioct_cnt - idx) * sizeof(wlc_ioct_ent_t));
	}

	high->ioct_wlc ++;

	high->ioct[idx].ioct = iocd->ioct;
	high->ioct[idx].num_cmds = iocd->num_cmds;
	high->ioct[idx].vld_fn = iocd->st_vld_fn;
	high->ioct_cnt ++;
	...
	void *ctx)
{
	...
	high->iovt[idx].iovt = iovt;
	high->iovt[idx].fn = disp_fn;
#ifdef WLC_PATCH_IOCTL
	high->iovt[idx].patch_iovt = patch_iovt;
	high->iovt[idx].patch_fn = patch_fn;
#endif /* WLC_PATCH_IOCTL */
	high->iovt[idx].ctx = ctx;
...
wlc_iocv_high_find_iov(wlc_iocv_info_t *ii, const char *name, uint16 *tid)
{
	...
	for (i = 0; i < high->iovt_cnt; i ++) {
		 ...
		 */
		if (high->iovt[i].patch_iovt != NULL) {
			if ((vi = bcm_iovar_lookup(high->iovt[i].patch_iovt, name))) {
	...
	/* Search given IOVAR name only in ROM tables only which are not patched */
	for (i = 0; i < high->iovt_cnt; i ++) {
		if ((high->iovt[i].patch_iovt == NULL) && (high->iovt[i].iovt != NULL)) {
			if ((vi = bcm_iovar_lookup(high->iovt[i].iovt, name))) {
				*tid = i;
				return vi;
			}
		}
...
#endif
		if ((err = (high->iovt[tid].fn)(high->iovt[tid].ctx, aid,
				p, p_len, a, a_len, var_sz, wlcif)) != BCME_OK) {
			WL_IOCV_DBG(("%s: fn %p failed, aid %u\n",
			             __FUNCTION__, high->iovt[tid].fn, aid));
			goto exit;
		}
	}
...
wlc_iocv_high_find_ioc(wlc_iocv_info_t *ii, uint32 cid, uint16 *tid)
{
	...
	for (i = 0; i < high->ioct_cnt; i ++) {
		for (j = 0; j < high->ioct[i].num_cmds; j++) {
			if (cid == high->ioct[i].ioct[j].cmd) {
				/* found command 'cid' in the table */
				*tid = i;
				return &high->ioct[i].ioct[j];
			}
		}
	}
	...
	/* forward to registered dispatch callback */
		...
		if ((err = (high->ioct[tid].fn)(high->ioct[tid].ctx, cid,
			...
			WL_IOCV_DBG(("%s: fn %p failed err %d cid %u\n",
			             __FUNCTION__, high->ioct[tid].fn, err, cid));
			goto exit;
		}
	}
	...
	for (high = g_iocv_high; high != NULL; high = high->next) {
		for (i = 0; i < high->iovt_cnt && out_count < out_max; i ++) {
			for (iovar_bank = 0; iovar_bank < max_iovar_bank; iovar_bank++) {
				if (iovar_bank == 0) {
					var = high->iovt[i].iovt;
				}
#ifdef WLC_PATCH_IOCTL
				else {
					var = high->iovt[i].patch_iovt;
				}
#endif
	...
	void *a, uint a_len)
{
	...
	if (high->ioct[tid].vld_fn == NULL)
		return BCME_OK;

	cid = ci->cmd;

	return (high->ioct[tid].vld_fn)(high->high_ctx, cid, a, a_len);
}
...
wlc_iocv_high_dump(wlc_iocv_info_t *ii, struct bcmstrbuf *b)
{
	...
	for (tid = 0; tid < high->iovt_wlc; tid ++) {
		bcm_bprintf(b, "    tid %u: tbl %p", tid, high->iovt[tid].iovt);
		if (high->iovt[tid].iovt[0].name != NULL)
			bcm_bprintf(b, " (%s...)", high->iovt[tid].iovt[0].name);
		bcm_bprintf(b, "\n");
	}
	...
	for (tid = 0; tid < high->ioct_wlc; tid ++) {
		bcm_bprintf(b, "    tid %u: tbl %p cnt %u",
		            tid, high->ioct[tid].ioct, high->ioct[tid].num_cmds);
		if (high->ioct[tid].ioct != NULL)
			bcm_bprintf(b, " (%u...)", high->ioct[tid].ioct[0].cmd);
		bcm_bprintf(b, " vld %p", high->ioct[tid].vld_fn);
		bcm_bprintf(b, "\n");
	}
