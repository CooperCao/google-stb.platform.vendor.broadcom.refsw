	...
	*/
		...
		FOREACH_BSS(wlc, i, cfg) {
			if (BSSCFG_STA(cfg)) {
				...
				wlc_iovar_getint(wlc, "scan_home_time",
				                 (int *)&appvt->sta_scan_home_time);
				if (((uint)wlc->default_bss->beacon_period +
				     EXTRA_SCAN_HOME_TIME_FOR_AP) > appvt->sta_scan_home_time)
...
#ifdef EXT_STA
		if (WLEXTSTA_ENAB(wlc->pub) && wlc->pub->associated) {
			ecsa = wlc_get_ecsa_capability(wlc, bsscfg);
		}
#endif /* EXT_STA */
	...
	if (!wlc_valid_chanspec_db(wlc->cmi, target_bss->chanspec) ||
	   ...
	   (BSS_P2P_ENAB(wlc, bsscfg) &&
		...
		target_bss->chanspec = chspec;
		WL_INFORM(("wl%d: use home chanspec 0x%04x in BSS %s\n",
		           wlc->pub->unit, chspec,
		           bcm_ether_ntoa(&target_bss->BSSID, eabuf)));
	}
#endif /* EXT_STA */
		...
		if (!ap_bsscfg_up_exist) {
			...
			FOREACH_BSS(wlc, i, cfg) {
				if (BSSCFG_STA(cfg)) {
					...
					if (appvt->sta_scan_home_time) {
						wlc_iovar_setint(wlc, "scan_home_time",
							appvt->sta_scan_home_time);
						/* zero out to signal we restored it */
						appvt->sta_scan_home_time = 0;
					}
					break;
				}
			}
	...
	void *rsp, uint rsp_len, struct scb *scb, wlc_assoc_req_t * param)
{
	...
	if (WLEXTSTA_ENAB(wlc->pub)) {
		...
		if (dc && !dc->assoc_approved) {
			...
			if (ap_scb->wpaie_len) {
				bcopy(ap_scb->wpaie, ptr, ap_scb->wpaie_len);
			}
