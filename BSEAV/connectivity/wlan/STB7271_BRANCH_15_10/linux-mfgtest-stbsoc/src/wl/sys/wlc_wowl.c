...
#if (defined(MACOSX) || defined(BCMULP)) && defined(WOWL_OS_OFFLOADS)
                 ...
                 void *p, uint plen, void *arg, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	defined(STB_SOC_WIFI)
		...
		if (M_WAKEEVENT_IND(wlc) == ID16_INVALID) {
...
wlc_wowl_pspoll_setup(wowl_info_t *wowl_info, wlc_bsscfg_t *cfg)
{
	...
	/* PS POLL 20 bytes plus PLCP 12 bytes */
	char pspoll[D11AC_PHY_HDR_LEN+DOT11_PS_POLL_LEN+DOT11_FCS_LEN];
	int len;
	uint8 plcp[D11AC_PHY_HDR_LEN];
	ratespec_t rspec;
	uint8 offset = 0;
	...
	if (D11REV_LT(wlc->pub->corerev, 40))
		...
		if (D11REV_GT(wlc->pub->corerev, 65))
			len = (D11AC_PHY_HDR_LEN+DOT11_PS_POLL_LEN+DOT11_FCS_LEN);
		else
			len = (D11AC_PHY_HDR_LEN+DOT11_PS_POLL_LEN+3) & ~3;
	}

	bzero(pspoll, len);
	bzero(plcp, D11AC_PHY_HDR_LEN);
	...
	wlc_compute_plcp(wlc, cfg, rspec, DOT11_PS_POLL_LEN + DOT11_FCS_LEN,
		fcs, (uint8*)plcp);
	if ((D11REV_LT(wlc->pub->corerev, 40)) ||
		(D11REV_GT(wlc->pub->corerev, 65))) {
		/* only the first 6 bytes of PLCP and store from offset 0 */
...
wlc_wowl_write_offload_ctxt(wowl_info_t *wowl, void *pkt, struct scb *scb, bool is_4gtk)
{
	...
	if (key_info.algo != CRYPTO_ALGO_OFF) {
		...
		if (D11REV_GE(wlc->pub->corerev, 42)) {
			...
			idx = (uint8)wlc_key_get_hw_slot(key);
			tmp |= idx;
			tmp <<= 8;
...
wlc_wowl_write_ucode_templates(wowl_info_t *wowl, int offset, void *pkt, wlc_bsscfg_t *cfg)
{
	...
	else if (D11REV_IS(wlc->pub->corerev, 65) || D11REV_IS(wlc->pub->corerev, 66)) {
		bzero(plcpBuf, D11AC_PHY_HDR_LEN);

		/* Use Beacon PLCP format. Ucode change accordingly. */
		bcopy((uint8*)plcp, (uint8*)&plcpBuf[D11AC_PHY_BEACON_PLCP_OFFSET],
			D11_PHY_HDR_LEN);
	...
	const uint nbytes, const d11init_t *inits, const d11init_t *binits)
{
	...
	if (BCMULP_ENAB() && (D11REV_IS(wlc_hw->corerev, 60) || D11REV_IS(wlc_hw->corerev, 62))) {
...
wlc_wowl_ucode_init(wlc_info_t *wlc, struct scb *scb)
{
	...
	if (scb && (key_info.hw_algo == WSEC_ALGO_AES)) {
		if (D11REV_IS(wlc->pub->corerev, 24) && WLCISNPHY(wlc->band))
...
#if defined(BCMULP)
			if (BCMULP_ENAB()) {
				ret = wlc_wowl_dnld_ucode_inits(wlc_hw, d11ucode_ulp46,
					d11ucode_ulp46sz, d11wakeac6initvals46,
					d11wakeac6bsinitvals46);
			} else {
				WL_ERROR(("wl: %s:WOWL ucode unavailable\n",  __FUNCTION__));
				ret = BCME_ERROR;
			}
#else
			ret = wlc_wowl_dnld_ucode_inits(wlc_hw, d11aeswakeucode46,
				d11aeswakeucode46sz, d11wakeac6initvals46, d11wakeac6bsinitvals46);
#endif
		} else if (D11REV_IS(wlc->pub->corerev, 60)) {
#if defined(BCMULP)
			/* ULP / rev60 / security */
			if (BCMULP_ENAB()) {
				ret = wlc_wowl_dnld_ucode_inits(wlc_hw,
					d11ucode_ulp60, d11ucode_ulp60sz,
					d11ulpac36initvals60,
					d11ulpac36bsinitvals60);
			} else {
				WL_ERROR(("wl: %s:WOWL ucode unavailable\n",  __FUNCTION__));
				ret = BCME_ERROR;
			}
			...
			/* ULP / rev62 / security */
			if (BCMULP_ENAB()) {
#ifdef UCODE_HOST_DNLD_ULP
				ret = wlc_wowl_dnld_ucode_inits(wlc_hw,
					NULL, NULL,
					d11ulpac36initvals62, d11ulpac36bsinitvals62);
#else
				ret = wlc_wowl_dnld_ucode_inits(wlc_hw,
					d11ucode_ulp62, d11ucode_ulp62sz,
					d11ulpac36initvals62, d11ulpac36bsinitvals62);
#endif /* UCODE_HOST_DNLD_ULP */
			} else {
				WL_ERROR(("wl: %s:WOWL ucode unavailable\n",  __FUNCTION__));
				ret = BCME_ERROR;
			}
...
#else
			ret = wlc_wowl_dnld_ucode_inits(wlc_hw, d11ucode_wowl46, d11ucode_wowl46sz,
			d11wakeac6initvals46, d11wakeac6bsinitvals46);
#endif
		} else if (D11REV_IS(wlc->pub->corerev, 60)) {
#if defined(BCMULP)
			/* ULP / rev60 / security */
...
wlc_wowl_verify_d3_exit(wlc_info_t * wlc)
{
	 ...
	 */
	WL_INFORM(("D3 Exit Sanity : OL_CAP:%d M:%d\n", WLOFFLD_CAP(wlc), WOWL_ACTIVE(wlc->pub)));
