	...
	if (csa->csa.count <= 1) {
		...
		if (WLEXTSTA_ENAB(wlc->pub)) {
			...
			FOREACH_UP_AP(wlc, idx, ap_bsscfg) {
				if (BSS_P2P_ENAB(wlc, ap_bsscfg)) {
					ecsa = wlc_get_ecsa_capability(wlc, ap_bsscfg);
					if (ecsa) {
						wlc_do_chanswitch(ap_bsscfg, csa->csa.chspec);
					}
				}
			}
		}
...
#ifdef EXT_STA
bool
wlc_get_ecsa_capability(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
	struct scb *scb;
	struct scb_iter scbiter;
	bool ecsa = FALSE;
