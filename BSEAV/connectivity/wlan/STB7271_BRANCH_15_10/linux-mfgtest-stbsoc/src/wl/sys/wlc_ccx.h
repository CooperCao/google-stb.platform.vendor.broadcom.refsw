...
#if defined(NDIS)
struct wl_info;
extern void *wl_get_sh_adapterhandle(struct wl_info *wl);
extern char *wl_get_DriverDesc(struct wl_info *wl);
extern uint wl_ccx_get_cur_phy_id(struct wl_info *wl);
extern uint wl_ccx_get_phy_type(struct wl_info *wl);
extern uint wl_ccx_get_band_type(struct wl_info *wl);

extern void wl_tx_convert_d11hdr(struct wl_info *wl, struct lbuf *lb, struct ether_header **peh);
extern NDIS_STATUS wl_ndis_queryinfo(NDIS_HANDLE MiniportAdapterContext, NDIS_OID ndis_oid,
	PVOID InfoBuf, ULONG InfoBufLen, PULONG BytesWritten, PULONG BytesNeeded,
	struct wl_oid *oid);
extern NDIS_STATUS wl_ndis_setinfo(NDIS_HANDLE MiniportAdapterContext, NDIS_OID ndis_oid,
	PVOID InfoBuf, ULONG InfoBufLen, PULONG BytesRead, PULONG BytesNeeded,
	struct wl_oid *oid);
extern NDIS_STATUS wl_method_oid(NDIS_HANDLE MiniportAdapterContext, NDIS_OID ndis_oid,
...
#define IHV_ENAB(ccx, cfg) ((ccx)->ccx_ihv && wlc_ccx_get_ccx_network(ccx, cfg))
#endif /* CCX_SDK */
