...
#ifdef WLCNT
typedef struct mutx_interm_stats
{
	ratespec_t rspec;
	uint16 frameid;
	uint16 txcnt;
...
typedef struct mutx_stats {
	...
	/* Number of MU-MIMO frames transmitted for each VHT MCS. Indexed by
	 * mcs + ((nss - 1) * mcs_num)
	 */
...
typedef struct mutx_scb {
	...
	mutx_interm_stats_t *interm_stats;
...
#define MU_STA_MAX_HOLD_DUR 60
	...
	bool blcklst;
	/* TRUE if MU client will be evicted from policy change. */
	bool policy_evict;
} mutx_scb_t;
...
typedef struct mutx_bsscfg_cubby {
	wlc_mutx_policy_t mutx;
} mutx_bsscfg_cubby_t;

#define BSSCFG_MUTX_CUBBY(mu_info, bsscfg) \
	((mutx_bsscfg_cubby_t *)BSSCFG_CUBBY((bsscfg), (mu_info)->bsscfg_handle))
...
/* The following table (not used for 4365c0) lists the aqmfifo allocated to each MU client */
...
static int wlc_mu_doiovar(void *hdl, uint32 actionid,
	...
	|| defined(BCMDBG_MU)
static int wlc_mutx_dump(void *ctx, struct bcmstrbuf *b);
...
static int bsscfg_mutx_init(void *context, wlc_bsscfg_t *cfg);
static void bsscfg_mutx_deinit(void *context, wlc_bsscfg_t *cfg);
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP) || defined(WLTEST) || defined(BCMDBG_AMPDU) \
	|| defined(BCMDBG_MU)
static void wlc_mutx_dump_stats(wlc_mutx_info_t *mu_info, mutx_scb_t *mutx_scb,
		struct bcmstrbuf *b);
#endif
#ifdef WL_PSMX
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP) || defined(WLTEST) || defined(BCMDBG_AMPDU) \
	|| defined(BCMDBG_MU)
static void wlc_muclient_clear_stats(wlc_mutx_info_t *mu_info, struct scb *scb);
...
mutx_blcklst_del(wlc_mutx_info_t *mu_info, mutx_scb_list_t *bkt)
{
	...
	|| defined(BCMDBG_MU)
static void
mutx_blcklst_dump(wlc_mutx_info_t *mu_info, struct bcmstrbuf *b)
{
	mutx_scb_list_t *head, *bkt;
	mutx_scb_list_t *pos, *next;
...
#endif /* BCMDBG || BCMDBG_DUMP || WLTEST || BCMDBG_AMPDU || defined(BCMDBG_MU) */
...
static  mutx_scb_list_t *
mutx_blcklst_match(wlc_mutx_info_t *mu_info, mutx_scb_list_t *head)
{
...
BCMATTACHFN(wlc_mutx_attach)(wlc_info_t *wlc)
{
	...
	if (D11REV_GT(mu_info->pub->corerev, 64)) {
		...
		if (D11REV_IS(mu_info->pub->corerev, 65) ||
			...
			D11REV_IS(mu_info->pub->corerev, 128) || /* 43684 */
#endif /* UNRELEASEDCHIP */
		FALSE)
			mu_info->pub->max_muclients = MUCLIENT_NUM_6;
	}
	else {
	...
	if ((mu_info->bsscfg_handle = wlc_bsscfg_cubby_reserve(wlc,
		sizeof(mutx_bsscfg_cubby_t), bsscfg_mutx_init, bsscfg_mutx_deinit,
		NULL, (void *)mu_info)) < 0) {
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP) || defined(WLTEST) || defined(BCMDBG_AMPDU) \
	|| defined(BCMDBG_MU)
void wlc_mutx_dump_stats(wlc_mutx_info_t *mu_info, mutx_scb_t *mutx_scb, struct bcmstrbuf *b)
{
	mutx_stats_t *mutx_stats = mutx_scb->mutx_stats;
	unsigned int total, last, i;
	/* determines highest MCS array *index* on which a transmit took place */
	for (i = 0, total = 0, last = 0; i < MUTX_MCS_INDEX_NUM; i++) {
		total += mutx_stats->mu_mcs_cnt[i];
		if (mutx_stats->mu_mcs_cnt[i]) last = i;
	}

	bcm_bprintf(b, "MU:\nTX VHT SUCC :");
	if (total) {
		for (i = 0; i <= last; i++) {
			bcm_bprintf(b, "  %d(%d%%)", mutx_stats->mu_mcs_cnt[i],
				(mutx_stats->mu_mcs_cnt[i] * 100) / total);
			if ((i % 12) == 11 && i != last)
				bcm_bprintf(b, "\n        :");
		}
	}
	...
	bcm_bprintf(b, "TX VHT TOT :");
	/* determines highest MCS array *index* on which a transmit took place */
	for (i = 0, total = 0, last = 0; i < MUTX_MCS_INDEX_NUM; i++) {
		total += mutx_stats->txmu[i];
		if (mutx_stats->txmu[i]) last = i;
	}

	if (total) {
		for (i = 0; i <= last; i++) {
			bcm_bprintf(b, "  %d(%d%%)",  mutx_stats->txmu[i],
					(mutx_stats->txmu[i] * 100) / total);
	...
	bcm_bprintf(b, "TX VHT PER :");

	if (total) {
		for (i = 0; i <= last; i++) {
			int unacked = 0, per = 0;
			unacked = mutx_stats->txmu[i] -
			mutx_stats->txmu_succ[i];
			if (unacked < 0) unacked = 0;
			if ((unacked > 0) && (mutx_stats->txmu[i]))
				per = (unacked * 100) / mutx_stats->txmu[i];
	...
	/* determines highest MCS array *index* on which a transmit took place */
	for (i = 0, total = 0, last = 0; i < MUTX_MCS_INDEX_NUM; i++) {
		total += mutx_stats->su_mcs_cnt[i];
		if (mutx_stats->su_mcs_cnt[i]) last = i;
	}

	bcm_bprintf(b, "SU:\nTX VHT SUCC :");
	if (total) {
		for (i = 0; i <= last; i++) {
			bcm_bprintf(b, "  %d(%d%%)", mutx_stats->su_mcs_cnt[i],
				(mutx_stats->su_mcs_cnt[i] * 100) / total);
			if ((i % 12) == 11 && i != last)
				bcm_bprintf(b, "\n        :");
		}
	}
	...
	for (i = 0, total = 0, last = 0; i < MUTX_MAXMUTXCNT; i++) {
		...
		if (mutx_stats->mutxcnt_stat[i]) last = i;
	}
...
static int
wlc_mutx_dump(void *ctx, struct bcmstrbuf *b)
{
	...
	mutx_bsscfg_cubby_t *mu_bsscfg_mutx;
	...
	mu_bsscfg_mutx = BSSCFG_MUTX_CUBBY(mu_info, mu_bsscfg);
	if (mu_bsscfg != NULL) {
		...
		bcm_bprintf(b, "    wl%d.%d %s bw_policy %u ac_policy %u\n",
			...
			mu_bsscfg_mutx->mutx.bw_policy, mu_bsscfg_mutx->mutx.ac_policy);
	}
	...
	FOREACH_LIST_POS(hold_pos, next, mu_info->on_hold_list) {
			...
			bcm_bprintf(b, "%s hold_dur %u(secs)\n",
					bcm_ether_ntoa(&mutx_scb->scb->ea, eabuf),
	...
	void *p, uint plen, void *a, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_MU_POLICY): {
		...
		if (policy->max_muclients != mu_info->pub->max_muclients) {
...
#ifdef UNRELEASEDCHIP
				/* 43684 */
				if (D11REV_IS(mu_info->pub->corerev, 128))
					max = MUCLIENT_NUM_16;
#endif /* UNRELEASEDCHIP */
			}
			...
			err = wlc_txbf_mu_max_links_upd(mu_info->wlc->txbf,
				(uint8)policy->max_muclients);
			if (err)
				break;

			mu_info->pub->max_muclients = (uint8)policy->max_muclients;
		}
		break;
	}
...
wlc_mutx_bw_policy_update(wlc_mutx_info_t *mu_info, wlc_bsscfg_t *bsscfg, bool force)
{
	...
	mutx_bsscfg_cubby_t *bsscfg_mutx = BSSCFG_MUTX_CUBBY(mu_info, bsscfg);

	if (!force && mu_info->ac_policy_on)
		return 0;

	bw_policy_old = bsscfg_mutx->mutx.bw_policy;
...
wlc_mutx_ac_update(wlc_mutx_info_t *mu_info)
{
	...
	FOREACH_BSS(wlc, idx, bsscfg) {
		...
		score = 0;
		bw_policy_old = bsscfg_mutx->mutx.bw_policy;
		ac_policy_old = bsscfg_mutx->mutx.effective_ac;
		...
		bsscfg_mutx->mutx.bw_policy = bw_policy;
		bsscfg_mutx->mutx.ac_policy = ac_policy;
		if (ac_policy) {
			/* To avoid unnecessary re-admissions */
			if (ac_policy_old == MUTX_AC_NONE)
				ac_policy_old = ac_policy;
			bsscfg_mutx->mutx.effective_ac = ac_policy;
		}
		if ((bw_policy != bw_policy_old) ||
			(bsscfg_mutx->mutx.effective_ac != ac_policy_old)) {
			for (bw = BW_80MHZ; bw >= BW_20MHZ; bw--) {
				for (ac = MUTX_AC_BEBK; ac <= MUTX_AC_VO; ac++) {
		...
		if (bsscfg_mutx->mutx.bw_policy == 0 || bsscfg_mutx->mutx.ac_policy == 0) {
			/* No suitable BW/AC combination, pick up the default bw */
			wlc_mutx_bw_policy_update(mu_info, bsscfg, TRUE);
		}
	}

	return (mu_bsscfg ? mu_bsscfg_mutx->mutx.ac_policy : 0);
}
...
wlc_ac_policy_scores_update(wlc_mutx_info_t *mu_info)
{
	...
	mutx_scb_t *mu_scb;
	uint32 ac;
	mutx_bsscfg_cubby_t *bsscfg_mutx;

	mu_bsscfg = wlc_txbf_get_mu_bsscfg(wlc->txbf);
	ASSERT(mu_bsscfg);

	bsscfg_mutx = BSSCFG_MUTX_CUBBY(mu_info, mu_bsscfg);
	ac = bsscfg_mutx->mutx.ac_policy;
...
#define MUTXCNT_SCORE_NBIN 2
	...
	|| defined(BCMDBG_MU)
static void
wlc_muclient_clear_stats(wlc_mutx_info_t *mu_info, struct scb *scb)
{
	mutx_scb_t *mu_scb;
...
wlc_muclients_eval_gpos_stats(wlc_mutx_info_t *mu_info)
{
	...
	for (gbmp = 0, i = 0; i < MUCLIENT_NUM_4; i++) {
		...
		mu_scb = MUTX_SCB(mu_info, scb);
		mutx_stats = mu_scb->mutx_pfmon_stats;
		if (mutx_stats->queued_cnt < MUTX_PKTRATE_EVICT_THRESH)
			continue;
	...
	for (i = 0;  i < MUCLIENT_NUM_4; i++) {
		...
		evict =  ((mutx_stats->queued_cnt > MUTX_PKTRATE_EVICT_THRESH) &&
				(gbmp & 1) == 0) ? TRUE: FALSE;
		gbmp >>= 1;
		mu_scb->blcklst = FALSE;
		if (evict) {
...
wlc_mutx_watchdog(void *mi)
{
	...
	if (!mu_info->mutx_active) {
		...
		FOREACH_BSS(wlc, idx, mu_bsscfg) {
			bsscfg_mutx = BSSCFG_MUTX_CUBBY(mu_info, mu_bsscfg);
			if (wlc_txbf_mu_cap_stas(wlc->txbf, mu_bsscfg, bsscfg_mutx->mutx.bw_policy)
				>= 2) {
	...
	bsscfg_mutx = BSSCFG_MUTX_CUBBY(mu_info, mu_bsscfg);
	if ((mu_bsscfg == NULL) ||
		wlc_txbf_mu_cap_stas(wlc->txbf, mu_bsscfg, bsscfg_mutx->mutx.bw_policy) < 2) {
		if (mu_bsscfg != NULL) {
...
mu_scb_cubby_alloc(wlc_mutx_info_t *mu_info, struct scb *scb)
{
	...
	if ((mu_scb->interm_stats = MALLOCZ(mu_info->osh,
			sizeof(mutx_interm_stats_t))) == NULL) {
		WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			mu_info->pub->unit, __FUNCTION__, MALLOCED(mu_info->osh)));
...
static int
bsscfg_mutx_init(void *context, wlc_bsscfg_t *bsscfg)
{
	wlc_mutx_info_t *mu_info = (wlc_mutx_info_t*)context;
	mutx_bsscfg_cubby_t *bsscfg_mutx = BSSCFG_MUTX_CUBBY(mu_info, bsscfg);
	ASSERT(bsscfg != NULL);

	bsscfg_mutx->mutx.bw_policy = 0;
	bsscfg_mutx->mutx.ac_policy = 0;
	bsscfg_mutx->mutx.effective_ac = 0;
...
static void
bsscfg_mutx_deinit(void *context, wlc_bsscfg_t *bsscfg)
{
	wlc_mutx_info_t *mu_info = (wlc_mutx_info_t*)context;
	mutx_bsscfg_cubby_t *bsscfg_mutx = BSSCFG_MUTX_CUBBY(mu_info, bsscfg);
...
wlc_mutx_admit_muclients(wlc_mutx_info_t *mu_info)
{
	...
	evictees_bmp = ~muclients_bmp & 0xf;
	...
	for (i = 0; i < MUCLIENT_NUM; i++) {
		...
		if (evictee) {
			...
			if (mu_scb_evict->policy_evict) {
				 ...
				 */
				mu_scb_evict->policy_evict = FALSE;
			} else {
...
mu_sta_mu_disable(wlc_mutx_info_t *mu_info, struct scb *scb)
{
	...
	if (cubby->mutx_scb) {
		if (cubby->mutx_scb->item != NULL) {
			mutx_blcklst_del_item(mu_info, cubby->mutx_scb->item);
...
mu_sta_mu_ready(wlc_mutx_info_t *mu_info, struct scb *scb, bool strict_check)
{
	uint32 bw_policy;
	uint8 link_bw;
	mutx_bsscfg_cubby_t *bsscfg_mutx = BSSCFG_MUTX_CUBBY(mu_info, scb->bsscfg);
	...
	/* Check BW policy */
	bw_policy = bsscfg_mutx->mutx.bw_policy;
...
wlc_mu_reinit(wlc_info_t *wlc)
{
	...
	/* Do init twice: this is actually a WAR. Sometimes we were hitting ampdu watchdogs
	 * during the switching. Need to revisit after the queue flush stuff is ready.
	 */
...
#ifdef UNRELEASEDCHIP
			D11REV_IS(wlc->pub->corerev, 128) || /* 43684 */
#endif /* UNRELEASEDCHIP */
			FALSE) {
...
mu_group_id_msg_send(wlc_info_t *wlc, struct scb *scb, uint8 *membership, uint8 *user_pos)
{
	...
	mutx_pkt_info_t *pkt_info = NULL;  /* packet info for callback */
	int ret;
...
mu_ut_client_set(wlc_mutx_info_t *mu_info)
{
	...
	/* Clean up */
	for (i = 0; i < MU_UT_STA_NUM; i++) {
		if (mu_sta_mu_disable(mu_info, scb_list[i]) != BCME_OK) {
			WL_ERROR(("\nCS17. Failed to disable MU on a STA."));
			err = BCME_ERROR;
		}
...
mu_ut_user_pos(wlc_mutx_info_t *mu_info)
{
	...
	if (mu_sta_mu_disable(mu_info, scb) != BCME_OK) {
		WL_ERROR(("\nUP-3. Failed to disable MU on a STA."));
		err = BCME_ERROR;
	}
	wlc_scbfree(wlc, scb);
...
mu_ut_l2(wlc_mutx_info_t *mu_info)
{
	...
	for (i = 0; i < MU_UT_STA_NUM; i++) {
		if (mu_sta_mu_disable(mu_info, scb_list[i]) != BCME_OK) {
			WL_ERROR(("\nL2-7. Failed to disable MU on a STA %d.", i));
			err = BCME_ERROR;
		}
		wlc_scbfree(wlc, scb_list[i]);
	}
...
mu_ut_membership(wlc_mutx_info_t *mu_info)
{
	...
	/* Disable MU on STA */
	if (mu_sta_mu_disable(mu_info, scb) != BCME_OK) {
		WL_ERROR(("\nM-19. Failed to disable MU on a STA."));
		err = BCME_ERROR;
	}
	wlc_scbfree(wlc, scb);
	return err;
...
#ifdef WLCNT
bool mutx_interm_stats_frameid_match(wlc_mutx_info_t *mu_info, struct scb *scb, uint16 frameid)
{
	mutx_scb_t *mu_scb;
	mutx_interm_stats_t *interm_stats;
	mu_scb = MUTX_SCB(mu_info, scb);
	...
	interm_stats = mu_scb->interm_stats;
	if ((interm_stats->txcnt == 0) ||
		(interm_stats->frameid != frameid))
		return FALSE;
	return TRUE;
}
	...
	tx_status_t *txs)
{
	...
	interm_stats->gid = TX_STATUS64_MU_GID(txs->status.s3),
	interm_stats->rspec = TX_STATUS64_MU_RSPEC(txs->status.s4);
	interm_stats->rspec += (1 << TX_STATUS64_MU_NSS_SHIFT);	/* Calculate actual NSS */
	interm_stats->gpos = TX_STATUS64_MU_GPOS(txs->status.s4);
	interm_stats->gbmp = TX_STATUS64_MU_GBMP(txs->status.s4);
	interm_stats->txcnt = TX_STATUS64_INTERM_MUTXCNT(txs->status.s3);
	interm_stats->frameid = txs->frameid;
	interm_stats->mutxcnt = TX_STATUS64_MU_TXCNT(txs->status.s4);

	mcs = interm_stats->rspec & RSPEC_VHT_MCS_MASK;
	nss = (interm_stats->rspec & RSPEC_VHT_NSS_MASK) >> RSPEC_VHT_NSS_SHIFT;
	...
	stats->txmu[rate_index] += interm_stats->txcnt;
	mutx_pfmon_stats->txmu[rate_index] += interm_stats->txcnt;
	...
	uint8 gid, tx_status_t *txs)
{
	...
	if (is_mu) {
		...
		mutx_stats->queued_cnt += ncons;
		mutx_pfmon_stats->queued_cnt += ncons;
		if (txs_mu) {
			uint8 gbmp = TX_STATUS64_MU_GBMP(txs4);
			uint8 gpos = TX_STATUS64_MU_GPOS(txs4);
			uint8 mutxcnt = TX_STATUS64_MU_TXCNT(txs4);
			uint8 gnum = 0;
			for (; gbmp; gbmp >>= 1) {
				if ((gbmp & 0x1)) {
					gnum++;
			...
			mutx_stats->mutx_cnt += ncons;
			mutx_pfmon_stats->mutx_cnt += ncons;
			/* check if frames reported in ncons were tried in previous attempt */
			if (mutx_interm_stats_frameid_match(mu_info, scb, txs->frameid)) {
				/* txcnt reflects no of frames tried in previous attempt */
				if (ncons > interm_stats->txcnt) {
					/* Adjust mutxcnt */
					...
					mutx_stats->mutxcnt_stat[mutxcnt - 1] += ncons;
					mutx_pfmon_stats->mutxcnt_stat[mutxcnt - 1] += ncons;
				}
				bzero(interm_stats, sizeof(mutx_interm_stats_t));
			} else {
				mutx_stats->mutxcnt_stat[mutxcnt - 1] += ncons;
				mutx_pfmon_stats->mutxcnt_stat[mutxcnt - 1] += ncons;
			}
		} else {
			/* update mutx_lost_cnt */
			if (mutx_interm_stats_frameid_match(mu_info, scb, txs->frameid)) {
				/* txcnt reflects no of frames tried in previous attempt as MU */
				mu_info->mutx_lost_cnt += interm_stats->txcnt;
				mutx_stats->mutx_lost_cnt += interm_stats->txcnt;
				bzero(interm_stats, sizeof(mutx_interm_stats_t));
			}

			mu_info->sutx_lost_cnt += nlost;
			mutx_stats->sutx_lost_cnt += nlost;
		}
	...
	if (is_mu && txs_mu) {
		mutx_stats->mu_mcs_cnt[rate_index] += ncons;
		mutx_stats->txmu[rate_index] += ncons;
		mutx_stats->txmu_succ[rate_index] += ncons;
		mutx_pfmon_stats->txmu[rate_index] += ncons;
	} else if (is_mu) {
		mutx_stats->su_mcs_cnt[rate_index] += ncons;
	}
	...
	if (is_mu && txs_mu) {
		...
		WL_INFORM(("wl%d: Sent MU frame to %s with group ID %u, mcs %ux%u.\n",
