...
#endif
#if defined(MACOSX) && defined(AWDL_FAMILY)
	/* MacOSX requirement: send ROAM_START/END event to OS when roam scan start/end.
		Use auth_type to indicate ROAM_END since WLC_E_XXX events limit is reached.
		- Failed case or non IEEE802.1X auth
	*/
	if ((cfg == wlc->cfg) && (cfg->assoc->type == AS_ROAM) &&
		(status || (cfg->WPA_auth == WPA_AUTH_DISABLED))) {
		wlc_bss_mac_event(cfg->wlc, cfg, WLC_E_ROAM_START, NULL, status,
		cfg->roam->reason, OFF, 0, 0);
	}
#endif
	if (status == WLC_E_STATUS_SUCCESS) {
