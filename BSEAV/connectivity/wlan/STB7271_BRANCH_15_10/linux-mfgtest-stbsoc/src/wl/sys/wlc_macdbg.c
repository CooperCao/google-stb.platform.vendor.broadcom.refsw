...
#endif 
#if defined(BCMDBG) || defined(BCMDBG_ERR) || defined(WLTEST)
...
wlc_suspend_mac_debug(wlc_info_t *wlc, uint32 phydebug)
{
	...
	for (loop = 0; loop < 8; loop++) {
		W_REG(osh, &regs->psm_pc_reg_1, loop);
		val = R_REG(osh, &regs->psm_pc_reg_2);
		WL_PRINT(("0x%04x\n", val));
	}
#endif /* WAR4360_UCODE */
}
...
wlc_fatal_error_phyrad_dump(wlc_info_t *wlc, uint8 type)
{
	...
	if (phyrad_size > 0) {
		...
		if ((p != NULL) && (allocated_size >= req_size)) {
			...
			phy_dbg_dump_binary(WLC_PI(wlc), type, p->data,
				(int *) &allocated_size);
		}
	}
}
...
wlc_handle_fatal_error_dump(wlc_info_t *wlc)
{
...
#if defined(WLC_SRAMPMAC)
	req_size = bcm_xtlv_size_for_data(BCM4345_BCM4350_BUF_SIZE_FOR_UCODE_DUMP,
	...
	if ((p != NULL) && (allocated_size >= req_size)) {
		...
		bcm_xtlv_pack_xtlv(p, 1,
			allocated_size - bcm_xtlv_hdr_size(BCM_XTLV_OPTION_ALIGN32),
				NULL, BCM_XTLV_OPTION_ALIGN32);
		/* store the information in Raw Data format */
		wlc_pd11regs_bylist(wlc, wlc->macdbg->pd11regs,
			wlc->macdbg->d11regs_sz, 0, NULL, &p_data);
	}
