...
static void wlc_config_debug_gpio(wlc_hw_info_t *wlc_hw);
...
static void wlc_bmac_mctrl1(struct wlc_hw_info *wlc_hw, uint32 mask, uint32 val);
#endif
#if defined(WL_PSMX)
...
/* the "#else" below is intentional */
	...
	if (D11REV_IS(wlc_hw->corerev, 128)) { /* 43684 */
		wlc_write_inits(wlc_hw, d11ax47bsinitvals128);
		wlc_write_inits(wlc_hw, d11ax47bsinitvalsx128);
	} else if (D11REV_IS(wlc_hw->corerev, 57)) {
		wlc_write_inits(wlc_hw, d11ac24bsinitvals55);
	} else
	...
	if (BUSTYPE(bustype) == PCI_BUS) {
		...
		if (D11REV_GE(wlc_hw->corerev, 32)) {
			if ((CHIPID(wlc_hw->sih->chip) == BCM4360_CHIP_ID) ||
				...
				*/
				dma_param_set(di, HNDDMA_PID_TX_BURSTLEN, DMA_BL_32);
				dma_param_set(di, HNDDMA_PID_RX_BURSTLEN, DMA_BL_32);
#else
	...
	uint32 defmacintmask)
{
    ...
    /* wlc_phy_init_acphy clearing ccreg->gpiocontrol enable debug after*/
	wlc_config_debug_gpio(wlc_hw);
...
BCMUCODEFN(wlc_mhfdef)(wlc_hw_info_t *wlc_hw, uint16 *mhfs)
{
	...
	if (BCM_DMA_CT_ENAB(wlc_hw->wlc)) {
		if (D11REV_IS(wlc_hw->corerev, 65) || D11REV_IS(wlc_hw->corerev, 66) ||
#ifdef UNRELEASEDCHIP
			D11REV_IS(wlc_hw->corerev, 128) || /* 43684a0 */
#endif /* UNRELEASEDCHIP */
			FALSE) {
...
#if defined(BCMINTDBG)
static void
wlc_bmac_mctrl1(wlc_hw_info_t *wlc_hw, uint32 mask, uint32 val)
{
	osl_t *osh = wlc_hw->osh;
	d11regs_t *regs = wlc_hw->regs;
	uint32 maccontrol;
	...
	maccontrol = (R_REG(osh, &regs->maccontrol1) & ~mask) | val;
	W_REG(osh, &regs->maccontrol1, maccontrol);
}
#endif 
...
static void
wlc_config_debug_gpio(wlc_hw_info_t *wlc_hw)
{
    d11regs_t *regs = wlc_hw->regs;
	osl_t *osh = wlc_hw->osh;

	if (BCM7271_CHIP(wlc_hw->sih->chip)) {
		uint32 temp;

		/* 7271 Debug enable additional gpios for debug use gpio[15:13] for the dot11mac gpio*/
		WL_INFORM(("wl%d: Seting up Debug gpio's for ucode", wlc_hw->unit));
		temp = si_gpiocontrol(wlc_hw->sih, (BOARD_GPIO_13|BOARD_GPIO_14|BOARD_GPIO_15) ,
			(BOARD_GPIO_13|BOARD_GPIO_14|BOARD_GPIO_15), GPIO_DRV_PRIORITY);
		OR_REG(osh, &regs->psm_gpio_oe, (BOARD_GPIO_13|BOARD_GPIO_14|BOARD_GPIO_15));
		OR_REG(osh, &regs->psm_gpio_out, (BOARD_GPIO_13|BOARD_GPIO_14|BOARD_GPIO_15));
		WL_INFORM(("%s: gpioControl=0x0%x psm_gpio_oe=0x%x psm_gpio_out=0x%x\n", __FUNCTION__,
			temp, R_REG(osh, &regs->psm_gpio_oe), R_REG(osh, &regs->psm_gpio_out)));
	}
}
/**
 * d11 core init
...
#if defined(PCIEDBG)
	/* For PCIE debug, don't let MAC control gpio */
	W_REG(osh, &regs->psm_gpio_oe, 0x0000);
#endif
...
/* the "#else" below is intentional */
	...
	(void)R_REG(wlc_hw->osh, &wlc_hw->regs->objaddr);
	uint16 val16 = R_REG(wlc_hw->osh, &wlc_hw->regs->objdata);
	W_REG(wlc_hw->osh, &wlc_hw->regs->objdata,
		(val16 & ~C_STREG_SLOWCAL_DN_NBIT) | C_STREG_SLOWCAL_PD_NBIT);
	W_REG(wlc_hw->osh, &wlc_hw->regs->u.d11regs.scc_ctl, 0x0);
	W_REG(wlc_hw->osh, &wlc_hw->regs->u.d11regs.scc_timer_l, 0x0);
	W_REG(wlc_hw->osh, &wlc_hw->regs->u.d11regs.scc_timer_h, 0x10);
#endif /* ATE_BUILD */
...
#ifdef DONGLEBUILD
			wl_set_copycount_bytes(wlc_hw->wlc->wl, tune->copycount, wlc->hwrxoff);
#endif
		}
	} else {
		for (i = 0; i < NFIFO; i++) {
...
#ifdef UNRELEASEDCHIP
	} else if (BCM43684_CHIP(wlc_hw->sih->chip)) {
		/* default 43684 MAC clk freq is 240.00002 MHz -> frac = 279620.24336... */
		W_REG(osh, &regs->u.d11regs.tsf_clk_frac_l, 0x4444);
		W_REG(osh, &regs->u.d11regs.tsf_clk_frac_h, 0x4); /* 'h4_4444 = 279620 */
#endif /* UNRELEASEDCHIP */
	} else if ((CHIPID(wlc_hw->sih->chip) == BCM4335_CHIP_ID) ||
		(CHIPID(wlc_hw->sih->chip) == BCM43430_CHIP_ID) ||
		(CHIPID(wlc_hw->sih->chip) == BCM43018_CHIP_ID) ||
...
BCMINITFN(wlc_gpio_init)(wlc_hw_info_t *wlc_hw)
{
...
#if defined(PCIEDBG)
	/* PCIE debug takes away all gpios */
	return;
#endif

#ifdef BCMINTDBG
	if (wlc_hw->ucode_dbgsel) {
		/* GPIO selects for ucode debugging
		 * 0: All GPIO (default)
		 * 1: PSM PC (0:10), PC-Valid (11), GPIO (12:15)
		 * 2: MAC/PHY interface signals (0:7), PSM PC (8:15)
		 * 3: PSM pipeline signals (0:7), PSM PC (8:15)
		 */
		if ((D11REV_GE(wlc_hw->corerev, 26) || D11REV_GE(wlc_hw->corerev, 29)) &&
			WLCISHTPHY(wlc_hw->band)) {
			/* rev26 MAC has all GPIOSel in MACControl1 register */
			wlc_bmac_mctrl1(wlc_hw, MCTL1_EGS_MASK_REV26,
				((wlc_hw->ucode_dbgsel) << MCTL1_EGS_SHIFT_REV26));

		} else {
			/* 2 bits are in maccontrol */
			wlc_bmac_mctrl(wlc_hw, MCTL_GPOUT_SEL_MASK,
				(wlc_hw->ucode_dbgsel << MCTL_GPOUT_SEL_SHIFT));

			/* from rev14, MAC has extra 2 bits for GPIOSel in MACControl1 register */
			wlc_bmac_mctrl1(wlc_hw, MCTL1_EGS_MASK,
				((wlc_hw->ucode_dbgsel >> 2) << MCTL1_EGS_SHIFT));
		}

		/* all gpios output enabled */
		W_REG(osh, &regs->psm_gpio_oe, 0xffff); /* IHR access */
	}
#endif 
	 ...
	 *	G4 = BTC
	 *	G5 = BTC
	 * 3 wire 7271 use G8 output, G9 and G10 Input
	 */
...
#ifdef BCMINTDBG
	/* override above settings to have all gpios allocated to non-PCI cores */
	if (wlc_hw->ucode_dbgsel) {
		gm = gc = 0xffff;

		wlc_bmac_mhf(wlc_hw, MHF1, MHF1_PACTL, 0, WLC_BAND_ALL);

		wlc_bmac_write_shm(wlc_hw, M_MACHW_CAP_H(wlc_hw),
			(uint16)(((wlc_hw->machwcap & ~MCAP_BTCX) >> 16) & 0xffff));
...
#if defined(WL_PSMX)
			ucodex32 = d11ucodex128;
			nbytes_x = d11ucodex128sz;
#endif
		} else if (D11REV_IS(wlc_hw->corerev, 57)) {
	...
	if (D11REV_IS(wlc_hw->corerev, 128)) { /* 43684a0 */
		if (BCM_DMA_CT_ENAB(wlc_hw->wlc) && load_mu_ucode) {
			...
			WL_ERROR(("%s: wl%d: Loading non-MU ucode\n",
			          __FUNCTION__, wlc_hw->unit));
			wlc_ucode_write(wlc_hw, d11ucode128, d11ucode128sz);
			wlc_ucodex_write(wlc_hw, d11ucodex128, d11ucodex128sz);
		}
	} else if (D11REV_IS(wlc_hw->corerev, 57)) {
		wlc_ucode_write(wlc_hw, d11ucode55, d11ucode55sz);
	} else
#endif /* UNRELEASEDCHIP */
	if (D11REV_IS(wlc_hw->corerev, 58)) {
		if (wlc_hw->macunit == 0) {
...
wlc_bmac_wowlucode_start(wlc_hw_info_t *wlc_hw)
{
	...
	WL_INFORM(("wl%d: wlc_bmac_wowlucode_start mctrl=0x%x  0x%x\n", wlc_hw->unit, mctrl,
		R_REG(wlc_hw->osh, &wlc_hw->regs->maccontrol)));

	/* let the PSM run to the suspended state, set mode to BSS STA */
	SET_MACINTSTATUS(wlc_hw->osh, regs, -1);
	wlc_bmac_mctrl(wlc_hw, ~0, (mctrl | MCTL_PSM_JMP_0));
	WL_INFORM(("wl%d: wlc_bmac_wowlucode_start (mctrl | MCTL_PSM_JMP_0)=0x%x 0x%x\n",
...
{
#ifdef BCMINTDBG
	if (wlc_hw->ucode_dbgsel != 0) {
		WL_ERROR(("%s is set, skip\n", __FUNCTION__));
		wlc_hw->led_gpio_mask = 0x0;
		return;
	}
#endif
...
#if defined(BCMINTDBG)
static int
wlc_bmac_si_ccreg_dump(wlc_hw_info_t *wlc_hw, struct bcmstrbuf *b)
{
...
#if defined(BCMINTDBG)
	wlc_bmac_add_dump_fn(wlc_hw, "regcc", (bmac_dump_fn_t)wlc_bmac_si_ccreg_dump, wlc_hw);
	wlc_bmac_add_dump_fn(wlc_hw, "gpio", (bmac_dump_fn_t)si_gpiodump, wlc_hw->sih);
	wlc_bmac_add_dump_fn(wlc_hw, "siid", (bmac_dump_fn_t)si_dump, wlc_hw->sih);
	wlc_bmac_add_dump_fn(wlc_hw, "sireg", (bmac_dump_fn_t)si_dumpregs, wlc_hw->sih);
	wlc_bmac_add_dump_fn(wlc_hw, "siclk", (bmac_dump_fn_t)si_clkctl_dump, wlc_hw->sih);
	wlc_bmac_add_dump_fn(wlc_hw, "pcieregs", (bmac_dump_fn_t)si_dump_pcieregs, wlc_hw->sih);
#endif /* BCMINTDBG */
#endif /* BCMDBG || BCMDBG_DUMP || BCMDBG_PHYDUMP */
...
wlc_bmac_pkteng(wlc_hw_info_t *wlc_hw, wl_pkteng_t *pkteng, void* p)
{
	...
	case WL_MUPKTENG_PER_TX_START:
	{
		...
		if (wlc_hw->pkteng_status != BCME_OK) {
			...
			err = BCME_BUSY;
#ifndef PKTENG_TXREQ_CACHE
			WL_ERROR(("pkteng TX running already, freeing the packet\n"));
			PKTFREE(wlc_hw->osh, p, TRUE);
#endif /* PKTENG_TXREQ_CACHE */
			break;
		}
...
#ifdef BCMINTDBG
uint16
wlc_bmac_ucode_dbgsel_get(wlc_hw_info_t *wlc_hw)
{
	return wlc_hw->ucode_dbgsel;
}

void
wlc_bmac_ucode_dbgsel_set(wlc_hw_info_t *wlc_hw, uint16 dbgsel)
{
	wlc_hw->ucode_dbgsel = dbgsel;
}
#endif	/* BCMINTDBG*/
...
#ifdef BCMINTDBG
	/* 2-wire ucode only present for corerev 0-13 and 15 */
	else if (btc_wire == WL_BTC_2WIRE) {
			WL_ERROR(("wl%d: 2-wire not supported for corerev %d\n",
				wlc_hw->unit, wlc_hw->corerev));
			return BCME_BADOPTION;
	}
	/* 3-wire ucode only present for corerev 11 or more */
	else if (btc_wire >= WL_BTC_3WIRE) {
		...
		case BCM7271_CHIP_ID:
			{
				if (wlc_hw->boardflags4 & BFL4_BTC3WIRE_VIA_GPIO )
				{
					...
					wlc_hw->btc->gpio_out = gm;
					wlc_hw->btc->gpio_mask = 0x700;
					return;
				}
				break;
			}
...
wlc_full_reset(wlc_hw_info_t *wlc_hw, uint32 val)
{
	...
	if (val & 4) {
		...
		buf = MALLOC(wlc_hw->osh, 8192);
		if (buf != NULL) {
			bcm_binit(&b, buf, 8192);
			...
			for (i = 0; i < wlc_hw->nfifo_inuse; i++) {
				...
				if (wlc_hw->di[i])	{
					bcm_binit(&b, buf, 8192);
					dma_dumptx(wlc_hw->di[i], &b, TRUE);
					if (i == RX_FIFO)
						dma_dumprx(wlc_hw->di[i], &b, TRUE);
					printbig(buf);
				}
			}
			MFREE(wlc_hw->osh, buf, 8192);
		}
	}
#endif 
	if (val & 8) {
...
#ifdef UNRELEASEDCHIP
static const bmc_params_t bmc_params_57 =
	{1, 128, 128, 1, 1, 0, {16, 16, 16, 16, 16, 0, 96, 0, 10}};
#endif
...
BCMATTACHFN(wlc_bmac_bmc_params)(wlc_hw_info_t *wlc_hw)
{
	...
	if (D11REV_IS(wlc_hw->corerev, 58)) {
		...
		bmc_params = &bmc_params_57;
	else if (D11REV_IS(wlc_hw->corerev, 128)) /* 43684a0 */
		bmc_params = &bmc_params_128;
#endif
	else if (D11REV_IS(wlc_hw->corerev, 64) ||
		D11REV_IS(wlc_hw->corerev, 65) ||
		D11REV_IS(wlc_hw->corerev, 66))
...
#ifdef BCMINTDBG
int
wlc_bmac_bmc_get(wlc_hw_info_t *wlc_hw)
{
	BCM_REFERENCE(wlc_hw);
...
int
wlc_bmac_bmc_set(wlc_hw_info_t *wlc_hw, uint16 nbufs)
{
	BCM_REFERENCE(wlc_hw);
	...
	if (nbufs > bmc_maxbufs || nbufs < bmc_params->minbufs[0])
		return BCME_RANGE;
	else
		bmc_nbufs = nbufs;
	return BCME_OK;
...
#endif
	if (BCMGCICOEX_ENAB_BMAC(wlc_hw)) {
		/* Update GCI bit 34: WLAN_IN_5G for BT while WLAN is up in 5G */

		if (wlc_rsdb_mode(wlc_hw->wlc) == PHYMODE_MIMO) {
			si_gci_direct(wlc_hw->wlc->pub->sih,
				GCI_OFFSETOF(wlc_hw->wlc->pub->sih, gci_control_1),
				GCI_WL_5G_UP_INFO, GCI_WL_5G_UP_INFO);
			if (chanspec && CHSPEC_IS5G(chanspec)) {
				si_gci_direct(wlc_hw->wlc->pub->sih,
					GCI_OFFSETOF(wlc_hw->wlc->pub->sih, gci_output[1]),
					GCI_WL_5G_UP_INFO, GCI_WL_5G_UP_INFO);
			} else {
				si_gci_direct(wlc_hw->wlc->pub->sih,
					GCI_OFFSETOF(wlc_hw->wlc->pub->sih, gci_output[1]),
...
#if defined(BCMINTDBG)
		/* get number of reclaim TXDMA descriptor(s) from AQM descriptor field */
		txdma_numd = (txdma_flags & D64_AQM_CTRL1_NUMD_MASK) >> D64_AQM_CTRL1_NUMD_SHIFT;
		pre_txin = dma_get_next_txd_idx(WLC_HW_DI(wlc, fifo), FALSE);
#endif
...
#if defined(BCMINTDBG)
		/* get how many TXDMA descriptor have been reclaimed */
		numd = dma_get_txd_count(WLC_HW_DI(wlc, fifo), pre_txin, FALSE);
		if (txdma_numd != numd) {
			WL_ERROR(("wlc%d %s: Number of reclaimed txdata descriptor mismatch,"
				" txdma_numd %d numd %d\n", wlc->pub->unit, __FUNCTION__,
				txdma_numd, numd));
		}
#endif
...
bool
wlc_bmac_pay_decode_war_get(wlc_hw_info_t *wlc_hw)
{
	return phy_ac_chanmgr_get_pay_decode_war((phy_info_t *)wlc_hw->band->pi);
}

void
wlc_bmac_pay_decode_war_set(wlc_hw_info_t *wlc_hw, bool val)
{
	phy_ac_chanmgr_set_pay_decode_war((phy_info_t *)wlc_hw->band->pi, val);
}
