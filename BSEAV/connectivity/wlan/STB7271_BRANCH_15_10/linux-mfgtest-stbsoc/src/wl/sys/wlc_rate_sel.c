...
#define MESH_MAX_PSR		100	/* packet success rate in percentage */
...
#ifdef WL_FRAGDUR
ratespec_t
wlc_ratesel_getfbrspec(rcb_t *state)
{
	uint8 fbrid;
	ratespec_t fbrspec;

	fbrid = wlc_ratesel_getfbrateid(state, state->rateid);
...
bool BCMFASTPATH
wlc_ratesel_hit_txaggthreshold(rcb_t *state)
{
	if (state->rateid >= state->txaggthreshold) {
		return TRUE;
	}
	else {
		return FALSE;
...
void
wlc_ratesel_calc_txaggthreshold(wlc_info_t *wlc, rcb_t *state)
{
	uint8 rateid = 0;

	if (wlc_fragdur_length(wlc->fragdur)) {
		/* find the lowest MCS fallback rate such that if the rate is used
		 * to transmit frame length of MAX_MPDU_SPACE, the transmission
		 * duration will be less than the maximum transmission duration.
		 */
		for (rateid = state->mcs_baseid;
			rateid < state->active_rates_num; rateid++) {

			ratespec_t fbrspec;
			fbrspec = known_rspec[state->fbrid[rateid]];
			if (IS_MCS(fbrspec)) {
				uint dur;

				dur = wlc_calc_frame_time(wlc,
					fbrspec, WLC_MM_PREAMBLE, MAX_MPDU_SPACE);

				if (dur < wlc_fragdur_length(wlc->fragdur)) {
					break;
				}
			}
		}

		/* use a rate higher by WLC_RATE_FRAG_TXAGGTHRESHOLD or
		 * the highest rate
		 */
		state->txaggthreshold = MIN(rateid + WLC_RATE_FRAG_TXAGGTHRESHOLD,
			state->active_rates_num - 1);
	} else {
		/* lowest mcs rate */
		state->txaggthreshold = state->mcs_baseid;
	}
}
#endif /* WL_FRAGDUR */
	...
	uint32 min_rate, uint start_rate)
{
	...
	*/
	...
	txstreams = MIN(state->rsi->wlc->stf->txstreams, state->rsi->nss_lmt);
 ...
 */
void
wlc_ratesel_init(ratesel_info_t *rsi, rcb_t *state, void *scb, wlc_rateset_t *rateset, /* [in] */
...
#ifdef WLMESH
uint32
wlc_ratesel_get_curpsr(rcb_t *state)
{
	if (state->psri[PSR_CUR_IDX].psr != PSR_UNINIT) {
		return ((state->psri[PSR_CUR_IDX].psr * MESH_MAX_PSR) >> RATESEL_EMA_NF);
	} else {
		return MESH_MAX_PSR;
	}
}
