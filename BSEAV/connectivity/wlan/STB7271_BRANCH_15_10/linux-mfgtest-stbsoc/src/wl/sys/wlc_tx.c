...
#ifdef EXT_STA
	if (WLEXTSTA_ENAB(wlc->pub) && BSSCFG_SAFEMODE(bsscfg)) {
		if (pkttotlen(osh, pkt) > (DOT11_MAX_MPDU_BODY_LEN + ETHER_HDR_LEN)) {
			...
			if (key_info.algo != CRYPTO_ALGO_OFF) {
				...
				if (key_info.algo == CRYPTO_ALGO_OFF) {
					...
					WL_TX_STS_UPDATE(toss_reason,
						WLC_TX_STS_TOSS_UNENCRYPT_FRAME1);
					goto toss;
				}
			}
		}
	...
	if (key_info.algo != CRYPTO_ALGO_OFF) {
		...
		if (WSEC_CKIP_MIC_ENABLED(bsscfg->wsec)) {
			...
			if (err != BCME_OK) {
				...
				WL_TX_STS_UPDATE(toss_reason, WLC_TX_STS_TOSS_KEY_PREP_FAIL);
				goto toss;
			}
...
#ifdef MACOSX
	/* change 8021x prio to match the fifo change later */
	...
	for (k = 0; k < cur_rate.num; k++) {
		...
		if (TXBF_ENAB(wlc->pub)) {
			...
			if (bfe_sts_cap && (D11REV_LE(wlc->pub->corerev, 64) ||
				...
				if ((uint)txbf_chains < WL_TX_CHAINS_4) {
					WL_TXBF(("%s: txbf_chains:%d bfe_sts_cap:%d bitmap:0x%x\n",
						__FUNCTION__, txbf_chains, bfe_sts_cap,
						wlc->stf->txchain));
				}
			} else {
	...
	for (k = 0; k < cur_rate.num; k++) {
		...
		*rate_hdr.FbwInfo =
		...
		if (bfe_sts_cap) {
			...
			if ((uint)txbf_chains < WL_TX_CHAINS_4) {
				WL_TXBF(("%s: txbf_chains:%d bfe_sts_cap:%d bitmap:0x%x\n",
					__FUNCTION__, txbf_chains, bfe_sts_cap, wlc->stf->txchain));
			}
		} else {
