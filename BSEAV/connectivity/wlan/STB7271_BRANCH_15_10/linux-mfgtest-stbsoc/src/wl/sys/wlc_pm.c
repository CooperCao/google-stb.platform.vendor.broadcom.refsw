...
#include <wlioctl.h>
#include <epivers.h>
#if defined(BCMSUP_PSK) || defined(BCMCCX) || defined(STA) || defined(LINUX_CRYPTO)
...
#ifdef BCMULP
static uint wlc_pm_ulp_get_retention_size_cb(void *handle, ulp_ext_info_t *einfo);
static int wlc_pm_ulp_enter_cb(void *handle, ulp_ext_info_t *einfo, uint8 *cache_data);
static int wlc_pm_ulp_recreate_cb(void *handle, ulp_ext_info_t *einfo, uint8 *cache_data,
	uint8 *p2_cache_data);

typedef struct wlc_pm_ulp_dat {
...
static const ulp_p1_module_pubctx_t ulp_pm_ctx = {
	MODCBFL_CTYPE_DYNAMIC,
	wlc_pm_ulp_enter_cb,
	NULL,
	wlc_pm_ulp_get_retention_size_cb,
	wlc_pm_ulp_recreate_cb,
	NULL
};
#endif /* BCMULP */
...
wlc_pm2_sleep_ret_timeout(wlc_bsscfg_t *cfg)
{
	...
	if (time_left <= 5 || time_left > pm->pm2_sleep_ret_threshold) {
...
#ifdef BCMULP
		/* Handle the ULP Timer Restart for TX/RX packet case  - only for PM2 */
		if (BCMULP_ENAB()) {
			if (pm->PM == PM_FAST) {
				wlc_ulp_perform_sleep_ctrl_action(cfg->wlc->ulp, ULP_TIMER_START);
			}
		}
#endif /* BCMULP */
	}
		...
		if (BCMULP_ENAB()) {
			if (pm->PM == PM_FAST) {
				wlc_ulp_perform_sleep_ctrl_action(cfg->wlc->ulp, ULP_TIMER_STOP);
			}
		}
#endif /* BCMULP */
...
#endif /* WLWNM */
#ifdef WLRSDB
		/* On entering sleep mode, move to 1x1 chain */
		if (WLC_RSDB_PM_MODESW_ENAB(wlc->pub) &&
				!RSDB_ACTIVE(wlc->pub) &&
				(wlc == WLC_RSDB_GET_PRIMARY_WLC(wlc))) {
			wlc_rsdb_move_connection(cfg, RSDB_MOVE_CHAIN1, TRUE);
		}
#endif
	}
...
wlc_get_ucode_dbg(wlc_info_t *wlc, wl_pmalert_ucode_dbg_t *ud)
{
	...
	ud->psm_maccommand = R_REG(wlc->osh, &regs->psm_maccommand);
	if (D11REV_GE(wlc->pub->corerev, 40)) {
		ud->txe_status1 = R_REG(wlc->osh, &regs->u.d11acregs.txe_status1);
		if (D11REV_GE(wlc->pub->corerev, 64)) {
			ud->AQMFifoReady = R_REG(wlc->osh, &regs->u.d11acregs.AQMFifoRdy_L);
			ud->AQMFifoReady = ud->AQMFifoReady |
				(R_REG(wlc->osh, &regs->u.d11acregs.AQMFifoRdy_H) << 16);
		} else {
			ud->AQMFifoReady = R_REG(wlc->osh, &regs->u.d11acregs.u0.lt64.AQMFifoReady);
		}
	} else {
		ud->xmtfifordy = R_REG(wlc->osh, &regs->u.d11regs.xmtfifordy);
	}
}
...
#ifdef BCMULP
		/* Handle the ULP Timer Restart for RX packet receive - Only PM1 Case */
		if (BCMULP_ENAB()) {
			if (pm->PM == PM_MAX) {
...
#ifdef BCMULP
static int
wlc_pm_ulp_init(wlc_info_t *wlc)
{
	return ulp_p1_module_register(ULP_MODULE_ID_WLC_PM,
			&ulp_pm_ctx, (void*)wlc);
}

static uint
wlc_pm_ulp_get_retention_size_cb(void *handle, ulp_ext_info_t *einfo)
{
	uint sz = 0;
	wlc_info_t *wlc = (wlc_info_t *)handle;
	int cfg_cnt = wlc_bss_count(wlc);
	ULP_DBG(("%s: sz: %d\n", __FUNCTION__, cfg_cnt * sizeof(wlc_pm_ulp_dat_t)));
	sz = sizeof(wlc_pm_ulp_dat_t) * cfg_cnt;
	return sz;
}

static int
wlc_pm_ulp_enter_cb(void *handle, ulp_ext_info_t *einfo, uint8 *cache_data)
{
	wlc_info_t *wlc = (wlc_info_t *)handle;
	wlc_pm_ulp_dat_t *pm_dat = (wlc_pm_ulp_dat_t *)cache_data;
	int i = 0;
	wlc_bsscfg_t *icfg;

	int cfg_cnt = wlc_bss_count(wlc);
	for (i = 0; i < cfg_cnt; i++) {
		icfg = wlc_bsscfg_find(wlc, i, NULL);
		if (!icfg)
			continue;
		pm_dat->pm2_sleep_ret_time = icfg->pm->pm2_sleep_ret_time;
		pm_dat->pm2_rx_pkts_since_bcn = icfg->pm->pm2_rx_pkts_since_bcn;
		pm_dat->pm2_bcn_sleep_ret_time = icfg->pm->pm2_bcn_sleep_ret_time;
		pm_dat->PMenabled = icfg->pm->PMenabled;
		ULP_DBG(("CAC:i:%d,cfg:%p,0x%p,rt_tm:%x,rx_pkts:%x,bcn_slp:%x,PMenb:%x\n",
			i, OSL_OBFUSCATE_BUF(icfg),
			OSL_OBFUSCATE_BUF(pm_dat), pm_dat->pm2_sleep_ret_time,
			pm_dat->pm2_rx_pkts_since_bcn,
			pm_dat->pm2_bcn_sleep_ret_time,
			pm_dat->PMenabled));
		pm_dat++;
	}
	return BCME_OK;
...
static int
wlc_pm_ulp_recreate_cb(void *handle, ulp_ext_info_t *einfo, uint8 *cache_data,
	uint8 *p2_cache_data)
{
	wlc_info_t *wlc = (wlc_info_t *)handle;
	wlc_pm_ulp_dat_t *pm_dat = (wlc_pm_ulp_dat_t *)cache_data;
	...
	for (i = 0; i < cfg_cnt; i++) {
		...
		if (!icfg)
			continue;
		ULP_DBG(("CAC:i:%d,cfg:%p,pm_dat:%p,rt_tm:%x,rx_pkts:%x,bcn_slp:%x,PMenb:%x\n",
			i, OSL_OBFUSCATE_BUF(icfg), OSL_OBFUSCATE_BUF(pm_dat),
			pm_dat->pm2_sleep_ret_time,
			pm_dat->pm2_rx_pkts_since_bcn,
			pm_dat->pm2_bcn_sleep_ret_time,
			pm_dat->PMenabled));
		icfg->pm->pm2_sleep_ret_time = pm_dat->pm2_sleep_ret_time;
		icfg->pm->pm2_rx_pkts_since_bcn =
			pm_dat->pm2_rx_pkts_since_bcn;
		icfg->pm->pm2_bcn_sleep_ret_time =
			pm_dat->pm2_bcn_sleep_ret_time;
		icfg->pm->PMenabled = pm_dat->PMenabled;
		pm_dat++;
	}
	return BCME_OK;
}
...
static const bcm_iovar_t pm_iovars[] = {
	...
	{"sleep_between_ps_resend", IOV_SLEEP_BETWEEN_PS_RESEND, IOVF_RSDB_SET, 0,  IOVT_UINT8, 0},
...
wlc_pm_doioctl(void *ctx, uint cmd, void *arg, uint len, struct wlc_if *wlcif)
{
	...
	case WLC_SET_PM:
		...
		if (AIBSS_ENAB(wlc->pub) && BSSCFG_IBSS(bsscfg) && val == PM_FAST) {
			val = PM_MAX;
		}
		bcmerror = wlc_set_pm_mode(wlc, val, bsscfg);
		if (WLC_RSDB_PM_MODESW_ENAB(wlc->pub)) {
			if (val == PM_FAST) {
				mboolclr(wlc->cmn->rsdb_move_ovr, WLC_RSDB_MOVE_OVR_PM);
			} else {
...
#else
#define PM_COPY_SIZE	0
#define wlc_pm_bss_get NULL
#define wlc_pm_bss_set NULL
#endif /* WLRSDB */
...
BCMATTACHFN(wlc_pm_attach)(wlc_info_t *wlc)
{
	...
	if (wlc_pm_ulp_init(wlc) != BCME_OK) {
		WL_ERROR(("%s: ulp_p1_module_register failed for PM\n", __FUNCTION__));
		goto fail;
	}
#endif
