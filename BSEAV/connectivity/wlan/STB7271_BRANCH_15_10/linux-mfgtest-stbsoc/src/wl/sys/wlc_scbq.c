 ...
 */
wlc_scbq_info_t*
BCMATTACHFN(wlc_scbq_module_attach)(wlc_info_t *wlc)
{
	...
	if (wlc_module_register(scbq_info->pub,
	                    scbq_iovars,
	                    "scbq", scbq_info,
	                    wlc_scbq_doiovar,
	                    NULL /* watchdog_fn_t watchdog_fn */,
	                    NULL /* up_fn_t up_fn */,
	                    NULL /* down_fn_t down_fn */) != BCME_OK) {
...
fail:
	...
	MODULE_DETACH(scbq_info, wlc_scbq_module_detach);
 ...
 */
void
BCMATTACHFN(wlc_scbq_module_detach)(wlc_scbq_info_t *scbq_info)
{
	if (scbq_info == NULL) {
		return;
	}
	...
	struct scb *scb, scbq_overflow_fn_t overflow_fn)
{
	...
	if (scbq_cubby->packet_overflow_handler != NULL) {
		...
		scbq_info->wlc->pub->unit, __FUNCTION__, OSL_OBFUSCATE_BUF(overflow_fn)));
	}
...
wlc_scbq_txpktcnt(void *ctx)
{
	...
	FOREACHSCB(scbq_info->wlc->scbstate, &scbiter, scb) {
		...
		if (scbq_cubby != NULL) {
			pktcnt += pktq_n_pkts_tot(&scbq_cubby->scb_txq);
		}
	}
