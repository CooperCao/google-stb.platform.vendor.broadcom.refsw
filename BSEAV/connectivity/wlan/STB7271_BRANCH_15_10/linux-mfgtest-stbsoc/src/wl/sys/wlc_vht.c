...
struct wlc_vht_info {
	...
	uint8 orig_vht_features_rates; /* store the original vht_features_rates */
	bool vht_features_rates_change;
};
...
wlc_write_vht_cap_ie(wlc_bsscfg_t *cfg, vht_cap_ie_t *cap_ie)
{
	...
	if (wlc->cmn->max_rateset) {
		 ...
		 */
		htol16_ua_store(wlc->cmn->max_rateset->vht_mcsmap, (uint8*)&cap_ie->rx_mcs_map);
		htol16_ua_store(wlc->cmn->max_rateset->vht_mcsmap, (uint8*)&cap_ie->tx_mcs_map);
	}
	else {
...
wlc_vht_scan_parse_op_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	...
	if ((op_p = wlc_read_vht_op_ie(vhti, data->ie, data->ie_len, &op_ie)) != NULL) {
		...
		if (cfg && cfg->oper_mode_enabled) {
			...
			cfg->oper_mode_enabled, cfg->oper_mode, cfg);
		} else {
			cspec = wlc_vht_chanspec(vhti, op_p, bi->chanspec, FALSE, FALSE, cfg);
		}
...
wlc_vht_up(void *ctx)
{
	...
	wlc_rateset_vhtmcs_build(&rateset, wlc->stf->op_rxstreams);
	rx_mcsmap = wlc_rateset_filter_mcsmap(rateset.vht_mcsmap,
		wlc->stf->rxstreams, (WLC_VHT_FEATURES_MCS_GET(wlc->pub)));

	/* update vht mcs for tx */
	wlc_rateset_vhtmcs_build(&rateset, wlc->stf->txstreams);
	tx_mcsmap = wlc_rateset_filter_mcsmap(rateset.vht_mcsmap,
		wlc->stf->txstreams, (WLC_VHT_FEATURES_MCS_GET(wlc->pub)));

	if (WLC_VHT_FEATURES_GET(wlc->pub, WL_VHT_FEATURES_1024QAM)) {
		wlc_rateset_prop_vhtmcs_build(&rateset, wlc->stf->txstreams);
	} else {
