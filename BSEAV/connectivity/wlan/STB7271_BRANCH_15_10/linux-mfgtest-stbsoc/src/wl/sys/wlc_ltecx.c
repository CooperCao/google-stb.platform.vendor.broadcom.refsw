...
BCMINITFN(wlc_ltecx_init)(wlc_ltecx_info_t *ltecx)
{
	...
	ltecx_hflags = wlc_bmac_read_shm(wlc_hw, ltecx->ltecx_shm_addr
		+ M_LTECX_HOST_FLAGS_OFFSET(wlc_hw));
	...
	if (BCMLTECOEXGCI_ENAB(ltecx->wlc->pub))
	{
		...
		if (ltecx_mode == LTECX_MUX_MODE_GPIO) {
			...
			if (D11REV_GE(ltecx->wlc->pub->corerev, 54))
			{
				/* Polarity needs to be set for Framesync (bit0) so that an event */
				/* is generated only on pos edge of the event */
				W_REG(wlc_hw->osh, &wlc_hw->regs->u.d11regs.btcx_eci_event_addr,
					(BTCX_ECI_EVENT_POLARITY | LTECX_ECI_SLICE_12));
				W_REG(wlc_hw->osh, &wlc_hw->regs->u.d11regs.btcx_eci_event_data,
	...
	wlc_bmac_write_shm(wlc_hw, ltecx->ltecx_shm_addr
		+ M_LTECX_HOST_FLAGS_OFFSET(wlc_hw), ltecx_hflags);
        ...
        void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_LTECX_WCI2_MSG):
		if (p_len >= sizeof (uint16))	{
			...
			wlc_bmac_write_shm(wlc->hw, wlc->hw->btc->bt_shm_addr +
				M_LTECX_WCI2_TST_MSG_OFFSET(wlc_hw), w);
		} else
			err = BCME_BUFTOOSHORT;
		break;
	case IOV_GVAL(IOV_LTECX_WCI2_LOOPBACK):
	{
		...
		rsp.nbytes_rx = (int)wlc_bmac_read_shm(wlc_hw, ltecx->ltecx_shm_addr +
			M_LTECX_WCI2_TST_LPBK_NBYTES_RX_OFFSET(wlc_hw));
		rsp.nbytes_tx = (int)wlc_bmac_read_shm(wlc_hw, ltecx->ltecx_shm_addr +
			M_LTECX_WCI2_TST_LPBK_NBYTES_TX_OFFSET(wlc_hw));
		rsp.nbytes_err = (int)wlc_bmac_read_shm(wlc_hw, ltecx->ltecx_shm_addr +
			M_LTECX_WCI2_TST_LPBK_NBYTES_ERR_OFFSET(wlc_hw));
		...
		if (!BCMLTECOEX_ENAB(wlc->pub)) {
			...
			if (wci2_loopback.loopback_type) {
				...
				w = wlc_bmac_read_shm(wlc_hw,
					ltecx->ltecx_shm_addr + M_LTECX_FLAGS_OFFSET(wlc_hw));
				...
				if (wci2_loopback.loopback_type == LTECX_FLAGS_LPBKSRC_MASK) {
					...
					wlc_bmac_write_shm(wlc_hw, ltecx->ltecx_shm_addr +
						M_LTECX_CRTI_INTERVAL_OFFSET(wlc_hw),
	...
	case IOV_GVAL(IOV_LTECX_MWS_CELLSTATUS):
		if (!wlc->clk) {
			...
			cellstatus = (wlc_bmac_read_shm(wlc->hw, wlc->hw->btc->bt_shm_addr +
				M_LTECX_STATE_OFFSET(wlc_hw)) &
...
void wlc_ltecx_check_tscoex_chmap(wlc_ltecx_info_t *ltecx)
{
	...
	if (ltecx->ltecx_shm_addr) {
		ltecx_flags = wlc_bmac_read_shm(wlc_hw,
			ltecx->ltecx_shm_addr + M_LTECX_FLAGS_OFFSET(wlc_hw));
		ltecx_host_flags = wlc_bmac_read_shm(wlc_hw,
			ltecx->ltecx_shm_addr + M_LTECX_HOST_FLAGS_OFFSET(wlc_hw));
		if (ltecx_ts_en == 1) {
...
wlc_ltecx_check_chmap(wlc_ltecx_info_t *ltecx)
{
	...
	ltecx_state = wlc_bmac_read_shm(wlc_hw,
		ltecx->ltecx_shm_addr + M_LTECX_STATE_OFFSET(wlc_hw));
...
void wlc_ltecx_set_wlanrx_prot(wlc_ltecx_info_t *ltecx)
{
	...
	if (ltecx->ltecx_shm_addr) {
		...
		wlc_bmac_write_shm(wlc_hw,
			ltecx->ltecx_shm_addr + M_LTECX_HOST_FLAGS_OFFSET(wlc_hw), shm_val);
	...
	if (CHSPEC_IS2G(wlc->chanspec))	{
		if (ltecx->ltecx_shm_addr)	{
			if (ltecx->ltetx_adv != ltecx->ltetx_adv_prev)	{
				wlc_bmac_write_shm(wlc_hw, ltecx->ltecx_shm_addr +
					M_LTECX_TX_LOOKAHEAD_DUR_OFFSET(wlc), ltecx->ltetx_adv);
			...
			prot_type = wlc_bmac_read_shm(wlc_hw,
				ltecx->ltecx_shm_addr + M_LTECX_HOST_FLAGS_OFFSET(wlc));
			...
			if (adv_tout != ltecx->adv_tout_prev)	{
				wlc_bmac_write_shm(wlc_hw,
					ltecx->ltecx_shm_addr +
					M_LTECX_PROT_ADV_TIME_OFFSET(wlc),
					adv_tout);
...
void wlc_ltecx_set_wlanrxpri_thresh(wlc_ltecx_info_t *ltecx)
{
	...
	shm_val = wlc_bmac_read_shm(wlc_hw, wlc_hw->btc->bt_shm_addr
				    + M_LTECX_RXPRI_THRESH_OFFSET(wlc_hw));
	/* Set to NONE if !2G */
	...
	wlc_bmac_write_shm(wlc_hw, wlc_hw->btc->bt_shm_addr +
		M_LTECX_RXPRI_THRESH_OFFSET(wlc_hw), shm_val);
}
void wlc_ltecx_set_noise_meas(wlc_ltecx_info_t *ltecx)
{
	...
	if (!si_iscoreup(wlc_hw->sih))
		...
		ltecx_noise_ms = wlc_bmac_read_shm(wlc_hw,
		wlc_hw->btc->bt_shm_addr + M_BTCX_CONFIG_OFFSET(wlc_hw));
	if (ltecx->mws_noise_meas)
		ltecx_noise_ms |= (1 << LTECX_DO_NOISE_EST_POS);
	else
		ltecx_noise_ms &= ~(1 << LTECX_DO_NOISE_EST_POS);
	wlc_bmac_write_shm(wlc_hw,
		wlc_hw->btc->bt_shm_addr + M_BTCX_CONFIG_OFFSET(wlc_hw), ltecx_noise_ms);
}
static int wlc_mws_lte_ant_errcheck(mws_ant_map_t *antmap)
{
	uint16 ant_map_temp;
...
void wlc_ltecx_ant_update(wlc_ltecx_info_t *ltecx)
{
	...
	ant_tx = (wlc_bmac_read_shm(wlc_hw,
		ltecx->ltecx_shm_addr + M_LTECX_FLAGS_OFFSET(wlc_hw)) &
		(LTECX_CELLANT_TX_MASK << C_LTECX_FLAGS_MWS_TYPE7_CELL_TX_ANT))
		>> C_LTECX_FLAGS_MWS_TYPE7_CELL_TX_ANT;
...
wlc_ltecx_process_oclmap(wlc_ltecx_info_t *ltecx, wlc_bss_info_t *bss)
{
	...
	if (CHSPEC_IS5G(bss->chanspec)) {
		while (!disable_ocl && bss_chnum <= ch_edge_hi) {
			...
			if (bss_chnum <= olympic_5g_chmap[15]) {
				disable_ocl = channel_disable_map
					& (~ltecx->mws_oclmap.bitmap_5g_lo);
			} else if (bss_chnum <= olympic_5g_chmap[31]) {
				disable_ocl = channel_disable_map
...
wlc_ltecx_assoc_in_prog(wlc_ltecx_info_t *ltecx, int val)
{
	...
	if (CHSPEC_IS2G(wlc->chanspec) && (ltecx->ltecx_shm_addr)) {
		...
		wlc_bmac_write_shm(wlc_hw, ltecx->ltecx_shm_addr
			+ M_LTECX_HOST_FLAGS_OFFSET(wlc_hw), ltecxHostFlag);
	}
}
...
void wlc_ltecx_interface_active(wlc_hw_info_t *wlc_hw)
{
		if (si_iscoreup(wlc_hw->sih) && wlc_hw->btc->bt_shm_addr) {
			uint16 ltecx_flags;
			ltecx_flags = wlc_bmac_read_shm(wlc_hw,
					wlc_hw->btc->bt_shm_addr + M_LTECX_FLAGS_OFFSET(wlc_hw));
...
wlc_ltecx_update_scanreq_bm_channel(wlc_ltecx_info_t *ltecx, chanspec_t chanspec)
{
	...
	if (ltecx->ltecx_shm_addr) {
		...
		wlc_bmac_write_shm(wlc_hw, ltecx->ltecx_shm_addr +
		              M_LTECX_MWSSCAN_BM_LO_OFFSET(wlc_hw), ltecx_mws_scan_bm_lo);
		wlc_bmac_write_shm(wlc_hw, ltecx->ltecx_shm_addr +
		        M_LTECX_MWSSCAN_BM_HI_OFFSET(wlc_hw), ltecx_mws_scan_bm_hi);
	}
}
