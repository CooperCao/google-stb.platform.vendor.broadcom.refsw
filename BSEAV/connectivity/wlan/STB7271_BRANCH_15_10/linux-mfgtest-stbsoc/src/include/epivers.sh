...
if [ -f epivers.h ]; then
	...
	if [ -z "$vc_version_num" ]; then
		...
		echo "No git functionality, attempting to use AVG_COMMIT_ID($vc_version_num)"
	else
		# git works
	...
	if [ -z "$vc_tag" ] && [ ! -z "$vc_version_num" ] ; then
		...
		output=$(git ls-remote --tags gitbsesw\@stband-git-1.and.broadcom.com:/refsw/shared.git | grep "$vc_version_num" | awk -F'/' '{print $NF}' | awk -F'^' '{print $1}')
		IFS=$'\n' out=(${output})
		unset IFS
	...
	if [ -z "$vc_tag" ] || [ -z "$vc_version_num" ]; then
     	vc_tag="No Published Tag for this Hash.STB7271_BRANCH_15_10"
     	vc_wl_ver="$vc_short_ver_date:NoTag"
     	echo "No Tag found: default to STB7271_BRANCH_15_10"
     	TAG="STB7271_BRANCH_15_10"
	else
