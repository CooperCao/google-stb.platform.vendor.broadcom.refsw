...
/* WPA */
static bool wpa_recv(hnd_dev_t *dev, void *ctx, struct lbuf *lb, uint16 ethtype);
...
static bool
wpa_recv(hnd_dev_t *dev, void *ctx, struct lbuf *lb, uint16 ethtype)
{
	uint8 *pkt = lb->data;
	int len = lb->len;
...
{
	/* not needed for RTE - all EAPOL messages are
