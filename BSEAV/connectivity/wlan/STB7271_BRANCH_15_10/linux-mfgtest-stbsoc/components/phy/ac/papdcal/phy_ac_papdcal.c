...
static int phy_ac_papd_tx_tone(phy_info_t *pi, int32 f_kHz, uint16 max_val, uint8 iqmode,
	uint8 mac_based, bool modify_bbmult);
static uint16 phy_ac_papd_gen_load_samples(phy_info_t *pi, int32 f_kHz, uint16 max_val,
	uint8 mac_based);
...
#define GAINCTRL_ITER 8
	...
	phy_ac_papd_tx_tone(pi, 2000, 186, 0, 0, FALSE);
	/* Binary search */
...
static int
phy_ac_papd_tx_tone(phy_info_t *pi, int32 f_kHz, uint16 max_val, uint8 iqmode,
	uint8 mac_based, bool modify_bbmult)
{
	uint16 num_samps;
	uint16 loops = 0xffff;
	...
	if (max_val == 0) {
		num_samps = 1;
	} else if ((num_samps = phy_ac_papd_gen_load_samples(pi, f_kHz, max_val, mac_based))
			   == 0) {
		return BCME_ERROR;
	}
...
static uint16
phy_ac_papd_gen_load_samples(phy_info_t *pi, int32 f_kHz, uint16 max_val, uint8 mac_based)
{
	uint8 phy_bw;
	uint16 num_samps, t;
	...
	/* allocate buffer */
	tone_buf = phy_malloc_fatal(pi, sizeof(*tone_buf) * phy_bw);
	...
	phy_mfree(pi, tone_buf, sizeof(*tone_buf) * phy_bw);
