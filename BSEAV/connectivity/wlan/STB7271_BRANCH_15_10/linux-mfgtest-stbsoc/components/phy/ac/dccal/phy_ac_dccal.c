...
phy_ac_dccal(phy_info_t *pi)
{
	...
	if ((ACMAJORREV_37(pi->pubpi->phy_rev) && ACMINORREV_1(pi)) ||
			ACMAJORREV_40(pi->pubpi->phy_rev)) {
		/* Perform DCOE CAL first. While doing DCOE, we disable the lo and enable
		   back during IDACC. Using the 2 STEP DCCAL, since it improve the DC estimation
		   quality, when leakage from LO is observed
		*/
		...
		if (ACMAJORREV_37(pi->pubpi->phy_rev)) {
			FOREACH_CORE(pi, core) {
				if (CHSPEC_IS2G(pi->radio_chanspec)) {
					MOD_RADIO_REG_20696(pi, RX2G_CFG1_OVR, core, ovr_rx2g_lo_en,
							1);
					MOD_RADIO_REG_20696(pi, RX2G_REG1, core, rx2g_lo_en, 0);
				} else {
					MOD_RADIO_REG_20696(pi, RX5G_CFG1_OVR, core,
							ovr_rx5g_mix_pu, 1);
					MOD_RADIO_REG_20696(pi, RX5G_REG5, core, rx5g_mix_pu, 0);
				}
			}
		}
	...
	if ((ACMAJORREV_37(pi->pubpi->phy_rev) && ACMINORREV_1(pi)) ||
		...
		if (ACMAJORREV_37(pi->pubpi->phy_rev) && ACMINORREV_1(pi)) {
			FOREACH_CORE(pi, core) {
				...
				// change this for 7271b0 and follow TCL - it differs from 4347
				// scaling is done properly in 20696 radio
				MOD_RADIO_REG_20696(pi, TIA_DCDAC_REG2, core, tia_dcdac_scale, 6);
			}
		} else {
			FOREACH_CORE(pi, core) {
				MOD_PHYREGCE(pi, dccal_control_15, core, dccal_sw_reset_h, 0);
...
phy_ac_load_gmap_tbl(phy_info_t *pi)
{
	...
	if (ACMAJORREV_37(pi->pubpi->phy_rev) && ACMINORREV_1(pi)) {
		uint16 gmap_table_r40_2g[] = {0x10a7, 0x10e7, 0x1127, 0x116b, 0x115b, 0x1143,
				0x1000};
		uint16 gmap_table_r40_5g[] = {0x112f, 0x116f, 0x11af, 0x11ef, 0x11df, 0x11c3,
				0x1000};
		uint16 tables[] = {AC2PHY_TBL_ID_IDAC_GMAP_TABLE0, AC2PHY_TBL_ID_IDAC_GMAP_TABLE1,
				AC2PHY_TBL_ID_IDAC_GMAP_TABLE2, AC2PHY_TBL_ID_IDAC_GMAP_TABLE3};
		uint8 i;
		uint16 *tablemap;


		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			tablemap = &gmap_table_r40_2g[0];
		} else {
			tablemap = &gmap_table_r40_5g[0];
		}
		for (i = 0; i < ARRAY_SIZE(tables); i++) {
			wlc_phy_table_write_acphy(pi, tables[i], 7, 0, 16, (void*)tablemap);
		}
