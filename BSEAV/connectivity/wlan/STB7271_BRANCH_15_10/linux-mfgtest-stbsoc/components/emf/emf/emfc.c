 ...
 *
 * $Id: emfc.c 669406 2016-11-09 11:02:40Z $
 */
#include <typedefs.h>
#include <bcmdefs.h>
#include <bcmendian.h>
...
uint32 emf_log_lvl = EMF_LOG_ERROR;
                        ...
                        uint32 dest_ip, bool rt_port)
{
	...
	for (ptr = emfc->iflist_head; ptr != NULL; ptr = ptr->next)
	{
		...
		if (proto == IP_PROT_IGMP)
		{
			/* need to copy mac for br, e.g. forward the igmp report from wifi to lan */
			if (!rt_port) {
				char *mac_src, *mac_dst;
				mac_src = (char *)PKTDATA(NULL, sdu) - ETH_HLEN;
				mac_dst = (char *)PKTDATA(NULL, sdu_clone) - ETH_HLEN;
				memcpy(mac_dst, mac_src, ETH_HLEN);
			}
		}
...
emfc_input(emfc_info_t *emfc, void *sdu, void *ifp, uint8 *iph, bool rt_port)
{
	...
	else
	{
		 ...
		 */
		/* For data frames received from LAN, push the ethernet header
...
emfc_rtport_add(emfc_info_t *emfc, void *ifp)
{
	...
	if (iflist == NULL) {
		OSL_UNLOCK(emfc->iflist_lock);
		EMF_ERROR("Invalid interface specified to rtport add\n");
		return (FAILURE);
	}
	iflist->rtport_ref++;
	OSL_UNLOCK(emfc->iflist_lock);
...
emfc_cfg_request_process(emfc_info_t *emfc, emf_cfg_request_t *cfg)
{
	...
	switch (cfg->command_id)
	{
		...
		case EMFCFG_CMD_LOG_LEVEL:
			if (cfg->oper_type == EMFCFG_OPER_TYPE_SET)
			{
				uint32 log_lvl = *(uint32 *)cfg->arg;
				if  (log_lvl  <=  EMF_LOG_ALL) {
					emf_log_lvl = log_lvl;
					cfg->status = EMFCFG_STATUS_SUCCESS;
				} else {
					EMF_DEBUG("Invalid log level %x. Expected to be <= %x\n",
						log_lvl,  EMF_LOG_ALL);
					cfg->status = EMFCFG_STATUS_FAILURE;
					cfg->size = sprintf(cfg->arg, "Invalid log level %x."
						"Expected to be <= %x \n", log_lvl,  EMF_LOG_ALL);
				}

			} else if (cfg->oper_type == EMFCFG_OPER_TYPE_GET)
			{
				*(uint32 *)cfg->arg = emf_log_lvl;
				cfg->status = EMFCFG_STATUS_SUCCESS;
			}
			else
			{
				EMF_DEBUG("Unknown opertype %d\n", cfg->oper_type);
				cfg->status = EMFCFG_STATUS_FAILURE;
				cfg->size = sprintf(cfg->arg, "Unknown opertype\n");
			}
			break;
		default:
