...
#ifdef WL_OCE
#define WLC_OCE_SCAN_EXTRA_TIME 10
#endif
...
BCMATTACHFN(wlc_scan_attach)(wlc_info_t *wlc, void *wl, osl_t *osh, uint unit)
{
	...
	scan_info->scan_pub->lq_chspec = 0;
	...
	void *cb, bool FIXED_SLOT)
{
	...
	if (OCE_ENAB(wlc->pub) && !SCAN_PASSIVE_5G_CHANSPEC(scan_info)) {
		ch_scan_time +=
			wlc_oce_get_probe_defer_time(wlc->oce) + WLC_OCE_SCAN_EXTRA_TIME;
	}
...
wlc_scan_sendprobe(scan_info_t *scan_info)
{
	...
	for (i = 0; i < n; i++) {
#if (defined(BCMINTERNAL) && defined(BCMDBG))|| (defined(BCMINTERNAL) && defined(WL_OCE_TB))
		/* borrow bits in the bssid field of wlc_scan_info_t struct to initiate
		 * probe request frames with the following address combinations:
		 *
		 * RA        BSSID     SSID
		 * --------- --------- --------
		 * BROADCAST BROADCAST BROADCAST
		 * BRORDCAST BROADCAST DIRECTED
		 * BROADCAST UNICAST   BROADCAST
		 * BORADCAST UNICAST   DIRECTED
		 * UNICAST   BROADCAST BROADCAST
		 * UNICAST   BROADCAST DIRECTED
		 * UNICAST   UNICAST   BROADCAST
		 * UNICAST   UNICAST   DIRECTED
		 *
		 * For testing purpose only.
		 */
		wlc_scan_info_t *wlc_scan_info = scan_info->scan_pub;
		struct ether_addr temp;
		/* broadcast bssid: broadcast RA and broadcast BSSID */
		if (ETHER_ISBCAST(&wlc_scan_info->bssid)) {
			da = &ether_bcast;
			bssid = &ether_bcast;
		}
		/* multicast bssid: broadcast RA and unicast BSSID */
		else if (ETHER_ISMULTI(&wlc_scan_info->bssid)) {
			da = &ether_bcast;
			bcopy(&wlc_scan_info->bssid, &temp, ETHER_ADDR_LEN);
			ETHER_SET_UNICAST(&temp);
			bssid = &temp;
		}
#ifndef WL_OCE_TB
		/* local bssid: unicast RA and broadcast BSSID */
		else if (ETHER_IS_LOCALADDR(&wlc_scan_info->bssid)) {
			bcopy(&wlc_scan_info->bssid, &temp, ETHER_ADDR_LEN);
			ETHER_CLR_LOCALADDR(&temp);
			da = &temp;
			bssid = &ether_bcast;
		}
#endif /* WL_OCE_TB */
		/* unicast bssid: unicast RA and unicast BSSID */
		else {
			da = &wlc_scan_info->bssid;
			bssid = &wlc_scan_info->bssid;
		}
#else /* !((defined(BCMINTERNAL) && defined(BCMDBG)) */
#ifdef WLFMC
		wlc_scan_info_t *wlc_scan_info = scan_info->scan_pub;
...
#endif /* WLFMC */
#endif /* (BCMINTERNAL && BCMDBG) || (BCMINTERNAL && WL_OCE_TB) */
