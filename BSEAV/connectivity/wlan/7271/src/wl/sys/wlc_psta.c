...
wlc_psta_watchdog(void *arg)
{
	...
	cfg = wlc_bsscfg_primary(psta->wlc);
                ...
                wlc_bsscfg_t *pcfg)
{
	...
	if (cfg == NULL) {
		...
		/* Allocate bsscfg */
		cfg = wlc_bsscfg_alloc(wlc, idx, &type, 0, ea);
		if (cfg == NULL)
			return BCME_NOMEM;
...
wlc_psta_pkt_alloc_copy(wlc_pub_t *pub, void *p)
{
	...
	if ((n = PKTGET(pub->osh, TXOFF + totlen, TRUE)) == NULL) {
		WL_ERROR(("wl%d: %s: PKTGET of length %d failed\n",
		          pub->unit, __FUNCTION__, (uint)(TXOFF + PKTLEN(pub->osh, p))));
		return NULL;
	}
		...
		if (bcmc && (!skip_proto_proc)) {
			...
			if ((n = PKTGET(psta->pub->osh, TXOFF + pull, TRUE)) == NULL) {
				WL_ERROR(("wl%d: %s: PKTGET headroom %d failed\n",
				          psta->pub->unit, __FUNCTION__, (uint)TXOFF));
				WLCNTINCR(psta->pub->_cnt->txnobuf);
				return BCME_NOMEM;
			}
...
wlc_psta_assoc_state_upd(void *ctx, bss_assoc_state_data_t *notif)
{
	...
	/* Clean up the proxy STAs before starting roam scan */
	         ...
	         notif->cfg == wlc_bsscfg_primary(wlc)) {
