	...
	actcb_fn_t act_cb, void *act_cb_arg)
{
	...
	if ((err = wlc_mac_request_entry(wlc, NULL, macreq)) == BCME_OK) {
		uint8 usage;
...
/*
 * ===================================================================
 * Note: these IE parser functions was moved from wlc_ie_misc_hndlrs.c
 * ===================================================================
 */
static int
...
/*
 * =====================================================================================
 * Note: code in this function was moved from wlc_scan_apply_scanresults() in wlc_scan.c
 * =====================================================================================
 */
void
	...
	struct dot11_management_header *hdr, uint8 *body, int body_len)
{
	...
	if (WLMESH_ENAB(wlc->pub) &&
	    ((scan->wlc_scan_cmn->bss_type == DOT11_BSSTYPE_MESH) &&
	     (!(cap & (DOT11_CAP_ESS | DOT11_CAP_IBSS))))) {
		/* find and validate the MESH ID IE */
		meshid_ie = bcm_parse_tlvs(body +
				DOT11_BCN_PRB_LEN, body_len - DOT11_BCN_PRB_LEN,
				DOT11_MNG_MESH_ID);
		if (meshid_ie) {
			mesh_match = wlc_scan_ssid_match(scan,
					meshid_ie->data, meshid_ie->len, filter);
		}
	}
#endif
	...
	if (((scan->wlc_scan_cmn->bss_type == DOT11_BSSTYPE_ANY ||
		...
		if (CCX_ENAB(wlc->pub) && wlc->rm_info->rm_state->ccx->scan_active &&
		    (wf_chspec_ctlchan(wrxh->rxhdr.lt80.RxChan) !=
			...
			WL_INFORM(("wl%d: %s: tossing bcn/prb resp "
				   ...
				   wlc->pub->unit, __FUNCTION__,
				   wf_chspec_ntoa_ex(wrxh->rxhdr.lt80.RxChan, chanbuf1),
		...
		if (!discard_prb_resp && (CCX_ENAB(wlc->pub) && wlc->rm_info->rm_state->ccx->scan_active &&
		...
		if (!discard_prb_resp && sui->scanresults_minrssi &&
			...
			/* XXX does not factor in off-channel beacon attenuation */
			WL_INFORM(("wl%d: %s: tossing bcn/prb resp "
		...
		if (!BSS && ISCAN_IN_PROGRESS(wlc)) {
			...
			chanspec = (CHSPEC_CHANNEL(wrxh->rxhdr.lt80.RxChan) <<
...
#ifdef BCMCCX
			if (CCX_ENAB(wlc->pub)) {
				uint bandunit = CHSPEC_BANDUNIT(wrxh->rxhdr.lt80.RxChan);
				uint bandtype = wlc->bandstate[bandunit]->bandtype;
				wlc_ccx_update_BSS(wlc->ccx, bandtype, BSS);
			}
#endif /* BCMCCX */
