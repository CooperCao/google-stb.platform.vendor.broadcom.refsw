...
wlc_eventq_register_ind_ext(wlc_eventq_t *eq, eventmsgs_ext_t* iovar_msg, uint8 *mask)
{
	...
	wlc_enable_probe_req(
		eq->wlc,
		PROBE_REQ_EVT_MASK,
		(wlc_eventq_test_ind(eq, WLC_E_PROBREQ_MSG_RX) || wlc_eventq_test_ind(eq, WLC_E_PROBREQ_MSG))? PROBE_REQ_EVT_MASK:0);
...
wlc_eventq_set_ind(wlc_eventq_t* eq, uint et, bool enab)
{
	...
	if (et == WLC_E_PROBREQ_MSG || et == WLC_E_PROBREQ_MSG_RX)
