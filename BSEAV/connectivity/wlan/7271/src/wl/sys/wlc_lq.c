...
uint8 BCMFASTPATH
wlc_lq_recv_snr_compute(wlc_info_t *wlc, wlc_d11rxhdr_t *wrxh, int8 rssi, int8 noise)
{
	...
	wlc_d11rxhdr_t *wrxh, bool ucast, rx_lq_samp_t *samp)
{
	...
	/* snr */
	snr = wlc_lq_recv_snr_compute(wlc, wrxh, rssi, lqi->noise);
...
wlc_lq_chanim_accum(wlc_info_t* wlc, chanspec_t chanspec, chanim_accum_t *acc)
{
	...
	if (last_cnt->timestamp)
		interval = cur_cnt.timestamp - last_cnt->timestamp;
...
static int8
wlc_lq_chanim_phy_noise(wlc_info_t *wlc, chanspec_t chanspec)
{
		...
		wlc_chanim_stats_t *cur_stats)
{
	...
	if (acc->stats_ms) {
		cur_stats->chanim_stats.chan_idle = 100 - (acc->ccastats_us[CCASTATS_TXDUR] + acc->ccastats_us[CCASTATS_INBSS] + acc->ccastats_us[CCASTATS_OBSS] + acc->ccastats_us[CCASTATS_NOCTG] + acc->ccastats_us[CCASTATS_NOPKT])/(acc->stats_ms * 10);
	} else {
		cur_stats->chanim_stats.chan_idle = 100;
	}
