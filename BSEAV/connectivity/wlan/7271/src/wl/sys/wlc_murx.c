...
BCMATTACHFN(wlc_murx_attach)(wlc_info_t *wlc)
{
	...
	cubby_params.fn_set = wlc_murx_bsscfg_config_set;
	cubby_params.config_size = WLC_MURX_BSSCFG_CONFIG_SIZE;
...
#ifdef WLCNT
	murx_stats = mu_bsscfg->murx_stats;
	BCM_REFERENCE(murx_stats);
#endif /* WLCNT */
...
#endif
	mu_bsscfg->murx_stats = murx_stats;
#endif /* defined(WLCNT) */
                    ...
                    uint8 *membership_status, uint8 *user_position)
{
	...
	ASSERT(mu_bsscfg->gid_info != NULL);
	/* Push group membership and user pos to hardware. */
