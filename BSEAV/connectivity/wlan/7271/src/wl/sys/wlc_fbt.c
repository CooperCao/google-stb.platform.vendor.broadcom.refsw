...
wlc_fbt_auth_calc_rsn_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	if (WLFBT_ENAB(fbt_priv->wlc->pub) == FALSE) {
		goto exit;
	}
...
void
wlc_fbt_set_akm(wlc_fbt_info_t *fbt_info, const wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
	int i;
	if ((cfg->WPA_auth & WPA2_AUTH_FT) && (bi->wpa2.flags & RSN_FLAGS_FBT)) {
		/* RSN_FLAGS_FBT set as FT AKM suites supported by target bss, nothing to do */
		for (i = 0; i < bi->wpa2.acount; i++) {
			if (IS_FBT_AKM(bi->wpa2.auth[i]))
				return;
		}
		/* RSN_FLAGS_FBT set as CCX Cap IE is present. Translate to FT akm suites */
		for (i = 0; i < bi->wpa2.acount; i++) {
			if (IS_WPA2_AKM(bi->wpa2.auth[i])) {
				if (bi->wpa2.auth[i] == RSN_AKM_UNSPECIFIED) {
					bi->wpa2.auth[i] = RSN_AKM_FBT_1X;
				}
				else if (bi->wpa2.auth[i] == RSN_AKM_PSK) {
					bi->wpa2.auth[i] = RSN_AKM_FBT_PSK;
				}
			}
		}
	...
	struct scb *scb, wlc_fbt_assoc_resp_t *fbt_assoc_resp)
{
	...
	rx_data_desc.hdr = &fbt_scb->hdr;
	rx_data_desc.plcp = NULL;
	rx_data_desc.wrxh = &fbt_scb->wrxh;
	rx_data_desc.body = fbt_scb->assoc_req_body;
	rx_data_desc.body_len = fbt_scb->assoc_req_len;

	wlc_ap_process_assocreq(wlc->ap, bsscfg, &rx_data_desc, scb,
		fbt_scb->short_preamble);
...
wlc_fbtap_cache_assoc_req(wlc_info_t *wlc, wlc_fbt_info_t *fbt_info,
	struct scb *scb, uint8 *body, int body_len, struct dot11_management_header *hdr, wlc_d11rxhdr_t *wrxh, bool short_preamble)
{
	wlc_fbt_scb_t *fbt_scb = FBT_SCB(fbt_info, scb);
...
#ifdef WL_OCE_AP
	bcopy(wrxh, &fbt_scb->wrxh, sizeof(wlc_d11rxhdr_t));
#endif
	return BCME_OK;
}
	...
	struct dot11_management_header *hdr, void *body, uint body_len)
{
	...
	fbt_rrb->len = htol16((uint16)body_len);
