...
#define TDLS_OFF_CHANNEL_TIME_BOOST	ON
#define TDLS_BASE_CHANNEL_TIME_BOOST	ON
#define TDLS_OFF_CHANNEL_TIME_EXTRA	(75000)
#define TDLS_BASE_CHANNEL_TIME_EXTRA	(25000)
...
bss_tdls_init(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	if (BSSCFG_IS_TDLS(cfg)) {
		...
		if ((err = wlc_bsscfg_rateset_init(wlc, cfg,
		...
		if (WLEXTSTA_ENAB(wlc->pub))
			cfg->wlcif->if_flags |= WLC_IF_PKT_80211;
#endif
	}
...
#if TDLS_OFF_CHANNEL_TIME_BOOST
		if ((cfg->tdls->chsw_req_enabled) &&
			(req_param.duration <
			(scb_tdls->parent->current_bss->beacon_period << 10)) &&
			(CHSPEC_WLCBANDUNIT(scb_tdls->cur_chanspec) !=
			CHSPEC_WLCBANDUNIT(scb_tdls->target_chanspec))) {
			req_param.duration += TDLS_OFF_CHANNEL_TIME_EXTRA;
		}
#endif /* #if TDLS_OFF_CHANNEL_TIME_BOOST */
...
wlc_tdls_msch_cb(void* handler_ctxt, wlc_msch_cb_info_t *cb_info)
{
	...
	if (type & MSCH_CT_ON_CHAN) {
		...
		if (scb_tdls->cur_chanspec != cb_info->chanspec) {
			...
			if (CHSPEC_WLCBANDUNIT(scb_tdls->cur_chanspec) !=
				CHSPEC_WLCBANDUNIT(cb_info->chanspec))
				band_chg = TRUE;
			if (CHSPEC_BW(scb_tdls->cur_chanspec) !=
				CHSPEC_BW(cb_info->chanspec))
				bw_chg = TRUE;

			bandunit = CHSPEC_WLCBANDUNIT(cb_info->chanspec);
			if (band_chg) {
...
wlc_tdls_create_oper_mode(wlc_info_t *wlc, struct scb *scb, wlc_bsscfg_t *cfg)
{
	uint8 width;
	wlcband_t *band;
	uint8 nss = MIN(wlc->stf->rxstreams, VHT_CAP_MCS_MAP_NSS_MAX);
	...
	return DOT11_OPER_MODE(0, nss, width);
}
...
wlc_tdls_bsscfg_create(wlc_info_t *wlc, wlc_bsscfg_t *parent, bool initiator)
{
	...
	if ((bsscfg = wlc_bsscfg_alloc(wlc, idx, &type,
			WLC_BSSCFG_NOIF | WLC_BSSCFG_NOBCMC | WLC_BSSCFG_PSINFO,
...
#ifdef EXT_STA
	if (WLEXTSTA_ENAB(wlc->pub)) {
		uint packet_filter = WLC_ACCEPT_DIRECTED;

		if (wlc_iovar_op(wlc, "packet_filter", NULL, 0,
		                 &packet_filter, sizeof(packet_filter),
		                 IOV_SET, bsscfg->wlcif) != BCME_OK) {
			goto fail;
		}
	}
#endif
...
wlc_tdls_process_setup_req(tdls_info_t *tdls, struct scb *scb, struct wlc_frminfo *f, uint offset)
{
	...
	wlc_tdls_parse_setup_ies(tdls, &parent->BSSID, pdata, plen, bi, TDLS_SETUP_REQ);
...
wlc_tdls_process_setup_resp(tdls_info_t *tdls, struct scb *scb, struct wlc_frminfo *f, uint offset)
{
	...
	wlc_tdls_parse_setup_ies(tdls, &parent->BSSID, pdata, plen, bi, TDLS_SETUP_RESP);
	scb_peer->cap = cap;
...
wlc_tdls_process_setup_cfm(tdls_info_t *tdls, struct scb *scb, struct wlc_frminfo *f, uint offset)
{
	...
	/* init bi->chanspec */
	bi->chanspec = scb_peer->bsscfg->current_bss->chanspec;
	wlc_tdls_parse_setup_ies(tdls, &parent->BSSID, pdata, plen, bi, TDLS_SETUP_CONFIRM);
...
retry:
	...
	if (!tdls->up)
		return DOT11_SC_DECLINED;
	if ((p = wlc_frame_get_action(wlc, f->sa, &parent->cur_etheraddr,
		&parent->BSSID, TDLS_ACTION_FRAME_DEFAULT_LEN, &data, DOT11_ACTION_CAT_PUBLIC))
		== NULL) {
		return BCME_NOMEM;
	}
	...
	chanspec_t target_chanspec)
{
	...
	chan_num = WLC_RX_CHANNEL(wlc->pub->corerev, rxhdr);
...
wlc_tdls_do_chanswitch(tdls_info_t *tdls, wlc_bsscfg_t *cfg, chanspec_t newchspec)
{
	...
	if (newchspec != scb_tdls->base_chanspec) {
		...
		uint duration = 0;
		if (scb_tdls->switch_time > (delta + wlc_proc_time_us(wlc))) {
			duration = (scb_tdls->switch_time - delta - wlc_proc_time_us(wlc))/1000;
		}
		wlc_tdls_del_chsw_timer(wlc, scb_tdls);
		scb_tdls->chsw_flags |= TDLS_SCB_CHSW_MSCH_REGISTER_PRE_SLOT_END;
...
wlc_tdls_post_chsw_base_chan(tdls_info_t *tdls, struct scb *scb)
{
	...
	if (scb_tdls->flags & TDLS_SCB_TEARDOWN_PENDING) {
		...
		duration = wlc_tdls_check_next_tbtt(tdls, SCB_BSSCFG(scb), FALSE);
#if TDLS_BASE_CHANNEL_TIME_BOOST
				if ((SCB_BSSCFG(scb)->tdls->chsw_req_enabled) &&
					((duration+TDLS_BASE_CHANNEL_TIME_EXTRA) <
					(scb_tdls->parent->current_bss->beacon_period << 10)) &&
					(CHSPEC_WLCBANDUNIT(scb_tdls->cur_chanspec) !=
					CHSPEC_WLCBANDUNIT(scb_tdls->target_chanspec))) {
					duration += TDLS_BASE_CHANNEL_TIME_EXTRA;
				}
#endif /* #if TDLS_BASE_CHANNEL_TIME_BOOST */
		wl_add_timer(tdls->wlc->wl, scb_tdls->peer_addr->chsw_timer,
			(duration/1000), FALSE);
	}
}
...
wlc_tdls_watchdog(void *hdl)
{
	...
	for (i = 0; i < (tdls->tdls_cmn->max_sessions); i++) {
		...
		if ((tdls->tdls_cmn->auto_op) && (dla->flags & TDLS_LOOKASIDE_INUSE)) {
