...
#include <wlc_bsscfg_viel.h>
		...
		(defined(WL_MBO) && !defined(WL_MBO_DISABLED)))
#include <wlc_wnm.h>
...
{
	wlc_rrm_info_t *rrmi = (wlc_rrm_info_t *)ctx;
	wlc_info_t *wlc = rrmi->wlc;

	/* if beacon is already having Advertisement protocol element IE
	 * enabled by hotspot or any other APP, return
	 */
	if (wlc_vndr_ie_find_by_type(wlc->vieli, data->cfg, DOT11_MNG_ADVERTISEMENT_ID)) {
		return 0;
	}
