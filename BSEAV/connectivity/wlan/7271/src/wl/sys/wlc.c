...
#ifdef WL_OCE
		if (OCE_ENAB(wlc->pub) && BAND_5G(wlc->band->bandtype)
			&& wlc_force_bcn_rspec(wlc) == LEGACY_RSPEC(WLC_RATE_5M5)) {
			wlc_set_force_bcn_rspec(wlc, 0);
		}
#endif
...
wlc_bss_set_bcn_rspec(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wlc_bss_info_t *bss)
{
	...
	ASSERT(wlc_valid_rate(wlc, rspec,
			    CHSPEC_IS2G(bss->chanspec) ? WLC_BAND_2G : WLC_BAND_5G,
			    TRUE));
