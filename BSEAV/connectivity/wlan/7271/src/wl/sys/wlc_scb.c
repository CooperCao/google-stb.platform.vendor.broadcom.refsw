...
wlc_scbfree(wlc_info_t *wlc, struct scb *scbd)
{
	...
	if (BSSCFG_AP(scbd->bsscfg)) {
		if (scbd->support_gbl_rclass) {
			scbd->bsscfg->scb_with_gbl_rclass--;
		}
		/* update bsscfg's global/non global support */
		wlc_bsscfg_update_rclass(wlc, scbd->bsscfg);
	}
#endif /* WL_SUPPORT_GLOBAL_REGCLASS */
