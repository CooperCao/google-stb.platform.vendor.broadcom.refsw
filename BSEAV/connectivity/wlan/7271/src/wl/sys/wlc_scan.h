...
#define SCAN_STATE_PRB_DEFERRED	(1 << 16)
...
struct wlc_scan_info {
	...
	chanspec_t	lq_chspec;		/* lq last chanspec */
};
...
typedef wl_join_scan_params_t scan_cfg_param_t;

typedef struct wlc_scan_req_params {
	int bss_type;
	int nssid;
	wlc_ssid_t *ssid;
	const scan_cfg_param_t *scan_cfg_params;
	...
	void *act_cb_arg;
	int bandinfo;
	chanspec_t band_chanspec_start;
} wlc_scanreq_params_t;
...
extern bool wlc_scan_ssid_match(wlc_scan_info_t *wlc_scan_info, uint8 *ssid_buf, uint8 ssid_len, bool filter);
