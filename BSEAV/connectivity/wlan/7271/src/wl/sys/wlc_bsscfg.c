...
{
	uint8 assoc_sta = 0;
	uint8 sta_nt_support_gbl_rclass = 0;
	uint8 cur_rclass;

	/* update bsscfg's global/non global support */
	assoc_sta = wlc_bss_assocscb_getcnt(wlc, bsscfg);

	sta_nt_support_gbl_rclass = assoc_sta - bsscfg->scb_with_gbl_rclass;
	 ...
	 * operation
	 */
	if ((!sta_nt_support_gbl_rclass) && (cur_rclass != BCMWIFI_RCLASS_TYPE_GBL)) {
		wlc_update_rcinfo(wlc->cmi, TRUE);
		wlc_channel_set_cur_rclass(wlc, BCMWIFI_RCLASS_TYPE_GBL);
		return;
	}

	if ((sta_nt_support_gbl_rclass) && (cur_rclass != BCMWIFI_RCLASS_TYPE_COUNTRY)) {
