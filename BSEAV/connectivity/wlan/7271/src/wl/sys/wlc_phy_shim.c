...
#define PHY_SCAN_IN_PROGRESS(scan_info) ((scan_info) && (scan_info)->in_progress)
...
bool
wlapi_is_lq_5g_scan(wlc_phy_shim_info_t *physhim)
{
	chanspec_t chspec = 0;
	bool ret = false;
	wlc_info_t *wlc = (wlc_info_t *)physhim->wlc;

	if (PHY_SCAN_IN_PROGRESS(wlc->scan)) {
		chspec = wlc->scan->lq_chspec;
		if (CHSPEC_IS5G(chspec)) {
			WL_COEX(("lq in 5G scan %d\n", chspec));
			ret = true;
		}
	}
