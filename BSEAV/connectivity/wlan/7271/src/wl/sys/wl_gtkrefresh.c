...
wlc_gtk_detach(wlc_sup_info_t *sup_info)
{
	...
	WL_TRACE(("wl%d: %s\n", UNIT(sup_info), __FUNCTION__));
	wlc_module_unregister(sup_info->pub, "gtkref", sup_info);
	...
	switch (actionid) {
		case IOV_SVAL(IOV_GTK_KEYINFO):
			...
			if (bsscfg->wsec & AES_ENABLED)
				sup_bss->wpa->ucipher = CRYPTO_ALGO_AES_CCM;
			else if (bsscfg->wsec & TKIP_ENABLED)
				sup_bss->wpa->ucipher = CRYPTO_ALGO_TKIP;
			err = BCME_OK;
			break;
...
wlc_gtk_refresh(wlc_info_t *wlc, struct scb *scb, struct wlc_frminfo *f)
{
	...
	if (!bsscfg || !(wlc->idsup || wlc->gtkref) ||
	...
	psup_bss = SUP_BSSCFG_CUBBY_LOC(wlc->gtkref, bsscfg);
	...
	if (!f->ismulti && !wepbit && !FC_SUBTYPE_ANY_NULL(f->subtype)) {
		...
		if (lsh->dsap == 0xaa && lsh->ssap == 0xaa && lsh->ctl == 0x03 &&
			...
			if (ntoh16(lsh->type) == ETHER_TYPE_802_1X) {
				...
				sup_bss = SUP_BSSCFG_CUBBY(wlc->gtkref, bsscfg);
...
#if defined(BCMULP) || defined(STB_SOC_WIFI)
