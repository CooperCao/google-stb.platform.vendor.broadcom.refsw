...
#ifndef ESP_IE_MAX_BUILD_CBS
	#define ESP_IE_MAX_BUILD_CBS 2
#endif /* ESP_IE_MAX_BUILD_CBS */
#ifndef ESP_IE_MAX_PARSE_CBS
	#define ESP_IE_MAX_PARSE_CBS 4
#endif /* ESP_IE_MAX_PARSE_CBS */
...
BCMATTACHFN(wlc_esp_attach)(wlc_info_t *wlc)
{
	...
	esp = obj_registry_get(wlc->objr, OBJR_ESP_IE_INFO);
	if (esp == NULL) {
		...
		obj_registry_set(wlc->objr, OBJR_ESP_IE_INFO, esp);
		esp->cmn = wlc->cmn;
		esp->osh = wlc->osh;
		esp->objr = wlc->objr;
		esp->pub = wlc->pub;
	...
	(void)obj_registry_ref(wlc->objr, OBJR_ESP_IE_INFO);
...
BCMATTACHFN(wlc_esp_detach)(wlc_esp_info_t* esp)
{
	...
	if (esp && obj_registry_unref(esp->objr, OBJR_ESP_IE_INFO) == 0) {
		obj_registry_set(esp->objr, OBJR_ESP_IE_INFO, NULL);
