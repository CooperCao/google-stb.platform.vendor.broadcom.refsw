...
static void wlc_bsscfg_update_gbl_rclass_support_scb(wlc_info_t* wlc, wlc_bsscfg_t* bsscfg,
	struct scb* scb);
	...
	wlc_rx_data_desc_t *rx_data_desc, struct scb *scb, bool short_preamble)
{
	...
	*/
	...
	if ((wlc_mbo_reject_assoc_req(wlc, bsscfg))) {
		status = DOT11_SC_ASSOC_BUSY_FAIL;
		goto done;
	}
...
wlc_restart_ap(wlc_ap_info_t *ap)
{
	...
	if (APSTA_ENAB(wlc->pub) &&
		(MAP_ENAB(wlc->cfg) || DWDS_ENAB(wlc->cfg) || (wlc->cfg->_psta)) &&
		(!(wlc->cfg->up))) {
...
#ifdef WL_GLOBAL_RCLASS
static void
wlc_bsscfg_update_gbl_rclass_support_scb(wlc_info_t* wlc, wlc_bsscfg_t* bsscfg, struct scb* scb)
{
	ASSERT(scb != NULL);
