...
wlc_asdb_mcs_nss_get(wlc_bsscfg_t *bsscfg, uint32 *mcs, uint32 *nss, uint8 *isvht)
{
	...
	*mcs = *nss = MCS_SPEC_INVALID;
	...
	if ((wl_rspec & WL_RSPEC_ENCODING_MASK) == WL_RSPEC_ENCODE_RATE) {
		/* legacy case (if required) to be handled here */
	} else if ((wl_rspec & WL_RSPEC_ENCODING_MASK) == WL_RSPEC_ENCODE_HT) {
		*mcs = (wl_rspec & WL_RSPEC_RATE_MASK);
	} else if ((wl_rspec & WL_RSPEC_ENCODING_MASK) == WL_RSPEC_ENCODE_VHT) {
		*mcs = (wl_rspec & WL_RSPEC_VHT_MCS_MASK);
		*nss = (wl_rspec & WL_RSPEC_VHT_NSS_MASK) >> WL_RSPEC_VHT_NSS_SHIFT;
		*isvht = TRUE;
	}
}
