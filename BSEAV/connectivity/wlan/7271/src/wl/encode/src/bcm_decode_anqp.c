...
#if 0
	WL_PRPKT("packet for ANQP venue name duple decoding",
		bcm_decode_current_ptr(pkt), bcm_decode_remaining(pkt));
...
{
#if 0
	WL_PRPKT("packet for ANQP network authentication unit decoding",
		bcm_decode_current_ptr(pkt), bcm_decode_remaining(pkt));
...
{
#if 0
	WL_PRPKT("packet for ANQP OI duple decoding",
		bcm_decode_current_ptr(pkt), bcm_decode_remaining(pkt));
...
{
#if 0
	WL_PRPKT("packet for authentication decoding",
		bcm_decode_current_ptr(pkt), bcm_decode_remaining(pkt));
...
#if 0
	WL_PRPKT("packet for EAP decoding",
		bcm_decode_current_ptr(pkt), bcm_decode_remaining(pkt));
...
#if 0
	WL_PRPKT("packet for NAI realm data decoding",
		bcm_decode_current_ptr(pkt), bcm_decode_remaining(pkt));
...
#if 0
	if (data->realmLen > BCM_DECODE_ANQP_MAX_REALM_LENGTH) {
		WL_ERROR(("realm length %d > %d\n",
			data->realmLen, BCM_DECODE_ANQP_MAX_REALM_LENGTH));
		return FALSE;
	}
#endif	/* 0 */
...
#if 0
	WL_PRPKT("packet for PLMN decoding",
		bcm_decode_current_ptr(pkt), bcm_decode_remaining(pkt));
...
{
#if 0
	WL_PRPKT("packet for domain name decoding",
		bcm_decode_current_ptr(pkt), bcm_decode_remaining(pkt));
