...
#if 0
	if (ieCount > 0)
		bcm_decode_hspot_anqp_print(hspot);
#endif
...
#if 0
	WL_PRPKT("packet for hotspot operator name duple decoding",
		bcm_decode_current_ptr(pkt), bcm_decode_remaining(pkt));

	memset(duple, 0, sizeof(*duple));
#endif
...
{
#if 0
	WL_PRPKT("packet for hotspot protocol port tuple decoding",
		bcm_decode_current_ptr(pkt), bcm_decode_remaining(pkt));

	memset(protoPort, 0, sizeof(*protoPort));
#endif
...
{
#if 0
	WL_PRPKT("packet for hotspot NAI home realm data decoding",
		bcm_decode_current_ptr(pkt), bcm_decode_remaining(pkt));
...
{
#if 0
	WL_PRPKT("packet for hotspot icon metadata decoding",
		bcm_decode_remaining(pkt), bcm_decode_current_ptr(pkt));
...
#if 0
	if (icon->binaryLength > BCM_DECODE_HSPOT_ANQP_MAX_ICON_BINARY_SIZE) {
		WL_ERROR(("icon binary exceeds buffer %d > %d\n",
			icon->binaryLength, BCM_DECODE_HSPOT_ANQP_MAX_ICON_BINARY_SIZE));
		return FALSE;
	}
#endif	/* 0 */
