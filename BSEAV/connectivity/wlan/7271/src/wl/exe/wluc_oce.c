 ...
 *
 * $Id: wluc_fils.c 684714 2017-02-14 07:45:24Z abhik $
 */
#include <bcmendian.h>
...
static subcmd_handler_t wl_oce_sub_cmd_cu_trigger;
static subcmd_handler_t wl_oce_sub_cmd_rej_retry_delay;
/* help handlers */
...
static help_handler_t wl_oce_cu_trigger_help_fn;
static help_handler_t wl_oce_rej_retry_delay_help_fn;
...
static const wl_oce_sub_cmd_t oce_subcmd_lists[] = {
	...
	{ "cu_trigger", 0x1, WL_OCE_CMD_CU_TRIGGER,
	IOVT_BUFFER, wl_oce_sub_cmd_cu_trigger,
	wl_oce_cu_trigger_help_fn
	},
	{ "rej_retry_delay", 0x1, WL_OCE_CMD_REJ_RETRY_DELAY,
	IOVT_BUFFER, wl_oce_sub_cmd_rej_retry_delay,
	wl_oce_rej_retry_delay_help_fn
	},
...
static void
wl_oce_cu_trigger_help_fn(void)
{
	printf("Roaming CU trigger:\n");
	printf("Set: wl oce cu_trigger <value>\n");
	printf("Get: wl oce cu_trigger\n");
}

static void
wl_oce_rej_retry_delay_help_fn(void)
{
	printf("RSSI based Assoc rejection retry delay:\n");
	printf("Set: wl oce rej_retry_delay <value>\n");
	printf("Get: wl oce rej_retry_delay\n");
}
...
static int
wl_oce_cu_trigger_cbfn(void *ctx, const uint8 *data, uint16 type, uint16 len)
{
	UNUSED_PARAMETER(ctx);
	UNUSED_PARAMETER(len);
	...
	switch (type) {
		case WL_OCE_XTLV_CU_TRIGGER:
			printf("Roaming CU Trigger : %d\n", (*data));
			break;
		default:
			printf("%s: Unknown tlv %u\n", __FUNCTION__, type);
	}
...
static int
wl_oce_rej_retry_delay_cbfn(void *ctx, const uint8 *data, uint16 type, uint16 len)
{
	UNUSED_PARAMETER(ctx);
	UNUSED_PARAMETER(len);
	if (data == NULL) {
	...
	switch (type) {
		case WL_OCE_XTLV_REJ_RETRY_DELAY:
			printf("RSSI Rejection Retry Delay: %d sec\n", *data);
			break;
		default:
			printf("%s: Unknown tlv %u\n", __FUNCTION__, type);
	}
...
wl_oce_sub_cmd_fd_tx_duration(void *wl, const wl_oce_sub_cmd_t *cmd, char **argv)
{
	...
	if (*argv == NULL) {
		...
		uint32 fd_tx_duration = strtoul(val_p, NULL, 0);
		/* TBD: validation */
		ret = bcm_pack_xtlv_entry(&pxtlv, &buflen, WL_OCE_XTLV_FD_TX_DURATION,
				sizeof(fd_tx_duration),(void*) &fd_tx_duration, BCM_XTLV_OPTION_ALIGN32);
...
static int
wl_oce_sub_cmd_cu_trigger(void *wl, const wl_oce_sub_cmd_t *cmd, char **argv)
{
	int ret = BCME_OK;
	bcm_iov_buf_t *iov_buf = NULL;
	uint8 *pxtlv = NULL;
	...
	/* only set */
	if (*argv == NULL) {
		ret = wl_oce_get_iov_resp(wl, cmd, wl_oce_cu_trigger_cbfn);
	} else {
		iov_buf = (bcm_iov_buf_t *)calloc(1, WLC_IOCTL_MEDLEN);
		if (iov_buf == NULL) {
			return BCME_NOMEM;
		...
		uint8 cu_trigger = strtoul(val_p, NULL, 0);
		if ((int8)cu_trigger < 0) {
			wl_oce_usage(WL_OCE_CMD_CU_TRIGGER);
			ret = BCME_USAGE_ERROR;
			goto fail;
		}

		/* TBD: validation */
		ret = bcm_pack_xtlv_entry(&pxtlv, &buflen, WL_OCE_XTLV_CU_TRIGGER,
				sizeof(cu_trigger), &cu_trigger, BCM_XTLV_OPTION_ALIGN32);
		if (ret != BCME_OK) {
			goto fail;
		}
		iov_buf->len = buflen_start - buflen;
...
static int
wl_oce_sub_cmd_rej_retry_delay(void *wl, const wl_oce_sub_cmd_t *cmd, char **argv)
{
	int ret = BCME_OK;
	bcm_iov_buf_t *iov_buf = NULL;
	uint8 *pxtlv = NULL;
	...
	/* only set */
	if (*argv == NULL) {
		ret = wl_oce_get_iov_resp(wl, cmd, wl_oce_rej_retry_delay_cbfn);
	} else {
		iov_buf = (bcm_iov_buf_t *)calloc(1, WLC_IOCTL_MEDLEN);
		if (iov_buf == NULL) {
			return BCME_NOMEM;
		...
		if (!val_p || *val_p == '-') {
			fprintf(stderr, "%s: No value following %s\n", __FUNCTION__, param);
			wl_oce_usage(WL_OCE_CMD_REJ_RETRY_DELAY);
			ret = BCME_USAGE_ERROR;
			goto fail;
		}
		uint8 rej_retry_delay = strtoul(val_p, NULL, 0);

		ret = bcm_pack_xtlv_entry(&pxtlv, &buflen, WL_OCE_XTLV_REJ_RETRY_DELAY,
				sizeof(rej_retry_delay), &rej_retry_delay, BCM_XTLV_OPTION_ALIGN32);
		if (ret != BCME_OK) {
			goto fail;
		}
		iov_buf->len = buflen_start - buflen;
