...
#if 0 /* reduce fragement size as some vendors cannot receive large fragments */
...
#if 0 /* Uncomment this to test p2po_gas_config iovar max_retries case 5 */
		static uint8 forced_retry_count = 0;
		if (++forced_retry_count < 7) {
			WL_P2PO(("%s: forced retry %u: drop actframe complete\n",
				__FUNCTION__, forced_retry_count));
			return;
		}
#endif /* Uncomment this to test p2po_gas_config iovar max_retries case 5 */
...
#if 0 /* Uncomment this to test p2po_gas_config iovar max_retries case 4 */
		static uint8 forced_retry_count = 0;
		++forced_retry_count;
		if ((forced_retry_count & 7) < 5) {
			WL_P2PO(("%s: forced retry %u: drop actframe rx\n",
				__FUNCTION__, forced_retry_count));
			return;
		}
#endif /* Uncomment this to test p2po_gas_config iovar max_retries case 4 */
...
#if 0	/* packet encoded directly into buffer */
	gas->tx.length = length;
	memcpy(gas->tx.data, buffer, gas->tx.length);
#endif
...
#if 0 /* Uncomment this to test p2po_gas_config iovar max_retries case 2 */
	{
		static uint8 forced_retry_count = 0;
		if (++forced_retry_count < 3) {
			WL_P2PO(("%s: forced retry %u: drop ANQP req tx\n",
				__FUNCTION__, forced_retry_count));
			memset(&gas->peer, 0, sizeof(gas->peer));
		}
	}
#endif /* Uncomment this to test p2po_gas_config iovar max_retries case 2 */
	...
	uint8 advertisementProtocol, uint16 rspLen, uint8 *rsp)
{
	...
	WL_NONE(("%s: sz=%d rspLen=%u rsp=%p tx.len=%u\n", __FUNCTION__,
		(int)sizeof(gas->tx.data), rspLen, rsp, gas->tx.length));
...
#if 0 /* Uncomment this to test p2po_gas_config iovar max_retries case 3 */
	{
		static uint8 forced_retry_count = 0;
		if (++forced_retry_count < 3) {
			static struct ether_addr null_peer = { {0, 0, 0, 0, 0, 0} };
			WL_P2PO(("%s: forced retry %u: drop comeback req tx\n",
				__FUNCTION__, forced_retry_count));
			tx_action_frame(gas, gas->channel, responseTimeout,
				&null_peer, &null_peer, GAS_COMEBACK_REQUEST_ACTION_FRAME,
				gas->tx.length, gas->tx.data);
			return;
		}
	}
#endif /* Uncomment this to test p2po_gas_config iovar max_retries case 3 */
...
#if 0
					/* wait small comeback delay - yield to others */
					tmrStart(gas->comebackDelayTimer, 250, FALSE);
					change_state(gas, STATE_WAIT_COMEBACK_DELAY);
#else
