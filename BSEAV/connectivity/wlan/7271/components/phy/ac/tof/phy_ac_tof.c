...
BCMATTACHFN(phy_ac_tof_register_impl)(phy_info_t *pi, phy_ac_info_t *aci, phy_tof_info_t *ti)
{
	...
	uint32 rtx_size = FTM_TPK_RI_PHY_LEN * sizeof(uint8);
	uint32 rrx_size = FTM_TPK_RR_PHY_LEN * sizeof(uint8);
...
static void phy_ac_tof_cross_corr_sum(const cint32* In, const int32 len, cint32* accum)
{
	...
	cint32 mult = { 0, 0 };
	for (i = 1; i < len; i++) {
		mult_cint32_cfixed((In + i), (In + i - 1), 0, &mult, TRUE);
		add_cint32(&mult, accum, accum);
	}
}
	...
	if (num_s != 0) {
		for (i = 0; i < len; i++) {
			...
			for (j = 1; j < band_length[i]; j++) {
				mult_cint32_cfixed((pTmp + j), (pTmp + j - 1), 0, &mult, TRUE);
				idx = i*(band_length[i]-1) + j - 1;
				theta[idx] = -cordic_ptr(&mult);
				if (theta[idx] < min) {
					min = theta[idx];
				}
				if (theta[idx] > max) {
	...
	int32 len, bool gd_corr)
{
	...
	cint32* tmp = &mult;
	for (i = 0; i < len; i++) {
		expj.i = cos_tbl(i*gd_theta + ph_offset);
		expj.q = sin_tbl(-(i*gd_theta + ph_offset));
		if (gd_corr) {
			tmp = In + i;
		}
		mult_cint32_cfixed((In + i), &expj, K_TOF_TWDL_SFT, tmp, FALSE);
		add_cint32(tmp, &accum, &accum);
	}
	return cordic_ptr(&accum);
}
	...
	const uint16* sc_idx_arr, cint32* chan, cint32* mf_out)
{
	...
	for (i = 0; i < len; i++) {
		for (j = 0; j < band_length[i]; j++) {
			...
			ph_corr.i = cos_tbl(ph_corr_th);
			ph_corr.q = sin_tbl(ph_corr_th);
			mult_cint32_cfixed((chan + idx), &ph_corr, K_TOF_TWDL_SFT,
				(chan + idx), FALSE);
		}
		band_offset += band_length[i];
	}
	...
	wl_proxd_snr_t *snr)
{
	...
	* (+1, -1).
	*/
	power_cint32_arr(mf_out, sc_idx_arr, len, &sig_pwr);
	sig_pwr = sqrt_int(sig_pwr);
	power_cint32_arr(chan, sc_idx_arr, len, &chan_pwr);
	...
	*/
	...
	for (k = 0; k < len; k++) {
		i = sc_idx_arr[k];
		mult_cint32_cfixed((mf_out + i), &sig_sc_cmplx, 0, &sig_val, FALSE);
		mult_cint32_cfixed((chan + i), &chan_sc_cmplx, 0, &chan_out, FALSE);
		/*
		...
		* easier
		*/
		mult_cint32_cfixed((mf_out + i), (chan + i), 0, &sig_val, TRUE);
...
phy_ac_tof_setup_rf_bundle(phy_info_t *pi)
{
	...
	afe_ctrl_flash17lvl = ((read_val[0] >> TOF_RFSEQ_AFE_CTRL_FLASH17LVL_SHIFT)
			& TOF_RFSEQ_AFE_IQADC_FLASHHSPD_SHIFT);
	...
	wlc_phy_table_read_acphy(pi, ACPHY_TBL_ID_RFSEQBUNDLE, 1, TOF_RFBNDL_ADC_OFFSET_38,
			TOF_RFSEQ_BUNDLE_SIZE, (void*)bundle_offs_38);
	wlc_phy_table_read_acphy(pi, ACPHY_TBL_ID_RFSEQBUNDLE, 1, TOF_RFBNDL_ADC_OFFSET_39,
			TOF_RFSEQ_BUNDLE_SIZE, (void*)bundle_offs_39);

	PHY_INFORM(("%s: Readval = 0x%x\n", __FUNCTION__, read_val[0]));
	PHY_INFORM(("RFSeq offset 38 Before = 0x%04x%04x%04x\n",
			bundle_offs_38[2], bundle_offs_38[1], bundle_offs_38[0]));
	PHY_INFORM(("RFSeq offset 39 Before = 0x%04x%04x%04x\n",
			bundle_offs_39[2], bundle_offs_39[1], bundle_offs_39[0]));
	...
	bundle_offs_38[0] &= ~TOF_RFBNDL_ADC_MASK_LB;
	bundle_offs_38[1] &= ~TOF_RFBNDL_ADC_MASK_HB;

	bundle_offs_39[0] &= ~TOF_RFBNDL_ADC_MASK_LB;
	bundle_offs_39[1] &= ~TOF_RFBNDL_ADC_MASK_HB;

	PHY_INFORM(("RFSeq offset 38 After Mask = 0x%04x%04x%04x\n", bundle_offs_38[2],
			bundle_offs_38[1], bundle_offs_38[0]));
	PHY_INFORM(("RFSeq offset 39 After Mask = 0x%04x%04x%04x\n", bundle_offs_39[2],
			bundle_offs_39[1], bundle_offs_39[0]));
...
static int
phy_ac_tof_setup_core_params(phy_info_t *pi, uint8 core)
{
	phy_ac_tof_info_t *tofi = (phy_ac_tof_info_t *)pi->u.pi_acphy->tofi;
	uint16 *pSrc, *pEnd;
	...
	if (TINY_RADIO(pi)) {
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			pSrc = (uint16 *)k_tof_seq_tiny_tbls_2G;
...
{
	/*
	 * 1) When firstCall = FALSE, enter and tx are ignored.
	 * 2) isInitiator value is ignored if TOF_TEST_TONE is not defined or if loadSPB is FALSE.
	*/
	...
	if (firstCall) {
		if (phy_ac_tof_seq_params(tofi) != BCME_OK)
			return BCME_ERROR;
	...
	phy_ac_tof_setup_core_params(pi, core);
	...
	if (arg >= 255) {
		...
		if (ACMAJORREV_1(pi->pubpi->phy_rev) || ACMAJORREV_3(pi->pubpi->phy_rev)) {
			n = (((int)R_REG(pi->sh->osh,
				&pi->regs->PHYREF_SampleCollectCurPtr) - p) / 3) << 2;
		} else
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_1) || defined(PHY_ACMAJORREV_3))) */
		{
	...
	bool seq_en, int core)
{
	...
	if (change) {
		...
		if (seq_en) {
			...
			retval = phy_ac_tof_seq_setup(pi, enter, tx, core, TRUE, loadSPB, TRUE);
			if (!suspend) {
				wlapi_suspend_mac_and_wait(pi->sh->physhim);
			}
			if (enter) {
	...
	*/
	...
	for (i = 0; i < num_max_cores; i++) {
		...
		cond_core_on[i] = (((stf_shdata->phytxchain >> i) & 0x1) &
			((stf_shdata->phyrxchain >> i) & 0x1)) ? TRUE : FALSE;
...
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_1) || defined(PHY_ACMAJORREV_3))) */
#if !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_2) || defined(PHY_ACMAJORREV_4) || defined(PHY_ACMAJORREV_40)))
	if (ACMAJORREV_2(pi->pubpi->phy_rev) ||
...
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_2) || defined(PHY_ACMAJORREV_4) || defined(PHY_ACMAJORREV_40))) */
		...
		const uint8 core, const bool isInitiator, const bool isSecure)
{
	...
	int ret = BCME_OK;
	uint8 tmp = 0;
	bool change_flag = FALSE, loadSPB;
	uint8 *r_ptr = (isInitiator) ? tofi->rtx : tofi->rrx;

	if (len != FTM_TPK_RI_RR_LEN) {
		return BCME_BADLEN;
	}
	...
	if (change_flag) {
		...
		if (isSecure) {
			...
			*/
			...
			loadSPB = FALSE;
#endif /* K_TOF_LOAD_SPB_ONLY */
		ret = phy_ac_tof_seq_setup(pi, FALSE, FALSE, core, isInitiator, loadSPB, FALSE);
	}
	tofi->tof_phy_error = 0;
	return ret;
}
...
{
	/*
	 * isInitiator value is ignored if TOF_TEST_TONE is not defined or if loadSPB is FALSE.
	*/
	...
	cint32 *pSeq = phy_malloc_fatal(pi, tof_seq_n * sizeof(*pSeq));
	...
	bool set, bool tx, int size)
{
	...
	if (tofi->tof_ucode_dlys_us[0][0] == 0) {
		if ((ret = phy_ac_tof_seq_params(ctx)) != BCME_OK) {
	...
	uint8 *Out)
{
	...
	uint8 state = (*In2 & (state_mask << 1)) >> 1;
	imax = (len / 8) + 1;
	for (i = 0; i < imax; i++) {
		byte_in = *(In1 + i);
		byte_out = 0;
...
void
phy_ac_tof_bytes_to_spb(const uint8* In, const uint16 len, uint32* Out)
{
	/*
	...
	*/
	for (i = 0; i < (2 * K_TOF_SEQ_SPB_LEN_20MHZ); i++) {
		...
		for (j = 0; j < bits_in_byte; j++) {
			...
			if ((idx == 0) || ((idx <= K_TOF_LTF_MASK_NEG) &&
				(idx >= K_TOF_LTF_MASK_POS))) {
				nibble = 0x0;
			} else {
				if (shift_ctr == 0) {
					in = *(In + k);
