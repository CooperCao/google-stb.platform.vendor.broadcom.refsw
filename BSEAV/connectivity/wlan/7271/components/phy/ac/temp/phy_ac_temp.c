...
phy_ac_temp_throttle(phy_type_temp_ctx_t *ctx)
{
	...
	/* No need to do tempsense/Throttle/Cal when not associated
	  ...
	  */
#if !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_4) || defined(PHY_ACMAJORREV_5)))
	if ((ACMAJORREV_4(pi->pubpi->phy_rev) ||
		(ACMAJORREV_5(pi->pubpi->phy_rev) && ACMINORREV_2(pi))) && PUB_NOT_ASSOC(pi))
		return stf_shdata->hw_phytxchain;
#endif /* !defined(PHY_VER)  || (defined(PHY_VER) && (defined(PHY_ACMAJORREV_4) || defined(PHY_ACMAJORREV_5))) */
...
wlc_phy_tempsense_radio_setup_acphy_20696(phy_info_t *pi, uint16 Av, uint16 Vmid)
{
	...
	FOREACH_ACTV_CORE(pi, stf_shdata->phyrxchain, core) {
		if (NO_7271_DFS_CORE || NO_RADAR_CHAN(pi->radio_chanspec)) {
...
wlc_phy_tempsense_phy_setup_acphy(phy_info_t *pi)
{
	...
	FOREACH_ACTV_CORE(pi, phyrxchain, core) {
		...
		if (ACMAJORREV_37(pi->pubpi->phy_rev)) {
			/* SWSTB-5569, force dvga to avoid tempsense slope dependency on NVRAM rxgain parms */
			MOD_PHYREGCE(pi, RfctrlOverrideGains,    core, rxgain, 1);
			MOD_PHYREGCE(pi, RfctrlCoreRXGAIN1,      core, rxgain_dvga, 1);
		}
...
wlc_phy_tempsense_vbatsense_acphy_20696(phy_info_t *pi, uint8 tempsense_vbatsense)
{
	...
	FOREACH_ACTV_CORE(pi, stf_shdata->phyrxchain, core) {
		if (NO_7271_DFS_CORE || NO_RADAR_CHAN(pi->radio_chanspec)) {
			wlc_phy_tempsense_poll_adc_war_20696(pi, TRUE, measured_voltage[core],
					core);
			radio_temp += (int32)(((measured_voltage[core][0]
					+ measured_voltage[core][2]
					- measured_voltage[core][1]
	...
	PHY_THERMAL(("Tempsense:   AuxPGA Av,Vmid = %d,%d\n",
	             auxPGA_Av, auxPGA_Vmid));
	FOREACH_ACTV_CORE(pi, stf_shdata->phyrxchain, core) {
		if (NO_7271_DFS_CORE || NO_RADAR_CHAN(pi->radio_chanspec)) {
			PHY_THERMAL((" Core%d\t Vref1,Vref2,Vctat1,Vctat2 = %6d,%6d,%6d,%6d\n", core,
			     measured_voltage[core][0], measured_voltage[core][2],
			     measured_voltage[core][1], measured_voltage[core][3]));
		}
	}
