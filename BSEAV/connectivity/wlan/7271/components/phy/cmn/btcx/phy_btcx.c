...
void
PHY_TRACE_DEFN(phy_btcx_disable_arbiter, phy_btcx_info_t *bi)
{
	phy_info_t* pi = bi->priv->pi;

	PHY_COEX_TRACE("arbiter off");
...
void
PHY_TRACE_DEFN(phy_btcx_enable_arbiter, phy_btcx_info_t *bi)
{
	phy_info_t* pi = bi->priv->pi;

	PHY_COEX_TRACE("arbiter on");
...
void
PHY_TRACE_DEFN(wlc_btcx_override_enable, phy_info_t *pi)
{
	phy_btcx_info_t *btcxi = pi->btcxi;
	phy_type_btcx_fns_t *fns = btcxi->priv->fns;

	PHY_COEX_TRACE("override on");

	if (fns->override_enable != NULL) {
		PHY_COEX_TRACE_CALL(fns->override_enable, fns->ctx);
	} else {
		PHY_COEX_TRACE_CALL(phy_btcx_override_enable_legacy_int, pi);
	}
}

static void
PHY_TRACE_DEFN(phy_btcx_override_disable_legacy, phy_info_t *pi)
{
	if ((pi->sh->machwcap & MCAP_BTCX) && CHSPEC_IS2G(pi->radio_chanspec)) {

		PHY_COEX_TRACE("legacy override off");
...
void
PHY_TRACE_DEFN(wlc_phy_btcx_override_disable, phy_info_t *pi)
{
	phy_btcx_info_t *btcxi = pi->btcxi;
	phy_type_btcx_fns_t *fns = btcxi->priv->fns;

	PHY_COEX_TRACE("override off");

	if (fns->override_disable != NULL) {
		PHY_COEX_TRACE_CALL(fns->override_disable, fns->ctx);
	} else {
		PHY_COEX_TRACE_CALL(phy_btcx_override_disable_legacy_int, pi);
	}
}
...
void
PHY_TRACE_DEFN(wlc_phy_btcx_wlan_critical_enter, phy_info_t *pi)
{
	PHY_COEX_TRACE("critical enter");
	wlapi_bmac_mhf(pi->sh->physhim, MHF1, MHF1_WLAN_CRITICAL, MHF1_WLAN_CRITICAL, WLC_BAND_2G);
}

void
PHY_TRACE_DEFN(wlc_phy_btcx_wlan_critical_exit, phy_info_t *pi)
{
	PHY_COEX_TRACE("critical exit");
