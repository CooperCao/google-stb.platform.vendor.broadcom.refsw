...
static bool wlc_msch_add_timeout(wlc_hrt_to_t *to, int timeout, wlc_hrt_to_cb_fn fun, void *arg, mbool requester);
...
_msch_chan_adopt(wlc_msch_info_t *msch_info, chanspec_t chanspec)
{
	...
	MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON,
		"_msch_chan_adopt Change Chanspec 0x%x\n", chanspec));

	_msch_dump_profiler(msch_info->profiler);

	wlc_suspend_mac_and_wait(wlc);
	/* For LCN20PHY, no cals during channel switch */
	if (WLCISLCN20PHY(wlc->band)) {
		wlc_phy_hold_upd(WLC_PI(wlc), PHY_HOLD_FOR_VSDB, TRUE);
	}
	wlc_set_chanspec(wlc, chanspec, CHANSW_MCHAN);
	if (WLCISLCN20PHY(wlc->band)) {
	...
	wlc_msch_req_handle_t *req_hdl, uint16 chan_idx)
{
	...
	if (!MSCH_START_FLEX(req_param->req_type)) {
		...
		/* To compare the request time with either current time or slot end time */
		if (msch_info->cur_msch_timeslot &&
			(msch_info->cur_msch_timeslot->end_time != -1)) {
			time_check = msch_info->cur_msch_timeslot->end_time;
		} else {
			time_check = cur_time;
		}
		...
		if (!(req_param->flags & MSCH_REQ_FLAGS_CHAN_CONTIGUOUS) &&
			...
			gap_in_intervals = bcm_uint64_div((uint32)(time_gap >> 32),
	...
	uint64 free_slot, msch_req_entity_t *entity, uint32 prep_dur)
{
	...
	if (chan_ctxt->bf_sch_pending) {
		/* already scheduled */
		MSCH_MESSAGE((msch_info, MSCH_ERROR_ON,
			"_msch_sch_both_flex: both flex already scheduled\n"));
		return MSCH_ALREADY_SCHD;
	}
	...
	if (entity->pend_slot.timeslot) {
		...
		if (entity->pend_slot.end_time != -1) {
...
_msch_chsw_timer(void *arg)
{
	...
	wlc_gptimer_wake_upd(msch_info->wlc, WLC_GPTIMER_AWAKE_MSCH, FALSE);
		...
		if (cur_ts->state == MSCH_TIMER_STATE_ONCHAN_FIRE) {
			...
			if (!msch_list_empty(&msch_info->msch_req_timing_list)) {
				...
				if (cur_ts->fire_time != -1) {
					MSCH_MESSAGE((msch_info, MSCH_INFORM_ON,
						"_msch_chsw_timer: cur_ts->fire_time is not "
						"infinite\n"));
					goto done;
				}
			}
			...
			if (cur_ts->end_time != -1) {
				if (cur_ts->end_time > cur_time) {
					MSCH_MESSAGE((msch_info, MSCH_INFORM_ON,
						"_msch_chsw_timer: end_time is greater than "
						"cur_time\n"));
					goto done;
				}
	...
	if (!msch_info->next_timeslot->fire_time || msch_info->next_timeslot->fire_time == -1) {
		MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON,
	...
	if (cur_ts->chan_ctxt->chanspec != WLC_BAND_PI_RADIO_CHANSPEC) {
		_msch_chan_adopt(msch_info, cur_ts->chan_ctxt->chanspec);
	}
	...
	if (cur_ts) {
		if (cur_ts->fire_time != -1) {
			...
			wlc_msch_add_timeout(msch_info->chsw_timer,
				(int)(cur_ts->fire_time - cur_time),
				_msch_chsw_timer, msch_info, WLC_GPTIMER_AWAKE_MSCH);
		}
	} else if (msch_info->next_timeslot && msch_info->next_timeslot->fire_time != -1) {
...
_msch_arm_chsw_timer(wlc_msch_info_t *msch_info)
{
	...
	if (msch_info->cur_msch_timeslot &&
		...
		if (msch_info->cur_msch_timeslot->end_time == -1) {
...
_msch_slotskip_timer(void *arg)
{
	...
	wlc_gptimer_wake_upd(wlc, WLC_GPTIMER_AWAKE_MSCH_SLOT_SKIP, FALSE);
	cur_time = msch_current_time(msch_info);
	MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON,
	...
	while ((req_elem = msch_list_iterate(req_elem))) {
		...
		if ((cur_ts && cur_ts->end_time == -1) ||
			(msch_info->next_timeslot && msch_info->next_timeslot->end_time == -1)) {
		...
		if ((cur_time >= slot_due) || (switch_dur < 0)) {
			...
			wlc_msch_add_timeout(msch_info->slotskip_timer,
				switch_dur, _msch_slotskip_timer, msch_info, WLC_GPTIMER_AWAKE_MSCH_SLOT_SKIP);
			break;
		}
	}
...
_msch_schd(wlc_msch_info_t *msch_info)
{
	...
	if (bflex_entity && (slot_dur >= bf_min_dur)) {
		...
		if (msch_info->flex_list_cnt > 1) {
			...
			else if (slot_dur != -1 && (slot_start_time < bf_start_time)) {
		...
		if (slot_dur && slot_dur != -1) {
			 ...
			 * minimum bf time
			 */
			if (dfix_entity && req_dur > 0 && slot_dur >
	...
	if (msch_info->next_timeslot) {
		...
		if (msch_info->next_timeslot->end_time == -1) {
			slot_dur = -1;
		} else {
...
_msch_check_sfix_slot_skip(wlc_msch_info_t *msch_info, chanspec_t chanspec, uint64 end_time)
{
	...
	while ((elem = msch_list_iterate(elem))) {
		...
		if (msch_info->slotskip_flag) {
			wlc_gptimer_wake_upd(msch_info->wlc, WLC_GPTIMER_AWAKE_MSCH_SLOT_SKIP, FALSE);
			/* delete the timer */
			wlc_hrt_del_timeout(msch_info->slotskip_timer);
		}
		msch_info->slotskip_flag = TRUE;
		wlc_msch_add_timeout(msch_info->slotskip_timer,
			0, _msch_slotskip_timer, msch_info, WLC_GPTIMER_AWAKE_MSCH_SLOT_SKIP);
		return TRUE;
	}
	return FALSE;
}
		...
		if (MSCH_START_FIXED(entity->req_hdl->req_param->req_type)) {
			...
			if (entity->pend_slot.start_time < start_time) {
				...
				if (next_avail_time < entity->pend_slot.start_time) {
					ts->pre_start_time = next_avail_time;
				} else {
...
static bool
wlc_msch_add_timeout(wlc_hrt_to_t *to, int timeout, wlc_hrt_to_cb_fn fun, void *arg, mbool requester)
{
	bool ret = FALSE;
	wlc_msch_info_t *msch_info = (wlc_msch_info_t *)arg;
	...
	ret = wlc_hrt_add_timeout(to, timeout, fun, arg);
	wlc_gptimer_wake_upd(msch_info->wlc, requester, TRUE);
	return ret;
}
...
_msch_check_param(wlc_msch_info_t *msch_info, wlc_msch_req_param_t *param)
{
	...
	if (param->flags & MSCH_REQ_FLAGS_CHAN_CONTIGUOUS) {
		...
		/* Check if we have atleast one slot to be scheduled for the requested
		 * start time
		 */
		time_check += param->interval;
		time_check -= param->duration;

		if (time_check < cur_time) {
			MSCH_MESSAGE((msch_info, MSCH_ERROR_ON,
	...
	wlc_msch_req_handle_t **p_req_hdl)
{
	...
	if (MSCH_PREEMPT_REQ(req_param->flags)) {
		...
		if (!msch_info->cur_msch_timeslot) {
			...
			chanspec_list[0], &new_end_time, req_param->flags, &split_curts)) == -1) {
			MSCH_MESSAGE((msch_info, MSCH_ERROR_ON,
	...
	*/
	if (!(msch_info->flags & MSCH_STATE_IN_TIEMR_CTXT)) {
		if (MSCH_PREEMPT_REQ(req_param->flags) ||
			((ret = _msch_schd_new_req(msch_info, req_hdl)) == MSCH_SCHD_NEXT_SLOT) ||
			!msch_info->next_timeslot) {
			if (!msch_info->slotskip_flag) {
				_msch_schd(msch_info);
			}
	...
	wlc_msch_req_param_t *preempt_param, chanspec_t preempt_chanspec)
{
	...
	while ((elem = msch_list_iterate(elem))) {
		 ...
		 */
		entity->pend_slot.end_time = (((uint64)split_slot_param->start_time_h << 32) |
			split_slot_param->start_time_l) +
			(entity->cur_chn_idx * split_slot_param->duration) +
			split_slot_param->duration;

		/* Update the actual start timme since the new pend slot is the cur slot */
	...
	int32 drift)
{
	...
	*/
	if (msch_info->cur_msch_timeslot->end_time > cur_time) {
		time_to_end = new_end = (int32)(msch_info->cur_msch_timeslot->end_time - cur_time);
		if (drift < 0) {
	...
	if (drift < 0) {
		...
		if (_msch_check_curts_preemption(msch_info, ABS(drift), entity->chanspec,
			&cur_ts_endtime, MSCH_REQ_FLAGS_PREMT_CURTS, &now) != -1) {
			MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON,
		...
		elem = &cur_ts->chan_ctxt->req_entity_list;
		while ((elem = msch_list_iterate(elem))) {
			cur_ts_entity = LIST_ENTRY(elem, msch_req_entity_t, chan_ctxt_link);
			if (cur_ts_entity->req_hdl == hdl) {
				break;
			}
		}
...
msch_timeslot_update_slot_endtime(wlc_msch_info_t *msch_info, wlc_msch_req_handle_t *p_req_hdl,
	...
	while ((elem = msch_list_iterate(elem))) {
		...
		if (entity->cur_slot.timeslot == msch_info->cur_msch_timeslot) {
			 ...
			 * next ts
			 */
			if (msch_info->next_timeslot->pre_start_time <
				msch_info->cur_msch_timeslot->end_time) {
				_msch_timeslot_destroy(msch_info, msch_info->next_timeslot, FALSE);
				msch_info->next_timeslot = NULL;
			}
	...
	msch_req_entity_t *entity, uint32 prep_time)
{
	...
	if (msch_info->cur_msch_timeslot &&
		(msch_info->cur_msch_timeslot->end_time > cur_ts_end)) {
		cur_ts_end = msch_info->cur_msch_timeslot->end_time;
	}
	if (WLC_BAND_PI_RADIO_CHANSPEC != entity->chan_ctxt->chanspec) {
		prep_time = wlc_msch_calc_avg_chswtime(msch_info,
			entity->chan_ctxt->chanspec);
	...
	while (idx <= req_hdl->last_chan_idx) {
		if (entity->chanspec == req_hdl->chn_list[idx]) {
			...
			if (entity->pend_slot.end_time >
				...
				/* tweak the start time for remaining slot
				* to be current slot's end time + prep time
				*/
				if (entity->pend_slot.start_time < (cur_ts_end + prep_time)) {
					entity->pend_slot.start_time = (cur_ts_end + prep_time);
				}
				break;
			}
		}
