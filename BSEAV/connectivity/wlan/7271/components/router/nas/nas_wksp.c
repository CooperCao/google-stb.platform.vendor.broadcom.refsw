 ...
 *
 * $Id: nas_wksp.c 686398 2017-02-22 10:00:44Z kugaurav $
 */
...
#if !defined(DEBUG) && !defined(vxworks) && !defined(__ECOS)
	if (!nwksp->foreground) {
		/* Daemonize */
		if (daemon(1, 1) == -1) {
...
#ifdef DSLCPE
	/* update authentication blockout retry interval */
	nwcb->nas.auth_blockout_time = info.auth_blockout_time;
	NASDBG("new nwcb's nas auth_blockout_time %d\n", nwcb->nas.auth_blockout_time);
#endif
