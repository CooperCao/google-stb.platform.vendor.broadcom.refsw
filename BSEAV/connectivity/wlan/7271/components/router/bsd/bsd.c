...
static int bsd_send_transreq(bsd_sta_info_t *sta)
{
	...
	*/
	...
	BSD_EXIT();
	return BSD_BSS_RESPONSE_UNKNOWN;
}
...
void bsd_dump_sta_info(bsd_info_t *info)
{
	...
	bool bounce, picky, psta, dualband;
	FILE *out;
	...
	fprintf(out, "%-17s %-9s %-9s %-7s %-5s %-6s %-5s %-4s %-8s\n",
		"STA_MAC", "Interface", "TimeStamp", "Tx_rate", "RSSI",
		"Bounce", "Picky", "PSTA", "DUALBAND");
	...
	for (idx = 0; idx < info->max_ifnum; idx++) {
		...
		for (bssidx = 0; bssidx < WL_MAXBSSCFG; bssidx++) {
			...
			if (bssinfo->valid) {
				...
				while (assoclist) {
					bounce = picky = psta = dualband = FALSE;
...
void bsd_steer_sta(bsd_info_t *info, bsd_sta_info_t *sta, bsd_bssinfo_t *to_bss)
{
	...
	if ((ret = bsd_send_transreq(sta)) == BSD_BSS_RESPONSE_REJECT) {
		sta->steerflag |= BSD_STA_BSS_REJECT;
		BSD_STEER("Skip STA:"MACF" reject BSSID\n", ETHER_TO_MACF(sta->addr));
		printf("Skip STA:"MACF" reject BSSID\n", ETHER_TO_MACF(sta->addr));
		return;
	}

	if (ret == BSD_BSS_RESPONSE_UNKNOWN) {
		if (!bsd_is_sta_dualband(info, &sta->addr) ||
			!bsd_qualify_sta_rf(info, sta->bssinfo, sta->to_bssinfo, &sta->addr)) {
			BSD_STEER("Skip STA: no response for single-band sta "MACF"\n",
				ETHER_TO_MACF(sta->addr));
			printf("Skip STA: no response for single-band sta "MACF"\n",
				ETHER_TO_MACF(sta->addr));
			return;
		}
		...
		if (info->steer_no_deauth) {
			...
			printf("NOT steer STA "MACF" from %s by steer_no_deauth\n",
				ETHER_TO_MACF(sta->addr), bssinfo->ifnames);
			return;
		}
	}
	...
	bsd_update_steering_record(info, &(sta->addr), bssinfo, steer_bssinfo,
		intf_info->steering_flags);

	/*
	 * For BSD_BSS_RESPONSE_ACCEPT:
	 *	STA will switch band, AP don't need to send deauth
	 * For BSD_BSS_RESPONSE_UNKNOWN: (legacy sta)
	 *	Will base on nvram setting to determine if send deauth or not
	 */
	if (ret == BSD_BSS_RESPONSE_UNKNOWN) {
		/* info->steer_no_deauth == 0 here */
		/* iovar to deaut and set maclist */
		BSD_RPC("---RPC name:%s cmd: %d(WLC_SCB_DEAUTHENTICATE)\n",
			bssinfo->ifnames, WLC_SCB_DEAUTHENTICATE);
		ret = bsd_wl_ioctl(bssinfo, WLC_SCB_DEAUTHENTICATE,
			&sta->addr, ETHER_ADDR_LEN);

		if (ret < 0) {
			BSD_ERROR("Err: ifnams[%s] send deauth\n", bssinfo->ifnames);
		} else {
