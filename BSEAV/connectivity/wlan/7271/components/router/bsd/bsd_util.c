	...
	foreach(var_intf, bsd_ifnames, next_intf) {
		...
		foreach(var, vifs, next) {
			...
			if (bsd_nvram_match(rpc, strcat_r(prefix, "bss_enabled", tmp), "1")) {
				...
				if (ret == BSD_OK) {
					num = sscanf(str, "%d %d %d %d %d %d %d %d %d %x",
						&policy.idle_rate, &policy.rssi, &policy.phyrate,
						&policy.wprio,
						&policy.wrssi, &policy.wphy_rate,
						&policy.wtx_failures,
...
*/
			...
			param = (char *)ioctl_buf;
			bssinfo->chanspec = (chanspec_t)(*((uint32 *)param));
			memcpy(&bssinfo->chanspec, ioctl_buf, sizeof(chanspec_t));
			...
			param = (char *)ioctl_buf;
			bssinfo->rclass = (uint8)(*((uint32 *)param));
