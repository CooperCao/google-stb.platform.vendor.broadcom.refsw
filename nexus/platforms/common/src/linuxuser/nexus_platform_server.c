...
nexus_driver_send_addr(void **paddr)
{
    ...
    void *addr = *paddr;
    if(addr) {
        *paddr = (void *)(long)NEXUS_AddrToOffset(addr);
    }
    return;
}
...
nexus_driver_recv_addr_cached(void **paddr)
{
    ...
    if(addr) {
        *paddr = NEXUS_OffsetToCachedAddr((long)addr);
    }
    return;
}
                    ...
                    if (i == 0) {
                        ...
                        for (i=0;i<NEXUS_MAX_HEAPS;i++) {
                            if (server->thread_state.init_data.config.heap[i]) {
                                ...
                                NEXUS_Heap_GetStatus(server->thread_state.init_data.heap[i].heap, &status);
    ...
    if (grant) {
        ...
        for (i=0,j=0;i<NEXUS_MAX_HEAPS && j<BCM_MAX_HEAPS;i++) {
            ...
            if (client->client_state.client.config.heap[i]) {
                rc = NEXUS_Heap_GetStatus(client->client_state.client.config.heap[i], &status);
