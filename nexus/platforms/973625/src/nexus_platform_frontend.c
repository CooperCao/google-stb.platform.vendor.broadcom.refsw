...
#if (BCHP_CHIP==7344) || (BCHP_CHIP==7358) || (BCHP_CHIP==7360) || (BCHP_CHIP==7362) || (BCHP_CHIP==73625)
...
NEXUS_Error NEXUS_Platform_InitFrontend(void)
{
    ...
    for ( i = 0; i < NEXUS_7346_MAX_FRONTEND_CHANNELS; i++ )
    {
        ...
        {
            BDBG_MSG(("NEXUS_Frontend_Open7346: Open success [%d] [%p]", i, (void *)pConfig->frontend[i] ));
    ...
    for ( j = 0; j < NEXUS_4505_MAX_FRONTEND_CHANNELS ; j++, i++ )
    {
        ...
        if ( pConfig->frontend[i] )
        {
            BDBG_MSG(("NEXUS_Frontend_Open4506: Open success [%d] [%p]", i, (void *)pConfig->frontend[i] ));
            NEXUS_Frontend_GetUserParameters(pConfig->frontend[i], &userParams);
            switch (j)
            {
