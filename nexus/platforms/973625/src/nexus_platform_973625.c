...
NEXUS_Error NEXUS_Platform_P_InitBoard(void)
{
    ...
    /* Enable Gpio 111 to activate the audio amp connected to dac */
    ulVal = BREG_Read32 (hReg, BCHP_GIO_IODIR_EXT_HI);
    ulVal &= ~(1<<15);
                BREG_Write32(hReg, BCHP_GIO_IODIR_EXT_HI, ulVal);

    ulVal = BREG_Read32 (hReg, BCHP_GIO_DATA_EXT_HI);
    ulVal |= (1<<15);
                BREG_Write32(hReg, BCHP_GIO_DATA_EXT_HI, ulVal);
