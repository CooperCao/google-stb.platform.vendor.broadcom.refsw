...
/***************************************************************************
Summary:
    Configure pin muxes for the 975525 reference platform
Description:
...
NEXUS_Error NEXUS_Platform_P_InitPinmux(void)
{
    ...
    */
    ...
    /* Add LED pin config if needed for SV board : aon_gpio_04 ~ 18 */
    reg = BREG_Read32(hReg,BCHP_AON_PIN_CTRL_PIN_MUX_CTRL_2);

    reg &=~(BCHP_MASK(AON_PIN_CTRL_PIN_MUX_CTRL_2, aon_sgpio_00 ));
    reg |=(BCHP_FIELD_DATA(AON_PIN_CTRL_PIN_MUX_CTRL_2, aon_sgpio_00,1 ));    /* BSC_M3_SCL */
