...
void b_pig_move(NEXUS_SurfaceClientHandle video_sc, struct b_pig_inc *pig_inc)
{
    ...
    if (settings.composition.position.x + settings.composition.position.width + pig_inc->x > settings.composition.virtualDisplay.width + PIG_MOVE_BOUNDARY ||
        settings.composition.position.x + pig_inc->x < -PIG_MOVE_BOUNDARY) {
        pig_inc->x *= -1;
    }
    if (settings.composition.position.y + settings.composition.position.height + pig_inc->y > settings.composition.virtualDisplay.height + PIG_MOVE_BOUNDARY ||
        settings.composition.position.y + pig_inc->y < -PIG_MOVE_BOUNDARY) {
        pig_inc->y *= -1;
    }
    settings.composition.position.x += pig_inc->x;
    settings.composition.position.y += pig_inc->y;
    if (settings.composition.position.width >= settings.composition.virtualDisplay.width ||
        !settings.composition.position.width ||
        !settings.composition.position.height) {
        pig_inc->width *= -1;
    }
