...
int main(int argc, const char **argv)  {
    ...
    while (argc > curarg) {
        if (!strcmp(argv[curarg], "-h") || !strcmp(argv[curarg], "--help")) {
            ...
            create_settings.maxFormat = lookup(g_videoFormatStrs, argv[++curarg]);
        }
            ...
            start_settings.sync = lookup(g_syncModeStrs, argv[++curarg]);
        }
        else if (!strcmp(argv[curarg], "-crypto") && curarg+1 < argc) {
            start_settings.decrypt.algo = lookup(g_securityAlgoStrs, argv[++curarg]);
        }
        else if (!strcmp(argv[curarg], "-dolby_drc_mode") && curarg+1 < argc) {
            start_settings.audio.dolbyDrcMode = lookup(g_dolbyDrcModeStrs, argv[++curarg]);
        }
            ...
            if (!strcmp(argv[curarg], "none")) {
                start_settings.source3d = source3d_none;
            }
            else if (!strcmp(argv[curarg], "lr")) {
                start_settings.source3d = source3d_lr;
            }
            else if (!strcmp(argv[curarg], "ou")) {
                start_settings.source3d = source3d_ou;
            }
            else if (!strcmp(argv[curarg], "auto")) {
                start_settings.source3d = source3d_auto;
            }
            else {
                print_usage(&cmdline);
                return -1;
            ...
            start_settings.audio.codec = lookup(g_audioCodecStrs, argv[++curarg]);
        }
    ...
    if (rc) {
        BDBG_ERR(("unable to play '%s'", start_settings.stream_url));
        goto err_start;
    }
