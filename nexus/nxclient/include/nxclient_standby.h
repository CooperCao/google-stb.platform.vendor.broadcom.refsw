...
typedef struct NxClient_StandbyStatus
{
    ...
    bool standbyTransition; /* true means transition is done, which means NxClient_StandbyStatus.settings is actual state */
...
typedef struct NxClient_StandbySetttings
{
    ...
    NxClient_StandbyCpuMode mode; /* Applies for S3 standby only */
