...
static void *nxclient_p_callback_thread(void *context)
{
    ...
    while (!g_callbackThread.done) {
        ...
        if (rc) {
            ...
            if (g_callbackThread.settings.thermalConfigChanged.callback && status.thermalConfigChanged != g_callbackThread.lastStatus.thermalConfigChanged) {
                (g_callbackThread.settings.thermalConfigChanged.callback)(g_callbackThread.settings.thermalConfigChanged.context, g_callbackThread.settings.thermalConfigChanged.param);
            }
