...
static void print_full_usage(void)
{
    ...
    printf(
    "  -memconfig display,pip=quarter    \tDon't allocate more than quarter screen BVN buffers for PIP window\n"
    );
    printf(
...
static int nxserverlib_apply_memconfig_str(NEXUS_PlatformSettings *pPlatformSettings, NEXUS_MemoryConfigurationSettings *pMemConfigSettings, const char * const *memconfig_str, unsigned memconfig_str_total)
{
    ...
    for (i=0;i<memconfig_str_total;i++) {
        ...
        else if (!strcmp(memconfig_str[i], "display,pip=quarter")) {
            for (index=0;index<NEXUS_MAX_DISPLAYS;index++) {
                pMemConfigSettings->display[index].window[1].sizeLimit = NEXUS_VideoWindowSizeLimit_eQuarter;
            }
        }
        else {
