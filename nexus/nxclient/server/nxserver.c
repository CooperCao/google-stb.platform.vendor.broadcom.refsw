...
static void print_prompt_usage(void)
{
    ...
    "heap(HEAPNUM,{secure|unsecure}) - set secure mode for heap\n"
    "mem [HEAPNUM] - print device memory allocations for one or all heaps\n"
    "kni    - print all BKNI memory allocation\n"
    "clients - print list of clients\n"
    "cap     - print capabilities\n"
    "kill CLIENTID   - run \"clients\" to learn CLIENTID\n"
    "focus CLIENTID  - run \"clients\" to learn CLIENTID\n"
    );
}
...
static void nxserver_prompt(void)
{
    ...
    while (1) {
        ...
        else if (strstr(buf, "heap(") == buf) {
            char mode[16];
            if (sscanf(&buf[5], "%u,%s", &i, mode) == 2) {
                ...
                if (i < NEXUS_MAX_HEAPS && platformConfig.heap[i]) {
                    ...
                    settings.secure = (strstr(mode, "secure") == mode);
                    rc = NEXUS_Platform_SetHeapRuntimeSettings(platformConfig.heap[i], &settings);
                    if (rc) {
                        BERR_TRACE(rc);
                    }
                    else {
                        BDBG_WRN(("heap[%u] %s", i, settings.secure?"secure":"unsecure"));
                    }
                }
            }
        }
