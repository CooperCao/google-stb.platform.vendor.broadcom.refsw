...
NEXUS_Error NxClient_GetStandbyStatus(NxClient_StandbyStatus *pStatus)
{
    NEXUS_Error rc;
    LOCK();
    rc = NxClient_P_GetStandbyStatus(g_state.client, pStatus);
    UNLOCK();
    return rc;
}
...
NEXUS_Error NxClient_SetSlaveDisplayGraphics( unsigned slaveDisplay, NEXUS_SurfaceHandle surface )
{
    ...
    LOCK();
    rc = NxClient_P_General(g_state.client, nxclient_p_general_param_type_set_slave_display_graphics, &param, &output);
    UNLOCK();
    return rc;
}
