    ...
    )
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Entry..." BIP_MSG_PRE_ARG, (void *)hArb ));

    if ( ! hArb->destroyMe) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Not marked for delete, returning..." BIP_MSG_PRE_ARG, (void *)hArb ));
        return(BIP_INF_ARB_IN_PROGRESS);
    }

    if (hArb->state != BIP_ArbState_eIdle) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Arb is not Idle, returning..." BIP_MSG_PRE_ARG, (void *)hArb));
        return(BIP_INF_ARB_IN_PROGRESS);
    }

    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Setting hDestroyEvent" BIP_MSG_PRE_ARG, (void *)hArb));
    ...
    )
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "BIP_ArbFactory: Created hDclMutex:%p" BIP_MSG_PRE_ARG, (void *)pArbFactory->hDclMutex ));
...
void BIP_ArbFactory_Uninit(void)
{
    ...
    {
        BDBG_MSG(( BIP_MSG_PRE_FMT "BIP_ArbFactory: Destroying hDclMutex:%p" BIP_MSG_PRE_ARG, (void *)pArbFactory->hDclMutex ));
    ...
    )
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Marking Arb for destruction!" BIP_MSG_PRE_ARG, (void *)hArb));
    hArb->destroyMe = true;
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Arb is finished! It's okay to destroy!  Here we go..." BIP_MSG_PRE_ARG, (void *)hArb));

    if (hArb->hDestroyEvent) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Destroying hDestroyEvent:%p" BIP_MSG_PRE_ARG, (void *)hArb, (void *)hArb->hDestroyEvent ));
        B_Event_Destroy( hArb->hDestroyEvent);
    }

    if (hArb->hArbEvent) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Destroying hArbEvent:%p" BIP_MSG_PRE_ARG, (void *)hArb, (void *)hArb->hArbEvent ));
        B_Event_Destroy( hArb->hArbEvent);
    }

    if (hArb->hApiMutex) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Destroying hApiMutex:%p" BIP_MSG_PRE_ARG, (void *)hArb, (void *)hArb->hApiMutex ));
        B_Mutex_Destroy( hArb->hApiMutex);
    }

    if (hArb->hArbMutex) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Destroying hArbMutex:%p" BIP_MSG_PRE_ARG, (void *)hArb, (void *)hArb->hArbMutex ));
        B_Mutex_Destroy( hArb->hArbMutex);
    }

    BDBG_OBJECT_DESTROY( hArb, BIP_Arb );

    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Freeing object memory" BIP_MSG_PRE_ARG, (void *)hArb));
    B_Os_Free( hArb );
...
BIP_ArbHandle BIP_Arb_Create(B_MutexHandle hApiMutex, B_MutexHandle hArbMutex)
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Allocated " BIP_MSG_PRE_ARG, (void *)hArb));

    hArb->hArbMutex = B_Mutex_Create(NULL);
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Created hArbMutex:%p" BIP_MSG_PRE_ARG, (void *)hArb, (void *)hArb->hArbMutex  ));
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Created hArb->hApiMutex:%p" BIP_MSG_PRE_ARG, (void *)hArb, (void *)hArb->hApiMutex ));
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Created hArbEvent:%p" BIP_MSG_PRE_ARG, (void *)hArb, (void *)hArb->hArbEvent ));
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Created hDestroyEvent:%p" BIP_MSG_PRE_ARG, (void *)hArb, (void *)hArb->hDestroyEvent ));
...
BIP_Status BIP_Arb_Acquire(BIP_ArbHandle hArb)
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Locked Api and Arb Mutexes" BIP_MSG_PRE_ARG, (void *)hArb));

    if (hArb->state != BIP_ArbState_eIdle) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Arb is already busy, state: %d" BIP_MSG_PRE_ARG, (void *)hArb, hArb->state ));
        rc = BIP_ERR_ALREADY_IN_PROGRESS;
        goto error_unlock;
    }
    else if (hArb->destroyMe) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Arb is being destroyed, Acquire not allowed" BIP_MSG_PRE_ARG, (void *)hArb));
        rc = BIP_ERR_ARB_BEING_DESTROYED;
        goto error_unlock   ;
    }
    else {
        /* Initialize ARB fields. */
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Initialize per request Arb fields..." BIP_MSG_PRE_ARG, (void *)hArb));
        BKNI_Memset(&hArb->settings, 0 , sizeof(hArb->settings));
        BKNI_Memset(&hArb->completionCallback, 0 , sizeof(hArb->completionCallback));
        hArb->rejected = false;
        B_Event_Reset(hArb->hArbEvent);
        B_Event_Reset(hArb->hDestroyEvent);
        hArb->pCompletionStatus = NULL;
   ...
   )
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Verified that Api and Arb are locked" BIP_MSG_PRE_ARG, (void *)hArb));
    ...
    if (hArb->state != BIP_ArbState_eAcquired) {
        BDBG_ERR(( BIP_MSG_PRE_FMT "hArb %p: Arb is not Acquired! state: %d  Did you call BIP_Arb_Acquire()?"
                   BIP_MSG_PRE_ARG, (void *)hArb, hArb->state ));
        BIP_CHECK_GOTO((false), ( "Arb is Busy/Idle but should be Acquired!" ), error, BIP_ERR_INVALID_API_SEQUENCE, rc );
    }

    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Verified that Arb state is Acquired" BIP_MSG_PRE_ARG, (void *)hArb));
    ...
    if (pApiSettings) {
        if (pApiSettings->asyncCallback.callback) {
            ...
            BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Set completionCallback from ApiSettings" BIP_MSG_PRE_ARG, (void *)hArb));
        }

        if (pApiSettings->pAsyncStatus) {
            hArb->pCompletionStatus = pApiSettings->pAsyncStatus;
            BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Set pCompletionStatus from ApiSettings" BIP_MSG_PRE_ARG, (void *)hArb));
        }
    }
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Set *pCompletionStatus to BIP_INF_IN_PROGRESS" BIP_MSG_PRE_ARG, (void *)hArb));

    if (hArb->hArbEvent) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Resetting Arb completionEvent" BIP_MSG_PRE_ARG, (void *)hArb));
        B_Event_Reset(hArb->hArbEvent);
    }

    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Setting Arb state to Submitted" BIP_MSG_PRE_ARG, (void *)hArb));
    hArb->state = BIP_ArbState_eSubmitted;  /* This hands the Arb to the server-side Arb-processor. */

    if (hArb->settings.arbProcessor) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Calling Arb's arbProcessor" BIP_MSG_PRE_ARG, (void *)hArb));
    ...
    if ( !hArb->settings.waitIfBusy) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Unlocking ApiMutex because waitIfBusy is false" BIP_MSG_PRE_ARG, (void *)hArb));
        B_Mutex_Unlock(hArb->hApiMutex); apiLocked = false;
    }

    if (hArb->state == BIP_ArbState_eCompleted && hArb->rejected) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Request was rejected. completionStatus:0x%X" BIP_MSG_PRE_ARG, (void *)hArb, hArb->completionStatus ));
        BDBG_ASSERT(hArb->completionStatus != BIP_INF_IN_PROGRESS);
        rc = hArb->completionStatus;
    }
    else if (hArb->completionCallback.callback) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Request has completionCallback, no need to wait" BIP_MSG_PRE_ARG, (void *)hArb));
        ...
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Request has a pCompletionStatus pointer, no need to wait" BIP_MSG_PRE_ARG, (void *)hArb));
    ...
    {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Waiting on *pCompletionEvent. completionStatus:0x%X" BIP_MSG_PRE_ARG, (void *)hArb, hArb->completionStatus ));
        B_Mutex_Unlock(hArb->hArbMutex); arbLocked = false;
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Returning: "BIP_STATUS_FMT"" BIP_MSG_PRE_ARG, (void *)hArb, BIP_STATUS_ARG(rc) ));
    ...
    if (hArb->state != BIP_ArbState_eCompleted) {
        ...
        if (hArb->hMyArbList) {     /* If dynamic Arb, destroy it. */
            BDBG_MSG(( BIP_MSG_PRE_FMT "hArbList %p: Destroying dynamic Arb: %p" BIP_MSG_PRE_ARG, (void *)hArb->hMyArbList, (void *)hArb));
            B_Mutex_Unlock(hArb->hArbMutex);
            BIP_Arb_Destroy(hArb); hArb = NULL;   /* Arb destroyed, hArb is now invalid. */
        }
        else {                      /* If static Arb, set back to idle. */
            BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Arb state is Completed, changeing to Idle" BIP_MSG_PRE_ARG, (void *)hArb));
            B_Mutex_Unlock(hArb->hArbMutex);
    ...
    )
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArbList %p: Entry..." BIP_MSG_PRE_ARG, (void *)hArbList ));
    ...
    if (hArbList->hArbListMutex) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArbList %p: Destroying hArbListMutex:%p" BIP_MSG_PRE_ARG, (void *)hArbList, (void *)hArbList->hArbListMutex ));
        B_Mutex_Destroy(hArbList->hArbListMutex);
    }

    BDBG_OBJECT_DESTROY( hArbList, BIP_ArbList );

    BDBG_MSG(( BIP_MSG_PRE_FMT "hArbList %p: Freeing object memory" BIP_MSG_PRE_ARG, (void *)hArbList ));
    B_Os_Free( hArbList );
...
BIP_ArbListHandle BIP_ArbList_Create(void * unused)
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArbList %p: Allocated " BIP_MSG_PRE_ARG, (void *)hArbList ));

    hArbList->hArbListMutex = B_Mutex_Create(NULL);
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArbList %p: Created hArbListMutex:%p" BIP_MSG_PRE_ARG, (void *)hArbList, (void *)hArbList->hArbListMutex ));
    ...
    )
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArbList %p: Entry... Locking ArbList Mutex..." BIP_MSG_PRE_ARG, (void *)hArbList ));

    B_Mutex_Lock(hArbList->hArbListMutex);
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArbList %p: Inserting hArb:%p at tail of arbNewList" BIP_MSG_PRE_ARG, (void *)hArbList, (void *)hArb ));
    ...
    )
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArbList %p: Getting oldest hArb:%p" BIP_MSG_PRE_ARG, (void *)hArbList, (void *)hArb ));
    B_Mutex_Unlock(hArbList->hArbListMutex);
    ...
    )
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Returning %s" BIP_MSG_PRE_ARG, (void *)hArb, rc?"TRUE":"FALSE" ));
    ...
    )
{
    ...
    if (BLST_Q_EMPTY(&hArbList->arbNewListHead)) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArbList %p: arbNewList NOT empty" BIP_MSG_PRE_ARG, (void *)hArbList ));
        rc = true;
    }

    if (BLST_Q_EMPTY(&hArbList->arbAcceptedListHead)) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArbList %p: arbAcceptedList NOT empty" BIP_MSG_PRE_ARG, (void *)hArbList ));
        rc = true;
    }

    BDBG_MSG(( BIP_MSG_PRE_FMT "hArbList %p: Returning %s" BIP_MSG_PRE_ARG, (void *)hArbList, rc?"TRUE":"FALSE" ));
   ...
   )
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Arb state is Submitted, changing to Accepted" BIP_MSG_PRE_ARG, (void *)hArb));
    ...
    if (hArb->hMyArbList) {
        ...
        B_Mutex_Lock(hArbList->hArbListMutex);
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArbList %p: Removing Arb: %p from New list" BIP_MSG_PRE_ARG, (void *)hArbList, (void *)hArb));
        ...
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArbList %p: Adding Arb: %p to Accepted list" BIP_MSG_PRE_ARG, (void *)hArbList, (void *)hArb));
        B_Mutex_Unlock(hArbList->hArbListMutex);
    }
...
error:
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: [Error] Returning: "BIP_STATUS_FMT"" BIP_MSG_PRE_ARG, (void *)hArb, BIP_STATUS_ARG(rc) ));
    B_Mutex_Unlock(hArb->hArbMutex);
    return rc;
   ...
   )
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Setting completionStatus to 0x%X" BIP_MSG_PRE_ARG, (void *)hArb, completionStatus ));
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Arb state is Submitted, changing to Completed" BIP_MSG_PRE_ARG, (void *)hArb));
    hArb->rejected = true;
    hArb->state = BIP_ArbState_eCompleted;

    if (!hArb->apiBusy)
    {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Arb state is Completed and !apiBusy, changing to Idle" BIP_MSG_PRE_ARG, (void *)hArb));
        hArb->state = BIP_ArbState_eIdle;
        B_Mutex_Unlock(hArb->hArbMutex);
        BIP_Arb_ReadyToDestroy(hArb);       /* Sets hArb->hDestroyEvent if Arb is ready to destroy. */
   ...
   )
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "Entry... pCallbackDesc: %p, context: %p, param: %d" BIP_MSG_PRE_ARG,
               (void *)pCallbackDesc, (void *)pCallbackDesc->context, pCallbackDesc->param ));
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: Allocated (%zu bytes)" BIP_MSG_PRE_ARG, (void *)pDclItem, sizeof *pDclItem ));
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: Returning: "BIP_STATUS_FMT"" BIP_MSG_PRE_ARG, (void *)pDclItem, BIP_STATUS_ARG(rc) ));
    return rc;

error:
    BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: [Error] Returning: "BIP_STATUS_FMT"" BIP_MSG_PRE_ARG, (void *)pDclItem, BIP_STATUS_ARG(rc) ));
    return rc;
   ...
   )
{
    ...
    if (hArb->hMyArbList) {
        ...
        B_Mutex_Lock(hArbList->hArbListMutex);
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArbList %p: Removing Arb: %p from Accepted list" BIP_MSG_PRE_ARG, (void *)hArbList, (void *)hArb));
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Arb state is Accepted, changing to Completed" BIP_MSG_PRE_ARG, (void *)hArb));
    hArb->state = BIP_ArbState_eCompleted;

    if (hArb->completionCallback.callback) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Queuing completionCallback" BIP_MSG_PRE_ARG, (void *)hArb));
        BIP_Arb_AddDeferredCallback(hArb, &hArb->completionCallback);
    }

    if (hArb->hArbEvent) {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hArb %p: Setting hArbEvent" BIP_MSG_PRE_ARG, (void *)hArb));
...
static void BIP_Arb_SchedulerCallbackWrapper(void *pContext)
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: Calling callback via Scheduler..." BIP_MSG_PRE_ARG, (void *)pDclItem));
    pCallbackDesc->callback(pCallbackDesc->context, pCallbackDesc->param);
    BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: Returned from callback for Deferred Completion List item" BIP_MSG_PRE_ARG, (void *)pDclItem ));

    BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: Removing item from Deferred Completion List" BIP_MSG_PRE_ARG, (void *)pDclItem ));
    B_Mutex_Lock(pArbFactory->hDclMutex);
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: Freeing DCL item memory" BIP_MSG_PRE_ARG, (void *)pDclItem ));
    B_Os_Free( pDclItem );
}
   ...
   )
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: Entry..." BIP_MSG_PRE_ARG, (void *)pDclItem ));

    BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: Creating timer to expire immediately" BIP_MSG_PRE_ARG, (void *)pDclItem ));
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: Timer created" BIP_MSG_PRE_ARG, (void *)pDclItem ));
   ...
   )
{
    ...
    for (;;) {
        ...
        for (pDclItem = BLST_Q_FIRST(&pArbFactory->dclItem_head) ; pDclItem != NULL ; pDclItem = BLST_Q_NEXT(pDclItem, dclItem_next)) {
            ...
            BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: Found Deferred Completion List item: callbackBusy: %d" BIP_MSG_PRE_ARG, (void *)pDclItem, pDclItem->callbackBusy ));
        ...
        BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: Calling callback for Deferred Completion List item" BIP_MSG_PRE_ARG, (void *)pDclItem ));

        if ((threadOrigin & BIP_Arb_ThreadOrigin_eMaybeUpstreamLocks) != 0) {
            BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: Calling callback using B_Os_Scheduler..." BIP_MSG_PRE_ARG, (void *)pDclItem ));
            rc = BIP_Arb_FireCallbackWithScheduler(pDclItem);
        }
        else
        {
            BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: Calling callback Directly..." BIP_MSG_PRE_ARG, (void *)pDclItem ));
            pCallbackDesc->callback(pCallbackDesc->context,pCallbackDesc->param);
            BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: Returned from callback for Deferred Completion List item" BIP_MSG_PRE_ARG, (void *)pDclItem ));

            BDBG_MSG(( BIP_MSG_PRE_FMT "pDclItem %p: Removing Deferred Completion List item" BIP_MSG_PRE_ARG, (void *)pDclItem ));
            B_Mutex_Lock(pArbFactory->hDclMutex);
            BLST_Q_REMOVE(&pArbFactory->dclItem_head, pDclItem, dclItem_next);
            B_Mutex_Unlock(pArbFactory->hDclMutex);
