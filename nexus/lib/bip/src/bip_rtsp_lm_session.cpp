    ...
    )
{
    ...
    BDBG_MSG(( "%s: hRtspLmSession %p", __FUNCTION__, hRtspLmSession ));
    ...
    )
{
    ...
    if (hRtspLmSession)
    {
        if ( hRtspLmSession->pidList ) {
            BDBG_MSG(( "%s: Free pidList (%p)", __FUNCTION__, hRtspLmSession->pidList ));
            BKNI_Free( hRtspLmSession->pidList );
        }

        if ( hRtspLmSession->lmRtspClientSession  )
        {
            BDBG_MSG(( "%s: delete hRtspLmSession->lmRtspClientSession (%p)", __FUNCTION__, hRtspLmSession->lmRtspClientSession ));
            delete hRtspLmSession->lmRtspClientSession;
        }

        BDBG_MSG(( "%s: Free hRtspLmSession (%p)", __FUNCTION__, hRtspLmSession ));
        BKNI_Free( hRtspLmSession );
    }
}
...
{
    BDBG_MSG(( "%s: hRtspLmSession (%p)", __FUNCTION__,  hRtspLmSession ));
    BDBG_MSG(( "%s: hRtspLmSession->lmRtspClientSession (%p)", __FUNCTION__,  hRtspLmSession->lmRtspClientSession ));
    BDBG_MSG(( "%s: sessionId %08X; calling hRtspLmSession->lmRtspClientSession->copyReceivedMessage(%p)", __FUNCTION__,
 ...
 **/
    ...
    if (freqMhz >= 10700 && freqMhz < 11700)
    {
        ...
        pSatelliteSettings->frequency = interFreqMhz * 1000000;
         ...
         BDBG_MSG(("Universal(Astra)LNB low band case: transponder freq in Mhz %lu, L-Band Freq in Mhz %d ", freqMhz, interFreqMhz));
    ...
    else if(freqMhz >=11700 && freqMhz <= 12750)
    {
         ...
         BDBG_MSG(("Universal(Astra)LNB high band case: transponder freq in Mhz %lu, L-Band Freq in Mhz %d ", freqMhz, freqMhz - 10600));
    ...
    else if (strcmp( clientSession->satelliteSettings.msys, "dvbs2" ) == 0)
    {
        ...
        {
            BDBG_ERR(( "%s:if msys is set mtype must be set. Error", __FUNCTION__, clientSession->satelliteSettings.mtype ));
            goto error;
        }
    ...
    {
        BDBG_ERR(( "%s:  Sat Modulation mode %s is not yet supported or not set", __FUNCTION__, pSatelliteSettings->mode ));
        goto error;
    }
    ...
    {
        BDBG_MSG(( "%s: roll_off value not set to 0.35,0.25, or 0.20. Defaulting to 0.20", __FUNCTION__,clientSession->satelliteSettings.ro ));
        pSatelliteSettings->nyquist20 = true;
    }
