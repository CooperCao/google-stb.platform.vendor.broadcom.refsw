...
error_play:
    ...
    if (hStreamer->hRecpump)
    {
        ...
        BDBG_WRN(( "RP: state=%s dataFifo=%zu / %zu indexFifo=%zu/%zu hasIdx=%s raveIdx=%d total=%"PRIu64,
...
{
    BIP_StreamerHandle    hStreamer = appCtx;

    BSTD_UNUSED(param);
    BDBG_ASSERT( hStreamer );

    BDBG_MSG(( BIP_MSG_PRE_FMT "hStreamer %p: Received endOfStream Callback from Nexus Playback" BIP_MSG_PRE_ARG, (void *)hStreamer));

    if ( hStreamer && hStreamer->hRecpump )
    {
        BDBG_MSG(( BIP_MSG_PRE_FMT "hStreamer %p: Calling NEXUS_Recpump_StopData()" BIP_MSG_PRE_ARG, (void *)hStreamer));
        NEXUS_Recpump_StopData(hStreamer->hRecpump);
    }
    ...
    )
{
    ...
    /* Now associate Playpump w/ the Playback */
    {
        ...
        if (playbackSettings.endOfStreamAction != NEXUS_PlaybackLoopMode_eLoop)
        {
            playbackSettings.endOfStreamCallback.callback = endOfStreamCallbackFromPlayback;
            playbackSettings.endOfStreamCallback.context = hStreamer;
        }
        else
        {
