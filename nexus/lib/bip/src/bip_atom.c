...
userFreeForBipString(batom_t atom, void *user)
{
    ...
    BSTD_UNUSED(atom);
    BDBG_MSG(( BIP_MSG_PRE_FMT "Destroying BIP_String=%p for atom=%p." BIP_MSG_PRE_ARG, hString, atom ));
...
userFreeForBOsMalloc(batom_t atom, void *user)
{
    void *ptr = *(void **)user;
    BSTD_UNUSED(atom);
    BDBG_MSG(( BIP_MSG_PRE_FMT "Freeing B_Os_Malloc memory=%p for atom=%p." BIP_MSG_PRE_ARG, ptr, atom ));
    B_Os_Free(ptr);
    return;
}
...
BIP_Atom_AtomFromBipString(batom_factory_t factory, BIP_StringHandle hString)
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "Created atom aString1=%p" BIP_MSG_PRE_ARG, aString ));
    return aString;
}
...
BIP_Atom_AtomFromDupedRange(batom_factory_t factory, const void *base, size_t len)
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT "Created atom aRange=%p" BIP_MSG_PRE_ARG, aRange ));
