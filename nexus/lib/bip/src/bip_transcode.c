...
static void
audioPtsErrorCallback(void *context, int param)
{
    BIP_TranscodeHandle hTranscode = context;
    BSTD_UNUSED( param );
    BDBG_WRN(( BIP_MSG_PRE_FMT "pTranscode=%p" BIP_MSG_PRE_ARG, (void *)hTranscode ));
    ...
    )
{
    ...
    /* Decoders Settings. */
    {
        ...
        if ( !hTranscode->transcodeProfile.disableAudio )
        {
            ...
            NEXUS_SimpleAudioDecoder_GetSettings( hTranscode->nexusHandles.simple.hAudio, &audioSettings );
            audioSettings.primary.firstPts.callback = audioFirstPtsCallback;
            audioSettings.primary.firstPts.context = hTranscode;
            audioSettings.primary.firstPts.param = 0;
            audioSettings.primary.ptsError.callback = audioPtsErrorCallback;
            audioSettings.primary.ptsError.context = hTranscode;
            audioSettings.primary.ptsError.param = 0;
            nrc = NEXUS_SimpleAudioDecoder_SetSettings( hTranscode->nexusHandles.simple.hAudio, &audioSettings );
    ...
    {
        {
            ...
            if ( !hTranscode->transcodeProfile.disableAudio )
            {
                ...
                BKNI_Memcpy(&encoderSettings.audioEncoder,&hTranscode->transcodeProfile.audio.settings,sizeof(NEXUS_AudioEncoderCodecSettings));
            }
...

        {
            ...
            if ( !hTranscode->transcodeProfile.disableAudio )
            {
                ...
                else
                {
                    ...
                    switch(startSettings.output.audio.codec)
                    {
                    case NEXUS_AudioCodec_eAacAdts:
                    case NEXUS_AudioCodec_eAacLoas:
                        if(hTranscode->transcodeProfile.audio.settings.codecSettings.aac.sampleRate != 0)
                        {
                            startSettings.output.audio.sampleRate = hTranscode->transcodeProfile.audio.settings.codecSettings.aac.sampleRate;
                        }
                        break;
                    case NEXUS_AudioCodec_eAacPlusAdts:
                    case NEXUS_AudioCodec_eAacPlusLoas:
                        if(hTranscode->transcodeProfile.audio.settings.codecSettings.aacPlus.sampleRate != 0)
                        {
                            startSettings.output.audio.sampleRate = hTranscode->transcodeProfile.audio.settings.codecSettings.aacPlus.sampleRate;
                        }
                        break;
                    default:
                        /* Ignore the sample rate of other codecs */
                        break;
                    }
                }
            }
