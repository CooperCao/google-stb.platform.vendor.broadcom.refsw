    ...
    )
{
    ...
    BDBG_MSG(( BIP_MSG_PRE_FMT " Success: hFrontend (%p)" BIP_MSG_PRE_ARG, hFrontend));
    ...
    )
{
    ...
    BIP_CHECK_GOTO(( nrc == NEXUS_SUCCESS ), ( "hHttpStreamer %p, state %s: NEXUS_Recpump_AddPidChannel() Failed, nrc %d", nrc), error, BIP_ERR_NEXUS, bipStatus );
    ...
    )
{
    ...
    /* Now Set any custom or app specific HTTP Headers that HttpStreamer should include when it sends out the Response Message. */
    {
        ...
        BIP_CHECK_GOTO(( bipStatus == BIP_SUCCESS || bipStatus == BIP_INF_HTTP_MESSAGE_HEADER_ALREADY_SET),
                ( "BIP_HttpStreamer_SetResponseHeader Failed: hHttpStreamer %p", pAppStreamerCtx->hHttpStreamer ), rejectRequest, bipStatus, responseStatus );
    ...
    /* And as a last step, start the HttpStreamer. */
    {
        ...
        responseStatus = BIP_HttpResponseStatus_e500_InternalServerError;
        BIP_CHECK_GOTO(( bipStatus == BIP_SUCCESS ), ( "BIP_HttpServer_StartStreamer Failed: hHttpStreamer %p", pAppStreamerCtx->hHttpStreamer ), rejectRequest, bipStatus, responseStatus );
    }
    return (bipStatus);
