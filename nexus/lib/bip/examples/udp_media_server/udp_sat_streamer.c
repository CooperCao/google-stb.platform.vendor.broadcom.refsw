...
#define UDP_SERVER_SAT_VOLTAGE         NEXUS_FrontendDiseqcVoltage_e13v
    ...
    )
{
...

    {
        ...
        runtimeSettings.selectedAdc = pAppStreamerCtx->adc;
    ...
    satSettings.frequency = pAppStreamerCtx->frequency;
    satSettings.mode      = pAppStreamerCtx->mode;
    satSettings.symbolRate = pAppStreamerCtx->symbolRate;
#if 0
    ...
    if (diseqcSupport)
    {
        ...
        diseqcSettings.toneEnabled = pAppStreamerCtx->toneEnabled;
        diseqcSettings.toneMode    = UDP_SERVER_SAT_TONE_MODE;
        diseqcSettings.voltage     = pAppStreamerCtx->diseqcVoltage;
...
void printUsage( char *pCmdName )
{
    ...
    printf(
        ...
        "  -freq        #   frequency in MHz (default is 1119 Mhz)\n"
        "  -mode        #   ( default is Dvbs2Qpsk )\n"
...
AppStreamerCtx *initAppStreamerCtx( void )
{
    ...
    pAppStreamerCtx->adc = 0;
    pAppStreamerCtx->frequency = UDP_SERVER_SAT_FREQ;
    pAppStreamerCtx->mode = UDP_SERVER_SAT_MODE;
    pAppStreamerCtx->symbolRate = UDP_SERVER_SAT_SYM_RATE;
    pAppStreamerCtx->streamerProtocol = BIP_UdpStreamerProtocol_ePlainUdp;
    pAppStreamerCtx->diseqcVoltage = UDP_SERVER_SAT_VOLTAGE;
    pAppStreamerCtx->toneEnabled = UDP_SERVER_SAT_TONE_ENABLED;
