...
uint64_t B_Time_DiffMicroseconds(const B_Time *pFuture, const B_Time *pPast)
{
    BDBG_ASSERT(NULL != pFuture);
    BDBG_ASSERT(NULL != pPast);

    return (uint64_t)1000000*(uint64_t)(pFuture->tv_sec - pPast->tv_sec) + (uint64_t)(pFuture->tv_usec - pPast->tv_usec);
}
