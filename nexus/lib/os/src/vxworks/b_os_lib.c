...
B_Error B_Os_Init(void)
{
    ...
    if (!g_initMutex) {
        g_initMutex = semMCreate(SEM_Q_FIFO);
        if (!g_initMutex) return B_ERROR_OS_ERROR;
    }

    rc = semTake(g_initMutex, WAIT_FOREVER);
    if (rc) return B_ERROR_OS_ERROR;
