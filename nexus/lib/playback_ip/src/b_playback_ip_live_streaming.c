           ...
           /* Send Null Packets for SES SATIP spec when no signal is being received,
            ...
            if (rc != 0  && liveStreamingHandle->settings.sendNullRtpPktsOnTimeout) {
                if(streamingNull == false)
                {
                    BDBG_WRN(("%s: Send Null RtspPacket(timeout Interval %d).No data in Rave.\n Most likely signal lost(sat coax unplugged) or invalid pids or zero pids specified",
                          __FUNCTION__, liveStreamingHandle->settings.dataReadyTimeoutInterval ));
                    streamingNull = true;
                }
                ...
                BDBG_ERR(("%s: NEXUS_Recpump_GetDataBuffer failed, breaking out of streaming loop rc %d", __FUNCTION__,rc));
                gotErrorInStreamingLoop = true;
                break;
            }
            ...
            /* check buffer for missing sync bytes */
            {
                ...
                for (idx=0; idx<bytesRead; idx+=188) {
                    if (readBuf[idx] != 0x47) {
                        BDBG_ERR(("%s: sync error ... bytesToRead %d, bytesRead %d, totalBytesRead %d", __FUNCTION__, bytesToRead, bytesRead, totalBytesRead));
                        B_PlaybackIp_UtilsDumpBuffer ( __FUNCTION__, &readBuf[idx], 48 );
                    }
                }
            }
#endif
    ...
    )
{
    ...
    if (enableRecording) {
        ...
        BDBG_MSG(("%s: recorded %d bytes of PAT & %d bytes of PMT", __FUNCTION__, HLS_PAT_PMT_BUFFER_SIZE, HLS_PAT_PMT_BUFFER_SIZE));
    }
    return 0;
}
    ...
    *bytesRead = 0;
    while (true)
    {
        ...
        if (NEXUS_Recpump_GetDataBuffer(liveStreamingHandle->recpumpHandle, readBuf, bytesRead) != NEXUS_SUCCESS) {
            BDBG_ERR(("%s: NEXUS_Recpump_GetDataBuffer failed, breaking out!", __FUNCTION__));
            break;
        }
    ...
    )
{
    ...
    while (!liveStreamingHandle->stop) {
        ...
        if (rc != 0 && rc != BERR_TIMEOUT) {
            /* error case, we are done */
            BDBG_WRN(("%s: dataReadyEvent ERROR (%d), breaking out of initial loop ", __FUNCTION__, rc));
            break;
        }
    ...
    while (!liveStreamingHandle->stop)
    {
        ...
        if (rc != B_ERROR_SUCCESS) {
            /* error case, we are done */
            BDBG_ERR(("%s: failed to read data from recpump, not breaking out of streaming loop, but continuing for app to stop the streaming ", __FUNCTION__));
        ...
        while (bytesToWrite && totalBytesWritten < bytesToWrite)
        {
            ...
            if (rc != B_ERROR_SUCCESS) {
                /* error case, we are done */
                BDBG_ERR(("%s: failed to read data from recpump, breaking out of streaming loop ", __FUNCTION__));
                gotErrorInStreamingLoop = true;
                break;
            }
        ...
        if (gopsSentInHlsSegment >= maxGopsInHlsSegment)
        {
            if (!liveStreamingSettings->dontCloseSocket) {
                ...
                close(streamingFd);
#endif
                BDBG_MSG(("%s: closed streamingFd %d", __FUNCTION__, streamingFd));
            }
    ...
    )
{
    ...
    BDBG_MSG(("%s: protocol (%s); socket current %u, updated %u", __FUNCTION__,  (liveStreamingSettings->protocol==B_PlaybackIpProtocol_eHttp)?"HTTP":
    ...
    )
{
    ...
    BDBG_MSG(("%s: protocol (%s); socket %u", __FUNCTION__,  (liveStreamingSettings->protocol==B_PlaybackIpProtocol_eHttp)?"HTTP":
 (liveStreamingSettings->protocol==B_PlaybackIpProtocol_eRtp)?"RTP":
              (liveStreamingSettings->protocol==B_PlaybackIpProtocol_eUdp)?"UDP":
    ...
    default:
        BDBG_ERR(("%s: non-supported socket type", __FUNCTION__ ));
        break;
    }
