...
B_PlaybackIp_HlsDownloadFile(B_PlaybackIpHandle playback_ip, int *pFd, char *buffer, int bufferSize, int *totalBytesRead, bool nullTerminate)
{
    ...
    while (true) {
        if ( breakFromLoop(playback_ip)) {
            BDBG_MSG(("%s: breaking file download loop due to state (%d) change", __FUNCTION__, playbackIpState(playback_ip)));
            break;
        }
    ...
    )
{
    ...
    if (bytesToRead <= 0) {
        BDBG_ERR(("%s: invalid bytesToRead %d for fd %d", __FUNCTION__, bytesToRead, fd));
        return false;
    }
    *serverClosed = false;
    ...
    while (true) {
        if ( breakFromLoop(playback_ip) || playback_ip->playback_state == B_PlaybackIpState_eEnteringTrickMode) {
            BDBG_MSG(("%s: breaking file download loop due to state ", __FUNCTION__));
            break;
        }
...
#define TCP_CONNECT_TIMEOUT_INTERVAL 1
            ...
            while (retryCount++ < maxRetryCount) {
                ...
                if (rc == B_ERROR_SUCCESS) {
                    ...
                    BDBG_ERR(("%s: ERROR: failed to send Socket Connect Request to Server: %s:%d, Retry it: retryCount %d, Max retryCount %d", __FUNCTION__, server, port, retryCount, maxRetryCount));
                }
            }
            if (rc != B_ERROR_SUCCESS) {
        ...
        if ((tag = B_PlaybackIp_UtilsStristr(nextLine, HLS_M3U8_VERSION_TAG)) != NULL) {
                ...
                if (B_PlaybackIp_UtilsStristr(attribute, HLS_M3U8_TAG_ATTRIBUTE_ENC_METHOD) != NULL) {
                    ...
                    if (http_absolute_uri(value)) {
                        ...
                        if ((encryptionKeyUri = B_PlaybackIp_UtilsStrdup(value)) == NULL) {
                            BDBG_ERR(("memory allocation for enc key failed at %s:%d", __FUNCTION__, __LINE__));
                            goto error;
                        }
                    }
                    else {
            ...
            if (mediaFileSegmentInfo->protocol == B_PlaybackIpProtocol_eHttps) {
                ...
                if (encryptionInfo.useMediaSegSeqAsIv == true) {
                    ...
                    BDBG_MSG(("%s: explicit IV not set, using seq # %d as iv: iv[0] %02x, iv[1] %02x, iv[2] %02x, iv[3] %02x", __FUNCTION__, mediaSequence,
    ...
    )
{
    ...
    if ( (playlistFileInfo->bounded == false && playbackIpState(playback_ip) == B_PlaybackIpState_ePlaying) ||
        ...
        BDBG_MSG(("%s: updating the duration with the newly updated one from the playlist to %d", __FUNCTION__, playback_ip->psi.duration));
    }
...
#ifdef BDBG_DEBUG_BUILD
                if (playback_ip->ipVerboseLog)
                    BDBG_WRN(("%s: HLS Session: Quick Probing for TS format", __FUNCTION__));
#endif
...
#ifdef BDBG_DEBUG_BUILD
                if (playback_ip->ipVerboseLog)
                    BDBG_WRN(("%s: HLS Session: Full Probing for TS format", __FUNCTION__));
#endif
            }
        }
        else {
    ...
    )
{
    ...
    if ((mediaProbe->segmentBuffer = BKNI_Malloc(sizeof(HlsSegmentBuffer))) == NULL) {
        BDBG_ERR(("%s: memory allocation failure for HlsSegmentBuffer structure", __FUNCTION__));
        return -1;
    }
    ...
    )
{
    ...
    BDBG_WRN(("%s: Downloaded & parsed playlist w/ uri=%s", __FUNCTION__, playlistFileInfo->absoluteUri));
    ...
    BDBG_MSG(("%s: SetupSession for downloading a media segment uri=%s", __FUNCTION__, mediaFileSegmentInfo->absoluteUri));
    ...
    if (B_PlaybackIp_HlsDownloadMediaSegment(playbackIp, playbackIp->socketState.fd, segmentBuffer->buffer+segmentBuffer->bufferDepth, segmentBuffer->bufferSize, bytesToRead, &segmentBuffer->bufferDepth, &networkBandwidth, &serverClosed) != true) {
        BDBG_ERR(("%s: failed to download the current media segment of an alternate rendition for media probe!", __FUNCTION__));
    ...
    BDBG_MSG(("%s: Downloaded 1st media segment w/ uri=%s", __FUNCTION__, mediaFileSegmentInfo->absoluteUri));
...
B_PlaybackIp_ParsePlaylistVariantFile(B_PlaybackIpHandle playback_ip, bool *parsingSuccess)
{
    ...
    *parsingSuccess = false;
    ...
    if ((tag = B_PlaybackIp_UtilsStristr(nextLine, HLS_M3U8_VERSION_TAG)) != NULL) {
        ...
        version = strtol(tag, NULL, 10);
        if (version > PLAYLIST_FILE_COMPATIBILITY_VERSION) {
            BDBG_WRN(("%s: Not all features of HLS Version %d are currently supported %d", __FUNCTION__, version, PLAYLIST_FILE_COMPATIBILITY_VERSION));
        }
        versionTagFound = true;
                ...
                *tmp = '\0'; /* null terminate attribute to convert it into a string */
                ...
                if (B_PlaybackIp_UtilsStristr(attribute, HLS_M3U8_TAG_ATTRIBUTE_TYPE) != NULL) {
                    if (B_PlaybackIp_UtilsStristr(value, "AUDIO") != NULL)
                        ...
                        BDBG_ERR(("%s: Unsupported value=%s for TYPE Attribute ", __FUNCTION__, value ));
                        /* TODO: goto free up current entry and move to the next program entry */
                        goto error;
                    }
                    BDBG_MSG(("%s: AltRendition TYPE %s", __FUNCTION__,
                    ...
                    BDBG_MSG(("%s: Default attribute %s=%s", __FUNCTION__, HLS_M3U8_TAG_ATTRIBUTE_DEFAULT, altRenditionInfo->defaultRendition?"YES":"NO"));
                }
                    ...
                    BDBG_MSG(("%s: Default attribute %s=%s", __FUNCTION__, HLS_M3U8_TAG_ATTRIBUTE_AUTOSELECT, altRenditionInfo->autoSelect?"YES":"NO"));
                }
                    ...
                    BDBG_MSG(("%s: Variant Playlist: server %s, port %d, protocol %d, uri %s", __FUNCTION__, altRenditionInfo->server, altRenditionInfo->port, altRenditionInfo->protocol, altRenditionInfo->uri));
                }
                else {
                    BDBG_ERR(("%s: unsupported/unknown attribute %s=%s, ignoring it for now!!", __FUNCTION__, attribute, value));
                }
...
B_PlaybackIp_HlsSetupNextMediaSegment(B_PlaybackIpHandle playback_ip)
{
    ...
    BDBG_MSG(("%s: choosing next media segment: sequence # %d", __FUNCTION__, mediaFileSegmentInfo->mediaSequence));
    ...
    if (mediaFileSegmentInfo) {
        if (B_PlaybackIp_HlsSetupHttpSessionToServer(playback_ip, mediaFileSegmentInfo, 0 /*persistentHttpSession*/, 1 /*reOpenSocketConnection */, &playback_ip->socketState.fd, playback_ip->temp_buf, &playback_ip->initial_data_len) == false) {
            BDBG_ERR(("%s: ERROR: Socket setup or HTTP request/response failed for downloading Media Segment", __FUNCTION__));
            return false;
        }
        ...
        BDBG_ERR(("%s: ERROR: Failed to download any media segment of playlist file (uri %s) for media probe", __FUNCTION__, hlsSession->currentPlaylistFile->absoluteUri));
        status = false;
    }
    return status;
}
...
doMediaProbeOnAltRenditions(B_PlaybackIpHandle playback_ip)
{
        ...
        )
    {
        ...
        if ((playlistFileInfo->absoluteUri = B_PlaybackIp_UtilsStrdup(altRenditionInfo->absoluteUri)) == NULL) {
            BDBG_ERR(("Failed to duplicate the alternateRendition URI at %s:%d", __FUNCTION__, __LINE__));
            goto error;
        }

        /* parse the URI into individual fields. */
        if ((hls_parse_url(&playlistFileInfo->protocol, &playlistFileInfo->server, &playlistFileInfo->port, &playlistFileInfo->uri, playlistFileInfo->absoluteUri) == false) || (playlistFileInfo->protocol != B_PlaybackIpProtocol_eHttp && playlistFileInfo->protocol != B_PlaybackIpProtocol_eHttps)) {
            BDBG_ERR(("Failed to parse URI into sub-fields at %s:%d", __FUNCTION__, __LINE__));
            goto error;
        }
        ...
        if (B_PlaybackIp_HlsMediaProbeCreate(&hlsSession->mediaProbe, playback_ip->indexCache, playback_ip->indexCacheSize) != 0) {
            BDBG_ERR(("%s: Failed to create the Media Probe context for HLS sessions ", __FUNCTION__));
            goto error;
        }
        ...
        if (B_PlaybackIp_HlsConnectDownloadAndParsePlaylistFileAnd1stSegment(playback_ip, hlsSession, playlistFileInfo, hlsSession->mediaProbe.segmentBuffer) < 0) {
            BDBG_ERR(("%s: Failed to download & parse playlist file entry w/ uri %s", __FUNCTION__, playlistFileInfo->absoluteUri));
            goto error;
        }

        if (B_PlaybackIp_HlsMediaProbeStart(playback_ip, &hlsSession->mediaProbe, hlsSession->mediaProbe.segmentBuffer) != true) {
            BDBG_ERR(("%s: Media Probe failed during the initial probe on alternate rendition.", __FUNCTION__));
            goto error;
        }
...
#define PLAYBACK_IP_MAX_NETWORK_BANDWIDTH 5800000 /* set to 1.8Mpbs */
    ...
    {
        if ( breakFromLoop(playback_ip)) {
            BDBG_MSG(("%s: breaking out of 1st HLS Media Segment Setup loop due to state (%d) change", __FUNCTION__, playbackIpState(playback_ip)));
            goto error;
        }
        if (B_PlaybackIp_HlsSetupHttpSessionToServer(playback_ip, mediaFileSegmentInfo, 0 /*persistentHttpSession*/, 1 /*reOpenSocketConnection */, &playback_ip->socketState.fd, playback_ip->temp_buf, &playback_ip->initial_data_len) == false) {
            BDBG_ERR(("%s: ERROR: Socket setup or HTTP request/response failed for downloading Media Segment, retrying next media segment", __FUNCTION__));
    ...
    if ( breakFromLoop(playback_ip)) {
        BDBG_MSG(("%s: Returning from HlsSessionSetup due to state (%d) change", __FUNCTION__, playbackIpState(playback_ip)));
        goto error;
    }
...
#define HLS_INTERNAL_PROBE
...
#ifdef HLS_INTERNAL_PROBE
    {
        ...
        if (playback_ip->initial_data_len)
        {
            ...
            BDBG_MSG(("%s: copied %u bytes of initial buffer", __FUNCTION__, playback_ip->initial_data_len));
        }
        ...
        if (B_PlaybackIp_HlsDownloadMediaSegment(playback_ip, playback_ip->socketState.fd,
            ...
            BDBG_ERR(("%s: failed to download the current media segment", __FUNCTION__));
            if (playback_ip->securityHandle) {
                playback_ip->netIo.close(playback_ip->securityHandle);
                playback_ip->securityHandle = NULL;
            }
        ...
        if (B_PlaybackIp_HlsMediaProbeStart(playback_ip, &hlsSession->mediaProbe, segmentBuffer) != true) {
            BDBG_ERR(("%s: Media Probe failed during the initial probe", __FUNCTION__));
            goto error;
        }
    ...
    )
{
    ...
    *pUseDifferentPlaylist = false;
    ...
    if (1)
    {
        ...
        {
            if (playlistFileInfo->segmentCodecType == HlsSegmentCodecType_eAudioOnly) {
                BDBG_WRN(("%s: switch to audioOnly segment (audioOnly env variable is set!)", __FUNCTION__));
         ...
         prevPlaylistFileInfo=playlistFileInfo, playlistFileInfo = BLST_Q_NEXT(playlistFileInfo, next))
    {
        ...
        if (playbackIpState(playback_ip) == B_PlaybackIpState_eTrickMode)
        {
...
#ifdef BDBG_DEBUG_BUILD
                    if (playback_ip->ipVerboseLog)
                        BDBG_WRN(("%s: useLowestBitRateSegmentOnly is set, state %d, playlist file: uri %s, b/w %d, network Bandwidth %d", __FUNCTION__,
            ...
            else
            {
                ...
                if (playlistFileInfo->bandwidth > hlsSession->playlistBandwidthToUseInTrickmode) {
                    ...
                    BDBG_ERR(("%s: Trickmode case: using playlist file: uri %s, b/w %d, desired b/w %d, network Bandwidth %d", __FUNCTION__,
...
#ifdef BDBG_DEBUG_BUILD
            if (playback_ip->ipVerboseLog)
                BDBG_WRN(("%s: restartPlaylistRampUpByBandwidth is set, state %d, playlist file: uri %s, b/w %d, network Bandwidth %d", __FUNCTION__,
                            playbackIpState(playback_ip), playlistFileInfo->uri, playlistFileInfo->bandwidth, networkBandwidth));
#endif
        ...
        if (playlistFileInfo->bandwidth > networkBandwidth ) {
            ...
            BDBG_MSG(("%s: found playlist entry whose b/w %d exceeds the network b/w %d, use previous playlist entry w/ b/w %d", __FUNCTION__, playlistFileInfo->bandwidth, networkBandwidth, prevPlaylistFileInfo->bandwidth));
            playlistFileInfo = prevPlaylistFileInfo;
            break;
        }
        ...
        /* enable this define if we want to linearly go up thru various bitrates instead of directly using highest bitrate allowed by the network. */
            ...
            BDBG_MSG(("%s: found playlist entry whose b/w %d exceeds the current b/w %u, network b/w %d", __FUNCTION__, playlistFileInfo->bandwidth, hlsSession->currentPlaylistBandwidth, networkBandwidth));
            break;
        }
#endif
        else {
    ...
    if (*pUseDifferentPlaylist == true || hlsSession->currentPlaylistFile->playlistHasChanged == true) {
...
#ifdef BDBG_DEBUG_BUILD
                    if (playback_ip->ipVerboseLog) {
                        BDBG_MSG(("%s: Rew case: Found next media segment using seq: asked %d, got %d", __FUNCTION__, currentMediaSegmentSequenceNumber, mediaFileSegmentInfo->mediaSequence));
...
#ifdef BDBG_DEBUG_BUILD
                    if (playback_ip->ipVerboseLog) {
                        BDBG_MSG(("%s: Fwd case: Found next media segment using seq: asked %d, got %d", __FUNCTION__, currentMediaSegmentSequenceNumber, mediaFileSegmentInfo->mediaSequence));
                        BDBG_MSG(("uriReq %s", mediaFileSegmentInfo->uri));
                    }
#endif
        ...
        else
        {
            ...
            for (mediaFileSegmentInfo = BLST_Q_FIRST(&playlistFileInfo->mediaFileSegmentInfoQueueHead);
                    ...
                    mediaFileSegmentInfo = BLST_Q_NEXT(mediaFileSegmentInfo, next))
            {
                BDBG_MSG_FLOW(("%s: segmentDuration %u, desired downloadedSegmentsDuration %d, uri %s", __FUNCTION__, segmentDuration, downloadedSegmentsDuration, mediaFileSegmentInfo->uri));
    ...
    )
{
    if (playback_ip->nexusHandles.simpleStcChannel) {
        if (NEXUS_SimpleStcChannel_SetRate(playback_ip->nexusHandles.simpleStcChannel, rate, 0 ) != NEXUS_SUCCESS) {
            BDBG_ERR(("%s: ERROR: failed to pause by setting stc rate to %d", __FUNCTION__, rate));
        ...
        BDBG_ERR(("%s: ERROR: neither simpleStcChannel or stcChannel rate set to %d", __FUNCTION__, rate));
        return B_ERROR_UNKNOWN;
    }
#ifdef BDBG_DEBUG_BUILD
    if (playback_ip->ipVerboseLog)
        BDBG_WRN(("%s: Updated STC Channel rate to %d", __FUNCTION__, rate));
#endif
    ...
    )
{
    ...
    BDBG_MSG(("%s: NEXUS_Playpump_SetPause() set to %s", __FUNCTION__, pausePlaypump ? "set pause":"unset pause"));
    ...
    else
    {
        ...
        if (rate < 0)
        {
            ...
            BDBG_ERR(("%s: DQT Settings: enabled %s, decodeMode %d", __FUNCTION__, videoDecoderTrickSettings.dqtEnabled ? "Y":"N", videoDecoderTrickSettings.decodeMode));
        }
        else
        {
            videoDecoderTrickSettings.reverseFields = false;
...

    {
        ...
        BDBG_WRN(("%s: Set the Video Decoder PTS discard threshold to %d", __FUNCTION__, videoDecoderSettings.discardThreshold));
    }
#ifdef BDBG_DEBUG_BUILD
    if (playback_ip->ipVerboseLog)
...
#ifdef BDBG_DEBUG_BUILD
    if (playback_ip->ipVerboseLog)
        BDBG_WRN(("%s: flushing Playpump", __FUNCTION__));
#endif
    ...
    )
{
    ...
    if (playback_ip->nexusHandles.simpleStcChannel)
    {
        ...
        {
            BDBG_WRN(("%s: NEXUS_SimpleStcChannel_Invalidate returned error", __FUNCTION__));
        }
    }
#endif
    if (playback_ip->nexusHandles.videoDecoder)
    ...
    if (playback_ip->nexusHandles.primaryAudioDecoder)
    {
        ...
        {
            BDBG_WRN(("%s: NEXUS_AudioDecoder_Flush returned error", __FUNCTION__));
        }
    }
    ...
    )
{
    ...
    BDBG_MSG(("%s: segment download thread is paused", __FUNCTION__));
    ...
    if (hlsSession->hlsPlaylistReDownloadThread && !hlsSession->hlsPlaylistReDownloadThreadDone) {
        ...
        if (rc == BERR_TIMEOUT) {
            BDBG_ERR(("%s: EVENT timeout: failed to receive event from HLS PlaylistReDownload thread indicating its paused", __FUNCTION__));
            brc = B_ERROR_TIMEOUT;
            goto error;
        } else if (rc!=0) {
            BDBG_ERR(("%s: failed to wait for event from HLS PlaylistReDownload thread indicating its paused, rc = %d", __FUNCTION__, rc));
            brc = B_ERROR_UNKNOWN;
            goto error;
        }
        BDBG_MSG(("%s: PlaylistReDownload is paused", __FUNCTION__));
    ...
    BDBG_MSG(("%s: now both segment download and playpump feeder threads are paused, so do the seek related work", __FUNCTION__));

    playlistFileInfo = playback_ip->hlsSessionState->currentPlaylistFile;
    BDBG_MSG(("%s: segmentCodecType %d", __FUNCTION__, playlistFileInfo->segmentCodecType));
...
#ifdef BDBG_DEBUG_BUILD
    if (playback_ip->ipVerboseLog)
        BDBG_WRN(("%s: successful to %0.3f sec", __FUNCTION__, seekPosition/1000.));
#endif
    return B_ERROR_SUCCESS;
    ...
    )
{
    ...
    if (currentState == B_PlaybackIpState_eTrickMode) {
        ...
        if (rc != B_ERROR_SUCCESS) {
            BDBG_ERR(("%s: Failed to update VideoDecoder State during trickMode operation", __FUNCTION__));
            goto error;
        }
        ...
        if (rc != B_ERROR_SUCCESS)
        {
            BDBG_ERR(("%s: Failed to update AudioDecoder State during trickMode operation", __FUNCTION__));
            goto error;
        }
    ...
    )
{
    ...
    if (currentState == B_PlaybackIpState_eTrickMode || (currentState == B_PlaybackIpState_ePaused && playback_ip->isPausedFromTrickMode == true)) {
        ...
        {
            BDBG_WRN(("%s: Failed to Pause during Trick -> Play", __FUNCTION__));
            goto error;
        }
        ...
        BDBG_MSG(("%s: Resume: Trick -> Play from position %0.3f", __FUNCTION__, currentPosition/1000.));
        ...
        if (brc != B_ERROR_SUCCESS)
        {
            BDBG_ERR(("%s: Failed to update secondaryAudioDecoder State during play operation", __FUNCTION__));
            goto error;
        }
    ...
    B_PlaybackIpTrickModesSettings *pIpTrickModeSettings)
{
    ...
    brc = updateStcRate(playback_ip, 0);
    if (brc != B_ERROR_SUCCESS) {
        BDBG_ERR(("%s: Failed to Pause STC during trickMode operation", __FUNCTION__));
        goto error;
    }
    ...
    if (brc != B_ERROR_SUCCESS)
    {
        BDBG_WRN(("%s: Failed to determine the current playback position during HLS Rewind", __FUNCTION__));
        goto error;
    }
#ifdef BDBG_DEBUG_BUILD
    if (playback_ip->ipVerboseLog)
        BDBG_WRN(("%s: %s -> %s at %d rate from position %0.3f", __FUNCTION__,
    ...
    else
    {
                ...
                playlistFileInfo = BLST_Q_NEXT(playlistFileInfo, next))
        {
            ...
            BDBG_WRN(("%s: playlist file: uri %s, b/w %d, requiredNetworkBandwidth %d, network Bandwidth %d", __FUNCTION__, playlistFileInfo->uri, playlistFileInfo->bandwidth, requiredNetworkBandwidth, networkBandwidth));
        ...
        if (prevPlaylistFileInfo != NULL)
        {
            ...
            BDBG_WRN(("%s: %d for this playlist b/w %d at %d rate can be played within the n/w b/w of  %d, playing at highest possible quality",
                        __FUNCTION__, requiredNetworkBandwidth, playlistFileInfo->bandwidth, playbackRate, networkBandwidth));
        }
        else {
            ...
            if (playlistFileInfo == NULL)
            {
                ...
                else
                {
                    BDBG_WRN(("%s: no AV playlist available, so using 1st audio only playlist w/ b/w %d for this requiredNetworkBandwidth %d at the current networkBandwidth %d",
                                __FUNCTION__, playlistFileInfo->bandwidth, requiredNetworkBandwidth, networkBandwidth));
                }
            }
            ...
            BDBG_WRN(("%s: Can't play any playlists at this rate %d, lowering the rate to %d using 1st playlist of b/w %d, n/w b/w %d",
                        __FUNCTION__, playback_ip->speedNumerator, adjustedPlaybackRate, playlistFileInfo->bandwidth, networkBandwidth));
    ...
    brc = updateAudioDecoderTrickModeState(playback_ip, playback_ip->nexusHandles.primaryAudioDecoder);
    if (brc != B_ERROR_SUCCESS) {
        BDBG_ERR(("%s: Failed to update primaryAudioDecoder State during trickMode operation", __FUNCTION__));
        goto error;
    }

    brc = updateAudioDecoderTrickModeState(playback_ip, playback_ip->nexusHandles.secondaryAudioDecoder);
    if (brc != B_ERROR_SUCCESS) {
        BDBG_ERR(("%s: Failed to update secondaryAudioDecoder State during trickMode operation", __FUNCTION__));
        goto error;
    }
    ...
    brc = updateStcRate(playback_ip, 1);
    if (brc != B_ERROR_SUCCESS) {
        BDBG_ERR(("%s: Failed to Pause during trickMode operation", __FUNCTION__));
        goto error;
    }
#ifdef BDBG_DEBUG_BUILD
    if (playback_ip->ipVerboseLog)
    ...
    )
{
    ...
    bool bufferContainsNextSegment = true; /* indicates that next buffer contains start of segment, needed for pre-processing some meta data on the ES only segment case */
    ...
    while (true) {
        ...
        if (playback_ip->playback_state == B_PlaybackIpState_eWaitingToEnterTrickMode && hlsSession->seekOperationStarted) {
            BDBG_MSG(("%s: App is trying for a trickmode command, pause this thread", __FUNCTION__));
        ...
        if (previousMediaSegmentCompletelyDownloaded == true) {
            ...
            if (mediaFileSegmentInfo->protocol == B_PlaybackIpProtocol_eHttps) {
                ...
                BDBG_MSG(("%s: turn off persistent HTTP sessions for SSL protocol", __FUNCTION__));
            }
#ifdef BDBG_DEBUG_BUILD
            if (playback_ip->ipVerboseLog)
        ...
        if (playlistSwitched)
        {
            ...
            if (B_PlaybackIp_HlsMediaProbeStart(playback_ip, &hlsSession->mediaProbe, segmentBuffer) != true) {
                BDBG_ERR(("%s: Media Probe failed during the playlist switch", __FUNCTION__));
                BDBG_ASSERT(NULL);
                goto error;
            }
...
#ifdef BDBG_DEBUG_BUILD
                if (playback_ip->ipVerboseLog)
                    BDBG_WRN(("%s: Trickmode operation is pending, so pause our work until that is complete!", __FUNCTION__));
#endif
                BKNI_ResetEvent(hlsSession->reDownloadPlaylistEvent);
                continue;
            }
        ...
        if (newPlaylistFileInfo->mediaSegmentBaseSequence != -1) {
            ...
            if (newPlaylistFileInfo->mediaSegmentBaseSequence == playlistFileInfo->mediaSegmentBaseSequence && newPlaylistFileInfo->numMediaSegments == playlistFileInfo->numMediaSegments) {
                ...
                if (hlsSession->hlsPlaylistReDownloadThreadDone) {
                    BDBG_ERR(("%s: new playlist has not changed & playlist ReDownload thread has existed, so breaking out...", __FUNCTION__));
                    BKNI_ReleaseMutex(hlsSession->lock);
                    break;
                }
                ...
                if (playlistFileInfo->bounded) {
                    BDBG_WRN(("%s: updated playlist has END TAG set, stopping the ReDownloading thread as Live Event has reached towards its end", __FUNCTION__));
                    BKNI_ReleaseMutex(hlsSession->lock);
                    goto bounded;
                }
            }
...
#ifdef BDBG_DEBUG_BUILD
        if (playback_ip->ipVerboseLog)
        BDBG_WRN(("%s: HLS Playlist ReDownload thread is exiting as playlist is now bounded.", __FUNCTION__));
#endif
        return;
    }
...
#ifdef BDBG_DEBUG_BUILD
                if (playback_ip->ipVerboseLog)
                    BDBG_WRN(("%s: %s isn't changing, so we may be done playing the stream", __FUNCTION__, (playback_ip->nexusHandles.videoDecoder || playback_ip->nexusHandles.simpleVideoDecoder)? "Displaed Picture Count" : "Audio PTS"));
#endif
                /* reached EOF */
                status = true;
            }
...
setupPlaybackIpSessionForAltAudio(B_PlaybackIpHandle playback_ip, B_PlaybackIpHlsAltAudioRenditionInfo *altAudioRenditionInfo, NEXUS_PlaypumpHandle playpump2)
{
    ...
    {
        BDBG_MSG(("%s: current alternateRendition: language=%s URI=%s", __FUNCTION__, altRenditionInfo->language, altRenditionInfo->absoluteUri));
    ...
    if (altRenditionInfo == NULL) {
        BDBG_ERR(("%s: Didn't find the matching alternate audio rendition: language=%s groupId=%s containerType=%d", __FUNCTION__,
    ...
    }
    {
        ...
        if (!pSetupStatus) {
            BDBG_ERR(("%s: BKNI_Malloc() failed for B_PlaybackIpSessionSetupStatus", __FUNCTION__));
            goto error;
        }
    ...
    )
{
    ...
    while (true) {
        ...
        if (playback_ip->playback_state == B_PlaybackIpState_eWaitingToEnterTrickMode && hlsSession->seekOperationStarted) {
            ...
            nextSegmentBufferIndex = 0;
            BDBG_MSG(("%s: resuming HLS Playback loop after trickmode command", __FUNCTION__));
        }
        ...
        while (!segmentBuffer) {
            ...
            if (!segmentBuffer) {
                if (hlsSession->downloadedAllSegmentsInCurrentPlaylist &&
                    ...
                    BDBG_MSG(("%s: No more segments available, check if we reaching the end (normal or fwd) or beginning (rew) of the stream, state %d", __FUNCTION__, playback_ip->playback_state));
                ...
                BDBG_MSG(("%s: wait %d msec for a signal from download thread", __FUNCTION__, HLS_EVENT_BUFFER_TIMEOUT_MSEC));
        ...
        if (playback_ip->ipVerboseLog) {
            if (segmentBuffer->markedDiscontinuity) {
                BDBG_WRN(("%s: TODO: this segment has marked discontinuity: need to check what can change & accordingly modify the av pipeline!!", __FUNCTION__));
            }
        }
#endif
