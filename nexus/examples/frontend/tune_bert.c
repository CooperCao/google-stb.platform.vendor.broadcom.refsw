...
int main(int argc, char **argv)
{
    ...
    while(1){
        if(count--){
            ...
            BDBG_MSG(("Layer A count = %llu", isdbtStatus.status.basic.layerA.bert.errorRate.count));
            BDBG_MSG(("Layer A total = %llu", isdbtStatus.status.basic.layerA.bert.errorRate.total));

            BDBG_MSG(("Layer B locked = %d", isdbtStatus.status.basic.layerB.bert.locked));
            BDBG_MSG(("Layer B count = %llu", isdbtStatus.status.basic.layerB.bert.errorRate.count));
            BDBG_MSG(("Layer B total = %llu", isdbtStatus.status.basic.layerB.bert.errorRate.total));

            BDBG_MSG(("Layer C locked = %d", isdbtStatus.status.basic.layerC.bert.locked));
            BDBG_MSG(("Layer C count = %llu", isdbtStatus.status.basic.layerC.bert.errorRate.count));
            BDBG_MSG(("Layer C total = %llu", isdbtStatus.status.basic.layerC.bert.errorRate.total));
        }
        else {
            count=ACCUMULATE_COUNT;
            BDBG_WRN(("Accumulated Layer A count = %llu", accLayerARate.count));
            BDBG_WRN(("Accumulated Layer A total = %llu", accLayerARate.total));
            BDBG_WRN(("Accumulated Layer A rate = %d", accLayerARate.rate));

            BDBG_WRN(("Accumulated Layer B count = %llu", accLayerBRate.count));
            BDBG_WRN(("Accumulated Layer B total = %llu", accLayerBRate.total));
            BDBG_WRN(("Accumulated Layer B rate = %d", accLayerBRate.rate));

            BDBG_WRN(("Accumulated Layer C count = %llu", accLayerCRate.count));
            BDBG_WRN(("Accumulated Layer C total = %llu", accLayerCRate.total));
