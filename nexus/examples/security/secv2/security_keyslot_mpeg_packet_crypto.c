    ...
    char **argv )
{
    ...
    /* There are two DMA transfers of a XTP packet: [pSrc] -> CPS -> [pDest] -> CPD -> [pTemp].
       ...
       We use the same keyslot for the above transfers without invalidating the keyslot's 9 key entry before the 2nd transfer.
       Nevertheless, the keys for CPS and CPD (or CA) need to be different polarities  to avoid the two keys being used simultanously
       in the following pipeline, which results in the SC bits are different between [pSrc] and [pTemp]:
       CPD [clear, odd, even ] -> CA [clear, odd, even ] -> CPS  [clear, odd, even ]
       A more flexible use is shown with example security_keyslot_mpeg_packet_crypto_flexible_app.c.
    */
    ...
    CompositTSPackets( pSrc, XPT_PACKET_SIZE, 0 /* Clear polority key used in Cps */  );
    ...
    keyslotEntrySettings.gPipeEnable = true;
    keyslotEntrySettings.outputPolarity.specify = true;
    keyslotEntrySettings.outputPolarity.gPipe = NEXUS_KeySlotPolarity_eOdd;
    keyslotEntrySettings.outputPolarity.rPipe = NEXUS_KeySlotPolarity_eOdd;

    /* NEXUS_KeySlot_Invalidate( keyslotHandle ) to remove the keys */

    rc = NEXUS_KeySlot_SetEntrySettings( keyslotHandle, entry, &keyslotEntrySettings );
    if( rc != NEXUS_SUCCESS ) { BERR_TRACE( rc ); goto exit; }

    BDBG_LOG( ( "Loads the same clear key to key entry #%d.\n", entry ) );
    ...
    /* Check the decrypt result is correct */
    if( BKNI_Memcmp( pTemp + XPT_TS_PACKET_HEAD_SIZE, pSrc + XPT_TS_PACKET_HEAD_SIZE, XPT_PACKET_SIZE - XPT_TS_PACKET_HEAD_SIZE ) ) {
