...
int main(
    ...
    char **argv )
{
    unsigned      data_size = 0, key_size = 0, iv_size = 0;
    ...
    BDBG_LOG( ( "Step4: Load a clear key and Iv to the configured key entry #%d.\n", entry ) );
    ...
    iv_size = 128 / 8; /* IV is 128 bits for AES, 64 bits for DES. */
    slotIv.size = iv_size;
    BKNI_Memcpy( slotIv.iv, IV_128, iv_size );

    NEXUS_KeySlot_SetEntryIv( keyslotHandle, entry, &slotIv, NULL );

    BDBG_LOG( ( "Step5: Use the clear key as the root key with the IV to encrypt a plain text, the cipher is pDest.") );
