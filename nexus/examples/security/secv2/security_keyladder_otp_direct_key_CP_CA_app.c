...
int main(int argc, char **argv)
{
    ...
    NEXUS_KeyLadderMode mode_ca[] = {
        NEXUS_KeyLadderMode_eCa_64_4,
        NEXUS_KeyLadderMode_eCa_128_4
    };

    NEXUS_KeyLadderMode mode_cp[] = {
    ...
    /* We are to use OtpKeyA as the root key for both CP (encryption) and CA (decryption) keyladder. */
    ...
    if ( !otpKeyInfo.caKeyLadderAllow || !otpKeyInfo.cpKeyLadderAllow )
    {
    ...
    if ( mspOtpRead.data & 1 ) {
        /* OtpKeyA needs route to ASKM, can not be used directly. */
        rc = NEXUS_NOT_SUPPORTED;
        SECURITY_CHECK_RC( rc );
    }
    ...
    for( a = 0; a < sizeof( algorithm ) / sizeof( NEXUS_CryptographicAlgorithm ); a++ ) {
        for( m = 0; m < sizeof( mode_ca ) / sizeof( NEXUS_KeyLadderMode ); m++ ) {
            config.settings.algorithm = algorithm[a];
            config.settings.mode = mode_cp[m];
            ...
            config.levelKey.route.keySlot.entry = NEXUS_KeySlotBlockEntry_eCaClear;
            BKNI_Memcpy( config.pSrc, config.pDest, config.data_size );
            BKNI_Memset( config.pDest, 0, config.data_size );
            config.settings.mode = mode_ca[m];
