...
int M2M_operation ( NEXUS_KeySlotHandle keyHandle,
                    unsigned char *outBuf,
                    unsigned int outBufLen,
                    unsigned char *ivBuf, unsigned int ivBufLen, unsigned char *inBuf, unsigned int inBufLen )
{
    NEXUS_DmaHandle dma;
...
#ifndef USE_KEY_LADDER
    uint8_t         swkeys[16] = { 0xbe, 0xf9, 0xb0, 0x67, 0x13, 0xb8, 0xbc, 0x87,
        0xbc, 0xfb, 0xb2, 0x69, 0x13, 0xba, 0xbe, 0x8b
    };
    /* load sw key */
    NEXUS_Security_GetDefaultClearKey ( &key );
    key.keyEntryType = NEXUS_SecurityKeyType_eEven;
    key.keySize = sizeof ( swkeys );
        /*--------*/
    key.keyIVType = NEXUS_SecurityKeyIVType_eNoIV;
        /*--------*/
    BKNI_Memcpy ( key.keyData, swkeys, sizeof ( swkeys ) );
...
int keySlotSetup ( NEXUS_KeySlotHandle keyHandle, NEXUS_SecurityOperation operation )
{
...
int main ( void )
{
    ...
    if ( M2M_operation ( encKeyHandle,
                         ...
                         ivkeys, sizeof ( ivkeys ), pEncSrc, DMA_BLOCK_SIZE * NUM_DMA_BLOCK ) )
    {
    ...
    if ( M2M_operation ( decKeyHandle,
                         pDecDest,
                         DMA_BLOCK_SIZE * NUM_DMA_BLOCK,
                         ivkeys, sizeof ( ivkeys ), pEncDest, DMA_BLOCK_SIZE * NUM_DMA_BLOCK ) )
    {
