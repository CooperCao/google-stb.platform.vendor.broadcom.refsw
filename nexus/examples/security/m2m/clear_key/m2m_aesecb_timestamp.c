...
/* This shall be 192, while there is a XPT module change to fix it, not an security issue. */
#define DMA_BLOCK   (188*NUM_PACKETS)
#define DMA_JOBS    1
...
int main ( void )
{
    ...
    for ( i = 0; i < NUM_PACKETS; i++ )
    {
        ...
        printf ( "\nHeader %d: %02x %02x %02x %02x %02x %02x %02x %02x\n", i,
                 ...
                 *( pSrc + offset + 2 ),
                 *( pSrc + offset + 3 ),
                 *( pSrc + offset + 4 ), *( pSrc + offset + 5 ), *( pSrc + offset + 6 ), *( pSrc + offset + 7 ) );
    }
