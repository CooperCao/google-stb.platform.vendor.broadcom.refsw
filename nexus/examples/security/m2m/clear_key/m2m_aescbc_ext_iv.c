...
int main(void)
{
    ...
    NEXUS_KeySlotHandle keySlotInt;   /* keyslot that will use internal software key*/
    NEXUS_KeySlotHandle keySlotExt;   /* keyslot that will use exteral key*/
    ...
    uint8_t *pExternalIv = NULL;
    uint8_t *pExternalIvShaddow = NULL;
    unsigned int externalSrcSize;
    unsigned int externalIvSize;
    unsigned int externalDstSize;
    unsigned int i;

    uint8_t key[16] = { 0x01, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F  };
    ...
    DEBUG_PRINT_ARRAY( "Ciphertext", DATA_BLOCK_SIZE, pInternalDestination );
    ...
    NexusConfig.aesCounterSize      = NEXUS_SecurityAesCounterSize_e128Bits;
    NexusConfig.aesCounterMode      = NEXUS_SecurityCounterMode_ePartialBlockInNextPacket;
    NexusConfig.enableExtKey        = false;
    ...
    if(  NEXUS_Security_LoadClearKey( keySlotExt, &nexusClearKey ) != 0 )
    {
        printf("\nLoad encryption key failed \n");
        return 1;
    }
    ...
    if( securityCaps.version.zeus >= NEXUS_ZEUS_VERSION_CALC(4,0) )
    {
        ...
        NEXUS_Memory_Allocate( externalIvSize, NULL, (void **)&pExternalIv );
        NEXUS_Memory_Allocate( externalIvSize, NULL, (void **)&pExternalIvShaddow );
        memset( pExternalIv, 0, externalIvSize );
        memset( pExternalIvShaddow, 0, externalIvSize );
        ...
        compileBtp( pExternalIv, &btp ); /* compile a Broadcom Transport Packet into  pExternalIv */

        DEBUG_PRINT_ARRAY( "External KEY BTP", externalIvSize, pExternalIv );
    }
    else
    {
        /* Zeus 3 ... external Key and IV are concatinated and inserted in front of DMA transfer. */

        externalIvSize = sizeof(iv) + sizeof(key);
        ...
        /* BKNI_Memcpy( pExternalIv, (key + 8), 8 ); */ /* Copy key.  H and L need to be swapped */
        /* BKNI_Memcpy( (pExternalIv + 8), key, 8 ); */
        BKNI_Memcpy( (pExternalIv + sizeof(key)), (iv+8), 8 );  /* Copy IV.  H and L need to be swapped */
        BKNI_Memcpy( (pExternalIv + sizeof(key) + 8) , iv, 8 );  /* Copy IV */
    }
    ...
    blockSettings[0].pSrcAddr                   = pExternalIv;
    blockSettings[0].pDestAddr                  = pExternalIvShaddow;
    blockSettings[0].blockSize                  = externalIvSize;
    blockSettings[0].resetCrypto                = true;
    ...
    DEBUG_PRINT_ARRAY( "External IV", externalIvSize, pExternalIv );
    DEBUG_PRINT_ARRAY( "External IV shaddow", externalIvSize, pExternalIvShaddow );

    DEBUG_PRINT_ARRAY( "Plaintext",    externalSrcSize, pExternalSource );
    DEBUG_PRINT_ARRAY( "Ciphertext",   externalDstSize, pExternalDestination );

    NEXUS_Dma_Close(dma);

    NEXUS_Security_FreeKeySlot( keySlotInt );
    NEXUS_Security_FreeKeySlot( keySlotExt );
    ...
    if( pExternalIv )         NEXUS_Memory_Free( pExternalIv );
    if( pExternalIvShaddow )  NEXUS_Memory_Free( pExternalIvShaddow );
...
static void compileBtp( uint8_t *pBtp, external_key_data_t *pBtpData )
{
    ...
    if( pBtpData->key.valid )
    {
        ...
        pBtpData->key.pData += (len-8);   /*  write the data into BTP in reverse!! */

        while( len )
        {
            memcpy(  p   , pBtpData->key.pData, MIN(len,8) );   /* set Key   */
            memset( (p+8), 0xFF               , MIN(len,8) );   /* set Mask */
            p += 16;                                        /* 8 for data, 8 for mask */
            len -= MIN(len,8);
