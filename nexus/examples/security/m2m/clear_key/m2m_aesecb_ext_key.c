...
int main(void)
{
    ...
    if( securityCaps.version.zeus >= NEXUS_ZEUS_VERSION_CALC(4,0) )
    {
        ...
        {
            assert(!"Nexus incorrectly indicated that there is an IV");
        }
    ...
    blockSettings[1].blockSize                  = DATA_BLOCK_SIZE;  /*externalDstSize;*/
    blockSettings[1].scatterGatherCryptoEnd     = true;

    rc = NEXUS_DmaJob_ProcessBlocks( dmaJob, &blockSettings[0], 2 );
...
static void compileBtp( uint8_t *pBtp, external_key_data_t *pBtpData )
{
    ...
    {               /* ( 0) */  0x47,
                    ...
                    /* (15) */  0x1a,
                    /* (16) */  0x00,
                    /* (17) */  /* 0x01,*/  /* drop packet after processing */
    };
