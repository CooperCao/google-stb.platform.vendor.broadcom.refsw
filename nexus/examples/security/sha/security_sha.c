...
int hex2bin( char* pkey, char *pKeyHex, unsigned sizeHex )
{
    ...
    for( x = i = 0; x < sizeHex; x+=2, i++ )
    {
        ...
        {
            printf("error [%s] x[%d] %X", __FUNCTION__, x, hexByte );
            return -1;
        }
        ...
        {
            printf("error [%s] x[%d] %X b", __FUNCTION__, x, hexByte );
            return -1;
        }
...
int main( int argc, char **argv )
{
    ...
    {
        rc = NEXUS_Memory_Allocate( bufferSize, NULL, (void *)&pBuffer );
    }

    if( pBuffer == NULL )
    {
        printf("%s ### FAILED  to allocate %d bytes for submit buffer", __FUNCTION__, bufferSize ) ;
        return -1;
    }
    ...
    while( fileBytesToRead > bufferSize )
    {
        ...
        {
            printf("%s NEXUS_HMACSHA_PerformOp Failed", __FUNCTION__);
            if(pKey) free(pKey);
            printf("ERROR \n");
            return -1;
        }
    ...
    if( NEXUS_HMACSHA_PerformOp( &shaOpSettings, &hmacOut) )
    {
        printf("ERROR ... %s NEXUS_HMACSHA_PerformOp Failed", __FUNCTION__);
        return -1;
    }
