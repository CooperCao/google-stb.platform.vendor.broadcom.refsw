...
void CmdReadPublicKey0Index(void)
{
	unsigned long ulData;
	unsigned char ucStatus;

	ucStatus = read_msp(0x49, &ulData);
	if(ucStatus)
		printf("read failed status = %x\n", ucStatus );
	else
	{
		if(ulData)
			printf("PublicKey0Index is programmed\n");
		else
			printf("PublicKey0Index is NOT programmed\n");	
	}
	
	return;
}

void CmdProgramPublicKey0Index(void)
{
	unsigned long ulData;
	unsigned char ucStatus;

	ucStatus = write_msp(0x49, 3, 0x7, 0x0);
	if(ucStatus)
