...
#define JTAG_OTP_STRAP_ADDR	736
#define JTAG_OTP_CUSTOMER_LOCK_ADDR 192
...
unsigned long readCustomerLock(unsigned int VIRTUAL_ADDRESS, long Row23_ADDR)
{
    ...
    *(volatile long*) (VIRTUAL_ADDRESS+BCHP_PHYSICAL_OFFSET+BCHP_JTAG_OTP_GENERAL_CTRL_0) = 0x00A00001;
...
void programCustomerLock(unsigned int VIRTUAL_ADDRESS,long data, long addr)
{
	long general_ctrl_1;
    ...
    *(volatile long*) (VIRTUAL_ADDRESS+BCHP_PHYSICAL_OFFSET+BCHP_JTAG_OTP_GENERAL_CTRL_3) = addr;
   	...
   	*(volatile long*) (VIRTUAL_ADDRESS+BCHP_PHYSICAL_OFFSET+BCHP_JTAG_OTP_GENERAL_CTRL_0) = 0x00A0000B;
...
int main (void)
{
	...
	strap_setting = readBootStrap(virtual_base, JTAG_OTP_STRAP_ADDR);	/*read boot strap: virtual_base, address for boot strap (736). Current value will be returned*/

	printf("Current strap Setting is: %08x\n", (unsigned int)strap_setting);

	/*programBootStrap(virtual_base,0xFFF, JTAG_OTP_STRAP_ADDR);	*//*program Boot Strap: virtual_base, value need to be programmed for boot strap, address for boot strap programming (736)*/
	

	programCustomerLock(virtual_base, 0x01, 195);
	programCustomerLock(virtual_base, 0x01, 203);
	programCustomerLock(virtual_base, 0x01, 211);
	strap_setting = readBootStrap(virtual_base, JTAG_OTP_STRAP_ADDR);
				
	printf("Strap Setting after programming is: %08x\n", (unsigned int)strap_setting);
				
	strap_setting = readCustomerLock(virtual_base, JTAG_OTP_CUSTOMER_LOCK_ADDR);

	printf("Customer lock is: %08x\n", (unsigned int)strap_setting);
	strap_setting = readCustomerLock(virtual_base, JTAG_OTP_CUSTOMER_LOCK_ADDR);
