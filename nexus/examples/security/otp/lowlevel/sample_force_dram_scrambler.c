...
#define VIRTUAL_ADDRESS 0xA0000000 
...
void CmdReadForceDramScrambler(void)
{
	unsigned long ulData;
	unsigned char ucStatus;
		
	ucStatus = read_msp(0x12, &ulData);
	if(ucStatus)
		printf("read failed status = %x\n", ucStatus );
	else
	{
		if(ulData)
			printf("ForceDramScrambler is enabled\n");
		else
			printf("ForceDramScrambler is disabled\n");	
	}
	
	return;
}

void CmdProgramForceDramScrambler(void)
{
	unsigned long ulData;
	unsigned char ucStatus;

	ucStatus = write_msp(0x12, 1, 1, 1);
	if(ucStatus)
		printf("progamming failure = %x\n", ucStatus);
	else
		printf("ForceDramScrambler bit is programmed\n");		
