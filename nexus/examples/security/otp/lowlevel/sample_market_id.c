...
void CmdReadMarketId(void)
{
	unsigned long ulData;
	unsigned char ucStatus;
	
	ucStatus = read_msp(0x5E, &ulData);
	if(ucStatus)
		printf("read failed status = %x\n", ucStatus );
	else
	{
		if(ulData)
			printf("Market ID is programmed\n");
		else
			printf("Market ID is NOT programmed\n");	
	}
	
	return;
}

void CmdProgramMarketId(void)
{
	unsigned long ulData;
	unsigned char ucStatus;

	ucStatus = write_msp(0x5E, 32, 0xFFFFFFFF, 0xXXXXXXXX);
	if(ucStatus)
