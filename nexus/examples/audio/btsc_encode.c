...
int main(int argc, char **argv)
{
...
#endif
    unsigned videoPid=0x11, audioPid=0x14;
    ...
    if ( argc >= 3 )
    {
        videoPid = strtoul(argv[1], NULL, 0);
        audioPid = strtoul(argv[2], NULL, 0);
    }

    printf("Using video PID 0x%x (%u) audio PID 0x%x (%u)\n", videoPid, videoPid, audioPid, audioPid);
