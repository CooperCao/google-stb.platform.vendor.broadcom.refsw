...
BDBG_MODULE(audio_loudness);

static bool hdmiHdcpEnabled = false;
static NEXUS_PlatformConfiguration platformConfig;

#define MPEG_ATSC_FILE_NAME "videos/Ref_Level_997Hz_24dB_DVB_h264_29fps.trp" /*MPEG -24*/
#define MPEG_EBU_FILE_NAME "videos/Ref_Level_997Hz_23dB_DVB_h264_29fps.trp" /*MPEG -23*/
#define DDP_FILE_NAME "videos/Loudness_-31dBFS_ddp_DVB_h264_29fps.trp" /*DDP -31*/
#define AAC_ATSC_FILE_NAME "videos/Loudness_-31dBFS_heaac_DVB_h264_29fps.trp" /*AAC+ -31*/
#define AAC_EBU_FILE_NAME "videos/Default_Dialnorm_23dB_heaac_DVB_h264_29fps.trp" /*AAC+ -23*/
#define TRANSPORT_TYPE NEXUS_TransportType_eTs
#define VIDEO_CODEC NEXUS_VideoCodec_eH264
#define VIDEO_PID 0x30
...
void print_usage(void) {
    printf("Usage:\n");
    printf("nexus audio_loudness\n");
    printf("-mpeg || -ddp || -aac -Configures application with stream based on codec\n");
    printf("-multichannel - Configures HDMI output for PCM Multichannel\n");
    printf("-compressed - Configures HDMI/Spdif outputs for AC3 Re-encode\n");
    printf("-atsc - Configures loudness mode for ATSC\n");
    printf("-ebu - Configures loudness mode for EBU\n");
    printf("-none - Disables loudness equivalence\n");
    printf("-ddre - Configures DDRE into the path\n");
    printf("\n");
}
...
int main(int argc, char **argv)
{
    ...
    NEXUS_AudioLoudnessEquivalenceMode loudnessMode = NEXUS_AudioLoudnessEquivalenceMode_eAtscA85;
    bool ddreEnabled = false;
    char *fname = DDP_FILE_NAME;
    ...
    while (curarg < argc) {
        if (!strcmp("--help", argv[curarg]) ||
        ...
        else if (!strcmp("-mpeg", argv[curarg])) {
            codec = NEXUS_AudioCodec_eMpeg;
        }
        else if (!strcmp("-ddp", argv[curarg])) {
            codec = NEXUS_AudioCodec_eAc3Plus;
        }
        else if (!strcmp("-aac", argv[curarg])) {
            codec = NEXUS_AudioCodec_eAacPlus;
        }
            ...
            loudnessMode = NEXUS_AudioLoudnessEquivalenceMode_eNone;
        }
    ...
    {
        BDBG_WRN(("Because DDRE is not enabled compressed will not be configured"));
        compressed = false;
    }
    ...
    case NEXUS_AudioCodec_eMpeg:
        if (loudnessMode == NEXUS_AudioLoudnessEquivalenceMode_eEbuR128) {
            fname = MPEG_EBU_FILE_NAME;
        } else {
            fname = MPEG_ATSC_FILE_NAME;
        }
        break;
    case NEXUS_AudioCodec_eAacPlus:
        if (loudnessMode == NEXUS_AudioLoudnessEquivalenceMode_eEbuR128) {
            fname = AAC_EBU_FILE_NAME;
        } else {
            fname = AAC_ATSC_FILE_NAME;
        }
        break;
    ...
    if (ddreEnabled) {
        ...
        NEXUS_AudioMixer_AddInput(mixer, NEXUS_AudioDecoder_GetConnector(audioDecoder, NEXUS_AudioConnectorType_eMultichannel));

        /* Set the Mixer to use DSP mixing */
        NEXUS_AudioMixer_GetSettings(mixer, &mixerSettings);
        mixerSettings.master = NEXUS_AudioDecoder_GetConnector(audioDecoder, NEXUS_AudioConnectorType_eMultichannel);
        ...
        stereoConnector = NEXUS_DolbyDigitalReencode_GetConnector(ddre, NEXUS_AudioConnectorType_eStereo);
        multichannelConnector = NEXUS_DolbyDigitalReencode_GetConnector(ddre, NEXUS_AudioConnectorType_eMultichannel);
    } else {
        stereoConnector = NEXUS_AudioDecoder_GetConnector(audioDecoder, NEXUS_AudioConnectorType_eStereo);
        multichannelConnector = NEXUS_AudioDecoder_GetConnector(audioDecoder, NEXUS_AudioConnectorType_eMultichannel);
    }
    ...
    NEXUS_AudioInput_Shutdown(NEXUS_AudioDecoder_GetConnector(audioDecoder, NEXUS_AudioConnectorType_eMultichannel));
