...
int main(int argc, char **argv)
{
    ...
    NEXUS_AudioMixerHandle dspMixer;
    ...
    if ( NULL == dspMixer )
    {
        fprintf(stderr, "Unable to open Dsp Mixer\n");
        return 0;
    }
    ...
    NEXUS_AudioMixer_AddInput(dspMixer, NEXUS_AudioDecoder_GetConnector(decoder, NEXUS_AudioConnectorType_eMultichannel));
    NEXUS_AudioMixer_GetSettings(dspMixer, &mixerSettings);
    mixerSettings.master = NEXUS_AudioDecoder_GetConnector(decoder, NEXUS_AudioConnectorType_eMultichannel);
    NEXUS_AudioMixer_SetSettings(dspMixer, &mixerSettings);
    #endif

    NEXUS_AudioMixer_AddInput(dspMixer, NEXUS_AudioPlayback_GetConnector(playback));
    ...
    audioProgram.mixingMode = NEXUS_AudioDecoderMixingMode_eSoundEffects;
    ...
    while (!done)
    {
        ...
        case 2:
            do
            {
                printf("  Enter new loopback Upmix profile(0-5): \n");
                printf("   0) Standard\n");
                printf("   1) MonoLeft -> Center\n");
                printf("   2) MonoRight -> Center\n");
                printf("   3) Upmix 2.0 -> 5.1\n");
                printf("  Selection: \n");
                scanf("%d", &tmp);
            } while ( tmp < 0 || tmp > 3 );

            BKNI_Memset(mixerSettings.loopbackVolumeMatrix, 0, sizeof(int32_t)*NEXUS_AudioChannel_eMax*NEXUS_AudioChannel_eMax);
            switch ( tmp )
            {
            default:
            case 0:
                mixerSettings.loopbackVolumeMatrix[0][0] = NEXUS_AUDIO_VOLUME_LINEAR_NORMAL;
                mixerSettings.loopbackVolumeMatrix[1][1] = NEXUS_AUDIO_VOLUME_LINEAR_NORMAL;
                mixerSettings.loopbackVolumeMatrix[2][2] = NEXUS_AUDIO_VOLUME_LINEAR_NORMAL;
                mixerSettings.loopbackVolumeMatrix[3][3] = NEXUS_AUDIO_VOLUME_LINEAR_NORMAL;
                mixerSettings.loopbackVolumeMatrix[4][4] = NEXUS_AUDIO_VOLUME_LINEAR_NORMAL;
                mixerSettings.loopbackVolumeMatrix[5][5] = NEXUS_AUDIO_VOLUME_LINEAR_NORMAL;
                break;
            case 1:
                mixerSettings.loopbackVolumeMatrix[4][0] = NEXUS_AUDIO_VOLUME_LINEAR_NORMAL;
                break;
            case 2:
                mixerSettings.loopbackVolumeMatrix[4][1] = NEXUS_AUDIO_VOLUME_LINEAR_NORMAL;
                break;
            case 3:
                mixerSettings.loopbackVolumeMatrix[0][0] = NEXUS_AUDIO_VOLUME_LINEAR_NORMAL - (NEXUS_AUDIO_VOLUME_LINEAR_NORMAL/8)*3; /* L-(3L/8)->L */
                mixerSettings.loopbackVolumeMatrix[0][1] = -NEXUS_AUDIO_VOLUME_LINEAR_NORMAL/8*3; /* L-(3R/8)->L */
                mixerSettings.loopbackVolumeMatrix[1][0] = -NEXUS_AUDIO_VOLUME_LINEAR_NORMAL/8*3; /* R-(3L/8)->R */
                mixerSettings.loopbackVolumeMatrix[1][1] = NEXUS_AUDIO_VOLUME_LINEAR_NORMAL - (NEXUS_AUDIO_VOLUME_LINEAR_NORMAL/8)*3; /* R-(3R/8)->R */
                mixerSettings.loopbackVolumeMatrix[4][0] = NEXUS_AUDIO_VOLUME_LINEAR_NORMAL / 2; /* L/2->C */
                mixerSettings.loopbackVolumeMatrix[4][1] = NEXUS_AUDIO_VOLUME_LINEAR_NORMAL / 2; /* R/2->C */
                break;
            }
            NEXUS_AudioMixer_SetSettings(dspMixer, &mixerSettings);
            break;
    ...
    NEXUS_AudioMixer_RemoveAllInputs(dspMixer);
    NEXUS_AudioInput_Shutdown(NEXUS_AudioDecoder_GetConnector(decoder, NEXUS_AudioDecoderConnectorType_eMultichannel));
    NEXUS_AudioInput_Shutdown(NEXUS_AudioPlayback_GetConnector(playback));
    NEXUS_AudioPlayback_Close(playback);
    NEXUS_AudioDecoder_Close(decoder);
    NEXUS_AudioMixer_Close(dspMixer);
#if NEXUS_HAS_HDMI_OUTPUT
