 ...
 * HDR signaling.
 *
 * This app now demonstrates how to override the default passthrough of HDR
 * parameters from input to output, which is accomplished internally in nexus
 * now.
 */
...
static void setHdmiOut(App *app, NEXUS_VideoDecoderStreamInformation *streamInfo, bool force)
{
    ...
    /* By default nexus will combine the content mega data and TV EDID to configure DynamicRangeMasteringInfoFrame
       for optimal result. Typically appplication does not need to override it.
       In case you want to override it, the following shows how to do it */
    NEXUS_HdmiOutput_GetExtraSettings(app->hdmi, &hdmiSettings);
    ...
    if (force || !app->lastStreamInfoValid || app->lastStreamInfo.eotf != streamInfo->eotf)
    {
        ...
        if (!app->sinkEdid.hdrdb.valid)
        {
            fprintf(stdout, "TV does not support HDR, no DRMInfoFrame sent. Displaying HDR video as SDR\n");
    ...
    if (apply)
    {
        rc = NEXUS_HdmiOutput_SetExtraSettings(app->hdmi, &hdmiSettings);
        BDBG_ASSERT(!rc);
    }
}
...
static void streamChangedCallback(void * context, int param)
{
    ...
    if (!rc)
    {
        ...
        BKNI_Memcpy(&app->lastStreamInfo, &streamInfo, sizeof(app->lastStreamInfo));
