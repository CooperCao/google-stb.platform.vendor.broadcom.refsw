...
int main(void) {
    ...
    recpumpOpenSettings.data.dataReadyThreshold = recpumpOpenSettings.data.bufferSize / 100 * 35;
    recpumpOpenSettings.index.dataReadyThreshold = recpumpOpenSettings.index.bufferSize / 100 * 35;
    ...
    recordSettings.priming.data.fifoDepth = recpumpOpenSettings.data.bufferSize / 100 * 25;
    recordSettings.priming.index.fifoDepth = recpumpOpenSettings.index.bufferSize / 100 * 25;
    rc = NEXUS_Record_SetSettings(record, &recordSettings);
    BDBG_ASSERT(!rc);
    ...
    printf("press ENTER to stop priming\n");
    getchar();

    NEXUS_Record_GetSettings(record, &recordSettings);
    recordSettings.priming.data.fifoDepth = 0;
    recordSettings.priming.index.fifoDepth = 0;
    rc = NEXUS_Record_SetSettings(record, &recordSettings);
    BDBG_ASSERT(!rc);
