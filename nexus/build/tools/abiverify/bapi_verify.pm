...
sub process_functions {
    ...
    return [\@ioctls, \%api_structs, \%references, \@api_ids, \%compat_info, $varargs_info, \%shared_types ];
}
...
{
    my ($fout, $class_handles, $structs, $name, $fields, $shared) = @_;
    my $field;
    for $field (@$fields) {
        my $type = $field->{TYPE};
        ...
        if($shared) {
            ...
            print $fout "    NEXUS_P_COMPAT_SHARED_MEMORY_VERIFY_$kind($name, $field_name, $type)\n";
            next;
        }
        my $union_pos;
        my $union_fields = parse_union_field($field);
...
sub generate_compat {
    my ($fout, $module, $functions, $referenced_structs, $structs, $class_handles, $api_ids, $varargs_info, $structs_attr, $shared_types) = @_;
    print $fout $autogenerated;
    print $fout "
#include \"nexus_${module}_module.h\"
...
";
    ...
    for my $name (sort keys %$structs) {
        ...
        if(exists $structs_attr->{$name} && exists $structs_attr->{$name}{'local'} && $structs_attr->{$name}{'local'} eq 'true') {
            ...
            verify_one_union ($fout, $class_handles, $structs, $name, $fields, exists $shared_types->{$name});
        }
        print $fout "\n\n";
    }
...
sub generate {
    my ($destdir, $module, $functions, $class_handles, $structs, $version, $ioctls, $referenced_structs, $api_ids, $compat_info, $varargs_info, $structs_attr, $shared_types) = @_;
    my $fout;
    my $file;
    my $fout_compat;
    my $file_compat;
    ...
    open ($fout, '>', $file) or die "Can't open '$file'";
    generate_compat($fout, $module, $functions, $referenced_structs, $structs, $class_handles, $api_ids, $varargs_info, $structs_attr, $shared_types);
    close ($fout);
