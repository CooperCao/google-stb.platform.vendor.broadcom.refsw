...
#define NEXUS_P_CLIENT_IN_REALLOC(_length) { unsigned __length = _length; void *new_in = BKNI_Malloc(__data_size + __length);if(new_in==NULL) {(void)BERR_TRACE(NEXUS_OUT_OF_SYSTEM_MEMORY);goto err_call;};BKNI_Memcpy(new_in, in_data, __variable_in_offset);if((void *)in_data != temp) {BKNI_Free(in_data);} in_data = new_in;__data_size = __data_size + __length;} while(0)
