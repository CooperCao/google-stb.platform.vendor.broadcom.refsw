...
static struct NEXUS_Scm_P_Utils_State {
...
static NEXUS_Error NEXUS_Scm_P_GetHeapConfig(NEXUS_MemoryType memoryType, uint32_t heap_index,
                                              NEXUS_MemoryAllocationSettings *allocSettings);

/* check if heap is accessible from both Nexus Scm and SCM-side */
int NEXUS_Scm_P_IsHeapValid(NEXUS_HeapHandle heap, NEXUS_MemoryType memoryType)
{
    ...
    if (status.memoryType == memoryType
        ...
        BDBG_MSG(("%s: VALID heap=%p [offset=" BDBG_UINT64_FMT ", size=%u]\n", BSTD_FUNCTION, (void*)heap, BDBG_UINT64_ARG(status.offset), status.size));
...
NEXUS_Error NEXUS_Scm_P_ConfigureAlloc(void)
{
    ...
    rc = NEXUS_Scm_P_GetHeapConfig(NEXUS_MemoryType_eFull,
...
#if 0
    rc = NEXUS_Scm_P_GetHeapConfig(NEXUS_MemoryType_eSecure,
...
{
    NEXUS_MemoryAllocationSettings s;
    NEXUS_Error rc;
    void *mem;

    s.heap = g_sage_utils.defaultAllocSettings.heap;
    s.alignment = alignment;
    rc = NEXUS_Memory_Allocate(size, &s, &mem);
    if (rc == NEXUS_SUCCESS) {
        return mem;
    }
    return NULL;
