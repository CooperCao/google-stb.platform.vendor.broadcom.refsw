    ...
    )
{
    ...
    BKNI_Memset(hBvn, 0x0, sizeof(*hBvn));
    ...
    for (i = 0; i < BBVN_P_TERMINAL_NODE_COUNT; i++)
    {
        ...
        if(BCHP_VNET_F_TERMINAL_SRC != pCurMux->ulAddress)
        {
            ...
            if(BBVN_P_VnetF_eDisabled != ulSource)
            {
                ...
                BKNI_Memset(pPath, 0, sizeof(*pPath));

                rc = BBVN_P_TraverseBvn_recursive(hBvnMonitor->hReg, pCurMux, pSecureCores, pPath);
                if(rc != BERR_SUCCESS)
                {
                    goto ERROR;
                }
