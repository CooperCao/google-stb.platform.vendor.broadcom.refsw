...
BERR_Code
b_playpump_p_add_request(NEXUS_PlaypumpHandle p, size_t skip, size_t amount_used, const NEXUS_PlaypumpScatterGatherDescriptor *pSgDesc, NEXUS_PlaypumpWriteCompleteSettings *settings, const void *crcSettings)
{
    struct bpvr_queue_item *item;
    unsigned nFree;
    ...
    if (pSgDesc) {
        ...
        case NEXUS_PlaypumpMode_eSegment:
            ...
            item->desc.descriptorSettings = *settings;
#if NEXUS_TRANSPORT_EXTENSION_CRC
            item->crc = *(NEXUS_PlaypumpCrcWriteSettings *)crcSettings;
#else
            BSTD_UNUSED(crcSettings);
#endif
