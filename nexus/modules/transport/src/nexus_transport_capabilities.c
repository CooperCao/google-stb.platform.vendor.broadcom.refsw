...
void NEXUS_GetTransportCapabilities(NEXUS_TransportCapabilities *pCap)
{
    ...
    pCap->numInputBands = NEXUS_MAX_INPUT_BANDS;
#endif
    ...
    pCap->numRecpumps = BXPT_NUM_RAVE_CONTEXTS;
    pCap->packetizer = true;
    pCap->vobRemap = true;
    pCap->numTpitPids = BXPT_NUM_TPIT_PIDS;
    pCap->numPacketSubs = BXPT_NUM_PACKETSUBS;
    pCap->numStcs = BXPT_NUM_STCS;
    pCap->numStcChannels = BXPT_NUM_PCR_OFFSET_CHANNELS;
    pCap->numTimebases = BXPT_NUM_PCRS;
    pCap->numRemux = BXPT_NUM_REMULTIPLEXORS;
    ...
    )
{
    ...
    BXPT_Rave_RevisionInfo revInfo;
    ...
    BXPT_Rave_GetFwRevisionInfo(pTransport->rave[0].channel, &revInfo);
    pStatus->rave.version.firmware = revInfo.fwRev;
    pStatus->rave.version.firmwareCrc = revInfo.fwCrc;
}
