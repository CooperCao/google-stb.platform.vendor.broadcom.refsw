...
NEXUS_RaveCapture * NEXUS_RaveCapture_Create(const NEXUS_RaveCaptureCreateSettings * pCreateSettings)
{
    ...
    baseOffset = BREG_Read32(g_pCoreHandles->reg, pXptContextMap->CDB_Base);
    ...
    baseOffset = BREG_Read32(g_pCoreHandles->reg, pXptContextMap->ITB_Base);
    BDBG_ASSERT(baseOffset);
    ...
    BDBG_MSG(("%p created for rave context %u", cap, cap->index));
    return cap;
}
...
static void NEXUS_RaveCapture_GetWritePointers(NEXUS_RaveCaptureDescriptor * desc)
{
    ...
    wrapOffset = BREG_Read32(g_pCoreHandles->reg, desc->wrapOffsetOffset);
