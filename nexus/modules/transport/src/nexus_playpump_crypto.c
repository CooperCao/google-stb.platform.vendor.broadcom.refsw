...
#else
            for (j=0,i=0;i<nvecs;i++) {
                if (j!=0 && (uint8_t *)crypto->dma.blocks[j-1].pSrcAddr + crypto->dma.blocks[j-1].blockSize == (uint8_t*)vec[i].base) {
                    crypto->dma.blocks[j-1].blockSize += vec[i].len;
                }
                else {
                    if (j == B_PUMP_CRYPT_MAX_TS_SEGMENTS) {
                        BERR_TRACE(BERR_NOT_SUPPORTED);
                        break;
                    }
                    crypto->dma.blocks[j].pSrcAddr = vec[i].base;
                    crypto->dma.blocks[j].pDestAddr = vec[i].base;
                    crypto->dma.blocks[j].blockSize = vec[i].len;
                    crypto->dma.blocks[j].scatterGatherCryptoEnd = false;
                    j++;
                }
            }
            nvecs = j;
...
b_pump_crypto_status(b_pump_crypto_t crypto)
{
    ...
    BDBG_MSG(("crypto_status: pending:%p:%u  pipe_in:%p ts_bytes:%u %s DMA:%u",
        (void *)BFIFO_READ(&crypto->pending), BFIFO_READ_PEEK(&crypto->pending)?BFIFO_READ(&crypto->pending)->ref_cnt:0,
        (void *)batom_pipe_peek(crypto->ts_feed.pipe_clear),
        (unsigned)crypto->ts_feed.ts_bytes, crypto->ts_feed.feed_stall?"STALL":"",
        crypto->dma.atom?(unsigned)batom_len(crypto->dma.atom):0 ));
