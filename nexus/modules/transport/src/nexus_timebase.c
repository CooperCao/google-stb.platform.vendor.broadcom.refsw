...
static void NEXUS_Timebase_P_RefreshRateFromFrameRate(bool interlaced, NEXUS_VideoFrameRate frameRate, unsigned * pRefreshRateNum, unsigned * pRefreshRateDen)
{
    ...
    if (refreshRate == 0) {
        BDBG_MSG(("Unsupported frameRateCode of %d; using %d/%d", frameRate, NEXUS_TIMEBASE_P_DEFAULT_REFRESH_RATE_NUM, NEXUS_TIMEBASE_P_DEFAULT_REFRESH_RATE_DEN));
...
NEXUS_Error NEXUS_Timebase_SetHdDviFrameRate( NEXUS_Timebase timebase, NEXUS_VideoFrameRate frameRate )
{
    ...
    if (handle->settings.sourceType != NEXUS_TimebaseSourceType_eHdDviIn) {
        BDBG_ERR(("NEXUS_Timebase_SetHdDviFrameRate(%u) called without NEXUS_TimebaseSourceType_eHdDviIn", handle->hwIndex));
    }
