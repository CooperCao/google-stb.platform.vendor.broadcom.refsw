...
typedef struct NEXUS_PlaypumpOpenSettings
{
    ...
    bool descriptorOptionsEnabled;    /* Optional per-descriptor flags. See descriptorOptions structure in NEXUS_PlaypumpWriteCompleteSettings */
...
typedef struct NEXUS_PlaypumpWriteCompleteSettings
{
    ...
    /* Optional per-descriptor options, used only with the in this write complete. */
    struct {
        bool transportOverride;      /* Data being written is in transport packet format, even though the originalTransportType is not
            NEXUS_TransportType_eTs. The data must be written at a source packet boundary, when it is ES or TS. For PES, this data can be
            written at any point after the PES header has been written. */
    } descriptorOptions;
