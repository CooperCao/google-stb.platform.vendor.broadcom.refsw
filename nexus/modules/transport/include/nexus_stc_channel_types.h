...
typedef struct NEXUS_StcChannelStatus
{
    NEXUS_StcChannelMode mode; /* The mode that the STC channel is currently in. This can vary from NEXUS_StcChannelSettings.mode
                                  because of ASTM. */
    uint32_t stc;              /* Stc is comprised of (serialStc + PCR_OFFSET) mod (2^32), it is evaluated by decoders for TSM */
    uint32_t serialStc;        /* Serial stc, a 45KHz counter that is broadcast to all decoders, combined with PCR_OFFSET
                                  to obtain stc for performing TSM */
    bool stcValid;             /* True if stc is valid and PCR_OFFSET is valid */
    unsigned index;            /* Index of the hardware PCR Offset channel being used. */
    unsigned stcIndex;         /* Index of the STC Channel (serial STC) */
    unsigned timebaseIndex;    /* Index of the timebase that drives this STC channel. */
