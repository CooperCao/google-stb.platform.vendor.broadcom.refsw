...
NEXUS_Error NEXUS_HdmiOutput_GetPreEmphasisConfiguration(
    ...
    NEXUS_HdmiOutputPreEmphasisConfiguration *pstPreEmphasis
)
{
    ...
    BDBG_CASSERT(NEXUS_HDMI_OUTPUT_TMDS_RANGES == BHDM_TMDS_RANGES) ;

    BKNI_Memset(pstPreEmphasis, 0, sizeof(NEXUS_HdmiOutputPreEmphasisConfiguration));
...
NEXUS_Error NEXUS_HdmiOutput_SetPreEmphasisConfiguration(
    NEXUS_HdmiOutputHandle handle,
    const NEXUS_HdmiOutputPreEmphasisConfiguration *pstPreEmphasis
)
{
    BERR_Code rc = BERR_SUCCESS ;
    uint8_t index ;
