...
NEXUS_VideoDecoder_P_DataReady_Generic_Prologue_isr(NEXUS_VideoDecoderHandle videoDecoder, const BAVC_MFD_Picture *pFieldData)
{
    ...
    if (videoDecoder->dataReadyCount == 0 && pFieldData->bMute) {
...

        {
            ...
            switch (pFieldData->stHdrMetadata.stDynamic.eType)
            {
...
NEXUS_VideoDecoder_P_PictureParams_isr(void *data, int unused, void *info_)
{
    ...
    /* convert CLL */
    NEXUS_P_ContentLightLevel_FromMagnum_isrsafe(&streamInfo->contentLightLevel, &info->stStaticHdrMetadata.stContentLightLevel);
    /* convert MDCV */
    NEXUS_P_MasteringDisplayColorVolume_FromMagnum_isrsafe(&streamInfo->masteringDisplayColorVolume, &info->stStaticHdrMetadata.stMasteringDisplayColorVolume);
