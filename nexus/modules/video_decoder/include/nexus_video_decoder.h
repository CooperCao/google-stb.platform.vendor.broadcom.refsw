...
typedef struct NEXUS_VideoDecoderOpenSettings
{
    ...
    bool spliceEnabled;       /* Enable the dynamic splicing feature used for ad insertion
                                 When this flag is set, video decoder open creates only SW rave
                                 context.HW rave context creation is deferred to video decoder start.
                              */
...
void NEXUS_VideoDecoder_GetCodecCapabilities(
    NEXUS_VideoDecoderHandle videoDecoder, /* attr{null_allowed=y} If NULL, Nexus will return information from HVD0,
                                                without reference to possible per-channel reductions. */
    NEXUS_VideoCodec codec,
...
typedef struct NEXUS_VideoDecoderModuleStatus
{
    struct {
        NEXUS_PowerStatus core[NEXUS_MAX_VIDEO_DECODERS];
    } power;
} NEXUS_VideoDecoderModuleStatus;
 ...
 * callback will be invoked so the application can supply the replacement stream or perform
 * cleanup.
 ...
 * used by the system.
 */
typedef struct NEXUS_VideoDecoderSpliceSettings
{
...
} NEXUS_VideoDecoderSpliceSettings;
 ...
 */
typedef struct NEXUS_VideoDecoderSpliceStatus
{
    NEXUS_DecoderSpliceState state;
    uint32_t pts;
} NEXUS_VideoDecoderSpliceStatus;
 ...
 * See Also:
 * NEXUS_VideoDecoder_GetSpliceSettings
 */
 ...
 * See Also:
 * NEXUS_VideoDecoder_SetSpliceSettings
 */
 ...
 * See Also:
 * NEXUS_VideoDecoder_SpliceStartFlow
 */
 ...
 */
typedef struct NEXUS_VideoDecoderSpliceStartFlowSettings
{
    NEXUS_PidChannelHandle pidChannel; /* pidChannel to use as a source of data from transport */
} NEXUS_VideoDecoderSpliceStartFlowSettings;

/**
 * Summary:
 * Initialize NEXUS_VideoDecoderSpliceStartFlowSettings structure to default values.
 */
...
/**
 * Summary:
 * Resume data flow which was stopped by NEXUS_VideoDecoder_SpliceStopFlow call.
 * Description:
 * This function is used to resume data flow, previously stopped by NEXUS_VideoDecoder_SpliceStopFlow
 ...
 * See Also:
 * NEXUS_VideoDecoder_SpliceStopFlow
 * NEXUS_VideoDecoderSpliceStartFlowSettings
 * NEXUS_VideoDecoder_GetDefaultSpliceStartFlowSettings
 */
