...
set_mapping:
        if (demodPb < pConfig->numDemodPb) {
            ...
            pConfig->demodPbSettings[demodPb].passthrough = parserConfig.passthrough = false;
            ...
            if (pConfig->slave && !daisyOverride) {
                ...
                #endif
                pChainedConfig->demodPbSettings[hostPb].passthrough = parserConfig.passthrough = true;
...
#if 0 /* MXT regdump */
{
    ...
    for (index=0; index<BXPT_NUM_MTSIF; index++) {
        uint32_t step = BCHP_XPT_FE_MTSIF_RX1_PKT_BAND0_BAND31_DETECT - BCHP_XPT_FE_MTSIF_RX0_PKT_BAND0_BAND31_DETECT;
        uint32_t addr = BCHP_XPT_FE_MTSIF_RX0_PKT_BAND0_BAND31_DETECT + step*index;
        uint32_t reg = BREG_Read32(g_pCoreHandles->reg, BCHP_XPT_FE_MTSIF_RX0_CTRL1 + step * index);
        BDBG_MODULE_LOG(nexus_frontend_proc, ("MTSIF_RX%u BAND_DETECT = %08x, decrypt %u, enable %u", index, BREG_Read32(g_pCoreHandles->reg, addr), (unsigned) BCHP_GET_FIELD_DATA(reg, XPT_FE_MTSIF_RX0_CTRL1, MTSIF_RX_DECRYPTION_ENABLE_STATUS), (unsigned) BCHP_GET_FIELD_DATA(reg, XPT_FE_MTSIF_RX0_CTRL1, PARSER_ENABLE) ));
    }
#endif
