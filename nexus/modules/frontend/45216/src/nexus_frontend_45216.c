...
#if NEXUS_FRONTEND_DEBUG_IRQ
    BDBG_MSG(("%s 45216 Gpio Interrupt %p", enable ? "Enable" : "Disable", (void *)gpioHandle));
#endif
...
#if NEXUS_FRONTEND_DEBUG_IRQ
    BDBG_MSG(("45216 ISR Callback (hab: %p)", (void *)pDevice->satDevice->habHandle));
#endif
...
static void NEXUS_Frontend_P_45216_L1_isr(void *param1, int param2)
{
    ...
    BSTD_UNUSED(param2);
#if NEXUS_FRONTEND_DEBUG_IRQ
    BDBG_MSG(("45216 L1 ISR (hab: %p)", (void *)pDevice->satDevice->habHandle));
#endif

    BHAB_HandleInterrupt_isr(pDevice->satDevice->habHandle);
#if NEXUS_FRONTEND_DEBUG_IRQ
    BDBG_MSG(("Done: 45216 L1 ISR (hab: %p)", (void *)pDevice->satDevice->habHandle));
#endif
}
...
static NEXUS_Error NEXUS_FrontendDevice_P_Init45216_PreInitAP(NEXUS_45216Device *pDevice)
{
    ...
    BHAB_Handle habHandle = NULL;
    BSAT_Handle satHandle = NULL;
    BWFE_Handle wfeHandle = NULL;
    BDSQ_Handle dsqHandle = NULL;
#if NEXUS_HAS_FSK
    BFSK_Handle fskHandle = NULL;
#endif
    unsigned i;
    void *regHandle;
    BERR_Code errCode;
...
err:
    ...
    if (pInitSettings) { BKNI_Free(pInitSettings); }
...
init_error:
    BDBG_ERR(("45216(%p) initialization failed...", (void *)pDevice));
...
NEXUS_FrontendDeviceHandle NEXUS_FrontendDevice_Open45216(unsigned index, const NEXUS_FrontendDevice45216OpenSettings *pSettings)
{
    ...
    BDBG_MSG(("NEXUS_FrontendDevice_Open45216: returning %p", (void *)pFrontendDevice));
...
static void NEXUS_Frontend_P_Uninit45216(NEXUS_45216Device *pDevice)
{
    ...
    BDBG_MSG(("Closing 45216 device %p handle", (void *)pDevice));
