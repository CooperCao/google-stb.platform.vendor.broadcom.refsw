...
*
* API Description:
*   API name: Frontend 45308
*    APIs to open, close, and setup initial settings for a BCM45308
...
#if NEXUS_FRONTEND_DEBUG_IRQ
    BDBG_MSG(("%s 45308 Gpio Interrupt %p", enable ? "Enable" : "Disable", gpioHandle));
#endif
...
#if NEXUS_FRONTEND_DEBUG_IRQ
    BDBG_MSG(("45308 ISR Callback (hab: %p)",pDevice->satDevice->habHandle));
#endif
...
static void NEXUS_Frontend_P_45308_L1_isr(void *param1, int param2)
{
    ...
    BSTD_UNUSED(param2);
#if NEXUS_FRONTEND_DEBUG_IRQ
    BDBG_MSG(("45308 L1 ISR (hab: %p)",pDevice->satDevice->habHandle));
#endif

    BHAB_HandleInterrupt_isr(pDevice->satDevice->habHandle);
#if NEXUS_FRONTEND_DEBUG_IRQ
    BDBG_MSG(("Done: 45308 L1 ISR (hab: %p)",pDevice->satDevice->habHandle));
#endif
}
    ...
    for (i=0; i < 3; i++) {
        ...
        if (!pSettings->mtsif[i].enabled) {
            ...
            if (pSettings->mtsif[i].clockRate != 0) {
                ...
                case 91000000: /* 91 MHz. */
                case 91125000: /* 91.125 MHz. (actual value) */
                    divider = 32; break;
                case 93000000: /* 93 MHz. */
                case 95000000: /* 95 MHz. */
                case 94000000: /* 94 MHz. (actual value) */
                    divider = 31; break;
                case 98000000: /* 98 MHz. */
                case 98100000:
                case 98200000:
                case 97200000: /* 97.2 MHz. (actual value) */
                    divider = 30; break;
                case 100000000: /* 100 MHz. */
                case 101000000: /* 101 MHz. */
                case 101250000:
                case 101500000:
                case 100550000: /* 100.55 MHz (actual value) */
                    divider = 29; break;
                case 104000000: /* 104.5 MHz. Also accept 104 Mhz. */
                case 104500000:
                case 104140000: /* 104.14 MHz (actual value) */
                    divider = 28; break;
                case 108000000: /* 108 MHz. */
                    divider = 27; break;
                case 135000000: /* 135 MHz. */
                case 136000000: /* 135 MHz. */
                case 132540000: /* 132.54... MHz. (actual value) */
                    divider = 22; break;
                default:
                    BDBG_WRN(("NEXUS_FrontendDevice_Open45308: Unrecognized rate (MTSIF%d): %u, defaulting to 81MHz.", i, clockRate));
                    divider = 36;
                    break;
                }
                val &= ~0x000001fe;
...
init_error:
    BDBG_ERR(("45308(%p) initialization failed...",pDevice));
...
NEXUS_FrontendDeviceHandle NEXUS_FrontendDevice_Open45308(unsigned index, const NEXUS_FrontendDevice45308OpenSettings *pSettings)
{
    ...
    BDBG_MSG(("NEXUS_FrontendDevice_Open45308: returning %p",pFrontendDevice));
...
static void NEXUS_Frontend_P_Uninit45308(NEXUS_45308Device *pDevice)
{
    ...
    BDBG_MSG(("Closing 45308 device %p handles", pDevice));
