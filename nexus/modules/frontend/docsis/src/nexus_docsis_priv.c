...
* API Description:
* API name: DOCSIS
* Private APIs for DOCSIS device and channels.
*
* Revision History:
*
* $brcm_Log: $
    ...
    bool locked)
{
    ...
    BDBG_ERR(("current host channels lock status %#lx",hostChannelLockStatus));
    ...
    BDBG_ERR(("changed host channels lock status %#lx",hostChannelLockStatus));
    return retCode;
}
    ...
    )
{
    ...
    BDBG_MSG(("RF freq %d",ausStatus.rfFreq));
    BDBG_MSG(("symbol rate %d",ausStatus.symbolRate));
