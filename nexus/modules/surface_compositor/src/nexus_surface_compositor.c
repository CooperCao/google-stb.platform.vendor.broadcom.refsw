        ...
        for (client = BLST_Q_FIRST(&server->clients); client; client = BLST_Q_NEXT(client, link)) {
            ...
            for (child = BLST_S_FIRST(&client->children); child; child = BLST_S_NEXT(child, child_link)) {
                ...
                if (child->window[0]) {
                    ...
                    int mosaicIndex;
                    NEXUS_VideoWindow_GetParentIndex_isrsafe(child->window[0], &parentIndex, &mosaicIndex);
                    if (mosaicIndex == -1) {
                        BKNI_Snprintf(buf, sizeof(buf), ", window %u", parentIndex);
                    }
                    else {
                        BKNI_Snprintf(buf, sizeof(buf), ", window %u.%d", parentIndex, mosaicIndex);
                    }
                }
                BDBG_LOG(("  %s %p: id %d; %d,%d,%d,%d; virtual display %d,%d; visible %c; zorder %d%s",
...
struct nexus_surface_compositor_p_update_graphics_settings_status {
    bool toggle_enabled[NEXUS_SURFACE_COMPOSITOR_MAX_DISPLAYS];
};
...
NEXUS_Error NEXUS_SurfaceCompositor_SetSettings( NEXUS_SurfaceCompositorHandle server, const NEXUS_SurfaceCompositorSettings *pSettings )
{
    ...
    if (!server->state.active) {
        ...
        if (enable_changed && !pSettings->enabled) {
            unsigned i;
            /*
             * If graphics are disabled and we then disable NSC, submitted can safely be
             * cleared here so that NSC will eventually declare itself inactive.
             */
            for (i=0;i<NEXUS_SURFACE_COMPOSITOR_MAX_DISPLAYS;i++) {
                if(server->settings.display[i].display && !nexus_surface_compositor_p_display_enabled(server, i)) {
                    struct NEXUS_SurfaceCompositorDisplay *display =  server->display[i];
