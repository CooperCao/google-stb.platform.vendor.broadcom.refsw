...
typedef struct NEXUS_SurfaceClientSettings
{
    ...
    bool synchronizeGraphics; /* If set true on video SurfaceClient, SurfaceCompositor will set NEXUS_GraphicsSettings.synchronized and
        delay the client's push-surface pipeline based on BVN delay in order to synchronize video window movement and graphics updates.
        The caller must call NEXUS_SurfaceClient_SetSettings on video before setting graphics and must use
        NEXUS_SurfaceClient_PushSurface with infront = true, not SetSurface. Does not work with allowCompositionBypass. */
...
/* deprecated */
#define NEXUS_SurfaceClient_AcquireTunneledSurface(handle,pSurface) NEXUS_NOT_SUPPORTED
#define NEXUS_SurfaceClient_ReleaseTunneledSurface(handle,pSurface)
#define NEXUS_SurfaceClient_PushTunneledSurface(handle,pSurfaec,pUpdateRect,infront) NEXUS_NOT_SUPPORTED
#define NEXUS_SurfaceClient_RecycleTunneledSurface(handle,recycled,num_entries,num_returned) NEXUS_NOT_SUPPORTED
