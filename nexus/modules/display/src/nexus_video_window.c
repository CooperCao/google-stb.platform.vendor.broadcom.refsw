...
NEXUS_VideoWindow_P_MultiBufLogEventHandler(void *data)
{
    NEXUS_VideoWindowHandle window = (NEXUS_VideoWindowHandle)data;
    BDBG_OBJECT_ASSERT(window, NEXUS_VideoWindow);
    NEXUS_Display_P_BufLogCapture(window->display, window->index);
}
...
NEXUS_VideoWindow_P_EnableMultiBufLog( NEXUS_VideoWindowHandle window, NEXUS_VideoBufLog_Trigger trigger, bool enable )
{
    ...
    if (enable)
    {
        BVDC_Dbg_Window_SetBufLogStateAndDumpTrigger(window->vdcState.window, (BVDC_BufLogState)trigger,
            NEXUS_VideoWindow_P_MultiBufLogCallBack_isr, (void *)window, 0);
    }
    else
    {
        BVDC_Dbg_Window_SetBufLogStateAndDumpTrigger(window->vdcState.window, (BVDC_BufLogState)NEXUS_VideoBufLog_Trigger_eReset,
            NULL, NULL, 0);
    }

    BVDC_Dbg_Window_EnableBufLog(window->vdcState.window, enable);
