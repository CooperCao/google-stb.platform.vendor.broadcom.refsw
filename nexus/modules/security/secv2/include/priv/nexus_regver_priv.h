...
/*
    ...
    (nexus_regver_signatures.h) that can be used against a particular firmware image.
...
typedef enum NEXUS_SecurityRegverRegionID
{
    ...
    NEXUS_SecurityRegverRegionID_eVDEC1_IL2A    = 0x23,
    NEXUS_SecurityRegverRegionID_eMax

} NEXUS_SecurityRegverRegionID;

#define NEXUS_REGVER_MAX_REGIONS (36)

#if (BHSM_ZEUS_VER_MAJOR >= 5)
 #define NEXUS_RV_REGION_STATUS_ENABLED                 (1<<0)
 #define NEXUS_RV_REGION_STATUS_AUTH_ENFORCED           (1<<1)
 #define NEXUS_RV_REGION_STATUS_SAGE_OWNED              (1<<2)
 #define NEXUS_RV_REGION_STATUS_LIVE_MERGE_IN_PROGRESS  (1<<3)
 #define NEXUS_RV_REGION_STATUS_LIVE_MERGE_FAILED       (1<<4)
 #define NEXUS_RV_REGION_STATUS_LIVE_MERGE_PASS         (1<<5)
 #define NEXUS_RV_REGION_STATUS_FAST_CHECK_STARTED      (1<<7)
 #define NEXUS_RV_REGION_STATUS_FAST_CHECK_FINISHED     (1<<8)
 #define NEXUS_RV_REGION_STATUS_FAST_CHECK_FAILED       (1<<9)
 #define NEXUS_RV_REGION_STATUS_BG_CHECK_ENABLED        (1<<6)
 #define NEXUS_RV_REGION_STATUS_BG_CHECK_STARTED        (1<<10)
 #define NEXUS_RV_REGION_STATUS_BG_CHECK_FINISHED       (1<<11)
 #define NEXUS_RV_REGION_STATUS_BG_CHECK_FAILED         (1<<12)
#else
...
typedef struct NEXUS_SecurityRegionConfiguration
{
    struct{
        ...
        uint8_t data[NEXUS_REGION_VERIFY_SIGNATURE_PLUS_HEADER_SIZE];              /* signature.  */
    }signature;
...
{
    unsigned  regionStatus[NEXUS_REGVER_MAX_REGIONS];   /* Region status can be interpreted with NEXUS_RV_REGION_STATUS_* masks. */
