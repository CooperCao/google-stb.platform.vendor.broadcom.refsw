...
NEXUS_Error NEXUS_KeySlot_SetEntryIv(
    NEXUS_KeySlotHandle handle,
    NEXUS_KeySlotBlockEntry entry,   /* block (cps/ca/cpd) and entry (odd/even/clear) */
    const NEXUS_KeySlotIv *pIv,      /* regular IV, Set to NULL to update pIv2 independently */
    const NEXUS_KeySlotIv *pIv2      /* secondary IV, required by some algorithm configurations for
