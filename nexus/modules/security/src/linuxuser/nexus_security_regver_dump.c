...
void NEXUS_Security_DumpFirmwareBinary_priv( NEXUS_SecurityRegverRegionID regionId, NEXUS_Addr regionAddress, unsigned regionSize )
{
    ...
    if( regionSize > 0)
    {
        rc = NEXUS_Security_CalculateCpuType_priv( &cpuType, regionId );
        if( rc != NEXUS_SUCCESS )
        {
            BERR_TRACE(rc);
            return;
    ...
    #ifndef NEXUS_REGION_VERIFICATION_DUMP_FIRMWARE_RAW
    {
        ...
        header[NEXUS_SECURITY_P_SIGNATURE_HEADER_OFFSET_CPU_TYPE]  = cpuType & 0xFF;
        ...
        header[NEXUS_SECURITY_P_SIGNATURE_HEADER_OFFSET_EPOCH_SELECT] = pRegionData->epochSelect;
