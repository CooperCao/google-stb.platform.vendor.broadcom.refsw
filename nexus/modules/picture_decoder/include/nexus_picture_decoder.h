...
typedef struct NEXUS_PictureDecoderOpenSettings
{
    ...
    void *bufferPtr; /* attr{memory=cached;null_allowed=y} Optional user-supplied memory buffer which is used for the compressed buffer */
    void *multiScanBufferPtr; /* attr{memory=cached;null_allowed=y} Optional used-supplied memory buffer which is used for multi scan decoding */
...
typedef struct NEXUS_PictureDecoderStatus
{
    ...
    uint32_t crc; /* CRC for completed segment. valid when state == NEXUS_PictureDecoderState_eSegmentDone. */
    struct {
        unsigned firmware;
    } version;
...
/**
Summary:
Status returned by NEXUS_PictureDecoderModule_GetStatus_priv
**/
typedef struct NEXUS_PictureDecoderModuleStatus
{
    struct {
        NEXUS_PowerStatus core;
    } power;
} NEXUS_PictureDecoderModuleStatus;
#ifdef __cplusplus
}
#endif
