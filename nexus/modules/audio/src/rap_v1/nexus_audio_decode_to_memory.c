...
BDBG_MODULE(nexus_audio_decode_to_host);
...
{
    BSTD_UNUSED(decoder);
    BSTD_UNUSED(pMemoryBlocks);
    BSTD_UNUSED(pFrameStatus);
    BSTD_UNUSED(maxFrames);
    BSTD_UNUSED(pNumFrames);
    return BERR_TRACE(BERR_NOT_SUPPORTED);
}
