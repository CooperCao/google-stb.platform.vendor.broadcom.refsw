...
NEXUS_Error NEXUS_AudioDspPrivate_GetLicenseStatus(
    NEXUS_AudioDspPrivateLicenseStatus *pStatus
    )
{
    BDSP_AudioLicenseStatus bdspStatus;
    BERR_Code rc;

    BKNI_Memset(pStatus, 0, sizeof(*pStatus));

    rc  = BDSP_GetAudioLicenseStatus(g_NEXUS_audioModuleData.dspHandle, &bdspStatus);
    if ( rc )
    {
        return BERR_TRACE(rc);
    }

    pStatus->status = bdspStatus.ui32AllAudioLicense;
    pStatus->bp3 = bdspStatus.ui32LicenseEnabledInBP3;
    pStatus->pak = bdspStatus.ui32LicenseEnabledInPAK;
    return NEXUS_SUCCESS;
}
