...
{
#if NEXUS_SHARED_SPI
    pCapabilities->osManaged = num_spi_devices==0?false:true;
#else
...
NEXUS_SpiOsSharedDeviceHandle NEXUS_Spi_OsShared_Open(unsigned index, NEXUS_SpiOsSharedDeviceSettings *pSettings)
{
    ...
    spiDevice->spidrv_handle = spidrv_driver_open(index);
    if(!spiDevice->spidrv_handle) {
        BDBG_ERR(("Unable to open spi device %u", index));
        goto err_open;
    }

    spidrv_driver_get_settings(spiDevice->spidrv_handle, &settings);
    ...
    if(spidrv_driver_set_settings(spiDevice->spidrv_handle, &settings)) {
...
err:
    if (spiDevice->spidrv_handle) {
        spidrv_driver_close(spiDevice->spidrv_handle);
    }
err_open:
    if (spiDevice) {
        BKNI_Free(spiDevice);
...
NEXUS_Error NEXUS_Spi_OsShared_Multiple_Write(void *context, const BREG_SPI_Data *pWriteData, size_t count)
{
    ...
    if(spidrv_driver_message_multiple(spiDevice->spidrv_handle, &msgs)) {
...
NEXUS_Error NEXUS_Spi_OsShared_Write(void *context, const uint8_t *pWriteData, size_t length)
{
    ...
    if(spidrv_driver_message(spiDevice->spidrv_handle, &msg)) {
