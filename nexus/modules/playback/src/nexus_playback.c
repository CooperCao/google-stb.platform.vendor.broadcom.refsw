...
static void nexus_playback_p_remove_playpump(NEXUS_PlaybackHandle playback)
{
    if (playback->params.playpump) {
        /* we defer removing the playpump as long as possible to minimize the need for StopCallbacks,
        but before NEXUS_Playback_Destroy, we must ensure playpump will not callback into freed memory. */
        NEXUS_StopCallbacks(playback->params.playpump);
        NEXUS_StartCallbacks(playback->params.playpump);
        playback->params.playpump = NULL;
    }
}
...
NEXUS_Playback_Destroy( NEXUS_PlaybackHandle playback)
{
    ...
    nexus_playback_p_remove_playpump(playback);
...
NEXUS_Playback_SetSettings(NEXUS_PlaybackHandle playback, const NEXUS_PlaybackSettings *settings)
{
    ...
    if (settings->playpump != playback->params.playpump) {
        nexus_playback_p_remove_playpump(playback);
    }
