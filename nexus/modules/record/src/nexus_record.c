...
static void nexus_record_p_remove_recpump(NEXUS_RecordHandle record)
{
    if (record->cfg.recpump) {
        /* we defer removing the recpump as long as possible to minimize the need for StopCallbacks,
        but before NEXUS_Record_Destroy, we must ensure recpump will not callback into freed memory. */
        NEXUS_StopCallbacks(record->cfg.recpump);
        NEXUS_StartCallbacks(record->cfg.recpump);
        record->cfg.recpump = NULL;
    }
}
...
NEXUS_Record_Destroy(NEXUS_RecordHandle record)
{
    ...
    nexus_record_p_remove_recpump(record);
...
NEXUS_Record_SetSettings(NEXUS_RecordHandle record, const NEXUS_RecordSettings *settings)
{
    ...
    if (settings->recpump != record->cfg.recpump) {
        nexus_record_p_remove_recpump(record);
    }
    record->cfg = *settings;
