...
static void NEXUS_Gpio_P_ReleaseInterrupt(NEXUS_GpioHandle gpio)
{
    ...
    if (g_NEXUS_gpio.osSharedBankCaps.osManaged && g_NEXUS_gpio.settings.osSharedBankSettings.closePin && gpio->osSharedBankGpio)
    {
        g_NEXUS_gpio.settings.osSharedBankSettings.closePin(gpio->osSharedBankGpio);
        gpio->osSharedBankGpio = NULL;
    }
...
{
    if (g_NEXUS_gpio.osSharedBankCaps.osManaged && g_NEXUS_gpio.settings.osSharedBankSettings.setPinInterruptMask_isr && gpio->osSharedBankGpio)
    {
...
NEXUS_Error NEXUS_Gpio_ClearInterrupt(NEXUS_GpioHandle gpio)
{
    NEXUS_OBJECT_ASSERT(NEXUS_Gpio, gpio);
    BKNI_EnterCriticalSection();
    if (g_NEXUS_gpio.osSharedBankCaps.osManaged && g_NEXUS_gpio.settings.osSharedBankSettings.clearPinInterrupt_isr && gpio->osSharedBankGpio)
    {
