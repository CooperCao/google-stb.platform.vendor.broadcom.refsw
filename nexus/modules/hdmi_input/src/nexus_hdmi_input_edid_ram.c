    ...
    uint16_t numEdidBytes)
{
    ...
    if (numEdidBytes)
    {
        ...
        /* remove Vendor Specific Data Blocks that cannot be supported */
        if (numEdidBytes > BHDM_EDID_BLOCKSIZE)
        {
            BHDM_EDID_Remove_CeaVSDB(hdmiInput->stEdidInfo.pDataBytes+128,
                BAVC_HDMI_GetOuiId(BAVC_HDMI_IEEERegId_eDolby)) ;
        }
    }
    else
    {
